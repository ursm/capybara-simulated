(() => {
  ...
  var adapter = {
    ...
    // NOT content), the node's text otherwise. css-select's `:empty` filter
    // relies on this to let a comment/PI child not disqualify an element while a
    // text node does (a Comment's DOM `textContent` is its data, which would
    // wrongly disqualify a programmatically-built `<p><!--x--></p>` from :empty).
