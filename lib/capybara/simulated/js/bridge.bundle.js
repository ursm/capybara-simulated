(() => {
  ...
  function installHtmlParser({ Document: Document2, Element: Element2, Text: Text2, Comment: Comment2, DocumentFragment: DocumentFragment2, DocumentType: DocumentType2, createHtmlPageDocument: createHtmlPageDocument2, registerNamedAccess: registerNamedAccess2 }) {
    ...
    const FIRST_NEWLINE_STRIP = /* @__PURE__ */ new Set(["textarea", "pre", "listing"]);
    ...
    __name(stripFirstNewline, "stripFirstNewline");
  ...
  function installParse5Adapter({
    ...
    __name(parse5ParseDocument, "parse5ParseDocument");
    ...
    return { treeAdapter, parse5ParseDocument, parse5ParseFragment: parse5ParseFragment2, parse5ParseIntoLive: parse5ParseIntoLive2, getParser };
  }
  ...
  var Node = class {
    ...
    blur() {
      ...
      try {
        dispatchEvent(this, new FocusEvent("blur", { bubbles: false, cancelable: false, composed: true }));
      } catch (_) {
      }
      try {
        dispatchEvent(this, new FocusEvent("focusout", { bubbles: true, cancelable: false, composed: true }));
      } catch (_) {
      }
    }
  ...
  var ShadowRoot = class extends DocumentFragment {
    ...
    // (`shadowrootserializable`). We don't model focus delegation behaviourally
    // yet, but the flags must reflect for DSD round-trips + feature detection.
    get delegatesFocus() {
  ...
  var { parseDocument } = installHtmlParser({ Document, Element, Text, Comment, DocumentFragment, DocumentType, createHtmlPageDocument, registerNamedAccess });
  var { parseXml } = installXmlParser({ Element, Text, Comment, ProcessingInstruction, CDATASection, DocumentType });
  var { parse5ParseIntoLive, parse5ParseFragment } = installParse5Adapter({
    Document,
    Element,
    Text,
    Comment,
