(() => {
  ...
  function shadowTemplate(el, opts) {
    ...
    return '<template shadowrootmode="' + sr.mode + '"' + (sr.delegatesFocus ? ' shadowrootdelegatesfocus=""' : "") + (sr.serializable ? ' shadowrootserializable=""' : "") + (sr.clonable ? ' shadowrootclonable=""' : "") + ">" + serializeChildrenWithShadow(sr, opts) + "</template>";
  }
  ...
  var Element = class _Element extends Node {
    ...
    attachShadow(init) {
      ...
      if (this._shadowRoot) {
        ...
        ex._slotAssignment = slotAssignment;
