(() => {
  ...
  function isConnected(node) {
    globalThis.__bc("isConnectedCall");
    let cur = node;
    while (cur) {
      globalThis.__bc("isConnectedStep");
  ...
  function registerSubtree(node) {
    if (!node) return;
    globalThis.__bc("registerNodeVisit");
  ...
  function recordChildList(target, added, removed, prevSibling, nextSibling) {
    globalThis.__bc("recordChildList");
    bumpSettleGen();
    if (observers.size === 0) return;
  ...
  function rebuildCascade(doc) {
    ...
    if (!doc || !doc.documentElement) return;
    globalThis.__bc("rebuildCascade");
    let hide, layout;
    const vp = currentViewport();
    const key = cascadeCacheKey(doc, vp);
    if (key === lastCascadeKey) {
      globalThis.__bc("rebuildCascade_gateskip");
      return;
    }
    lastCascadeKey = key;
    ...
    if (cached) {
      try {
        ...
        hide = obj.hide;
        layout = obj.layout;
        globalThis.__bc("cascadeCacheHit");
      } catch (_) {
        hide = null;
      }
    }
    if (!hide) {
      globalThis.__bc("collectCascadeRules");
  ...
  var Node = class {
    ...
    addEventListener(type, handler, options) {
      globalThis.__bc("addEventListener");
      let fn = null;
  ...
  var Element = class _Element extends Node {
    ...
    // roots, not `this`.
    querySelector(sel) {
      globalThis.__bc("querySelector");
      return selectFirst(this._children, sel, this);
    }
    querySelectorAll(sel) {
      globalThis.__bc("querySelectorAll");
  ...
  var DocumentFragment = class extends Node {
    ...
    getElementById(id) {
      globalThis.__bc("getElementById");
  ...
  (function() {
    ...
    globalThis.__BC = /* @__PURE__ */ Object.create(null);
    globalThis.__bc = function(k, n) {
      const b = globalThis.__BC;
      b[k] = (b[k] || 0) + (n === void 0 ? 1 : n);
    };
    globalThis.__csimFireCEConnect = fireCEConnect;
    function fireCEConnect(subtree) {
      globalThis.__bc("fireCEConnect");
      walkSubtree(subtree, (el) => {
        globalThis.__bc("ceWalkVisit");
    ...
    function maybeRunScript(el) {
      globalThis.__bc("maybeRunScript");
      const type = (el._attrs.type || "").toLowerCase();
