(() => {
  ...
  var Element = class _Element extends Node {
    ...
    attachInternals() {
      const def = this._ns === HTML_NS2 ? getCustomElementCtor(this._localName) : null;
      if (!def) {
    ...
    get control() {
      if (this._ns !== HTML_NS2 || this._localName !== "label") return void 0;
  ...
  var ElementInternals = class {
    ...
    constructor(target, token) {
      ...
      this._target = target;
      const ctor = target._ns === HTML_NS2 ? getCustomElementCtor(target._localName) : null;
  ...
  function processDeclarativeShadowRoots(root, contextEl = null) {
    ...
    for (const node of root._children.slice()) {
      ...
      if (node._tag === "template" && node._ns === HTML_NS2) {
