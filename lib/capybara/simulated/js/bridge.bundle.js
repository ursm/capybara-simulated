(() => {
  ...
  function validateInsertion(node, parent, child, isReplace) {
    ...
    if (t !== NODE_FRAGMENT && t !== NODE_DOCTYPE && t !== NODE_ELEMENT && t !== NODE_TEXT && t !== NODE_COMMENT) {
      throw hierarchyError(`Cannot insert a ${nodeTypeName(node)} node`);
    }
    if (t === NODE_TEXT && pt === NODE_DOC || t === NODE_DOCTYPE && pt !== NODE_DOC) {
  ...
  var Document = class _Document extends Node {
    ...
    // The document's DocumentType child (e.g. from createDocument), or null. (The
    // HTML parser doesn't synthesize a doctype node for `<!DOCTYPE html>`, so the
    // main document's doctype stays null.)
    get doctype() {
