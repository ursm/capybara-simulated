(() => {
  ...
  var Element = class _Element extends Node {
    ...
    setHTMLUnsafe(markup) {
      ...
      this.innerHTML = html;
      processDeclarativeShadowRoots(this._tag === "template" ? this.content : this);
    }
  ...
  function processDeclarativeShadowRoots(root) {
    ...
    for (const node of root._children.slice()) {
      ...
      if (node._tag === "template" && node._ns === HTML_NS2) {
        ...
        let sr = null;
        if ((mode === "open" || mode === "closed") && host && host.nodeType === NODE_ELEMENT && !host._shadowRoot) {
          try {
