(() => {
  ...
  var Node = class {
    ...
    // `Element.children` is an HTMLCollection (not a plain Array): it carries
    // `item`/`namedItem` and the empty-name / named-getter semantics. Recomputed
    // per access off the current child list, so it tracks the live tree. Built in
    // a single pass (no intermediate filtered Array) — this is a hot DOM-traversal
    // path, same rationale as firstElementChild/lastElementChild below.
    get children() {
      const out = new HTMLCollection();
      for (const c of this._children) if (c.nodeType === NODE_ELEMENT) out.push(c);
      return out;
    }
