(() => {
  ...
  function __seqCandidate(el) {
    const isHost = !!el._shadowRoot;
    const isSlot = isHtmlSlot(el);
    const owner = isHost ? "host" : isSlot ? "slot" : null;
    const tiAttr = el._attrs.tabindex;
    let n = null;
    if (tiAttr != null) {
      const p = parseInt(tiAttr, 10);
      if (!isNaN(p)) n = p;
    }
    if (isSlot) return { el, stop: false, owner, group: n != null && n >= 0 ? n : 0 };
    let stop, group;
    if (n != null && n < 0) {
      if (owner === "host") return { el, stop: false, owner, group: -1 };
      return null;
    }
    if (n != null) {
      stop = isFocusable(el);
    ...
    if (!stop && !owner) return null;
    return { el, stop, owner, group };
  }
  __name(__seqCandidate, "__seqCandidate");
  function __seqGather(nodes) {
    const out = [];
    const visit = /* @__PURE__ */ __name((el) => {
      const cand = __seqCandidate(el);
      if (cand) out.push(cand);
      if ((!cand || !cand.owner) && !__seqIsCEHost(el) && el._children) {
  ...
  __name(__seqGather, "__seqGather");
  function __seqEmit(candidates, out) {
    const positives = candidates.filter((c) => c.group > 0).sort((a, b) => a.group - b.group);
    const zeros = candidates.filter((c) => c.group === 0);
    const ordered = positives.concat(zeros);
    for (let i = 0; i < ordered.length; i++) {
      const c = ordered[i];
      if (c.stop) out.push(c.el);
      if (c.owner === "host") {
        __seqEmit(__seqGather(c.el._shadowRoot._children), out);
      } else if (c.owner === "slot") {
        const assigned = findSlottables(c.el);
        __seqEmit(__seqGather(assigned.length ? assigned : c.el._children || []), out);
      }
    }
  }
  __name(__seqEmit, "__seqEmit");
  globalThis.__csimSequentialFocusOrder = function() {
    const doc = globalThis.document;
    if (!doc || !doc._children) return [];
    const out = [];
    __seqEmit(__seqGather(doc._children), out);
    return out;
  };
  function orderIsInDocumentOrder(order) {
    for (let i = 1; i < order.length; i++) {
      if (compareDocOrder(order[i - 1], order[i]) > 0) return false;
    }
    return true;
  }
  __name(orderIsInDocumentOrder, "orderIsInDocumentOrder");
  globalThis.__csimAdvanceFocus = function(reverse) {
    const order = globalThis.__csimSequentialFocusOrder();
    if (order.length === 0) return false;
    const doc = globalThis.document;
    const current = doc && doc._activeElement;
    let idx = current ? order.indexOf(current) : -1;
    if (idx === -1 && current && isConnected(current) && orderIsInDocumentOrder(order)) {
      if (reverse) {
        let pick = -1;
        for (let i = 0; i < order.length; i++) if (compareDocOrder(order[i], current) < 0) pick = i;
        idx = pick === -1 ? order.length - 1 : pick;
      } else {
        let pick = -1;
        for (let i = 0; i < order.length; i++) {
          if (compareDocOrder(current, order[i]) < 0) {
            pick = i;
            break;
          }
        }
      ...
      idx = reverse ? idx <= 0 ? order.length - 1 : idx - 1 : (idx + 1) % order.length;
    }
