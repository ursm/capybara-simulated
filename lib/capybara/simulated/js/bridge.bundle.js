(() => {
  ...
  var LAYOUT_PROPS = ["top", "left", "width", "height", "text-transform"];
  ...
  function closeDialog2(dlg, returnValue) {
  ...
  function applyHistoryUrl(self, state2, url, push) {
    self.state = state2;
    if (!url) return;
    const s = String(url);
  ...
  var history = {
    ...
    pushState(state2, _title, url) {
      applyHistoryUrl(this, state2, url, true);
    },
    replaceState(state2, _title, url) {
      applyHistoryUrl(this, state2, url, false);
    },
    back() {
  ...
  function installDomClassAliases({ Element: Element2, Document: Document2, Text: Text2 }) {
    for (const name of PLAIN_ELEMENT_ALIASES) globalThis[name] = Element2;
    globalThis.HTMLFormElement = makeTagCtor("form", Element2);
    globalThis.HTMLBodyElement = makeTagCtor("body", Element2);
    globalThis.HTMLAnchorElement = makeTagCtor("a", Element2);
  ...
  globalThis.__csim_resolveSpecifier = function(specifier, baseUrl) {
    ...
    if (mapped) return resolveAgainst2(mapped, baseUrl);
    if (specifier.charAt(0) === "/" || specifier.startsWith("./") || specifier.startsWith("../") || /^[a-z]+:\/\//i.test(specifier)) {
      return resolveAgainst2(specifier, baseUrl);
    }
    return specifier;
  };
  function resolveAgainst2(url, base) {
    try {
  ...
  var Node = class {
    ...
    focus() {
      ...
      if (typeof isContenteditable === "function" && isContenteditable(this) && typeof globalThis.getSelection === "function") {
        try {
          ...
          const inside = r0 && r0.startContainer && nodeContains2(this, r0.startContainer);
          if (!inside) {
    ...
    getBoundingClientRect() {
      if (isVisibleNode(this)) {
        return { top: 0, left: 0, right: 1, bottom: 1, width: 1, height: 1, x: 0, y: 0 };
      }
      return { top: 0, left: 0, right: 0, bottom: 0, width: 0, height: 0, x: 0, y: 0 };
    }
    getClientRects() {
      return isVisibleNode(this) ? [{ top: 0, left: 0, right: 1, bottom: 1, width: 1, height: 1, x: 0, y: 0 }] : [];
    }
    get offsetWidth() {
    ...
    submit() {
      ...
      globalThis.globalThis.__csimPendingFormSubmit = { form: this, submitter: null };
    }
    requestSubmit(submitter) {
      ...
      globalThis.globalThis.__csimPendingFormSubmit = { form: this, submitter: submitter || null };
    }
        ...
        if (!ev.defaultPrevented && isSubmitButton(this)) {
          ...
          if (form) {
            ...
            if (!submitEv.defaultPrevented) {
              globalThis.globalThis.__csimPendingFormSubmit = { form, submitter: this };
            }
          }
        }
        if (!ev.defaultPrevented && !globalThis.globalThis.__csimPendingFormSubmit) {
          let anchor = this;
          while (anchor && anchor.nodeType === NODE_ELEMENT && anchor._tag !== "a") {
            anchor = anchor._parent;
          }
          if (anchor && anchor.nodeType === NODE_ELEMENT && anchor._tag === "a" && anchor._attrs.href != null && (anchor._attrs.href || "").trim() !== "" && !(anchor._attrs.href || "").toLowerCase().startsWith("javascript:")) {
            globalThis.globalThis.__csimPendingNavigation = {
              url: String(anchor._attrs.href),
  ...
  var DocumentOrderRange = class _DocumentOrderRange {
    ...
    intersectsNode(node) {
      return rangeIntersectsNode2(this, node);
    }
    ...
    cloneContents() {
      return cloneRangeContents2(this);
    }
    extractContents() {
      const frag = cloneRangeContents2(this);
  ...
  function nodeContains2(ancestor, descendant) {
  ...
  function rangeIntersectsNode2(range, node) {
    ...
    if (nodeContains2(node, range.startContainer)) return true;
    if (nodeContains2(node, range.endContainer)) return true;
    if (nodeContains2(range.startContainer, node) && nodeContains2(range.endContainer, node)) return true;
  ...
  function __csimInsertTextAtSelection2(text) {
  ...
  globalThis.__csimInsertTextAtSelection = __csimInsertTextAtSelection2;
  ...
  function cloneRangeContents2(range) {
    const frag = new DocumentFragment();
  ...
  (function() {
    ...
    globalThis.__csimClickResolve = function(h, modifiers) {
      ...
      if (isSubmitButton2(n)) {
        const form = formForControl2(n);
        if (!form) return null;
        ...
        if (methodAttr === "dialog") {
          ...
          if (dlg) closeDialog2(dlg, String(n._attrs.value == null ? "" : n._attrs.value));
          return null;
        }
    ...
    function implicitSubmitFormFor(control) {
      ...
      const form = formForControl2(control);
      if (!form) return null;
