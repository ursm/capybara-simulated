(() => {
  ...
  function installNodeConstants(Node) {
    const c = {
      ELEMENT_NODE: 1,
      ATTRIBUTE_NODE: 2,
      TEXT_NODE: 3,
  ...
  function installHtmlParser({ Document, Element, Text, DocumentFragment }) {
  ...
  var __layoutRules = [];
  var __ruleSerial = 0;
  var __hideRuleIdx = null;
  var __layoutRuleIdx = null;
  function rebuildCascade(doc) {
    doc = doc || globalThis.document;
    if (!doc || !doc.documentElement) return;
    ({ hide: __hideRules, layout: __layoutRules } = collectCascadeRules(doc));
  ...
  function collectCascadeRules(doc) {
    ...
    for (const s of doc.documentElement.querySelectorAll("style")) {
      let txt = "";
      for (const c of s._children) if (c.nodeType === NODE_TEXT) txt += c.data;
      if (txt) consume(txt);
    }
  ...
  var EMPTY_CLASS_TOKENS = Object.freeze([]);
  ...
  function installOnHandlerSlots(Element) {
  ...
  function installDomClassAliases({ Element, Document, Text }) {
  ...
  (function() {
    ...
    class Element extends Node {
      ...
      insertAdjacentText(position, text) {
        this.insertAdjacentHTML(position, escapeText2(String(text == null ? "" : text)));
      }
    ...
    const { parseDocument, parseFragment, serializeElement, serializeChildren, escapeText: escapeText2 } = installHtmlParser({ Document, Element, Text, DocumentFragment });
