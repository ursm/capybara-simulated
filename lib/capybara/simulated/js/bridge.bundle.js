(() => {
  ...
  var Element = class _Element extends Node {
    ...
    // NB: object.data / object.codeBase are deliberately NOT defined here — `data`
    // especially is too generic a property name to put on every Element prototype
    // (it would turn `el.data = …` expandos into silent no-ops, diverging from real
    // browsers where non-<object> elements have no `data` accessor). <object> is niche;
    // those ~88 subtests stay in the in-scope backlog rather than risk app breakage.
