(() => {
  ...
  function installParse5Adapter({ Document: Document2, Element: Element2, Text: Text2, Comment: Comment2, DocumentFragment: DocumentFragment2, DocumentType: DocumentType2, createHtmlPageDocument: createHtmlPageDocument2 }) {
    ...
    const treeAdapter = {
      // ── node creation ──────────────────────────────────────────────
      createDocument() {
        return createHtmlPageDocument2(false);
      },
      ...
      insertTextBefore(parentNode, text, referenceNode) {
        ...
        this.insertBefore(parentNode, new Text2(text), referenceNode);
      },
      ...
      getTemplateContent(templateElement) {
        if (!templateElement._templateContent) templateElement._templateContent = new DocumentFragment2();
  ...
  var { parse5ParseDocument: __p5ParseDocument } = installParse5Adapter({ Document, Element, Text, Comment, DocumentFragment, DocumentType, createHtmlPageDocument });
  globalThis.__csimParse5ParseDocument = __p5ParseDocument;
  ...
  (function() {
    ...
    globalThis.__csimLoadDocument = function(html, contentType) {
      ...
      const freshDoc = parseDocument(srcHtml);
      if (/shadowrootmode/i.test(srcHtml)) {
        ingestImportmaps(freshDoc);
        processDeclarativeShadowRoots(freshDoc);
      }
