(() => {
  ...
  function installParse5Adapter({
    ...
    function parse5ParseIntoLive2(liveDoc, html) {
      const Parser = getParser();
  ...
  function processDeclarativeShadowRoots(root, contextEl = null) {
    ...
    for (const node of root._children.slice()) {
      ...
      if (node._tag === "template" && node._ns === HTML_NS3) {
        ...
        let sr = null;
        if ((mode === "open" || mode === "closed") && host && host.nodeType === NODE_ELEMENT && host !== contextEl && !host._shadowRoot) {
          try {
        ...
        if (sr) {
          ...
          const i = host._children.indexOf(node);
          if (i >= 0) host._children.splice(i, 1);
          unregisterSubtree(node);
          node._parent = null;
          registerSubtree(sr);
          ceUpgradeTree(sr);
  ...
  (function() {
    ...
    function fireCEConnect(subtree) {
      ...
      if (connectedSelects) for (const sel of connectedSelects) runSelectednessAlgorithm(sel, null);
    }
