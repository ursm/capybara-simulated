(() => {
  ...
  function cssTreeFlatten(cssText, vp) {
    ...
    let ast;
    try {
      ast = CT.parse(cssText, { parseValue: false });
    } catch (_) {
      return out;
    }
    const emitRule = /* @__PURE__ */ __name((ruleNode, parentSel) => {
      const sel = composeNestedSelector(CT.generate(ruleNode.prelude).trim(), parentSel);
      const decls = ruleDecls(ruleNode.block);
      if (decls.length) out.push({ selectorText: sel, decls });
      visit(ruleNode.block && ruleNode.block.children, sel);
    }, "emitRule");
    const visit = /* @__PURE__ */ __name((children, parentSel) => {
      if (!children) return;
      children.forEach((node) => {
        if (node.type === "Rule") {
          emitRule(node, parentSel);
        } else if (node.type === "Raw") {
          if (parentSel && node.value.indexOf("{") !== -1) {
            try {
              const r = CT.parse("& " + node.value, { context: "rule", parseValue: false });
              if (r && r.type === "Rule") emitRule(r, parentSel);
            } catch (_) {
            }
          }
          ...
          visit(node.block && node.block.children, parentSel);
        }
      });
    }, "visit");
    visit(ast.children, null);
    return out;
  }
