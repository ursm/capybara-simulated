(() => {
  ...
  function collectVisibleText(node, transform, preserveWs) {
    if (node.nodeType === NODE_TEXT) {
      ...
      const raw = preserveWs ? data : data.replace(INLINE_WS_RE, " ");
  ...
  globalThis.__csimSendKeys = function(h, atoms) {
    ...
    const pressKey = (info, modifiers) => {
      ...
      if (!blocked && (info.key === "Enter" || info.key === " ") && n._attrs.disabled == null && (n._tag === "button" || n._tag === "a" && n._attrs.href != null) && (!modifiers || !modifiers.ctrlKey && !modifiers.metaKey && !modifiers.altKey)) {
        try {
          globalThis.__csimClickResolve(n._id, modifiers || null);
        } catch (_) {
        }
  ...
  globalThis.__csimSelectWordAt = function(h) {
    ...
    if (r && r.startContainer && n.contains(r.startContainer) && r.startContainer.nodeType === NODE_TEXT) {
      ...
      const leaf = deepestFirstText(n);
      if (!leaf || leaf.nodeType !== NODE_TEXT || !(leaf.data || "").length) return;
      textNode = leaf;
      off = 0;
    }
    const data = textNode.data || "";
    const wordChar = /[\p{L}\p{N}_]/u;
    let lo = Math.min(off, data.length);
    let hi = Math.min(off, data.length);
    while (lo > 0 && wordChar.test(data[lo - 1])) lo -= 1;
    while (hi < data.length && wordChar.test(data[hi])) hi += 1;
    if (lo === hi) return;
    if (typeof sel.setBaseAndExtent === "function") {
      sel.setBaseAndExtent(textNode, lo, textNode, hi);
    } else if (typeof sel.collapse === "function") {
      sel.collapse(textNode, lo);
      if (typeof sel.extend === "function") sel.extend(textNode, hi);
    }
  };
  ...
  (function() {
    ...
    const __csimAtomLikeForCE = /* @__PURE__ */ new Set(["img", "video", "audio", "iframe", "object", "embed", "br", "hr", "input", "wbr", "svg", "math"]);
    function __csimEditableHost(n) {
      for (let p = n; p; p = p._parent) {
        const v = p._attrs && p._attrs.contenteditable;
        if (v == null) continue;
        const lower = String(v).toLowerCase();
        if (lower === "false") return null;
        if (lower === "" || lower === "true" || lower === "plaintext-only") return p;
      }
      return null;
    }
    ...
    function __csimHitTestRetarget(n) {
      ...
      if (__csimEditableHost(n)) return n;
      function dfs(node) {
    ...
    globalThis.__csimClickResolve = function(h, modifiers) {
      ...
      if (!mousedownEv.defaultPrevented && !__csimAtomLikeForCE.has(n._tag)) {
        const ceHost = __csimEditableHost(n);
        if (ceHost && ceHost !== n) {
          if (globalThis.document._activeElement !== ceHost) {
            try {
              ceHost.focus();
            } catch (_) {
            }
          ...
          try {
            const sel = globalThis.getSelection && globalThis.getSelection();
            if (sel && typeof sel.collapse === "function") sel.collapse(n, 0);
          } catch (_) {
          }
        }
      }
