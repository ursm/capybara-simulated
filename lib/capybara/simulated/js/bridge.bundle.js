(() => {
  ...
  function compile(sel, scopeRoot) {
    if (typeof sel === "string" && sel.indexOf("\0") !== -1) sel = sel.replace(/\x00/g, "\uFFFD");
  ...
  var adapter2 = {
    ...
    attributes: /* @__PURE__ */ __name((el) => el.attributes || [], "attributes"),
  ...
  function validateInsertion(node, parent, child, isReplace) {
    ...
    if (t !== NODE_FRAGMENT && t !== NODE_DOCTYPE && t !== NODE_ELEMENT && t !== NODE_TEXT && t !== NODE_COMMENT && t !== NODE_PI) {
  ...
  var Node = class {
    ...
    get textContent() {
      let s = "";
      for (const c of this._children) {
        if (c.nodeType === NODE_TEXT) s += c.data;
  ...
  var Element = class _Element extends Node {
    ...
    // indexed + `getNamedItem(name)`. The XPath engine's attribute axis
    // slices it and reads each item's name / namespace; Capybara's
    // `Element#native.attributes` reads `{name, value}` pairs. We give
    // each item the Attr fields these consumers touch (`specified`,
    // `namespaceURI`, `prefix`, `localName`, `ownerElement`).
    get attributes() {
      const el = this;
      const names = Object.keys(this._attrs);
      const list = names.map((n) => el._attrNodeFor(n));
      names.forEach((n, i) => {
        Object.defineProperty(list, n, { value: list[i], enumerable: false, configurable: true, writable: true });
      });
      const methods = {
        // `item(index)` — the NamedNodeMap accessor; null when out of range.
        item: /* @__PURE__ */ __name((index) => {
          const a = list[index >>> 0];
          return a === void 0 ? null : a;
        }, "item"),
        getNamedItem: /* @__PURE__ */ __name((name) => {
          const k = attrKey(el, name);
          return Object.prototype.hasOwnProperty.call(el._attrs, k) ? makeAttr(el, k) : null;
        }, "getNamedItem"),
        getNamedItemNS: /* @__PURE__ */ __name((ns, ln) => el.getAttributeNodeNS(ns, ln), "getNamedItemNS"),
        setNamedItem: /* @__PURE__ */ __name((attr) => el.setAttributeNode(attr), "setNamedItem"),
        setNamedItemNS: /* @__PURE__ */ __name((attr) => el.setAttributeNode(attr), "setNamedItemNS"),
        removeNamedItem: /* @__PURE__ */ __name((name) => {
          const removed = el.getAttributeNode(name);
          if (!removed) throw new globalThis.DOMException("No attribute named '" + name + "'.", "NotFoundError");
          return el.removeAttributeNode(removed);
        }, "removeNamedItem")
      };
      for (const m in methods) {
        Object.defineProperty(list, m, { value: methods[m], enumerable: false, configurable: true, writable: true });
      }
      return list;
    }
    ...
    get implementation() {
      return {
        createHTMLDocument: /* @__PURE__ */ __name((...args) => {
          ...
          html._parent = d;
          d._children = [doctype, html];
          if (args.length > 0 && args[0] !== void 0) {
            const t = new Element("title");
            t._children = [Object.assign(new Text(String(args[0])), { _parent: t })];
            t._parent = head;
