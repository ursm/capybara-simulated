(() => {
  ...
  (function() {
    ...
    function parseXMLDocument(xml) {
      ...
      const XMLNS_NS2 = "http://www.w3.org/2000/xmlns/";
      walkSubtree(doc, (n) => {
        if (n.nodeType !== 1 || !n._attrs) return;
        for (const key in n._attrs) {
          if (key !== "xmlns" && key.slice(0, 6) !== "xmlns:") continue;
          const colon = key.indexOf(":");
          const prefix = colon === -1 ? null : key.slice(0, colon);
          const localName = colon === -1 ? key : key.slice(colon + 1);
          (n._attrNS || (n._attrNS = {}))[key] = { ns: XMLNS_NS2, prefix, localName };
        }
      });
      return doc;
    }
