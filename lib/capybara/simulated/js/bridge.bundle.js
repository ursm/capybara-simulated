(() => {
  ...
  var __scopeRoot2 = null;
  var userPseudos = {
    ...focusPseudoMatchers,
    scope: (el) => __scopeRoot2 != null && el === __scopeRoot2
  };
  ...
  function selectAll(roots, sel, scopeRoot) {
    ...
    if (scopeRoot === void 0) return adapter.findAll(fn, roots);
    const prev = __scopeRoot2;
    __scopeRoot2 = scopeRoot;
    try {
      return adapter.findAll(fn, roots);
    } finally {
      __scopeRoot2 = prev;
    }
  }
  function selectFirst(roots, sel, scopeRoot) {
    const fn = compile(sel);
    if (scopeRoot === void 0) return adapter.findOne(fn, roots);
    const prev = __scopeRoot2;
    __scopeRoot2 = scopeRoot;
    try {
      return adapter.findOne(fn, roots);
    } finally {
      __scopeRoot2 = prev;
    }
  }
  ...
  globalThis.__csimSendKeys = function(h, atoms) {
    ...
    const pressKey = (info, modifiers) => {
      ...
      const doc = globalThis.document;
      const kupTarget = doc && doc.activeElement || n;
  ...
  (function() {
    ...
    globalThis.__csimLoadDocument = function(html) {
      ...
      resetCascadeState();
      if (typeof globalThis.__csimResetLayoutY === "function") globalThis.__csimResetLayoutY();
