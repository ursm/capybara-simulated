(() => {
  ...
  function resolvedDisplayIsFlexLike(el) {
    ...
    return best ? FLEX_LIKE_DISPLAY.has(String(best.value).trim().toLowerCase()) : false;
  }
  ...
  var Document = class _Document extends Node {
    ...
    // exercises (`insertText`, `delete`); everything else is a tolerant
    ...
    queryCommandSupported(command) {
      return String(command || "").toLowerCase() === "inserttext";
    }
    ...
    globalThis.__csimVisible = function(h) {
      ...
      while (cur) {
        ...
        if (cur.nodeType === NODE_ELEMENT) {
          if (INVISIBLE_TAGS.has(cur._tag)) return false;
          if (selfHidden(cur)) return false;
          if (cur !== n && cur._tag === "details" && cur._attrs.open == null && !summarySeen) return false;
