(() => {
  ...
  function collectVisibleText(node, transform) {
    if (node.nodeType === NODE_TEXT) {
      ...
      const raw = preservesWhitespace(node._parent) ? data : data.replace(INLINE_WS_RE, " ");
  ...
  var __CE_BLOCK_TAGS = /* @__PURE__ */ new Set([
    "p",
    "div",
    "h1",
    "h2",
  ...
  function findBlockAncestor(node) {
    for (let cur = node; cur; cur = cur._parent) {
      if (cur.nodeType !== NODE_ELEMENT) continue;
      if (__CE_BLOCK_TAGS.has(cur._tag)) return cur;
    }
    return null;
  }
  ...
  globalThis.__csimSendKeys = function(h, atoms) {
    ...
    const pressKey = (info, modifiers) => {
      ...
      if (!blocked && (info.key === "Enter" || info.key === " ") && (n._tag === "button" || n._tag === "a" && n._attrs.href != null) && (!modifiers || !modifiers.ctrlKey && !modifiers.metaKey && !modifiers.altKey)) {
        try {
          globalThis.__csimClickResolve(n._id, modifiers || null);
        } catch (_) {
        }
  ...
  globalThis.__csimSelectWordAt = function(h) {
    ...
    let textNode = null;
    let off = 0;
    if (r && r.startContainer && n.contains && n.contains(r.startContainer) && r.startContainer.nodeType === NODE_TEXT) {
      textNode = r.startContainer;
      off = r.startOffset | 0;
    } else {
      const visit = (x) => {
        if (textNode) return;
        if (x.nodeType === NODE_TEXT && (x.data || "").length > 0) {
          textNode = x;
          return;
        }
        if (x._children) for (const c of x._children) {
          visit(c);
          if (textNode) return;
        }
      };
