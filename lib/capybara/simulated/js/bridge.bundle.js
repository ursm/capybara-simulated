(() => {
  ...
  var Event = class {
    constructor(type, init) {
      ...
      this.eventPhase = 0;
      this.timeStamp = typeof globalThis.performance !== "undefined" && globalThis.performance.now ? globalThis.performance.now() : Date.now();
  ...
  globalThis.matchMedia = function matchMedia(query) {
    ...
    const mql = {
      ...
      onchange: null,
      _listeners: [],
      _lastMatches: mediaMatches(text, currentViewport()),
      addEventListener(type, handler) {
      ...
      dispatchEvent(event) {
        const evt = event || { type: "change", matches: this.matches, media: this.media };
  ...
  globalThis.__csimViewportChanged = function() {
    for (const mql of _activeQueries) {
      ...
      if (now !== mql._lastMatches) {
        ...
        mql.dispatchEvent({ type: "change", matches: now, media: mql.media });
      }
    }
  };
  ...
  function applyHistoryUrl(state2, url, push) {
    ...
    if (!url) {
      if (push && typeof globalThis.__pushHistoryEntry === "function") {
        globalThis.__pushHistoryEntry(globalThis.location && globalThis.location.href, _state);
      } else if (typeof globalThis.__setCurrentUrl === "function") {
        globalThis.__setCurrentUrl(globalThis.location && globalThis.location.href, _state);
      }
      return;
    }
    const s = String(url);
    if (push) {
      if (typeof globalThis.__pushHistoryEntry === "function") globalThis.__pushHistoryEntry(s, _state);
      _length += 1;
    } else if (typeof globalThis.__setCurrentUrl === "function") {
      globalThis.__setCurrentUrl(s, _state);
    }
    if (typeof globalThis.__csimUpdateLocation === "function") globalThis.__csimUpdateLocation(s);
  }
  var history = {
    get length() {
      return _length;
  ...
  globalThis.structuredClone = function structuredClone(v, _options, seen) {
    ...
    if (v instanceof Map) {
      ...
      seen.set(v, out2);
      for (const [k, val] of v) out2.set(structuredClone(k, _options, seen), structuredClone(val, _options, seen));
      return out2;
    }
    if (v instanceof Set) {
      const out2 = /* @__PURE__ */ new Set();
      seen.set(v, out2);
      for (const x of v) out2.add(structuredClone(x, _options, seen));
      return out2;
    }
    ...
    if (ArrayBuffer.isView && ArrayBuffer.isView(v)) {
      ...
      const out2 = new Ctor(buf, v.byteOffset, v.length);
      seen.set(v, out2);
      return out2;
    }
    if (Array.isArray(v)) {
      ...
      seen.set(v, out2);
      for (let i = 0; i < v.length; i++) out2[i] = structuredClone(v[i], _options, seen);
      return out2;
    }
    const out = {};
    seen.set(v, out);
    ...
    postMessage(data, _transfer) {
      ...
      Promise.resolve().then(() => {
        ...
        if (typeof peer.onmessage === "function") {
          try {
            peer.onmessage.call(peer, ev);
          } catch (_) {
          }
        }
        for (const h of peer._listeners.slice()) try {
          h.call(peer, ev);
        } catch (_) {
        }
      });
      ...
      Promise.resolve().then(() => {
        for (const ch of set) {
          ...
          if (typeof ch.onmessage === "function") {
            try {
              ch.onmessage.call(ch, ev);
            } catch (_) {
            }
          }
          for (const h of ch._listeners.slice()) {
            try {
              h.call(ch, ev);
            } catch (_) {
            }
  ...
  Promise.prototype.then = function(onF, onR) {
    ...
    const next = origThen.call(this, wrappedOnF, function(err) {
  ...
  var XMLHttpRequestUpload = class {
    constructor() {
    ...
    dispatchEvent(event) {
      this._fire(event && event.type, event || {});
      return true;
    }
    _fire(type, extra) {
      const evt = Object.assign({ type, target: this, currentTarget: this }, extra);
    ...
    function __csimMakeDataTransfer(items) {
      ...
      for (const it of items || []) {
        if (it.kind === "file") {
          ...
          dt.items._items.push(new globalThis.DataTransferItem("file", "application/octet-stream", null, file));
          dt.files.push(file);
          if (!dt.types.includes("Files")) dt.types.push("Files");
        } else {
          dt.items._items.push(new globalThis.DataTransferItem("string", it.type, it.value, null));
