(() => {
  ...
  function matchesAnyHideRule(el, ignoreVisibility = false, inline = null, hidden = false) {
    ...
    const shadowHide = sr ? scopedRulesFor(sr).hide : null;
  ...
  function ownVisibility(el) {
    ...
    const sr = globalThis.__csimShadowHostCount ? enclosingShadowRootOf(el) : null;
    if (sr) {
      for (const r of scopedRulesFor(sr).hide) {
  ...
  function cascadedProperty(el, prop) {
    ...
    const sr = globalThis.__csimShadowHostCount ? enclosingShadowRootOf(el) : null;
    if (sr) {
      const srRules = scopedRulesFor(sr).layout;
      for (const r of srRules) {
        const cap = r.captured[prop];
        if (!cap) continue;
        if (!safeMatches(el, r.selectorText)) continue;
        if (winsProp(best, r.spec, r.source, cap.important, r.layerRank)) {
  ...
  function scopedRulesFor(sr) {
    ...
    const addSheet = /* @__PURE__ */ __name((cssText) => {
      ...
      for (const r of parsed.hide) hide.push({ ...r, source: r.source + serial });
      for (const r of parsed.layout) layout.push({ ...r, source: r.source + serial });
      serial += parsed.count;
    }, "addSheet");
