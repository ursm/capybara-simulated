(() => {
  ...
  function getWindowForwardedHandler(name) {
    return winStore[name] || null;
  }
  __name(getWindowForwardedHandler, "getWindowForwardedHandler");
  ...
  function fireWindowOnHandler(event) {
    const h = getWindowForwardedHandler(event.type);
  ...
  function fireWindowListeners(event, capture) {
    ...
    const hasIdlHandler = !capture && typeof getWindowForwardedHandler(event.type) === "function";
  ...
  var Node = class {
    ...
    click() {
      try {
        ...
        if (!ev.defaultPrevented && this._tag !== "label" && !LABELABLE.has(this._tag) && !isInteractiveForLabel(this)) {
          const label = enclosingLabelFor(this);
          if (label) {
            const labeled = labeledControlFor(label);
            if (labeled && labeled !== this) labeled.click();
          }
        }
      } catch (_) {
      }
  ...
  var Element = class _Element extends Node {
    ...
    get relList() {
      ...
      if (ns === HTML_NS && (ln === "a" || ln === "area" || ln === "link" || ln === "form") || ns === SVG_NS && ln === "a") return tokenListFor(this, "rel");
      return void 0;
    }
    get sandbox() {
