(() => {
  ...
  var Element = class _Element extends Node {
    ...
    get classList() {
      const el = this;
      const commit = (next) => {
        const old = el._attrs["class"];
        const nv = next == null ? "" : String(next);
        if (old === nv) return;
        el._attrs["class"] = nv;
        recordAttrMutation(el, "class", old == null ? null : old);
      };
      return {
        contains(c) {
        ...
        add(...names) {
          ...
          if (changed) commit(cs.join(" "));
        },
        remove(...names) {
          const drop = new Set(names);
          const filtered = classes(el).filter((x) => !drop.has(x));
          commit(filtered.join(" "));
        },
        toggle(c, force) {
          const cs = classes(el);
