  ...
  var Node = class {
    ...
    removeEventListener(type, handler, options) {
      ...
      this._listeners[type] = this._listeners[type].filter((l) => {
        const isMatch = l.capture === capture && (l.handler === handler || handler && typeof handler.handleEvent === "function" && l.handler._csimEventListenerObject === handler);
        if (isMatch) l.removed = true;
        return !isMatch;
      });
    }
  ...
  var Element = class _Element extends Node {
    ...
    getElementsByTagName(tag) {
      ...
      for (const n of all) {
        ...
        const ok = htmlDoc ? n._ns === HTML_NS2 ? qn === qLower : qn === q : qn === q;
        if (ok) out.push(n);
      }
