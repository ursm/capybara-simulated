...
// each rewrite introduces is written unitless, exactly as an omitted argument is.
const zeroArg = (identity) => ({ n: identity, unit: '' });
...
function asPrimitive(fn, prim) {
  ...
  const zero = () => zeroArg(spec.identity);
  let args;
  switch (prim) {
    case 'translate':
...
const TYPE_HANDLERS = {
  ...
  transform: (f, t, p, prop) => {
    ...
    return combineList('transform', f, t, { p }, prop);
  }
};
