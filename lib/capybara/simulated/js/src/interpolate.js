...
// Two values ADDED, for `composite: 'add'` — the numeric types compose, and anything else takes
// the effect's own value (the spec's fallback when addition is not defined for a type).
export function addValues(prop, under, value) {
  if (under == null) return value;
  if (value == null) return under;
  const type = ANIMATION_TYPES[prop];
  const handler = TYPE_HANDLERS[type];
  if (handler !== TYPE_HANDLERS.number && handler !== TYPE_HANDLERS.integer &&
      handler !== TYPE_HANDLERS.length && handler !== TYPE_HANDLERS.lpc) return value;
  const a = numericValue(under), b = numericValue(value);
  if (!a || !b) return value;
  if (a.unit !== b.unit && a.n !== 0 && b.n !== 0) return value;   // no common unit to add in
  const unit = a.unit || b.unit;
  const sum = formatNumber(a.n + b.n);
  return sum == null ? value : clampToRange(prop, sum + unit);
}
