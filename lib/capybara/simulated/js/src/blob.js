...
globalThis.__csimReadBlobForWindow = function (url) {
  ...
  if (ids && ids.size && globalThis.RustyRacer && typeof globalThis.RustyRacer.contextGlobal === 'function') {
    for (const id of ids) {
      try {
        const g = globalThis.RustyRacer.contextGlobal(id);
        if (g && typeof g.__csimReadBlobForWindow === 'function') {
          const r = g.__csimReadBlobForWindow(url);
          if (r) return r;
        }
      } catch (_) {}
    }
