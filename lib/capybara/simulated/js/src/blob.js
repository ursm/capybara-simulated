...
// multipart/form-data serialiser.
//
// Blob bodies are stored as latin-1 byte strings (one char per byte,
// 0-255) so the mini_racer / quickjs.rb marshalling boundary survives
...
// Single entry-point for "give me the latin-1 bytes of this Blob"
// — FormData multipart, createImageBitmap, arrayBuffer all converge
// here so host vs in-memory branching lives in one place.
...
export class Blob {
  constructor(parts, opts) {
    ...
    this._parts = (parts || []).map(p => {
      ...
      // serialise the underlying bytes, not `String(typedArray)`
