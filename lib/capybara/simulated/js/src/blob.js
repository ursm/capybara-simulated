...
// The `Blob.prototype` reachable from `obj`'s own prototype chain — i.e. the Blob
// interface of the realm that created `obj` (one hop past File.prototype for a
// File). slice() uses this so a cross-realm call (e.g. Blob.prototype.slice taken
// from a detached frame and applied to a Blob of another realm) produces a result
// that is `instanceof` the caller's Blob, and always a Blob — never a File.
...
// hand over raw bytes, which must NOT be re-encoded). `proto` is the
// Blob.prototype to create the result under (the caller's realm; see blobProtoOf).
...
// The blob's text: its bytes UTF-8-decoded. A charset in `type` is ignored — blob
// text is always UTF-8. Shared by `text()` and `textStream()`.
function blobTextOf(blob) {
  const bytes = blob._csimHost ? readHostFile(blob) : blob._parts.join('');
...
export class Blob {
  ...
  text() {
    // The DECODED UTF-8 string (not the raw byte-string); arrayBuffer()/slice
    // operate on the raw bytes instead.
    return Promise.resolve(blobTextOf(this));
  }
  arrayBuffer() {
    return Promise.resolve(bytesToArrayBuffer(blobBytes(this)));
  }
  textStream() {
    // A ReadableStream of the blob's text — the UTF-8 DECODED string (like text(),
    // and like text() it ignores any charset in `type`: blob text is always UTF-8).
    // Non-byte stream of string chunks; the whole body is one chunk (in-memory here),
    // then it closes. An empty blob yields a stream that closes with no chunk.
    const text = blobTextOf(this);
    let pulled = false;
  ...
  slice(start, end, type) {
    ...
    const proto = blobProtoOf(this);   // create the result in this's realm, as a Blob
    if (this._csimHost) {
...
// `toBytes(str)` converts a field name / filename / string value to its output
// byte string. It defaults to the UTF-8 encoder (the only encoding fetch /
// FormData.send use); the form-submission path passes a legacy single-byte
// encoder when the form's accept-charset selects one (unrepresentable code
// points → `&#N;`). The structural bytes (boundary, CRLF, headers) are ASCII.
export function serializeMultipart(formData, toBytes) {
  ...
  formData.forEach((value, key) => {
    ...
    if (value instanceof Blob) {
      const filename    = toBytes(escapeFormField(value.name != null ? String(value.name) : 'blob'));
...
export function resolveBlobBytes(url) {
  ...
  if (blob) return { bytes: blobBytes(blob), type: blob.type };   // the blob's own type ('' when untyped)
  if (hostVal == null || hostVal === '') return null;   // existence marker only, no cross-realm bytes
  let bytes = '';
  try { bytes = globalThis.atob(String(hostVal)); } catch (_) {}
...
// structuredClone of a Blob/File → a new Blob/File in THIS realm with the same
// bytes + type (+ File name/lastModified). Used by the same-page postMessage clone
// path (platform-globals cloneInto) so a Blob posted to an iframe arrives as a
// real, usable Blob in that realm rather than a plain `{}`.
...
export function installBlobURL() {
  ...
  globalThis.URL.createObjectURL = function (blob) {
    ...
    // absence as revoked). Carry the bytes (base64) when ANOTHER context might
    // resolve them: a worker (cross-isolate) OR a frame realm (cross-realm — the
    // parent fetches a blob URL the iframe created); otherwise a cheap existence
    // marker keeps single-realm createObjectURL off the base64 path. A blob URL
    ...
    if (globalThis.top && globalThis.top !== globalThis && NS && typeof NS.contextOf === 'function') {
      try { ownerRealm = NS.contextOf(globalThis); } catch (_) {}
    }
    try { globalThis.__csim_blobRegister(url, crossCtx ? globalThis.btoa(blobBytes(blob) || '') : '', ownerRealm); } catch (_) {}
    return url;
  };
...
// Read a blob URL's bytes (base64) AND content type from THIS realm's local
...
globalThis.__csimReadBlobForWindow = function (url) {
  const blob = blobs.get(String(url).split('#')[0]);
  if (blob) {
    try { return { b64: globalThis.btoa(blobBytes(blob)), type: blob.type || 'text/html' }; }
    catch (_) { return null; }
  }
...
// URL entry stays valid in the document it was navigated to). `b64` is the bytes
// base64-encoded; the resulting blob is reconstructed verbatim (latin-1, no UTF-8
// re-encode). No host re-registration — the blob keeps its original partition entry.
globalThis.__csimAdoptBlobBytes = function (url, b64, type) {
  try {
    blobs.set(String(url).split('#')[0], blobFromBytes(Blob.prototype, globalThis.atob(b64), type));
  } catch (_) {}
};
