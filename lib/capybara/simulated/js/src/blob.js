...
export function installBlobURL() {
  ...
  globalThis.URL.createObjectURL = function (blob) {
    ...
    blobs.set(url, blob);
    // Register the URL's existence with the cross-realm host registry so a revoke
    // in ANY realm is visible everywhere (resolveBlobBytes treats host-absence as
    // revoked). Carry the bytes (base64) only when workers exist — a worker may
    // need to resolve them cross-isolate; otherwise a cheap existence marker keeps
    // createObjectURL off the base64 path.
    try { globalThis.__csim_blobRegister(url, hasWorkers() ? globalThis.btoa(blobBytes(blob) || '') : ''); } catch (_) {}
    return url;
  };
