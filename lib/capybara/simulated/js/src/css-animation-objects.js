...
// An animation on a `::before` / `::after` fires its events at the ORIGINATING element with
// `pseudoElement` naming the box (CSS Animations 2 §6): the generated box is no event target.
function fireAnimationEvent(el, type, name, elapsed, anim) {
  const target = el._pseudo ? el._parent : el;
  target.dispatchEvent(new AnimationEvent(type, {
    animationName: name, elapsedTime: elapsedSeconds(elapsed), pseudoElement: el._pseudo ? '::' + el._pseudo : '',
...
function fireTransitionEvent(el, type, prop, elapsed, anim) {
  ...
  target.dispatchEvent(new TransitionEvent(type, {
    propertyName: prop, elapsedTime: elapsedSeconds(elapsed), pseudoElement: el._pseudo ? '::' + el._pseudo : '',
...
export function seedStyleFlush() {
  ...
  discoverStyleChanges();
}

// What both the flush and the rendering update do to find work: take what changed, discover the
// animations the cascade now declares, and record what each transitioned property computes to.
// Neither fires anything.
function discoverStyleChanges() {
