...
export function installHtmlParser({ Document, Element, Text, Comment, DocumentFragment, DocumentType, createHtmlPageDocument, registerFormName }) {
  ...
  function parseDocument(html) {
    ...
    // Synthesize a DocumentType node for a leading `<!DOCTYPE …>` so
    // `document.doctype` and doctype namespace lookups behave.
    const dt = DocumentType && DOCTYPE_RE.exec(html);
    if (dt) {
      const node = new DocumentType(dt[1], unquote(dt[2]), unquote(dt[3] || dt[4]), doc);
      node._parent = doc;
  ...
  // tag) — one that is NOT script/style raw-text or comment data. A single
  // global regex whose leading alternatives consume comment / `<script>` /
  // `<style>` runs (so a tag literal sitting in script data or a comment — e.g.
  // a test passing "<body>…" as a string — is skipped) and whose last
  // alternative captures the wanted tag; we early-exit at the first real hit, so
  // there is no full-document copy or rescan. Returns {index, attrs, end} | null.
  function findStructuralTag(html, want, from) {
    const re = new RegExp(
      '<!--[\\s\\S]*?-->|<script\\b[^>]*>[\\s\\S]*?<\\/script\\s*>|' +
      '<style\\b[^>]*>[\\s\\S]*?<\\/style\\s*>|<(' + want + ')\\b([^>]*)>',
      'gi');
    re.lastIndex = from || 0;
  ...
  function stripHtmlWrapper(html) {
    ...
    if (bodyTag) {
      ...
      // before <body>; a bounded scan of that prefix is enough.
      const pre       = html.slice(0, bodyTag.index);
      const htmlMatch = /<html\b([^>]*)>/i.exec(pre);
      const headMatch = /<head\b([^>]*)>/i.exec(pre);
      const head = pre
        .replace(/<!doctype[^>]*>/i, '')
        .replace(/<\/?html\b[^>]*>/gi, '')
        .replace(/<\/?head\b[^>]*>/gi, '');
    ...
    // No <body> tag: an explicit <head>…</head> is the head, and the whole
  ...
  function applyAttributes(el, rest, foreign) {
    ...
    while ((m = re.exec(rest)) !== null) {
      ...
      el._attrs[k] = dv;
      // A parsed control's name/id becomes a form named-property; the parser
      // bypasses setAttribute, so register here too (registerFormName is
      // passed through the install seam — no per-attr global lookup).
      if (k === 'id' || k === 'name') registerFormName(el, dv);
    }
  }
...
export function serializeElement(el) {
  ...
  // Serialize each attribute under its qualified NAME. The store key usually IS
  // the qualified name; a collision-keyed namespaced attr has a synthetic key, so
  // recover the qualified name from _attrNS (same as the XML serializer).
