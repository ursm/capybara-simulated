...
// Whether a serialized declarative root declares `shadowrootcustomelementregistry`:
// a scoped root always does; a null root only when the host itself is non-null
// (a null host implies null roots).
...
function shadowTemplate(el, opts) {
  ...
  return '<template shadowrootmode="' + sr.mode + '"' +
    ...
    // A root whose registry DIVERGES from "the host's, null-or-global" must
    // declare it: a scoped root always serializes the attribute; a null root
    // only when the host itself is non-null (a null host implies null roots).
    (serializeRegistryAttr(sr, el) ? ' shadowrootcustomelementregistry=""' : '') +
