...
// Each code point goes through the shared `__csimSingleByteEncoder` (ASCII → self,
// a representable high code point → its byte, else null); a null result becomes a
// decimal numeric character reference `&#N;` (the "html" encoder error mode), and
// a lone surrogate first becomes U+FFFD (unrepresentable in every legacy single
// -byte set → `&#65533;`). Returns null for a non-single-byte encoding — callers
// gate on `isSingleByteEncoding(name)`; UTF-8 keeps its own (TextEncoder) path.
export function legacyFormEncode(str, name) {
  const enc = globalThis.__csimSingleByteEncoder(name);
  if (!enc) return null;
  let out = '';
  for (const ch of String(str)) {
    ...
    const b = enc(cp);
    out += (b === null) ? ('&#' + cp + ';') : String.fromCharCode(b);
  }
  return out;
}
...
// Base64 straight from a byte VIEW, without the latin-1 string `btoa` needs in between. For the
// megabyte-scale buffers that cross the engine boundary (a screenshot's pixels) that intermediate
// is the whole cost: `bytesToLatin1` + `btoa` builds two strings the size of the image before the
// first byte moves. Reads the typed array directly and appends in 3-byte groups.
//
// CHUNKED, for the same reason `bytesToLatin1` is: QuickJS grows a string in place only while the
// allocator has slack after it, and the slack runs out as the string does. Measured in the VM, `+=`
// alone takes 3.8 s for 33.5 MB and 27.4 s for 67 MB — doubling the input costs 7x the time —
// where chunk-and-join stays flat at ~52 ms/MB (1.7 s and 3.5 s). A Worker frame is hundreds of MB
// (see `transfer_buffer_stash`), so the quadratic arm is not hypothetical. It also halves the
// PEAK: `+=` holds a full transient copy on every reallocation, and the VM's own memory limit is
// 512 MB.
const B64_CHUNK = 0x2000;
export function base64FromBytes(view) {
  const n = view.length;
  const parts = [];
  let chunk = '', i = 0;
  for (; i + 2 < n; i += 3) {
    const c1 = view[i], c2 = view[i + 1], c3 = view[i + 2];
    chunk += B64_CHARS[c1 >> 2] + B64_CHARS[((c1 & 3) << 4) | (c2 >> 4)] +
             B64_CHARS[((c2 & 15) << 2) | (c3 >> 6)] + B64_CHARS[c3 & 63];
    if (chunk.length >= B64_CHUNK) { parts.push(chunk); chunk = ''; }
  }
  if (i < n) {
    const c1 = view[i], c2 = i + 1 < n ? view[i + 1] : 0;
    chunk += B64_CHARS[c1 >> 2] + B64_CHARS[((c1 & 3) << 4) | (c2 >> 4)] +
             (i + 1 < n ? B64_CHARS[(c2 & 15) << 2] : '=') + '=';
  }
