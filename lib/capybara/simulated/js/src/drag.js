...
import { rectOf, applyScrollTo }      from './layout.js';
import { dispatchEventForUserAction } from './dispatch.js';
import { pointerEventInit }           from './events.js';

// The box a pointer coordinate is measured against. A real browser has exactly ONE geometry, which
// both the driver's synthetic pointer and the page's own math read; here they're still separate —
// the public `getBoundingClientRect` is a coarse placeholder (see layout.js) while the layout engine
// is real-ish — so the two can't agree yet. We use the layout engine: a pointer-driven library that
// computes "where is the pointer relative to my box" from the public rect therefore can't be driven
// (jQuery UI droppable), which is precisely what the public-rect increment fixes.
function dragRect(el) { return rectOf(el); }

function rectCenter(r) { return { x: r.x + r.width / 2, y: r.y + r.height / 2 }; }

// A drag can only start from, and land on, something the user could reach — so bring an off-screen
// end into view first, the way the reference driver's `scroll_if_needed` /
// `scrollIntoView({block: 'center'})` do. Only when needed: a gratuitous scroll would move the page
...
function ensureInView(el) {
  ...
  if (r.x + r.width < 0 || r.y + r.height < 0 || r.x > vw || r.y > vh) applyScrollTo(root, el, 'center');
}
...
// Drag `source` onto `target`. `opts.modifiers` is a `{altKey: true, …}` map held down for the drop
// (Capybara's `drop_modifiers:`); `opts.html5` forces a mode when the caller knows better than the
// press-based sniff.
export function dragOnto(source, target, opts) {
  ...
  // The opening press doubles as the mode sniff: a page that cancels it (jQuery UI draggable does)
  // has taken over the pointer, so a drag-event sequence would be wrong — and a source with no
  // draggable ancestor can't produce one at all.
  ...
  ensureInView(target);
  if (html5) html5Drag(dragSource || source, target, sourcePt, modifiers);
  else       pointerDrag(target, sourcePt, modifiers);
  return true;
}

// Pointer-driven drag: the moves the page's own drag implementation listens for. Two steps — one
// entering the target's box, one at its centre — so a handler that reacts to direction (and any
// start threshold, jQuery UI's default is 1px) sees real movement before the release.
function pointerDrag(target, sourcePt, modifiers) {
  const targetRect = dragRect(target);
  const centre = rectCenter(targetRect);
  firePointerStep(target, 'mousemove', pointOnRect(sourcePt, targetRect), modifiers);
  firePointerStep(target, 'mousemove', centre, modifiers);
  firePointerStep(target, 'mouseup',   centre, modifiers);
}

// The HTML5 sequence. `dragstart` fires before the modifiers go down (they're the DROP modifiers),
// matching what a user holding a key at the end of a drag produces. The button is released at the
// end either way — the gesture that began with a press has to finish with one.
function html5Drag(source, target, sourcePt, modifiers) {
  const dt = new globalThis.DataTransfer();
  // A link / image drag seeds the data store with its URL, like the UA does.
  if (source._tag === 'a' && source.href)  { dt.setData('text/uri-list', source.href); dt.setData('text', source.href); }
  if (source._tag === 'img' && source.src) { dt.setData('text/uri-list', source.src);  dt.setData('text', source.src); }

  const targetRect = dragRect(target);
  const centre     = rectCenter(targetRect);
  // Cancelling `dragstart` aborts the drag: no drag events follow, and the press is released.
  if (fireDrag(source, 'dragstart', sourcePt, dt).defaultPrevented) {
    firePointerStep(source, 'mouseup', sourcePt, modifiers);
    return;
  }
  ...
  const accepted = fireDrag(target, 'dragover', centre, dt, modifiers).defaultPrevented;
