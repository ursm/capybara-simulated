...
// A (static) HTMLCollection over a fixed snapshot. Reuses the live-collection
// Proxy with a constant backing array, so the exotic semantics (non-Array,
// indexed/named access, read-only legacy-platform-object write traps) are
// identical to the live collections. An iterable (e.g. a live collection) is
// materialised into a plain array first so the result is a true snapshot.
export function htmlCollection(src) {
...
export function liveHTMLCollection(rawQuery) {
  ...
  proxy = new Proxy(target, {
    ...
    getOwnPropertyDescriptor(t, prop) {
      ...
      // reporting it absent here is invariant-safe.
