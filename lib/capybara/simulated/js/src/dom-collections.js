...
// `<select>.options` is an HTMLOptionsCollection — an HTMLCollection that is ALSO
// writable: `options[i] = optionEl | null`, `options.length = n`, plus `add` /
// `remove` / `selectedIndex`. Distinct subclass so its prototype chain stays
// separate (same core-js rationale as HTMLCollection) and `instanceof
// HTMLCollection` still holds.
export class HTMLOptionsCollection extends HTMLCollection {}
globalThis.HTMLOptionsCollection = HTMLOptionsCollection;
// Override the inherited `[object HTMLCollection]` tag.
...
// `add` / `remove` / `selectedIndex` are HTMLOptionsCollection PROTOTYPE members
// (like `item` / `namedItem` on HTMLCollection), so they are NOT own properties
// of a collection — `Object.getOwnPropertyNames(select.options)` stays indices +
// named props, matching real browsers. Each resolves the owning <select> from a
// WeakMap keyed by the live collection (the Proxy), populated in
// `liveOptionsCollection`. `this` is that Proxy when called as `options.add(…)`.
const optionsCollSelect = new WeakMap();   // options-collection Proxy → owning <select>
Object.defineProperty(HTMLOptionsCollection.prototype, 'add', {
  value: function (element, before) { return optionsCollSelect.get(this).add(element, before); },
  writable: true, enumerable: true, configurable: true
});
Object.defineProperty(HTMLOptionsCollection.prototype, 'remove', {
  value: function (index) { return optionsCollSelect.get(this).remove(index); },
  writable: true, enumerable: true, configurable: true
});
Object.defineProperty(HTMLOptionsCollection.prototype, 'selectedIndex', {
  get() { return optionsCollSelect.get(this).selectedIndex; },
  set(v) { optionsCollSelect.get(this).selectedIndex = v; },
  enumerable: true, configurable: true
});

// Live HTMLOptionsCollection for `select`. `rawQuery()` returns the select's
// list of options (select children + optgroup children, tree order). Mirrors
// `liveHTMLCollection`'s read traps; the extra write traps implement the HTML
// "HTMLOptionsCollection setter" (opts[i] = option|null) and the length / add /
// remove / selectedIndex operations, all delegating to the owning <select>.
export function liveOptionsCollection(select, rawQuery) {
  ...
  const target = new HTMLOptionsCollection();

  // HTML "setting a new indexed property value" on an HTMLOptionsCollection:
  //   null     → remove the option at i (no-op if out of range)
  //   option   → pad with blank <option>s up to i, then append; or, when i is
  //              within range, replace the i-th option.
  const setByIndex = (i, val) => {
    const opts = live();
    const len  = opts.length;
    if (val == null) {
      if (i >= 0 && i < len && opts[i] && opts[i]._parent) opts[i]._parent.removeChild(opts[i]);
      return;
    }
    if (!val || val._tag !== 'option') {
      throw new globalThis.TypeError(
        "Failed to set an indexed property on 'HTMLOptionsCollection': The provided value is not of type 'HTMLOptionElement'.");
    }
    const n = i - len;
    // Defensive cap (mirrors the `<select>.length` setter): an index parsed from
    // INDEX_RE can be up to ~4 billion, and padding to it one appendChild at a
    // time would hang the VM. A real grow is tiny; skip an absurd one.
    if (n > 100000) return;
    if (n > 0) {
      const doc = select.ownerDocument || globalThis.document;
      for (let k = 0; k < n; k++) select.appendChild(doc.createElement('option'));
    }
    if (n >= 0) select.appendChild(val);
    else { const ref = opts[i]; (ref._parent || select).replaceChild(val, ref); }
  };

  const isSupportedNamed = (t, prop) =>
    typeof prop === 'string' &&
    !Object.prototype.hasOwnProperty.call(t, prop) &&
    !(prop in HTMLOptionsCollection.prototype) &&
    !!hcNamedItem(live(), prop);
  ...
  proxy = new Proxy(target, {
    get(t, prop, recv) {
      ...
      if (Object.prototype.hasOwnProperty.call(t, prop)) return t[prop];   // page-set expandos
      if (typeof prop === 'string' && !(prop in HTMLOptionsCollection.prototype)) {
        const named = hcNamedItem(live(), prop);
        if (named) return named;
      }
      return Reflect.get(t, prop, recv);   // add / remove / selectedIndex / item / namedItem / length / iterator (prototype)
    },
    set(t, prop, val, recv) {
      if (recv !== proxy) return Reflect.set(t, prop, val, recv);
      if (typeof prop === 'string' && INDEX_RE.test(prop)) { setByIndex(+prop, val); return true; }
      if (prop === 'length') { select.length = val; return true; }
      // selectedIndex is a prototype accessor resolving the select via a WeakMap
      // keyed by the Proxy; `t[prop] = val` would invoke that setter with the raw
      // target (not the Proxy) as `this`, missing the WeakMap — so route it here.
      if (prop === 'selectedIndex') { select.selectedIndex = val; return true; }
      if (isSupportedNamed(t, prop)) return false;
      t[prop] = val;   // page-set expandos
      return true;
    },
    has(t, prop) {
      if (prop === 'length') return true;
      if (typeof prop === 'string' && INDEX_RE.test(prop)) return (+prop) < live().length;
      if (prop in t) return true;
      if (typeof prop === 'string' && !(prop in HTMLOptionsCollection.prototype) && hcNamedItem(live(), prop)) return true;
      return false;
    },
    getOwnPropertyDescriptor(t, prop) {
      if (prop === 'length') {
      ...
      if (typeof prop === 'string' && INDEX_RE.test(prop)) {
        const arr = live(), i = +prop;
        if (i < arr.length) return { value: arr[i], writable: true, enumerable: true, configurable: true };
        return undefined;
      }
      if (Object.prototype.hasOwnProperty.call(t, prop)) return Object.getOwnPropertyDescriptor(t, prop);
      if (typeof prop === 'string' && !(prop in HTMLOptionsCollection.prototype)) {
    ...
    ownKeys(t) {
      const arr = live(), keys = [];
      for (let i = 0; i < arr.length; i++) keys.push(String(i));
      for (const n of hcSupportedNames(arr)) if (!INDEX_RE.test(n) && !(n in HTMLOptionsCollection.prototype)) keys.push(n);
      for (const k of Reflect.ownKeys(t)) if (keys.indexOf(k) === -1) keys.push(k);   // page-set expandos
      return keys;
    },
    defineProperty(t, prop, desc) {
      if (typeof prop === 'string' && INDEX_RE.test(prop)) {
        // The indexed property is reported {value, writable:true, enumerable:true,
        // configurable:true} by getOwnPropertyDescriptor; only honour a data
        // descriptor compatible with that (else the Proxy invariant — a returned
        // `true` must match the reported descriptor — would throw). A more
        // restrictive define (configurable/writable/enumerable:false) is rejected.
        if ('value' in desc && desc.configurable !== false && desc.writable !== false && desc.enumerable !== false) {
          setByIndex(+prop, desc.value);
          return true;
        }
        return false;
      }
  ...
  optionsCollSelect.set(proxy, select);   // prototype add/remove/selectedIndex resolve the select from here
  return proxy;
}
...
// `RadioNodeList` — the NodeList subclass returned by a form / form.elements named
// getter when more than one control shares a name. Adds the radio-group `value`
// accessor (read: the first checked member's value; write: check the member whose
// value matches). It is iterable + array-like via NodeList, and `instanceof
// NodeList` holds (form-elements-nameditem-01 / form-nameditem).
export class RadioNodeList extends NodeList {
  get value() {
    for (let i = 0; i < this.length; i++) {
      const el = this[i];
      if (el && (el._tag === 'input') && (el._attrs.type || '').toLowerCase() === 'radio' && el.checked) {
        const v = el.value;
        return v == null ? '' : v;
      }
    }
  ...
  set value(v) {
    ...
    for (let i = 0; i < this.length; i++) {
      const el = this[i];
      if (el && el._tag === 'input' && (el._attrs.type || '').toLowerCase() === 'radio' && el.value === s) { el.checked = true; return; }
    }
  }
}
globalThis.RadioNodeList = RadioNodeList;
Object.defineProperty(RadioNodeList.prototype, Symbol.toStringTag, { value: 'RadioNodeList', configurable: true });

// A live RadioNodeList — `liveNodeList`, but the backing target is a RadioNodeList
// so `instanceof RadioNodeList` holds and `.value` is available.
export function liveRadioNodeList(rawQuery) {
  const target = new RadioNodeList();
  let cacheGen = NaN;
  const sync = () => {
    const g = currentSettleGen();
    if (g !== cacheGen) {
...
// `form.elements` is an HTMLFormControlsCollection — an HTMLCollection whose
// `namedItem` (and named-property access) returns a RadioNodeList when more than
// one control shares the name (vs HTMLCollection's first-match). Distinct subclass
// so `instanceof HTMLFormControlsCollection` AND `instanceof HTMLCollection` hold.
...
// namedItem: 0 → null, 1 → the element, >1 → a (fresh-snapshot) RadioNodeList.
// The live, cached form returns through the proxy's get trap (fccNamedValue);
// this prototype method is the explicit `.namedItem(name)` call.
Object.defineProperty(HTMLFormControlsCollection.prototype, 'namedItem', {
  ...
  value: function (name) {
    ...
    const matches = [];
    const len = this.length;
    for (let i = 0; i < len; i++) { const el = this[i]; if (el && el._attrs && (el._attrs.id === name || (el._ns === HTML_NS_HC && el._attrs.name === name))) matches.push(el); }
    if (matches.length === 0) return null;
    if (matches.length === 1) return matches[0];
    const nl = new RadioNodeList();
...
function fccMatches(arr, name) {
  const out = [];
  for (const el of arr) if (el && el._attrs && (el._attrs.id === name || (el._ns === HTML_NS_HC && el._attrs.name === name))) out.push(el);
  return out;
}

// Live HTMLFormControlsCollection. Mirrors `liveHTMLCollection`'s read traps, but
// the named-property path returns a RadioNodeList (cached [SameObject] + live) when
// a name matches >1 control. `rawQuery()` returns the form's controls in tree order.
export function liveFormControlsCollection(rawQuery) {
  ...
  const target = new HTMLFormControlsCollection();
  const radioLists = new Map();   // name → cached live RadioNodeList ([SameObject] for repeated/held access)
  // Named value: 0 → undefined (not a supported name), 1 → the element, >1 → the
  // cached live RadioNodeList.
  const namedValue = (name) => {
    const m = fccMatches(live(), name);
    if (m.length === 0) return undefined;
    if (m.length === 1) return m[0];
    let nl = radioLists.get(name);
    if (!nl) { nl = liveRadioNodeList(() => fccMatches(live(), name)); radioLists.set(name, nl); }
    return nl;
  };
  const isSupportedNamed = (t, prop) =>
    typeof prop === 'string' &&
    !Object.prototype.hasOwnProperty.call(t, prop) &&
    !(prop in HTMLFormControlsCollection.prototype) &&
    fccMatches(live(), prop).length > 0;
  let proxy;
  proxy = new Proxy(target, {
    get(t, prop, recv) {
      if (prop === 'length') {
      ...
      if (typeof prop === 'string' && INDEX_RE.test(prop)) { const arr = live(), i = +prop; return i < arr.length ? arr[i] : undefined; }
      if (Object.prototype.hasOwnProperty.call(t, prop)) return t[prop];   // expandos
      if (typeof prop === 'string' && !(prop in HTMLFormControlsCollection.prototype)) {
        const v = namedValue(prop);
        if (v !== undefined) return v;
      }
      return Reflect.get(t, prop, recv);   // item / namedItem / length getter / iterator (prototype)
    },
    set(t, prop, val, recv) {
      if (recv !== proxy) return Reflect.set(t, prop, val, recv);
      if (typeof prop === 'string' && INDEX_RE.test(prop)) return false;
    ...
    has(t, prop) {
      ...
      if (typeof prop === 'string' && INDEX_RE.test(prop)) return (+prop) < live().length;
      if (prop in t) return true;
      if (typeof prop === 'string' && !(prop in HTMLFormControlsCollection.prototype) && fccMatches(live(), prop).length > 0) return true;
      return false;
    },
    getOwnPropertyDescriptor(t, prop) {
      if (prop === 'length') {
        return Object.prototype.hasOwnProperty.call(t, 'length') ? Object.getOwnPropertyDescriptor(t, 'length') : undefined;
      }
      if (typeof prop === 'string' && INDEX_RE.test(prop)) {
        const arr = live(), i = +prop;
        if (i < arr.length) return { value: arr[i], writable: false, enumerable: true, configurable: true };
    ...
    ownKeys(t) {
      const arr = live(), keys = [];
      for (let i = 0; i < arr.length; i++) keys.push(String(i));
      for (const n of hcSupportedNames(arr)) if (!INDEX_RE.test(n) && !(n in HTMLFormControlsCollection.prototype)) keys.push(n);
