...
export function easingFunction(text) {
  ...
  m = /^linear\(([^)]*)\)$/.exec(v);
  if (m) {
    // Piecewise linear through the stops: the output walks from one to the next as the input
    // crosses their positions (Chrome-measured: `linear(0, 1, 0)` samples 0, 0.5, 1, 0.5, 0).
    const stops = linearStops(m[1]);
    if (stops) {
      return (x) => {
        const at = x * 100;
        if (at < stops[0].p) return stops[0].v;
        // The segment is the LAST one that starts at or before `at`, so a pair of stops sharing a
        // position is a vertical jump the output lands on the far side of (Chrome-measured:
        // `linear(0, 0.5 50%, 1 50%, 0)` is 1 at the halfway point, not 0.5).
        let i = 1;
        while (i < stops.length - 1 && at >= stops[i].p) i++;
        const a = stops[i - 1], b = stops[i];
        if (at > b.p) return b.v;
        return b.p === a.p ? b.v : a.v + (b.v - a.v) * ((at - a.p) / (b.p - a.p));
      };
    }
  }
  m = /^steps\(([^)]*)\)$/.exec(v);
...
// Whether a CSS animation on `el` that is CURRENT — waiting out its delay, or running — or IN EFFECT (a fill holding
// its value) declares any of `props`: the side effect such an animation has of making the element a stacking context
// (web-animations, "Side effects of animations"). A CSS animation plays forwards, so its before phase is current.
export function cssAnimationCurrentlyDeclares(el, props, read) {
  if (!documentHasKeyframes()) return false;
  const anims = animationsFor(el, read);
  ...
  for (const anim of anims) {
    ...
    propsIn(anim.blocks, declared);
    if (!props.some((prop) => declared.has(prop))) continue;
    const t = computedTimingAt(anim, now - started);
    if (t.phase === 'before' || t.phase === 'active' || t.progress != null) return true;
  }
  return false;
}
...
export function transitionSpecs(el, read) {
...
export function runKey(prop, twin) { return twin && twin < prop ? twin : prop; }
