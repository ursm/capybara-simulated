...
//   * `animation-play-state` and `animation-composition` are parsed and ignored: a paused
//     animation keeps running, and every keyframe replaces rather than adds.
//   * Web Animations (`element.animate`, `getAnimations`) is a separate surface and still a stub.
import { keyframesNamed, documentHasKeyframes } from './cascade.js';
import { interpolateProperty, discreteValue, numericValue } from './interpolate.js';
...
import { lastMutationAt } from './mutation-observer.js';
...
// ── Timing ───────────────────────────────────────────────────────────────────────────────────
// Where an animation is, as a progress in [0,1] along ONE iteration — or `null` when it is not in
// effect at all (before it starts or after it ends, with no fill mode to hold a value there).
function iterationProgress(anim, localTime) {
  const active = anim.duration * anim.iterations;
  const fillsBackwards = anim.fill === 'backwards' || anim.fill === 'both';
  const fillsForwards  = anim.fill === 'forwards'  || anim.fill === 'both';
  let progress;
  let iteration = 0;
  if (localTime < 0) {
    if (!fillsBackwards) return null;
    progress = 0;
  } else if (anim.duration === 0) {
    // A zero-duration animation is entirely at its end from the instant it starts.
    if (localTime > 0 && !fillsForwards) return null;
    progress = endProgress(anim);
    iteration = Math.max(0, Math.ceil(anim.iterations) - 1);
  } else if (localTime >= active) {
    if (!fillsForwards) return null;
    progress = endProgress(anim);
    iteration = Math.max(0, Math.ceil(anim.iterations) - 1);
  } else {
    const overall = localTime / anim.duration;
    iteration = Math.floor(overall);
    progress = overall - iteration;
  }
  ...
  if ((dir === 'alternate' || dir === 'alternate-reverse') && iteration % 2 === 1) reverse = !reverse;
  if (reverse) progress = 1 - progress;
  return easingFunction(anim.easing)(progress);
}

// Where a FINISHED animation holds: at the end of its last iteration, which for a fractional
// iteration count is that fraction through it and not the end (web-animations §4.5.5 — Chrome
// holds `iteration-count: 2.5` half way, not at 100%).
function endProgress(anim) {
  const frac = anim.iterations % 1;
  return Number.isFinite(anim.iterations) && frac !== 0 ? frac : 1;
}
...
function stopsFor(blocks, prop, twin) {
  ...
  for (const block of blocks) {
    ...
    for (const d of block.decls) {
      ...
      // at it (css-animations §4.1).
      else if (d.prop === 'animation-timing-function') easing = d.value;
    }
    if (value === undefined) continue;
    for (const off of block.offsets) byOffset.set(off, { value, easing });
  }
...
export function animatedValue(el, prop, twin, underlying, read, resolve) {
  ...
  for (const anim of anims) {
    const stops = stopsFor(anim.blocks, prop, twin);
    if (!stops) continue;
    const progress = iterationProgress(anim, now - started - anim.delay);
...
function valueAt(el, prop, stops, progress, underlying, resolve) {
  ...
  if (ends.length === 1) return endpointValue(el, prop, ends[0], resolve);
  ...
  const from = endpointValue(el, prop, lo, resolve);
  const to   = endpointValue(el, prop, hi, resolve);
...
// keyframe's declared text is not.
function endpointValue(el, prop, stop, resolve) {
  return stop.computed ? stop.value : resolve(el, prop, stop.value);
}
...
function longhandsOf(name, depth = 0) {
