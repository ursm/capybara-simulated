...
// element (a custom property inherits). So it is never expanded structurally — the cascade keeps
// the declaration whole and the resolved-value read re-expands it against the element.
...
function placeTokens(components, initials, toks) {
  ...
  for (const tok of toks) {
    // A SUBSTITUTION can only be assigned to a component once it RESOLVES, and most of these
    // groups end in a catch-all matcher (`animation-name`, `column-rule-color`) that would
    // swallow the whole reference: `animation: var(--spin)` came back as an animation NAME of
    // `spin 2s linear infinite` with a duration of `0s`. Failing the expansion keeps the
    // shorthand whole, which is what the resolved-value read re-expands per element.
    if (SUBSTITUTION_RE.test(tok)) return null;
    let placed = false;
...
export function serializeDeclBlock(decls) {
  ...
  for (const name of keys) {
    ...
    for (const shName of LONGHAND_TO_SHORTHANDS[name] || []) {
      ...
      const combined = def.serialize(split.map(s => serializeCssValue(s.value)), true);
