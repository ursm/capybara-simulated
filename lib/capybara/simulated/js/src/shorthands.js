  ...
  return {
    ...
    serialize(vals) {
      ...
      for (let i = 0; i < count; i++) {
        const parts = layers.map((l, c) => (l[i] || l[0] || initials[c]).trim())
...
function slashDef(longhands, mirror) {
  ...
  return {
    ...
    expand(value) {
      ...
      return longhands.map((_, i) => parts[i] !== undefined ? parts[i] : omitted(parts[mirror[i]]));
    },
  };
}
...
function pairDef(longhands) {
  return {
    ...
    expand(value) {
      ...
      if (parts.length === 1) return [parts[0], parts[0]];
      if (parts.length === 2) {
        // Two tokens are two VALUES only when neither is a modifier bound to the other.
        if (ALIGN_MODIFIER_RE.test(parts[0])) {
          // `first baseline` is just `baseline` — the modifier is the default and a browser
          // drops it (Chrome measured). `safe center` keeps both tokens.
          const one = /^first$/i.test(parts[0]) && /^baseline$/i.test(parts[1]) ? parts[1] : value.trim();
          return [one, one];
        }
