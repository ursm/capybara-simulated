...
import { serializeCssValue, splitTopLevel, isValidDeclarationValue } from './css-utils.js';
import { LONGHANDS, SHORTHAND_LONGHANDS } from './css-property-data.js';
import { isStaticallyInvalidMath, simplifySpecifiedMath, unprefixMath } from './calc.js';
...
// including those still at their initial, differs per shorthand, in opposite directions: Chrome
// reads `flex-flow` as `row nowrap` but writes `flex-flow: row`, while `animation` reads back
// `2s linear infinite spin` and writes `animation: 2s linear 0s infinite normal none running spin`
// (both measured). One `serialize` feeds both surfaces, so each def names its exhaustive one.
  ...
  return {
    ...
    serialize(vals, specified) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const set = all.filter((v, c) => v !== initials[c]);
...
// Whether `prop: value` is a declaration the property can take — the ONE answer for every surface that
// admits one: the per-property setter, the BLOCK parse (`style.cssText`, `setAttribute('style', …)`), the
// cascade, and `CSS.supports` / `@supports`. They have to agree, or a page gets a different answer depending on which way it wrote the declaration.
// A shorthand is invalid when ANY component it expands to is (CSS Cascade "shorthand properties"), and the whole
// declaration is then dropped rather than the bad component alone: `padding-block: none` sets neither side. The
// longhand grammars are the ones that know — a shorthand has no classification of its own. The cascade judged the
// shorthand alone, so `margin: 0 min(10px, 26%) + max(5em, 20px)` — dropped by the two CSSOM surfaces, and by
// Chrome — still applied `margin-left: max(5em, 20px)`. Not at all while a SUBSTITUTION is in the value:
// `margin-inline: var(--one) var(--two)` cannot be split into sides before `--one` resolves, so the expanders hand
// the whole value to one longhand, and judging it as that longhand's grammar sees two values where one is allowed.
export function declarationIsValid(prop, value) {
  const bare = String(value).replace(/\s*!\s*important\s*$/i, '');
  if (!isValidDeclarationValue(prop, bare)) return false;
  if (!isRegularShorthand(prop) || hasSubstitution(bare)) return true;
  // (…and one that does not DECOMPOSE is no declaration either: the setter always dropped it, while the block parse
  // kept it whole and `CSS.supports` said yes — `flex: 1 2 calc(0)`, which Chrome drops everywhere.)
  const pairs = shorthandExpand(prop, bare);
  if (!pairs) return false;
  for (const [lh, v] of pairs) if (!isValidDeclarationValue(lh, String(v))) return false;
  return true;
}
...
// can't be split yet — one holding an unresolved substitution — is kept whole; one that can't be split at all
// is no declaration (`declarationIsValid`).
...
export function expandDeclList(list) {
  ...
  for (const { prop, value: raw } of list) {
    const value = prop.startsWith('--') ? raw : unprefixMath(raw);
    // A statically-invalid math function is a PARSE error — the declaration never enters the block.
    // This is the BLOCK-parse surface (`style.cssText`, `setAttribute('style', …)`); the
    // per-property setter rejects the same value through `isValidDeclarationValue`, and the cascade
    // drops it in `expandDeclaration`. All three have to agree, or a page gets a different answer
    // depending on which way it wrote the declaration.
    if (!prop.startsWith('--') && !hasSubstitution(value) && isStaticallyInvalidMath(value)) continue;
    // The SPECIFIED surface keeps a `calc()` wrapper with its contents reduced (Chrome measured:
    // `calc(10px + 5px)` reads back as `calc(15px)`). Applied at STORE time so the per-property read
    // and the serialized attribute agree — the computed surface reduces separately, with an element
    // in hand for the font- and viewport-relative units this stage can't touch.
    const stored = prop.startsWith('--') ? value : simplifySpecifiedMath(value);
    // …and so is a value the property's GRAMMAR rejects, on THIS surface too. The per-property
    // setter has always dropped one; a block parse that kept it left the two disagreeing about the
    // same declaration (`el.style.blockSize = 'none'` empty while `setAttribute('style',
    // 'block-size: none')` kept it, where Chrome drops both — its `cssText` comes back empty).
    if (!prop.startsWith('--') && !declarationIsValid(prop, stored)) continue;
    ...
    if (isRegularShorthand(prop)) {
      const pairs = shorthandExpand(prop, stored);
      if (pairs) { for (const [lh, v] of pairs) setDecl(out, lh, v, true); continue; }
    }
    setDecl(out, prop, stored, involvesAll);
  }
  return out;
}
...
function declText(name, value) {
  ...
  const { value: bare, important } = splitImp(value);
  // A pending substitution has no serialization of its own (css-variables-1 §3). `serializeDeclBlock`
  // skips an orphaned one before reaching here, so this is only a backstop.
  return name + ': ' + (pendingSource(bare) ? '' : serializeCssValue(bare, name)) + (important ? ' !important' : '') + ';';
}
...
export function serializeDeclBlock(decls) {
  ...
  for (const name of keys) {
    ...
    for (const shName of LONGHAND_TO_SHORTHANDS[name] || []) {
      ...
      // The SAME reconstruction the getter runs — this had drifted into a second copy of it, which
      // is how a pending substitution serialized as its internal marker here while `el.style.margin`
      // gave back `var(--m)`. `shorthandGet` also owns the mixed-importance and
      // not-jointly-representable bails, both of which return ''.
      const combined = shorthandGet(decls, shName, true);
      if (combined === '') continue;
      out.push(shName + ': ' + combined + (splitImp(decls[def.longhands[0]]).important ? ' !important' : '') + ';');
      def.longhands.forEach(lh => done.add(lh));
      emitted = true;
      break;
    }
    if (emitted) continue;
    // An ORPHANED pending slot — one whose siblings were overwritten, so the shorthand can no longer
    // be reconstructed — is skipped entirely. Chrome writes a value-less `margin-right: ;` here, but
    // that text is not re-parseable (its own re-parse drops it, measured) and our inline store IS
    // the attribute text, so emitting it left a non-empty style attribute whose declaration block
    // read back empty. Dropping it is the same information, in a form that round-trips.
    if (pendingSource(splitImp(decls[name]).value)) { done.add(name); continue; }
...
export function allGet(decls, name) {
  ...
  if (isRegularShorthand(name)) {
    ...
    return shorthandGet(eff, name, true);
  }
