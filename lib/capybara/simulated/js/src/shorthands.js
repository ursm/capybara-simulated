...
// EXPANDED to its ultimate longhands on the way in (`expandShorthandsInMap`), and the block
...
const SHORTHANDS = {
  ...
  'list-style': freeDef([
    ...
    ['list-style-image',    'none',    isUrlOrNone],
...
export function expandShorthandsInMap(decls) {
  // Take the loop only when there's something to reshape: a shorthand to expand, or a
  // flow-relative longhand whose presence can trigger the logical-group move rule. A block
  // of only physical longhands (`margin-top: 10px` — extremely common) short-circuits to the
  // identity map with zero allocation (rule 3).
  let run = false;
  for (const k in decls) if (isRegularShorthand(k) || isLogicalLonghand(k)) { run = true; break; }
  if (!run) return decls;
  const out = {};
  for (const [k, v] of Object.entries(decls)) {
    if (isRegularShorthand(k)) {
      const pairs = shorthandExpand(k, v);
      if (pairs) { for (const [lh, lv] of pairs) setDecl(out, lh, lv, true); continue; }
    }
    setDecl(out, k, v, false);
  }
  return out;
}
...
export function serializeDeclBlock(decls) {
  ...
  // shorthand's non-longhand interleaver is always a logical sibling); or when an `all`
  // declaration is present, since a shorthand must NOT be reconstructed across `all` (that
  // would lose which of its longhands were declared after `all`). Both are uncommon, so the
  // position bookkeeping — the whole `pos` Map — is skipped otherwise (the common case).
  const anyLogical = keys.some(isLogicalLonghand);
  const hasAll = decls.all !== undefined;
  const pos = (anyLogical || hasAll) ? new Map(keys.map((k, i) => [k, i])) : null;
  for (const name of keys) {
    ...
    for (const shName of LONGHAND_TO_SHORTHANDS[name] || []) {
      ...
      if (hasAll && straddlesAll(def, pos)) continue;
...
// A shorthand straddles an `all` declaration when some of its longhands are declared before
// `all` and some after — `all` resets the earlier ones but not the later, so the shorthand
// isn't a single value and must serialize as individual longhands (css-cascade "all").
function straddlesAll(def, pos) {
  const allPos = pos.get('all');
  let before = false, after = false;
  for (const lh of def.longhands) {
    const i = pos.get(lh);
    if (i < allPos) before = true; else if (i > allPos) after = true;
  }
  return before && after;
}
...
// Effective value of longhand `lh` given an `all: kw` at key index `allPos` and a
// name→index `pos` map: its own value when (re)declared after `all`, else `all`'s keyword
// when covered, else its own value (set before `all` but not covered, e.g. a custom prop)
// or undefined (unset).
function allResolvedLonghand(decls, pos, allPos, kw, lh) {
  const i = pos.has(lh) ? pos.get(lh) : -1;
  if (i > allPos) return decls[lh];
  if (isCoveredByAll(lh)) return kw;
  return i >= 0 ? decls[lh] : undefined;
}
...
export function allGet(decls, name) {
  ...
  const allPos = pos.get('all');
  const kw     = splitImp(decls.all).value.trim().toLowerCase();
  // `all` serializes to its keyword iff no covered property is declared after it.
  if (name === 'all') {
    for (let i = allPos + 1; i < keys.length; i++) if (isCoveredByAll(keys[i])) return '';
    return kw;
  }
  ...
  if (isRegularShorthand(name)) {
    ...
    for (const lh of shorthandLonghands(name)) {
      const v = allResolvedLonghand(decls, pos, allPos, kw, lh);
      if (v == null) return '';
      eff[lh] = v;
    }
  ...
  if (mdnLhs) {
    // A shorthand we DON'T expand (font, background, …) is stored as a single key. If it was
    // (re)declared after `all`, that declaration wins verbatim.
    const i = pos.has(name) ? pos.get(name) : -1;
    if (i > allPos) return splitImp(decls[name]).value;
    ...
    const vals = [];
    for (const lh of mdnLhs) {
      const v = allResolvedLonghand(decls, pos, allPos, kw, lh);
      if (v == null) return '';
      vals.push(splitImp(v).value.trim().toLowerCase());
    }
    return vals.every(x => x === vals[0] && CSS_WIDE.has(x)) ? vals[0] : '';
  }
  // A longhand (or a covered plain property).
  const v = allResolvedLonghand(decls, pos, allPos, kw, name);
  return v == null ? '' : splitImp(v).value;
}

// getPropertyPriority resolved through an `all` declaration: a covered property re-declared
// after `all` uses its OWN priority; otherwise it inherits `all`'s (`all: revert !important`
// makes every covered property important).
export function allGetPriority(decls, name) {
  const keys   = Object.keys(decls);
  const allPos = keys.indexOf('all');
  if (name === 'all') return splitImp(decls.all).important ? 'important' : '';
  const i = keys.indexOf(name);
  if (i > allPos) return splitImp(decls[name]).important ? 'important' : '';
  if (isCoveredByAll(name)) return splitImp(decls.all).important ? 'important' : '';
  return i >= 0 && splitImp(decls[name]).important ? 'important' : '';
}
