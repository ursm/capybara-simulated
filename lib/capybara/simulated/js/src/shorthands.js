...
function declText(name, value) {
  ...
  const { value: bare, important } = splitImp(value);
  // A pending substitution has no serialization of its own (css-variables-1 §3) — it reaches here
  // only when its siblings were overwritten, so the shorthand can no longer be reconstructed, and
  // Chrome writes the bare `margin-right: ;` this produces (measured).
