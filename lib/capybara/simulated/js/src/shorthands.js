...
function freeDef(components, opts) {
  ...
  // `showAll` is a COMPUTED-value rule, not a block-serialization one: Chrome reads `flex-flow` as
  // `row nowrap` from getComputedStyle but writes `flex-flow: row` into a style attribute. One
  // `serialize` feeds both, so the block path asks for the terse form explicitly.
  const showAll = !!(opts && opts.showAll);
  return {
    longhands,
    serialize(vals, forBlock) {
      if (anyCssWide(vals)) return combineCssWide(vals);
      if (showAll && !forBlock) return vals.join(' ');
  ...
  return {
    ...
    serialize(vals) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        // Nothing set in this layer at all → the shorthand's own "nothing" token. Otherwise
        // `animation` lists EVERY component while `transition` keeps only what isn't at its
        // initial (Chrome measured, writing the same style attribute both ways).
        const parts = set.length === 0 ? [] : (showAll ? all : set);
        // Nothing set in this layer: a browser reports the shorthand's own "nothing" token —
        // `none` for `animation`, `all` for `transition` — not whichever component happens to be
        // listed first.
...
const PREFERRED = ['border', 'border-width', 'border-style', 'border-color',
  ...
  'scroll-margin', 'scroll-padding', 'grid-area', 'grid-row', 'grid-column',
