...
// Whether a declaration parsed out of a BLOCK is one the property can take. A shorthand is invalid
// when any component it expands to is (CSS Cascade), and an unresolved substitution defers the
// whole question — the same two rules the per-property setter applies.
function declarationIsValid(prop, stored) {
  const bare = String(stored).replace(/\s*!\s*important\s*$/i, '');
...
export function shorthandGet(decls, name, specified) {
  ...
  const combined = def.serialize(split.map(s => serializeCssValue(s.value)), specified);
...
function declText(name, value) {
  ...
  return name + ': ' + (pendingSource(bare) ? '' : serializeCssValue(bare)) + (important ? ' !important' : '') + ';';
}
