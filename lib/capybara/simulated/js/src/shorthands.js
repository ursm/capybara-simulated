...
function flexExpand(v) {
  ...
  for (const tok of topLevelTokens(trimmed)) {
    ...
    if (FLEX_NUMBER.test(tok) && nums.length < 2) nums.push(tok);
...
// declaration with it.
const DURATION_VALUE = (t) => TIME_VALUE.test(t) && parseFloat(t) >= 0;
...
const SHORTHANDS = {
  ...
  transition: layerDef([
    ['transition-property',        'all',    t => !TIME_VALUE.test(t) && !TIMING_FN.test(t) && !TRANS_BEHAVIOR.test(t)],
...
// Whether `prop: value` is a declaration the property can take — the ONE answer for all three surfaces that
// admit one: the per-property setter, the BLOCK parse (`style.cssText`, `setAttribute('style', …)`) and the
// cascade. They have to agree, or a page gets a different answer depending on which way it wrote the declaration.
...
export function declarationIsValid(prop, value) {
  ...
  if (pairs) for (const [lh, v] of pairs) if (!isValidDeclarationValue(lh, String(v))) return false;
  return true;
}
