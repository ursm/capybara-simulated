...
const SHORTHANDS = {
  ...
  margin:   { longhands: BORDER_SIDES.map(s => `margin-${s}`),  serialize: combineBox, expand: v => expandBox(topLevelTokens(v)) },
  padding:  { longhands: BORDER_SIDES.map(s => `padding-${s}`), serialize: combineBox, expand: v => expandBox(topLevelTokens(v)) },
...
const PREFERRED = ['border', 'border-width', 'border-style', 'border-color',
  ...
  'outline', 'list-style', 'margin', 'padding', 'overflow'];
...
// existing important one. `moveToEnd` (shorthand expansions) re-appends the winning
// declaration at the end, reproducing the order browsers serialize.
