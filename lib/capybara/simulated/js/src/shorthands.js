...
function slashDef(longhands, mirror) {
  return {
    ...
    serialize(vals) {
      ...
      while (parts.length > 1 && parts[parts.length - 1] === (mirror ? parts[mirror[parts.length - 1]] : 'auto')) parts.pop();
      return parts.join(' / ');
    },
    expand(value) {
      const parts = splitTopLevel(value, '/').map(t => t.trim()).filter(Boolean);
      if (!parts.length || parts.length > longhands.length) return null;
      return longhands.map((_, i) => parts[i] !== undefined ? parts[i] : (mirror ? parts[mirror[i]] || 'auto' : 'auto'));
    },
  };
}
...
function pairDef(longhands) {
  return {
    ...
    expand(value) {
      ...
      return [parts[0], parts[1] !== undefined ? parts[1] : parts[0]];
    },
  };
}
