...
export function expandDeclList(list) {
  ...
  for (const { prop, value } of list) {
    if (isRegularShorthand(prop)) {
      ...
      if (pairs) { for (const [lh, v] of pairs) out[lh] = v; continue; }
    }
    out[prop] = value;
  }
  return groupNeedsMove(out) ? expandShorthandsInMap(out) : out;
}
...
export function expandShorthandsInMap(decls) {
  ...
  if (!run) return decls;
  const out = Object.create(null);              // prototype-less, as parseStyleDecls builds it
