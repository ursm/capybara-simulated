...
export function expandShorthandsInMap(decls) {
  ...
  for (const [k, v] of Object.entries(decls)) {
    if (isRegularShorthand(k)) {
      ...
      if (pairs) { for (const [lh, lv] of pairs) mergeDecl(out, lh, lv); continue; }
    }
    mergeDecl(out, k, v);
  }
  return out;
...
// Set `out[name] = value`, but keep an existing `!important` declaration when the new one
// is normal — within a declaration block `!important` wins regardless of order, so a later
// non-important `padding-left` must not override a `padding: … !important` expansion.
function mergeDecl(out, name, value) {
  if (name in out && splitImp(out[name]).important && !splitImp(value).important) return;
  out[name] = value;
}
