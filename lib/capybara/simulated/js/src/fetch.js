...
import { serializeRequestBody, isReadableStreamBody, findHeaderKey } from './request-body.js';
...
// Realm-independent Request brand-check. A Request constructed in ANOTHER realm — the classic
// case is the top window passing `new Request(url, {mode, credentials})` into a controlled
// iframe's `fetch` (fetch-response-taint) — fails `instanceof` against THIS realm's Request, so
// `fetch()` silently dropped its mode / credentials / redirect / body and defaulted mode to
// 'cors'. `@@toStringTag` ('[object Request]') is realm-independent — same pattern as the
// cross-realm Blob / MessagePort / ReadableStream brand-checks.
function isRequest(v) {
  if (!v || typeof v !== 'object') return false;
  return (globalThis.Request && v instanceof globalThis.Request) ||
...
// WebIDL emptiness for a Request `init` dictionary (Fetch "Request(input, init)" step 28: a
// non-empty init detaches the request from a source Request's navigation context). A member set
// to `undefined` counts as ABSENT — `{signal: undefined}` is empty — matching this file's
// `init.x != null` idiom, and a non-object init (misuse) is empty. Cheaper than `Object.keys`
// on the hot Request path: no array is materialized.
function requestInitHasMember(init) {
  if (!init || typeof init !== 'object') return false;
  for (const k in init) {
    if (Object.prototype.hasOwnProperty.call(init, k) && init[k] !== undefined) return true;
  }
  return false;
}
...
class Request {
  constructor(input, init, _internal) {
    ...
    // A NON-EMPTY init detaches the new request from a source Request's navigation context
    // (Fetch "Request(input, init)" step 28): mode 'navigate' → 'same-origin', origin + referrer
    // reset to "client", referrer policy cleared, and the URL list collapsed (dropping the redirect
    // chain's Sec-Fetch-Site seed / Origin taint). So `new Request(event.request, {mode})` — a SW's
    // change-request re-fetch — re-derives everything from the SW's own global scope, while
    // `fetch(event.request)` (empty init) / clone() preserve the navigation's request exactly. Only
    // a Request input has a context to reset (a string input starts fresh), so gate on that — which
    // also keeps the emptiness scan off the hot string-fetch / XHR path.
    const initIsNonEmpty = !isClone && isRequest(input) && requestInitHasMember(init);
    ...
    this._mode = init.mode || (isRequest(input) ? input.mode : null) || 'cors';
    if (initIsNonEmpty && this._mode === 'navigate') this._mode = 'same-origin';
    ...
    this._keepalive   = init.keepalive != null ? !!init.keepalive : !!(from && from.keepalive);
    ...
    } else if (initIsNonEmpty) {
      // Non-empty init resets the referrer to "client" — a SW's `new Request(event.request, init)`
      // re-fetch drops the navigation's referrer and re-derives it from the SW's own script URL.
      this._referrer = 'about:client';
    } else {
      this._referrer = from ? from._referrer : 'about:client';
    }
    this._referrerPolicy = init.referrerPolicy || (initIsNonEmpty ? '' : (from && from.referrerPolicy)) || '';
    // A navigation request re-issued by a service worker carries the navigating frame's origin
    // and the redirect chain's latched Sec-Fetch-Site seed / Origin taint (set on `event.request`
    // by the worker host). `fetch(event.request)` (empty init) PRESERVES them so the network hop
    // reports the frame's origin; `new Request(event.request, init)` (non-empty init) RESETS the
    // origin to "client" — this SW's own origin — and drops the redirect seeds, mirroring the
    // referrer reset above. A plain request has no initiator slot (rack_fetch derives the origin
    // from the document).
    if (from) {
      if (initIsNonEmpty) {
        // Reset the origin to "client" — this context's origin (the SW's own, for a change-request
        // re-fetch) — and drop the redirect seeds, but only when the source WAS a navigation (it
        // carried an initiator); a plain Request has nothing to reset.
        if (from._csimInitiator != null) {
          const here = globalThis.location && globalThis.location.origin;
          this._csimInitiator  = (here && here !== 'null') ? here : null;
          this._csimSiteSeed   = null;
          this._csimOriginNull = false;
        }
      } else {
        // Empty init / clone: preserve whatever navigation slots the source carried (all undefined
        // for a non-navigation request — harmless). Copied independently of the initiator so a
        // clone keeps the widened Sec-Fetch-Site seed / Origin taint even when the initiator origin
        // is opaque (an about:blank / srcdoc frame whose origin serializes to nil).
        this._csimInitiator  = from._csimInitiator;
        this._csimSiteSeed   = from._csimSiteSeed;
        this._csimOriginNull = from._csimOriginNull;
      }
      // The ancestor-chain cookie verdict (RFC 6265bis site-for-cookies) is a property
      // of the FRAME the navigation commits into, not of who issues the request — so it
      // survives BOTH the clone and the `new Request(event.request, init)` rewrite that
      // resets the initiator above.
      this._csimCookieCrossSite = from._csimCookieCrossSite;
    }
    ...
    // A NON-EMPTY init unsets the reload / history flags (Fetch "Request" constructor step) — a
    // `new Request(event.request, init)` re-fetch is no longer that navigation.
    if (from) {
    ...
    if (isRequest(input) && init.body == null) {
  ...
  get keepalive()           { return this._keepalive || false; }
...
class FetchResponse {
  constructor(raw, url, signal) {
    ...
    // Response ctor when the body IS a stream). A streaming service-worker respondWith delivers
    // its body incrementally, so the client reassembles it as a live stream handed in via
    // `raw.bodyStream` (sw-client.js) — reads pull from it as chunks arrive.
    this._bodyStream   = (raw && raw.bodyStream) || null;
    this._bodyIsStream = !!(raw && raw.bodyStream);
    // Aborting the fetch cancels the response body stream (Fetch "abort a fetch"). For a streaming
    // service-worker body that cancel must be observable in the SW's source stream (readable-stream
    // abort) EVEN while the page is reading it — a locked stream, where the public `cancel()` would
    // throw. The stream exposes `__csimAbort` (sw-client.js) which routes the cancel to the SW and
    // errors the body with the abort reason; fall back to the public cancel if it's absent.
    if (raw && raw.bodyStream && this._signal && typeof this._signal.addEventListener === 'function') {
      const s = raw.bodyStream, sig = this._signal;
      sig.addEventListener('abort', () => {
        if (typeof s.__csimAbort === 'function') s.__csimAbort(sig.reason);
        else try { s.cancel(sig.reason); } catch (_) {}
      });
    }
  ...
  clone() {
    ...
    if (this._bodyStream) {
      ...
      this._bodyStream = a;
      c._bodyStream    = b;
      // Carry the GENUINE-stream flag onto the clone so a cloned streaming SW respondWith is still
      // delivered incrementally (workers.js streamDeliver), not silently buffered.
      c._bodyIsStream  = this._bodyIsStream;
    }
    return c;
  }
...
class Response extends FetchResponse {
  constructor(bodyOrRaw, initOrUrl, signal) {
    ...
    // (response-error-from-stream) and its disturbed/locked state gates re-reads. `_bodyIsStream`
    // marks it a GENUINE stream (not a lazily-exposed byte body) so a controlling service worker
    // delivers it incrementally (workers.js streamDeliver) rather than buffering it whole.
    if (isStream) { this._bodyStream = bodyOrRaw; this._bodyIsStream = true; }
  }
...
globalThis.fetch = function fetch(input, init) {
  ...
  // A non-aborted fetch disturbs the request body inside `__csimFetch` (which does a combined
  // check-and-disturb before its scheme dispatch); marking it here would make
  // `fetch(requestWithBody)` trip that guard. The abort branch above handles the aborted case
  // itself because it returns early, never reaching `__csimFetch`.
...
globalThis.__csimFetch = function (input, init, signal) {
  // Resource Timing: the entry's start, and who asked (`navigator.sendBeacon` fetches with
  // its initiator named — an internal member of its init).
  const timingStart = globalThis.performance.now();
  const timingInitiator = (init && init.__csimInitiator) || 'fetch';
  init = init || {};
  ...
  // Resolve a relative URL against THIS realm's own base — its `document.baseURI` (which honours
  // `<base href>`), falling back to `location` (workers have no document). The host `rack_fetch`
  // resolves a bare relative URL against the MAIN document, which is the wrong base for a worker
  // (its `fetch('x')` targets a sibling of the worker's OWN location) and for a subframe (the
  // FRAME's document base). Doing it here in JS matches how `new Request('rel')` and
  // `xhr.open(m,'rel')` already resolve, so all three agree even in the main window. The result
  // is absolute, so rack_fetch's re-resolution is a no-op; an opaque / unparseable base (an
  // about:blank / srcdoc frame with no inherited base) throws and leaves the relative URL for
  // rack_fetch. data:/blob: inputs are absolute, so the base is ignored and the payload preserved.
  ...
  // Performing a fetch disturbs the request's OWN body (Fetch reads it to the wire) — but only
  // when the input Request's own body is used (an `init.body` override replaces it). Do the
  // check-and-disturb HERE, synchronously and before any scheme branch (blob:/data: return
  // early below), so `request.bodyUsed` is true the instant fetch() returns and a later
  // `request.text()` / second `fetch(request)` rejects. Splitting it — mark in the caller,
  // check in the callee — is what made `fetch(requestWithBody)` trip its own "already used"
  // guard. A GENUINE stream body is drained (and disturbed) by the wire read below, so it's
  // only flagged used here — locking it now (markBodyConsumed) would fail that read; a byte
  // body goes through markBodyConsumed, which also locks any already-exposed `.body` stream so
  // bodyUsed and read-usability stay in sync. (The abort branch in the public `fetch` wrapper
  // disturbs the aborted case itself, returning before it reaches here.)
  if (isRequest(input) && init.body == null && input._body != null) {
    if (bodyUsedOf(input)) return Promise.reject(new TypeError('Failed to fetch: Request body is already used'));
    if (input._bodyIsStream) input._bodyUsed = true;
    else                     markBodyConsumed(input);
  }
  ...
  const mode  = init.mode || (isRequest(input) ? input.mode : null) || 'cors';
  ...
  const credentials = init.credentials || (isRequest(input) ? input.credentials : null) || 'same-origin';
  ...
  const redirect = init.redirect || (isRequest(input) ? input.redirect : null) || 'follow';
  ...
  const cache = init.cache || (isRequest(input) ? input.cache : null) || 'default';
  const referrerPolicy = init.referrerPolicy || (isRequest(input) ? input.referrerPolicy : '') || '';
  const keepalive = init.keepalive != null ? !!init.keepalive : (isRequest(input) ? input.keepalive : false);
  // Subresource-integrity metadata for the response body (validated at resolve).
  const integrityMeta = (init.integrity != null ? String(init.integrity) : (isRequest(input) ? input._integrity : '')) || '';
  ...
  const rawReferrer = init.referrer != null ? init.referrer
                    : (isRequest(input) ? input.referrer : 'about:client');
  let referrer;
  if (rawReferrer === '') referrer = '';
  // 'about:client' = the current settings object's URL: the document in a window, the SCRIPT URL
  // in a worker / service worker (so a SW's `new Request(event.request, init)` re-fetch reports
  // its own script as the referrer, not the top document rack_fetch's @current_url would assume).
  else if (rawReferrer === 'about:client') referrer = (globalThis.location && globalThis.location.href) || undefined;
  else { try { referrer = new URL(rawReferrer, globalThis.location.href).href; } catch (_) { referrer = undefined; } }
  // A SW re-issuing a navigation (`fetch(event.request)`) threads the navigating frame's origin +
  // the redirect chain's Sec-Fetch-Site seed / Origin taint to the network hop; a plain request
  // carries none. Absent values are `null`, NOT `undefined` — QuickJS marshals a JS `undefined`
  // host-fn argument to the truthy Ruby symbol `:undefined` (which would poison rack_fetch's
  // origin), whereas `null` round-trips to Ruby `nil`. NB: these read the INPUT request's slots
  // directly, so the `new Request(event.request, init)` reset (above) applies; the `fetch(event.
  // request, init)` shorthand — which per spec is `new Request(input, init)` then fetch, but here
  // calls this directly — does NOT yet re-derive them (a backlog unification, untested here).
  const navInitiator  = (isRequest(input) && input._csimInitiator)  || null;
  const navSiteSeed   = (isRequest(input) && input._csimSiteSeed)   || null;
  const navOriginNull = (isRequest(input) && input._csimOriginNull) ? true : null;
  const navCookieCross = (isRequest(input) && input._csimCookieCrossSite) ? true : null;
  // The navigation's destination ('document' / 'iframe'), for a SW's passthrough
  // `fetch(event.request)` re-fetch: rack_fetch reports it as Sec-Fetch-Dest and
  // gates the Lax cookie exception (top-level only) on it. Only meaningful when the
  // mode stayed 'navigate' — a copied Request's mode is 'same-origin' and rack_fetch
  // ignores this for it.
  const navDest = (isRequest(input) && input._destination) || null;
  ...
  if (typeof url === 'string' && url.startsWith('blob:')) {
    ...
    const snapshot = blobMethod !== 'GET'
      ? null
      : ((isRequest(input) && input._blobSnapshot) ? input._blobSnapshot : resolveBlobBytes(url));
  ...
  const runRack = function (bodyStr, b64) {
    ...
    if (keepalive && !(typeof globalThis.__csimSWControllerHandle === 'function' && (globalThis.__csimSWControllerHandle() | 0) > 0)) {
      return new Promise(function (resolve, reject) {
        ...
        let abs = url;
        try { abs = new URL(url, (globalThis.location && globalThis.location.href) || undefined).href; } catch (_) {}
        let id = -1;
        try {
          id = globalThis.__csim_keepaliveStart(normalizeMethod(method), abs, bodyStr, headers, redirect, mode, credentials, referrerPolicy, referrer, cache, String((globalThis.location && globalThis.location.href) || '')) | 0;
        } catch (_) { id = -1; }
        ...
        const poll = function () {
          ...
          if (r && r.pending) { globalThis.setTimeout(poll, 0); return; }
          if (typeof globalThis.__csimRecordResource === 'function') globalThis.__csimRecordResource({ name: url, initiatorType: timingInitiator, startTime: timingStart, resp: r });
          ...
          try {
            const kaResp = new Response(r, abs, signal);
            if (integrityMeta && (kaResp.type === 'opaque' || !sriValidate(kaResp._bodyBytesRaw(), integrityMeta))) {
    ...
    return new Promise(function (resolve, reject) {
      globalThis.setTimeout(function () {
        ...
        const finish = function (resp) {
          if (typeof globalThis.__csimRecordResource === 'function') globalThis.__csimRecordResource({ name: url, initiatorType: timingInitiator, startTime: timingStart, resp });
        ...
        const doNetwork = function () {
          ...
          if (b64) headers['X-Csim-Body-B64'] = '1';
          try { finish(globalThis.__rackFetch(normalizeMethod(method), url, bodyStr, headers, redirect, mode, credentials, referrerPolicy, referrer, cache, navInitiator, navSiteSeed, navOriginNull, String((globalThis.location && globalThis.location.href) || ''), navCookieCross, navDest)); }
          catch (e) { reject(e); }
        };
        ...
        if (mode === 'navigate' && !globalThis.__csim_isWorker) {
          let navUrl = url;
          try { navUrl = new URL(url, (globalThis.location && globalThis.location.href) || undefined).href; } catch (_) {}
          if (globalThis.__csimSWMayInterceptNavigation && globalThis.__csimSWMayInterceptNavigation(navUrl)) {
            ...
            const navBody = (bodyStr == null || bodyStr === '' || m === 'GET' || m === 'HEAD')
        ...
        if (ctrl) {
          ...
          if (mode === 'same-origin') {
            let tgt = null;
            try { const u = new URL(url, (globalThis.location && globalThis.location.href) || undefined); if (/^https?:$/.test(u.protocol)) tgt = u.origin; } catch (_) {}
            ...
            const here = (globalThis.location && globalThis.location.origin) || null;
          ...
          let swUrl = url;
          try { swUrl = new URL(url, (globalThis.location && globalThis.location.href) || undefined).href; } catch (_) {}
          ...
          const dispatchSwHop = function (hopUrl, hopMethod, hopHeaders, hopBody, hopB64, hops) {
            globalThis.__csimSWInterceptFetch(ctrl, normalizeMethod(hopMethod), hopUrl, hopHeaders, hopBody, hopB64, swReq, function (swResp) {
              ...
              if (swResp == null) {
                ...
                try {
                  // Mirrors doNetwork: the wire-private b64 marker, and this
                  // client's own document URL as the origin identity (a controlled
                  // iframe's, not the top window's).
                  if (hopB64) hopHeaders['X-Csim-Body-B64'] = '1';
                  const r = globalThis.__rackFetch(normalizeMethod(hopMethod), hopUrl, hopBody, hopHeaders, redirect, mode, credentials, referrerPolicy, referrer, cache, null, null, null, String((globalThis.location && globalThis.location.href) || ''));
              ...
              if (loc != null && t !== 'opaqueredirect') {
                ...
                if (redirect === 'manual') {
                  ...
                  finish({ status: 0, statusText: '', headers: {}, body_b64: '', body: '', body_null: true, url: hopUrl, type: 'opaqueredirect' });
                  return;
                }
                ...
                let m2 = normalizeMethod(hopMethod), h2 = hopHeaders, b2 = hopBody, bb2 = hopB64;
                ...
                dispatchSwHop(next, m2, h2, b2, bb2, hops + 1);
                return;
              }
          ...
          dispatchSwHop(swUrl, method, headers, bodyStr, b64, 0);
          return;
        }
        doNetwork();
      }, 0);
  ...
  const reusedRequestBody = isRequest(input) && init.body == null;
  const reqStream = reusedRequestBody
    ? (input._bodyIsStream ? input._bodyStream : null)
    : (body != null && isReadableStreamBody(body) ? body : null);
  if (reqStream) {
