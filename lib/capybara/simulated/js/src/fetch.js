...
globalThis.__csimFetch = function (input, init, signal) {
  ...
  // when the caller set neither. `Headers.forEach` lower-cases names, so a plain lookup
  // suffices. (Mirrors the XHR defaults in xhr.js.)
  if (headers['accept'] == null) headers['accept'] = '*/*';
  if (headers['accept-language'] == null) headers['accept-language'] = 'en-US,en;q=0.9';
  ...
  const runRack = function (bodyStr, b64) {
    ...
    return new Promise(function (resolve, reject) {
      globalThis.setTimeout(function () {
        if (signal && signal.aborted) { reject(signal.reason); return; }
        try {
          const resp = globalThis.__rackFetch(normalizeMethod(method), url, bodyStr, headers, redirect, mode, credentials, referrerPolicy, referrer, cache);
