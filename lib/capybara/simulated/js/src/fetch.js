...
class Request {
  constructor(input, init) {
    ...
    if (init.method != null) this.method = normalizeMethod(String(init.method));   // validated in validateRequestInit
    ...
    validateRequestCombination(this.mode, this.method, this.cache);
    ...
    this.referrer       = init.referrer != null ? init.referrer : (from ? from.referrer : 'about:client');
    ...
    this.signal      = globalThis.AbortSignal.any(sigSource ? [sigSource] : []);
    this.bodyUsed    = false;
...
class Response extends FetchResponse {
  ...
  static error()             { const r = new Response({status: 0, body: '', headers: {}, url: ''}, ''); r.type = 'error'; r.headers._guard = 'immutable'; return r; }
...
globalThis.fetch = function fetch(input, init) {
  ...
  if (signal && signal.aborted) {
    ...
    if (from && from._body != null && init.body == null) from.bodyUsed = true;
