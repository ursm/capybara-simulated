      ...
      globalThis.setTimeout(function () {
        ...
        if (ctrl) {
          ...
          const swReq = {
            ...
            cache,
            keepalive,
            // The request's EFFECTIVE referrer policy: a fetch that names none inherits the
            // document's policy, which — absent a `<meta name=referrer>` / `Referrer-Policy`
            // header (not modeled) — is the platform default. The SW's `event.request.referrerPolicy`
            // reports this resolved value, and the host resolves the referrer under the same policy.
            referrerPolicy: referrerPolicy || 'strict-origin-when-cross-origin',
            integrity:      integrityMeta,
            referrerSource: referrer === undefined ? ((globalThis.location && globalThis.location.href) || '') : referrer
          };
