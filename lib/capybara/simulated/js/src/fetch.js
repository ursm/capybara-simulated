...
globalThis.__csimFetch = function (input, init, signal) {
  ...
  // A Worker / ServiceWorker isolate shares the host `rack_fetch`, which resolves a relative
  // URL against the MAIN document — the wrong base for a worker (its `fetch('x')` must resolve
  // against the worker's OWN location, e.g. an SW script in `resources/` reaching a sibling).
  // The window/frame path leaves resolution to rack_fetch (its base is the document's).
  if (globalThis.__csim_isWorker && typeof url === 'string') {
    try { url = new URL(url, globalThis.location && globalThis.location.href).href; } catch (_) {}
  }
