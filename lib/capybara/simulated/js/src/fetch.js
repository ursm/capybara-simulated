  ...
  const runRack = function (bodyStr, b64) {
    return new Promise(function (resolve, reject) {
      globalThis.setTimeout(function () {
        ...
        // registration covering its TARGET url, NOT the initiating client's controller: route it to
        // that SW's `fetch` event as a navigation request carrying the form's method + body (the SW
        // reads a POST body via `event.request.text()`). A fall-through / uncontrolled target loads
        // from the network; a `respondWith(Response.error())` fails the navigation. Navigation
        // requests never use the subresource intercept below.
        if (mode === 'navigate') {
          let navUrl = url;
          ...
          if (globalThis.__csimSWMayInterceptNavigation && globalThis.__csimSWMayInterceptNavigation(navUrl)) {
            ...
            const swNav = globalThis.__csim_swNavigationFetch(navUrl, false, false, navReferrer, m, navBody, navCt);
            if (swNav) {
              if (swNav.networkError) { reject(new TypeError('Failed to fetch: ' + url)); return; }
              finish(swNav); return;
            }
          }
          doNetwork();
        ...
        if (ctrl) {
          ...
          globalThis.__csimSWInterceptFetch(ctrl, normalizeMethod(method), swUrl, headers, bodyStr, b64, swReq, function (swResp) {
            ...
            // redirect mode (non-manual ↛ opaqueredirect) forbids (fetch-response-taint). A SW's own
            // synthetic Response is 'basic'/'default', which every mode accepts.
            const t = swResp.type || 'default';
            if ((mode === 'same-origin' && t === 'cors') ||
                (mode !== 'no-cors'     && t === 'opaque') ||
                (redirect !== 'manual'  && t === 'opaqueredirect')) {
              reject(new TypeError('Failed to fetch: response type "' + t + '" is disallowed for request mode "' + mode + '"'));
              return;
            }
            finish(swResp);
          });
