...
export function installDomClassAliases({ Element, Document, Text }) {
  ...
  // key the `instanceof` matchers use. No per-element prototype reparenting, so
  // element creation (hot) and `Object.getPrototypeOf(el) === Element.prototype`
  // (apps that probe proto identity) are untouched. `this` with no `_localName`
