...
import { registerTagProto } from './dom-nodes.js';
...
function makeTagCtor(name, tag, Element) {
  ...
  // Each interface gets its OWN prototype chained to Element.prototype, so
  // tag-specific IDL members relocated onto it are own properties of this
  // interface (`'readOnly' in button` false; getOwnPropertyDescriptor works) and
  // don't leak to other elements. Elements of this interface's tag(s) get this
  // prototype at construction (TAG_PROTO_REGISTRY). Universal members stay on the
  // inherited Element.prototype, so `'download' in HTMLAnchorElement.prototype` and
  // the like still resolve.
...
export function installDomClassAliases({ Element, Document, Text }) {
  ...
  // (`'readOnly' in button` false; getOwnPropertyDescriptor(HTMLInputElement
  // .prototype,'readOnly') works) and absent from other elements. The accessor is
  // the same descriptor; only WHICH prototypes expose it changes.
  ...
  relocateMember('readOnly', ['input', 'textarea']);
  // <template>-specific members (declarative shadow DOM feature detection reads
  // HTMLTemplateElement.prototype.hasOwnProperty('shadowRootMode'), etc.). `content`
  // is shared with <meta> (the getter dispatches on tag), so it goes on both.
  relocateMember('content', ['template', 'meta']);
