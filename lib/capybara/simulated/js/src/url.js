...
function buildHref(u) {
  // `_search`/`_hash` already carry their leading `?`/`#` when non-empty. When
  // empty but the component is present (`?`/`#` with nothing after), href keeps
  // the bare delimiter even though the IDL getter reports ''.
  let search = u._search && u._search[0] !== '?' && u._search.length ? '?' + u._search : (u._search || '');
  let hash   = u._hash   && u._hash[0]   !== '#' && u._hash.length   ? '#' + u._hash   : (u._hash   || '');
  if (!search && u._hasQuery)    search = '?';
  if (!hash   && u._hasFragment) hash   = '#';
  // Opaque-path URLs (`javascript:`, `data:`, `mailto:`) have no `//` authority.
  if (u._opaque) {
    return u._protocol + u._pathname + search + hash;
  }
  // Non-special scheme with no authority (`foo:/path`, null host): no `//`
  // either. Guard a path that begins with `//` with a leading `/.` so it isn't
  // reparsed as an authority (matches whatwg-url's serializeURL; our `pathname`
  // is the bare serializePath without that guard).
  if (!u._hasAuthority) {
    const p = u._pathname.slice(0, 2) === '//' ? '/.' + u._pathname : u._pathname;
    return u._protocol + p + search + hash;
  }
  return u._protocol + '//' +
...
export class URL {
  constructor(input, base) {
    ...
    this._hasAuthority = u.hasAuthority;
    this._hasQuery     = u.hasQuery;
    this._hasFragment  = u.hasFragment;
  ...
  set href(v) {
    ...
    this._opaque = u.opaque; this._hasAuthority = u.hasAuthority;
    this._hasQuery = u.hasQuery; this._hasFragment = u.hasFragment;
    this.searchParams._reset(this._search);
  }
  ...
  get host()     { return this._host; }     set host(v)     { this._host = String(v); if (this._host !== '') this._hasAuthority = true; }
  ...
  set search(v)  { this._search = normSearch(v); this._hasQuery = this._search !== ''; this.searchParams._reset(this._search); }
  get hash()     { return this._hash; }    set hash(v)     { this._hash = normHash(v); this._hasFragment = this._hash !== ''; }
...
export class URLSearchParams {
  ...
  _writeBack() {
    ...
    this._url._search   = s ? '?' + s : '';
    this._url._hasQuery = s !== '';
  }
