...
// Fetch "forbidden request-header" (name lowercased): UA-controlled, so a request's
// Headers (guard "request") silently ignores a script set/append of these. Plus any
// `proxy-`/`sec-` prefix, and a method-override header carrying a forbidden method.
...
function isForbiddenRequestHeader(name, value) {
  ...
  if (METHOD_OVERRIDE_HEADERS.has(name)) {
    return String(value).split(',').some((t) => FORBIDDEN_METHODS.has(t.trim().toUpperCase()));
  }
  return false;
}
...
export class Headers {
  ...
  constructor(init, guard) {
    ...
    if (typeof init[Symbol.iterator] === 'function') {
      ...
      for (const k of Object.keys(init)) this.append(k, init[k]);   // record<ByteString, ByteString>
    }
  }
  ...
  delete(k)    { const key = validHeaderName(k); if (this._guardForbids(key, this._map.get(key) || '')) return; this._map.delete(key); }
  get(k)       { const v = this._map.get(validHeaderName(k)); return v == null ? null : v; }
  has(k)       { return this._map.has(validHeaderName(k)); }
  set(k, v)    { const key = validHeaderName(k), val = validHeaderValue(v); if (this._guardForbids(key, val)) return; this._map.set(key, val); }
  ...
  _fillRaw(obj) {
    if (obj) for (const k of Object.keys(obj)) this._map.set(String(k).toLowerCase(), String(obj[k]));
    return this;
  }
  ...
  // "Headers iterator" mandates (headers-basic live-mutation subtests).
  _sortedEntries() {
    return [...this._map.entries()].sort((a, b) => (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0));
  }
  forEach(fn, thisArg) {
    for (let i = 0; ; i++) {
