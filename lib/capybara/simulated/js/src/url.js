...
export class URLSearchParams {
  ...
  constructor(init, _url) {
    ...
    if (typeof init === 'string') {
      ...
      // A sequence<sequence<USVString>> — each item is a [name, value] pair.
      // MUST come before the `forEach` branch: an Array also has `forEach`, but
      // `Array.prototype.forEach(cb)` calls cb(element, index), so an array of
      // pairs routed there would store [String(index), String(pair)] — e.g.
      // `new URLSearchParams(Array.from(formData))` (Turbo's form-submit path)
      // would serialize to `?0=name,value&1=...` and corrupt the query.
      // Each pair must have exactly two elements (spec: a non-2 inner sequence
      // is a TypeError).
      for (const e of init) {
      ...
      // FormData / Map / another URLSearchParams: forEach(cb) yields cb(value, key).
