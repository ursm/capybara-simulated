...
export class Headers {
  ...
  delete(k)    { this._map.delete(validHeaderName(k)); }
  get(k)       { const v = this._map.get(validHeaderName(k)); return v == null ? null : v; }
  has(k)       { return this._map.has(validHeaderName(k)); }
  set(k, v)    { this._map.set(validHeaderName(k), validHeaderValue(v)); }
  // Populate from an already-final network header map (the fetch RESPONSE path):
  // lowercased names, values taken VERBATIM — NO script-side normalization, since
  // the value is what the HTTP stack delivered (e.g. the ", " it inserted between
  // duplicate headers, which trailing-whitespace stripping would corrupt —
  // header-value-combining). Returns `this`.
  _fillRaw(obj) {
