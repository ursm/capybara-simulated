...
export class FormData {
  constructor(form, submitter) {
    ...
    if (form && form._tag === 'form') {
      const spec = globalThis.__csimFormSerialize(form._id, 0, false);   // fields only; skip the action URL parse
      ...
      // HTML "construct the entry list" step: a `_charset_`-named control's value is
      // the encoding. `new FormData(form)` is always UTF-8 (the form's accept-charset
      // only applies to actual submission). The hidden control has an empty value.
      for (const e of this._entries) {
        if (e[0] === '_charset_' && e[1] === '') e[1] = 'UTF-8';
      }
      // ...then fire the `formdata` event so listeners can mutate the entry list
