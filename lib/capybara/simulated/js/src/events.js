...
export class EventTarget {
  ...
  dispatchEvent(event) {
    const self = unwrapWin(this == null ? globalThis : this);
    // Public IDL method → the event is untrusted (DOM "dispatchEvent" sets
    // isTrusted to false). UA-internal firing uses dispatch.js helpers instead.
...
// Compile an inline event-handler source (`onclick="…"`, `<body onload="…">`) to a
// function. A syntax error → null (the handler simply never fires). Single home for
// the compile contract (arg name `event`, String coercion) shared by RawHandler and
// the window-forwarded path. `report` (lazy RawHandler path only) fires HTML "compile
// a handler"'s error report — the window `error` event — for a syntax error; the
// eager window-forwarded caller omits it to avoid reporting mid-parse.
function compileInlineHandler(src, report) {
  try { return new Function('event', String(src)); }
  catch (e) {
...
// An uncompiled inline-handler source, compiled LAZILY on first use (getter read or
// first dispatch) and memoized — a never-triggered inline handler is never compiled,
// matching the old lazy-compile perf, and a syntax error is reported exactly ONCE.
// A distinct type so an IDL-assigned plain object (`el.onclick = {}`, which
// round-trips but never fires) is never mistaken for one.
class RawHandler {
  constructor(src) { this.src = src; this.fn = undefined; }
  compile() {
    if (this.fn === undefined) this.fn = compileInlineHandler(this.src, true);
    return this.fn;
  }
}
...
// Whether a stored value gets a registered listener: a function, or a RawHandler
// (an inline source — registered WITHOUT compiling; a syntax error still registers
// a listener that no-ops on dispatch, matching a real browser). A non-callable
// object never fires, so it gets no listener. Decided without compiling, so a
// never-triggered inline handler is never parsed.
function isRegistrableHandler(v) {
  return typeof v === 'function' || v instanceof RawHandler;
}
...
// ShadowRoot). `attrNames` is the authoritative superset (`onabort` …
// `onwebkittransitionend`); each becomes an accessor that registers a listener,
// and its lowercase content-attribute name is recorded so `syncInlineEventHandler`
// recognises it. The accessors are enumerable to match the plain `null` slots they
// replace (so React-DOM's `'oninput' in element` probe still answers true).
const INLINE_HANDLER_TYPE = Object.create(null);   // 'onclick' → 'click', recognised handlers only
export function installEventHandlerAttrs(proto, attrNames) {
...
export function syncInlineEventHandler(node, attrName, raw, fromClone) {
  ...
  setHandlerSlot(node, type, '_eh_' + type, '_ehw_' + type, raw == null ? null : new RawHandler(raw));
}
...
// A compile error → null.
export function activateWindowForwardedHandler(name, code) {
  winStore[name] = code == null ? null : compileInlineHandler(code);
}
