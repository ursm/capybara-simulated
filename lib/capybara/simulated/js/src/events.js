...
// the window-forwarded path.
function compileInlineHandler(src) {
...
// matching the old lazy-compile perf. A distinct type so an IDL-assigned plain object
// (`el.onclick = {}`, which round-trips but never fires) is never mistaken for one.
class RawHandler {
  constructor(src) { this.src = src; this.fn = undefined; }
  compile() {
    if (this.fn === undefined) this.fn = compileInlineHandler(this.src);
    return this.fn;
  }
}
