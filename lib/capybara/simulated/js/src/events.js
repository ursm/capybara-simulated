...
export class EventTarget {
  ...
  dispatchEvent(event) {
    ...
    // The Window fires its IDL on-handler (`onload` / `onpopstate` / legacy
    // 5-arg `onerror`) as a bubble-phase listener. Plain EventTargets fire their
    // `on<type>` via `dispatchWithOnHandler` AFTER dispatch instead, so gate this
    // to the global to avoid double-firing those.
