...
// The Window's event-handler IDL attribute for `name` (e.g. `window.onload`,
// which `<body>`/`document.body.onload` also reflect). Per HTML these attributes
// act as a bubble-phase listener, so the window-dispatch paths must invoke this
// alongside addEventListener-registered listeners — otherwise `window.onload` /
// `body.onload` never fire on a dispatched `load` event.
export function getWindowForwardedHandler(name) {
