  ...
  dispatchEvent(event) {
    ...
    if (list) {
      for (const entry of list.slice()) {
        ...
        try {
          if (entry.isObject) {
            // DOM "inner invoke": resolve `handleEvent` on every dispatch; a
            // non-callable value is a TypeError (per WebIDL "invoke a callback").
          ...
          // HTML "report the exception" — fire an `error` event on the LISTENER
          // object's realm global as that realm's error type; do NOT propagate to
          // the dispatchEvent() caller. (The DOM-node path mirrors this.)
