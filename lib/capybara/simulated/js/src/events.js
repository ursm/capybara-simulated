...
// event handler set" (blur/error/focus/load/resize/scroll) PLUS the Window event handlers body also
// exposes. Only the ones NOT already defined on the Window elsewhere are listed here (e.g. `onmessage`
// has its own path) — `storage` is such a Window event handler (`<body onstorage>` → `window.onstorage`).
const WINDOW_FORWARDED_HANDLERS = ['blur', 'error', 'focus', 'load', 'scroll', 'resize', 'storage'];
