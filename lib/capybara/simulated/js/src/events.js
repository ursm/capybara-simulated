...
export class Event {
  constructor(type, init) {
    // WebIDL: `type` is a REQUIRED argument — `new Event()` with none present is a TypeError
    // (Event-constructors). `new Event(undefined)` is NOT (undefined coerces to "undefined").
    if (arguments.length < 1) throw new globalThis.TypeError("Failed to construct 'Event': 1 argument required, but only 0 present.");
    init = init || {};
...
// WebIDL gives every interface prototype a `@@toStringTag` equal to the interface name, so
// `Object.prototype.toString.call(new MessageEvent(...))` is `[object MessageEvent]`. Our event
// classes are named after their interfaces, so one getter on Event.prototype — reading the live
// constructor name — covers Event and every subclass without a per-class property.
Object.defineProperty(Event.prototype, Symbol.toStringTag, {
  configurable: true,
  get() { return (this.constructor && this.constructor.name) || 'Event'; }
});
...
export class StorageEvent extends Event {
  ...
  // while the event is mid-dispatch is a no-op (initEvent's dispatch-flag guard).
...
export class TransitionEvent extends Event {
  constructor(type, init) {
    ...
    defineReadonly(this, {
      ...
      pseudoElement: idlString(init.pseudoElement, ''),
      transition:    init.transition === undefined ? null : init.transition   // css-transitions-2
    });
  }
}
...
defineIdlAttributes(TransitionEvent, ['propertyName', 'elapsedTime', 'pseudoElement', 'transition']);
...
export class EventTarget {
  addEventListener(type, handler, options) {
    ...
    // its conversion runs as part of argument coercion — BEFORE the null-callback
    // early return — so `{signal: null}` (or a non-signal) throws a TypeError even
    // when the callback is also null. A duck-type (`aborted` boolean + `addEventListener`)
    // accepts a cross-realm signal without an `instanceof` (see per-frame realms).
    // The AddEventListenerOptions members are read HERE — as part of WebIDL argument
    // coercion, BEFORE the null-callback early return — so feature-detection probes
    // that put a getter on `passive` (or pass `signal`) see it read even when the
    // callback is null (AddEventListenerOptions-passive / -signal).
    const optObj = options && typeof options === 'object';
    const passive = (optObj && options.passive !== undefined) ? !!options.passive : defaultPassiveValue(type, self);
    const once    = !!(optObj && options.once);
    const signal  = optObj ? options.signal : undefined;
    if (signal !== undefined &&
    ...
    // AddEventListenerOptions.signal (DOM "add an event listener"): a listener whose
    // AbortSignal is ALREADY aborted is never added; otherwise, aborting the signal
    // removes the listener. `abort` fires once, so a `{once:true}` abort listener
    // suffices (a listener removed earlier makes the removal a no-op).
    ...
    if (signal !== undefined) {
      signal.addEventListener('abort', () => self.removeEventListener(type, handler, { capture }), { once: true });
    }
  }
  ...
  dispatchEvent(event) {
    ...
    event.eventPhase = 2;   // AT_TARGET — a plain EventTarget (or the Window) is always the target of its own flat dispatch
    ...
    event.eventPhase = 0;   // NONE (post-dispatch)
...
// `<body>`/`<frameset>` forward these handlers to the Window: the "window-reflecting body element
// event handler set" (blur/error/focus/load/resize/scroll) PLUS the `WindowEventHandlers` mixin body
// also exposes. `storage` / `messageerror` are such Window event handlers (`<body onstorage>` →
// `window.onstorage`; `<body onmessageerror>` → `window.onmessageerror`). `onmessage` is omitted
// deliberately: `window.onmessage` already works via the generic `on<type>` read in
// fireWindowOnHandler, and no surface needs `<body>` to reflect it.
const WINDOW_FORWARDED_HANDLERS = ['blur', 'error', 'focus', 'load', 'scroll', 'resize', 'storage', 'messageerror'];
