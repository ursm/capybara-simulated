...
// Make `on<type>` a spec-faithful EventHandler IDL attribute on `proto`: assigning a
// function REGISTERS a real event listener, so the handler fires INTERLEAVED with
// addEventListener listeners in registration order (not always last, as the plain-field
// fallback in dispatchEvent does). A function→null assignment removes that listener; a
// function→function replacement keeps the SAME listener position (the wrapper reads the
// current handler live). Used where handler-vs-listener ordering is observable — MessagePort
// / BroadcastChannel `onmessage`. An instance that has ever activated a handler carries a
// non-enumerable `_onwrap_<type>`, which dispatchEvent checks to avoid double-firing.
...
export function defineEventHandler(proto, type) {
  ...
  Object.defineProperty(proto, 'on' + type, {
    ...
    set(v) {
      ...
      const had    = typeof this[slot] === 'function';   // was the ACTIVE (callable) handler set?
      // Keep the handler backing off the enumerable surface (Object.keys / for-in / JSON):
      // a WebIDL on-handler is a prototype ACCESSOR, not an own data property.
      if (Object.prototype.hasOwnProperty.call(this, slot)) this[slot] = stored;
      else Object.defineProperty(this, slot, { value: stored, writable: true, enumerable: false, configurable: true });
      const cb = typeof stored === 'function' ? stored : null;   // only a callable registers a listener
      if (cb && !had) {
        let wrap = this[wrapKey];
        if (!wrap) {
          wrap = (ev) => { const h = this[slot]; if (typeof h === 'function') return h.call(this, ev); };
          Object.defineProperty(this, wrapKey, { value: wrap, writable: true, enumerable: false, configurable: true });
        }
        this.addEventListener(type, wrap);            // registration-order position
      } else if (!cb && had && this[wrapKey]) {
        this.removeEventListener(type, this[wrapKey]);  // a later re-set re-adds at the new end
      }
    }
  });
}
