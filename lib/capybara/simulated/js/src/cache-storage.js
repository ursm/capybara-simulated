...
// `instanceof` is realm-bound: a Request/Response built in an IFRAME realm (the
// redirected-response tests fetch via frame.contentWindow.fetch, then cache in
// the top page) carries the same shape — accept by duck-brand.
function isRequestLike(v) {
  return v instanceof globalThis.Request ||
    (v != null && typeof v === 'object' && typeof v.url === 'string' && typeof v.method === 'string' && v.headers != null && typeof v.clone === 'function');
}
function isResponseLike(v) {
  return v instanceof globalThis.Response ||
    (v != null && typeof v === 'object' && typeof v.status === 'number' && typeof v.type === 'string' && v.headers != null && typeof v.clone === 'function' && typeof v.url === 'string' && v.method === undefined);
}
function toRequest(request) {
  return isRequestLike(request) ? request : new globalThis.Request(request);
}
...
function requestFromMeta(e) {
  const r = new globalThis.Request(e.url, {method: e.method, headers: e.headers || {}});
  if (e.mode === 'navigate') r._mode = 'navigate';
  if (e.destination)         r._destination = e.destination;
  if (e.isReloadNavigation)  r._isReloadNavigation = true;
  if (e.isHistoryNavigation) r._isHistoryNavigation = true;
  return r;
}
...
class Cache {
  ...
  keys(request, options) {
    ...
    return new Promise((resolve, reject) => {
      try {
        resolve((this._select(request, options) || []).map(requestFromMeta));
      } catch (e) { reject(e); }
    });
  }
