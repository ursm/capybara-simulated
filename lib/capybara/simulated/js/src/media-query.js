...
import { splitTopLevel, cssPropertyName, isSupportedCssPropertyName } from './css-utils.js';
...
function evalSupportsFeature(text) {
  ...
  // A VENDOR-prefixed name is not a guess. Which `-webkit-…` spellings exist is a measured table
  // (css-utils), and Chrome answers false for every `-moz-` / `-ms-` / `-o-` one, so answer
  // honestly — the bias exists for standard properties mdn's table lags behind, and stripping the
  // prefix instead applied iOS-only blocks (`@supports (-webkit-touch-callout: none)`) and dropped
  // the `not (…)` fallbacks a real Chrome keeps.
  // A custom property is always supported — it needs no table and must not be read as a prefix.
  if (prop.indexOf('--') === 0) return true;
  if (prop.charCodeAt(0) === 45) return isSupportedCssPropertyName(cssPropertyName(prop));
  if (!/^[-a-z]+$/.test(prop)) return false;
  // …and where the VALUE's grammar is one we hold a measured answer for, give it: `@supports
  // (line-height: -1)` must not apply, and its `not (…)` fallback must. The bias toward "supported"
  // stays for everything the table says nothing about — that is what keeps a progressive-enhancement
  // block alive when mdn lags Chrome.
  return declarationIsValid(cssPropertyName(prop), value);
}
