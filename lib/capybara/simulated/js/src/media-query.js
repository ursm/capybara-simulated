...
// what the cascade applies. Listeners are stored and notified when
// `Browser#set_viewport` calls `__csimViewportChanged` after a viewport
// flip, so responsive component libs see the transition.
  ...
  const mql = {
    ...
    dispatchEvent(event) {
      ...
      if (typeof this.onchange === 'function') {
        try { this.onchange.call(this, evt); } catch (_) {}
      }
      return true;
    }
  };
