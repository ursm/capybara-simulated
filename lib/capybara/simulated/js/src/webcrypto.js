...
function aesGenerateKey(name, params, extractable, usages) {
  // Spec "generate key" order: an illegal usage entry (normalizeUsages → SyntaxError) is checked
  // first, THEN a bad length (OperationError), and empty usages is the LAST step — a bad length
  // with empty usages must surface the OperationError, not the empty-usages SyntaxError.
  const use = normalizeUsages(usages, aesAllowedUsages(name));
...
// A valid RSA public exponent is an odd integer >= 3 (big-endian bytes). Browsers reject 1
// and even values (e.g. 65536) with an OperationError; 3 and 65537 are the common valid ones.
function rsaPublicExponentValid(bytes) {
  if (bytes.length === 0 || (bytes[bytes.length - 1] & 1) === 0) return false;   // empty or even
  for (let i = 0; i < bytes.length - 1; i++) if (bytes[i] !== 0) return true;    // > 1 byte of magnitude
  return bytes[bytes.length - 1] >= 3;                                           // single byte: reject 1
}
...
function rsaGenerateKey(name, kind, params, extractable, usages) {
  ...
  const exponent = Array.from(copyBytes(params.publicExponent));
  // "Generate the key" (OperationError on a bad exponent) precedes the empty-usages SyntaxError.
  if (!rsaPublicExponentValid(exponent)) throw domError('RSA publicExponent must be an odd integer >= 3', 'OperationError');
  if (privUse.length === 0) throw domError('RSA key generation requires a private-key usage', 'SyntaxError');
  const privDer = hostCall('__csim_rsaGenerate', params.modulusLength, exponent);
...
function ecImportKey(name, kind, format, keyData, params, extractable, usages) {
  ...
  if (format === 'spki') {
    ...
    checkJwkExtractable(jwk, extractable);
    // 'crv' is a mandatory EC JWK member (RFC 7518) and must match the requested curve.
    if (jwk.crv === undefined) throw domError("Missing JWK 'crv' parameter", 'DataError');
    if (jwk.crv !== curve) {
      throw domError(`JWK curve "${jwk.crv}" does not match "${curve}"`, 'DataError');
    }
    // A present 'use' must match the key's role: 'sig' for ECDSA, 'enc' for ECDH key agreement.
    if (jwk.use !== undefined && usages.length > 0 && jwk.use !== (name === 'ECDH' ? 'enc' : 'sig')) {
      throw domError("Invalid EC JWK 'use' field", 'DataError');
    }
    const b = (s) => base64UrlToBytes(s);
    if (jwk.d !== undefined) {
      type = 'private';
...
function kdfAlgorithm(name, hostFn, extraParams) {
  return {
    ...
    importKey(format, keyData, params, extractable, usages) {
      ...
      // A secret key imported with no usages can never be used — SyntaxError (spec importKey step).
      if (use.length === 0) throw domError(`${name} key import requires at least one usage`, 'SyntaxError');
...
function okpImportKey(name, format, keyData, extractable, usages) {
  const curve = name.toUpperCase();
  let der, type;
  // Wrong-length / malformed key material must reject with DataError. The host parses
  // every format (a truncated SPKI/PKCS#8 DER, a raw key that isn't 32 bytes, or a JWK
  // whose x/d decodes to the wrong length all make OpenSSL raise) — surface any such
  // failure as DataError, while preserving DOMExceptions we raise deliberately below.
  try {
    if (format === 'spki') {
      type = 'public';
      der = hostCall('__csim_okpImportDer', 'public', copyBytes(keyData));
    } else if (format === 'pkcs8') {
      type = 'private';
      der = hostCall('__csim_okpImportDer', 'private', copyBytes(keyData));
    } else if (format === 'raw') {
      type = 'public';
      der = hostCall('__csim_okpImportRaw', curve, copyBytes(keyData));
    } else if (format === 'jwk') {
      const jwk = keyData;
      if (!jwk || jwk.kty !== 'OKP' || jwk.crv !== name || typeof jwk.x !== 'string') throw domError('Invalid OKP JWK', 'DataError');
      checkJwkExtractable(jwk, extractable);
      // A signing OKP JWK's alg, if present, is the algorithm name ('Ed25519') or 'EdDSA' (RFC 8037) —
      // wrong casing ('ed25519' / 'ED25519') is rejected. X25519 ignores the alg field.
      if (name === 'Ed25519' && jwk.alg !== undefined && jwk.alg !== 'Ed25519' && jwk.alg !== 'EdDSA') {
        throw domError("Invalid OKP JWK 'alg' field", 'DataError');
      }
      // A present 'use' must match the key's role: 'sig' for Ed25519, 'enc' for X25519 key agreement.
      if (jwk.use !== undefined && usages.length > 0 && jwk.use !== (name === 'Ed25519' ? 'sig' : 'enc')) {
        throw domError("Invalid OKP JWK 'use' field", 'DataError');
      }
      const b = (s) => base64UrlToBytes(s);
      type = jwk.d !== undefined ? 'private' : 'public';
      der = hostCall('__csim_okpImportJwk', curve, type === 'private', b(jwk.x), type === 'private' ? b(jwk.d) : []);
    } else {
      throw domError(`Unsupported key format "${format}" for ${name}`, 'NotSupportedError');
    }
  } catch (e) {
    if (e instanceof globalThis.DOMException) throw e;
    throw domError('Invalid OKP key data', 'DataError');
  }
