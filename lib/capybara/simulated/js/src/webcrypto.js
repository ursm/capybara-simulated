...
function ecImportKey(name, kind, format, keyData, params, extractable, usages) {
  ...
  if (format === 'spki') {
    ...
    der = hostEc('__csim_ecImportRaw', c.ossl, copyBytes(keyData));
    type = 'public';
  } else if (format === 'jwk') {
    const jwk = keyData;
    ...
    if (jwk.d !== undefined) {
      type = 'private';
      der = hostEc('__csim_ecImportJwk', c.ossl, true, b(jwk.x), b(jwk.y), b(jwk.d), c.bytes);
    } else {
      type = 'public';
      der = hostEc('__csim_ecImportJwk', c.ossl, false, b(jwk.x), b(jwk.y));
    }
  } else {
    throw domError(`Unsupported key format "${format}" for ${name}`, 'NotSupportedError');
  }
  ...
  let detected;
  try {
    detected = EC_OSSL_TO_NAME[hostEc('__csim_ecKeyInfo', der).curve];
  } catch (e) {
...
function ecExportKey(name, format, key) {
  ...
  if (format === 'spki') return bytesToArrayBuffer(hostEc('__csim_ecExport', der, 'spki'));
  if (format === 'pkcs8') return bytesToArrayBuffer(hostEc('__csim_ecExport', der, 'pkcs8'));
  if (format === 'raw') return bytesToArrayBuffer(hostEc('__csim_ecExport', der, 'raw'));
  if (format === 'jwk') {
    const j = hostEc('__csim_ecExportJwk', der, c ? c.bytes : 0);
    const jwk = {
      kty: 'EC',
...
function ecGenerateKey(name, kind, params, extractable, usages) {
  ...
  const privDer = hostEc('__csim_ecGenerate', c.ossl);
  const pubDer = hostEc('__csim_ecExport', privDer, 'spki');
...
function ecAlgorithm(name, kind) {
  ...
  if (kind === 'ecdsa') {
    ...
    base.sign = (params, key, dataBytes) => {
      ...
      return bytesToArrayBuffer(hostEc('__csim_ecdsaSign', keyMaterial(key), hash, dataBytes, c.bytes));
    };
    base.verify = (params, key, sigBytes, dataBytes) => {
      const c = EC_CURVES[key.algorithm.namedCurve];
      const hash = params.hash.name.toUpperCase().replace(/-/g, '');
      return hostEc('__csim_ecdsaVerify', keyMaterial(key), hash, dataBytes, sigBytes, c.bytes);
    };
  }
  return base;
}
