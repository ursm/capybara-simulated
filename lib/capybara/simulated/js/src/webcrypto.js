...
function kdfAlgorithm(name, hostFn, extraParams) {
  return {
    ...
    deriveBits(params, key, length) {
      ...
      const hash = params.hash.name.toUpperCase().replace(/-/g, '');
      return hostFn === '__csim_hkdf'
        ? hostCall(hostFn, hash, keyMaterial(key), copyBytes(params.salt), copyBytes(params.info), nbytes)
        : hostCall(hostFn, hash, keyMaterial(key), copyBytes(params.salt), params.iterations | 0, nbytes);
    },
  };
}
