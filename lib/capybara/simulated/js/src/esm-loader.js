...
// transforms each module body into a function whose `import`
// statements become `__csim_require(url)` calls and whose `export`
// statements become `__csim_defineExport(...)` assignments. Cached
// source comes back from Ruby via `__csim_fetchModuleSource(url)`.
//
// `ingestImportmaps(doc)` is the bridge-internal entry point —
// `__csimLoadDocument` calls it after each visit to merge any
// `<script type="importmap">` tags into `__csim_importmap.imports`.

import { NODE_TEXT } from './constants.js';
