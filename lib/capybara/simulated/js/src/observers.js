...
// Run the update steps for every live observer. Called wherever the geometry can have changed: the
// rendering update, a mutation batch, and a scroll. The first two fire many times per settle on an
// app-scale page, so a pass whose geometry generation hasn't moved returns immediately (rule 3) —
// a scroll passes `force`, since it moves boxes without bumping either counter.
let lastGeneration = null;
export function updateIntersectionObservations(force = false) {
  if (activeIOs.size === 0) return;
  const gen = layoutGeneration();
  if (!force && gen === lastGeneration) return;
  lastGeneration = gen;
