...
class PerformanceObserverImpl {
  ...
  observe(opts) {
    ...
    if (opts && Array.isArray(opts.entryTypes)) {
      if (opts.type !== undefined || opts.buffered !== undefined) throw new TypeError("Failed to execute 'observe' on 'PerformanceObserver': An observe() call must not include both entryTypes and type arguments.");
  ...
  // One callback per turn for everything queued (the spec's queued "PerformanceObserver task").
  _schedule() {
