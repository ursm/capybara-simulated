...
// `mutation-observer.js` and `bridge.entry.js`'s script-runner path
// poke this hook to re-evaluate pending IOs after a mutation batch
// (Avo's tab-class flip from `block hidden` to `block` must trigger
// the lazy turbo-frame inside the now-revealed panel). Without
// exposure here the `typeof === 'function'` gate fell to false and
// the recheck only fired on timer drains — under macrotask-paced
// Promise (core-js polyfill) the chain happened to land in a timer
// boundary; under native Promise it doesn't, so the recheck skipped.
