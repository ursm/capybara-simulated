...
function decodeAndDispatch(video, src) {
  ...
  if (/^https?:/i.test(abs) && typeof globalThis.__csimSwFetchDest === 'function') {
    const sw = globalThis.__csimSwFetchDest(abs, 'video', 'no-cors', 'include', true);
    if (sw && sw.blocked) { queueMicrotask(() => dispatchVideoEvent(video, 'error')); return; }
    if (sw && sw.body_b64 != null && sw.body_b64 !== '') b64 = sw.body_b64;
  }
  if (b64 == null) b64 = videoBytesB64(/^https?:/i.test(abs) ? abs : src);
  if (!b64) {
