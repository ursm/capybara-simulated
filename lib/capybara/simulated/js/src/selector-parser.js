...
// Walks `root`'s element descendants (depth-first) until `seq`
// matches one. Used by `:has()` — scope-relative descendant search.
// `__scopeRoot` is already pointing at `root` so seqs starting with
// `>` / `+` / `~` work relative to the host.
...
function matchPseudo(el, p) {
  switch (p.name) {
    ...
    case 'has': {
      // Selectors L4 :has(selector) — matches when the relative
      // selector matches *anything* among the element's descendants
      // (or, for explicit combinators starting with `>`/`+`/`~`,
      // the appropriate kin set). Modern Tailwind uses :has() to
      // style based on contained matches; without support those
      // rules fall through to default styling and visibility tests
      // can flip the wrong way.
      const seqs = p.list;
      const prev = __scopeRoot;
      __scopeRoot = el;
      try {
        // Walk descendants and check each as candidate. We treat the
        // element itself as the scope root; subselector matching
        // re-uses matchComplex on the candidate.
