...
import { walk, walkFind, classes }           from './walk.js';
...
export function findFirst(root, group) {
  const prev = __scopeRoot; __scopeRoot = root;
  // `walkFind` short-circuits at the first tree-order match.
  try { return walkFind(root, el => matchOne(el, group)); }
  finally { __scopeRoot = prev; }
}
...
export function findById(root, id) {
  ...
  // `walkFind` stops at the first match — `getElementById` returns the
  // first element in tree order, so there's no reason to walk past it.
