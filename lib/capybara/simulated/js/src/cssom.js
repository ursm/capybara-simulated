...
class CSSKeyframesRule extends CSSRule {
  constructor(node, sheet, parent) {
    ...
    if (node && node.block && node.block.children) {
      node.block.children.forEach(frame => {
        if (frame.type === 'Rule') this._rules.push(new CSSKeyframeRule(frame, sheet, this));
      });
    }
  }
  ...
  get cssRules()  { return this._rules; }
  _serialize() {
    const inner = this._rules.map(r => r.cssText).join(' ');
    return '@keyframes ' + this._name + ' {' + (inner ? ' ' + inner + ' ' : ' ') + '}';
  }
}
...
class MediaList {
  constructor(text) { this._items = parseMediaText(text); }
  ...
  appendMedium(m)  { const s = String(m).trim(); if (s && !this._items.includes(s)) this._items.push(s); }
  deleteMedium(m)  {
    const s = String(m).trim();
    const before = this._items.length;
    this._items = this._items.filter(x => x !== s);
    if (this._items.length === before) throw new DOMException(`'${s}' not found`, 'NotFoundError');
  }
...
class CSSStyleSheet {
  ...
  _syncText() { this._cssText = this._rules.map(r => r.cssText).join(' '); }
