...
import { parseStyleDecls, serializeStyleDecls, splitTopLevel } from './css-utils.js';
...
function serializeSelector(preludeNode) {
  ...
  try { return normalizeSelectorText(CT.generate(preludeNode)); } catch (_) { return ''; }
}
...
// through the shared parse+serialize dedups repeated properties (last value wins,
// first position kept — the CSSOM declaration semantics) so `_declText` stays
// canonical and rule `cssText` can wrap it directly without re-parsing per read.
function blockDeclText(blockNode) {
  ...
  return serializeStyleDecls(parseStyleDecls(parts.join(' ')));
}
...
class CSSStyleRule extends CSSRule {
  ...
  set selectorText(v) {
    ...
    try {
      ...
      this._selectorText = normalizeSelectorText(CT.generate(p));
...
class CSSStyleSheet {
  ...
  // A constructed sheet may be adopted into the document or a shadow root; a CSSOM
  // edit to it changes the resolved cascade, so signal a mutation (coalesced; a no-op
  // for a sheet not adopted anywhere). Owned `<style>`/`<link>` sheets are NOT covered
  // here: their `<style>` TEXT feeds the cascade (a text change refreshes via the DOM
  // mutation path), but a CSSOM `insertRule` on an owned sheet leaves the text
  // unchanged and does not re-apply — a known limitation, not handled by this hook.
  _notifyCascade() {
    if (this._constructed && globalThis.__csimScheduleCascadeRefresh) globalThis.__csimScheduleCascadeRefresh();
  }
