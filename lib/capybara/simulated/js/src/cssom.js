...
import { parseStyleDeclList, splitTopLevel, decodeDataUrlCss, CSS_PROPERTY_BY_IDL_ATTRIBUTE } from './css-utils.js';
import { serializeDeclBlock, expandDeclList } from './shorthands.js';
import { PSEUDO_NAMES } from './css-property-data.js';
...
// Canonicalize a parsed selector AST in place for CSSOM serialization, in ONE walk (rule 3:
// one traversal per selector on rule construction). Two things are normalized:
//
// (a) `:nth-*(An+B)` arguments → the canonical form (`even` → `2n`, `odd` → `2n+1`, `1n` → `n`,
//     signed constant), independent of how they were authored.
//
// (b) type / universal selector namespaces. css-tree keeps the prefix verbatim in `name`
//     (`*|e`, `|e`, `ns|e`), which is already canonical except:
//       1. a named prefix that maps to the DEFAULT namespace's URL is dropped (`nsdefault|e`
//          → `e` when @namespace nsdefault shares the default @namespace's URL);
//       2. an any-namespace prefix `*|` is dropped when the sheet declares NO default
//          namespace — there `*|e` matches the same elements as the unprefixed `e`; with a
//          default they differ (`e` = default ns, `*|e` = any ns), so `*|` is kept;
//       3. an unprefixed universal `*` is omitted when another simple selector shares its
//          compound (`*.c` → `.c`, `*::before` → `::before`); a lone `*`, or a namespaced
//          `|*` / `ns|*` / (with a default namespace) `*|*`, is kept.
//     A `|`-less name (`e`, `*`) has no prefix. Emptying `name` makes css-tree generate nothing.
// The four CSS Level 2 pseudo-elements that accept the legacy single-colon syntax on input
// but serialize double-colon (Selectors "grammar" + CSSOM "serialize a group of selectors").
const LEGACY_PSEUDO_ELEMENTS = new Set(['before', 'after', 'first-line', 'first-letter']);

// `:is()` / `:where()` take a FORGIVING selector list — an unknown pseudo inside them is
// dropped, not fatal — so validation must not descend into them. (`:not()`/`:has()` are NOT
// forgiving, so an unknown pseudo there still invalidates the whole selector.)
const FORGIVING_PSEUDOS = new Set(['is', 'where']);

// css-tree parses pseudo names leniently (any identifier), but a selector naming an UNKNOWN
// pseudo-class / pseudo-element (`:gibberish`, `::gibberish`) is invalid — the `selectorText`
// setter must reject it. Vendor-prefixed pseudos (`:-webkit-…`) are accepted (forward-compat),
// and pseudos inside a forgiving `:is()`/`:where()` are not validated.
function selectorHasUnknownPseudo(ast) {
  let bad = false, forgiving = 0;
  const isForgiving = (n) => n.type === 'PseudoClassSelector' && FORGIVING_PSEUDOS.has(n.name && n.name.toLowerCase());
  CT.walk(ast, {
    enter(node) {
      if (bad) return;
      if (isForgiving(node)) { forgiving++; return; }
      if (forgiving > 0 || (node.type !== 'PseudoClassSelector' && node.type !== 'PseudoElementSelector')) return;
      const name = node.name && node.name.toLowerCase();
      if (name && name.charCodeAt(0) !== 45 /* '-' vendor prefix */ && !PSEUDO_NAMES.has(name)) bad = true;
    },
    leave(node) { if (isForgiving(node)) forgiving--; },
  });
  return bad;
}

function normalizeSelectorAst(preludeNode, ns) {
  CT.walk(preludeNode, function (node, item) {
    if (node.type === 'Nth') {
      const arg = node.nth;
      let text = null;
      ...
      if (text !== null) node.nth = { type: 'Raw', value: text };
      return;
    }
    // The four CSS 2.1 pseudo-elements are parsed as legacy single-colon pseudo-CLASSES
    // (`:after`), but serialize with the modern double-colon syntax (`::after`).
    if (node.type === 'PseudoClassSelector' && node.name && LEGACY_PSEUDO_ELEMENTS.has(node.name.toLowerCase())) {
      node.type = 'PseudoElementSelector';
      return;
    }
    // An attribute selector with the empty (null) namespace prefix — `[|attr]` — means "no
    // namespace" and serializes without the bar (`[attr]`). A named prefix is left as-is.
    if (node.type === 'AttributeSelector' && node.name && typeof node.name.name === 'string'
        && node.name.name.charCodeAt(0) === 124 /* '|' */) {
      node.name = { type: 'Identifier', loc: null, name: node.name.name.slice(1) };
      return;
    }
    if (node.type !== 'TypeSelector' || !node.name) return;
    const bar = node.name.indexOf('|');
    if (bar >= 0) {
      const prefix = node.name.slice(0, bar);
      const local  = node.name.slice(bar + 1);
      if (prefix === '*') {
        if (ns.default == null) node.name = local;                 // any-ns, no default → bare
      } else if (prefix !== '') {                                  // named prefix (not the null `|`)
        const uri = ns.prefixes.get(prefix);
        if (uri != null && ns.default != null && uri === ns.default) node.name = local;
      }
    }
    if (node.name === '*' && item && item.next && item.next.data.type !== 'Combinator') node.name = '';
  });
}

// The stylesheet's declared namespaces: `{ default: <uri|null>, prefixes: Map<prefix,uri> }`.
  ...
  if (sheet && sheet._rules) {
    for (const r of sheet._rules) {
      if (r._type === RULE_TYPE.NAMESPACE_RULE) {
        ...
        else prefixes.set(r._prefix, r._namespaceURI);
...
function serializeSelector(preludeNode, ns) {
  if (!preludeNode) return '';
  try {
    normalizeSelectorAst(preludeNode, ns || { default: null, prefixes: new Map() });
...
function blockDeclText(blockNode) {
  ...
  return serializeDeclBlock(expandDeclList(parseStyleDeclList(parts.join(' '))));
}
...
// What pays is going through the prototype for a member: `el.style.length` 41 → 68 ns,
// `getPropertyValue(x)` 226 → 279 — and the `-webkit-` MIRROR family the table omits, 513 → 621.
{
...
class CSSStyleRule extends CSSRule {
  constructor(node, parentStyleSheet, parentRule) {
    ...
    this._selectorText = serializeSelector(node.prelude, sheetNamespaces(parentStyleSheet));
  ...
  set selectorText(v) {
    ...
    try {
      ...
      if (selectorHasUnknownPseudo(p)) return;   // unknown pseudo → invalid, leave unchanged
      this._selectorText = serializeSelector(p, sheetNamespaces(this._parentStyleSheet));
...
class CSSMediaRule extends CSSConditionRule {
  ...
  _serialize()       { return wrapGroupingRule('@media ' + this._media.mediaText, this._rules.map(r => r.cssText)); }
}
...
class CSSKeyframesRule extends CSSRule {
  ...
  _serialize() { return wrapNested('@keyframes ' + serializeKeyframesName(this._name), this._rules.map(r => r.cssText).join(' ')); }
}
...
// Fetch the CSS text for a stylesheet URL: a `data:text/css` inline, else the external asset
// (the script/stylesheet asset cache). Returns null when the resource can't be loaded (a 404
// / unreachable URL) — distinct from '' (a reachable but empty sheet). Shared by the `<link>`
// / `<?xml-stylesheet?>` `.sheet` getters and the `@import` rule loader.
export function fetchStyleSheetText(url) {
  const dataCss = decodeDataUrlCss(url);   // `data:text/css[;base64],…` (null if not a data: CSS URL)
  if (dataCss != null) return dataCss;
  // A controlling service worker answers first (destination 'style'; memoized per
  // URL in bridge.entry so this, the cascade, and the link load event share ONE
  // fetch-event dispatch and one body).
  ...
  if (typeof globalThis.__csimExternalAsset === 'function') {
    try { return globalThis.__csimExternalAsset(url); } catch (_) { return null; }
  }
  return null;
}
