import { base64FromBytes } from './encoding.js';
...
//
// HOW the bytes cross depends on the engine, and the difference is not a nicety. V8 marshals a
// typed array to Ruby as one BINARY string. QuickJS hands it over as a Hash of `"index" => byte`
// (hmsk/quickjs.rb#89), so a 1024x768 screenshot — 3.1 million bytes — crosses as a
// 3.1-million-entry Hash. What ONE such screenshot costs a process, over a baseline that has
// visited a page but taken no picture (measured, peak RSS / wall):
//
//     V8              153 MB -> 209 MB   +56 MB   0.73 s -> 1.04 s
//     QuickJS, Hash   429 MB -> 961 MB  +532 MB   1.34 s -> 5.11 s
//     QuickJS, base64 429 MB -> 486 MB   +57 MB   1.34 s -> 1.62 s
//
// ~175x the picture's own 3 MB, and enough of them in parallel took a 16 GB CI runner down — which
// is how this was found. So on that engine the buffer crosses as BASE64 instead: ASCII, 4/3 the
// size, one string on each side, and the same marginal cost as V8's. `fetchedToBytes` already
// reads base64 coming the other way, for the same reason.
export function stashTransfer(view) {
  ...
  // bytes — and `base64FromBytes` needs a real view to read.
  if (!view || typeof view.length !== 'number') return 0;
  if (globalThis.RustyRacer) return globalThis.__csim_transferStash(view) | 0;
  return globalThis.__csim_transferStash(base64FromBytes(view), 'base64') | 0;
}
