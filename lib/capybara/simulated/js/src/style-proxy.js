...
const COMPUTED_INITIAL_OVERRIDES = Object.assign(Object.create(null), {
  ...
  'word-spacing': '0px',
  // mdn says `auto`; Chrome computes `0px` for a block box — but `auto` for a FLEX or GRID item,
  // which is resolved per element in `computedInitialValue`. The physical and flow-relative names
  // must agree either way, being the same declaration.
...
// A shadow length is reported as the USED value, so `1em` on a 16px element is `16px` — the
// colour-first form exists to be parsed, and a font-relative unit in it defeats that.
function shadowLengthPx(tok, el) {
  const t = String(tok).trim();
  if (/^-?[\d.]+$/.test(t)) return `${t}px`;
  const m = /^(-?[\d.]+)(px|em|rem|pt|pc|in|cm|mm|q)$/i.exec(t);
  if (!m) return t;                                            // `%` / viewport units need layout
  const n = parseFloat(m[1]), unit = m[2].toLowerCase();
  if (unit === 'px') return `${n}px`;
  const px = unit === 'em'  ? n * computedFontSizePx(el)
           : unit === 'rem' ? n * computedFontSizePx(el && el.ownerDocument ? el.ownerDocument.documentElement : null)
           : n * (CSS_ABSOLUTE_UNIT_PX[unit] || 1);
...
function serializeShadow(value, lengths, ownColor, el) {
  ...
  for (const layer of layers) {
    ...
    for (const tok of toks) {
      ...
      if (/^-?[\d.]+(px|em|rem|%|pt|pc|in|cm|mm|q|vh|vw|vmin|vmax)?$/i.test(tok)) { nums.push(shadowLengthPx(tok, el)); continue; }
...
function computedInitialValue(prop, el) {
  // A flex / grid ITEM's automatic minimum size stays `auto` (Chrome measured); a block box
  // computes `0px`.
  if (el && MIN_SIZE_PROPS.has(prop)) {
    const parent = el._parent;
    const pd = parent && parent.nodeType === NODE_ELEMENT ? computedDisplayFor(parent) : '';
    // Absolutely positioned children are OUT OF FLOW — not flex items — and compute `0px`
    // (Chrome measured). A float still is one, since the container blockifies it, so the guard is
    // on positioning rather than on floats.
    const pos = String(cascadedProperty(el, 'position') || 'static').trim().toLowerCase();
    if (/(^|-)(flex|grid)$/.test(String(pd)) && pos !== 'absolute' && pos !== 'fixed') return 'auto';
  }
...
function readComputed(el, key) {
  ...
  if (PX_REPORTABLE_LAYOUT_PROPS.has(key)) {
    ...
    // NOTHING declares it. A property whose initial is a LENGTH (or the keyword `none`) computes
  ...
  if (cascaded != null && cascaded !== '') {
    ...
    if (cw === 'inherit' || cw === 'unset' || cw === 'initial' || cw === 'revert' || cw === 'revert-layer') {
      ...
      if (isColorValued(key)) {
        ...
        else if (key === 'box-shadow')                    value = serializeShadow(value, 4, readComputed(el, 'color').value, el);
        else if (key === 'text-shadow')                   value = serializeShadow(value, 3, readComputed(el, 'color').value, el);
