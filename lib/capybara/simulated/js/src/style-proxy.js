...
import { parseStyleDecls, serializeCssValue, serializeFontFamily, SYSTEM_COLORS, ABSOLUTE_FONT_SIZE_PX, resolveCssUrls, documentBaseUrl } from './css-utils.js';
...
function readComputed(el, key) {
  ...
  if (cascaded != null && cascaded !== '') {
    ...
    if (cw === 'inherit' || cw === 'unset' || cw === 'initial' || cw === 'revert' || cw === 'revert-layer') {
      ...
      // document base is its correct base.
      const value = (key === 'color' || key.endsWith('-color')) ? normalizeColor(resolved)
                  : key === 'background-image' ? resolveCssUrls(resolved, documentBaseUrl())
                  : resolved;
