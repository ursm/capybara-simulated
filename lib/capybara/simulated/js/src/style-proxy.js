...
import { animatedValue, animatedProperties, transitionedValue, noteAnimatedElement } from './animation.js';
...
export function declaredValueEntry(el) {
  ...
  if (entry === undefined || entry.epoch !== epoch || entry.ctx !== ctx) {
    // The OUTGOING map is the element's before-change style: every value it reported at the last
    // style change event, which is exactly what a transition needs to know it has changed
    // (`computeDeclaredValue`). Carrying it costs one reference — the alternative, recording each
    // property as it is read, is a map write on every declared-value read in the driver.
...
export function declaredValueIn(entry, el, key) {
  ...
  let value, mine;
  try { value = computeDeclaredValue(el, key, entry.prev); }
...
function computeDeclaredValue(el, key, before) {
  ...
  if (!animations && !scripted && !transitions) return base;
  ...
  if (!transitions || RESOLVING_ENDPOINT) return base;
  // Both ends of a transition are COMPUTED values, and a property the cascade does not declare
  // still has one — its initial, or its inherited value. Leaving those as `null` meant a class that
  // ADDS a declaration had nothing to transition from, and one that REMOVES it nothing to
  // transition to: the everyday hover-on and hover-off.
  // The ends are handed over UNRESOLVED, and `transitionEndpoint` resolves them only where a
  // transition is actually starting. Resolving them here instead put the `unset` fallback — which
  // for an inherited property reads the PARENT's computed value — on every declared-value read of
  // every element on a page that declares any transition, and each of those reads did the same for
  // its own parent: quadratic up the tree, and a stack overflow on a deeply nested one.
  const from = beforeChangeValue(el, before, key, twin);
  const t = transitionedValue(el, key, twin, base, from, declaredValue, transitionEndpoint);
...
// What the property reported at the last style change event — under EITHER of its names, since a
// flow-relative twin is the same value: a page that set `padding-inline` and reads `padding-left`
// has still recorded the before-change value, under the name it happened to read.
//
// `undefined` when nothing read it then, which is the one place this model differs from a browser:
// a browser snapshots every property at every style change, where this remembers what was actually
// asked for. A property nobody read is one nobody can observe transitioning.
function beforeValue(before, key, twin) {
  if (!before) return undefined;
  const own = before.get(key);
  return own !== undefined || !twin ? own : before.get(twin);
}

// The value this property had at the last style change event — or `undefined` where there was
// none. A NOT-RENDERED element has no computed style for a transition to be measured against
// (css-transitions §3), so one that appears transitions from nothing: the epoch in which it is
// first read while rendered re-establishes the baseline, and only a change after THAT transitions.
function beforeChangeValue(el, before, key, twin) {
  if (!(globalThis.__isLaidOutNode && globalThis.__isLaidOutNode(el))) {
    el._csimNoBaseline = true;
    el._csimNoBaselineKey = null;
    return undefined;
  }
  if (el._csimNoBaseline) {
    const epoch = `${cascadeStyleEpoch()}:${ctxEpochOf(el)}`;
    if (el._csimNoBaselineKey == null) el._csimNoBaselineKey = epoch;
    if (el._csimNoBaselineKey === epoch) return undefined;
    el._csimNoBaseline = false;
  }
  return beforeValue(before, key, twin);
}
...
// tracks either, so they are equally uncacheable.
...
function computeAnimationEndpoint(el, key, declared, ownColorFn = () => readComputed(el, 'color').value) {
  ...
  if (type === 'shadowList') {
    return serializeShadow(out, SHADOW_LENGTHS[key], ownColorFn, el);
  }
...
function serializeShadow(value, lengths, ownColorFn, el) {
  const out = [];
...
const FILTER_PROPORTIONS = new Set(['grayscale', 'sepia', 'invert', 'opacity', 'brightness', 'contrast', 'saturate']);
function serializeFilter(value, ownColorFn, el) {
  ...
  for (const fn of splitValues(v, ' ')) {
    ...
    if (!m) return normalizeEmbeddedColors(value);
    const name = m[1].toLowerCase(), arg = m[2].trim();
    if (name === 'url') { out.push(fn); continue; }
    if (name === 'drop-shadow') {
      // A shadow it could serialize always leads with the colour; anything else means the
      // argument is not one we model.
      const shadow = arg && serializeShadow(arg, SHADOW_LENGTHS['drop-shadow'], ownColorFn, el);
      if (!shadow || !/^rgba?\(/.test(shadow)) return normalizeEmbeddedColors(value);
    ...
    if (name === 'blur') {
      ...
      if (px === null) return normalizeEmbeddedColors(value);
      out.push(`blur(${px})`);
      continue;
    }
    if (name === 'hue-rotate') {
      const deg = arg ? angleDeg(arg) : 0;
      if (deg === null) return normalizeEmbeddedColors(value);
      out.push(`hue-rotate(${roundTo(deg, 1e4)}deg)`);
      continue;
    }
    if (!FILTER_PROPORTIONS.has(name)) return normalizeEmbeddedColors(value);
    const pct = /^([+-]?[\d.]+)%$/.exec(arg);
    const n = arg === '' ? 1 : pct ? parseFloat(pct[1]) / 100 : (/^[+-]?[\d.]+$/.test(arg) ? parseFloat(arg) : null);
    if (n === null || !Number.isFinite(n)) return normalizeEmbeddedColors(value);
    out.push(`${name}(${roundTo(n, 1e6)})`);
  }
  return out.join(' ');
}
  ...
  if (cascaded != null && cascaded !== '') {
    ...
    if (cw === 'inherit' || cw === 'unset' || cw === 'initial' || cw === 'revert' || cw === 'revert-layer') {
      ...
      if (isColorValued(key)) {
        ...
        // one an animation produced, one the author typed.
        if (value.indexOf('%') !== -1 && value.indexOf('calc(') !== -1) {
        ...
        if (key === 'transform')                          value = transformMatrix(value, () => borderBoxOf(el));
          ...
          const shadow = serializeShadow(value, SHADOW_LENGTHS[key], () => readComputed(el, 'color').value, el);
          value = shadow === null ? computedInitialValue(key, el) : shadow;
        }
        else if (COLOR_BEARING_PROPS.has(key))            value = serializeFilter(value, () => readComputed(el, 'color').value, el);
      }
