...
// A negative px result in a non-negative context clamps to zero (CSS Values 4). Only a value this
// stage actually REDUCED can be negative here, so the scan is over the reduced text.
function clampNonNegativePx(value) {
  return /-\d/.test(value) ? String(value).replace(/-\d*\.?\d+px/g, '0px') : value;
}
