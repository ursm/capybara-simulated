...
function lengthPx(tok, box, axis) {
  const t = String(tok).trim();
  const px = /^(-?[\d.]+)(px)?$/i.exec(t);
  if (px) return parseFloat(px[1]);
  const pct = /^(-?[\d.]+)%$/.exec(t);
  if (pct && box) return parseFloat(pct[1]) / 100 * (axis === 'x' ? box.width : box.height);
  return null;
}
...
function transformMatrix(value, box) {
  ...
  while ((match = FN.exec(v))) {
    ...
    switch (name) {
      case 'translate': {
        const tx = lengthPx(args[0], box, 'x'), ty = args.length > 1 ? lengthPx(args[1], box, 'y') : 0;
        if (tx == null || ty == null) return v;                // unresolvable — don't invent a matrix
        m = multiply2d(m, [1, 0, 0, 1, tx, ty]);
        break;
      }
      case 'translatex': { const t = lengthPx(args[0], box, 'x'); if (t == null) return v; m = multiply2d(m, [1, 0, 0, 1, t, 0]); break; }
      case 'translatey': { const t = lengthPx(args[0], box, 'y'); if (t == null) return v; m = multiply2d(m, [1, 0, 0, 1, 0, t]); break; }
      case 'translatez': { const t = lengthPx(args[0], box, 'y'); if (t == null) return v; has3d = true; z += t; break; }
      case 'scale':       m = multiply2d(m, [num(0), 0, 0, args.length > 1 ? num(1) : num(0), 0, 0]); break;
      case 'scalex':      m = multiply2d(m, [num(0), 0, 0, 1, 0, 0]); break;
      case 'scaley':      m = multiply2d(m, [1, 0, 0, num(0), 0, 0]); break;
      ...
      case 'matrix': m = multiply2d(m, args.map(parseFloat)); break;
...
const SHORTHANDS_SETTING = (() => {
  ...
  for (const sh of Object.keys(SHORTHAND_LONGHANDS)) {
    // FLOW-RELATIVE shorthands are excluded: mdn's entry for one names whichever physical side the
    // value would RESOLVE to, which depends on the writing mode — and we expand every one of them
    // anyway, so the cascade already holds their longhands and there is nothing to be unsure about.
    if (sh.indexOf('-block') !== -1 || sh.indexOf('-inline') !== -1) continue;
...
function readComputed(el, key) {
  ...
  // A line's width computes to 0 while its STYLE is `none` (CSS UI), and a keyword otherwise
  // resolves to px — the same model `computedBorderWidth` gives the per-side border widths. It has
  // to see the CASCADED value, so it sits here rather than in the initial-value fallback, where it
  // could only ever fire for an element nothing declares.
  if (key === 'outline-width' || key === 'column-rule-width') {
    const style = cascadedProperty(el, key.replace('-width', '-style'));
    if (style == null || /^(none|hidden)$/i.test(String(style).trim())) return { hit: true, value: '0px' };
  ...
  if (PX_REPORTABLE_LAYOUT_PROPS.has(key)) {
    ...
    return initial !== null && /^-?\d*\.?\d+px$/.test(initial) ? { hit: true, value: initial } : { hit: false };
  }
