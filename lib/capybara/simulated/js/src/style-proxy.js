...
import { cascadedProperty, matchesAnyHideRule, splitImportant, computedVisibility, ownDisplay, inlineDecls, cascadeDeclaresProperty, canExpandShorthand } from './cascade.js';
import { parseStyleDeclList, serializeCssValue, serializeFontFamily, SYSTEM_COLORS, ABSOLUTE_FONT_SIZE_PX, resolveCssUrls, documentBaseUrl, canonicalizeBgPosition, splitTopLevel, isSupportedCssPropertyName, isValidDeclarationValue } from './css-utils.js';
import { serializeDeclBlock, expandDeclList, isRegularShorthand, shorthandGet, shorthandExpand, shorthandLonghands, clearNamedLonghands, groupNeedsMove, allGet, allGetPriority, isCoveredByAll, isCssWideKeyword, combineBox } from './shorthands.js';
...
const PX_REPORTABLE_LAYOUT_PROPS = new Set([
  ...
  // Their flow-relative twins resolve to the same declaration, so they report the same way.
...
const COMPUTED_INITIAL_OVERRIDES = Object.assign(Object.create(null), {
  ...
  'word-spacing': '0px',
  // mdn says `auto`; Chrome computes `0px` — for the physical AND the flow-relative name, which
  // have to agree since they are the same declaration.
  'min-width': '0px', 'min-height': '0px', 'min-inline-size': '0px', 'min-block-size': '0px',
});
...
function angleDeg(tok) {
  const m = /^(-?[\d.]+)(deg|grad|rad|turn)?$/i.exec(String(tok).trim());
  if (!m) return null;
...
function transformMatrix(value, boxFn) {
  ...
  while ((match = FN.exec(v))) {
    ...
    switch (name) {
      ...
      case 'translatez': { const t = lengthPx(args[0], boxFn, 'y'); if (t == null) return v; has3d = true; z += t; break; }
      case 'scale':       if (!finite(num(0)) || (args.length > 1 && !finite(num(1)))) return 'none';
                          m = multiply2d(m, [num(0), 0, 0, args.length > 1 ? num(1) : num(0), 0, 0]); break;
      case 'scalex':      if (!finite(num(0))) return 'none'; m = multiply2d(m, [num(0), 0, 0, 1, 0, 0]); break;
      case 'scaley':      if (!finite(num(0))) return 'none'; m = multiply2d(m, [1, 0, 0, num(0), 0, 0]); break;
      case 'rotate': case 'rotatez': {
        const r = (angleDeg(args[0]) || 0) * Math.PI / 180;
        m = multiply2d(m, [Math.cos(r), Math.sin(r), -Math.sin(r), Math.cos(r), 0, 0]);
        break;
      }
      case 'skew':  m = multiply2d(m, [1, Math.tan((angleDeg(args[1]) || 0) * Math.PI / 180), Math.tan((angleDeg(args[0]) || 0) * Math.PI / 180), 1, 0, 0]); break;
      case 'skewx': m = multiply2d(m, [1, 0, Math.tan((angleDeg(args[0]) || 0) * Math.PI / 180), 1, 0, 0]); break;
      case 'skewy': m = multiply2d(m, [1, Math.tan((angleDeg(args[0]) || 0) * Math.PI / 180), 0, 1, 0, 0]); break;
      case 'matrix': {
  ...
  // `1.414214`. Rounding to six decimal places agreed only for components below 1.
  // Six SIGNIFICANT digits, which is what a browser reports — `Math.sqrt(2)` is `1.41421`, not
  // `1.414214`. A value that is only floating-point noise is zero: `cos(90deg)` is 6.1e-17, and
  // Chrome prints `0`.
  const round = n => {
...
function serializeShadow(value, lengths, ownColor) {
  ...
  for (const layer of layers) {
    ...
    for (const tok of toks) {
      ...
      if (/^-?[\d.]+(px|em|rem|%|pt|pc|in|cm|mm|q|vh|vw|vmin|vmax)?$/i.test(tok)) { nums.push(/^-?[\d.]+$/.test(tok) ? `${tok}px` : tok); continue; }
...
// Properties whose value CONTAINS colours among other components. Their colours normalise in
// place; the surrounding syntax is left as the author wrote it.
const COLOR_BEARING_PROPS = new Set(['box-shadow', 'text-shadow', 'background', 'border', 'outline', 'column-rule', 'text-decoration', 'text-emphasis', 'filter', 'backdrop-filter']);
...
function readComputed(el, key) {
  ...
  if (key === 'outline-width' || key === 'column-rule-width') {
    const w = cascadedProperty(el, key);
    if (w == null || w === '') return { hit: true, value: BORDER_WIDTH_KEYWORDS.medium };
    const t = String(resolveCssVars(el, w)).trim().toLowerCase();
  ...
  if (isRegularShorthand(key)) {
    ...
    // A shorthand whose longhands are unknowable may still be readable once its own `var()`
    // resolves — same reason, same route.
    const own = cascadedProperty(el, key);
    if (own != null) {
      const resolved = resolveCssVars(el, own);
      const pairs = resolved !== own ? shorthandExpand(key, resolved) : null;
      if (pairs) {
        const m = Object.create(null);
        for (const [lh, v] of pairs) m[lh] = splitImportant(v).value;
        return { hit: true, value: shorthandGet(m, key) };
      }
    }
    return { hit: false };
  }
  ...
  if (key === 'text-decoration') {
    ...
    const parts = [line];
    const style = cascadedProperty(el, 'text-decoration-style');
    if (style != null && String(style).trim().toLowerCase() !== 'solid') parts.push(readComputed(el, 'text-decoration-style').value);
    const color = cascadedProperty(el, 'text-decoration-color');
  ...
  if (PX_REPORTABLE_LAYOUT_PROPS.has(key)) {
    ...
    // NOTHING declares it. A property whose initial is a LENGTH computes to that length with no
    // layout at all — `margin-left` on a plain div is `0px` in every browser, and answering ''
    // there was the same "no value" that page code reads as an answer. `width` / `height` and the
    // min/max pair keep falling through: their initials are `auto` / `none`, where the reported
    // value is the USED one and we would be guessing.
    const initial = computedInitialValue(key);
    if (initial === null) return { hit: false };
    return (/^-?\d*\.?\d+px$/.test(initial) || initial === 'none') ? { hit: true, value: initial } : { hit: false };
  }
  ...
  if (cascaded != null && cascaded !== '') {
    ...
    if (cw === 'inherit' || cw === 'unset' || cw === 'initial' || cw === 'revert' || cw === 'revert-layer') {
      ...
      if (isColorValued(key)) {
        ...
        else if (key === 'box-shadow')                    value = serializeShadow(value, 4, readComputed(el, 'color').value);
        else if (key === 'text-shadow')                   value = serializeShadow(value, 3, readComputed(el, 'color').value);
  ...
  const setters = shorthandsSetting(key);
  if (setters) for (const sh of setters) {
    const shValue = cascadedProperty(el, sh);
    if (shValue == null) continue;
    // A SUBSTITUTION can only be expanded once it resolves, and that happens per element — so a
    // `transition: all var(--d, 2s) ease` the parser had to keep whole is decomposed here.
    const resolved = resolveCssVars(el, shValue);
    const pairs = resolved !== shValue && isRegularShorthand(sh) ? shorthandExpand(sh, resolved) : null;
    if (pairs) {
      const hit = pairs.find(([lh]) => lh === key);
      if (hit) return { hit: true, value: serializeCssValue(splitImportant(hit[1]).value) };
    }
    return { hit: false };
  }
