...
function lengthPx(tok) {
  const m = /^(-?[\d.]+)(px)?$/i.exec(String(tok).trim());
...
function transformMatrix(value) {
  ...
  while ((match = FN.exec(v))) {
    ...
    switch (name) {
      case 'translate':   m = multiply2d(m, [1, 0, 0, 1, lengthPx(args[0]) || 0, lengthPx(args[1]) || 0]); break;
      case 'translatex':  m = multiply2d(m, [1, 0, 0, 1, lengthPx(args[0]) || 0, 0]); break;
      case 'translatey':  m = multiply2d(m, [1, 0, 0, 1, 0, lengthPx(args[0]) || 0]); break;
      case 'translatez':  has3d = true; z += lengthPx(args[0]) || 0; break;
...
function serializeShadow(value, lengths) {
  ...
  for (const layer of layers) {
    ...
    out.push([color, ...nums, inset ? 'inset' : null].filter(Boolean).join(' '));
  }
  return out.join(', ');
}
...
function normalizeEmbeddedColors(value) {
  return String(value).replace(HEX_OR_NAMED_RE, (tok) => {
  ...
  if (cascaded != null && cascaded !== '') {
    ...
    if (cw === 'inherit' || cw === 'unset' || cw === 'initial' || cw === 'revert' || cw === 'revert-layer') {
      ...
      if (isColorValued(key)) {
        ...
        // reads.)
        if (key === 'transform')                          value = transformMatrix(value);
        else if (key === 'box-shadow')                    value = serializeShadow(value, 4);
        else if (key === 'text-shadow')                   value = serializeShadow(value, 3);
