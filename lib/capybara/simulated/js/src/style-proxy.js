...
import { cascadedProperty, expandShorthandValue, twinName, cascadeGeneration, matchesAnyHideRule, splitImportant, computedVisibility, ownDisplay, inlineDecls, cascadeDeclaresProperty, canExpandShorthand } from './cascade.js';
...
// cached content-addressably — the same shape as the driver's other four caches. That is the whole
// safe caching surface here: a per-ELEMENT memo is not available, because `cascadedRecord` re-matches
// selectors live on every read (which is how `:state()` / `:focus` / an attribute flip take effect
// at all) and no generation counter moves when they change. Keying one on `cascadeGeneration()`
// looked right and served stale values to eight WPT subtests. See the perf note below.
...
// KNOWN COST, measured: reading a longhand behind a substitution runs an ancestor walk per `var()`
// that the literal form doesn't. On 400 elements x 5 rounds reading four box longhands + gBCR:
// 1.9-3.0 ms literal vs 4.1-5.2 ms behind `var()` (6.1-7.7 ms before the cache above). Closing the
// rest means caching a per-element RESULT, which needs a real cascade-invalidation epoch — one that
// moves on a `:state()` / `:focus` / attribute change — and that is its own piece of work.
export function declaredValue(el, key) {
  // Memoised per element on the cascade generation — which, since the epoch landed, moves for every
  // input the cascade actually reads: the stylesheets, the DOM / attributes / form state / location,
  // and the style-state counter that carries what neither of those covers (`:state()`, focus,
  // `:defined`, `:filtered`, a shadow root's adopted sheets). The FIRST attempt at this cache keyed
  // on a generation that was blind to those and served stale values to eight WPT subtests, so the
  // key is not taken on trust: `CSIM_VERIFY_STYLE_CACHE=1` recomputes every hit and reports any
  // disagreement, and the gate is run with it on.
  const gen = cascadeGeneration();
  let memo = el._dvMemo;
  if (memo === undefined || el._dvGen !== gen) { memo = el._dvMemo = new Map(); el._dvGen = gen; }
...
// Every cache hit recomputed and compared. A BUILD-TIME constant, so the shipped bundle carries no
// flag at all and the check folds away; the validation run is a rebuild:
//
...
//
// then the full gate + app suites, then a plain rebuild. That is what turns "did I enumerate every
// invalidation source?" from a hope into a measurement — the first attempt at this cache was a
// careful enumeration that missed three. Run when the cache landed: V8 2134/0/5, QuickJS 2099/0/61,
// WPT 4784/0/29 and all four app suites, zero disagreements.
//
// Worth (same machine, back to back, 400 elements x 5 rounds, four box longhands + gBCR):
// literal 4.7-6.1 ms -> 2.2-2.9 ms, behind `var()` 8.5-10.2 ms -> 2.0-2.6 ms.
...
function verifyDeclaredValue(el, key, cached) {
  ...
  const msg = `[csim] style cache STALE on ${where} ${key}: cached ${JSON.stringify(cached)}, fresh ${JSON.stringify(fresh)}`;
...
function computeDeclaredValue(el, key) {
  ...
  return hit ? serializeCssValue(splitImportant(String(hit[1])).value) : null;
}
...
function computedFontSizePx(el) {
  ...
  const cascaded = declaredValue(el, 'font-size');
