...
import { CSS_PROPERTY_BY_IDL_ATTRIBUTE, parseStyleDeclList, serializeCssValue, serializeFontFamily, SYSTEM_COLORS, ABSOLUTE_FONT_SIZE_PX, resolveCssUrls, documentBaseUrl, canonicalizeBgPosition, normalizeZeroLengths, splitTopLevel, isSupportedCssPropertyName, isValidDeclarationValue } from './css-utils.js';
...
function camelToKebab(name) {
  ...
  // Anything else is not a property of ours; fold it so the name it is TESTED against is stable.
  // The one fold that must still land on a real property is the `-webkit-` MIRROR family
  // (`webkitTransform` → `-webkit-transform`), which the table above deliberately omits.
  // `-moz-…` / `-ms-…` get no such spelling: `style.mozOsxFontSmoothing = 'grayscale'` is a plain
  // expando that writes no declaration, and neither prefix appears among the IDL attributes
  // (measured, Chrome 151.0.7922.169 — 151 `webkit`-cased attributes, zero `moz`/`ms` ones).
...
function writableDeclaration(store) {
  return {
    ...
    getPropertyValue:    (property) => propValue(store, cssName(property)),
    getPropertyPriority: (property) => propPriority(store, cssName(property)),
    ...
    removeProperty: (property) => removeStoreProp(store, cssName(property))
  };
}
...
function resolvedDeclaration(el, inline, names) {
  return {
    ...
    getPropertyValue: (property) => {
      const r = readComputed(el, String(property).toLowerCase());
