...
import { hasMathFunction, evaluateMath }  from './calc.js';
...
// element.
// NOT memoised per element, and that is a decision with three rounds of evidence behind it.
//
...
export function declaredValue(el, key) {
  return computeDeclaredValue(el, key);
}
...
// `15px` (measured) — and an INVALID one (`calc(1px + 1)`) makes the whole declaration invalid, so
// the property falls to its initial, which is what a null answer already means here.
function resolveMath(el, key, value) {
  ...
  const fsPx = computedFontSizePx(el);
  const out = evaluateMath(value, (n, unit) => fontLengthToPx(n, unit, fsPx, el));
  if (out === 'invalid') return null;
  // Unresolvable (a percentage — no per-property basis here) keeps the author's text, which is what
  // downstream already reads as "needs layout". Returning null would claim the property is unset.
