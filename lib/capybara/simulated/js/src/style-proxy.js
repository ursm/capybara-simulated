...
const SHORTHANDS_SETTING = (() => {
  ...
  const add = (lh, sh) => { (map[lh] || (map[lh] = [])).push(sh); };
  for (const sh of Object.keys(SHORTHAND_LONGHANDS)) {
    const logical = sh.indexOf('-block') !== -1 || sh.indexOf('-inline') !== -1;
    for (const lh of SHORTHAND_LONGHANDS[sh]) {
      add(lh, sh);
      // A FLOW-RELATIVE shorthand's entry names one physical side, but which side it really sets
      // depends on the writing mode — mdn is recording a computed-value resolution, not an
      // expansion. Claim every side of the family instead: `border-block-end: 3px solid` then
      // makes all four `border-*-style` unknowable rather than confidently reporting `none` for
      // the side it actually set and blanking a side it didn't touch.
      if (logical) for (const side of SIDES) {
        const mirrored = lh.replace(/-(top|right|bottom|left)-/, `-${side}-`);
        if (mirrored !== lh) add(mirrored, sh);
      }
    }
  }
  return map;
