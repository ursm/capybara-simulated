...
function verifyDeclaredValue(el, key, cached) {
  ...
  let fresh, deps;
  try { fresh = computeDeclaredValue(el, key); }
...
export function declaredValueEntry(el) {
  ...
  if (entry === undefined || entry.epoch !== epoch || entry.ctx !== ctx) {
    entry = { epoch, ctx, map: new Map(), vdeps: undefined, vgen: varGenTotalNow() };
...
export function declaredValueIn(entry, el, key) {
  if (entry === null) return computeDeclaredValue(el, key);
  ...
  let value, mine;
  try { value = computeDeclaredValue(el, key); }
...
const UA_DEFAULTS = Object.assign(Object.create(null), {
  ...
  select:   { 'overflow-x': 'hidden', 'overflow-y': 'scroll' },   // …a LISTBOX one; see UA_APPLIES
...
const UA_APPLIES = Object.assign(Object.create(null), {
  ...
  // A LISTBOX `<select>` — `size` above 1, or `multiple` — is a scroll container: Chrome computes
  // `overflow-x: hidden; overflow-y: scroll` on it and honours `select.scrollTop = 40` (measured;
  // a dropdown `<select>` gets neither). Without the UA rule the cascade saw no overflow at all,
  // so restoring a listbox's scroll position was silently dropped.
  select: isListBox
});
// A LISTBOX `<select>` is a different control from a dropdown: it scrolls, it is white rather than
// grey, and it is as tall as the rows it shows. One predicate answers all three — the UA overflow
// rule above, the chrome below, and the intrinsic height layout gives it.
export function isListBox(el) {
  if (el._tag !== 'select' || !el._attrs) return false;
  if (el._attrs.multiple != null) return true;
  return parseInt(el._attrs.size, 10) > 1;
}
// The rows a listbox shows: its `size`, or the 4 Chrome falls back to for a bare `multiple`.
export function listBoxRows(el) {
  const size = parseInt(el._attrs && el._attrs.size, 10);
...
//
// Chrome 137-measured on this machine, per control FAMILY rather than per tag. The font is the
// one Chrome reports for controls here (13.3333px Arial, monospace for `<textarea>`), resolved
// through fontconfig exactly as every other family is.
...
function controlChrome({ width, style, color, padY, padX, bg, family = 'Arial',
                         sizing = 'content-box', align = 'start', cursor = 'default', margin = '0px' }) {
  const out = { 'background-color': bg, 'font-size': '13.3333px', 'font-family': family,
...
const FIELD_CHROME  = controlChrome({ width: '2px', style: 'inset', color: 'rgb(118, 118, 118)',
                                      ...
                                      cursor: 'text' });
// A checkbox / radio / file input paints its own widget with no CSS box around it: no border, no
// padding, nothing behind it — the box is the widget's own size, which `intrinsicSize` gives it.
const WIDGET_CHROME = controlChrome({ width: '0px', style: 'none', color: 'rgb(0, 0, 0)',
                                      padY: '0px', padX: '0px', bg: 'rgba(0, 0, 0, 0)' });
// A checkbox / radio carries a MARGIN of its own — `margin: 3px 3px 3px 4px` — which is the only
// thing that spaces it from the label beside it. A range is 2px all round.
...
const COLOR_CHROME = controlChrome({ width: '1px', style: 'solid', color: 'rgb(0, 0, 0)',
                                     padY: '1px', padX: '2px', bg: 'rgb(239, 239, 239)',
                                     sizing: 'border-box' });
// A listbox is a white scrolling pane, not a grey button face.
...
const INPUT_WIDGET_TYPES = new Set(['file', 'hidden']);
// `<input>`'s chrome follows its TYPE, which a tag-keyed table cannot express — a submit button
// is a button, a checkbox is a widget, and everything else is a text field.
function inputChrome(el) {
  const t = String((el._attrs && el._attrs.type) || 'text').toLowerCase();
  if (INPUT_WIDGET_TYPES.has(t)) return WIDGET_CHROME;
  if (t === 'checkbox' || t === 'radio') return CHECKBOX_CHROME;
  if (t === 'range') return RANGE_CHROME;
  if (t === 'color') return COLOR_CHROME;
  return INPUT_BUTTON_TYPES.has(t) ? BUTTON_CHROME : FIELD_CHROME;
}
const TAG_CHROME = Object.assign(Object.create(null), {
  button: BUTTON_CHROME, textarea: TEXTAREA_CHROME
});
function controlChromeFor(el) {
  if (el._tag === 'input') return inputChrome(el);
  if (el._tag === 'select') return isListBox(el) ? LISTBOX_CHROME : SELECT_CHROME;
  return TAG_CHROME[el._tag];
}

export function uaDefault(el, key) {
  const pres = presentationSize(el, key);
  if (pres !== undefined) return pres;
  // Control chrome first: it is keyed per control family, and `UA_DEFAULTS` (tag-keyed) already
  // carries other values for the same tags — `button`'s `text-align`, `select`'s overflow.
