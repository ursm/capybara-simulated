...
const transformArgs = (text) => splitTopLevel(text, ',').map((t) => t.trim()).filter(Boolean);
...
function lengthPx(tok, boxFn, axis) {
  ...
  if (!hasMathFunction(t) || t.indexOf('%') === -1) return null;
  const box = boxFn();
  if (!box) return null;
...
export function usedTransformMatrix(el) {
  ...
  let m = null;
  for (const [prop, toMatrix] of INDIVIDUAL_TRANSFORMS) {
    const text = readComputed(el, prop).value;
    if (!text || /^none$/i.test(String(text).trim())) continue;
    const step = toMatrix(String(text).trim(), boxFn);
    if (!step) continue;
    m = m ? multiply2d(m, step) : step;
  }
  const value = readComputed(el, 'transform').value;
  if (value && !/^none$/i.test(value)) {
    const own = MATRIX_RE.test(value) ? matrixNumbers(value) : projected3dMatrix(value, boxFn);
    if (own) m = m ? multiply2d(m, own) : own;
  }
  if (!m) return null;
  if (m[0] === 1 && m[1] === 0 && m[2] === 0 && m[3] === 1 && m[4] === 0 && m[5] === 0) return null;
  const origin = splitTopLevelWhitespace(usedOrigin(el, 'transform-origin'));
  return { m, ox: parseFloat(origin[0]) || 0, oy: parseFloat(origin[1]) || 0 };
}
...
const INDIVIDUAL_TRANSFORMS = [
  ['translate', (text, boxFn) => {
    const parts = splitTopLevelWhitespace(text);
    const tx = lengthPx(parts[0], boxFn, 'x');
    const ty = parts.length > 1 ? lengthPx(parts[1], boxFn, 'y') : 0;
    return tx == null || ty == null ? null : [1, 0, 0, 1, tx, ty];
  }],
  ['rotate', (text) => {
    const parts = splitTopLevelWhitespace(text);
    const axis = parts.length > 1 ? parts.slice(0, -1).join(' ').toLowerCase() : 'z';
    const deg = angleDeg(parts[parts.length - 1]);
    if (deg == null) return null;
    const r = deg * Math.PI / 180;
    if (axis === 'z' || axis === '0 0 1') return [Math.cos(r), Math.sin(r), -Math.sin(r), Math.cos(r), 0, 0];
    // …and an X or Y rotation foreshortens the other axis, the same flattening `rotateX` gets.
    if (axis === 'x' || axis === '1 0 0') return [1, 0, 0, Math.cos(r), 0, 0];
    if (axis === 'y' || axis === '0 1 0') return [Math.cos(r), 0, 0, 1, 0, 0];
    return null;
  }],
  ['scale', (text) => {
    const parts = splitTopLevelWhitespace(text).map(scaleFactor);
    if (parts[0] == null) return null;
    const sy = parts.length > 1 ? parts[1] : parts[0];
    return sy == null ? null : [parts[0], 0, 0, sy, 0, 0];
  }]
];
...
function matrixNumbers(value) {
  ...
  if (nums.length === 16) return [nums[0], nums[1], nums[4], nums[5], nums[12], nums[13]];
  return null;
}
// …and the 3D function forms, projected. Only the ones whose flattening is a plain 2D matrix:
// anything else (a `perspective()`, a `rotate3d` about a tilted axis) leaves the box where it is,
// which is what this driver drew before and is honest about not modelling.
function projected3dMatrix(value, boxFn) {
  let m = [1, 0, 0, 1, 0, 0];
  let seen = false, flattened = 0;
  const FN = TRANSFORM_FN_RE();
  let match;
  while ((match = FN.exec(String(value)))) {
    const name = match[1].toLowerCase();
    const args = transformArgs(match[2]);
    const flat = FLAT_3D[name];
    if (!flat) {
      // A 2D function inside an otherwise-3D list still composes: the computed value only stayed a
      // function list because of its 3D neighbour.
      const two = transformMatrix(`${name}(${args.join(', ')})`, boxFn);
      const nums = MATRIX_RE.test(two) ? matrixNumbers(two) : null;
      if (!nums) return null;
      m = multiply2d(m, nums);
      seen = true;
      continue;
    }
    // Flattening happens ONCE, after the 4x4 is composed — this driver composes no 4x4, so it can
    // project a SINGLE 3D function and no more. `rotateX(90deg) rotateX(-90deg)` is the identity,
    // and flattening each half in turn collapses the box to nothing.
    if (++flattened > 1) return null;
    const step = flat(args);
    if (!step) return null;
    m = multiply2d(m, step);
    seen = true;
  }
...
// Each 3D function's flattening, as a 2D matrix. A rotation about X or Y foreshortens the OTHER
// axis by the cosine of the angle; `translateZ` / `perspective` move nothing on their own.
const FLAT_3D = Object.assign(Object.create(null), {
  rotatex:      (a) => { const d = angleDeg(a[0]); return d == null ? null : [1, 0, 0, Math.cos(d * Math.PI / 180), 0, 0]; },
  rotatey:      (a) => { const d = angleDeg(a[0]); return d == null ? null : [Math.cos(d * Math.PI / 180), 0, 0, 1, 0, 0]; },
  rotate3d:     (a) => {
    // A CARDINAL axis is one this projection can express — about Z it is a real rotation, about X
    // or Y the same cosine foreshortening `rotatex` / `rotatey` above do. A tilted axis still needs
    // the 4x4 this driver does not compose.
    //
    // X and Y were refused until the interpolator started GENERATING this form: rewriting a
    // rotate-family pair to their shared primitive spells `rotateX(90deg)` as
    // `rotate3d(1, 0, 0, 90deg)`, and the box then measured untransformed where the un-rewritten
    // spelling measured exactly right (Chrome: `0, 40, 120, 0`; refused: `0, 0, 120, 80`).
    const [x, y, z] = a.slice(0, 3).map(parseFloat);
    const d = angleDeg(a[3]);
    if (d == null || !Number.isFinite(x) || !Number.isFinite(y) || !Number.isFinite(z)) return null;
    if ([x, y, z].filter((c) => c !== 0).length !== 1) return null;
    if (z !== 0) {
      const r = d * Math.PI / 180 * Math.sign(z);
      return [Math.cos(r), Math.sin(r), -Math.sin(r), Math.cos(r), 0, 0];
    }
    const c = Math.cos(d * Math.PI / 180);
    return x !== 0 ? [1, 0, 0, c, 0, 0] : [c, 0, 0, 1, 0, 0];
  },
  scale3d:      (a) => { const sx = parseFloat(a[0]), sy = parseFloat(a[1]);
                         return Number.isFinite(sx) && Number.isFinite(sy) ? [sx, 0, 0, sy, 0, 0] : null; },
  scalez:       () => [1, 0, 0, 1, 0, 0],
  translatez:   () => [1, 0, 0, 1, 0, 0],
  perspective:  () => [1, 0, 0, 1, 0, 0]
});
