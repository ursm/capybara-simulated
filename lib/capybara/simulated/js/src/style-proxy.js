...
function computeAnimationEndpoint(el, key, declared, ownColorFn = () => readComputed(el, 'color').value) {
  ...
  // …and every OTHER colour-valued property, whose type mdn records as something else: `color`
  // itself is "by computed value type", `stroke-color` is "by computed value", `stop-color` is
  // "discrete". An endpoint left as `#0f0` or `hsl(120, 100%, 50%)` is a pair the interpolator
  // cannot read, so it flipped discretely where Chrome fades (measured, both forms). Last, so a
  // property with a serializer of its own — the shadows above, whose value BEGINS with a colour —
  // is never mistaken for one.
  if (COLOR_VALUED_PROPERTIES.has(key)) {
