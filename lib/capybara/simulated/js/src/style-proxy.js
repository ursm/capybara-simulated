...
import { animatedValue, animatedProperties, transitionedValue, transitionSpecs, noteAnimatedElement } from './animation.js';
...
function computeDeclaredValue(el, key, entry) {
  ...
  // A value computed while THIS PROPERTY's endpoint is being resolved is the cascade's alone — the
  // transition layer is off for it, so an element whose own transition is running would report
  // where it is heading rather than where it is. Per PROPERTY, not a global flag: resolving a
  // `1em` border width reads `font-size`, and that read has to see the font size the element
  // REPORTS — an ancestor transitioning it moves it gradually, and reading past that made every
  // em-sized descendant start a transition of its own. The value must not be memoised either, or
  // the first descendant to resolve an `inherit` endpoint freezes its ancestor's transition before
  // it starts (WPT properties-value-inherit-001 and -003 are the two halves).
  const resolving = RESOLVING_ENDPOINT.size !== 0 &&
                    (RESOLVING_ENDPOINT.has(key) || (twin && RESOLVING_ENDPOINT.has(twin)));
  ...
  const running = el._csimTrans !== undefined && el._csimTrans !== null &&
                  el._csimTrans.runs.has(baselineKey(key, twin));
  if (!spec && !running) {
    // …and it still RECORDS what the property was. A page that declares the transition and changes
    // the value in one go is measured against the style from before both, and the property was not
    // transitioned then — and an element whose styles were reset needs the reset itself recorded,
    // or the next change is measured against whatever it last transitioned to.
    // Recorded through `baselineFor` rather than `beforeChangeValue`: the "an element that has
    // just appeared has no before-change style" rule belongs to the decision to START one, not to
    // the recording, and running it here let a read taken before this pass had boxes throw the
    // baseline away.
    if (transitions) baselineFor(el, key, twin, base, false);
    return base;
  }
  // A RUNNING transition is serviced whether or not the element still declares one: with no
  // declaration the property is covered by the initial `all`, so the run goes on — redirected if
  // the cascade sends the property somewhere else, retired when it arrives. Only starting a NEW
  // one needs a declaration, which `startTransition` asks for itself.
...
// ONE record per flow-relative PAIR, under the name the run itself is keyed by: `block-size` and
// `height` are the same value, and two records drift apart — a read under one name leaves the other
// stale, and the stale one then starts a transition from a value the element had two style changes
// ago (WPT css-logical/animation-004). Keyed by the PHYSICAL name instead, a writing-mode change
// re-pairs the logical name with a different physical one and the record splits anyway — measured
// worse on the same file.
const baselineKey = (key, twin) => (twin && twin < key ? twin : key);

function baselineFor(el, key, twin, value, transitioned) {
  ...
  // A record made while the property was NOT transitioned is not a before-change value: the page
  // may have moved the value again since, at a style change event this element was not asked
  // about, and comparing against the older one invents a change that never happened. (The
  // declared-value memo's outgoing map covers the case that does matter — the transition and the
  // change declared together — because it is filled by every read.)
  const before = rec === undefined || !rec.transitioned ? undefined : rec.value;
  if (rec === undefined) store.set(shared, { gen, value, transitioned });
  else if (rec.gen !== gen) { rec.gen = gen; rec.value = value; rec.transitioned = transitioned; }
  return before;
}

// The timing this element gives this property, or null where it transitions it not at all. The
// parsed `transition-*` lists are memoised on the entry: they are read once per element per style
// epoch instead of once per property read.
function transitionSpecOf(entry, el, key, twin) {
  ...
  if (entry) {
    ...
    specs = transitionSpecs(el, declaredValue);
  }
...
function beforeChangeValue(el, key, twin, value, fallback) {
  ...
  const before = baselineFor(el, key, twin, value, true);
...
function transitionEndpoint(el, key, declared) {
  const outer = RESOLVING_ENDPOINT.has(key);
  RESOLVING_ENDPOINT.add(key);
  try {
    return animationEndpoint(el, key, declared) ?? cssWideEndpoint(el, key, 'unset');
  } finally {
    if (!outer) RESOLVING_ENDPOINT.delete(key);
  }
}
