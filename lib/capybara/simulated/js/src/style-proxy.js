...
import { parseStyleDecls, serializeCssValue, serializeFontFamily, SYSTEM_COLORS, ABSOLUTE_FONT_SIZE_PX, resolveCssUrls, documentBaseUrl, canonicalizeBgPosition, splitTopLevel, isSupportedCssPropertyName, isValidDeclarationValue } from './css-utils.js';
...
import { LONGHANDS, INITIAL_VALUES, INHERITED_PROPERTIES } from './css-property-data.js';
...
// form, `font-weight: normal` to its numeric weight, `text-indent: 0` to a length. The rest —
...
const COMPUTED_INITIAL_OVERRIDES = Object.assign(Object.create(null), {
  'font-weight': '400', 'text-indent': '0px', 'color': 'rgb(0, 0, 0)',
...
function readComputed(el, key) {
  ...
  if (cascaded != null && cascaded !== '') {
    ...
    if (cw === 'inherit' || cw === 'unset' || cw === 'initial' || cw === 'revert' || cw === 'revert-layer') {
      if (cw === 'inherit' || (cw === 'unset' && INHERITED_PROPERTIES.has(key))) {
        ...
        if (parent && parent.nodeType === NODE_ELEMENT) {
          const r = readComputed(parent, key);
          // An explicit `inherit` takes the parent's computed value whatever its origin, so this
          // element's value is a real one — it does not carry the parent's `fromInitial` marker.
          return r.hit ? { hit: true, value: r.value } : r;
        }
      }
    } else {
      ...
      if (key === 'color' || key.endsWith('-color')) {
        // `currentcolor` is the INITIAL of most colour longhands, and it never survives to a
        // resolved value — a browser reports the element's own computed `color` instead, which is
        // what a colour parser downstream expects to receive. (The per-side border branch above
        // resolves it the same way.)
        value = /^currentcolor$/i.test(String(resolved).trim())
          ? (readComputed(el, 'color').value || 'rgb(0, 0, 0)')
  ...
  } else if (INHERITED_PROPERTIES.has(key)) {
    ...
    // colour rather than the root's. So find the nearest ancestor that declares one and resolve
  ...
  // a browser reports the element's own computed `color` (`color`'s own initial is a real colour,
  // so the recursion terminates).
  const value = key !== 'color' && /^currentcolor$/i.test(initial)
    ? (readComputed(el, 'color').value || 'rgb(0, 0, 0)')
    : initial;
  return { hit: true, value, fromInitial: true };
}
