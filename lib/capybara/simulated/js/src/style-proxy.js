...
import { parseStyleDecls, serializeStyleDecls, joinStyleDecls, serializeCssValue } from './css-utils.js';
...
export function makeDeclProxy(store) {
  ...
  const handler = {
    get(_t, prop) {
      ...
      if (prop === 'cssText') return joinStyleDecls(storeDecls(store));
      ...
      const v = readStoreProp(store, camelToKebab(prop));
      if (v !== '') return v;
