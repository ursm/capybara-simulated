...
//
// Chrome 137-measured on this machine, per control FAMILY rather than per tag.
//
...
function controlChrome({ width, style, color, padY, padX, bg, family = 'Arial', textColor = 'rgb(0, 0, 0)',
  ...
  const out = Object.assign(Object.create(null), {
    'background-color': bg, 'color': textColor,
...
// `3px 3px 3px 5px` for a radio. A range is 2px all round.
const CHECKBOX_CHROME = { ...WIDGET_CHROME, 'box-sizing': 'border-box',
                          'margin-top': '3px', 'margin-right': '3px',
                          'margin-bottom': '3px', 'margin-left': '4px' };
const RADIO_CHROME  = { ...CHECKBOX_CHROME, 'margin-left': '5px' };
...
const INPUT_CHROME = Object.assign(Object.create(null), {
  button: BUTTON_CHROME, submit: BUTTON_CHROME, reset: BUTTON_CHROME,
  checkbox: CHECKBOX_CHROME, radio: RADIO_CHROME,
  range: RANGE_CHROME, color: COLOR_CHROME, file: WIDGET_CHROME, image: WIDGET_CHROME,
...
// A CSS-WIDE KEYWORD resolved to the value it stands for, for the LAYOUT side. `revert` on a
// control's padding means the UA sheet's; `inherit` means the parent's computed value. The layout
// engine reads the cascade directly (`resolveLayoutProp`) and a keyword parses as no length at
// all, so without this the box silently lost the edge while getComputedStyle reported it —
// `<button style="padding: revert">` measured 10.67 wide where Chrome says 22.67.
export function cssWideComputed(el, prop) {
  const g = readComputedGeneric(el, prop);
  return g && g.hit ? g.value : null;
}
