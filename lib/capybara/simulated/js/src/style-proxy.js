...
function propValue(store, name) {
  ...
  return isRegularShorthand(name) ? shorthandGet(decls, name, true)
...
// The token forms a shadow LENGTH can take: an optionally-signed number with an absolute,
// font-relative or viewport unit — plus a bare `0`. A PERCENTAGE is deliberately absent: it is not
// a valid shadow length at all, and Chrome drops the whole declaration for one.
...
function shadowLengthPx(tok, el) {
  ...
  if (!m) return null;
  const px = fontLengthToPx(parseFloat(m[1]), (m[2] || 'px').toLowerCase(), computedFontSizePx(el), el);
  return `${Math.round(px * 1e4) / 1e4}px`;
}

// Returns null when a layer is INVALID (a percentage length), which drops the declaration — the
// caller then reports the property's initial, as a browser does. A token we merely can't model
// (a `calc()`) still reports the author's value unchanged.
function serializeShadow(value, lengths, ownColor, el) {
  ...
  for (const layer of layers) {
    ...
    for (const tok of toks) {
      ...
      if (/^[+-]?[\d.]+%$/.test(tok)) return null;             // no <percentage> shadow length exists
      const len = shadowLengthPx(tok, el);
      if (len !== null) { nums.push(len); continue; }
