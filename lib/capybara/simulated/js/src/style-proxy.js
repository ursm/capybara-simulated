...
import { parseStyleDeclList, serializeCssValue, serializeFontFamily, SYSTEM_COLORS, ABSOLUTE_FONT_SIZE_PX, resolveCssUrls, documentBaseUrl, canonicalizeBgPosition, normalizeZeroLengths, splitTopLevel, isSupportedCssPropertyName, isValidDeclarationValue, htmlDimensionValue } from './css-utils.js';
...
// The UA stylesheet's display for `el`, or undefined where it has nothing to say. Almost all of it
// is the per-tag table above; the one exception is `<input type=hidden>`, which HTML's UA sheet
// gives `display: none` and no tag-keyed table can express. ONE door, because the layout engine's
// hot path reads this default too and the two must not disagree about whether a box exists.
export function uaDisplay(el) {
  if (el._tag === 'input' && inputType(el) === 'hidden') return 'none';
  return DEFAULT_DISPLAY[el._tag];
}
...
function computedDisplayFor(el) {
  ...
  // …except the one UA rule Chrome marks `!important`: a hidden input is never rendered, whatever
  // the page declares (measured: `input[type=hidden] { display: block }` still computes `none`).
  if (el._tag === 'input' && inputType(el) === 'hidden') return 'none';
  ...
  if (d === 'revert' || d === 'revert-layer') return uaDisplay(el) || 'block';
...
function presentationSize(el, key) {
  ...
  // `<input type=image>` is an image, and takes the hint like one (Chrome: `width=100 height=40`
  // makes it 100x40 where it is 0x0 undecoded). No other input type does — a text field's `width`
  // attribute is not a thing at all.
  if (el._tag === 'input') { if (inputType(el) !== 'image') return undefined; }
  else if (!PRESENTATION_SIZE_TAGS.has(el._tag)) return undefined;
  const raw = el._attrs && el._attrs[key];
  if (raw == null) return undefined;
  const t = String(raw).trim();
  // SVG's geometry attributes are CSS lengths — `width="2em"` is a length, and `width="40"` is
  // 40px. HTML's are not: they follow their own grammar, below.
  if (el._tag === 'svg') {
    if (/^\d+(\.\d+)?$/.test(t)) return t + 'px';
    return /^-?\d*\.?\d+(px|em|rem|ex|ch|pt|pc|in|cm|mm|q|%|vw|vh|vmin|vmax)$/i.test(t) ? t : undefined;
  }
  // The HTML grammar, shared with the cascade's presentational hint (css-utils). Written strictly
  // there because the value now reaches `getComputedStyle` through the same door an author's
  // declaration does (`resolveSizeDeclaration`): a percentage used to be dropped further down for
  // not being px, which hid every invalid one behind the right answer.
  return htmlDimensionValue(t) ?? undefined;
}
// ── Form-control chrome ──────────────────────────────────────────────────────────────────────
...
const FILE_CHROME = { ...WIDGET_CHROME, 'overflow-x': 'clip', 'overflow-y': 'clip' };
delete FILE_CHROME.color;
const IMAGE_CHROME = { ...WIDGET_CHROME, 'cursor': 'pointer',
...
export function uaDefault(el, key) {
  ...
  if (own === undefined) {
    if (!LOGICAL_UA_ENTRIES.has(entry)) return undefined;
    const twin = twinName(el, key);
    if (!twin || !Object.prototype.hasOwnProperty.call(entry, twin)) return undefined;
    own = twin;
  }
  const applies = UA_APPLIES[el._tag];
  return (applies && !applies(el)) ? undefined : entry[own];
}
// Which UA table entries hold at least one flow-relative name. Computed once: the check is a
// string test per key, and `uaDefault` is on the hot path.
const LOGICAL_UA_ENTRIES = new globalThis.Set(
  Object.values(UA_DEFAULTS).filter(e => Object.keys(e).some(k => /-(block|inline)-(start|end)$|^(min|max)?-?(block|inline)-size$/.test(k)))
);
...
function readComputedGeneric(el, key) {
  ...
  if (cssWide !== 'initial' && !(cssWide === 'unset' && !INHERITED_PROPERTIES.has(key))) {
    ...
    if (uaOwn !== undefined) {
      const value = isColorValued(key) ? normalizeColor(uaOwn) : absolutizeLengths(el, key, String(uaOwn));
      return { hit: true, value };
    }
  }
