...
// flag and the check folds away; the validation run is a rebuild:
//
...
export function declaredValue(el, key) {
  ...
  // `!= null`, not `!== undefined`: a parser-built node carries `_ownerDoc === null` and leans on
  // the `ownerDocument` getter's "null means this realm's document" fallback — which is `<html>`,
  // `<head>` and `<body>`, i.e. every inherited-property walk's terminus. Excluding them left the
  // memo missing on the hottest ancestors in the driver.
  if (el._ownerDoc != null && el._ownerDoc !== globalThis.document) return computeDeclaredValue(el, key);
