...
import { cascadedProperty, dynamicReadSeq, restoreDynamicSeq, expandShorthandValue, twinName, cascadeGeneration, matchesAnyHideRule, splitImportant, computedVisibility, ownDisplay, inlineDecls, cascadeDeclaresProperty, canExpandShorthand } from './cascade.js';
...
// Does THIS realm's document own `el`? Only then may its computed value be cached: a cross-realm
// read resolves against the reading realm's rules and its own generation counter, and both realms'
// counters start at 0, so a cached answer would be handed back as current in a cascade that never
// produced it — permanently, since nothing in the reading realm evicts it.
//
// `_ownerDoc` alone can't answer. It is NULL on the `html`/`head`/`body` skeleton
// `createHtmlPageDocument` builds — for EVERY document, a frame's included — and the
// `ownerDocument` getter's "null means this realm's document" fallback is exactly the wrong answer
// for a frame's body read from the parent. (Testing `!== undefined` excluded the main document's
// own skeleton, the terminus of every inherited-property walk; testing `!= null` let a frame's
// skeleton in. The property is ambiguous, so ask the TREE instead.)
//
// The walk only runs for those skeleton nodes, and it is one or two steps from `<body>`.
function ownedByThisRealm(el) {
...
export function declaredValue(el, key) {
  ...
  if (entry === undefined || entry.gen !== gen) { entry = { gen, map: new Map() }; DV_MEMO.set(el, entry); }
  ...
  if (dynamicReadSeq() === before) memo.set(key, value);
  return value;
}
