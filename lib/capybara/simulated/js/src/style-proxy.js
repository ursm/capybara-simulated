...
import { parseStyleDecls, serializeStyleDecls } from './css-utils.js';
...
function storeDecls(store) {
  ...
  if (holder._declKey !== s) {
    holder._declCache = parseStyleDecls(s);
    holder._declKey = s;
  }
...
function writeStoreProp(store, name, value, priority) {
  const decls = parseStyleDecls(store.read() || '');
  if (value === '' || value == null) {
    delete decls[name];
  } else {
    let v = String(value);
  ...
  store.write(serializeStyleDecls(decls));
}

function removeStoreProp(store, name) {
  const v = readStoreProp(store, name);
  const decls = parseStyleDecls(store.read() || '');
  delete decls[name];
  store.write(serializeStyleDecls(decls));
  return v;
...
export function makeDeclProxy(store) {
  ...
  const handler = {
    get(_t, prop) {
      ...
      if (prop === 'cssText') return serializeStyleDecls(storeDecls(store));
      ...
      if (prop === 'getPropertyValue')    return name => readStoreProp(store, cssName(name));
      if (prop === 'getPropertyPriority') return name => readStorePriority(store, cssName(name));
      ...
      // Object-model members (toString / valueOf / constructor / Symbol.toStringTag /
      // …) resolve from the target's prototype, not as CSS properties — so
      // `style.toString()` is callable rather than the '' of an absent property.
      if (prop in _t) return _t[prop];
      if (typeof prop !== 'string') return undefined;
      if (/^\d+$/.test(prop)) return Object.keys(storeDecls(store))[+prop] || '';
      return readStoreProp(store, camelToKebab(prop));
    },
    set(_t, prop, value) {
      ...
      if (prop === 'cssText') {
        store.write(serializeStyleDecls(parseStyleDecls(String(value == null ? '' : value))));
        return true;
      }
    ...
    has(_t, prop) {
      ...
      return readStoreProp(store, camelToKebab(String(prop))) !== '';
    }
  };
