...
import { animatedValue, animatedProperties, transitionedValue } from './animation.js';
...
function computeDeclaredValue(el, key, before) {
  ...
  if (!ANIMATION_TYPES[key] || key.startsWith('animation-') || key.startsWith('transition-')) return base;
  ...
  if (!animations && !transitions) return base;
  ...
  const animated = animatedOver(el, key, twin, base, animations);
  ...
  if (animated !== base) { noteUncacheableRead(); return animated; }
  ...
  const t = transitionedValue(el, key, twin, base, beforeValue(before, key, twin),
                              declaredValue, animationEndpoint);
...
function animatedOver(el, key, twin, base, props) {
  if (!props || !(props.has(key) || (twin && props.has(twin)))) return base;
  ...
  const v = animatedValue(el, key, twin, under, declaredValue, animationEndpoint);
