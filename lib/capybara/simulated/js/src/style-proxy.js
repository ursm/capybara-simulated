...
function propValue(store, name) {
  ...
  if (isRegularShorthand(name)) return shorthandGet(decls, name, true);
  if (decls[name] == null) return '';
  const v = stripImportant(decls[name]);
  // A slot still holding a PENDING substitution has no specified value of its own — `el.style
  // .marginTop` is '' after `margin: var(--m)`, while `el.style.margin` gives back `var(--m)`
  // (measured). `shorthandGet` above is what answers the second half.
  return pendingSource(v) ? '' : v;
}
...
// while the generic tail handled it.
function declaredValue(el, key) {
  ...
  if (!src) {
    ...
    // regex only runs on a value that actually held one.
    return v === raw || !hasSubstitution(v) ? v : null;
  }
  const resolved = String(resolveCssVars(el, src.value)).trim();
  if (!resolved || hasSubstitution(resolved)) return null;
  const pairs = expandShorthandValue(src.shorthand, resolved);
  ...
  const hit = pairs.find(([lh]) => lh === key) ||
              (twinName(el, key) ? pairs.find(([lh]) => lh === twinName(el, key)) : null);
  return hit ? splitImportant(String(hit[1])).value : null;
}
...
// Called only once a longhand has come up empty, which is why it can afford to re-expand.
function setterLeavesUnknown(el, key, setters) {
...
function readComputed(el, key) {
  ...
  if ((cascaded == null || cascaded === '' || revertsToOrigin) && INHERITED_PROPERTIES.has(key)) {
    ...
    for (let p = el._parent; p && p.nodeType === NODE_ELEMENT; p = p._parent) {
      ...
      if (setterLeavesUnknown(p, key, setters)) return { hit: false };
    }
  }
  ...
  if (setterLeavesUnknown(el, key, shorthandsSetting(key))) return { hit: false };
