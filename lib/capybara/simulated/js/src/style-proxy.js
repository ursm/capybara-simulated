...
// The element's declared-value memo entry for the CURRENT (rules + dynamic style state,
// structural context) key, opened once — or null when the element refuses caching (cross-realm,
...
export function declaredValue(el, key) {
  ...
  // in `declaredValueIn`, and `:has()` via the ctx-unsafe bracket.
