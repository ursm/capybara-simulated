...
//
// then the full gate + app suites, then a plain rebuild.
//
// It only verifies what the run EXERCISES, and that caveat has teeth: the first clean sweep of it
// still shipped five stale pseudo-classes, because no gated test happened to read computed style
// right after a hover / `indeterminate` / `showPopover` / `value` write. Its coverage of dynamic
// state comes from `spec/cascade_invalidation_spec.rb`, whose table is derived from `selectors.js`
// — keep that table complete and this check means something; let it drift and a green run means
// only that the untested paths went untested.
//
