...
const runLoopStepLocal = function (maxMs, maxIter, yieldOnGen) {
  ...
  while (true) {
    if (iter++ >= maxIter) { installVirtualDateIfAdvanced(startNow); const g = currentSettleGen(); return { fired, gen: g, dirtied: g !== startGen, raf: rafCallbacks.length > 0 }; }
    ...
    if (yieldOnGen && currentSettleGen() !== startGen) {
      ...
      { const g = currentSettleGen(); return { fired, gen: g, dirtied: g !== startGen, raf: rafCallbacks.length > 0 }; }
    }
  }
  ...
  return { fired, gen: g, dirtied: g !== startGen, raf: rafCallbacks.length > 0 };
};
