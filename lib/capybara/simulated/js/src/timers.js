...
// One microtask checkpoint — the native, in-isolate `MiniRacer.drainMicrotasks`
...
// native `MiniRacer.drainMicrotasks` checkpoint, run on every `__runLoopStep`
...
function bumpVirtualOffset(startNow) {
  if (virtualNow > startNow) { virtualOffsetMs += virtualNow - startNow; installVirtualDate(); }
}
...
// `{ fired, gen }` — `fired` = tasks run (the find-cache-invalidation count that
// `__drainTimers` callers want), `gen` = the settleGen after the step (Ruby
// dirties the find cache on a gen delta). When `yieldOnGen`, returns to Ruby the
...
const runLoopStepLocal = function (maxMs, maxIter, yieldOnGen) {
  ...
  while (true) {
    if (iter++ >= maxIter) { bumpVirtualOffset(startNow); const g = currentSettleGen(); return { fired, gen: g, dirtied: g !== startGen }; }
    ...
    if (yieldOnGen && currentSettleGen() !== startGen) {
      ...
      runRenderPhase();
      bumpVirtualOffset(startNow);      // yielded early — do NOT over-advance virtualNow to `limit`
      if (pendingEmpty()) setTimersActive(false);   // last work drained by this task → clear flag (don't strand polling)
      { const g = currentSettleGen(); return { fired, gen: g, dirtied: g !== startGen }; }
    }
  }
