...
const runLoopStepLocal = function (maxMs, maxIter, yieldOnGen) {
  ...
  while (true) {
    if (iter++ >= maxIter) { installVirtualDateIfAdvanced(startNow); const g = currentSettleGen(); return { fired, gen: g, dirtied: g !== startGen }; }
...
//
// `raf` / `nextTimer` reflect the MAIN realm only (`run_loop_step` itself drives
// child iframe realms via drainChildRealms, but this probe reads the main realm's
// queues). A child-realm-only parked timer or rAF therefore reports as idle here;
// a caller's force-timeout backstops that. No vendored test depends on it.
globalThis.__csimEventLoopProbe = function () {
  return {
    ...
    nextTimer: typeof __nextTimerDelay === 'function'  ? __nextTimerDelay()         : -1
  };
};
