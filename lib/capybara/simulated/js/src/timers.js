...
import { updateIntersectionObservations, hasPendingIntersections } from './observers.js';
import { logThrew }                                        from './console.js';
import { flushAnimationFrame } from './animation.js';
...
// The one sanctioned way for ANOTHER module's pending-work source (observers.js's
// pending intersections) to make the driver keep stepping: the same
// idle→active transition a newly scheduled timer makes. Exported so the flag
// access stays on this cached path rather than growing second, differently
// guarded `globalThis` loads elsewhere.
export function wakeLoop() { setTimersActive(true); }
...
let   rafIdSeq = 1;
// Pending IntersectionObserver observations are runnable work exactly like a
// queued rAF: they run in the render phase of the next step, so they must keep
// the pending-set non-empty or the driver would stop stepping before delivery.
function pendingEmpty() { return timers.size === 0 && rafCallbacks.length === 0 && !hasPendingIntersections(); }
// Any pending timer / rAF in this realm's SUBTREE (self + child realms, recursively)?
// Exported so a parent realm's clear-check can recurse through grandchildren.
function subtreePending() {
  if (!pendingEmpty()) return true;
  const ids = globalThis.__csimChildRealmIds;
  if (!ids || !ids.size || !globalThis.RustyRacer || typeof globalThis.RustyRacer.contextGlobal !== 'function') return false;
  for (const id of ids) {
    try {
      const g = globalThis.RustyRacer.contextGlobal(id);
      if (g && typeof g.__csimSubtreePending === 'function' && g.__csimSubtreePending()) return true;
    } catch (_) {}
  }
  return false;
}
globalThis.__csimSubtreePending = subtreePending;
// Clear the PROCESS-WIDE "timers active" flag (`Driver#wait?`) — but only when
// nothing in the WHOLE realm tree (from the top window down) is pending. A FRAME
// realm's step sees only its own queue, and the callback it just ran may have
// queued work in the PARENT realm — a cross-realm postMessage delivery task — so
// a child-side "my queue is empty → flag off" stops the driver before that task
// ever runs (navigation-redirect-to-http: the helper frame's statechange-time
// `parent.postMessage` was stranded exactly this way). Any realm may still clear
// when the whole tree is idle, so an idle page's flag drops as eagerly as before.
// The tree walk runs only when a step/clear finds this realm's own queue empty —
// never on the hot non-empty path. (A worker's flag host fn is a no-op; an aux
// window is its own isolate and thus its own root.)
function maybeClearTimersActive() {
  if (!pendingEmpty()) return;   // own queue non-empty ⇒ tree pending ⇒ nothing to clear
  try {
    let root = globalThis;
    const t = globalThis.top;
    if (t && t !== globalThis) root = t.__csimRawWindow || t;
    const pending = root === globalThis
      ? subtreePending()
      : (typeof root.__csimSubtreePending === 'function' ? root.__csimSubtreePending() : false);
    if (!pending) setTimersActive(false);
  } catch (_) {}
}
...
globalThis.clearTimeout  = function (id) {
  if (timers.delete(id)) maybeClearTimersActive();
};
...
// The document's "pending scroll event targets" (CSSOM-View): `scroll` first, then `scrollend`,
// each coalesced to one per scroller. Also kicked from a microtask when a scroll happens on a page
// with nothing else pending — see `scheduleScrollFlush` in dom-nodes.js.
function flushScrollEvents() {
  if (typeof globalThis.__csimFlushScroll    === 'function') globalThis.__csimFlushScroll();
...
function runRenderPhase() {
  ...
  // rAF queue, not after.
  microcheck();
  // "Run the scroll steps" comes FIRST in the HTML rendering update — before this frame's
  // animation callbacks and before the intersection observations (Chrome, measured on a page that
  // scrolls with an rAF pending: `scroll`, `scrollend`, then the rAFs, then the IO callback).
  // Running them after rAF instead handed a sticky header or a virtual scroller the frame's
  // animation callback before the scroll event that frame was about.
  flushScrollEvents();
  // An animated element's box follows its animation: the clock has moved since the last update, so
  // whatever reported an interpolated value then needs its layout again now.
  flushAnimationFrame();
  // …and the CSS-embedded image resources rendered content needs (background / cursor / list-style
  // url()s) — a browser fetches and times them at the rendering update. Gated inside on whether the
  // document declares any, so an ordinary page pays a single boolean.
  if (typeof globalThis.__csimFlushCssResources === 'function') globalThis.__csimFlushCssResources();
  ...
  // …and again for anything the rAFs and the observations scrolled, so it does not wait a frame.
  flushScrollEvents();
...
const runLoopStepLocal = function (maxMs, maxIter, yieldOnGen) {
  ...
  while (true) {
    ...
    if (yieldOnGen && currentSettleGen() !== startGen) {
      ...
      maybeClearTimersActive();   // last work drained by this task → clear flag (don't strand polling)
...
globalThis.__csimEventLoopProbe = function () {
  ...
  // A running CSS animation or transition owes an event at a known moment, and nothing else on the
  // page may be scheduled — so it counts as a due time, exactly as a parked timer does. Without it
  // a page waiting on a transition reads as idle and the runner jumps the clock past it.
  if (typeof globalThis.__csimNextCssEventDelay === 'function') {
    const cssDue = globalThis.__csimNextCssEventDelay();
    if (cssDue >= 0 && (nextTimer < 0 || cssDue < nextTimer)) nextTimer = cssDue;
  }
