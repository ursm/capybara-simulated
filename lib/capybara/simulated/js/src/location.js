...
export function tryFragmentNavigate(destHref, replace = false) {
  ...
  bumpSettleGen();
  // `:target` derives from the fragment: the flip must reach the style-state generation, or a
  // `:target`-driven rule keeps stale style and boxes across the navigation.
  bumpStyleState();
