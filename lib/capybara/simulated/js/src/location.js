...
const _location = {
  ...
  assign:  (next) => { if (!tryFragmentNavigate(String(next)))       globalThis.__locationAssign(next); },
  replace: (next) => { if (!tryFragmentNavigate(String(next), true)) globalThis.__locationAssign(next); },
