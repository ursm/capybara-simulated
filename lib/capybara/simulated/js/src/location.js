...
import { bumpSettleGen, bumpStyleState, TARGET_KINDS } from './mutation-observer.js';
...
export function tryFragmentNavigate(destHref, replace = false) {
  ...
  bumpSettleGen();
  bumpTargetState(cur.hash, dest.hash);
...
globalThis.__csimUpdateLocation = function (url) {
  ...
  const oldHash = hashOf(_u.href);
  setLocationFromUrl(s);
  // URL change is observable progress; settle yields on it the same
  // way it yields on DOM mutations. The fragment may have changed with it — `:target`.
  bumpSettleGen();
  bumpTargetState(oldHash, hashOf(s));
};
...
// `:target` derives from the fragment. When it changes, the flip reaches the style-state
// generation HINTED with the elements that stopped being / became the target (resolved the way
// the `:target` matcher resolves them: every connected element whose id is the decoded
// fragment), so the scoped layout dirtying touches those and nothing else. When it does not
// change, no generation moves at all — an SPA's pushState per route is a URL change, not a state
// flip, and an unhinted bump here swept every dynamic rule's subjects over the page per route.
function bumpTargetState(oldHash, newHash) {
  if (oldHash === newHash) return;
  const doc = globalThis.document;
  const targets = (hash) => {
    if (!doc || !hash || hash.length <= 1) return [];
    let frag = hash.slice(1);
    try { frag = decodeURIComponent(frag); } catch (_) {}
    if (typeof globalThis.CSS === 'object' && typeof globalThis.CSS.escape === 'function' && doc.querySelectorAll) {
      try { return [...doc.querySelectorAll('#' + globalThis.CSS.escape(frag))]; } catch (_) {}
    }
    const one = doc.getElementById && doc.getElementById(frag);
    return one ? [one] : [];
  };
  bumpStyleState({ kinds: TARGET_KINDS, elements: targets(oldHash).concat(targets(newHash)) });
}
