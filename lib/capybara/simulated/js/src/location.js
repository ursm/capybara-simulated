...
const _location = {
  ...
  assign:  (next) => { if (!tryFragmentNavigate(String(next)))       dispatchNav(next, false); },
  replace: (next) => { if (!tryFragmentNavigate(String(next), true)) dispatchNav(next, true); },
...
Object.defineProperty(globalThis, 'location', {
  ...
  set(v) { dispatchNav(String(v), false); }
});
