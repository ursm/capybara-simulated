...
import { shadowNow, recordConstruct, invalidateArena, registerNativeElement, updateNativeElementNs } from './native-query-shadow.js';   // F3 thinning probe + arena reset on in-place reparse + eager node (re)create
...
export function installParse5Adapter({
  ...
  const treeAdapter = {
    ...
    adoptAttributes(recipient, attrs) { applyAttrs(recipient, attrs); bumpSel(recipient); bumpSel(recipient._parent); bumpParserTreeGen(); },
