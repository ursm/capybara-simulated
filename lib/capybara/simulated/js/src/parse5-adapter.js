  ...
  globalThis.__csimWriteAtInsertionPoint = (doc, html) => {
    ...
    try {
      for (;;) {
        ...
        const el = parsedScripts[parsedScripts.length - 1];
        if (stream.blocksParser(el)) break;           // the pending parsing-blocking script
        runScriptAt(parsedScripts.pop());
      }
    } finally {
      pp.html += rest;
