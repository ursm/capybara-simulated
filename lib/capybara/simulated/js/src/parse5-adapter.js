...
export function installParse5Adapter({
  ...
  const treeAdapter = {
    ...
    createElement(tagName, namespaceURI, attrs) {
      ...
      finalizeElementProto(el);   // foreign content (SVG/MathML) reverts to base Element.prototype
      applyAttrs(el, attrs);
      // A parsed <img src> in the LIVE document starts fetching + decoding its
      // resource immediately, so naturalWidth/complete and the load event are
      // populated for later script. Gated on `live`: a DOMParser document, an
      // innerHTML/`<template>` fragment, and other non-browsing-context parses are
      // inert per spec — they must not load resources (and skipping them avoids a
      // fetch + decode per image for parse-only work).
      if (live && el._tag === 'img' && el._attrs.src) el._loadImageResource();
      return el;
    },
