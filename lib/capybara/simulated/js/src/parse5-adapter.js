...
export function installParse5Adapter({
  ...
  // only queues the element and PAUSES the tokenizer, and `runParsedScripts` runs it and resumes.
  ...
  // </script> four</div>` read "one four two" (Chrome and Firefox: "one two four"). A `<script>` in the written markup
  // pauses that nested run and waits its turn, after the script that wrote it.
  // (`insertionPoint` is an ABSOLUTE input offset — the preprocessor drops consumed input as it goes.)
  const parsedScripts = [];
  let insertionPoint = -1;
  const queueParsedScript = (el) => { parsedScripts.push(el); curParser.tokenizer.pause(); };
  function runParsedScripts(onScript) {
    const tk = curParser.tokenizer, pp = tk.preprocessor;
    while (tk.paused && parsedScripts.length) {
      const el = parsedScripts.shift();
      insertionPoint = pp.droppedBufferSize + pp.pos + 1;
      try { onScript(el); } finally { insertionPoint = -1; }
      // (A script its markup WROTE is queued with the tokenizer paused on that script's `</script>`: it runs next,
      // from there, before anything after it is tokenized.)
      if (!parsedScripts.length) tk.resume();
    }
  }
  globalThis.__csimWriteAtInsertionPoint = (doc, html) => {
    if (!curParser || curDoc !== doc || insertionPoint < 0) return false;
    const tk = curParser.tokenizer, pp = tk.preprocessor;
    const at = insertionPoint - pp.droppedBufferSize;
    const rest = pp.html.slice(at);
    pp.html = pp.html.slice(0, at) + html;
