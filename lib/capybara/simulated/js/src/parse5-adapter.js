...
import { bumpCtxAll, ctxChildListEffect, ctxCharDataEffect, bumpCtxInserted } from './cascade.js';
import { noteDocumentWideAttribute } from './mutation-observer.js';
import { shadowNow, recordConstruct, invalidateArena, registerNativeElement, updateNativeElementNs, syncNativeAttrs } from './native-query-shadow.js';   // F3 thinning probe + arena reset on in-place reparse + eager node (re)create + batched attr flush

let parserTreeGen = 0;
globalThis.__csimParserTreeGen = () => parserTreeGen;
...
export function installParse5Adapter({
  ...
  function applyAttrs(el, attrs, adopt) {
    ...
    for (let i = 0; i < attrs.length; i++) {
      ...
      noteDocumentWideAttribute(name, a.value);
    ...
    // Flush the whole set at once. On QuickJS (no native store) `_attrs` is a
    // plain object and syncNativeAttrs declines (returns false) — write the flat
    // list into it directly there.
  ...
  function resetReusedElement(el, attrs) {
    // Re-create this reused skeleton element's EAGER arena node. The reload's invalidateArena (called at
    // the top of parse5ParseIntoLive) reset the arena and dropped the previous page's nodes, so the
    // skeleton's `_nid` / attrsView(old_nid) are stale — registerNativeElement stamps a fresh `_nid` and
    // repoints `_attrs` to attrsView(new_nid) in the fresh arena, into which applyAttrs then writes the
    // new opening-tag attributes. (A freshly-constructed element gets this in its ctor; the skeleton is
    // reused, not reconstructed, so it needs the explicit re-create here.) No-op without `__dom`, leaving
    // `_attrs` a plain JS object for QuickJS.
    el._attrs = {};
    registerNativeElement(el);
    ...
    el._attrNodes = null;
    applyAttrs(el, attrs);
    // A reused skeleton node's later appendChild is a no-op (it never left its parent), so this
    // is the one structural-ish op the presence gate would otherwise not hear about — an empty
    // `<body class="dd">` must reopen it just as an inserted one would.
    bumpParserTreeGen();
  }
  ...
  // settleGen deliberately doesn't move mid-parse), but the element-local context counters
  // (cascade.js ctxEpochOf) MUST move — the same gated child-list / text effects a script-driven
  // mutation takes (`ctxChildListEffect` & co.), so a page that reads style mid-parse does not pay
  // a whole-subtree re-key per parsed node: a mid-parse style read (an inline script's
  ...
  function bumpSel(node) { if (node) bumpCtxAll(node); }
  function bumpKids(parent, node) {
    if (parent) {
      const placed = node && node._parent === parent;
      ctxChildListEffect(parent, placed ? [node] : undefined, undefined, placed ? node.previousSibling : undefined, placed ? node.nextSibling : undefined);
    }
    if (node) bumpCtxInserted(node);
  }
  // …and one COUNTER must move for the whole-document caches: the dynamic-layout presence gate
  // (cascade.js DYN_ARMED) asks "does any element with this class exist?", an answer any parser
  // insertion can change — and with recordChildList observer-gated, `dirtySeq` stays still while
  // the parser builds the page. A bare increment per structural op is all a memo key needs;
  // reads can only observe between parser-blocking scripts, so precision costs nothing.
  function bumpParserTreeGen() { parserTreeGen = (parserTreeGen + 1) | 0; }
  ...
  function loadParsedImage(node) {
    if (live && node.nodeType === 1 && typeof node._imageResourceSrc === 'function' && node._imageResourceSrc()) {
  ...
  const treeAdapter = {
    ...
    createElement(tagName, namespaceURI, attrs) {
      const _t0 = shadowNow();   // F3 probe: time node construction (0 = off); skeleton-reuse returns below skip it
      ...
      el._ownerDoc = curDoc;
      if (namespaceURI && namespaceURI !== HTML_NS) { el._ns = namespaceURI; updateNativeElementNs(el); }
      ...
      // fresh `new Element` is empty, so its apply seeds nothing and pays no read.
    ...
    createCommentNode(data) { const _t0 = shadowNow(); const c = new Comment(data); c._ownerDoc = curDoc; recordConstruct(_t0); return c; },
    createTextNode(value)   { const _t0 = shadowNow(); const t = new Text(value);   t._ownerDoc = curDoc; recordConstruct(_t0); return t; },
    ...
    insertText(parentNode, text) {
      ...
      if (last && last.nodeType === NODE_TEXT) { last._data += text; ctxCharDataEffect(parentNode); bumpParserTreeGen(); if (stream) stream.onTextAppend(parentNode); return; }
      const t = new Text(text);
      t._ownerDoc = curDoc;
      reownIntoTemplateDoc(parentNode, t);
      t._parent = parentNode;
      kids.push(t);
      bumpKids(parentNode, t);
      bumpParserTreeGen();   // text is tree content too: a parse-time innerText read keys on this
    ...
    insertTextBefore(parentNode, text, referenceNode) {
      ...
      if (prev && prev.nodeType === NODE_TEXT) { prev._data += text; ctxCharDataEffect(parentNode); bumpParserTreeGen(); if (stream) stream.onTextAppend(parentNode); return; }
      const t = new Text(text);
      t._ownerDoc = curDoc;
      this.insertBefore(parentNode, t, referenceNode);   // re-owns into template content itself
    },
    adoptAttributes(recipient, attrs) { applyAttrs(recipient, attrs, true); bumpSel(recipient); bumpSel(recipient._parent); bumpParserTreeGen(); },
  ...
  // A parser-blocking script runs AFTER its end tag is processed (HTML's "text" insertion mode pops the `<script>` and
  // restores the insertion mode first), not inside parse5's script handler, which it calls before either: the handler
  // only queues the element and PAUSES the tokenizer, and `runParsedScripts` runs it and resumes. A `<script>` in
  // TEMPLATE content is never prepared (inert, as the insertion hook treats it) — Chrome runs none of them.
  // …which is what gives `document.write` its INSERTION POINT (HTML §8.4.3): while such a script runs, the markup goes
  // into the input stream right after its `</script>`, and is tokenized THERE, before `write` returns — up to the
  // insertion point, which then moves past it. Appended to `<body>` instead, `<div>one<script>document.write("two")
  // </script> four</div>` read "one four two" (Chrome and Firefox: "one two four").
  // A `<script>` in the written markup runs right there, inside `write`, with an insertion point of its own — unless it
  // is an EXTERNAL parser-blocking one: that is the pending parsing-blocking script, it runs after the writer returns,
  // and until it has, a further `write` only inserts (nothing is tokenized past a pending script).
  // (`insertionPoints` are ABSOLUTE input offsets, innermost last — the preprocessor drops consumed input as it goes —
  // and a write moves every one at or after it.)
  const parsedScripts = [];
  const insertionPoints = [];
  let parsedOnScript = null;
  const queueParsedScript = (el) => {
    if (el._inStreamTC === true) {
      // …except a DECLARATIVE SHADOW ROOT's content, which only looks inert here: this parser builds it into the
      // template's content and converts it at `</template>` (HTML attaches the root at the start tag and parses into
      // it), so its scripts wait on the template and run once it has converted (`releaseDsdScripts`).
    ...
    parsedScripts.push(el);
    curParser.tokenizer.pause();
  };
  ...
  function releaseDsdScripts(t) {
    ...
    if (scripts[0].isConnected) {
      for (const s of scripts) { s._csimParserInserted = true; parsedScripts.push(s); }
      curParser.tokenizer.pause();
    } else if (t._dsdOuter) {
  ...
  function runScriptAt(el) {
    const pp = curParser.tokenizer.preprocessor;
    insertionPoints.push(pp.droppedBufferSize + pp.pos + 1);
    try { parsedOnScript(el); } finally { insertionPoints.pop(); }
  }
  function runParsedScripts(onScript) {
    const tk = curParser.tokenizer;
    parsedOnScript = onScript;
    while (tk.paused && parsedScripts.length) {
      runScriptAt(parsedScripts.shift());
      // (A pending script its markup WROTE is queued with the tokenizer paused on that script's `</script>`: it runs
      // next, from there, before anything after it is tokenized.)
      if (!parsedScripts.length) tk.resume();
    }
  }
  globalThis.__csimWriteAtInsertionPoint = (doc, html) => {
    if (!curParser || curDoc !== doc || !insertionPoints.length) return false;
    const tk = curParser.tokenizer, pp = tk.preprocessor;
    const ip = insertionPoints[insertionPoints.length - 1];
    const at = ip - pp.droppedBufferSize;
    pp.html = pp.html.slice(0, at) + html + pp.html.slice(at);
    for (let i = 0; i < insertionPoints.length; i++) if (insertionPoints[i] >= ip) insertionPoints[i] += html.length;
    if (parsedScripts.length) return true;          // a pending parsing-blocking script: inserted, not tokenized
    const end = insertionPoints[insertionPoints.length - 1] - pp.droppedBufferSize;
    const rest = pp.html.slice(end);
    pp.html = pp.html.slice(0, end);
    const lastChunk = pp.lastChunkWritten;
    pp.lastChunkWritten = false;   // the end of what was written is the insertion point, not the end of the input
    pp.endOfChunkHit = false;
    try {
      for (;;) {
        tk.paused = false;
        tk.active = true;
        tk._runParsingLoop();
        if (!tk.paused) break;                        // the insertion point: everything written is tokenized
        if (stream.blocksParser(parsedScripts[0])) break;   // the pending parsing-blocking script
        runScriptAt(parsedScripts.shift());
      }
    } finally {
      pp.html += rest;
      pp.lastChunkWritten = lastChunk;
      pp.endOfChunkHit = false;
      tk.active = true;
      tk.paused = true;              // the writing script is still running
    }
    return true;
  };
  ...
  globalThis.__csimParserScriptRunning = (doc) => !!curParser && curDoc === doc && insertionPoints.length > 0;
  function parse5ParseIntoLive(liveDoc, html, streamHooks) {
    ...
    // A re-parse into the live document rewrites the reused skeleton's children/attrs directly, off
    // the sync seams — drop any built arena so the native matcher re-mirrors the new tree (see
    // invalidateArena). No-op unless an arena was built.
    invalidateArena();
    ...
    try {
      ...
      // streaming mode each `</script>` pauses the tokenizer (`queueParsedScript`) and `runParsedScripts` runs
      // `stream.onScript` against the partial tree — where `document.write` re-enters the tokenizer at the insertion
      // point — before tokenization resumes.
      const parser = new Parser({ treeAdapter, scriptingEnabled: true }, liveDoc, null, stream ? queueParsedScript : null);
      curParser = parser;   // its open-element stack answers tokenMayConstruct
      // Flag the tokenize window so the native arena declines to build mid-parse (ensureBuilt): the
      // tree is mutated off the sync seams here, and a mid-parse `<script>` reading style must not
      // trust a half-built arena. Saved/restored so a re-entrant document.write nests correctly.
      const prevParsing = globalThis.__csimParsingInPlace;
      globalThis.__csimParsingInPlace = true;
      try {
        parser.tokenizer.write(String(html == null ? '' : html), true);
        if (stream) runParsedScripts(stream.onScript);
      } finally {
        globalThis.__csimParsingInPlace = prevParsing;
