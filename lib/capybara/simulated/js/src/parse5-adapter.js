...
import { shadowNow, recordConstruct, invalidateArena } from './native-query-shadow.js';   // F3 thinning probe + arena reset on in-place reparse
...
export function installParse5Adapter({
  ...
  function resetReusedElement(el, attrs) {
    // Fresh JS `_attrs` object. This also UN-SWAPS the store-flip's native `_attrs`: a reused skeleton
    // whose `_attrs` was swapped to attrsView(old_nid) (native-query-shadow.js mirrorCreate) must not
    // repopulate into the previous page's arena node — that node is dropped by the reload's
    // invalidateArena. Re-parsing writes to this JS object; the next cascade re-mirrors it and re-swaps
    // to attrsView(new_nid). (For a non-native / QuickJS run this is just a clear, one identity swap.)
    el._attrs = {};
