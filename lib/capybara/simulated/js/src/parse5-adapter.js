...
export function installParse5Adapter({
  ...
  function bumpKids(parent, node) { if (parent) ctxChildListEffect(parent); if (node) bumpCtxInserted(node); }
  ...
  const queueParsedScript = (el) => {
    if (el._inStreamTC === true) {
      ...
      if (t) (t._dsdScripts || (t._dsdScripts = [])).push(el);
      return;
    }
  ...
  function releaseDsdScripts(t) {
    ...
    if (scripts[0].isConnected) {
      ...
      const outer = dsdTemplateOf(t);
      if (outer) (outer._dsdScripts || (outer._dsdScripts = [])).push(...scripts);
    }
  }
