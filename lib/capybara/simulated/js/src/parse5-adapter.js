...
import { noteDocumentWideAttribute, noteParsedChange } from './mutation-observer.js';
...
export function installParse5Adapter({
  ...
  const treeAdapter = {
    ...
    insertText(parentNode, text) {
      ...
      if (last && last.nodeType === NODE_TEXT) { last._data += text; ctxCharDataEffect(parentNode); noteParsedChange(parentNode); return; }
    ...
    insertTextBefore(parentNode, text, referenceNode) {
      ...
      if (prev && prev.nodeType === NODE_TEXT) { prev._data += text; ctxCharDataEffect(parentNode); noteParsedChange(parentNode); return; }
      const t = new Text(text);
      t._ownerDoc = curDoc;
