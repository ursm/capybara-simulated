...
export class WebSocket extends EventTarget {
  constructor(url, protocols) {
    ...
    if (record.port !== '' && BAD_PORTS.has(record.port | 0)) {
      ...
      globalThis.queueMicrotask(() => {
        if (this._readyState === 3) return;   // fail exactly once (a close() meanwhile still lands here)
  ...
  close(code, reason) {
    // IDL: `close(optional [Clamp] unsigned short code, …)` — the argument is ToNumber'd and
    // CLAMPED into [0, 65535] (NaN → 0), and THEN the spec's range check applies: only 1000 or
    // [3000, 4999] is a valid close code; anything else throws InvalidAccessError. The clamp
    // matters for the error message ordering: `close(2**16 + 1000)` reports the clamped 65535.
    if (code !== undefined) {
      let n = Number(code);
      n = Number.isNaN(n) ? 0 : Math.min(Math.max(Math.round(n), 0), 65535);
      if (n !== 1000 && (n < 3000 || n > 4999)) {
        throw new DOMException(`Failed to execute 'close' on 'WebSocket': The close code must be either 1000, or between 3000 and 4999. ${n} is neither.`, 'InvalidAccessError');
      }
      code = n;
    }
...
globalThis.__csim_deliverWebSocketEvents = function (events) {
  ...
  for (const e of events) {
    ...
    if (e.type === '__close' || e.type === '__error') {
      ...
      const fireClose = ws._readyState !== 3;
      ws._readyState = 3;                // CLOSED
      if (e.type === '__error') {
