...
export function runSelectednessAlgorithm(select, justSelected) {
  ...
  if (select._attrs.multiple != null) { if (select._hasSelectedContent) updateSelectedContent(select); return; }
...
export function updateSelectedContent(select) {
  ...
  for (const o of select.querySelectorAll('option')) {
    if (o._selectedness === true) { selected = o; break; }
  }
  ...
  try {
    for (const sc of scs) {
      ...
      sc.replaceChildren(...(selected ? Array.from(selected.childNodes, (c) => c.cloneNode(true)) : []));
    }
  } finally {
