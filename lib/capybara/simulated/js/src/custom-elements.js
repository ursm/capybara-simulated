...
// from dom-nodes.js (it owns `formForControl` + the form-name registry). Called at
// every seam where a form-associated element's owner can change — upgrade, connection,
// disconnection — from within this module; dom-nodes drives the attribute-change seams
// (`form` on the control, `id` on a <form>) directly. Mirrors setScopeFormOwnerResolver.
let formOwnerReset = null;
export function setFormOwnerReset(fn) { formOwnerReset = fn; }
...
export function upgradeElement(el, ctor) {
  ...
  // A newly upgraded form-associated element resets its form owner (HTML upgrade
  // steps: enqueue formAssociatedCallback with the owner when non-null) — this is
  // the single seam covering every upgrade path (define's match sweep,
  // customElements.upgrade, importNode's ceUpgradeTree, ceTryConnect). The reset is
  // idempotent (guarded on an owner change), so the extra ceTryConnect call below
  // is a no-op here.
  if (formOwnerReset) formOwnerReset(el);
}
...
export function ceTryConnect(el) {
  ...
  // Connecting can change the form owner of an ALREADY-upgraded form-associated
  // element (a move into a form's subtree, or its `form=` target now resolving).
  // upgradeElement above already reset a fresh upgrade; this covers the rest.
  if (formOwnerReset) formOwnerReset(el);
}
...
export function fireCEDisconnect(subtree) {
  ...
  walkInclShadow(subtree, el => {
    ...
    // Removal detaches the element from its form (the subtree is already unlinked),
    // so a form-associated element resets its owner to null (or the new ancestor
    // form) — enqueuing formAssociatedCallback(null) per the removing steps.
    if (formOwnerReset) formOwnerReset(el);
  });
}
