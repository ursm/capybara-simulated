...
export function ceUpgradeTree(subtree) {
  // No definitions → nothing can upgrade; skip the subtree walk entirely. cloneNode
  // calls this on every deep clone (Turbo StreamMessage / template.content), so a page
  // with no custom elements must pay O(1) here, not an O(N) walk (rule 3).
