...
// An element's associated registry: `_ceRegistry` unset → the global registry (the
// common case, no per-element field); an explicit scoped registry → that; `null` → the
// "null registry" state (an upgrade candidate that never matches a definition).
function registryForElement(el) {
  const r = el && el._ceRegistry;
  return r === undefined ? customElements : r;
}
...
function ceCtorFor(el) {
  ...
  const reg = registryForElement(el);
  return reg ? reg._defs.get(el._localName) : undefined;   // null registry → no definition matches
}
...
export function isFormAssociatedCustomElement(el) {
  ...
  const ctor = registryForElement(el)._defs.get(el._localName);
...
class CustomElementRegistry {
  constructor() {
    this._defs = new Map();          // name -> constructor
  ...
  get(name) { return this._defs.get(String(name)) || undefined; }
...
export function getCustomElementCtor(tag) {
  return customElements._defs.get(tag);
}

// The constructor registered for `localName` in a SPECIFIC registry (a scoped
// custom element registry), or undefined. `_createElement` uses it for an element
// whose customElementRegistry option names a non-global registry.
export function ceCtorInRegistry(registry, localName) {
  return registry ? registry._defs.get(localName) : undefined;
}
