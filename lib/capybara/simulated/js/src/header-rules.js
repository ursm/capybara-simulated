...
export function isForbiddenRequestHeader(name, value) {
  // `x-csim-*` are the driver's private request markers (auth-cache signal, base64-body flag) that
  // internal code sets on the header map directly; the author must not be able to forge them via
  // setRequestHeader (e.g. marking a raw Authorization as cacheable).
  if (FORBIDDEN_REQUEST_HEADERS.has(name) || name.startsWith('proxy-') || name.startsWith('sec-') || name.startsWith('x-csim-')) return true;
