...
export function isSubmitButton(n) {
  if (n._tag === 'button') {
    // A `<button>`'s type defaults to submit, and an INVALID/unknown type also
    // falls back to the submit state — only an explicit `reset` / `button` is
    // not a submit button.
...
export function checkedRadioInGroup(n) {
  let found = n._attrs.checked != null ? n : null;
  forEachRadioInGroup(n, (o) => { if (o._attrs.checked != null) found = o; });
  return found;
}
...
export function uncheckOtherRadios(n) {
  forEachRadioInGroup(n, (o) => { delete o._attrs.checked; });
}
