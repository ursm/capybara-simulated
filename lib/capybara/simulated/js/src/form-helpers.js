...
export function labeledControlFor(label) {
  ...
  if (forId) {
    // `for` resolves by id within the label's own node tree — its shadow root
    // when the label is in a shadow tree, else the document (HTML "labeled
    // control": the element in the same tree whose id is `for`). getRootNode()
    // returns the ShadowRoot/Document, both of which expose getElementById.
...
export function formForControl(n) {
  ...
  if (formId) {
    // A present `form` attribute OVERRIDES ancestry and requires the control to
    // be connected (HTML "reset the form owner"): a disconnected control has no
    // owner via form=, even if its tree holds a matching <form>. When connected,
    // resolve by id within the control's own node tree (its shadow root in a
    // shadow tree, else the document); the first element with that id is the
    // owner only if it is a <form>.
  ...
  // No form= attribute: the owner is the nearest ancestor <form> (intrinsic to
  // the tree, so it holds in a disconnected subtree too). ancestorForm walks
  // `_parent` and naturally stops at a shadow-root boundary.
...
export function checkedRadioInGroup(n) {
  ...
  // Radio-group scope is the control's form owner, else its own tree root (the
  // shadow root in a shadow tree, else the document) — NOT documentElement,
  // whose querySelectorAll never crosses into a shadow tree.
...
export function setRadio(n) {
  const name = n._attrs.name;
  if (name) {
    // siblings in the same group (form owner, else the control's own tree root —
    // a shadow root scopes the group; documentElement would miss shadow radios)
    const root = ancestorForm(n) || (n.getRootNode ? n.getRootNode() : globalThis.document);
