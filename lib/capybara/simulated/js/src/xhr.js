...
import { serializeRequestBody, fixContentTypeCharset } from './request-body.js';
...
export class XMLHttpRequest extends EventTarget {
  ...
  send(body) {
    ...
    const fire = () => {
      ...
      this._doFetch(body, hasUpload ? total : null);
    };
  ...
  _completeWith(resp) {
    ...
    // The exposed response header map: lowercased names, minus the forbidden
    // response-header names (`Set-Cookie` / `Set-Cookie2`), which getResponseHeader
    // and getAllResponseHeaders never reveal to script (getresponseheader-cookies).
    const norm = {};
  ...
  _serializeBody(body) {
    // A string / Document / URLSearchParams body is sent UTF-8-encoded (its default
    // type carries `charset=UTF-8`), so an author Content-Type's charset is forced to
    // UTF-8. Blob (opaque bytes) and FormData (multipart, no charset) keep the author
    // type verbatim — they're NOT in this set.
    const charsetFixable = typeof body === 'string' ||
                           (body != null && body.nodeType === 9) ||
