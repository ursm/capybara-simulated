...
export class XMLHttpRequest extends EventTarget {
  ...
  overrideMimeType(mime) { this._overrideMime = parseOverrideMime(mime); }
  abort() {
    ...
    if (sending) { this._terminate('abort'); return; }
    ...
    if (this.readyState === 4) {
      // DONE → UNSENT: the response becomes a network error, so a subsequent read of
      // responseText / response / responseXML must not expose the completed response.
      this.readyState    = 0;
      this.status        = 0;
      this._responseText = '';
      this.response      = '';
