...
export class XMLHttpRequest extends EventTarget {
  ...
  _doFetch(body, uploadTotal) {
    ...
    // Async XHRs go through the hijack-aware Rack path: Ruby
    // installs a `rack.hijack` lambda before calling `@app.call`,
    // and if the middleware actually hijacks (Discourse's
    // MessageBus long-poll, anything else relying on the hijack
    // contract) Ruby spawns a background reader thread that drains
    // the pipe when the middleware writes its response. Most XHRs
    // never trigger the hijack path — Ruby returns immediately
    // with the sync response, no thread spawn, and the JS-visible
    // event sequence is identical to the old `__rackFetch` path
    // (just one settle drain later). Sync XHRs (deprecated
    // `xhr.open(_, _, false)`) fall through to the sync path
    // because a hijacked connection can't satisfy a sync XHR
    // contract anyway.
    if (this._async && typeof globalThis.__csim_rackFetchAsync === 'function') {
      const bodyStr = typeof body === 'string' ? body : this._serializeBody(body);
      const headersJson = JSON.stringify(this._headers || {});
      const handle = globalThis.__csim_rackFetchAsync(this._method, this._url, bodyStr, headersJson) | 0;
      if (handle > 0) {
        this._asyncFetchHandle = handle;
        if (!globalThis.__csim_asyncFetchPending) globalThis.__csim_asyncFetchPending = {};
        globalThis.__csim_asyncFetchPending[handle] = this;
        return;
      }
    }
