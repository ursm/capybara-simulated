...
export class XMLHttpRequest extends EventTarget {
  ...
  _applyCredentials() {
    ...
    // Mark this as AUTHENTICATION credentials (from open()'s user/password or the URL userinfo), so
    // the host caches them for the origin's protection space and re-sends pre-emptively — distinct
    // from a raw setRequestHeader('Authorization'), which is just a header and must NOT be cached.
    this._headers['X-Csim-Auth-Cache'] = '1';
  }
  send(body) {
