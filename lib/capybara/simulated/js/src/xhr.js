...
import { Event, ProgressEvent, EventTarget, dispatchWithOnHandler, defineEventHandlers } from './events.js';
...
defineEventHandlers(XMLHttpRequestUpload.prototype, ['loadstart', 'progress', 'load', 'loadend', 'error', 'abort', 'timeout']);
...
export class XMLHttpRequest extends EventTarget {
  ...
  constructor() {
    ...
    this._sendStart         = null;   // virtual time at send(); the `timeout` deadline counts from here
    this._deliveryTimer     = null;   // a virtual server-delay (delay.py) deferral, raced by _timeoutId
  ...
  set timeout(v) {
    ...
    // Changing timeout mid-flight re-evaluates it against the send start: a new value already
    // elapsed fires the timeout at once, a longer one extends it (xmlhttprequest-timeout-overrides*).
    this._armTimeout();
  }
  // (Re)arm the timeout timer for the remaining time relative to send() — `timeout` counts from the
  // send, so a reset partway through schedules `timeout - elapsed` (0, i.e. next task, if already past).
  _armTimeout() {
    if (this._timeoutId != null) { try { globalThis.clearTimeout(this._timeoutId); } catch (_) {} this._timeoutId = null; }
    if (!this._async || this._timeout <= 0 || this._aborted || this.readyState === 4 || !this._sendFlag) return;
    const now      = typeof globalThis.__virtualNow === 'function' ? globalThis.__virtualNow() : 0;
    const remaining = Math.max(0, this._timeout - (this._sendStart != null ? now - this._sendStart : 0));
  ...
  open(method, url, async, username, password) {
    ...
    let requestUrl = u;
    // The request URL is resolved against the API base URL AT OPEN TIME, to an
    // absolute URL — capturing the base as it is at open() (a `<base>` inserted
    // after open() must not apply: open-url-base-inserted-after-open), and because
    // the fetch layer resolves only against the TOP document URL, which is the
    // WRONG base for a subframe's XHR (a frame page in resources/ fetching
    // 'simple.txt' 404'd against the main document's directory — the same
    // wrong-base class the fetch() path already unified on document.baseURI).
    // Only an http(s) base resolves here: an about:blank / srcdoc frame's baseURI
    // is opaque (its inherited parent base isn't carried by the realm), so its
    // path stays RELATIVE for the fetch layer to resolve against the inherited
    // base — with just the query re-encoded in the document's encoding
    // (open-url-encoding; the URL parser's encoding override applies to the
    // query only).
    const apiBase = (doc && doc.baseURI) || base;
    const parsed = /^https?:/i.test(apiBase) && globalThis.__csim_parseUrl(u, apiBase, doc && doc.characterSet);
    if (parsed && !parsed.error && parsed.href) {
      ...
      requestUrl = parsed.href.split('#')[0];
    } else {
  ...
  abort() {
    ...
    if (this._timeoutId != null) { try { globalThis.clearTimeout(this._timeoutId); } catch (_) {} this._timeoutId = null; }
    if (this._deliveryTimer != null) { try { globalThis.clearTimeout(this._deliveryTimer); } catch (_) {} this._deliveryTimer = null; }
  ...
  _terminate(reason) {
    if (this._timeoutId != null) { try { globalThis.clearTimeout(this._timeoutId); } catch (_) {} this._timeoutId = null; }
    // A timeout/abort during a deferred (server-delayed) response cancels the pending delivery.
    if (this._deliveryTimer != null) { try { globalThis.clearTimeout(this._deliveryTimer); } catch (_) {} this._deliveryTimer = null; }
    if (this._asyncFetchHandle && typeof globalThis.__csim_rackFetchAsyncAbort === 'function') {
      try { globalThis.__csim_rackFetchAsyncAbort(this._asyncFetchHandle); } catch (_) {}
      if (globalThis.__csim_asyncFetchPending) delete globalThis.__csim_asyncFetchPending[this._asyncFetchHandle];
  ...
  _applyCredentials() {
    ...
    // AUTHENTICATION credentials (open()'s user/password or the URL userinfo) are CHALLENGE creds, not
    // sent proactively: the request goes out WITHOUT Authorization, and the host answers a 401 "Basic"
    // challenge by re-sending with them (transparent HTTP auth — the 401 is never exposed to script),
    // then caches them for the origin's protection space. A raw setRequestHeader('Authorization') is a
    // plain header sent as-is (checked above — it wins). `x-csim-*` markers are author-unforgeable.
    this._headers['X-Csim-Auth-Challenge'] = globalThis.btoa((user || '') + ':' + (pass || ''));
  }
  send(body) {
    this._timingStart = globalThis.performance.now();     // Resource Timing: the entry's start
    ...
    // The `timeout` deadline counts from here (the send). A response delayed past it
    // (X-Csim-Server-Delay-Ms) loses the race; a `timeout` reset mid-flight re-arms from this instant.
    this._sendStart = typeof globalThis.__virtualNow === 'function' ? globalThis.__virtualNow() : 0;
    if (this._async) {
      globalThis.setTimeout(doFetch, 0);
    } else {
      doFetch();
    }
    if (this._async) this._armTimeout();
  }
  ...
  _doFetch(bodyStr, uploadTotal) {
    ...
    // NB: the `timeout` timer armed in send() is deliberately NOT cleared here — a delayed response
    // (X-Csim-Server-Delay-Ms) keeps the request in-flight while it runs, so the timeout must stay live
    // to race the deferred delivery. It's cleared when the request actually completes (_completeWith).
    ...
    // XHR-over-SW: an ASYNC controlled client's request goes to the controlling SW's `fetch`
    // event first (like fetch()). When the SW RESPONDS, no upload progress fires — nothing was
    // uploaded over the wire — so `_uploadActive` is cleared before completing; a fall-through
    // does the network request (with upload progress). A SYNC XHR can't await the async worker
    // round-trip, so it stays on the network path.
    const ctrl = this._async && globalThis.__csimSWControllerHandle && globalThis.__csimSWControllerHandle();
    if (ctrl) {
      const self = this;
      // The SW's `event.request.url` is a RESOLVED URL — open() may keep `_url` relative (the
      // network path resolves it Ruby-side against the document base), so resolve it here against
      // this client's own base, matching fetch.js.
      let swUrl = this._url;
      try { swUrl = new URL(this._url, (globalThis.location && globalThis.location.href) || undefined).href; } catch (_) {}
      // XHR is always mode 'cors'; credentials follow withCredentials. The rest are Request defaults.
      const swReq = buildSwRequest({ credentials: this._withCredentials ? 'include' : 'same-origin' });
      globalThis.__csimSWInterceptFetch(ctrl, this._method, swUrl, this._headers, bodyStr, false, swReq, function (swResp) {
        if (self._aborted) return;
        if (swResp == null) { self._doNetworkFetch(bodyStr); return; }        // fall-through → network
        // A SW-served request had no network upload, so no upload progress/load (nor an upload
        // error on Response.error()) fires — clear the active flag before completing either way.
        self._uploadActive = false;
        // Response-to-request validity (Handle Fetch): XHR is mode 'cors' redirect
        // 'follow', so an opaque (no-cors) or opaqueredirect (manual) respondWith
        // is a network error, exactly like fetch.js's filtered-type gate.
        const t = swResp && swResp.type;
        self._completeWith((swResp.__networkError || t === 'opaque' || t === 'opaqueredirect') ? null : swResp);
      });
      return;
    }
    this._doNetworkFetch(bodyStr);
  }
  _doNetworkFetch(bodyStr) {
    let resp;
    try {
      ...
      // `client_url` (arg 13) = this realm's own document URL, exactly as fetch.js
      // threads it: it is what rack_fetch's `crossed` verdict (doc_origin) compares
      // hops against, so a FRAME's same-origin XHR stays same-origin instead of
      // being judged cross-origin vs the TOP document (the cross-origin login
      // iframe's Basic-auth XHR — fetch-canvas-tainting &Auth — depends on it, as
      // does any frame page XHRing its own directory).
      resp = globalThis.__rackFetch(this._method, this._url, bodyStr, this._headers, 'follow', 'cors', this._withCredentials ? 'include' : 'same-origin', null, null, null, null, null, null, String((globalThis.location && globalThis.location.href) || ''));
    } catch (_) { resp = null; }
    if (typeof globalThis.__csimRecordResource === 'function') globalThis.__csimRecordResource({ name: this._url, initiatorType: 'xmlhttprequest', startTime: this._timingStart, resp });
    // A handler that `time.sleep`s (delay.py / trickle.py) returns instantly with a virtual
    // `X-Csim-Server-Delay-Ms` (see wpt_py_handler.py). An ASYNC request defers its whole response
    // delivery by that many virtual ms — so the `timeout` timer (also virtual, armed in send()) fires
    // first when it's shorter, marking the request timed-out; a synchronous request can't wait, so it
    // completes immediately regardless. Strip the marker so getResponseHeader never exposes it.
    const delayMs = this._takeServerDelayMs(resp);
    if (delayMs > 0) {
      if (this._async) {
        if (!this._aborted) this._deliveryTimer = globalThis.setTimeout(() => { this._deliveryTimer = null; this._completeWith(resp); }, delayMs);
        return;
      }
      // A SYNCHRONOUS request can't wait, so decide the timeout race here: if the server would take at
      // least as long as the timeout, send() throws TimeoutError (no events) rather than completing
      // (xmlhttprequest-timeout-worker-synconworker's sync `timeLimit < TIME_XHR_LOAD` cases).
      if (this._timeout > 0 && delayMs >= this._timeout) {
        this.readyState = 4;
        this._uploadActive = false;
        throw new globalThis.DOMException("Failed to execute 'send' on 'XMLHttpRequest': The request timed out.", 'TimeoutError');
      }
    }
    this._completeWith(resp);
  }
  _takeServerDelayMs(resp) {
  ...
  _completeWith(resp) {
    if (this._aborted) return;
    // The request has now actually completed — cancel the still-armed timeout timer so it can't fire
    // after DONE (and doesn't strand the driver active on a far-future deadline).
    if (this._timeoutId != null) { try { globalThis.clearTimeout(this._timeoutId); } catch (_) {} this._timeoutId = null; }
    if (!resp) {
      this.readyState = 4;
      this.status     = 0;
    ...
    // ProgressEvent `total` is the RESPONSE's Content-Length (0 when absent), NOT how many bytes
    // happened to arrive, and per XHR `lengthComputable` is `total !== 0`: a streamed / chunked body
    // with no Content-Length (firing-events-http-no-content-length) AND an explicit empty body with
    // `Content-Length: 0` (send-no-response-event-order) are both non-computable. `loaded` stays the
    // amount received.
    const clHeader = this._respHeaders['content-length'];
    const progressTotal = (clHeader != null && clHeader !== '') ? (parseInt(clHeader, 10) || 0) : 0;
...
defineEventHandlers(XMLHttpRequest.prototype, ['readystatechange', 'load', 'loadstart', 'loadend', 'error', 'abort', 'timeout', 'progress']);
