...
export class XMLHttpRequest extends EventTarget {
  ...
  open(method, url, async) {
    ...
    // Spec § "open()": parse `url` against the document base; a parse FAILURE
    // throws a SyntaxError DOMException synchronously (before any send/fetch) —
    // so a malformed URL never reaches the network path (url/failure.html).
  ...
  _completeWith(resp) {
    ...
    this.responseText = applyResponseCharset(this.responseText, this._overrideCharset || charsetOf(contentType));
    ...
    if (wantsDoc && typeof globalThis.DOMParser === 'function') {
      ...
      try { this.responseXML = new globalThis.DOMParser().parseFromString(this.responseText, mime); }
      catch (_) { this.responseXML = null; }
