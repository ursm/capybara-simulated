...
function responseValue(responseType, text, bytes, blobType, jsonText) {
  switch (responseType) {
    ...
    case 'blob':        return new globalThis.Blob([bytes], {type: blobType || 'application/octet-stream'});
    case 'json': {
...
export class XMLHttpRequest extends EventTarget {
  ...
  _doFetch(bodyStr, uploadTotal) {
    ...
    // in one go. Fire progress(loaded=total) + load + loadend on the upload object
    // before processing the response — ASYNC only: a synchronous request fires no
    // upload (or download) progress events (send-sync-response-event-order).
    if (this._async && uploadTotal != null) {
    ...
    // The body has been transmitted — the upload is complete, so a later abort()/timeout
    // (now during the response) fires no upload error events (abort-during-loading).
    this._uploadActive = false;
    ...
    let resp;
    try {
      resp = globalThis.__rackFetch(this._method, this._url, bodyStr, this._headers, 'follow', 'cors', this._withCredentials);
    } catch (_) { resp = null; }
