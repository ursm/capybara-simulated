...
const NL_CB_RECT = -2;
const NL_RUN_STRIDE = 11; // inline run record [kind, font, size, ls, ws, line_height, metric, asc, ws_mode, tab_px, tab_min]; must match layout::Run
...
function nlInlineMeasurable(el, atomicsPushed) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (va && va.mode !== 'shift' && cd === 'inline') return false;      // placed against a box, not by an offset
...
function nlAtomicMeasurable(c, cd) {
  ...
  const va = verticalAlignFor(c);
  return !(va && (va.mode === 'top' || va.mode === 'bottom')) && nlAtomicNative(c, cd);   // line-relative: declined
}
...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  for (const c of children) {
    ...
    if (cdisp === 'inline-block' || cdisp === 'inline-flex' || cdisp === 'inline-table' || cdisp === 'inline-grid' ||
      ...
      // `vertical-align` on an atomic. `top` / `bottom` align it to the LINE box — whose height is not known
      // until every run on the line is placed — and native's line layout has no line-relative slot, so defer
      // those. EVERY OTHER value only moves the atomic's ASCENT within the line: a baseline SHIFT (sub / super /
      // a length / a percentage) offsets it, and `middle` / `text-top` / `text-bottom` place it against the
      // PARENT's font box. `growAtomic` applies exactly this via `alignedAscent(el, va, asc, outer)`, and so does
      // native — on the box it lays out, or on the ascent pushed with the oracle's box — and the line layout then
      // grows the line box around it (asc above the baseline, outer − asc below).
      const va = verticalAlignFor(c);
      if (va && (va.mode === 'top' || va.mode === 'bottom')) return { ok: false, reason: 'atomic-valign-line' };
      ...
      if (atomic && nlAtomicNative(c, cdisp)) {
        ...
        if (ci !== NL_ATOMIC_PUSHED) {
          ...
          runs.push({ kind: NL_RUN_ATOMIC, rec: ci, asc: va ? va.px || 0 : 0, lh: align.code, metric: align.param, wsMode: wsCode });
          hasContent = true;
          hasReal = true;
          continue;
        }
      ...
      if (va) asc = alignedAscent(c, va, asc, outer);   // baseline shift / middle / text-top / text-bottom
      runs.push({ kind: NL_RUN_ATOMIC, rec: NL_ATOMIC_PUSHED, metric: c._lb.width + ce.ml + ce.mr, asc, lh: outer, wsMode: wsCode });
      hasContent = true;
      hasReal = true;
      continue;
    }
