...
function nlTableSupported(el) {
  ...
  // row for. A CAPTION's own rtl placement (a narrower caption at the inline-start = right) and a COLLAPSE
  // frame's left/right swap aren't reflected yet, so an rtl table with either still declines.
  if (flowSides(el).rtl && (grid.captions.length || tableCollapses(el))) return false;
