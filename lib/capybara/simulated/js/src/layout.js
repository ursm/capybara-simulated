...
// child records of the text block and returns the record index — or NL_ATOMIC_PUSHED to fall back to the
// pushed box, or NL_ATOMIC_DECLINED where no pushed box may be (the text block is inside a subtree native
// measures) — see `nlAtomicNative`. Null in a context that has no record to hang the subtree on (an anonymous
// block's group), or whose lines `justify`.
const NL_ATOMIC_PUSHED = -1;
const NL_ATOMIC_DECLINED = -2;
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode, atomic = null) {
  ...
  for (const c of children) {
    ...
    if (openW !== 0 || closeW !== 0) {
      ...
      const sub = nlGatherRuns(layoutChildren(c), cf, runs, wsMode, cShift, cWrap, atomic);
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (hasInline) {
      ...
      const atomic = align === 'justify' ? null : (c) => {
        ...
        if (walkMeasured(c, idx)) { nativeAtomics++; return ci; }
