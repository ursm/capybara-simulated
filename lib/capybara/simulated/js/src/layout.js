...
const NL_STRIDE = 96;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start / table columns, 56-61 = declared sizing, 62-67 = flex sizing (65 = layout flags, by BIT: 0/1 scrolls x/y, 2 reverse flex, 3-4 legacy descendant align, 5-6 text-align, 7 button), 68-70 = replaced intrinsic, 71-75 = out-of-flow, 76 = auto-margin mask, 77-79 = control baseline, 80-88 = table, 89-90 = basis-less edges, 91 = block axis is horizontal, 92-95 = out-of-flow containing-block rect)
...
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode, atomic = null, rel = null) {
  ...
  for (const c of children) {
    ...
    if (c._tag === 'br') { runs.push({ kind: NL_RUN_BR }); hasContent = true; hasReal = true; hasBr = true; continue; }
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  // is a second one to keep in sync, and the refusals it missed (a `text-indent`, a sticky child, an auto margin
  // on a DESCENDANT record) took whole passes down. Every route with a fallback goes through here — a table cell
  ...
  function walk(el, parentIdx) {
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      if (textIndentOf(flowContainer(el), el._lb.width) !== null) return false;
    ...
    if (hasInline) {
      ...
      if (textIndentOf(flowContainer(el), el._lb.width) !== null) return false; // text-indent shifts wrapping → JS
      const bf = nlFontInfo(el);
      if (bf.nf < 0) return false;                             // block's own font non-system → JS
...
function contentIntrinsicWidths(el) {
  ...
  // leaves an inline-block its padding alone).
  const indent = textIndentOf(el, 0);
  const firstIndent = indent && !indent.hanging ? indent.px : 0;
  let min = 0, max = 0, line = firstIndent, word = firstIndent, inlineOnLine = false;
  ...
  const walk = (node) => {
    for (const child of layoutChildren(node)) {
      ...
      if (child.tagName === 'WBR') { opportunity(); continue; }
      const pos = positionOf(child);
      if (pos === 'absolute' || pos === 'fixed') continue;   // out of flow: sizes nothing
