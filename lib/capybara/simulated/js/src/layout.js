...
import { isLaidOutNode, selfNotRendered, resolveLayoutProp, resolveCascadeDisplay, hasFallbackOnlyContent,
         ...
         maybeCssWide, cssWideStandsFor } from './cascade.js';
...
// oracle. Declines a % MAIN gap (0 with no basis, where the record carries the resolved px) and any item that
...
function nlIndentOf(owner) {
  // The LENGTH part and the FRACTION, never a resolved figure: `text-indent` is one or the other (a `%` or a
  // length — `textIndentOf` parses no mixed form), so the pair is exact, and native resolves the fraction against
  // the block's own content width the way it resolves every other percentage. An INTRINSIC measure takes the
  // length alone: a percentage has nothing to resolve against before the box has been given any room.
  const plain = textIndentOf(owner, 0);
  const atOne = textIndentOf(owner, 1);
  if (!plain && !atOne) return null;
  const px = plain ? plain.px : 0;
  const flags = plain || atOne;
  return { px, frac: (atOne ? atOne.px : 0) - px,
...
const NL_PIECEWISE_MATH_RE = /(?:^|[^\w-])(?:min|max|clamp)\s*\(/i;
...
function nlLinearSizeSpec(el, prop, text) {
  if (text == null || NL_PIECEWISE_MATH_RE.test(text)) return null;
  const probe = { percent: false, keyword: null };
  const at = (b) => resolveLayoutProp(el, prop, b, probe);
  const px = at(0);
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (oof.length) {
      nlWriteIndent(rec, nlIndentOf(flowContainer(el)), false);
...
function axisGap(el, axis, basis) {
  ...
  return g.frac ? (basis == null ? 0 : g.frac * basis) : g.px;
}
// …as `{px, frac}` BEFORE a basis is known: a gap is a length or a percentage of the container's own content box
// along that axis (0 where that box is indefinite), and whoever lays the container out has the box — the native
// grid resolves the fraction against its own.
function gapSpec(el, axis) {
  const own = declaredValue(el, axis + '-gap');
  if (own != null && String(own).trim() !== 'normal') {
    const g = lengthOrFraction(el, String(own).trim());
    if (g != null) return g;
  }
  ...
  return lengthOrFraction(el, text) ?? GAP_NONE;
}
const GAP_NONE = { px: 0, frac: 0 };
// A length as `{px, frac: 0}`, or a percentage as `{px: 0, frac}` of whatever basis the caller has; null otherwise.
function lengthOrFraction(el, text) {
  const pct = /^(-?\d*\.?\d+)%$/.exec(text);
  if (pct) return { px: 0, frac: parseFloat(pct[1]) / 100 };
  const px = lengthPx(el, text);
  return px == null ? null : { px, frac: 0 };
}
...
function resolveVerticalAlign(el) {
  ...
  const px = len == null ? null : len.frac ? len.frac * lineHeightOf(el) : len.px;
