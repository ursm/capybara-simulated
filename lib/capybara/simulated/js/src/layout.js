...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsGrid(el)) {
      ...
      // column's min / max-content, or a fit-content cap) — 7 values per column. An INTRINSIC side is left as its
...
function trackSideSpec(t, contentW, which) {
  ...
  if (t.pct != null) return [0, (t.pct / 100) * contentW];
  if (t.fit) {
    if (which === 'min') return SIDE_MIN;
    return [3, t.fit.px != null ? t.fit.px : (t.fit.pct != null ? (t.fit.pct / 100) * contentW : Infinity)];
  }
...
function nlTrackSideSpec(t, which) {
  if (t.pct != null && t.px == null) return [4, t.pct / 100];
  if (t.fit && t.fit.px == null && t.fit.pct != null) return which === 'min' ? SIDE_MIN : [5, t.fit.pct / 100];
...
function contentIntrinsicWidths(el) {
  ...
  if (laysOutAsGrid(el)) {
    ...
    const pctTrack = (spec) => spec && spec.pct != null && spec.px == null;
