...
function tableIntrinsicWidths(table) {
  ...
  // intrinsicWidths; a `<col>` sets only cols.spec, so fold it in here. (A `%` column sets cols.pct, not spec,
  // and a percentage is indefinite for intrinsic sizing, so it contributes only its content.)
  let minSum = 0, maxSum = 0;
  for (let i = 0; i < grid.colCount; i++) {
    minSum += Math.max(cols.min[i], cols.spec[i]);
    maxSum += Math.max(cols.max[i], cols.spec[i]);
  }
  return { min: Math.max(minSum + frame, floor), max: Math.max(maxSum + frame, floor) };
}
