...
const NL_STRIDE = 80;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start, 56-61 = declared sizing, 62-67 = flex sizing, 68-70 = replaced intrinsic, 71-76 = out-of-flow, 77-79 = control baseline)
...
// A t1 native TABLE (§17): a border-collapse:SEPARATE, auto-layout `display:table` in normal flow —
// table > (table-row-group | table-row)* > table-cell*, a rectangular grid, LTR, no spans, each cell's
// content itself native-layable. The cell SIZES (column widths × row heights) are resolved by the oracle and
// pushed; native reassembles the tracks and positions everything. The grid is read from the oracle's
// `tableGrid` (the single source of truth for render order, anonymous boxes, and cell coordinates).
// Everything the gate can't satisfy DECLINES to JS.
function nlTableSupported(el) {
  ...
  if (isFloated(el)) return false;
  // table-layout:fixed (t6) is IN scope: fixedColumnWidths distributes the whole assignable width across the
  // columns (auto columns split the remainder; with none, it is spread proportionally over the fixed widths),
  // so the columns FILL the declared width — native reassembles those pushed cell widths and self-sizes to the
  // same box, exactly as for an auto-layout table. (An overflow grows the table via the need-grow below, which
  // native reproduces too.)
  // (A declared table WIDTH is fine — the oracle distributes it into the columns, which the pushed cell widths
  // already reflect. An imposed table HEIGHT is checked below, once the grid is known.)
  ...
  let grouped = 0;
  // Native reassembles the column/row tracks from NON-spanning cells (t2): a column's width comes from a
  // colSpan==1 cell in it, a row's height from a rowSpan==1 cell in it. A column or row that ONLY spanning
  // cells cover is not reconstructible (the oracle distributed a span across it) → decline.
  const colHasSingle = new Array(grid.colCount).fill(false);
  for (const row of grid.rows) {
    ...
    for (const cell of row.cells) {
      ...
      if (cell.colSpan === 1) colHasSingle[cell.col] = true;
  ...
  if (colHasSingle.some((seen) => !seen)) return false; // a column only spanning cells cover → JS
...
function nlIntrinsicMeasurable(el) {
  ...
  const d = displayOf(el);
  if (d === 'contents' || laysOutAsTable(el) || laysOutAsGrid(el)) return false;
  ...
  // A MIXED block (block-level children AND inline content) wraps its inline runs in ANONYMOUS blocks, whose
  // atomics the walk always pushes (they have no record of their own to hang a subtree on), and a pushed
  // atomic's box is not in the run stream `text_intrinsic` reads. So an atomic is only measurable where the
  // element has no block-level child at all — asked once, lazily, and only when an atomic turns up.
  let blockChild = null;
  const hasBlockChild = (parent) => {
    if (blockChild === null) {
      blockChild = false;
      for (const k of layoutChildren(parent)) {
        if (k.nodeType !== NODE_ELEMENT || selfNotRendered(k) || displayOf(k) === 'none') continue;
        const kp = positionOf(k);
        if (kp === 'absolute' || kp === 'fixed') continue;
        const kd = displayOf(k);
        if (kd !== 'contents' && !INLINE_LEVEL.has(kd) && kd !== 'inline') { blockChild = true; break; }
      }
    }
    return blockChild;
  };
  for (const c of layoutChildren(el)) {
    ...
    if (cd === 'inline' && !isFloated(c) && !intrinsicSize(c)) {
      if (!nlInlineMeasurable(c)) return false;
      continue;
    }
    if (INLINE_LEVEL.has(cd) || (cd === 'inline' && !isFloated(c))) {  // an atomic inline: native's own, or its box is pushed
      if (!nlAtomicMeasurable(c, cd) || hasBlockChild(el)) return false;   // an ANONYMOUS block's atomics are all pushed
      continue;
    }
...
// text, `<br>` / `<wbr>`, such inlines, and the atomic inlines native lays out itself. (A different
// `white-space` is declined by the walk itself.)
function nlInlineMeasurable(el) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (cd === 'inline' && !intrinsicSize(c)) {
      if (!nlInlineMeasurable(c)) return false;
    } else if (!nlAtomicMeasurable(c, cd)) return false;
  }
  return true;
}
...
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode, atomic = null) {
  ...
  for (const c of children) {
    ...
    if (c._tag === 'br') { runs.push({ kind: NL_RUN_BR }); hasContent = true; hasReal = true; continue; }
  ...
  return { ok: true, hasContent, hasReal };
}
// An atomic inline native lays out itself: an `inline-block` (a block container inside — not a flex / grid /
// table, whose width can depend on their own layout, the oracle's `_lbFlowRight` growth) or an inline IMAGE
// (its baseline is its bottom margin edge; a text-drawing control's is its font's, still the oracle's), at its
// baseline (no `vertical-align`), whose content native can measure for the shrink-to-fit width.
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  const gridsAll = [];   // flat computed-grid data, referenced by a grid container's rec[55] (grid_start)
  ...
  function walk(el, parentIdx) {
    ...
    // TABLE (§17, t1): a fourth container kind. The cell SIZES are resolved by the oracle (pushed like flex
    // item sizes); native reassembles the column/row tracks and positions every cell / row / row-group and
    // the table box. The structure is emitted from the oracle's `tableGrid` (render order, anonymous-box
    // handling), pushing a record per row-group (2), row (6) and cell (via walk) under the table (4).
    if (laysOutAsTable(el)) {
      ...
      const emitRow = (row, parentIdx) => {
        ...
        for (const cell of row.cells) {
          const ci = nodes.length;
          if (!walk(cell.el, ri)) return false;            // the cell's own subtree (block / text)
          const cr = recs[ci];
          cr[3] = 1;                                        // its used size is a border box…
          cr[4] = cell.el._lb.width;                        // …= its column width
          cr[5] = cell.el._lb.height;                       // …= its (unified) row height
          cr[6] = cr[7] = cr[8] = cr[9] = NaN;              // already final — no re-clamp
          cr[54] = 0;                                       // cell box replayed definite — not self-sized, no two-phase
          ...
          cr[47] = cell.colSpan;                            // native derives tracks from colSpan==1 cells only
          cr[48] = cell.rowSpan;
...
function fixedColumnWidths(table, grid, assignable) {
  ...
  for (const cell of (grid.rows[0] ? grid.rows[0].cells : [])) {
    ...
    const cw = resolveLayoutProp(cell.el, 'width', assignable);
    if (cw == null) continue;
