...
function replacedLaysOutChildren(el) {
  ...
  for (const c of layoutChildren(el)) {
    if (c.nodeType === NODE_ELEMENT && !selfNotRendered(c) && displayOf(c) !== 'none' && c._lb) return true;
  }
  return false;
}
...
function nlIntrinsicMeasurableOf(el) {
  if (intrinsicSize(el)) return nlReplacedLeaf(el);                     // a replaced LEAF: its intrinsic size rides the record
  const d = displayOf(el);
...
function nlAtomicNative(c, cdisp) {
  ...
  // chrome's (`controlBaseline`, rec[77]). A LIST BOX is the exception: it is a block container whose rows
  // native stacks, which this path does not take.
  if (intrinsicSize(c)) return !nlListBoxWithRows(c);
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (nlListBoxWithRows(el)) {
      settleHeightFraction(rec, el);
      rec[100] = rec[101] = rec[102] = rec[103] = rec[104] = rec[105] = NaN;
      rec[3] = 1;                                            // the pinned figures are BORDER boxes
      rec[4] = el._lb.width;
      rec[5] = el._lb.height;
      rec[6] = rec[7] = rec[8] = rec[9] = NaN;               // …and no clamp re-derives them
      rec[56] = el._lb.width;
      rec[57] = rec[58] = NaN;
      rec[61] = 1;
...
function intrinsicSize(el) {
  const size = intrinsicSizeOf(el);
  // …noted only where there IS one: for any other element the answer is a tag check, nothing the oracle computes.
  if (ORACLE_TRACE && size) ORACLE_TRACE.helper('intrinsicSize');
  return size;
}
