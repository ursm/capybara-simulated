...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    // Placement the native engine can't reproduce yet: a horizontal `auto` margin centres a block
    // (§10.3.3) and an `rtl` flow places block children from the inline-start = RIGHT edge — native
    // puts every block at the physical left. Decline the whole subtree to JS rather than misplace it.
    if (!isFlexItem && (e.autoMargins & AUTO_MARGIN_X)) return false;
    if (flowSides(el).rtl) return false;
