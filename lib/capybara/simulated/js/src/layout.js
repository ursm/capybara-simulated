...
import { isLaidOutNode, selfNotRendered, resolveLayoutProp, resolveCascadeDisplay,
         ...
         cascadeDeclaresProperty, inlineDecls } from './cascade.js';
...
const MAIN_AXES = {
  row:              { lead: 'left',   trail: 'right',  leadM: 'ml', trailM: 'mr', reverse: false },
  'row-reverse':    { lead: 'right',  trail: 'left',   leadM: 'mr', trailM: 'ml', reverse: true },
  column:           { lead: 'top',    trail: 'bottom', leadM: 'mt', trailM: 'mb', reverse: false },
  'column-reverse': { lead: 'bottom', trail: 'top',    leadM: 'mb', trailM: 'mt', reverse: true }
};
...
function flexItems(el, content, ctx) {
  const items = [];
  let ordered = false;
  for (const child of layoutChildren(el)) {
...
function mainAxisOffsets(el, items, sizes, gap, free, axis) {
  ...
  const just = autos > 0 && free > 0 ? ZERO_JUSTIFY : justifyOffsets(el, free, n, axis.reverse);
  const offsets = [];
  let at = just.lead;
  for (let i = 0; i < n; i++) {
...
function justifyOffsets(el, free, count, reverse) {
  ...
  switch (k) {
    ...
    case 'space-around':  return free > 0 ? { lead: free / count / 2, between: free / count } : ZERO_JUSTIFY;
    case 'space-evenly':  return free > 0 ? { lead: free / (count + 1), between: free / (count + 1) } : ZERO_JUSTIFY;
...
function layoutFlexRow(el, box, content, ctx, { equalShare = false } = {}) {
  const items = flexItems(el, content, ctx);
  const n = items.length;
  ...
  let rowH = 0;
  const itemsAlign = n ? alignKeyword(declaredValue(el, 'align-items')) : null;
  for (let i = 0; i < n; i++) {
...
function automaticMinHeight(it, content, ctx) {
  if (it.autoMin == null) {
    // …unless the item SCROLLS. §4.5 gives a scroll container an automatic minimum of zero, and
    ...
    if (isScrollContainer(it.child)) return (it.autoMin = 0);
...
function resolveFlexRowWidths(items, box, avail, share) {
  ...
  // A scroll container's automatic minimum is ZERO (§4.5) — an `overflow: hidden` item is squeezed
  ...
  const minOf = (i) => {
    if (minima[i] === undefined) {
      const child = items[i].child;
      minima[i] = isScrollContainer(child) ? -Infinity : minContentWidth(child);
    }
    return minima[i];
  };
  const clampOf = (i, size) => {
