...
import { usedDisplay, DEFAULT_DISPLAY, declaredValue, declaredValueEntry, declaredValueIn, usedOverflow, propagatedOverflow,
         ...
         isListBox, listBoxRows } from './style-proxy.js';
...
const CONTROL_SIZES = {
  ...
  select:   { width: 43,  height: 17 },
  textarea: { width: 195, height: 36 }
};
const CHECKBOX_SIZE = { width: 13, height: 13 };
const FILE_SIZE     = { width: 253, height: 21 };      // the widget is a button + a filename label
const RANGE_SIZE    = { width: 129, height: 16 };
...
const BUTTON_INPUT_LABELS = { submit: 'Submit', reset: 'Reset', button: '', image: '' };
function buttonInputSize(el, type) {
  const value = el._attrs && el._attrs.value;
  const label = value != null ? String(value) : BUTTON_INPUT_LABELS[type];
...
function intrinsicSize(el) {
  ...
  if (t === 'input') {
    ...
    if (BUTTON_INPUT_LABELS[type] !== undefined) return buttonInputSize(el, type);
    return CONTROL_SIZES.input;
  }
  if (t === 'svg') return svgIntrinsic(el);
  // A LISTBOX is as tall as the rows it shows, where a dropdown is one row (Chrome: `size=4` is
  // 70 tall against a dropdown's 19, and a bare `multiple` shows 4 rows).
  if (t === 'select' && isListBox(el)) {
    return { width: CONTROL_SIZES.select.width, height: CONTROL_SIZES.select.height * listBoxRows(el) };
  }
