...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  // TRY to walk `c` as a MEASURED subtree, and on a decline roll every record, run, grid and statistic it
  // emitted back off the streams — so a caller with a FALLBACK (`walkParked` plus a pushed figure) can simply
  // try again, and the WALK decides what native can measure. That is what keeps the table's cells and caption,
  // a shrink-to-fit out-of-flow box and an atomic inline from needing a predicate that ENUMERATES the walk's own
  // refusals: such a list is a second one to keep in sync, and the refusals it missed (a `text-indent`, a sticky
  // child, an auto margin on a DESCENDANT record) took whole passes down. The grid-track and flex-item routes
  // still decide by predicate — their retry has to roll back the CONTAINER's own emission (a grid pushes its
  // track data before its items) and re-marshal it, which is the next step here.
  const walkAttempt = (c, parentIdx) => {
    ...
    if (walkMeasured(c, parentIdx, true)) return true;
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      // align-content:stretch grows the lines from their NATURAL (pre-stretch) cross sizes, but native only
      // has the FINAL (pushed, post-stretch) crosses, from which it recovers the grow EXACTLY only when the
      // lines grow uniformly — every item stretch-fills its line, or none do. A wrap container with a
      // DEFINITE cross whose items MIX stretch-filled and explicit cross sizes can't be recovered → decline.
      ...
      // native only places.
      const nativeSizing = nlFlexNativeSizable(el, plan, items, itemsAlign);
      rec[67] = nativeSizing ? 1 : 0;
      rec[65] |= isReverseFlex(el) ? 4 : 0;                // a *-reverse direction
      if (nativeSizing) nativeFlexRows++;
      if (!nativeSizing && plan.wrapMode !== 'nowrap' && flexAlignContentCode(el) === 6) {
        const crossDefinite = plan.mainIsX ? (resolveLayoutProp(el, 'height', cbH) != null) : true;
        if (crossDefinite) {
          ...
          if (anyStretch && anyPlain) return false;
        }
      }
      rec[32] = items.length > 1 ? axisGap(el, plan.mainGap, plan.mainIsX ? contentW : colMainH) : 0;
      for (const c of items) {
        const itemIdx = nodes.length;
        if (!walkMeasured(c, idx, nativeSizing)) return false;   // build the item's own record + subtree
    ...
    if (laysOutAsGrid(el)) {
      ...
      const cols = needsContent && !nativeIntrinsic ? gridColumnContent(el, tracks.length) : null;
      ...
      for (const c of items) {
        const ci = nodes.length;
        if (!walkMeasured(c, idx, nativeIntrinsic)) return false;   // the item's own subtree (a normal block record)
      ...
      if (nativeIntrinsic) nativeIntrinsicGrids++;              // counted once the grid is really in the pass
      return true;
    }
    ...
    // widths. Two things follow, and BOTH are settled here rather than in Rust, where a subtree it cannot
