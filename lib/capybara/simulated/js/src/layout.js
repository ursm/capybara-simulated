...
const NL_STRIDE = 62;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start, 56-61 = declared sizing)
...
// A native GRID (§12): reproduced by pure REPLAY. The oracle resolves the whole track layout — column sizing
// (fixed / fr / auto / minmax / repeat / auto-fill·fit), row heights, spans, gaps, item margins — and lays out
// every item at its `_lb`; native just holds the container's box and positions each item at its resolved
// offset (an out-of-flow displacement), re-laying-out only the item's OWN subtree. So the track math is never
// re-derived and the gate is minimal: a normal-flow, block-level `display:grid` whose items native can lay
// out. (`inline-grid` is an atomic inline EXCEPT as a flex / grid item, where it blockifies to `grid` and lays
// out as a block-level grid container — see nlGridSupported; an abspos / floated grid is placed by its PARENT's
// out-of-flow replay before it is walked here.)
...
function nlGridSupported(el) {
  ...
  // oracle-resolved box is pushed, its items replayed within it) — admit it there. Elsewhere (root / block
  ...
  const p = positionOf(el);
  // An abspos / fixed grid container is out of flow — its PARENT replays its oracle-resolved box and walk replays
  // its items within it (grid is pure replay anyway), so the position never enters layout. Only sticky declines.
  if (p === 'sticky') return false;
  if (isFloated(el)) return false;
  return true;
}
...
// Whether native can COMPUTE this grid's column tracks, rather than replay the oracle's boxes. The bounded
// subset: an in-flow (static / relative), LTR grid with a parseable column template (fixed lengths, percentages,
// `fr`, and the intrinsic sizes — `auto` / `min|max-content` / `minmax` / `fit-content` — whose content
// contribution native measures itself or, failing that, receives resolved); content-height rows (no
// `grid-auto-rows` length yet); no anonymous item (bare text) floor; and no out-of-flow item (the oracle
// static-corners those). Everything else stays on the REPLAY path (nlGridSupported). measure_grid reproduces
// gridColumnWidths + layoutGrid.
function nlGridComputable(el) {
  if (!nlGridSupported(el)) return false;
  if (isFlexOrGridItem(el)) return false;                  // a grid that is itself a flex/grid item is box-pushed by its parent → replay (1a)
  const p = positionOf(el);
  if (p !== 'static' && p !== 'relative') return false;   // an abspos/fixed grid box is replayed by its parent
  if (flowSides(el).rtl) return false;                     // LTR only (rtl mirrors the columns)
  if (gridRowHeight(el) != null) return false;             // a declared grid-auto-rows length → replay (1a)
  if (hasBareText(el)) return false;                       // an anonymous flex-like item floor → replay (1a)
  const e = edgeInsets(el, el._lbCbW != null ? el._lbCbW : el._lb.width);
  const contentW = el._lb.width - e.left - e.right;
  const gap = axisGap(el, 'column', contentW);
  const tracks = gridTrackList(el, contentW, gap);
  if (!tracks.length) return false;                        // empty / invalid template → replay
  // Intrinsic tracks (auto / min|max-content / minmax / fit-content) ARE admitted (Phase 1b): their base/limit
  // are resolved from the oracle's per-column content at marshal time. Only an EMPTY / unparseable template
  // (handled above) declines.
  for (const c of layoutChildren(el)) {
    if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c) || displayOf(c) === 'none') continue;
    const cp = positionOf(c);
    if (cp === 'absolute' || cp === 'fixed') return false; // an out-of-flow item is static-cornered → replay
  }
  return true;
}
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    rec[60] = isFlexItem && flexParts(el).grow > 0 ? 1 : 0;
    rec[61] = rec[3];
    recs.push(rec);
    ...
    if (laysOutAsFlex(el)) {
      ...
      // read the resolved el._lb rather than just the `height` declaration. A genuinely auto height with no
      // floor is 0 (the flex pass precedes the min/max clamp). Sharing one basis fed the floor into a row's
      // row-gap; gating on "declared" dropped a stretch-definite container's basis to 0. Px gaps are free.
      const heightDefinite = resolveLayoutProp(el, 'height', cbH) != null || el._lb.autoHeight === false;
      ...
      for (const c of layoutChildren(el)) {
        ...
        if (nlReplacedLeaf(c)) {
          ...
          if (!nlGridSupported(c)) return false;           // a grid item (blockified) — walk replays its own items
      ...
      if (plan.wrapMode !== 'nowrap' && flexAlignContentCode(el) === 6) {
      ...
      for (const c of items) {
        ...
        // KEEP min/max-HEIGHT (r[8]/r[9]): re-clamping the pushed height by them is idempotent (the oracle
        // already clamped), but native READS them to reproduce the auto-height two-phase for a flex-item flex
    ...
    // GRID (§12): reproduced by REPLAY. The oracle resolved the track layout and every item's box, so native
    // holds this container's box (pushed) and positions each item — in-flow OR abspos — at its resolved offset
    // from the container's border-box origin, marked out-of-flow (native neither sizes nor re-flows them). Each
    // item's OWN subtree is still laid out natively (walk), then its box is overridden with the oracle's. This
    // is the same push-and-replay the flex / block out-of-flow paths use; the grid track math lives in the JS
    // oracle only.
    if (laysOutAsGrid(el)) {
      ...
      // auto-height back-fill, but native's pushed edges resolve % against cbW — they diverge on the block axis.
      // This mis-sizes BOTH the compute path (measure_grid's auto-height) AND the replay path (the pushed box's
      // border-box floor uses the cbW-resolved edges), so decline the whole grid. (Horizontal % padding is
      // resolved against cbW consistently by both, during width determination, so it stays native.)
      if (el._lb.width !== cbW && (declaredPct(el, 'padding-top') || declaredPct(el, 'padding-bottom'))) return false;
      // COMPUTE path: native sizes the columns (fixed / % / fr / intrinsic) and lays each item out at its track
      // width. Marshal the parsed column template + gaps + per-item (col-start, col-span) into `gridsAll`; the
      // grid container keeps its DECLARED box (native computes it), and each item is walked as a NORMAL block
      // record (not box-pushed) so native sizes the item's subtree at its track width. See measure_grid.
      if (nlGridComputable(el)) {
        const e2 = edgeInsets(el, el._lbCbW != null ? el._lbCbW : el._lb.width);
        const contentW = el._lb.width - e2.left - e2.right;
        const colGap = axisGap(el, 'column', contentW);
        // Row gap resolves the way layoutGrid resolves it — against the container's content height when that is
        // DEFINITE, else `null` (an auto-height grid's box is still 0 at gap time). Marshalled as resolved px.
        const rowBasis = el._lb.autoHeight === false ? (el._lb.height - e2.top - e2.bottom) : null;
        const rowGap = axisGap(el, 'row', rowBasis) || 0;
        const tracks = gridTrackList(el, contentW, colGap);
        const items = [];
        for (const c of layoutChildren(el)) {
          if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c) || displayOf(c) === 'none') continue;
          items.push(c);
        ...
        gridsAll.push(tracks.length, colGap, rowGap);
        ...
        // Per-item placement, in the SAME order native collects children (in-flow element items, doc order).
        for (const c of items) {
      ...
      if (!nlGridSupported(el)) return false;
      rec[2] = 1;                                               // DISPLAY_BLOCK — native holds the box + oof items
      rec[3] = 1; rec[4] = el._lb.width; rec[5] = el._lb.height; // the container holds the oracle's box…
      rec[6] = rec[7] = rec[8] = rec[9] = NaN;                   // …already sized, no min/max re-clamp
      for (const c of layoutChildren(el)) {
        if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c) || displayOf(c) === 'none') continue;
        if (c._nid == null || c._nid < 0) return false;          // generated-content item with no arena id
        const ci = nodes.length;
        if (!walk(c, idx)) return false;                         // the item's own subtree
        const r = recs[ci];
        r[3] = 1; r[4] = c._lb.width; r[5] = c._lb.height;       // its oracle-resolved (track-fitted) box
        r[6] = r[7] = NaN;                                       // width already track-fitted — no width re-clamp
        // KEEP min/max-HEIGHT (r[8]/r[9]): a grid item that is an auto-height flex container (rec[54]=1) two-phases
        // its OWN min/max-height clamp (measure_flex ROW branch) — box = clamp(content, min, max) = the track it
        // filled, items aligned in the pre-clamp content. Re-clamping a stretched item's pushed height (rec[54]=0)
        // by them is idempotent (the track already respected min/max). (A block/replaced grid item respects its
        // block-axis min/max like any block — unlike a table cell, whose block min/max the oracle ignores.)
        r[43] = 1;                                               // out of flow: placed by the displacement below
        r[39] = c._lb.x - el._lb.x;
        r[40] = c._lb.y - el._lb.y;
      }
      return true;
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (laysOutAsGrid(c)) {                              // a grid nested in this block (walk replays its items)
  ...
  return { ok: true, nodes: nodes.length, compared, mismatches, sample, nativeIntrinsicGrids };
};
