...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  for (const c of children) {
    ...
    if (c._tag === 'br') {
      // A `<br clear>` CLEARS the floats before the next line (the oracle's `brClear`), which native's line
      // layout does not model — its `RUN_BR` just ends the line (measured: a `<br clear=left>` beside a 40px
      // float left the block 40 tall where Chrome and the oracle say 58, and nothing caught it because the walk
      // read the tag and not the attribute). Refused whatever the floats are: this function has no float
      // context in scope, and one with nothing to clear is rare enough to pay a decline until native models it.
      if (clearOf(c)) return { ok: false, reason: 'br-clear' };
      runs.push({ kind: NL_RUN_BR, wsMode: wsCode });
      hasContent = true;
      hasReal = true;
      hasBr = true;
      continue;
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    rec[29] = ({ left: 1, right: 2, both: 3 })[clearOf(el)] || 0;
  ...
  for (let i = 0; i < runsAll.length; i++) {
    ...
    if (r.kind === NL_RUN_TEXT) {
      ...
      runBuf[o + 6] = r.metric || 0;   // OPEN/CLOSE edge width (0 for BR)
