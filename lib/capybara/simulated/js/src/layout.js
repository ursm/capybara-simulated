...
// blockifies to flex, and as an ATOMIC INLINE), wrap-reverse, a float, and a WRAPPING auto-height
...
function nlFlexSupported(el) {
  ...
  // A POSITION never enters the flex sizing. An `absolute` / `fixed` container is out of flow — its PARENT
  // replays its oracle-resolved box (insets / static position) and the walk lays out its items within that
  // box — and a `sticky` one is in flow, its box exactly a static one's (the scroll shift is a read-time
  // one, see `stickyDelta`). So nothing here asks what the position is.
  if (isFloated(el)) return false;
...
function nlTableSupported(el) {
  ...
  // which is where tables go further than flex, which still refuses a floated container. Measured over 168
...
function nlGridSupported(el) {
  ...
  // position never enters layout, so nothing here asks for it.
  if (isFloated(el)) return false;
  return true;
}
...
function nlIntrinsicMeasurableOf(el) {
  ...
  // the WIDTH question, and a grid `nlGridSupported` refuses (a floated one, a `display: grid` <button>)
  // still declines when the walk comes to emit it. That is a decline either
...
// mismatches). A declared LENGTH or percentage width ends the question for every arm.
function nlRootAutoWidthIsNotItsRoom(root) {
  ...
  const parent = flatTreeParent(root);                     // a FLEX ITEM: the flex algorithm's, never the room
  return !!parent && parent.nodeType === NODE_ELEMENT && laysOutAsFlex(parent);
}
