...
function nlTableSupported(el) {
  ...
  for (const row of grid.rows) {
    if (!row.el || row.el._nid == null || row.el._nid < 0) return false; // anonymous row → JS
    if (row.group) grouped++;
    let rowHasSingle = false;
    for (const cell of row.cells) {
      if (cell.el._nid == null || cell.el._nid < 0) return false;
      if (displayOf(cell.el) !== 'table-cell') return false; // anonymous / stray cell → JS
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsTable(el)) {
      ...
      const emitRow = (row, parentIdx) => {
        ...
        rr[0] = row.el._nid; rr[1] = parentIdx; rr[2] = 6; rr[22] = -1; // TABLE_ROW, no size
        recs.push(rr);
        for (const cell of row.cells) {
