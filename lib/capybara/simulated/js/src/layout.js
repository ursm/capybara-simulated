...
function nlFlexNativeSizable(el, plan, items, itemsAlign) {
  for (const c of items) {
    ...
    if (align === 'baseline' || align === 'last baseline') return false;
    ...
    if (laysOutAsFlex(el)) {
      ...
      for (const c of items) {
        ...
        if (nativeSizing) {
          ...
          r[65] = scrollsInAxis(c, plan.mainIsX ? 'x' : 'y') ? 1 : 0;
          r[66] = plan.mainIsX
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  if (measuresCell || (box.height === 0 && box.autoHeight !== false)) {
    ...
    const grown = (Math.max(flowY, floatsTo) - content.y) + edge.top + edge.bottom;
