...
const NL_CB_RECT = -2;
const NL_RUN_STRIDE = 9; // inline run record [kind, font, size, ls, ws, line_height, metric, asc, ws_mode]; must match layout::Run
...
function nlIntrinsicMeasurableOf(el) {
  ...
  const ownerWraps = NL_WS_WRAPS.has(ws);   // …asked once, not per child (as `nlGatherRuns` does)
  let hasBlock = false;
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (cd === 'inline' && !isFloated(c) && !intrinsicSize(c)) {
      ...
      // A non-wrapping inline inside a wrapping one is the one shape left out — see `nlGatherRuns`.
      const cws = whiteSpaceOf(c);
      if (WS_MODE[cws] === undefined || (!NL_WS_WRAPS.has(cws) && ownerWraps)) return false;
      if (!nlInlineMeasurable(c, cws, atomicsPushed())) return false;
      continue;
    }
...
// WALK makes per inline (`nlGatherRuns`) — a `white-space` of its own, a non-shift `vertical-align`, an edged
...
// contribution comes off the record instead of failing the pass. `wsMode` is the owning block's mode, and
// `atomicsPushed` says the block hands its atomics no hook (see `nlIntrinsicMeasurable`), which the walk
// threads down the inline chain exactly the same way.
function nlInlineMeasurable(el, wsMode, atomicsPushed) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    // …and an inline of its OWN `white-space` is refused HERE where `nlIntrinsicMeasurableOf` admits one at
    // depth 1: the run stream carries the mode, but this predicate threads one mode down its whole walk, so it
    // cannot answer for a subtree that changes it. Stricter than the walk, which is the safe direction — a
    // decline, never a wrong answer — and the bound to lift if the deeper shapes ever matter.
    if (whiteSpaceOf(c) !== wsMode && cd === 'inline') return false;
    if (cd === 'inline' && !intrinsicSize(c)) {
      if (!nlInlineMeasurable(c, wsMode, atomicsPushed)) return false;
      hasReal = true;
...
// What text native's pen-walk (`text_intrinsic` / `line_layout`) can measure AND break: a tab / CR / FF only
// where the mode PRESERVES it (tab stops are the block's business, so `line_layout` declines one it has to
// advance to — a collapsing mode never measures the whitespace run at all), and, where the owner breaks
// in-word per character (`nlWrapMode` 1 / 3), no ZWJ (the oracle's per-character advance carries the previous
// one); and no soft hyphen, which is a break rule rather than a measure (below). A WIDE character does NOT
// decline: native cuts the same break units the oracle's `charUnits` does (`break_unit_len`).
...
function nlTextMeasurable(text, wrapMode, preserved = true) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (preserved && (u === 0x09 || u === 0x0D || u === 0x0C)) return false;
...
// The `white-space` modes that SOFT-WRAP. The other two (`nowrap`, `pre`) make everything under them one
// unbreakable unit, which is why an inline declaring one inside a wrapping line is refused (`nlGatherRuns`).
const NL_WS_WRAPS = new globalThis.Set(['normal', 'pre-wrap', 'pre-line']);
...
// native lays out ITSELF: it walks the element's subtree as child records of the text block, offsets it by the
// `rel` it is handed (the accumulated `position: relative` of the inlines above it), and returns the record
// index — or NL_ATOMIC_PUSHED to fall back to the pushed box, or NL_ATOMIC_DECLINED where no pushed box may be
// (see `nlAtomicNative`). `pushBlocked()` answers that same question BEFORE the fact, for an atomic no walk is
// attempted on: only the harness knows whether this text block sits in a subtree native MEASURES, where a box
// it cannot re-measure may not appear. Null in a context that has no record to hang a subtree on (an anonymous
// block's group), or whose lines `justify` — neither of which any measured subtree contains.
const NL_ATOMIC_PUSHED = -1;
const NL_ATOMIC_DECLINED = -2;
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode, atomic = null, rel = null, oof = null,
                      ownerOpen = false) {
  ...
  const ownerWraps = NL_WS_WRAPS.has(wsMode);   // …asked once, not per child
  const appendText = (fi, text) => {
    ...
    const mergeable = last && last.kind === NL_RUN_TEXT && last.shift === shift && last.wrapMode === wrapMode &&
                     last.wsMode === wsCode && nlSameFi(last.fi, fi);
  ...
  for (const c of children) {
    if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) {
      ...
      // advance carries the previous character), and a PRESERVED tab — tab stops are the block's, not the
      // run's. Only preserved: a collapsing mode eats the whitespace run without measuring it, which is why
      // tab-INDENTED markup laid out natively all along and must keep doing so. Tested here and not only in
      // the intrinsic predicate, because a run that reaches Rust unmeasurable discards the whole PASS, where
      // the rule is that the walk refuses what native cannot do.
    ...
    // …with one carve-out. An inline that does NOT soft-wrap, inside a line that does, is ONE unbreakable
    // unit: the line must decide BEFORE it whether the whole of it fits (Chrome puts `aaa` alone above
    // `bbb ccc` in an 80px block). Native's breaker is greedy word by word and has no lookahead to the next
    // opportunity, so it would place the first word and overflow the rest — a wrong answer, not a decline.
    // Where the line itself never wraps there is nothing to decide, so that case stays native.
    if (!NL_WS_WRAPS.has(cWs) && ownerWraps) return { ok: false, reason: 'nowrap-inline-unit' };
    ...
    if (openW !== 0 || closeW !== 0 || openPlain !== 0 || closePlain !== 0) {
      ...
      const sub = nlGatherRuns(layoutChildren(c), cf, runs, cWs, cShift, cWrap, atomic, cRel, oof, openW !== 0);
      ...
      const sub = nlGatherRuns(layoutChildren(c), cf, runs, cWs, cShift, cWrap, atomic, cRel, oof, false);
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      const flushGroup = () => {
        ...
        const g = [];
        const gr = nlGatherRuns(group, bf, g, wsMode, 0, nlWrapMode(el));
        group = [];
    ...
    if (hasInline) {
      ...
      const g = nlGatherRuns(layoutChildren(el), bf, local, wsMode, 0, nlWrapMode(el), atomic, null, oofHook);
  ...
  for (let i = 0; i < runsAll.length; i++) {
    ...
    // rides rec[53]: it is what an empty block and the anonymous groups are read under.)
    ...
    segments.forEach((segment, i) => {
      ...
      if (!wraps) {
        ...
        if (body) {
          ...
          if (tab && outerWraps && lineHasContent && overflows(lineX + openEdgeWidth() + runW, lineRight)) {
            forceBreak('wrap');
