...
function nlTableSupported(el) {
  ...
  for (const g of grid.groups) {
    ...
    if (displayOf(g.el) !== 'table-row-group') return false; // thead / tfoot render-reorder → JS
    if (!grid.rows.some((row) => row.group === g)) return false; // empty row group → JS (the oracle boxes it
                                                                 // below the grid; measure_table leaves it unset)
  }
  let grouped = 0;
