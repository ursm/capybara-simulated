...
const NL_STRIDE = 39;   // node record; must match layout::Input / LAYOUT_STRIDE
...
function nlSupported(el) {
  ...
  if (p === 'relative' && nlHasRelativeShift(el)) return false; // native doesn't apply the inset yet → JS
  return true;
}
// A `position: relative` box with a non-zero top/left/right/bottom is SHIFTED off its flow position by
// the oracle (folded into el._lb), but the native engine lays every box at its flow position and carries
// no inset — so a shifted box (and its whole subtree) would be silently mislaid. The extremely common
// zero-inset case (relative purely to be a containing block for abspos children) is unaffected and stays
// native. Harness-only (this runs in the shadow classifier, never on the production path).
function nlHasRelativeShift(el) {
  const r = relativeOffset(el, el._lbCbW != null ? el._lbCbW : null, el._lbCbH != null ? el._lbCbH : null);
  return r.x !== 0 || r.y !== 0;
}
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    // Placement the native engine can't reproduce yet: a horizontal `auto` margin centres the box
    ...
    if (e.autoMargins & AUTO_MARGIN_X) return false;
    ...
    if (laysOutAsFlex(el)) {
      ...
      rec[38] = (plan.mainStart === 'right' || plan.mainStart === 'bottom') ? 1 : 0; // main runs from the far edge
      const items = [];
      for (const c of layoutChildren(el)) {
        ...
        if (positionOf(c) !== 'static') return false;      // abspos / fixed / relative item → JS
        if (isFloated(c)) return false;
        if (c._nid == null || c._nid < 0) return false;    // generated-content item with no arena id
        if (edgeInsets(c, contentW).autoMargins !== 0) return false; // auto margin eats free space → JS
