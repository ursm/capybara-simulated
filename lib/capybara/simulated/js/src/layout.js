...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  for (const c of children) {
    ...
    if (isOutOfFlowChild(c)) {
      ...
      const oi = oof(c);
      if (oi == null) return { ok: false, reason: 'oof-in-inline' };
    ...
    if (isFloated(c)) {
      ...
      if (fi == null) return { ok: false, reason: 'float-in-inline' };
    ...
    if (cdisp === 'inline-block' || cdisp === 'inline-flex' || cdisp === 'inline-table' || cdisp === 'inline-grid' ||
      ...
      if (c._nid == null || c._nid < 0) return { ok: false };
      ...
      if (atomic && nlAtomicNative(c, cdisp)) {
        ...
        if (ci === NL_ATOMIC_DECLINED) return { ok: false };
    ...
    if (cdisp !== 'inline') return { ok: false };               // a block-level box in the IFC → JS
    ...
    if (cWsCode === undefined) return { ok: false };           // a vendor / unparseable value → JS
    ...
    if (cva && cva.mode !== 'shift') return { ok: false };
    ...
    if (cf.nf < 0) return { ok: false };                        // non-system / @font-face run → JS
    ...
    if (openW !== 0 || closeW !== 0 || openPlain !== 0 || closePlain !== 0) {
      ...
      if (fontContentHeight(c) > lineHeightOf(c) + 0.01) return { ok: false };
      ...
      if (!sub.ok || !sub.hasReal) return { ok: false };
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      for (const c of layoutChildren(el)) {
        ...
        if (nlReplacedLeaf(c)) {
          ...
          if (!nlTableSupported(c)) return false;          // a TABLE item — walk emits its grid, native sizes it
      ...
      const emitItems = (native) => {
        rec[67] = native ? 1 : 0;
        if (!native && !pushedCrossRecoverable()) return false;
    ...
    if (laysOutAsTable(el)) {
      if (!nlTableSupported(el)) return false;
    ...
    if (laysOutAsGrid(el)) {
      if (!nlGridSupported(el)) return false;
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (laysOutAsFlex(c)) {                              // a flex container nested in this block
        if (!nlFlexSupported(c) || c._nid == null || c._nid < 0) return false;
        orderedKids.push(c);
        hasNonFloatBlock = true;
        continue;
      }
      if (laysOutAsTable(c)) {                             // a table nested in this block (walk builds it)
        if (!nlTableSupported(c) || c._nid == null || c._nid < 0) return false;
        orderedKids.push(c);
        hasNonFloatBlock = true;
        continue;
      }
      if (laysOutAsGrid(c)) {                              // a grid nested in this block (walk computes its tracks)
        if (!nlGridSupported(c) || c._nid == null || c._nid < 0) return false;
        orderedKids.push(c);
        hasNonFloatBlock = true;
        continue;
      }
      ...
      if (intrinsicSize(c) && !nlReplacedLeaf(c) && !nlListBoxWithRows(c)) return false;
      ...
      return false;                                            // a block-level box native can't place → JS
    }
    ...
    const atomicHook = (parentIdx) => ({
      lay: (c, rel) => {
        ...
        return measured > 0 ? NL_ATOMIC_DECLINED : NL_ATOMIC_PUSHED;
      },
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      if (oof.length) return false;                            // abspos in the mix — defer
      const wsMode = whiteSpaceOf(el);
      const wsCode = WS_MODE[wsMode];
      if (wsCode === undefined || wsMode === 'pre' || wsMode === 'pre-wrap') return false; // preserve mix → JS
      ...
      if (groupIndent && measured > 0) return false;
      ...
      const flushGroup = () => {
        ...
        const kept = emitAttempt(() => {
          ...
          if (!gr.ok) { bailed = true; return false; }
  ...
  if (!walk(root, -1)) return { ok: false, reason: 'unsupported subtree' };
