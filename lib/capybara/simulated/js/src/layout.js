...
function ensureLayout() {
  ...
  //
  // The rendered check inside it reads the PREVIOUS pass's boxes — this one has not run yet — so
  // an element that has only just become rendered establishes its baseline one pass later, which
  // is the conservative direction (it transitions from nothing, as an appearing element should).
