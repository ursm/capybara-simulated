...
//   * one under a GRID item (its basis is the track), a TABLE part, or an out-of-flow box.
...
function nlWalkResolvesPct(el) {
  ...
  return pos === 'absolute' || pos === 'fixed' ||
         laysOutAsGrid(p) || laysOutAsTable(p) || TABLE_INTERNAL_DISPLAY.has(displayOf(el)) ||
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    // that block: an in-flow child of a block or flex container (a table among them), which native resolves at the
    // parent's measure (`Input::with_percent_sizes`). There a PLAIN percentage goes over as a fraction
    // (rec[100..105]). Anywhere else — the pass root, an out-of-flow box, a grid item (its track), a part of a table,
...
function layoutGrid(el, box, content, ctx) {
  ...
  for (const child of layoutChildren(el)) {
    ...
    const size = usedSize(child, avail, declaredRowH != null ? declaredRowH : 0, trackW, content.definiteHeight);
    // A `position: relative` item is shifted by its insets at paint time like any other flow's box (Chrome
    // moves a relative grid item; the flow — its row, its track — is computed from the unshifted place).
    const rel = flowShift(child, pos === 'relative' ? relativeOffset(child, trackW, content.definiteHeight) : null);
