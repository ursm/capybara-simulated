...
//
// What is still out is what the oracle GROWS past its intrinsic figure (`growAtomic`'s caller): an auto-width
// WRAPPING flex container, whose width can depend on its own layout (`_lbFlowRight`). And an `inline-table`,
// whose shrink-to-fit is the table algorithm's rather than a measure.
function nlAtomicNative(c, cdisp) {
  // An intrinsic-size KEYWORD width. The block flow carries one natively (it measures the box and pins its
  // contribution), but an ATOMIC's width is its line's shrink-to-fit and this path does not — so it stays a
  // refusal here, and is answered here so the predicate says what the walk will actually DO: where it does
  // not, a caller that reads "measurable" as "native lays it out" marks a subtree measured that then declines
  // mid-walk, and a `<td>` holding such an inline-block took its whole table down that way. Saying no pushes
  // the cell's own contribution instead (rec[84..85]) and the table is laid out. Asked before the `inline`
  // branch because the walk refuses an inline `<img>` on the same ground.
  if (nlDeclaresSizeKeyword(c)) return false;
  ...
  //
  // A WRAPPING flex container with an AUTO width is the one shape left out, and the condition is the oracle's
  // own (`growAtomic`'s caller): such a box is GROWN from its laid-out extent (`_lbFlowRight`) because its
  // width can depend on its own layout, and `flex_intrinsic_widths` never does that. NOT a question of axis —
  // a column's lines add up (130 for two columns of 50 and 80 where the intrinsic figure knows only the
  // widest item), and a ROW's item that cannot shrink overflows the line it is on just the same (`max-width:
  // 40px` over a `flex-shrink: 0` 80px item: the oracle grows to 80, the intrinsic figure stops at 40). A
  // DECLARED width is never grown — `size.autoWidth` is false — so it stays native.
  if (cdisp === 'inline-flex') {
    if (!nlFlexSupported(c)) return false;   // …first: it rejects on the display / a float / a shrink-wrap
                                            // before either of us builds an axis plan
    // …and the width is asked against the box's OWN basis (`_lbCbW`, what the oracle resolved its percentages
    // against), so a percentage counts as declared here exactly as it does in `size.autoWidth`.
    const grown = wrapsFlexLines(flexAxisPlan(c).wrapMode) &&
                  resolveLayoutProp(c, 'width', c._lbCbW != null ? c._lbCbW : 0) == null;
    return !grown && nlIntrinsicMeasurable(c);
  }
...
// `nlAtomicNative`, which does not and refuses. NO basis: the keyword answer needs none, and a null one returns
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      // Carry this container's OWN autoHeight past the parent-push (which overwrites height with the final box):
      // when it is a flex ITEM whose height is content-derived, native recomputes its cross + two-phases the
      // min/max-height clamp rather than aligning its items in the pushed definite box (measure_flex ROW branch).
      rec[54] = el._lb.autoHeight !== false ? 1 : 0;
      const plan = flexAxisPlan(el);
    ...
    if (laysOutAsGrid(el)) {
      ...
      const contentW = Math.max(0, el._lb.width - e2.left - e2.right);
      const colGap = axisGap(el, 'column', contentW);
      // Row gap resolves the way layoutGrid resolves it — against the container's content height when that
      // was DEFINITE at gap time (`_lbDefiniteH`: the box arrived with a height — declared, stretched, or a
      // parent grid's row — else it is the 0 auto placeholder the flow back-fills later). Marshalled as px.
      const rowBasis = el._lbDefiniteH ? (el._lb.height - e2.top - e2.bottom) : null;
      const rowGap = axisGap(el, 'row', rowBasis) || 0;
      // An empty / invalid template (`none`, an unparseable token) is ONE implicit column the full width.
      let tracks = gridTrackList(el, contentW, colGap);
      if (!tracks.length) tracks = [{ px: contentW, pct: null, fr: null, auto: false, floor: null }];
      ...
      const emitTracksAndItems = (native) => {
        ...
        gridsAll.push(tracks.length, colGap, rowGap, declaredRowH != null ? declaredRowH : NaN);
        for (let i = 0; i < tracks.length; i++) {
          const t = tracks[i];
          let base = trackBaseSpec(t, contentW);
          let limit = t.fr != null ? base : trackLimitSpec(t, contentW);
          if (cols) {
            base = [0, resolveSideSpec(base, cols[i])];
            limit = [0, resolveSideSpec(limit, cols[i])];
          }
