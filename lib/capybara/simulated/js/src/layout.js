...
function placeAbsolute(child, pos, staticX, staticY, ctx, order = null) {
  ...
  // A box held back for an inline was numbered where it sits in the tree, not where it was
  // finally laid out (see the deferral above).
  if (order != null) child._lbOrder = order;
...
function layoutElement(el, box, ctx, cbW = box.width) {
  ...
  const placeTextRun = (text, owner) => {
    ...
    segments.forEach((segment, i) => {
      ...
      if (!wraps) {
        ...
        // Placed whole, so `placeOnLine` saw no white space in it — but a run that ENDS in one
        // still ends the line in one, and the space after it collapses against it.
        lineEndsWithSpace = CSS_WS_RE.test(run.slice(-1));
        return;
      }
  ...
  // An inline box's own padding / border / margin advances the line without being TEXT, so it
  // takes no part in white-space collapsing: the space either side of an empty padded `<b>`
  // still collapses to one (Chrome).
  const placeEdge = (w, h) => {
    const wasSpace = lineEndsWithSpace;
    placeOnLine(w, h, true);
    lineEndsWithSpace = wasSpace;
  };
  ...
  const placeInlineBox = (child, pos) => {
    ...
    if (ce.left) placeEdge(ce.left, ownH);
    ...
    if (ce.right) placeEdge(ce.right, ownH);
    openInlines.pop();
    OPEN_INLINE_BOXES.pop();
    if (ce.mr) placeEdge(ce.mr, ownH);
