...
import { usedDisplay, DEFAULT_DISPLAY, declaredValue, declaredValueEntry, declaredValueIn, usedOverflow,
...
function clipsContent(el) {
  ...
  let cx = usedOverflow(el, 'x') !== 'visible';
  let cy = usedOverflow(el, 'y') !== 'visible';
  if (el === root) {
    cx = cy = false;
  } else if (el === (doc && doc.body) && root) {
    // The body propagates to the viewport unless the ROOT took an overflow of its own — asked as
    // one question, as it always was: the propagation is of the whole `overflow`, not per axis.
    const rootClips = usedOverflow(root, 'x') !== 'visible' || usedOverflow(root, 'y') !== 'visible';
    if (!rootClips) cx = cy = false;
  }
  el._ccX = cx;
  el._ccY = cy;
  return (el._ccVal = cx || cy);
}

// Is `eb` pushed entirely out of `p`'s box in an axis `p` actually clips? Reading `_ccX` / `_ccY`,
// which `clipsContent(p)` has just stamped for this pass.
