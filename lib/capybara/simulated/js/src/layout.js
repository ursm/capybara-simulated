...
function nlTableSupported(el) {
  if (displayOf(el) !== 'table') return false;             // inline-table → JS (atomic shrink-to-fit)
...
function nlAtomicNative(c, cdisp) {
  ...
  // baseline the container's own, which native already derives for a block-level one. An `inline-table` is
  // NOT here: its shrink-to-fit is the table algorithm's, and admitting one moved boxes (corpus 2770 → 2768).
  //
