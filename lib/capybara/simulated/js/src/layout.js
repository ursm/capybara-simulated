...
function nlInlineMeasurable(el, atomicsPushed) {
  ...
  const e = edgeInsets(el, el._lbCbW != null ? el._lbCbW : 0);
...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  for (const c of children) {
    ...
    const e = edgeInsets(c, c._lbCbW != null ? c._lbCbW : 0);
...
function nlIndentOf(owner, el, e) {
  const contentW = Math.max(0, el._lb.width - e.left - e.right);
  const indent = textIndentOf(owner, contentW);
...
// difference over a decimal basis came out an ulp off), then CHECKED against `atBasis`, the edges the record resolved
// at its own basis. Null where they disagree — a math function or clamp that is not linear across them
// (`min(5%, 100px)` bends at 2000, `calc(10px - 0.5%)` clamps at 0) — which the caller resolves against the oracle's
// basis instead.
const NL_EDGE_FIELDS = ['mt', 'mr', 'mb', 'ml', 'top', 'right', 'bottom', 'left'];
const NL_EDGE_PROBE = 1048576;
function nlEdgeParts(el, atBasis, basis) {
  ...
  for (let i = 0; i < 8; i++) {
    if (Math.abs(atBasis[NL_EDGE_FIELDS[i]] - (px[i] + frac[i] * basis)) > 1e-6) return null;
  }
  return { px, frac };
}
...
function nlShadowRun(rootEl, trace) {
  ...
  const pushBorderBox = (r, c, keepHeightClamps) => {
    ...
    if (keepHeightClamps || r[2] === 3) {
      if (!r[61]) {
        const pe = edgeInsets(c, recordCbW(c));               // (its padding may be a fraction on the record)
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    nodes.push(el);
    recIndex.set(el, idx);
    const cbW = recordCbW(el);
    const cbH = el._lbCbH != null ? el._lbCbH : null;
    ...
    if (parentIdx < 0 && blockAxisOf(el, flowSides(el)) === 'width' &&
        resolveLayoutProp(el, 'width', cbW) == null) {
      return false;
    }
    ...
    const size = (slot, prop, basis, info) => {
      if (nativeBasis) {
        ...
        if (spec.frac != null && !underImposedRoot(slot)) { rec[slot] = NaN; rec[slot + 96] = spec.frac; return; }
    ...
    const edgeParts = nativeBasis && el._lbEdgePct ? nlEdgeParts(el, e, cbW) : null;
    if (edgeParts) {
      ...
      rec[10] = e.mt; rec[11] = e.mr; rec[12] = e.mb; rec[13] = e.ml;
      rec[14] = e.top - e.bt; rec[15] = e.right - e.br; rec[16] = e.bottom - e.bb; rec[17] = e.left - e.bl;
    }
    ...
    rec[65] |= autoHeightAdjoinsBottom(el, cbH) ? 16384 : 0;
    ...
    if (laysOutAsFlex(el)) {
      ...
      if ((declaresLayoutProp(el, 'min-height') || declaresLayoutProp(el, 'max-height')) &&
          resolveLayoutProp(el, 'height', cbH) == null && (el._lb.autoHeight === false || outOfFlow)) {
        const vEdge = isBorderBox(el) ? 0 : (e.top + e.bottom);   // clamps are content-basis unless border-box
        const binds = (prop) => {
      ...
      const contentW = () => (oracleW ??= el._lb.width - e.left - e.right);   // the items' containing-block width
      ...
      const colMain = () => {
        if (oracleColMain === undefined) {
          oracleColMain = el._lbDefiniteH ? (el._lb.height - e.top - e.bottom) : null;
          const minH = oracleColMain == null && declaresLayoutProp(el, 'min-height') ? resolveLayoutProp(el, 'min-height', cbH) : null;
          if (minH != null && minH > 0) oracleColMain = Math.max(0, isBorderBox(el) ? minH - (e.top + e.bottom) : minH);
        }
      ...
      const pushedCrossRecoverable = () => {
        ...
        if (plan.mainIsX && resolveLayoutProp(el, 'height', cbH) == null) return true;   // an auto cross grows to its lines
        const crossProp = plan.mainIsX ? 'height' : 'width';
        let anyStretch = false, anyPlain = false;
        for (const c of items) {
          if (stretchesInCross(nlCrossAlign(itemsAlign, c, plan), c, crossProp, edgeInsets(c, contentW()))) anyStretch = true;
          else anyPlain = true;
        }
      ...
      const emitItems = (native) => {
        ...
        for (const c of items) {
          ...
          const ce = edgeInsets(c, contentW());
          ...
          if (native) {
            ...
            r[66] = plan.mainIsX
              ? (((resolveLayoutProp(c, 'height', c._lbCbH != null ? c._lbCbH : null) == null && !ci) || (ci && !ci.ratio)) && stretchesInCross(align, c, 'height', ce) ? 1 : 0)
    ...
    if (laysOutAsGrid(el)) {
      ...
      const emitTracksAndItems = (native) => {
        ...
        for (const c of items) {
          ...
          if (declaredRowH != null && declaredRowH !== 0 && resolveLayoutProp(c, 'height', c._lbCbH != null ? c._lbCbH : null) == null && !intrinsicSize(c)) {
            const r = recs[ci];
            const ce = edgeInsets(c, c._lbCbW != null ? c._lbCbW : 0);
    ...
    for (const c of orderedKids) {
      ...
      if (!keywordWidth) {
        ...
        if (resolveLayoutProp(c, 'width', recordCbW(c)) != null) continue;
      }
      if (!nlIntrinsicMeasurable(c)) return false;
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      const groupIndent = nlIndentOf(flowContainer(el), el, e);
    ...
    if (hasInline) {
      ...
      const ownIndent = nlIndentOf(flowContainer(el), el, e);
    ...
    if (oof.length) {
      nlWriteIndent(rec, nlIndentOf(flowContainer(el), el, e), false);
...
function intrinsicSize(el) {
  if (ORACLE_TRACE) ORACLE_TRACE.helper('intrinsicSize');
  const t = el._tag;
...
function anonymousItemHeight(el) {
  if (ORACLE_TRACE) ORACLE_TRACE.helper('anonymousItemHeight');
