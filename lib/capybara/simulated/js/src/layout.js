...
const NL_RUN_TEXT = 0, NL_RUN_OPEN = 1, NL_RUN_CLOSE = 2, NL_RUN_BR = 3, NL_RUN_ATOMIC = 4, NL_RUN_WBR = 5;
...
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode, atomic = null, rel = null) {
  ...
  const appendText = (fi, text) => {
    ...
    if (mergeable && !glues) last.text += text;
  ...
  for (const c of children) {
    ...
    if (openW !== 0 || closeW !== 0 || openPlain !== 0 || closePlain !== 0) {
      ...
      const sub = nlGatherRuns(layoutChildren(c), cf, runs, wsMode, cShift, cWrap, atomic, cRel);
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  const emitOutOfFlow = (c, el, idx, crossAlignCode) => {
    ...
    if (!measuredSelf && !walkParked(c, idx)) return false;
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (hasInline) {
      ...
      if (oof.length) return false;                            // an abspos child under a text block → JS (no static position in native's line layout)
      ...
      const g = nlGatherRuns(layoutChildren(el), bf, local, wsMode, 0, nlWrapMode(el), atomic);
      if (!g.ok) return false;
      if (!g.hasContent) { rec[2] = 1; return true; }          // only whitespace/empty inlines → empty block
