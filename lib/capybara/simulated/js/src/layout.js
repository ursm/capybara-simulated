...
function nlIntrinsicMeasurableOf(el) {
  ...
  // NATIVELY. It hands the hook to no atomic of a MIXED block (block-level children AND inline content: the
  // anonymous groups wrapping its runs have no record to hang a subtree on) nor of a JUSTIFIED one (the oracle
  // spreads a line's spaces, positions native does not hold) — and it threads that same absence down every
  // nested inline, so ONE fact decides the whole inline formatting context. Asked once, lazily, and only when
  // an atomic turns up anywhere in it.
  let pushed = null;
  const atomicsPushed = () => {
    if (pushed === null) {
      pushed = textAlignOf(flowContainer(el), startsInlineAtRight(el)) === 'justify';
      for (const k of layoutChildren(el)) {
        if (pushed) break;
        if (k.nodeType !== NODE_ELEMENT || selfNotRendered(k) || displayOf(k) === 'none') continue;
        const kp = positionOf(k);
        if (kp === 'absolute' || kp === 'fixed') continue;                // out of flow: in no inline context
        const kd = displayOf(k);
        // Block-LEVEL, so an inline-block / inline-table / inline-flex child does not count (it is an atomic on
        // a line, not a box that splits the flow). A float is block-level here, though the walk declines one
        // alongside inline content anyway, so that arm never decides an answer on its own.
        if (kd !== 'contents' && !INLINE_LEVEL.has(kd) && kd !== 'inline') pushed = true;
      }
    }
    return pushed;
  };
...
//              re-measure may not appear. Null in a context that has no record to hang a subtree on (an
//              anonymous block's group), or whose lines `justify` — neither of which any measured subtree
//              contains.
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      // (`justify`, which native cannot spread, maps to 0 here as it does in the text-block branch — a mixed
      // group's atomics are pushed either way). Without this an rtl mixed block laid its lines out ltr.
      ...
      // Returns false only on a genuine bail (a run the text path can't build).
      const flushGroup = () => {
        if (!group.length) return true;
        const g = [];
        const gr = nlGatherRuns(group, bf, {runs: g, block: el, atomic: null, oof: null}, wsMode, 0, nlWrapMode(el));
        group = [];
        if (!gr.ok) return false;
        if (!gr.hasContent) return true;                       // whitespace-only group collapses (no box)
    ...
    if (hasInline) {
      ...
      const local = [];
      // An atomic inline the walk lays out natively becomes a child record of THIS text block; one whose subtree
      // declines is rolled back off the record stream and keeps the pushed box.
      const atomic = align === 'justify' ? null : {
        lay: (c, rel) => {
          const ci = nodes.length;
          if (walkAttempt(c, idx)) {
