...
function nlFlexSupported(el) {
  ...
  // like a block does, so a row may declare it. Still deferred: a COLUMN's min/max-HEIGHT is the MAIN clamp
  // (a min-height FLOOR that only divides once the content underflows it, a max-height CAPACITY that breaks
  // lines — two-phase); and a COLUMN's cross axis is width, so min/max-WIDTH there is its cross clamp. (A
  // ROW's min/max-width is its MAIN clamp, which native already applies in resolve_width.)
  if (!plan.mainIsX && (declaresLayoutProp(el, 'min-height') || declaresLayoutProp(el, 'max-height'))) return false;
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      // A column's row-gap % resolves against its DEFINITE main height — null (→ 0) when auto, matching the
      // oracle's `definiteMainHeight` (min/max-height is bailed, so no floor). Px gaps are basis-independent.
      const contentH = resolveLayoutProp(el, 'height', cbH) != null ? (el._lb.height - e.top - e.bottom) : 0;
      ...
      rec[32] = items.length > 1 ? axisGap(el, plan.mainGap, plan.mainIsX ? contentW : contentH) : 0;
      for (const c of items) {
