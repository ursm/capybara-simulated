...
function nlIntrinsicMeasurableOf(el) {
  ...
  if (WS_MODE[ws] === undefined) return false;
  let hasBlock = false;
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (cd === 'inline' && !isFloated(c) && !intrinsicSize(c)) {
      ...
      if (WS_MODE[whiteSpaceOf(c)] === undefined) return false;          // a vendor / unparseable value → JS
  ...
  // does not reach it.
  return laysOutAsGrid(el) || !(hasBlock && (ws === 'nowrap' || ws === 'pre'));
}
...
// (`inlineStyleOwner`). The gate has to mirror exactly what `nlGatherRuns` will refuse on the same node,
...
function nlSplicedTextMeasurable(c, from, wrapMode) {
  ...
  if (WS_MODE[ws] === undefined) return false;                      // …`inline-white-space-unparseable`
...
// (normal/pre-wrap/pre-line), and a NEWLINE forces a break (pre/pre-wrap/pre-line). Everything else declines.
const WS_MODE = { normal: 0, nowrap: 1, pre: 2, 'pre-wrap': 3, 'pre-line': 4 };
...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false, styledBy = null, from = null) {
  ...
  // collapsed instead of declining. Unreachable today (the `WS_MODE` gate declines `break-spaces` first) and
  // exactly the shape of bug the fifth `whiteSpaceOnlyIsContent` asker was.
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      const wsCode = WS_MODE[wsMode];
      if (wsCode === undefined || wsMode === 'pre' || wsMode === 'pre-wrap') return nlNo('preserve-white-space-in-mixed-block');
    ...
    if (hasInline) {
      ...
      // its inline elements (text runs per font, OPEN/CLOSE edges, BR). Bails to JS on an atomic/inline-
      // block, non-normal white-space, a non-system-font run, or an empty edged inline (nlGatherRuns).
      ...
      // `pre-line` collapses spaces but newline-breaks + soft-wraps. All five are modelled (WS_MODE code in
      // rec[53]); anything else (a vendor / unparseable value) declines.
      const wsMode = whiteSpaceOf(el);
      const wsCode = WS_MODE[wsMode];
