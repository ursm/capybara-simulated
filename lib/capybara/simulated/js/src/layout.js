...
function nlTableSupported(el) {
  ...
  for (const row of grid.rows) {
    ...
    for (const cell of row.cells) {
      // Stray non-cell content now arrives WRAPPED in an anonymous cell (anonTableCell) that has no `_nid`, so
      // line 338 already declines any table holding one. A real cell always has `display: table-cell`; the check
      // below is thus a defensive assertion (never false today), kept to pin the contract native relies on.
      if (cell.el._nid == null || cell.el._nid < 0) return false;
...
function nlShadowRun(rootEl, trace) {
  ...
  for (const el of nodes) {
    if (el == null) continue;                          // an ANONYMOUS row (t9): native laid it out, but it has
                                                       // no element / _lb to compare — only its cells are checked
...
function flatTreeChildren(el) {
  ...
  if (!el._pseudo && documentHasGeneratedContent() && !(NO_GENERATED_CONTENT.has(el._tag) && !isBaseSelect(el))) {
...
function baselineCandidates(el, last, inlineBlock = false) {
  ...
  for (const child of layoutChildren(el)) {
    ...
    if (inlineBlock && laysOutAsTable(child)) continue;
    kids.push(child);
  }
  if (laysOutAsFlex(el)) {
