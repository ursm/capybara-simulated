...
function nlTableSupported(el) {
  ...
  // An IMPOSED table height (declared / attribute / min / max) is reproducible only when the oracle DISTRIBUTED
  // it into the rows so the tracks FILL the box — a declared/min height TALLER than the grid does this (Chrome
  // shares the excess over the rows). A too-small declared / max height leaves the box below the grid with the
  // rows overflowing, and a min-height above nothing to distribute leaves empty space; native self-sizes from
  // Σtracks, so it can't reproduce a box the tracks don't fill — decline those. A caption or a collapsed border
  // adds extents outside the plain row tracks, so an imposed height alongside either is likewise declined.
...
function layoutTable(el, box, content, ctx) {
  ...
  const tabEdge = edgeInsets(el, box.width);
  const tMinH = resolveLayoutProp(el, 'min-height', el._lbCbH);
  const tMinContentH = tMinH != null && tMinH >= 0 ? (isBorderBox(el) ? Math.max(0, tMinH - tabEdge.top - tabEdge.bottom) : tMinH) : 0;
  ...
  if (imposedContentH > 0 && hasGrid) {
    const room = imposedContentH - (y - content.y);
    if (room > 0) {
      const autos = [];
  ...
  if (box.height === 0) {
    box.height = Math.max(0, y - content.y) + tabEdge.top + tabEdge.bottom;   // `tabEdge` = this table's own edges
  }
  stampExtent(el, box);
}
