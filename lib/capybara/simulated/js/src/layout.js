...
const NL_RUN_TEXT = 0, NL_RUN_OPEN = 1, NL_RUN_CLOSE = 2, NL_RUN_BR = 3;
...
function nlGatherRuns(el, owner, runs) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (displayOf(c) !== 'inline') return { ok: false };        // atomic / inline-block / block in the IFC
    if (intrinsicSize(c)) return { ok: false };                 // an INLINE replaced element (svg / img) is an atomic inline: it has intrinsic size and a baseline that grow the line box, which the text path doesn't model
