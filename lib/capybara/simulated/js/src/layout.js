...
function nlTableSupported(el) {
  ...
  if (grid.captions.length === 1) {
    ...
    // A caption WIDER than the table's content box overflows it without growing the table (a `%` width; a
    // definite length grows the table so it fits). Native's grid_w.max(caption_w) wrapper would over-grow to
    // the overflowing caption, so decline — the oracle lays it out correctly.
    const te = edgeInsets(el, el._lb ? el._lb.width : 0);
    const tContentW = (el._lb ? el._lb.width : 0) - te.left - te.right;
    if (cap._lb && cap._lb.width > tContentW + 0.5) return false;
  }
...
function ensureCollapseBorders(table) {
  ...
  const tbT = usedBorderWidth(table, 'top', 0, COLLAPSE_EDGE_INFO, tdv);
  const tbR = usedBorderWidth(table, 'right', 0, COLLAPSE_EDGE_INFO, tdv);
  const tbB = usedBorderWidth(table, 'bottom', 0, COLLAPSE_EDGE_INFO, tdv);
  const tbL = usedBorderWidth(table, 'left', 0, COLLAPSE_EDGE_INFO, tdv);
  ...
  for (const row of grid.rows) {
    for (const cell of row.cells) {
      ...
      cell._rbT = usedBorderWidth(el, 'top', 0, COLLAPSE_EDGE_INFO, dv);
      cell._rbR = usedBorderWidth(el, 'right', 0, COLLAPSE_EDGE_INFO, dv);
      cell._rbB = usedBorderWidth(el, 'bottom', 0, COLLAPSE_EDGE_INFO, dv);
      cell._rbL = usedBorderWidth(el, 'left', 0, COLLAPSE_EDGE_INFO, dv);
  ...
  // The widest facing border across an edge — one edge can abut several cells, each offering the
  // side that faces this one (`prop`), and the widest wins (that whole border is what the two split).
  const facingRows = (r0, rs, col, prop) => {
    let w = 0;
    for (let dr = 0; dr < rs && r0 + dr < rowCount; dr++) {
      const nb = occ[(r0 + dr) * n + col];
      if (nb && nb[prop] > w) w = nb[prop];
    }
    return w;
  };
  const facingCols = (c0, cs, row, prop) => {
    let w = 0;
    const base = row * n;
  ...
  // unchanged siblings.
  let changed = false;
  let outT = tbT, outR = tbR, outB = tbB, outL = tbL;
  for (const row of grid.rows) {
    for (const cell of row.cells) {
      const atLeft = cell.col === 0, atRight = cell.col + cell.colSpan >= n;
      const atTop = cell.row === 0, atBottom = cell.row + cell.rowSpan >= rowCount;
      const left   = atLeft   ? tbL : facingRows(cell.row, cell.rowSpan, cell.col - 1, '_rbR');
      const right  = atRight  ? tbR : facingRows(cell.row, cell.rowSpan, cell.col + cell.colSpan, '_rbL');
      const top    = atTop    ? tbT : facingCols(cell.col, cell.colSpan, cell.row - 1, '_rbB');
      const bottom = atBottom ? tbB : facingCols(cell.col, cell.colSpan, cell.row + cell.rowSpan, '_rbT');
      const bl = Math.max(cell._rbL, left) / 2, br = Math.max(cell._rbR, right) / 2;
      const bt = Math.max(cell._rbT, top) / 2, bb = Math.max(cell._rbB, bottom) / 2;
      ...
      if (atLeft   && cell._rbL > outL) outL = cell._rbL;
      if (atRight  && cell._rbR > outR) outR = cell._rbR;
      if (atTop    && cell._rbT > outT) outT = cell._rbT;
      if (atBottom && cell._rbB > outB) outB = cell._rbB;
    }
  }
  const o = table._lbCollapseOuter;
  if (!o) { table._lbCollapseOuter = { top: outT / 2, right: outR / 2, bottom: outB / 2, left: outL / 2 }; changed = true; }
  else {
    if (o.top !== outT / 2 || o.right !== outR / 2 || o.bottom !== outB / 2 || o.left !== outL / 2) changed = true;
    o.top = outT / 2; o.right = outR / 2; o.bottom = outB / 2; o.left = outL / 2;
  }
...
// The table CONTENT width a caption demands (§17.5.2): the table is never narrower than the caption's own used
// outer width, and then distributes that width across its columns (Chrome: a `width: 300px` caption stretches a
// 156px grid's columns to fill 300, exactly like an explicit table width). intrinsicWidths pins a definite
// width and takes a `%`/auto one as its content MIN-width (a `%` is indefinite while the table width is still
// being decided — an unbreakable word still pushes the table out, but a `%` does not). That border box is
// clamped by the caption's own `min|max-width` (a definite LENGTH; a `%` min/max is likewise indefinite here)
// and its margins are added, since Chrome floors at the caption's MARGIN box (`margin:10;width:300` → 320).
...
// The min table CONTENT width its captions require, 0 when there are none.
...
function tableIntrinsicWidths(table) {
  ...
  for (const caption of grid.captions) {
    const c = captionOuterWidth(caption) + edge.left + edge.right;
    if (c > floor) floor = c;
  }
...
function layoutTable(el, box, content, ctx) {
  ...
  // A caption wider than the table floors its width (§17.5.2), and the columns then distribute over the floored
  // width — so fold the caption floor into `content.width` BEFORE sizing the columns (they stretch to fill it).
  // For an auto-width table this is a no-op: its used width already came through tableIntrinsicWidths, which
  // carries the same floor. It bites only when the table's width is explicitly SMALLER than a wide caption.
  const capFloor = captionsFloor(grid);
  if (capFloor > content.width) { box.width += capFloor - content.width; content.width = capFloor; }
  ...
  const layCaption = (caption) => {
    // A table-caption is an ordinary block laid out in the table's content width (§17.4): an AUTO width fills
    // that width minus its own margins (auto horizontal margins take the leftover per §10.3.3 — a narrower
    // caption centres), a declared width / `%` / `min|max-width` and the height are honored (usedSize clamps
    // and returns a border box), and the MARGIN box drives the vertical flow. A caption with a definite width
    // wider than the grid has ALREADY grown the table (tableIntrinsicWidths, which stretches the columns to
    // fill it, exactly like an explicit table width), so `content.width` is final here.
    const cm = edgeInsets(caption, content.width);
    const availW = Math.max(0, content.width - cm.ml - cm.mr);
    const size = usedSize(caption, availW, 0, content.width, content.height || null);
    const lead = autoMarginSplit(cm, 'left', 'right', content.width, size.width).lead;
    ...
    const rel = flowShift(caption, positionOf(caption) === 'relative' ? relativeOffset(caption, content.width, content.height || null) : null);
    const capBox = {
      x: content.x + lead + (rel ? rel.x : 0), y: y + cm.mt + (rel ? rel.y : 0),
      width: size.width, height: size.height, autoHeight: size.autoHeight
    };
  ...
  // `y` already carries the last row's spacing; the collapse bottom-border half is in `edge.bottom`.
