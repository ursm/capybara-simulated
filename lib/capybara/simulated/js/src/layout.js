...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (isFloated(c)) {
        // Slice 1 handles only a float whose OWN parent establishes the float context (fresh, one hop),
        // is in normal flow, and has a resolved (non-shrink-to-fit) width — otherwise defer to JS.
        if (cpos !== 'static') return false;
        if (!establishesBFC(el, displayOf(el))) return false;
