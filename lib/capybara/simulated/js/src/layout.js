...
const NL_STRIDE = 28;   // node record; must match layout::Input / LAYOUT_STRIDE
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    // Classify children into block-level boxes vs inline content (text + inline elements).
    const blockKids = [];
    let hasInline = false;
    for (const c of layoutChildren(el)) {
      if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) {
      ...
      if (nlSupported(c) && c._nid != null && c._nid >= 0) { blockKids.push(c); continue; }
      return false;                                            // inline-block / flex / float / abspos → JS
    }
    if (blockKids.length > 0 && hasInline) return false;       // mixed block + inline (anonymous blocks) → JS
    ...
    // BLOCK container (all block children, or empty).
    rec[2] = 1;
    for (const c of blockKids) {
