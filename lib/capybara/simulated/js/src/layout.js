...
// Concatenate the inline text of `el` (a text-block), descending through inline elements — but ONLY
// while every inline element inherits `blockFont` exactly, carries no edges, is not <br>, and keeps
// white-space:normal. Returns the folded text, or null when an inline construct L2 can't model appears
// (a different font, edges, a hard break, an atomic inline / block inside the IFC) → caller falls to JS.
function nlGatherIFCText(el, blockFont) {
  let text = '';
  for (const c of layoutChildren(el)) {
    if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) { text += c._data || ''; continue; }
    if (c.nodeType !== NODE_ELEMENT) continue;
    if (selfNotRendered(c) || displayOf(c) === 'none') continue;
    if (displayOf(c) !== 'inline') return null;              // atomic / inline-block / block in the IFC
    if (c._tag === 'br') return null;                        // hard break → later stage
    if (whiteSpaceOf(c) !== 'normal') return null;
    if (fontOf(c) !== blockFont) return null;                // bold / italic / size change → later stage
    if (nlHasInlineEdges(c)) return null;                    // margin/border/padding → later stage
    const sub = nlGatherIFCText(c, blockFont);
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  // text, no element children) — a mix, or any inline/floated/abspos/unsupported element child, bails.
  function walk(el, parentIdx) {
    ...
    rec[22] = -1;   // font handle (text blocks fill it)
    recs.push(rec);
    ...
    if (hasInline) {
      // TEXT BLOCK (inline formatting context). L2 handles a SINGLE font: gather the text across the
      // block's own text nodes AND any inline elements that inherit the same font and carry no edges
      // (a/span/… ); a different font (bold/italic/size), inline edges, an atomic inline, <br>, or a
      // non-normal white-space bails the whole subtree to JS (later stages).
      if (whiteSpaceOf(el) !== 'normal') return false;
      const f = fontOf(el);
      const nf = nlNativeFontFor(el);
      if (!f || nf < 0) return false;                          // non-system / @font-face → JS
      const text = nlGatherIFCText(el, f);
      if (text == null) return false;                          // an inline construct L2 can't fold → JS
      if (text.trim() === '') { rec[2] = 1; return true; }     // only empty inlines → an empty block
      rec[2] = 2;                                              // DISPLAY_TEXT_BLOCK
      rec[22] = nf;
      rec[23] = f.size;
      rec[24] = f.ls;
      rec[25] = f.ws;
      rec[26] = lineHeightOf(el);
      texts[idx] = text;
      return true;
    }
  ...
  if (!d.layoutPass(buf, texts, root._lb.x, root._lb.y, rootCbW)) return { ok: false, reason: 'native declined' };
