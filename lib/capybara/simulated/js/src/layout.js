...
import { usedDisplay, DEFAULT_DISPLAY, declaredValue, computedFontSizePx, computedLineHeight } from './style-proxy.js';
...
function layoutElement(el, box, ctx) {
  ...
  for (const child of layoutChildren(el)) {
    ...
    if (isInlineLevel(child)) {
      ...
      const inlineW = Math.min(textWidth(subtreeTextLength(child)), content.width);
      const ce   = edgeInsets(child, content.width);
      const size = usedSize(child, inlineW + ce.left + ce.right, lineHeightOf(child) + ce.top + ce.bottom,
...
function textHeight(el, width) {
  ...
  return Math.ceil(len / perLine) * lineHeightOf(el);
}
...
function lineHeightOf(el) {
  ...
  const ownLh = declaredValue(el, 'line-height');
  const ownFs = declaredValue(el, 'font-size');
  const parent = el._parent;
  if (ownLh == null && ownFs == null && parent && parent.nodeType === NODE_ELEMENT) {
  ...
  el._lbLh = Math.round(isFinite(px) ? px : (computedFontSizePx(el) || 16) * NORMAL_LINE_FACTOR);
  el._lbLhPass = layoutPass;
  return el._lbLh;
}
