...
// fit; flex layout along both axes (line breaking, grow / shrink distribution, alignment); a coarse
...
//   - the flex axes are PHYSICAL: `flex-direction: row` runs left-to-right whatever the container's
//     `writing-mode` / `direction` says, so a `vertical-rl` row stacks across the page instead of
//     down it and an RTL row packs from the left (the css-flexbox writing-mode matrix);
...
function usedBorderWidth(el, side, cbW, info, dv) {
  ...
  // No width at all means the initial `medium`, which is 3px wherever the style paints.
  return bw == null ? BORDER_WIDTH_KEYWORD_PX.medium : bw;
}
...
function relativeOffset(el, cbW = null, cbH = null) {
  ...
  // The shift is folded straight into the box the caller builds, so the FLOW position it moved
  // from is gone by the time anything reads `_lb` — and the scrollable overflow region needs it:
  // a relatively-positioned child extends its scroll container's region from where it SITS, but
  ...
  // it — while `left: 5px` reports 130, the unshifted edge plus the padding). Stamped per pass, so
  // a box that stops being relative cannot leave one behind.
  el._lbRel = (x || y) ? { x, y } : null;
...
// Which PHYSICAL edge each flex axis starts at for this container's flow. `flowSides` already
// resolves writing-mode and direction into physical sides, so `row` is the inline axis whichever
// way that runs — vertical in a vertical writing mode, right-to-left in an RTL one — and `column`
// is the block axis. `-reverse` takes the far edge of its axis, and `wrap-reverse` does the same to
// the cross one.
function flexPhysicalAxes(el) {
  ...
  const crossFlip = flexWrapMode(el) === 'wrap-reverse';
  ...
  const mainStart = sides[main];
  return { mainStart, crossStart: sides[cross], crossFlip, inlineMain: !blockMain, mode: sides.mode,
...
function placeFlexOutOfFlow(el, box, content, ctx, outOfFlow, axis, itemsAlign) {
  ...
  // the page. `justifyOffsets` and `crossOffset` answer a distance from the axis's own START, which
  // is all the alignment needs to know; only the last step cares which edge that is.
  const axes = flexPhysicalAxes(el);
...
function crossAlignPhysical(containerAlign, child, axes, baselineMode = 'axis') {
  ...
  if (axes == null || !axes.crossFlip) return align;
...
function stackFlexLines(el, lines, start, containerCross, crossGap, crossFlip) {
  let stacked = crossGap * Math.max(0, lines.length - 1);
  for (const line of lines) stacked += line.cross;
  const spread = alignContentLines(el, lines.length, containerCross - stacked, crossFlip);
...
function alignContentLines(el, count, freeCross, crossFlip) {
...
function layoutFlexRow(el, box, content, ctx, { equalShare = false } = {}) {
  ...
  const gap = items.length > 1 ? axisGap(el, 'column', content.width) : 0;
  ...
  const wrap = (!equalShare && items.length && declaresLayoutProp(el, 'flex-wrap'))
    ? flexWrapMode(el) : 'nowrap';
  const multiline = wrapsFlexLines(wrap);
  const crossFlip = wrap === 'wrap-reverse';
  const axes = crossFlip ? flexPhysicalAxes(el) : null;
  ...
  const crossGap = lines.length > 1 ? axisGap(el, 'row', content.height || 0) : 0;
  ...
  for (const line of lines) {
    ...
    const offsets = mainAxisOffsets(el, members, widths, gap, content.width - used, axis);
    ...
    for (let i = 0; i < n; i++) {
      ...
      it.size = size;
      it.align = crossAlignPhysical(itemsAlign, child, axes, 'keep');
  ...
  if (!multiline) {
    ...
    stackFlexLines(el, lines, content.y, containerCross, crossGap, crossFlip);
  }
  ...
  for (const line of lines) {
    for (const it of line.members) {
      ...
      const m = edges.autoMargins ? autoMarginSplit(edges, 'top', 'bottom', line.cross, b.height)
                                  : { lead: edges.mt, trail: edges.mb };
      const off = edges.autoMargins ? 0
...
function layoutFlexColumn(el, box, content, ctx) {
  ...
  const wrap = (n && declaresLayoutProp(el, 'flex-wrap')) ? flexWrapMode(el) : 'nowrap';
  ...
  for (const it of items) {
    ...
    it.align = crossAlignPhysical(itemsAlign, child, axes);
  ...
  const crossGap = lines.length > 1 ? axisGap(el, 'column', content.width) : 0;
  ...
  if (multiline) {
    ...
    stackFlexLines(el, lines, content.x, content.width, crossGap, crossFlip);
  ...
  for (const line of lines) {
    ...
    const offsets = mainAxisOffsets(el, members, heights, gap, extent - used, axis);
...
function shiftRecordedRuns(from, to, dy) {
  if (!RUN_LIST || !dy) return;
  for (let i = from; i < to; i++) { RUN_LIST[i].y += dy; RUN_LIST[i].baseline += dy; }
}
    ...
    if (waiting) {
      ...
      for (const w of waiting) {
        const ext = w.child._lb && clipsContent(w.child)
          ? { left: w.child._lb.x, top: w.child._lb.y,
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const forceBreak = () => {
    ...
    if (linePlaced) {
      for (const box of lineBoxes) {
        ...
        // at the line's top.
        shiftRecordedRuns(box.runsFrom, box.runsTo, dy);
      }
  ...
  const breakLine = () => {
    if (!linePlaced && lineX === lineLeft) return;
    forceBreak();
  };
  ...
  const placeTextRun = (text, owner) => {
    ...
    segments.forEach((segment, i) => {
      ...
      if (!run) return;
      if (!wraps) {
        const runW = measureRun(run, owner);
        const at0 = placeOnLine(runW, lh, !outerWraps, false, false, ownAsc);
        if (rec) noteRun(lineRuns, run, at0, owner, runW);
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        if (CSS_WS_RE.test(token)) {
          ...
          if (lineX > lineLeft || preserves) {
            const tokenW = preserves ? measureRun(token, owner) : spaceW;
            const at = placeOnLine(tokenW, lh, true, !preserves, false, ownAsc);
            if (rec && preserves) noteRun(lineRuns, token, at, owner, tokenW);
          }
          continue;
        }
        ...
        for (const unit of split.units) {
          ...
          if (lineHasContent && lineX + openEdgeWidth() + w > lineRight) forceBreak();
  ...
  const placeOnLine = (w, h, decided, hangs, edge, asc = h) => {
    ...
    if (!decided && lineHasContent && lineX + pending + w > lineRight) forceBreak();
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (pos === 'absolute' || pos === 'fixed') {
      ...
      placeAbsolute(child, pos, lineX, flowY, ctx, null, staticAlignFor());
      return true;
    }
    ...
    // its whole border box. So does an `inline-flex`, now that a flex container can
    // say what its items want: an icon-and-label badge is 56.47 wide in Chrome,
    // where the text estimate saw only the label's 40.47 and, once the row could
    // wrap, put the label on a second line. Everything else inline is as wide as
    // the text it holds, plus its own edges.
    const contentSized = laysOutAsTable(child) || laysOutAsFlex(child) || shrinkWrapsToFit(child);
    ...
    const autoInlineW = contentSized      ? shrinkToFitWidth(child, content.width)
                      : ratioOnlyBox(child) ? Math.max(0, content.width - ce.ml - ce.mr)
...
function stampExtent(el, box) {
  ...
  for (const child of layoutChildren(el)) {
    ...
    if (child._lb && clipsContent(child)) {
      const own = child._lb;
      ce = {
        left:   child._ccX ? own.x                : (ce ? ce.left   : own.x),
        top:    child._ccY ? own.y                : (ce ? ce.top    : own.y),
    ...
    if (cx - m.ml               < iLeft)   iLeft   = cx - m.ml;
    if (cy - m.mt               < iTop)    iTop    = cy - m.mt;
    if (cx + cb.width  + m.mr   > iRight)  iRight  = cx + cb.width  + m.mr;
    if (cy + cb.height + m.mb   > iBottom) iBottom = cy + cb.height + m.mb;
  }
  el._lbExt     = { left, top, right, bottom, iLeft, iTop, iRight, iBottom };
...
function flexIntrinsicWidths(el) {
  const column = isColumnFlex(el);
  const wrap = !column && declaresLayoutProp(el, 'flex-wrap') && wrapsFlexLines(flexWrapMode(el));
  ...
  if (!column && count > 1) {
    const gaps = axisGap(el, 'column', null) * (count - 1);
    max += gaps;
    if (!wrap) min += gaps;
  }
  return { min, max };
...
function contentIntrinsicWidths(el) {
  ...
  // …and whether the line holds INLINE content, which is what decides a collapsible space: the
  // white space between two floats is not on a line at all (Chrome: a box holding two 50px floats
  // written on one source line wants 100, not 104.45).
  let inlineOnLine = false;
  // A collapsible space only counts once something follows it on the line — a TRAILING one hangs
  // at the end and contributes nothing (Chrome: `<td>td` and `<td>td\n` are the same width, and
  // measuring the newline made one column of an evenly-split table 14px wider than the other).
  let pendingSpace = 0;
  const takePending = () => { line += pendingSpace; pendingSpace = 0; };
  const endLine = () => { if (line > max) max = line; line = 0; inlineOnLine = false; pendingSpace = 0; };
  const widest = (w) => { if (w > min) min = w; };
  for (const child of layoutChildren(el)) {
    if (child.nodeType === 3 /* text */) {
      const t = child._data || child.data || '';
      if (!/\S/.test(t)) { if (inlineOnLine) pendingSpace = measureRun(' ', el); continue; }
      takePending();
      const run = collapseRun(t, el, false);
      // A TRAILING collapsible space hangs at the end of the line and counts only if something
      // follows it there — the newline before a closing tag is the everyday case, and measuring it
      // made one column of an evenly-split table a space wider than the other.
      const trails = CSS_WS_RE.test(run.slice(-1));
      const body = trails ? run.slice(0, -1) : run;
      line += measureRun(body, el);
      widest(longestWordWidth(body, el));
      inlineOnLine = true;
      if (trails) pendingSpace = measureRun(' ', el);
      continue;
    }
    if (child.nodeType !== NODE_ELEMENT || selfNotRendered(child)) continue;
    ...
    if (pos === 'absolute' || pos === 'fixed') continue;   // out of flow: sizes nothing
    const w = intrinsicWidths(child);
    if (isInlineLevel(child)) { takePending(); line += w.max; widest(w.min); inlineOnLine = true; continue; }
    // A FLOAT packs beside its neighbours exactly as an inline-level box does — a box holding two
    // 50px floats wants 100 at max-content and 50 at min-content (`flex-basis-intrinsics-001`) —
    // but it is not inline CONTENT: the white space around it collapses to nothing. Its own
    // MARGINS are part of what it asks for, because they are part of the rectangle the lines route
    // around (Chrome: a button around a 100px float with 10px margins shrink-wraps to 132, its
    // chrome included).
    if (isFloated(child)) {
...
function noteRun(bucket, text, at, owner, width) {
  if (!at) return;
  ...
  const run = { text, x: at.x, y: at.y, baseline: at.y, owner, width };
  bucket.push(run);
  RUN_SINK(run);
}
...
export function frameContentBox(frameEl) {
  ...
  const e = edgeInsets(frameEl, frameEl._lbCbW != null ? frameEl._lbCbW : box.width);
...
export function contentExtent(el) {
  ...
  const pad = paddingsOf(el);
  const padLeft = b.x + bw.left, padTop = b.y + bw.top;
  const padRight = b.x + b.width - bw.right, padBottom = b.y + b.height - bw.bottom;
  ...
  const w = axisExtent(origin.x === 'left', padLeft, padRight,
                       kidLeft, kidRight, ext.iLeft - pad.left, ext.iRight + pad.right);
  const h = axisExtent(origin.y === 'top', padTop, padBottom,
                       kidTop, kidBottom, ext.iTop - pad.top, ext.iBottom + pad.bottom);
...
function insetsOf(el) {
  return edgeInsets(el, el._lbCbW != null ? el._lbCbW : (el._lb && el._lb.width) || 0);
}
...
// Used padding per side, from the same memo: `edgeInsets` keeps padding + border together (that
// sum is the box model's edge) and the border alone alongside it, so each padding is the
// difference.
function paddingsOf(el) {
  if (!el || el.nodeType !== NODE_ELEMENT) return { top: 0, right: 0, bottom: 0, left: 0 };
  const e = insetsOf(el);
  return { top: e.top - e.bt, right: e.right - e.br, bottom: e.bottom - e.bb, left: e.left - e.bl };
}

// Which physical edge each axis SCROLLS FROM — the scroll origin corner (css-overflow-3 §3.1).
// It is the edge the box lays its content out from, so it follows `writing-mode` / `direction`,
// and for a flex container `flex-direction` on top of them: content a `row-reverse` row pushes
// off the left is as reachable as the content a plain row pushes off the right (Chrome reports
// 370 for both), while the same overflow made by a NEGATIVE MARGIN in a plain LTR block is behind
// the origin and reports nothing.
function scrollOriginSides(el) {
  if (laysOutAsFlex(el)) {
    const ax = flexPhysicalAxes(el);
    return ax.mainIsX ? { x: ax.mainStart, y: ax.crossStart } : { x: ax.crossStart, y: ax.mainStart };
  }
  const sides = flowSides(el);
  const block = sides['block-start'];
  return (block === 'top' || block === 'bottom') ? { x: sides['inline-start'], y: block }
                                                 : { x: block, y: sides['inline-start'] };
}
...
function insetContainingBox(el, pos) {
  ...
  if (pos === 'sticky') {
    for (let p = flatTreeParent(el); p; p = flatTreeParent(p)) {
      ...
      const box = inlineContainingBox(p);
      const e = edgeInsets(p, p._lbCbW != null ? p._lbCbW : box.width);
      return {
        x: box.x + e.left,
        y: box.y + e.top,
        width:  Math.max(0, box.width  - e.left - e.right),
