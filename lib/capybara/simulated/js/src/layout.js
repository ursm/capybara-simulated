...
const NL_STRIDE = 25;   // node record; must match layout::Input / LAYOUT_STRIDE
const NL_RUN_STRIDE = 7; // inline run record [kind, font, size, ls, ws, line_height, metric]; must match layout::Run
...
// An element's inline run-info: native font handle + size/spacing + resolved line-height.
function nlFontInfo(el) {
  const f = fontOf(el);
  return { nf: nlNativeFontFor(el), size: f ? f.size : 16, ls: f ? f.ls : 0, ws: f ? f.ws : 0, lh: lineHeightOf(el) };
}
...
function nlGatherRuns(el, owner, runs) {
  ...
  for (const c of layoutChildren(el)) {
    if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) { appendText(owner, c._data || ''); continue; }
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  for (let i = 0; i < runsAll.length; i++) {
    ...
    if (r.kind === NL_RUN_TEXT) {
      runBuf[o + 1] = r.fi.nf; runBuf[o + 2] = r.fi.size; runBuf[o + 3] = r.fi.ls; runBuf[o + 4] = r.fi.ws; runBuf[o + 5] = r.fi.lh;
    } else {
