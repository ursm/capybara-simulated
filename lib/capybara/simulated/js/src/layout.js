...
// Text native's pen-walk (`text_intrinsic` / `line_layout`) measures: no tab / CR / FF (tab stops are not
// modelled), no wide (CJK) character — each breaks between characters — and, where the owner breaks in-word
// per character (`nlWrapMode` 1 / 3), no ZWJ (the oracle's per-character advance carries the previous one).
function nlTextMeasurable(text, wrapMode) {
  for (let i = 0; i < text.length; i++) {
    const u = text.charCodeAt(i);
    if (u === 0x09 || u === 0x0D || u === 0x0C) return false;
    if (u === 0x200D && (wrapMode === 1 || wrapMode === 3)) return false;
    if (u >= 0x1100 && isWideChar(text.codePointAt(i))) return false;
  }
  return true;
}
    ...
    segments.forEach((segment, i) => {
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        for (let u = 0; u < units.length; u++) {
          ...
          const mayBreak = u > 0 || textMayBreak();       // a unit boundary is an opportunity
...
function endsWithBreak(text) { return BREAK_AFTER_RE.test(text); }
