...
// A table cell's `vertical-align` keyword, for placing its content in the row-tall cell (§17.5.3). Only
// `top` / `middle` / `bottom` move the content; the UA default is `middle`. Deferred (each a separate pass):
// `baseline` (and the length / sub / super values a cell treats as baseline) is approximated as `top`, since
// cross-cell baseline alignment also shapes the row height; a cell whose OWN declared height exceeds its content
// aligns only the row-stretch slack, not the intra-cell slack; and the shift moves child ELEMENT boxes only —
// bare TEXT stays at the cell top (painter-only: text has no DOM geometry in this driver, so getBoundingClientRect
// is unaffected).
...
function layoutTable(el, box, content, ctx) {
  ...
  if (hasGrid) grid.rows.forEach((row, r) => {
    ...
    for (const cell of row.cells) {
      ...
      const span = { cell, box: cbox, startRow: r, contentH, pass2, declaredH: size.autoHeight === false, cellFloor, runsFrom, runsTo: recordedRunCount() };
      placed.push(span);
  ...
  for (const s of placed) {
    ...
    // Vertical-align the content within the row-tall cell (§17.5.3): the row is `extra` taller than an
    // auto-height cell's content, and `middle` / `bottom` push the content down its share of that (the box
    // still FILLS the row — a click lands in the box, not the content). Shift the whole cell then put the box
    // back, so the content moves but the box stays at the row top. baseline ≈ top (a separate pass), as is a
    // cell whose OWN declared height already exceeds its content.
    let vaShift = 0;
    // The slack the content aligns within is the row-tall cell MINUS the content's NATURAL height (§17.5.3),
    // not minus the floored box: a `middle` cell whose own declared height already exceeds its content still
    // centres that content (Chrome), which `h - s.box.height` (0 once the floor filled the box) would miss.
    const extra = h - contentH;
    if (extra > 0.01) {
      const va = cellVAlign(s.cell.el);
      vaShift = va === 'bottom' ? extra : va === 'middle' ? extra / 2 : 0;
      if (vaShift > 0) {
