...
function replacedLaysOutChildren(el) {
  ...
  for (const c of layoutChildren(el)) {
    if (c.nodeType === NODE_ELEMENT && !selfNotRendered(c) && displayOf(c) !== 'none') return true;
  }
  return false;
}
...
function nlIntrinsicMeasurableOf(el) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c) || displayOf(c) === 'none') continue;
...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  for (const c of children) {
    ...
    if (selfNotRendered(c) || displayOf(c) === 'none') continue;
...
function inFlowGridItems(el, colCount) {
  let n = 0;
  for (const child of layoutChildren(el)) {
    if (child.nodeType !== NODE_ELEMENT || selfNotRendered(child) || displayOf(child) === 'none') continue;
    const pos = positionOf(child);
    if (pos === 'absolute' || pos === 'fixed') continue;
...
function tableGrid(table) {
  ...
  const rowContent = (rowEl) => {
    ...
    for (const c of layoutChildren(rowEl)) {
      ...
      if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c)) continue;
