...
function nlTableSupported(el) {
  ...
  // An IMPOSED table height is a two-phase the pure track self-size can't reproduce: below the natural grid
  // height the oracle keeps the smaller declared box while the rows overflow it; a min-height floors the box
  // above the tracks; a max-height caps it. Native self-sizes from Σtracks, so decline any imposed height.
  // (A declared table WIDTH is fine — the oracle distributes it into the columns, which the pushed cell
  // widths already reflect.)
  if (el._lb && el._lb.autoHeight === false) return false; // declared / attribute height → JS
  if (declaresLayoutProp(el, 'min-height') || declaresLayoutProp(el, 'max-height')) return false;
