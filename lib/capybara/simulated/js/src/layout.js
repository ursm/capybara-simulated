...
// Whether a flex ROW's item widths can be resolved by the native engine (`flex_row_sizes`) from the items'
// declarations and content: every in-flow item's content must be natively measurable (its flex base and
// automatic minimum come from `intrinsic_widths` / `content_intrinsic`), no item is replaced (its intrinsic
// size is not yet data native holds), and none aligns on a baseline (its baseline is still the oracle's).
// A column, and anything this declines, keeps the pushed path (the oracle's item boxes replayed).
function nlFlexRowNativeSizable(el, plan, items, itemsAlign, contentW) {
  if (!plan.mainIsX) return false;
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      if (!heightDefinite) {
        ...
        if (minH != null && minH > 0) colMainH = Math.max(0, isBorderBox(el) ? minH - (e.top + e.bottom) : minH);
      }
      ...
      // NATIVE SIZING (a row, for now): the items' widths are resolved by the native engine — flex base,
      // automatic minimum, line breaking, grow / shrink, stretch — from the records' declarations, when every
      // item's content is natively measurable (`nlFlexRowNativeSizable`). Otherwise each item's oracle-resolved
      // box is pushed and native only places.
      const nativeSizing = nlFlexRowNativeSizable(el, plan, items, itemsAlign, contentW);
      ...
      for (const c of items) {
        ...
        if (nativeSizing) {
          ...
          // (NaN = auto / a keyword), the keyword itself, whether the item scrolls in the main axis (its automatic
          // minimum is zero), and whether it stretches across (an auto cross size, `stretch`, no auto cross
          // margin). The record keeps its declared width / height / min / max and box-sizing — native resolves.
          const kw = basisKeyword(c);
          r[63] = kw ? NaN : num(resolveLayoutProp(c, 'flex-basis', contentW));
          r[64] = kw === 'content' ? 1 : kw === 'min-content' ? 2 : kw === 'max-content' ? 3 : kw === 'fit-content' ? 4 : 0;
          r[65] = scrollsInAxis(c, 'x') ? 1 : 0;
          r[66] = resolveLayoutProp(c, 'height', c._lbCbH != null ? c._lbCbH : null) == null && stretchesInCross(align, c, 'height', ce) ? 1 : 0;
        } else {
