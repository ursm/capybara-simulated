...
// Lay a subtree out WITHOUT recording its text runs — a table cell that will be laid out a second time
// (its percentage-height content re-resolved against the final row height) sizes itself here, then records
// its runs once, in that second pass, at the final position. Without this the first pass's runs would
// double up with the second's. A no-op outside the painter's recording pass (RUN_LIST is already null).
function suppressingRuns(fn) {
  const prevSink = RUN_SINK, prevList = RUN_LIST;
  RUN_SINK = null;   // the sink is what actually records a run; the list only counts / shifts them
  RUN_LIST = null;
  try { return fn(); } finally { RUN_SINK = prevSink; RUN_LIST = prevList; }
}
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const takeBreak = () => {
    if (barrier && barrier.shy) {
      ...
      if (barrier.rec) noteRun(lineRuns, '-', at, owner, hy);
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        for (let u = 0; u < units.length; u++) {
          ...
          if (hyphen) {
            ...
            if (rec) noteRun(lineRuns, '-', at2, owner, hy);
            barrier = null;
          } else {
...
function placeFloat(child, pos, ctx, content, right, top0) {
  ...
  //
  // The first pass is a PROBE and must leave nothing behind: its text runs are suppressed (they would be
  // recorded at a position the float is about to leave — a wrapped row of `.col` floats painted every label
  // at the previous row's y), and the real placement then REFUSES REUSE, because a subtree whose boxes only
  // need shifting records no runs at all and the glyphs would be recorded nowhere. The same pair the table
  // cell's second pass uses, for the same reason.
  let outerH = size.height + cm.mt + cm.mb;
  const probed = size.autoHeight;
  if (probed) {
    suppressingRuns(() => place(top0));
  ...
  if (probed) ctx.forceLayout = true;
  let x;
  try { x = place(marginTop); } finally { ctx.forceLayout = prevForce; }
...
function layoutTable(el, box, content, ctx) {
  ...
  if (hasGrid) grid.rows.forEach((row, r) => {
    ...
    for (const cell of row.cells) {
      ...
      const runsFrom = recordedRunCount();   // the cell's text runs, so vertical-align can move them (paint pass)
      // A pass2 cell records its runs in the SECOND layout, at the final position — suppress the first so they
      // don't double up.
      if (pass2) suppressingRuns(() => layoutElement(cell.el, cbox, ctx, content.width));
      ...
      const span = { cell, box: cbox, startRow: r, contentH, pass2, declaredH: size.autoHeight === false, cellFloor, va, cellBase, runsFrom, runsTo: recordedRunCount() };
      placed.push(span);
      if (cell.rowSpan === 1) {
  ...
  for (const s of placed) {
    ...
    if (s.pass2 && h > 0) {
      ...
      const runsFrom = recordedRunCount();
      ...
      s.box = cbox2;
      s.runsFrom = runsFrom;
      s.runsTo = recordedRunCount();
      contentH = s.cell.el._lbCellContentH != null ? s.cell.el._lbCellContentH : h;
    } else {
      const dy = rowTops[s.startRow] - s.box.y;
      if (dy) { shiftSubtreeY(s.cell.el, dy); shiftRecordedRuns(s.runsFrom, s.runsTo, 0, dy); }   // move the cell's text with it (paint)
    }
    ...
    if (vaShift > 0) {
      ...
      unshift(s.cell.el);
      // `shiftSubtreeY` moves child ELEMENT boxes but not the cell's recorded text runs (they belong to the
      // pass, not the box tree) — move them too, so a cell's bare text vertical-aligns in the paint like its
      // block children do.
      shiftRecordedRuns(s.runsFrom, s.runsTo, 0, vaShift);
    }
