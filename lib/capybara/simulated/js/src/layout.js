...
const NL_STRIDE = 34;   // node record; must match layout::Input / LAYOUT_STRIDE
...
// A flex container the native engine can place (§9.7, increment f1): a plain LTR horizontal `row`,
// `nowrap`, in normal flow, with no min/max-height (the two-phase cross-size clamp) and no bare text (an
// anonymous flex item). The item SIZING is resolved JS-side; native does only the placement. Everything
// else — inline-flex, column, reverse, rtl / vertical writing modes, wrap — declines to JS.
function nlFlexSupported(el) {
  ...
  if (plan.mode !== 'horizontal-tb' || plan.mainStart !== 'left' || plan.crossStart !== 'top' ||
      plan.flexReverse || plan.wrapMode !== 'nowrap') return false;
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      rec[32] = items.length > 1 ? axisGap(el, plan.mainGap, contentW) : 0;
      for (const c of items) {
        const itemIdx = nodes.length;
        if (!walk(c, idx)) return false;                   // build the item's own record + subtree
        const r = recs[itemIdx];
        r[3] = 1;                                          // its used size is a border box…
        r[4] = c._lb.width;                                // …main
        r[5] = c._lb.height;                               // …cross (both flex-resolved)
