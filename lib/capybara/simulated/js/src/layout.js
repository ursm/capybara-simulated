...
const NL_STRIDE = 43;   // node record; must match layout::Input / LAYOUT_STRIDE
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      for (const c of layoutChildren(el)) {
        ...
        if (selfNotRendered(c) || displayOf(c) === 'none') continue;
        // Checks common to every item, whatever kind:
        const cp = positionOf(c);
        if (cp !== 'static' && cp !== 'relative') return false; // abspos / fixed item → JS (relative is applied in place)
        ...
        // Item KIND: a supported nested flex container (f2 — walk recurses into it), or a block-level box.
        if (laysOutAsFlex(c)) {
          if (!nlFlexSupported(c)) return false;
        } else if (!nlBlockDisplay(c)) {
          return false;                                    // inline / inline-block / table item → JS
