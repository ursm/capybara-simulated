...
function memoStamp(el) {
  ...
  return ((d > i ? d : i) * 4294967296) + cascadeVersion();
}
...
function structFresh(el, key) {
  const m = el[key];
  return m !== undefined && m === ((el._lbStruct || 0) * 4294967296) + cascadeVersion();
}
function structStamp(el) { return ((el._lbStruct || 0) * 4294967296) + cascadeVersion(); }
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  for (const child of layoutChildren(el)) {
    ...
    layoutElement(child, { x: content.x + cm.ml + (rel ? rel.x : 0), y: flowY + (rel ? rel.y : 0),
...
function layoutGrid(el, box, content, ctx) {
  ...
  for (const child of layoutChildren(el)) {
    ...
    const cx = content.x + offsets[col];
    ...
    layoutElement(child, { x: cx, y: rowTop, width: trackW, height: size.height, autoHeight: size.autoHeight }, ctx, content.width);
    if (child._lb.height > rowH) rowH = child._lb.height;
...
function gridColumnWidths(el, contentW, gap) {
  ...
  if (frIdx.length) {
    ...
    const weight = frIdx.reduce((a, i) => a + tracks[i].fr, 0);
    const spare  = Math.max(0, inner - taken);
    for (const i of frIdx) {
      const share = weight > 0 ? (spare * tracks[i].fr) / weight : spare / frIdx.length;
      base[i] = Math.max(base[i], share);
    }
  } else if (free > 0.01) {
...
function gridColumnContent(el, colCount) {
  ...
  for (const child of layoutChildren(el)) {
    ...
    const span = Math.max(1, Math.min(gridColumnSpan(child), colCount));
    if (col + span > colCount) col = 0;
...
function gridTrackList(el, contentW, gap) {
  ...
  for (const tok of splitTopLevel(s)) {
    ...
    if (rep) {
      const body = splitTopLevel(rep[2].trim()).map((t) => parseTrack(el, t));
...
function repeatCount(spec, body, contentW, gap, el) {
  ...
  let per = 0;
  for (const t of body) {
    const side = t.floor || t;
    const fixed = side.px != null ? side.px : (side.pct != null ? (side.pct / 100) * contentW : null);
  ...
  if (/^auto-fit$/i.test(spec)) {
    const items = inFlowGridItems(el);
...
function inFlowGridItems(el) {
  let n = 0;
  for (const child of layoutChildren(el)) {
    if (child.nodeType !== NODE_ELEMENT || selfNotRendered(child)) continue;
    const pos = positionOf(child);
    if (pos === 'absolute' || pos === 'fixed') continue;
    n += Math.max(1, gridColumnSpan(child));
  }
  return n;
}
...
function parseTrack(el, text) {
  ...
  if (fr) return { px: null, pct: null, fr: Math.max(0, parseFloat(fr[1])), auto: false, floor: null };
  const pct = /^(-?\d*\.?\d+)%$/.exec(s);
  if (pct) return { px: null, pct: parseFloat(pct[1]), fr: null, auto: false, floor: null };
  const px = lengthPx(el, s);
  if (px != null) return { px, pct: null, fr: null, auto: false, floor: null };
