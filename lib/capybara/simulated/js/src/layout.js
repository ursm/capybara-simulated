...
const NL_STRIDE = 77;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start, 56-61 = declared sizing, 62-67 = flex sizing, 68-70 = replaced intrinsic, 71-76 = out-of-flow)
...
// A REPLACED element native replays as a LEAF box: the oracle resolved its border box from the intrinsic
// size + CSS (`intrinsicSize` — svg / img / canvas / input / select / textarea / …), and it lays out no
// CSS-box children of its own, so native pushes that box and emits no subtree, exactly like an out-of-flow
// replay but in flow. A control that DOES lay out content (a `<button>` wrapping a sized icon, whose child
// carries its own `_lb`) is NOT a leaf — it still declines to JS. (`_lb` is set on every laid-out box.)
function nlReplacedLeaf(el) {
  if (intrinsicSize(el) == null) return false;
  // An `<svg>` is sized by its intrinsic (viewBox) size and RENDERS its content through the SVG model, not the
  // CSS box model — its `<path>`/`<g>`/… descendants are painted, never laid out as CSS boxes (the oracle does
  // stamp a degenerate `_lb` on them, which must NOT make the svg a non-leaf). So a sized svg is ALWAYS a leaf:
  // native replays its border box and emits no subtree. (A `<foreignObject>` reintroduces CSS content inside the
  // svg viewport — SVG-internal layout we don't model either — so it is covered by the leaf box, not declined.)
  if (el._tag === 'svg') return true;
  // A control that DOES lay out CSS-box content (a `<button>` wrapping a sized icon, a block `<select>` stacking
  // its options — each child carrying its own `_lb`) is NOT a leaf: the oracle sizes it from those children, so
  // native declines it to JS rather than replay a box whose subtree it can't reproduce.
  for (const c of layoutChildren(el)) {
    if (c.nodeType === NODE_ELEMENT && !selfNotRendered(c) && displayOf(c) !== 'none' && c._lb) return false;
  }
  return true;
}
...
// (no atomic inline — its box is replayed, not measured), and its block children / flex items must pass in
// turn. A box whose
...
function nlIntrinsicMeasurable(el) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (cd === 'inline' && !isFloated(c)) {
      if (!nlInlineMeasurable(c)) return false;
      continue;
    }
    if (INLINE_LEVEL.has(cd)) return false;                              // an atomic inline: its own intrinsic box
...
// text, `<br>` / `<wbr>`, and such inlines. (A different `white-space` is declined by the walk itself.)
function nlInlineMeasurable(el) {
  ...
  for (const c of layoutChildren(el)) {
    if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c) || displayOf(c) === 'none') continue;
    if (c._tag === 'br' || c._tag === 'wbr') continue;
    if (displayOf(c) !== 'inline' || !nlInlineMeasurable(c)) return false;
  }
  return true;
}
...
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode) {
  ...
  for (const c of children) {
    ...
    if (cdisp === 'inline-block' || cdisp === 'inline-flex' || cdisp === 'inline-table' || cdisp === 'inline-grid' ||
      ...
      runs.push({ kind: NL_RUN_ATOMIC, metric: c._lb.width + ce.ml + ce.mr, asc, lh: outer });
      hasContent = true;
      hasReal = true;
      continue;
    }
    ...
    // The oracle grows the line box per-run from `verticalAlignFor(the run's OWNER).px`, so a nested BASELINE
    // child (`cva` null) is NOT raised by an ancestor's shift — `<sup><a>1</a></sup>` stays on the baseline
    // (measured: oracle 22, not 25). So a null `cva` threads 0, NOT the caller's shift; only an element whose
    // OWN vertical-align resolves to a shift raises its directly-owned text (`cva.px`, inheritance already folded
    // in by `verticalAlignFor`).
    ...
    if (openW !== 0 || closeW !== 0) {
      ...
      const sub = nlGatherRuns(layoutChildren(c), cf, runs, wsMode, cShift, cWrap);
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  const emitOutOfFlow = (c, el, idx, crossAlignCode) => {
    ...
    // intrinsic widths — which native must be able to measure (`nlIntrinsicMeasurable`), else replay.
    ...
    if (cbIdx != null && !cbEl._lbEdgePct && (!shrinksToFit || nlIntrinsicMeasurable(c))) {
      r[71] = cbIdx;
  ...
  function walk(el, parentIdx) {
    ...
    // and native routes an rtl block's text around a float it owns just as it does LTR. rtl only moves a text
    // block's GLYPHS within its box (which the shadow doesn't compare), not the box, so a text block needs no
    // rtl handling.
    ...
    if (laysOutAsFlex(el)) {
      ...
      rec[65] = isReverseFlex(el) ? 4 : 0;                 // layout flags on the container: a *-reverse direction
      ...
      for (const c of items) {
        ...
        if (!walk(c, idx)) return false;                   // build the item's own record + subtree
        ...
        if (nativeSizing) {
          ...
          r[65] |= (scrollsInAxis(c, 'x') ? 1 : 0) | (scrollsInAxis(c, 'y') ? 2 : 0);  // layout flags: scrolls across x / y (a nested container's own bits stay)
    ...
    if (laysOutAsGrid(el)) {
      ...
      for (const c of items) {
        const ci = nodes.length;
        if (!walk(c, idx)) return false;                        // the item's own subtree (a normal block record)
    ...
    if (hasInline) {
      ...
      if (oof.length) return false;                            // an abspos child under a text block → JS (native's text-block path lays out no child records)
      ...
      if (bf.nf < 0) return false;                             // block's own font non-system → JS
      const local = [];
      const g = nlGatherRuns(layoutChildren(el), bf, local, wsMode, 0, nlWrapMode(el));
  ...
  for (let i = 0; i < runsAll.length; i++) {
    ...
    if (r.kind === NL_RUN_TEXT) {
      ...
      runBuf[o + 5] = r.lh;            // margin-box height (asc + desc), for the line box
      runBuf[o + 6] = r.metric;        // advance = margin-box width
      runBuf[o + 7] = r.asc;           // ascent above the line's baseline
    } else {
  ...
  return { ok: true, nodes: nodes.length, compared, mismatches, sample, nativeIntrinsicGrids, nativeFlexRows, nativeOutOfFlow };
};
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (extBottom !== undefined && (size.autoWidth || size.autoHeight)) {
