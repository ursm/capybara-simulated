...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  // it does not cross is a MEASURE BOUNDARY, which `walkMeasureBoundary` parks it at.
  let measured = 0;
  ...
  // Walk `c` across a MEASURE BOUNDARY — a subtree no ancestor's intrinsic contribution reads — so the flag is
  // PARKED there rather than inherited, and `measuredSelf` alone decides it: whether the BOX's own sizing asks
  // an intrinsic measure of its content. Two boundaries qualify, each on the same two facts.
  //
  ...
  // makes the walk agree with its own predicate). A TABLE CELL whose contribution is PUSHED (rec[84..85]):
  // `table_columns` reads that figure and never descends into the cell.
  ...
  // measure that never happens — a centred inline-block in a `<td>`, an absolute box holding one. Restored
  // symmetrically, failure included.
  const walkMeasureBoundary = (c, parentIdx, measuredSelf) => {
    if (measuredSelf) return walkMeasured(c, parentIdx, true);
  ...
  const emitOutOfFlow = (c, el, idx, crossAlignCode) => {
    ...
    const native = cbIdx != null && !cbEl._lbEdgePct && (!shrinksToFit || nlIntrinsicMeasurable(c));
    const ci = nodes.length;
    if (!walkMeasureBoundary(c, idx, native && !!shrinksToFit)) return false;
    const r = recs[ci];
    r[43] = 1;                                               // out of flow
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsTable(el)) {
      ...
      const emitRow = (row, parentIdx) => {
        ...
        for (const cell of row.cells) {
          ...
          // intrinsics — and a measure BOUNDARY where it cannot (the oracle's contribution rides rec[84..85]
          // below, which `table_columns` reads without descending).
          if (!walkMeasureBoundary(cell.el, ri, nlIntrinsicMeasurable(cell.el))) return false;
          const cr = recs[ci];
          ...
          if (!nlIntrinsicMeasurable(cell.el)) {
      ...
      if (grid.captions.length) {
        ...
        const capMeasured = measured > 0 && nlIntrinsicMeasurable(cap);
        if (!walkMeasureBoundary(cap, idx, capMeasured)) return false;
        ...
        if (!capMeasured) {
          ...
          cr[84] = iw.min; cr[85] = iw.max;
          if (!nlIntrinsicMeasurable(cap)) pushedContributions++;
        }
      }
      return true;
    }
    ...
    if (hasInline) {
      ...
      const atomic = align === 'justify' ? null : {
        lay: (c, rel) => {
          const ci = nodes.length, runsAt = runsAll.length, gridsAt = gridsAll.length;
          const stats = [nativeIntrinsicGrids, nativeFlexRows, nativeOutOfFlow, nativeAtomics, pushedContributions];
          if (walkMeasured(c, idx)) {
          ...
          for (const n of nodes.splice(ci)) if (n) recIndex.delete(n);
          recs.length = ci; runsAll.length = runTexts.length = runsAt; gridsAll.length = gridsAt;
          [nativeIntrinsicGrids, nativeFlexRows, nativeOutOfFlow, nativeAtomics, pushedContributions] = stats;
