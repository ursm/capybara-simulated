...
function nlSupported(el) {
  ...
  if (p !== 'static' && p !== 'relative') return false;   // abspos / fixed / sticky → later/JS
...
// blockifies to flex — see below), wrap-reverse, `position: sticky`, a float, and a WRAPPING auto-height
...
function nlFlexSupported(el) {
  ...
  const p = positionOf(el);
  // An `absolute` / `fixed` flex container is out of flow: its PARENT replays its oracle-resolved box (insets /
  // static position) and walk lays out its items within that box — the position never enters the flex sizing,
  // so it is fine. Only `sticky` (a scroll-driven shift native doesn't model) declines here.
...
function nlTableSupported(el) {
  ...
  const p = positionOf(el);
  if (p !== 'static' && p !== 'relative') return false;    // abspos / fixed / sticky → JS
  if (isFloated(el)) return false;
...
function nlGridSupported(el) {
  ...
  // computes the tracks within it, so the position never enters layout. Only sticky declines.
  if (p === 'sticky') return false;
  if (isFloated(el)) return false;
  return true;
}
...
function nlAtomicMeasurable(c, cd) {
  ...
  if (cp !== 'static' && cp !== 'relative') return false;
  const va = verticalAlignFor(c);
...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  for (const c of children) {
    ...
    if (cdisp === 'inline-block' || cdisp === 'inline-flex' || cdisp === 'inline-table' || cdisp === 'inline-grid' ||
      ...
      // A `sticky` atomic has a scroll-driven shift native doesn't model — decline. (An abspos / fixed one never
      ...
      const cp = positionOf(c);
      if (cp !== 'static' && cp !== 'relative') return { ok: false, reason: 'atomic-oof' };
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  // is a second one to keep in sync, and the refusals it missed (a sticky child, an intrinsic-size KEYWORD on a
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      for (const c of layoutChildren(el)) {
        ...
        if (cp !== 'static' && cp !== 'relative') return false; // sticky → JS (relative stays in flow)
    ...
    for (const c of layoutChildren(el)) {
      ...
      // origin (rel below), exactly as a flex container replays its own out-of-flow children. `sticky` stays
      // in flow with a scroll-driven shift native doesn't model — decline.
