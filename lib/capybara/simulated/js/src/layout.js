...
function nlIntrinsicMeasurableOf(el) {
  ...
  for (const c of layoutChildren(el)) {
    if ((c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) && !nlTextMeasurable(c._data || '', wrapMode)) return false;
...
function nlInlineMeasurable(el, atomicsPushed) {
  ...
  for (const c of layoutChildren(el)) {
    if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) {
      const text = c._data || '';
      if (!nlTextMeasurable(text, wrapMode)) return false;
...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  const preserveWs = wsMode === 'pre' || wsMode === 'pre-wrap';
  ...
  for (const c of children) {
    ...
    if (openW !== 0 || closeW !== 0 || openPlain !== 0 || closePlain !== 0) {
      ...
      const sub = nlGatherRuns(layoutChildren(c), cf, ctx, cWs, cShift, cWrap, cRel, openW !== 0);
      ...
      const sub = nlGatherRuns(layoutChildren(c), cf, ctx, cWs, cShift, cWrap, cRel, false);
      if (!sub.ok) return sub;
  ...
  return {
    ...
    install(rootEl) {
      ...
      while (stack.length) {
        ...
        for (const c of layoutChildren(el)) stack.push(c);
      }
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    let hasInline = false;
    const elWs = whiteSpaceOf(el);
    let sawPreWs = false;
    for (const c of layoutChildren(el)) {
      if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) {
        const td = c._data || '';
        if (CSS_CONTENT_RE.test(td)) hasInline = true;
        else if (whiteSpaceOnlyIsContent(td, elWs)) sawPreWs = true;   // …direct whitespace-only text that is a LINE
        continue;
      }
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      const flushGroup = () => {
        ...
        if (COLLAPSING_WS.has(wsMode) && kids.every((k) => (k.nodeType === NODE_TEXT || k.nodeType === NODE_CDATA) &&
        ...
        const kept = emitAttempt(() => {
          ...
          const gr = nlGatherRuns(kids, bf, {runs: g, block: el, atomic, oof: oofHook, float: floatHook(anonIdx, floats)}, wsMode, 0, nlWrapMode(el));
    ...
    if (hasInline) {
      ...
      const g = nlGatherRuns(layoutChildren(el), bf, {runs: local, block: el, atomic, oof: oofHook, float: floatHook(idx)}, wsMode, 0, nlWrapMode(el));
...
function isBlockLevelChild(node) {
  ...
  // A `display: contents` box is no box: it is block-level content exactly when it HOLDS some.
  if (displayOf(node) === 'contents') {
    for (const grand of layoutChildren(node)) if (isBlockLevelChild(grand)) return true;
    return false;
  }
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeTextRun = (text, owner) => {
    ...
    let ownAsc = owner === el ? baselineWithin(owner)
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (child.nodeType === 3) {
      ...
      if (!NON_WS_RE.test(t)) {
        ...
        // for all four places that ask it.
        ...
        if (linePlaced && t && !lineEndsWithSpace) {
          flushMargin();
          // …on the owner's own baseline, `vertical-align` and all: this is the ONE space of the
          // inline-block gap, and it is inside that inline. `placeTextRun` resolves the same figure
          ...
          placeOnLine(measureRun(' ', owner), lineHeightOf(owner), true, true, false,
                      owner === el ? baselineWithin(owner) : inlineAscent(owner, baselineWithin(owner)));
        }
        return true;
      }
    ...
    // A `display: contents` box holding inline content puts that content on THIS line, in place —
    // its own box does not exist, so it can neither start a line nor end one (Chrome keeps
    // `x<span style="display:contents">aaaa bbbb</span>y` on one line; treating it as block-level
    // content made three). One holding a block is handed back like any block-level child.
    if (displayOf(child) === 'contents') {
      if (isBlockLevelChild(child)) return false;
...
function contentIntrinsicWidths(el) {
  ...
  const walk = (node) => {
    for (const child of layoutChildren(node)) {
      ...
      // A `display: contents` box is no box: its children are this line's.
      if (d === 'contents') { walk(child); continue; }
...
function separatesMargins(child) {
  if (child.nodeType === NODE_TEXT) {
    ...
    const t = child._data || child.data || '';
    if (NON_WS_RE.test(t)) return true;
...
const COLLAPSING_WS = new globalThis.Set(['normal', 'nowrap']);
