...
// CSS (`intrinsicSize` — svg / img / canvas / input / select / textarea / …), and it lays out no CSS-box children
// of its own, so native pushes that box and emits no subtree, exactly like an out-of-flow replay but in flow. One
// that DOES lay out children still declines to JS: the oracle sizes it from them (a dropdown is 30px wide for
// `a`, 52 for `aaaa`), a box whose subtree native can't reproduce.
function nlReplacedLeaf(el) {
  return intrinsicSize(el) != null && !replacedLaysOutChildren(el);
}
...
function nlFlexNativeSizable(el, plan, items, itemsAlign) {
  for (const c of items) {
    if (intrinsicSize(c) && (!nlReplacedLeaf(c) || basisKeyword(c))) return false;   // a non-leaf control; a keyword basis on a replaced item
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (nlBlockDisplay(c) && shrinkWrapsToFit(c) && !nlReplacedLeaf(c)) return false;
      ...
      // A replaced / form control that is NOT a leaf (it lays out box children — e.g. a display:block <select>
      // whose options carry their own _lb) is sized by the oracle from its INTRINSIC size, not from stacking
      // those children; nlSupported would wrongly walk it as an ordinary block container, so decline it.
      if (intrinsicSize(c) && !nlReplacedLeaf(c)) return false;
