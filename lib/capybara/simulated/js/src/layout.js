...
const NL_STRIDE = 42;   // node record; must match layout::Input / LAYOUT_STRIDE
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      for (const c of layoutChildren(el)) {
        ...
        if (alignsOnBaseline(crossAlignPhysical(itemsAlign, c, plan, plan.baselineMode))) return false;
      ...
      for (const c of items) {
        ...
        r[33] = align === 'center' ? 1 : align === 'flex-end' ? 2 : 0; // stretch / flex-start → 0
        const am = edgeInsets(c, contentW).autoMargins;    // auto margins: main bit0/bit1, cross bit2/bit3
