...
function placeAbsolute(child, pos, staticX, staticY, ctx, order = null) {
  ...
  if (cbEl && cbEl._lb.height === 0 && cbEl._lb.autoHeight && LAYING_OUT.has(cbEl)) {
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const forceBreak = () => {
    ...
    for (const frag of openInlines) {
      ...
      frag.lines.pop();
      if (line.hangPending) dropHangs(true);
      // The margin the opening placement carried is outside `minX`, so the width that moves
      // down is measured from where the placement STARTED.
      const from = line.minX - frag.ce.ml;
      carried.push({ frag, width: Math.max(0, Math.max(line.maxRight, line.hangRight) - from) });
    }
    ...
    hadInFlow = true;
    // …re-opening each carried box at the start of the new line, outermost first.
    for (const { frag, width } of carried) {
      placeOnLine(width, frag.ownH, true, false, true);
      const line = frag.lines[frag.lines.length - 1];
      if (line) line.minX += frag.ce.ml;
    }
  };
  ...
  const placeOnLine = (w, h, decided, hangs, edge) => {
    ...
    flushMargin();
    if (!decided && lineX > content.x && lineX + w > right) forceBreak();
    ...
    for (const frag of openInlines) {
      ...
      if (!line || line.y !== at.y) {
        line = { y: at.y, minX: at.x, maxRight: -Infinity, hangRight: -Infinity,
                 hangPending: false, edgeOnly: true };
        frag.lines.push(line);
      }
      if (!edge) line.edgeOnly = false;
      if (at.x < line.minX) line.minX = at.x;
      if (hangs) {
        if (!line.hangPending) { line.hangPending = true; lineHangs.push(line); }
        if (lineX > line.hangRight) line.hangRight = lineX;
      } else if (lineX > line.maxRight) {
        line.maxRight = lineX;
      }
    }
    return at;
...
// Each positioned box on the way down contributes its paint RANK and, to break a tie between
// two of the same rank, where it sits in the tree. Rank alone left the comparison to the chain
// LENGTH, which says a deeper box paints above a shallower one — so a `position: relative`
// child of one dropdown covered the dropdown declared after it.
function stackChain(el) {
  ...
  if (positionOf(el) !== 'static') chain.push(paintRank(el), el._lbOrder);
