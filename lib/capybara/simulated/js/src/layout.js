...
function nlTableSupported(el) {
  ...
  // A declared or min height TALLER than the grid shares its excess over the rows (Chrome), and one SMALLER than
  // the grid is just a floor the box grows past — either way the tracks fill it, so native (which self-sizes
  // from Σtracks) reproduces it. A `max-height` below the grid is the one that DOESN'T fill: the oracle clamps
  // the box under the tracks and they overflow it, which native can't self-size to — decline. A caption or a
  // collapsed border adds extents outside the plain row tracks, so an imposed height alongside either is
  // likewise declined. The `gridFillH` check below is the real gate for all of these.
...
function layoutTable(el, box, content, ctx) {
  ...
  // The table's IMPOSED content height — the larger of a declared `height` and a `min-height` — is what the
  // rows share out and resolve their percentages against (Chrome distributes a `min-height`'s surplus to the
  // rows exactly as it does a `height`'s; `<table min-height:100>` over rows of 10 and 30 makes them 25 and 75,
  // not 10 and 30 with empty space under them). Both are content-box figures here; `content.height` already is
  // (0 for an auto-height table), and a `min-height` is lifted to the content box for a border-box table.
  const tMinH = resolveLayoutProp(el, 'min-height', el._lbCbH);
  const tMinContentH = tMinH != null && tMinH >= 0 ? (isBorderBox(el) ? Math.max(0, tMinH - tEdge.top - tEdge.bottom) : tMinH) : 0;
  const imposedContentH = Math.max(content.height, tMinContentH);
  ...
  // The table box wraps everything the flow laid down — the row grid (grown to the imposed height) plus any
  // caption on top (§17.4). It is never SHORTER than that: a declared or min height is a floor the grid fills,
  // and a caption or content taller than it grows the box past it (Chrome grows a `height: 10px` table to its
  // 50px content, and a `height: 100px` table with a 16px caption to 116). `tEdge` = this table's own edges.
  box.height = Math.max(box.height, Math.max(0, y - content.y) + tEdge.top + tEdge.bottom);
  stampExtent(el, box);
}
