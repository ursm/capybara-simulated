...
function ensureLayout() {
  ...
  if (root) {
    ...
    const consumed = Math.max(bodyRendered ? bb.y + bb.height + marginOf(body, 'margin-bottom') : 0,
...
function isFloated(el) {
  ...
  // POSITIONED, not floated (Chrome: it sits at its insets and shortens no lines at all).
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (child.nodeType === NODE_ELEMENT && !selfNotRendered(child) && isFloated(child)) {
      placeFloat(child, positionOf(child), ctx, content, right, flowY + (openMargin || 0));
  ...
  for (const child of layoutChildren(el)) {
    ...
    if (clear) flowY = Math.max(flowY, clearanceY(ctx.floats, flowY, clear));
    ...
    const m = distributes ? autoMarginSplit(cm, 'left', 'right', band ? band.right - band.left : content.width, size.width) : cm;
    const lead  = distributes ? m.lead  : cm.ml;
    const trail = distributes ? m.trail : cm.mr;
    const rel  = pos === 'relative' ? relativeOffset(child, content.width, content.height || null) : null;
    layoutElement(child, { x: (band ? band.left : content.x) + lead + (rel ? rel.x : 0), y: flowY + (rel ? rel.y : 0),
...
function startsFloatContext(el, disp) {
  ...
  // and nothing above it can contain anything.
  if (!el._parent || el._parent.nodeType !== NODE_ELEMENT) return true;
