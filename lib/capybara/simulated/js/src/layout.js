...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    // FLOAT written inside an inline box, whose record hangs under the block it floats in — or under an anonymous
    // group of that block, as wide as its content box — which is its containing block, rather than under the inline.)
    ...
    const nativeBasis = parentIdx >= 0 && !TABLE_INTERNAL_DISPLAY.has(displayOf(el)) &&
                        (isOutOfFlow ? oofBasis()
                          : parIsElem && (recParent === parLayout || isFloated(el)) &&
                            !(recParent && laysOutAsGrid(recParent)) &&
                            !(recParent && laysOutAsTable(recParent) && displayOf(el) !== 'table-caption'));
