...
function layoutTable(el, box, content, ctx) {
  ...
  // A PERCENTAGE row height resolves against the space the rows share — the table's content box minus the
  // border-spacing around and between them ((n+1) gaps) — and only when the table's own height is definite;
  // with an auto-height table a `%` row height has no basis and is `auto` (Chrome). A resolved `%` row is a
  // fixed track like a declared length (a MINIMUM, excluded from the surplus distribution).
  const rowPctBasis = content.height > 0 ? Math.max(0, content.height - (grid.rows.length + 1) * sp.y) : null;
  ...
  if (hasGrid) grid.rows.forEach((row, r) => {
    ...
    for (const cell of row.cells) {
      ...
      const pass2 = cellHasPctHeightChild(cell.el) && (size.autoHeight === false || box.height > 0);
      const cbox = pass2
  ...
  // bottom of a cell has to land inside it. The surplus goes to the AUTO rows (no declared height) in
  ...
  if (box.height > 0 && hasGrid) {
    const edge = edgeInsets(el, box.width);   // includes the collapse outer halves (top & bottom)
    const room = box.height - edge.top - edge.bottom - (y - content.y);
    if (room > 0) {
      const autos = [];
