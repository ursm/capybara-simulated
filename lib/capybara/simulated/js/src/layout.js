...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeTextRun = (text, owner) => {
    ...
    // of, and what a bigger font on the line raises for everything on it.
    const ownAsc = baselineWithin(owner);
  ...
  const placeInlineChild = (child, owner) => {
    ...
    const at   = placeOnLine(size.width, 0, false, false, false, 0);
    const slot = { el: child, asc: 0, runsFrom: recordedRunCount(), runsTo: 0 };
    lineBoxes.push(slot);
    const cbox = {
      x: at.x + (rel ? rel.x : 0), y: at.y + ce.mt + (rel ? rel.y : 0),
      width: size.width, height: size.height, autoHeight: size.autoHeight
    };
  ...
  const placeInlineBox = (child, pos) => {
    ...
    if (ce.right) placeOnLine(ce.right, ownH, true, false, true, fontAscent(child));
    openInlines.pop();
    OPEN_INLINE_BOXES.pop();
    // The right margin is outside the box, so it advances the line after the box has closed.
    if (ce.mr) placeOnLine(ce.mr, ownH, true, false, true, fontAscent(child));
  ...
  const lineTopOffset = (rec, box) => (rec ? rec.asc - fontAscent(box) : 0);
