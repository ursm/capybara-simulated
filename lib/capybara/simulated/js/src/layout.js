...
function ensureLayout() {
  ...
  layoutPass++;
  OPEN_INLINE_RELS.length = 0;
  PENDING_INLINE_OOF.length = 0;
...
// The relative offsets of the inline boxes being fragmented right now, innermost last, and
// the out-of-flow boxes waiting on them. Per PASS rather than per block: a box nested one
// layout deeper — an `<i>` inside an `inline-block` inside the inline, the everyday dropdown
// — is still inside the inline, and placing it there resolved it against a containing block
// that had no geometry yet.
const OPEN_INLINE_RELS = [];
const PENDING_INLINE_OOF = [];

function placeAbsolute(child, pos, staticX, staticY, ctx) {
  if (OPEN_INLINE_RELS.length) {
    // The static position moves with every relative inline this box sits in, the same as the
    // content around it — and those offsets are applied to that content at settle time, long
    // after this position was read off the line.
    let dx = 0, dy = 0;
    for (const rel of OPEN_INLINE_RELS) if (rel) { dx += rel.x; dy += rel.y; }
    PENDING_INLINE_OOF.push({ child, pos, staticX: staticX + dx, staticY: staticY + dy, ctx });
    return;
  }
  const vp = viewport();
...
function layoutElement(el, box, ctx, cbW = box.width) {
  ...
  const placeTextRun = (text, owner) => {
    ...
    segments.forEach((segment, i) => {
      ...
      if (!run) return;
      if (!wraps) { placeOnLine(measureRun(run, owner), lh, !outerWraps); return; }
  ...
  const placeOnLine = (w, h, decided, hangs) => {
    ...
    for (const frag of openInlines) {
      ...
      if (!line || line.y !== at.y) {
        line = { y: at.y, minX: at.x, maxRight: -Infinity, hangRight: -Infinity, hangPending: false };
  ...
  const placeInlineBox = (child, pos) => {
    ...
    OPEN_INLINE_RELS.push(frag.rel);
  ...
  const settleInlineBoxes = () => {
    ...
    for (const frag of inlineBoxes) {
      ...
      const rects = frag.lines.map((line) => {
        const right = Math.max(line.maxRight, line.hangRight);
        return { x: line.minX, y: line.y - ce.top,
                 width: Math.max(0, right - line.minX), height: ownH + ce.top + ce.bottom };
      });
      // Nothing was ever placed — not even a padding edge. The box is empty, and it is a
      // zero-WIDTH fragment where the line exists and a zero-SIZE one where it does not.
      if (!rects.length) {
    ...
    // Now every inline box in scope can be a containing block, the out-of-flow boxes held back
    // for them can be placed — and only then does an inline box know its scrollable extent.
    // A block nested INSIDE one settles its own boxes while that one is still open, and the
    // boxes waiting on it must keep waiting.
    if (!OPEN_INLINE_RELS.length && PENDING_INLINE_OOF.length) {
      const pending = PENDING_INLINE_OOF.splice(0, PENDING_INLINE_OOF.length);
      for (const p of pending) placeAbsolute(p.child, p.pos, p.staticX, p.staticY, p.ctx);
    }
...
export function pointerRectOf(el) {
  return clientRectsOf(el)[0] || rectOf(el);
}
