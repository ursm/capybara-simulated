...
// The container's main-axis distribution (justify-content) as a physical code native reads. Exact for the
// gated plain LTR row (no flexReverse / crossFar), where flow-relative and physical coincide.
function flexJustifyCode(el) {
  switch (alignKeyword(declaredValue(el, 'justify-content'))) {
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      const contentH = el._lb.height - e.top - e.bottom;   // …and content height (for a column's row-gap %)
      const items = [];
      for (const c of layoutChildren(el)) {
