...
function layoutElement(el, box, ctx, cbW = box.width) {
  ...
  const lineHangs = [];
  const lineEmpties = [];
  // Out-of-flow boxes inside a fragmented inline. One of them may be their containing
  // block, so they wait until every inline box has its final geometry.
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (pos === 'absolute' || pos === 'fixed') {
      // Inside a fragmented inline the box around this one has no geometry yet, and it may
      // be this one's containing block, so the placement waits for `settleInlineBoxes`.
      if (openInlines.length) deferredOutOfFlow.push({ el: child, pos, staticX: lineX, staticY: flowY });
      else placeAbsolute(child, pos, lineX, flowY, ctx);
      return true;
    }
  ...
  const settleInlineBoxes = () => {
    ...
    for (const frag of inlineBoxes) {
      ...
      // known, so it is carried over now. The line records keep the UNSHIFTED positions:
      // an enclosing inline box, and the line itself, are laid out as if this had not moved.
      if (rel) for (const grand of layoutChildren(box)) {
        if (grand.nodeType === NODE_ELEMENT) shiftSubtree(grand, rel.x, rel.y);
      }
    }
    // Now every inline box can be a containing block, the out-of-flow boxes inside them can
    // be placed — and only then does an inline box know its scrollable extent.
    for (const d of deferredOutOfFlow) placeAbsolute(d.el, d.pos, d.staticX, d.staticY, ctx);
  ...
  for (const child of layoutChildren(el)) {
    ...
    const autoW = laysOutAsTable(child) ? shrinkToFitWidth(child, availW) : availW;
...
function layoutGrid(el, box, content, ctx) {
  ...
  for (const child of layoutChildren(el)) {
    if (child.nodeType !== NODE_ELEMENT || selfNotRendered(child)) continue;
    const pos = positionOf(child);
    if (pos === 'absolute' || pos === 'fixed') { placeAbsolute(child, pos, content.x, rowTop, ctx); continue; }
...
// broke over, and its single box otherwise. `getClientRects` reports exactly this.
export function clientRectsOf(el) {
  const r = rectOf(el);
  if (!el._lbFrags) return [r];
  const { sx, sy } = scrollShift(el);
  return el._lbFrags.map((b) => ({ x: b.x - sx, y: b.y - sy, width: b.width, height: b.height }));
}
