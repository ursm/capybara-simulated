...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    // (…a CAPTION among them: `measure_table` resolves it against the table's border box, the block it spans.)
    const nativeBasis = parentIdx >= 0 && !TABLE_INTERNAL_DISPLAY.has(displayOf(el)) &&
                        (isOutOfFlow ? oofBasis()
                          : parIsElem && nodes[parentIdx] === parLayout && !laysOutAsGrid(parLayout) &&
                            (!laysOutAsTable(parLayout) || displayOf(el) === 'table-caption'));
