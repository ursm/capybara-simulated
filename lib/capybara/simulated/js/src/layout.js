...
function nlTableSupported(el) {
  ...
  for (const row of grid.rows) {
    ...
    for (const cell of row.cells) {
      ...
      if (displayOf(cell.el) !== 'table-cell') return false; // a stray non-cell child (an anonymous CELL the
                                                             // oracle does not model, only anonymous ROWS) → JS
...
function edgeInsets(el, cbW) {
  ...
  const tableOuter = mode === COLLAPSE_TABLE ? tableCollapseOuter(el) : null;
...
function collapseMode(el) {
  ...
  const d = displayOf(el);
  el._lbCollapseMode = (d === 'table-cell' || d === 'table') && tableCollapses(el)
    ? (d === 'table' ? COLLAPSE_TABLE : COLLAPSE_CELL)
    : COLLAPSE_NONE;
