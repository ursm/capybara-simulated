...
function layoutTable(el, box, content, ctx) {
  ...
  if (hasGrid) grid.rows.forEach((row, r) => {
    ...
    for (const cell of row.cells) {
      ...
      const cbox = { x: cellX, y, width: cw, height: size.height, autoHeight: size.autoHeight };
      ...
      const span = { cell, box: cbox, startRow: r, runsFrom, runsTo: recordedRunCount() };
      placed.push(span);
