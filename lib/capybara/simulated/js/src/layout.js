...
// box; a TEXT block is its run stream (`text_intrinsic` reads plain text runs, `<br>`, `<wbr>` under `normal` /
// `nowrap`); a BLOCK container is its block children (each measured the same way). So this mirrors that: the
// element's inline sizing must read the same with no percentage basis as the walk's cbW-resolved record
// (`nlIntrinsicSizingIsPlain`), its content must be text runs native's pen-walk measures (no inline edges,
// atomics, floats, in-word breaking, preserved white-space), and its block children must pass in turn. A
// `nowrap` box is admitted only as a pure text block: the oracle pins the WHOLE box's min-content to its
// max-content, block children included, where native's record tree would keep each child's own minimum. What
// this declines falls back to the oracle's contribution, marshalled resolved — never a wrong measure.
function nlIntrinsicMeasurable(el) {
  ...
  if (nlWrapMode(el) !== 0) return false;                                // break-all / anywhere break inside a word
  let hasBlock = false;
  for (const c of layoutChildren(el)) {
    if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c) || displayOf(c) === 'none') continue;
    const cp = positionOf(c);
    if (cp === 'absolute' || cp === 'fixed') continue;                   // out of flow: sizes nothing
    if (isFloated(c)) return false;                                      // packs on the line like an inline box
    ...
    if (!nlIntrinsicMeasurable(c)) return false;                         // a block-level child, measured in turn
    hasBlock = true;
  }
  return !(hasBlock && ws === 'nowrap');
}
// An inline element whose content native's pen-walk measures as plain text runs: edgeless (an edged inline
// puts its OPEN / CLOSE widths on the line — not measured yet), no in-word breaking, holding only text, `<br>`
// / `<wbr>`, and such inlines. (A different `white-space` is declined by the walk itself.)
function nlInlineMeasurable(el) {
  ...
  if (e.ml + e.left + e.right + e.mr !== 0) return false;
  if (nlWrapMode(el) !== 0) return false;
  for (const c of layoutChildren(el)) {
    if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c) || displayOf(c) === 'none') continue;
    if (c._tag === 'br' || c._tag === 'wbr') continue;
...
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode) {
  ...
  // break-word/anywhere — stamped on each text run so the Rust line breaker may break inside an over-long word.
  shift = shift || 0;
  wrapMode = wrapMode || 0;
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    // An ENTIRELY-whitespace preserve block (only whitespace-only DIRECT text, no real inline/block content):
    // the oracle gives it a line box (its own height) that does NOT propagate to the parent's height (measured:
    // the block is 22 tall, its body 0) — a quirk native's block flow can't reproduce. Decline. (Whitespace
    // alongside real content, or INSIDE an inline element, is ordinary content and lays out through the text
    // path below.)
    if (sawPreWs && !hasInline && !hasNonFloatBlock) return false;
  ...
  for (let i = 0; i < runsAll.length; i++) {
    ...
    if (r.kind === NL_RUN_TEXT) {
      ...
      runBuf[o + 6] = r.wrapMode || 0; // in-word break mode (metric is unused for a text run — 0/1/2)
  ...
  const walk = (node) => {
    for (const child of layoutChildren(node)) {
      ...
      if (isContinuedInline(child)) {
        ...
        if (ce.ml + ce.left + ce.right + ce.mr) takePending();
...
// fills the line it is on, breaking between characters), 2 = `overflow-wrap: break-word` / `anywhere` (such a
// word first moves to a FRESH line, then breaks). `break-all` suppresses the fresh line, so it wins over 2.
function nlWrapMode(el) {
  const w = wrapRulesOf(el);
  if (!w.any) return 0;
  return w.breakAll ? 1 : 2;
}
