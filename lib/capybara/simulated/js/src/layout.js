...
// Everything else — inline-flex, wrap-reverse, rtl-column with a cross auto margin or wrap, vertical writing
// modes — declines to JS.
...
// out. (`inline-grid` is an atomic inline, handled by the text path; an abspos / floated grid is placed by its
// PARENT's out-of-flow replay before it is walked here.)
function laysOutAsGrid(el) {
  return displayOf(el) === 'grid';
}
function nlGridSupported(el) {
  if (displayOf(el) !== 'grid') return false;
  const p = positionOf(el);
