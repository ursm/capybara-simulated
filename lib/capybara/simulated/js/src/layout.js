...
function layoutTable(el, box, content, ctx) {
  ...
  // visible bottom of a cell has to land inside it. The surplus goes to the AUTO rows (no declared height) in
  // proportion to their content, NOT equally over all of them (Chrome: rows of 10 and 30 in a 100px table
  // become 25 and 75, and a declared-height row keeps its height and takes none). When the auto rows have no
  // content between them it is split equally among THEM; when every row has a declared height there are no auto
  // rows to feed, so it spreads over all of them in proportion to their heights. Row positions move, so this
  // happens before any of them is stamped.
  if (imposedContentH > 0 && hasGrid) {
    ...
    if (room > 0) {
      const autos = [];
      for (let i = 0; i < rowHeights.length; i++) if (!rowDeclared[i]) autos.push(i);
      const targets = autos.length ? autos : rowHeights.map((_, i) => i);
      let weightSum = 0;
