...
function flexPhysicalAxes(el) {
  ...
  const crossFlip = String(declaredValue(el, 'flex-wrap') || '').trim().toLowerCase() === 'wrap-reverse';
...
function crossAlign(containerAlign, child, axes = null) {
  ...
  if (align === 'last baseline') return atEnd;
  if (align === 'baseline') return atStart;
  return align;
}
