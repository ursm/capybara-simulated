...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  // `rewind` marks a break that OVERFLOW forced — the line ran out of room, and there was no
  // break opportunity in it. Only then does an inline box's opening edge move down with the
  // content it was waiting for (see below); a `<br>` or a preserved newline is a break
  // opportunity AFTER that edge, so Chrome leaves the fragment it opened where it is.
  const forceBreak = (rewind) => {
    dropHangs(true);
    // An inline box's opening edge is placed when the box OPENS, which is before its first word
    // is measured — so when that word doesn't fit, the edge is left stranded on the line above
    // and the box reports a fragment there that a browser never made. Chrome opens the box on
    // the line its content starts on: the edge moves down with it.
    const carried = [];
    if (rewind) for (let i = 0; i < openInlines.length; i++) {
      const frag = openInlines[i];
      const line = frag.lines[frag.lines.length - 1];
      if (!line || line.y !== flowY || !line.edgeOnly) continue;
      frag.lines.pop();
      carried.push({ frag, depth: i });
    }
    ...
    hadInFlow = true;
    // …re-opening each carried box at the start of the new line, outermost first. Its OWN edges
    // move, and they grow the boxes around it — never the ones inside it, which are carrying
    // their own.
    for (const { frag, depth } of carried) {
      const w = frag.ce.ml + frag.ce.left;
      if (!w) continue;
      const at = lineX;
      lineX += w;
      if (frag.ownH > lineH) lineH = frag.ownH;
      linePlaced = true;
      hadInFlow = true;
      for (let i = 0; i <= depth; i++) {
        notePlacement(openInlines[i], at, lineX, flowY, false, true, i === depth ? frag.ce.ml : 0);
      }
    }
  };
  ...
  const placeTextRun = (text, owner) => {
    ...
    segments.forEach((segment, i) => {
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        if (split.freshLine && lineHasContent) forceBreak(true);
        for (const unit of split.units) {
          const w = measureRun(unit, owner);
          if (lineHasContent && (lineX - content.x) + w > content.width) forceBreak(true);
  ...
  const placeOnLine = (w, h, decided, hangs, edge) => {
    ...
    flushMargin();
    if (!decided && lineHasContent && lineX + w > right) forceBreak(true);
  ...
  const notePlacement = (frag, fromX, toX, y, hangs, edge, inset) => {
    ...
    if (!line || line.y !== y) {
      line = { y, minX: fromX + inset, maxRight: -Infinity, hangRight: -Infinity,
               hangPending: false, edgeOnly: true };
      frag.lines.push(line);
    }
    if (!edge) line.edgeOnly = false;
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (pos === 'absolute' || pos === 'fixed') { placeAbsolute(child, pos, lineX, flowY, ctx); return true; }
  ...
  const placeInlineBox = (child, pos) => {
    ...
    const frag = {
      ...
      // `ownH` is also what re-opening a carried edge on the next line places (see `forceBreak`).
    ...
    // The margin and the left padding/border open the box on the line its content starts on —
    // as ONE placement, so that a break carries them down together (see `forceBreak`). The
    // margin sits OUTSIDE the box, so the fragment starts after it; the right edge closes the
    // box wherever the content ended.
    if (ce.ml || ce.left) {
      const at = lineX;
      placeOnLine(ce.ml + ce.left, ownH, true, false, true);
      // `placeOnLine` grew every open box from the placement's start; this one starts after its
      // own margin, which sits outside it.
      const line = frag.lines[frag.lines.length - 1];
      if (line && line.minX === at) line.minX = at + ce.ml;
    }
  ...
  const settleInlineBoxes = () => {
    ...
    for (const d of deferredOutOfFlow) placeAbsolute(d.child, d.pos, d.staticX, d.staticY, d.ctx, d.order);
...
function establishesStackingContext(el) {
  if (positionOf(el) === 'fixed') return true;
  if (positionOf(el) === 'static') return false;
  const z = declaredValue(el, 'z-index');
...
function paintsAbove(cand, best) {
  ...
  // The chains agree as far as the shorter one goes, so one box is inside the other's stacking
  // context — an element's own box never covers its own content.
  ...
  // Same context, so the boxes compete directly: by paint rank, then by where they sit.
  const rc = paintRank(cand), rb = paintRank(best);
