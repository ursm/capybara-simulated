...
function nlIndentOf(owner, el, edges) {
  const indent = textIndentOf(owner, () => Math.max(0, el._lb.width - edges().left - edges().right));
  if (!indent) return null;
  return { px: indent.px, bits: (indent.hanging ? 256 : 0) | (indent.eachLine ? 512 : 0) };
}
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      const groupIndent = nlIndentOf(flowContainer(el), el, eOracle);
    ...
    if (hasInline) {
      ...
      const ownIndent = nlIndentOf(flowContainer(el), el, eOracle);
      if (ownIndent && measured > 0) return false;   // …the intrinsic measure of an indented block: see above
    ...
    if (oof.length) {
      nlWriteIndent(rec, nlIndentOf(flowContainer(el), el, eOracle), false);
