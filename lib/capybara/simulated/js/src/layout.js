...
const NL_STRIDE = 54;   // node record; must match layout::Input / LAYOUT_STRIDE
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      // (2) A min/max-height flex container whose final height native can't reconstruct as the oracle used it
      // declines rather than mislay the main extent / gap basis. With an UNDECLARED (auto) height the oracle's
      // two-phase lays out against the pre-clamp extent (the stretch room, or the content floored by
      // min-height) THEN clamps the box — but native gets only the POST-clamp height: definiteness from
      // cross-STRETCH / abspos (autoHeight false) loses the pre-clamp stretch room; the same container as a
      // flex ITEM loses the auto two-phase (the parent's item-push overwrites its record with the post-clamp
      // height and wipes min/max). A DECLARED height is pre-clamped consistently, so it stays native.
      if ((declaresLayoutProp(el, 'min-height') || declaresLayoutProp(el, 'max-height')) &&
          resolveLayoutProp(el, 'height', cbH) == null && (el._lb.autoHeight === false || isFlexItem)) return false;
      ...
      for (const c of items) {
        ...
        r[6] = r[7] = r[8] = r[9] = NaN;                   // already clamped — no min/max re-clamp
