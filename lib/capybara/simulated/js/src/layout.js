...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    // An `rtl` block places its in-flow BLOCK children from the inline-start = RIGHT edge (r1, rec[51] below).
    ...
    rec[51] = rtl ? 1 : 0;   // direction:rtl — native places this block's children from the RIGHT edge (r1)
    recs.push(rec);
    ...
    if (laysOutAsTable(el)) {
      ...
      const sp = borderSpacingOf(el);                      // 0 when border-collapse:collapse
      rec[44] = sp.x; rec[45] = sp.y;
      rec[49] = tableCollapses(el) ? 1 : 0;                // collapse adds the half-border frame (measure_table)
      const grid = tableGrid(el);
      if (grid.captions.length) rec[50] = captionAtBottom(grid.captions[0]) ? 1 : 0; // t4: 0 top / 1 bottom
      ...
      // child — and stacks it above / below the grid per rec[50] (caption-side).
