...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  const d = globalThis.__dom;
  ...
  if (!root || root._nid == null || root._nid < 0 || !root._lb) return { ok: false, reason: 'no root box' };
  ...
  const rootCbW = root._lbCbW != null ? root._lbCbW : root._lb.width;
  const gridBuf = Float64Array.from(gridsAll);
  if (!d.layoutPass(buf, runBuf, runTexts, root._lb.x, root._lb.y, rootCbW, gridBuf)) return { ok: false, reason: 'native declined' };
  ...
  for (const el of nodes) {
    ...
    const nb = d.boxOf(el._nid), jb = el._lb;
