...
// The `flex` shorthand's three parts, defaulted per spec: `flex: <n>` is `<n> 1 0%`, `flex: auto`
// is `1 1 auto`, `flex: none` is `0 0 auto`, and an absent `flex` leaves the initial `0 1 auto`.
function flexParts(el) {
  if (el._lbFlexPass === layoutPass) return el._lbFlex;
  el._lbFlexPass = layoutPass;
  el._lbFlex = computeFlexParts(el);
  return el._lbFlex;
}
// Four cascade reads per item per pass without the memo above — the same shape that made the
// used-display lookup the dominant cost before it was cached.
function computeFlexParts(el) {
  const grow = cascadedProperty(el, 'flex-grow');
  const shrink = cascadedProperty(el, 'flex-shrink');
  const basis = cascadedProperty(el, 'flex-basis');
  let g = 0, sh = 1, b = 'auto';
  const f = cascadedProperty(el, 'flex');
  if (f != null) {
    const s = String(f).trim().toLowerCase();
    if (s === 'none')            { g = 0; sh = 0; b = 'auto'; }
    else if (s === 'auto')       { g = 1; sh = 1; b = 'auto'; }
    else if (s === 'initial')    { g = 0; sh = 1; b = 'auto'; }
    else {
      const parts = s.split(/\s+/);
      const n = parseFloat(parts[0]);
      if (isFinite(n)) {
        g = n;
        sh = parts.length > 1 && isFinite(parseFloat(parts[1])) ? parseFloat(parts[1]) : 1;
        b  = parts.length > 2 ? parts[2] : (parts.length === 2 && !isFinite(parseFloat(parts[1])) ? parts[1] : '0');
      }
    }
  }
  if (grow   != null && isFinite(parseFloat(grow)))   g  = parseFloat(grow);
  if (shrink != null && isFinite(parseFloat(shrink))) sh = parseFloat(shrink);
  if (basis  != null) b = String(basis).trim().toLowerCase();
  return { grow: Math.max(0, g), shrink: Math.max(0, sh), basis: b };
}
...
function layoutFlexRow(el, box, ctx, { equalShare = false } = {}) {
  ...
  for (const { child, pos } of items) {
    ...
    // The flex pass above already resolved this axis: a declared width is an item's flex BASE, not
    // its final width, so it must not win here the way it does in block flow (two 500px items in a
    // 600px row shrink to fit, they don't overflow to 900).
    size.width = content;
...
function resolveFlexRowWidths(items, box, share) {
  ...
  const mins   = items.map(({ child }) => {
    const declared = resolveLayoutProp(child, 'min-width', box.width);
    if (declared != null) return declared;
    // `min-width: auto` on a flex item is its content size — the rule that keeps a label or a
    // button from being squeezed to nothing.
  ...
  const bases = items.map(({ child }, i) => {
    ...
    if (declared != null) return declared;
    const b = parts[i].basis;
    if (b !== 'auto' && b !== 'content') {
      const px = parseFloat(b);
      if (isFinite(px)) return b.endsWith('%') ? (px / 100) * box.width : px;
    }
    return Math.min(textWidth(subtreeTextLength(child)), box.width) || 0;
  });

  const total = bases.reduce((a, b) => a + b, 0);
  const free  = box.width - total;
  if (free > 0) {
    const totalGrow = parts.reduce((a, p) => a + p.grow, 0);
    if (totalGrow <= 0) return bases.map((b) => b || share);
    return bases.map((b, i) => Math.round(b + (free * parts[i].grow) / totalGrow));
  }
  if (free < 0) {
    // Shrink proportionally to `flex-shrink x base` (the spec's weighting), never past the minimum.
    const weights = bases.map((b, i) => b * parts[i].shrink);
    const totalW  = weights.reduce((a, b) => a + b, 0);
    if (totalW <= 0) return bases.map((b) => b || share);
    return bases.map((b, i) => Math.round(Math.max(mins[i], b + (free * weights[i]) / totalW)));
  }
  return bases.map((b) => b || share);
}
...
// Of two boxes that both contain the point, which paints on top? A DESCENDANT always paints above
// its ancestors — an element's own box never covers its own content, whatever its `position` is.
// Comparing paint rank globally got this backwards: a `position: fixed` container (rank 0.5)
// outranked the static content inside it, so a click aimed at an image inside Discourse's fixed
// composer hit the composer instead, and the editor never saw a selection. Unrelated subtrees still
// compare by paint rank, then tree order.
