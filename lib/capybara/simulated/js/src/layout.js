...
const NL_STRIDE = 35;   // node record; must match layout::Input / LAYOUT_STRIDE
...
// A flex container the native engine can place (§9.7, f1 row / f3 column): a plain LTR horizontal-tb
// `row` OR `column`, `nowrap`, in normal flow, with no cross-axis min/max clamp (the two-phase stretch
// clamp) and no bare text (an anonymous flex item). The item SIZING is resolved JS-side; native does only
// the placement. Everything else — inline-flex, any `-reverse`, rtl / vertical writing modes, wrap —
// declines to JS.
function nlFlexSupported(el) {
  ...
  // Plain LTR horizontal-tb, nowrap, no flex-direction reverse: a row (main inline-start=left, cross
  // block-start=top) or a column (main block-start=top, cross inline-start=left).
  const rowPlain = plan.mainStart === 'left' && plan.crossStart === 'top';
  const colPlain = plan.mainStart === 'top' && plan.crossStart === 'left';
  if (plan.mode !== 'horizontal-tb' || plan.flexReverse || plan.wrapMode !== 'nowrap' || !(rowPlain || colPlain)) return false;
