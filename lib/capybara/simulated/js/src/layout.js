...
function ensureLayout() {
  ...
  if (root) {
    ...
    const bodyRendered = bb && globalThis.__isLaidOutNode && globalThis.__isLaidOutNode(body);
...
function stampExtent(el, box) {
  ...
  for (const child of layoutChildren(el)) {
    // Content that OVERFLOWS a scroll container is scrollable within it, not part of what its
    // ancestors wrap: only the container's own box counts (Chrome — a 200px-tall `overflow: auto`
    // box holding 2400px of rows leaves `body.scrollHeight` at the viewport height, and it is that
    // box's `scrollHeight` that reads 2400).
    // Content that OVERFLOWS a scroll container is scrollable within it, not part of what its
    // ancestors wrap: only the container's own box counts toward their scrollHeight (Chrome — a
    // 200px `overflow: auto` box holding 2400px of rows leaves `body.scrollHeight` at 200, and it
    // is that box's own `scrollHeight` that reads 2400). The FLOW extent below is left alone: it
    // sizes auto-height ancestors, and shrinking those relaid the editor Avo's code field is built
    // on into a loop.
    const ce = (child._lb && isScrollContainer(child))
...
function isClipped(el) {
  ...
  for (let p = layoutParent(el); p; p = layoutParent(p)) {
    ...
    if (p === root) continue;
    if (p === bodyEl && !isScrollContainer(root)) continue;
...
export function ensureInView(el) {
  ...
  for (let p = el; p; p = layoutParent(p)) {
    if (p !== root && !(p !== el && isScrollContainer(p))) continue;
