...
const NL_STRIDE = 92;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start / table columns, 56-61 = declared sizing, 62-67 = flex sizing, 68-70 = replaced intrinsic, 71-76 = out-of-flow, 77-79 = control baseline, 80-88 = table, 89-90 = basis-less edges, 91 = block axis is horizontal)
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  // its static position. Where the CB lies outside the pass (the viewport, an ancestor above the root, an
  // inline box), the oracle's resolved box is replayed: pushed with its displacement from THIS container.
  const emitOutOfFlow = (c, el, idx, crossAlignCode) => {
    const pos = positionOf(c);
    const cbEl = containingBlockElementFor(c, pos === 'fixed');
    ...
    const cb = cbEl && cbIdx != null ? paddingBoxOf(cbEl) : null;
