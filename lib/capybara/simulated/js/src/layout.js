...
function nlShadowRun(rootEl, trace) {
  ...
  // A push that writes the oracle's FINAL height over a grid's record (rec[5]) hides whether that height was definite
  // when the grid resolved its percentage row gap — `layoutGrid` resolves it against the content height only then,
  // and an auto-height box's is still 0 there. So the push says it (rec[54], read for a grid as "the height was not
  // definite at gap time"): the oracle's `_lbDefiniteH`. A record whose height is its own declaration's needs
  // nothing — definite exactly when rec[5] is set — and neither does a height native imposes.
  const markPushedGridHeight = (r, c) => { if (r[2] === 7) r[54] = c._lbDefiniteH ? 0 : 1; };
  const emitOutOfFlow = (c, el, idx, crossAlignCode) => {
    ...
    r[3] = 1; r[4] = c._lb.width; r[5] = c._lb.height;       // pushed border box (insets / static corner resolved)
    markPushedGridHeight(r, c);
    r[6] = r[7] = r[8] = r[9] = NaN;                          // already sized — no min/max re-clamp
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      const contentW = el._lb.width - e.left - e.right;    // the items' containing-block content width
      // Two gap bases, which DIFFER for an auto-height container. A ROW's cross-gap (row-gap) % resolves
      // against content.height; a COLUMN's main-gap (row-gap) % against definiteMainHeight (the min-height
      // FLOOR when the height is auto). Both turn on "is the height DEFINITE?" — which includes a height
      // imposed by cross-stretch or abspos (oracle: `box.autoHeight === false`), not only a declared one, so
      // read what the oracle stamped at layout entry (`_lbDefiniteH`) rather than just the `height`
      // declaration. A genuinely auto height with no floor is 0 (the flex pass precedes the min/max clamp).
      // Sharing one basis fed the floor into a row's row-gap; gating on "declared" dropped a stretch-definite
      // container's basis to 0 and a grid-row-sized one's too. Px gaps are free.
      const heightDefinite = el._lbDefiniteH;   // the box arrived with a height (declared / stretched / a grid row)
      const contentH = heightDefinite ? (el._lb.height - e.top - e.bottom) : 0;
      let colMainH = contentH;
      // …and the column's MAIN size as the oracle's `definiteMainHeight` has it: the definite content height, else
      // a min-height FLOOR, else null (nothing to resolve a % flex-basis against — the gap reads it as 0).
      let colMain = heightDefinite ? contentH : null;
      if (!heightDefinite) {
        const minH = declaresLayoutProp(el, 'min-height') ? resolveLayoutProp(el, 'min-height', cbH) : null;
        if (minH != null && minH > 0) colMain = colMainH = Math.max(0, isBorderBox(el) ? minH - (e.top + e.bottom) : minH);
      }
      ...
      rec[37] = axisGap(el, plan.crossGap, plan.mainIsX ? contentH : contentW); // cross gap between lines
      ...
      const emitItems = (native) => {
        ...
        rec[32] = items.length > 1 ? axisGap(el, plan.mainGap, plan.mainIsX ? contentW : colMainH) : 0;
        for (const c of items) {
          ...
          if (native) {
            ...
            r[63] = kw ? NaN : num(resolveLayoutProp(c, 'flex-basis', plan.mainIsX ? contentW : colMain));
          ...
          r[3] = 1;                                          // its used size is a border box (flex-resolved)…
          r[4] = c._lb.width;                                // …physical width  (native swaps main/cross)
          r[5] = c._lb.height;                               // …physical height
          // …which hides whether that height was the item's own CONTENT, so a flex container item says it here
          // (rec[54], `item_auto_height`): the oracle's `autoHeight` for the box it pushed, false where a stretch or
          // its main size imposed it. Nothing else needs saying — a record whose height is its own declaration's is
          // auto exactly when rec[5] is, and a height native imposes clears the flag there (`with_imposed_height`).
          if (r[2] === 3) r[54] = c._lb.autoHeight !== false ? 1 : 0;
          markPushedGridHeight(r, c);
          r[6] = r[7] = NaN;                                 // main width already flex-resolved — no width re-clamp
          // KEEP min/max-HEIGHT (r[8]/r[9]) — as BORDER-box figures now that r[3] says border-box: a content-box
          // item's declared min/max-height gains its vertical edges (else a `max-height: 50px; padding: 10px` item
          // is clamped to 50 where the oracle's box is 70). Re-clamping the pushed height by them is idempotent
          // (the oracle already clamped), but native READS them to reproduce the auto-height two-phase for a flex-item flex
          // ROW (measure_flex, gated on rec[54]) — a min-height FLOOR aligns its children in the pre-floor content
          // and a max-height CAP lets a taller row overflow, both computed from the item's own content, box
          // floors/caps around them (the Avo `field-wrapper`). rec[8]/rec[9] carry the item's resolved
          // min/max-height from `walk`; the flex gate declines only a box the FLOW already made DEFINITE that a
          // clamp then binds (autoHeight === false — native holds no pre-clamp extent).
          if (!r[61]) {
    ...
    if (laysOutAsTable(el)) {
      ...
      if (grid.captions.length) {
        ...
        cr[3] = 1; cr[4] = cap._lb.width; cr[5] = cap._lb.height; // pushed like a cell; measure_table stacks it
        cr[6] = cr[7] = cr[8] = cr[9] = NaN;
        cr[54] = 0;                                        // caption box replayed definite — not self-sized, no two-phase
        markPushedGridHeight(cr, cap);
