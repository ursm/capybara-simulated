...
function nlTableSupported(el) {
  ...
  // row for. A COLLAPSE frame's left/right swap isn't reflected yet, so an rtl collapse table still declines.
  if (flowSides(el).rtl && tableCollapses(el)) return false;
  // Native lays every caption at the table's left edge spanning its full width. That IS a caption's place in
  // both directions — UNLESS an rtl caption is narrower than the table (or auto-margin-offset), which belongs at
  // the inline-start = RIGHT (native doesn't mirror the caption's own inline placement yet). Decline just those:
  // a full-width, left-flush caption native reproduces exactly.
  if (flowSides(el).rtl && grid.captions.length) {
    if (!el._lb) return false;
    for (const cap of grid.captions) {
      const cb = cap._lb;
      if (!cb || Math.abs(cb.x - el._lb.x) > 0.01 || Math.abs(cb.width - el._lb.width) > 0.01) return false;
    }
  }
...
function clampToMinMax(el, size, axis, basis, extra) {
  ...
  // A table cell IGNORES min-height / max-height (their effect on cells is undefined in CSS 2.2 §17.5.3;
  // Chrome and Firefox treat both as `auto`): a cell's block size comes only from its `height` — itself a
  // MINIMUM — and its content. `min-width` / `max-width` are not affected (they feed the column width).
  // …unless the `table-cell` is a flex / grid ITEM: a real browser BLOCKIFIES it (CSS Display §2.7), so it
  // is a block whose min/max-height DO apply — the sizing path treated it as a flex item all along, only
  // this clamp had to stop reading it as a cell.
  if (axis === 'height' && displayOf(el) === 'table-cell' && !isFlexOrGridItem(el)) return size;
...
function ensureCollapseBorders(table) {
  ...
  for (const col of grid.columns) {
    ...
    for (let i = 0; i < span && cIdx + i < n; i++) {
      if (!group || i === 0) colL[cIdx + i] = combineW(colL[cIdx + i], cl);
      if (!group || i === span - 1) colR[cIdx + i] = combineW(colR[cIdx + i], cr);
  ...
  for (const row of grid.rows) {
    for (const cell of row.cells) {
      const atLeft = cell.col === 0, atRight = cell.col + cell.colSpan >= n;
      ...
      // rows / row-group boundary (horizontal).
      const bl = atLeft
        ? outerV(cell._rbL, tbL, colL[0], cell.row, cell.rowSpan, rowL, grpL)
        : faceRows(cell._rbL, combineW(colR[cell.col - 1], colL[cell.col]), cell.row, cell.rowSpan, cell.col - 1, '_rbR');
      const br = atRight
        ? outerV(cell._rbR, tbR, colR[n - 1], cell.row, cell.rowSpan, rowR, grpR)
