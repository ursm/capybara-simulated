...
// A REPLACED element native replays as a LEAF box: the oracle resolved its border box from the intrinsic size +
// CSS (`intrinsicSize` — svg / img / canvas / input / select / textarea / …), so native pushes that box and emits
// no subtree, exactly like an out-of-flow replay but in flow. Whether it lays out CSS boxes INSIDE itself does
// not change that — a dropdown's `<option>`s have no box in Chrome at all, and the oracle sizes the control from
// its intrinsic size either way, never by stacking them. (That the control's own box is the ORACLE's remains a
// standing dependency of the campaign, rec[68..70]; see the replay ledger in the campaign notes.)
//
...
function nlFlexSupported(el) {
  ...
  if (nlButtonContainer(el)) return false;
  // …a button that is ITSELF a flex container included: its own algorithm sizes it, not the block-flow
  // shrink-wrap native does now (`nlButtonContainer` carries every qualifier that says which).
...
function nlGridSupported(el) {
  ...
  if (nlButtonContainer(el)) return false;   // a `display: grid` <button>, the twin of the flex case
...
function nlAtomicNative(c, cdisp) {
  ...
  if (cdisp === 'inline') return c._tag === 'img' && nlReplacedLeaf(c);
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    for (const c of layoutChildren(el)) {
      ...
      // its intrinsic size; an INLINE replaced element is an atomic inline (handled by the text path, which
      // still declines) so it is NOT taken here — it falls through to the inline fold below.
...
// …and the one shape of it native still gets wrong: a button that is ITSELF a flex or grid container. The
// block-flow shrink-wrap is native's own (rec[65] bit 128 reaches `block_child_width`), but a flex or grid
// container's own algorithm sizes it, and native sized a `display: grid` button at 16 where the oracle said
// 242.76 -- 43 mismatches in a 4,860-case sweep. Asked by those two gates only.
// …and the one shape of it native still gets wrong: a button that is ITSELF a flex or grid container. The
// block-flow shrink-wrap is native's own now (rec[65] bit 128 reaches `block_child_width`), but a flex or
// grid container's own algorithm sizes it, and native answered 16 where the oracle said 242.76 -- 43
// mismatches in a 4,860-case sweep.
//
// Asked with every qualifier the block-flow shrink-wrap has, because those are what say the shrink-wrap
// DECIDES this box's width at all: a flex or grid ITEM is sized by its container's algorithm, an
// out-of-flow box by the inset rules, a float by its own shrink-to-fit, an inline box by its content, and a
// declared or keyword width by itself -- native is right in every one of those, and dropping the qualifiers
// cost 447 shapes of a 4,860-case sweep nothing but coverage.
function nlButtonContainer(el) {
  if (!shrinkWrapsToFit(el)) return false;
  const p = positionOf(el);
  if (p === 'absolute' || p === 'fixed' || isFloated(el) || isFlexOrGridItem(el)) return false;
  if (displayOf(el) === 'inline') return false;             // an inline box's width is not a block-flow auto width
  // …and a keyword width (`fit-content` / `min-content` / `max-content`) is not `auto` either: it is native's
  // own (rec[65] bits 11-12 pin the box's contribution to the figure the keyword names). `resolveLayoutProp`
  // answers null for one, so the keyword has to be asked for separately or every keyword-width box is refused.
  const info = {};
  if (el._lbCbW == null) return true;                       // no basis to resolve a % against — refuse, never guess
  return resolveLayoutProp(el, 'width', el._lbCbW, info) == null && !info.keyword;
}
