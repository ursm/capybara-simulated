...
function nlTableSupported(el) {
  ...
  const imposedH = (el._lb && el._lb.autoHeight === false) || declaresLayoutProp(el, 'min-height') ? 1 : 0;
  ...
  if (usesFixedLayout(el, el._lb ? el._lb.width : 0)) {
  ...
  // t4: a SINGLE caption (top or bottom) is placed natively — a block box in the table's content width,
  ...
  if (grid.captions.length === 1) {
    ...
    const cm = edgeInsets(cap, el._lb ? el._lb.width : 0);
    if (cm.mt || cm.mr || cm.mb || cm.ml || cm.autoMargins) return false; // caption margin → JS
    // A caption spans the table's BORDER box (§17.4 wrapper). A `%` width wider than that overflows the table
    // without growing it; a definite length grows the border box so the caption EXACTLY fits (equal widths).
    // Native's wrapper union would over-grow to an OVERFLOWING caption, so decline only that — the oracle lays
    // it out correctly. The threshold is the shadow compare epsilon (0.01): a flooring caption sits at diff 0
    // (captionsFloor grew el._lb.width to the same value usedSize gives the caption), so even a sub-pixel `%`
    // overflow is declined rather than silently mislaid by native.
    if (cap._lb && el._lb && cap._lb.width > el._lb.width + 0.01) return false;
  }
  ...
  for (const row of grid.rows) {
    ...
    for (const cell of row.cells) {
      ...
      if ((imposedH > 0 || resolveLayoutProp(cell.el, 'height', cell.el._lbCbH) != null) &&
          cellHasPctHeightChild(cell.el)) return false;
    }
...
const NL_RUN_TEXT = 0, NL_RUN_OPEN = 1, NL_RUN_CLOSE = 2, NL_RUN_BR = 3, NL_RUN_ATOMIC = 4, NL_RUN_WBR = 5, NL_RUN_OOF = 6;
...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  const {runs, block, atomic, oof} = ctx;
...
function nlShadowRun(rootEl, trace) {
  ...
  // (rec[84..85]): `table_columns` / `table_min_max_with_caption` read that figure and never descend.
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    // that block: an in-flow child of a block or flex container, which native resolves at the parent's measure
    // (`Input::with_percent_sizes`). There a PLAIN percentage goes over as a fraction (rec[100..105]). Anywhere
    // else — the pass root, an out-of-flow box, a grid item (its track), a table or a part of one, a box whose
    // record parent is an anonymous block or an inline's text block — and for a percentage inside a math
    ...
    const nativeBasis = parentIdx >= 0 && !laysOutAsTable(el) && !TABLE_INTERNAL_DISPLAY.has(displayOf(el)) &&
                        displayOf(el) !== 'table-caption' &&
                        (isOutOfFlow ? oofBasis()
                          : parIsElem && nodes[parentIdx] === parLayout && !laysOutAsGrid(parLayout) && !laysOutAsTable(parLayout));
    ...
    if (laysOutAsTable(el)) {
      ...
      const columnsMeasureCells = !usesFixedLayout(el, cbW());
      const grid = tableGrid(el);
      ...
      const emitRow = (row, parentIdx) => {
        ...
        for (const cell of row.cells) {
          ...
          if (!cellMeasured) {
            ...
            if (columnsMeasureCells) pushedContributions++;   // asked for, and native could not produce it
          }
      ...
      // The caption (t4): a block box under the table (wrapper), pushed like a cell (its own block/text subtree
      // laid out; its display is KEPT). measure_table finds it structurally — the table's only non-row/-group
      // child — and stacks it above / below the grid per rec[49] (caption-side).
      if (grid.captions.length) {
        const cap = grid.captions[0];
        // A caption FLOORS the table's intrinsic width (§17.4 — its margin box spans the wrapper), so native
        // measures it like a cell, with the oracle's contribution where it can't.
        const capIdx = nodes.length;
        // Its own block/text subtree (display kept), and a measure BOUNDARY where native cannot measure it: the
        // oracle's contribution rides rec[84..85] below and `table_min_max_with_caption` reads that instead of
        // descending. But a caption is NOT the cell's twin in WHEN it is read: `table_columns` reads every
        // measurable cell on every table measure, while a caption's contribution is read only by
        // `table_intrinsic_widths` — that is, only where the TABLE's own contribution is asked, which is exactly
        // what `measured` already says. So this one measures where it is asked AND native can, and parks
        // otherwise; marking it unconditionally declined a normal-flow table over a measure nobody makes.
        const capMeasured = measured > 0 && nlIntrinsicMeasurable(cap) && walkAttempt(cap, idx);
        if (!capMeasured && !walkParked(cap, idx)) return false;
        const cr = recs[capIdx];
        pushBorderBox(cr, cap, false);                     // pushed like a cell; measure_table stacks it
        cr[54] = 0;                                        // caption box replayed definite — not self-sized, no two-phase
        // The contribution rides the record for every caption this walk did NOT measure — the same condition the
        // parking uses, so no parked subtree is ever left without a figure to answer with. (A measurable caption
        // nobody asked about is one of those, and pushing its figure costs nothing: an ask implies a mark, and if
        // that ever stopped holding, Rust would otherwise descend into a run stream it cannot measure.) The
        // COUNTER stays on what native genuinely cannot measure, so a spec can see that fallback being taken.
        if (!capMeasured) {
          const iw = intrinsicWidths(cap);
          cr[84] = iw.min; cr[85] = iw.max;
          if (measured > 0) pushedContributions++;
        }
      }
      return true;
    }
    ...
    if (hasInline) {
      ...
      const g = nlGatherRuns(layoutChildren(el), bf, {runs: local, block: el, atomic, oof: oofHook}, wsMode, 0, nlWrapMode(el));
  ...
  const placeTextRun = (text, owner) => {
    ...
    segments.forEach((segment, i) => {
      ...
      if (!wraps) {
        ...
        if (trailing && lineX > lineLeft) placeOnLine(spaceW, lh, true, true, false, ownAsc);
...
function tableGrid(table) {
  if (ORACLE_TRACE) ORACLE_TRACE.helper('tableGrid');
...
function layoutTable(el, box, content, ctx) {
  ...
  const layCaption = (caption, atBottom) => {
    ...
    const size = usedSize(caption, availW, 0, box.width, box.height || null);
