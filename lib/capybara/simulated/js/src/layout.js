...
const NL_STRIDE = 38;   // node record; must match layout::Input / LAYOUT_STRIDE
...
// A flex container the native engine can place (§9.7, f1 row / f3 column / f4 wrap): a plain LTR
// horizontal-tb `row` OR `column`, `nowrap` OR `wrap`, in normal flow, with no cross-axis min/max clamp
// (the two-phase stretch clamp) and no bare text (an anonymous flex item). The item SIZING is resolved
// JS-side; native does only the placement. Everything else — inline-flex, any `-reverse`, `wrap-reverse`,
// rtl / vertical writing modes — declines to JS.
function nlFlexSupported(el) {
  ...
  // Plain LTR horizontal-tb, no flex-direction reverse, no wrap-reverse (which flips the cross axis): a
  // row (main inline-start=left, cross block-start=top) or a column (main block-start=top, cross left).
  const rowPlain = plan.mainStart === 'left' && plan.crossStart === 'top';
  const colPlain = plan.mainStart === 'top' && plan.crossStart === 'left';
  if (plan.mode !== 'horizontal-tb' || plan.flexReverse || plan.wrapMode === 'wrap-reverse' || !(rowPlain || colPlain)) return false;
...
// The container's main-axis distribution (justify-content) as a code native reads, resolved to the MAIN
// axis. The physical keywords `left`/`right` name the inline axis: on a ROW's inline main axis they are
// start/end (LTR), but on a COLUMN's block main axis they name nothing and resolve to `start` — exactly
// what justifyPhysicalTarget does (it returns null for left/right when the main axis isn't inline, → start).
function flexJustifyCode(el, plan) {
  const k = alignKeyword(declaredValue(el, 'justify-content'));
  if (!plan.mainIsX && (k === 'left' || k === 'right')) return 0; // physical inline keyword on a block axis → start
  switch (k) {
    ...
    default: return 0;                                     // flex-start / start / left / normal / null
  }
}
