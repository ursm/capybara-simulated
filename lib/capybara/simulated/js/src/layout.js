...
// column-reverse / rtl-row), but whose CROSS axis still runs FORWARD (block-start=top for a row,
// inline-start=left for a column). In normal flow, with no cross-axis min/max clamp and no bare text.
// Everything else — inline-flex, wrap-reverse, rtl-column, vertical writing modes — declines to JS.
function nlFlexSupported(el) {
  ...
  if (plan.crossStart !== 'top' && plan.crossStart !== 'left') return false; // cross must run forward
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    // A child that itself establishes a float context (a BFC — `display:flow-root`, `overflow` ≠ visible, …)
    // must keep its whole border box CLEAR of the sibling float (§9.5.2, the media-object shift), not merely
    // route its lines around it. Native shifts only a text block's lines, so it would place such a child's box
    // overlapping the float — decline (LTR + rtl alike). Backlog: native BFC-box float avoidance.
    if (hasFloatChild) {
      for (const k of orderedKids) {
        if (!isFloated(k) && startsFloatContext(k, displayOf(k))) return false;
      }
    }
