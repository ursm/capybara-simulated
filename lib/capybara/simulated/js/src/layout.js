...
function noteEscapingAbs(child, cbEl) {
  ...
  for (let n = layoutParent(child); n && n !== cbEl; n = layoutParent(n)) n._lbEscAbs = true;
}
...
// in one axis and spill in the other. So `overflow-x: hidden; overflow-y: clip` is `hidden hidden`,
// not a box that clips vertically and scrolls horizontally. (The `clip` half is what
// css-flexbox/flex-minimum-size-single-axis-scroll-container.html assumes away; Chrome 151 computes
// it the same way we do and fails those two subtests too.)
// Both longhands are read through ONE opened declared-value entry — the cascade expands every
// `overflow` shorthand, sheet and inline alike, so the shorthand itself never has to be consulted.
const OVERFLOW_SCROLLS = new globalThis.Set(['scroll', 'auto', 'hidden']);
function usedOverflow(el, axis) {
  const dv = declaredValueEntry(el);
  const keyword = (prop) => {
    const v = declaredValueIn(dv, el, prop);
    return v == null ? 'visible' : String(v).trim().toLowerCase();
  };
  const x = keyword('overflow-x'), y = keyword('overflow-y');
