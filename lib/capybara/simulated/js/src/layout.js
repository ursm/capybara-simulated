...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      // The main/cross physical margin sides, for resolving which auto margins native can place (main) vs
      // must decline (cross — autoMarginSplit centres the item).
      ...
      const crossAutoMask = plan.mainIsX ? (AUTO_MARGIN_BIT.top | AUTO_MARGIN_BIT.bottom) : (AUTO_MARGIN_BIT.left | AUTO_MARGIN_BIT.right);
      const items = [];
      for (const c of layoutChildren(el)) {
        ...
        if (edgeInsets(c, contentW).autoMargins & crossAutoMask) return false; // cross-axis auto margin (autoMarginSplit) → JS
      ...
      for (const c of items) {
        ...
        const am = edgeInsets(c, contentW).autoMargins;    // auto main margins (bit0 start-side, bit1 end-side)
