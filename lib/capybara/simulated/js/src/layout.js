...
function placeAbsolute(child, pos, staticX, staticY, ctx, order = null) {
  ...
  if (cbEl && cbEl._lb.height === 0 && LAYING_OUT.has(cbEl)) {
...
function layoutElement(el, box, ctx, cbW = box.width) {
  ...
  try {
    ...
    // Its size is final now, so anything that was waiting on it can be placed — and this box
    // wraps what it holds, so its extent has to take them in.
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeInlineBox = (child, pos) => {
    ...
    if (ce.ml) placeOnLine(ce.ml, ownH, true, false, true);
    const frag = {
    ...
    if (ce.left) placeOnLine(ce.left, ownH, true, false, true);
