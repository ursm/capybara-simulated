...
const NL_STRIDE = 91;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start / table columns, 56-61 = declared sizing, 62-67 = flex sizing, 68-70 = replaced intrinsic, 71-76 = out-of-flow, 77-79 = control baseline, 80-88 = table, 89-90 = basis-less edges)
...
function nlIntrinsicMeasurable(el) {
  ...
  const atomicsPushed = () => {
    if (pushed === null) {
      pushed = textAlignOf(flowContainer(el), flowSides(el).rtl) === 'justify';
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    nodes.push(el);
    recIndex.set(el, idx);
    const cbW = el._lbCbW != null ? el._lbCbW : (root._lbCbW != null ? root._lbCbW : root._lb.width);
    ...
    // An `rtl` block places its in-flow BLOCK children from the inline-start = RIGHT edge (r1, rec[50] below),
    // and native routes an rtl block's text around a float it owns just as it does LTR. In a TEXT block rtl
    // moves the glyphs (not compared) and, through the lines' alignment (rec[65] below), the atomics on them.
    ...
    rec[50] = rtl ? 1 : 0;   // direction:rtl — native places this block's children from the RIGHT edge (r1)
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      for (const c of layoutChildren(el)) {
        ...
        if (blockSet.has(c)) {
          ...
          if (!walk(c, idx)) return false;                     // the block child, in document order
        } else {
    ...
    if (hasInline) {
      ...
      const align = textAlignOf(flowContainer(el), rtl);
...
function blockAxisOf(el) {
  const bs = flowSides(el)['block-start'];
