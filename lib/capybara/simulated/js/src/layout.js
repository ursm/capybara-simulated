...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeTextRun = (text, owner) => {
    ...
    // A tab survives only in a preserving mode, and then its advance is the block's tab stop.
    const tab = preserves && text.indexOf('\t') !== -1 ? tabStopOf(el) : null;
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        if (CSS_WS_RE.test(token)) {
          ...
          if (lineX > lineLeft || preserves) {
            const from = lineX - content.x;
            const advances = tab ? charAdvances(token, owner, from, tab) : null;
            const tokenW = advances ? advances.reduce((a, b) => a + b, 0) : preserves ? measureRun(token, owner) : spaceW;
...
function tabAdvance(pen, tab, ls) {
  ...
  return dist < tab.min ? dist + tab.px : dist;
}
...
// The tab stop of `el`'s lines: `px`, the `tab-size` as a length or as a count of the block's own
// space advances — letter- and word-spacing included (Chrome: 8 x (9.6 + 2) under
// `letter-spacing: 2px`) — and `min`, half that space, the least a tab advances. The BLOCK's font
// rather than the tab's own: a 32px span's tab in a 16px `<pre>` stops at 76.8.
function tabStopOf(el) {
  if (memoFresh(el, '_lbTabPass')) return el._lbTab;
  el._lbTabPass = memoStamp(el);
  const v = tabSizeOf(el);
  const f = fontOf(el);
  const bare = f && f.table ? (f.table.adv[' '] !== undefined ? f.table.adv[' '] : f.table.avg) * f.size : AVG_CHAR_PX;
  const px = /^[+-]?[\d.]+$/.test(v) ? parseFloat(v) * (bare + (f ? f.ls + f.ws : 0)) : parseFloat(v);
  return (el._lbTab = { px: Number.isFinite(px) && px > 0 ? px : 0, min: bare / 2 });
}
