...
function clipsContent(el) {
  ...
  const ox = usedOverflow(el, 'x'), oy = usedOverflow(el, 'y');
  let cx = ox !== 'visible', cy = oy !== 'visible';
  ...
  let scrolls = OVERFLOW_SCROLLS.has(ox) || OVERFLOW_SCROLLS.has(oy);
  if (el === root) {
    cx = cy = scrolls = false;
    ...
    if (!rootClips) cx = cy = scrolls = false;
  }
  el._ccX = cx;
  el._ccY = cy;
  el._ccScroll = scrolls;
