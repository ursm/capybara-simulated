...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    // rtl only moves a text block's GLYPHS within its box (which the shadow doesn't compare), not the box, so a
    // text block needs no rtl handling; but native's rtl block path doesn't route around floats, so an rtl block
    // holding a float still declines (below).
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (isFloated(c)) {
        ...
        if (rtl) return false;                                  // native's rtl block doesn't route around floats (r1)
