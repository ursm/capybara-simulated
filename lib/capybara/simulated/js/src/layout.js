...
// Does a non-zero RELATIVE shift lie between a float's parent and the box whose context records it? The
// float's own is cancelled where it is placed; an ancestor's is not, and only the oracle bakes it into the
// recorded rectangle (native applies rec[39..40] after the flow). A box that establishes the context moves
// with its own floats, so its shift is no divergence and the walk stops there — as it does at the pass root,
// whose shift is already folded into the origin the pass is given.
function nlFloatUnderShiftedAncestor(el, root) {
  for (let a = el; a && a !== root && a.nodeType === NODE_ELEMENT; a = flatTreeParent(a)) {
    if (establishesBFC(a, displayOf(a))) return false;
    // The shift the oracle ACTUALLY applied, as `flowShift` stamped it — not a second resolution of
    // `top` / `left`, which would need the containing-block basis this walk has no business guessing
    // (a percentage inset against a missing `_lbCbW` reads as no shift at all, and the divergence
    // would reach the comparison instead of declining). A guard fails CLOSED: an ancestor the flow
    // never stamped is one whose shift nobody can vouch for, so it declines rather than assume none.
    if (a._lbRelPass === undefined) return true;
    if (memoFresh(a, '_lbRelPass') && a._lbRel) return true;
  }
  return false;
}
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (isFloated(c)) {
        ...
        // …and a float under a RELATIVELY SHIFTED ancestor, where the two engines disagree and the ORACLE is
        // the wrong one: it lays a relative box's subtree out at the SHIFTED origin, so the rectangle it
        // records carries the ancestor's offset — it cancels the float's OWN offset (§9.4.3 is a paint-time
        // shift that changes no other box's layout) but nothing cancels an ancestor's. Native applies
        // rec[39..40] after the flow, so its rectangle is the unshifted one Chrome uses (measured: the
        // `clear` box after a `position: relative; top: 10px` wrapper's 50px float is at 50 in Chrome and in
        // native, 60 in the oracle). Declined until the oracle is fixed — see the float spec's guard.
        if (nlFloatUnderShiftedAncestor(el, root)) return false;
        if (c._nid == null || c._nid < 0) return false;
        orderedKids.push(c);
        continue;
      }
