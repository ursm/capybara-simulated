...
function ensureLayout() {
  ...
  layoutElement(body, { x: BODY_MARGIN, y: BODY_MARGIN, width: Math.max(0, vp.width - 2 * BODY_MARGIN), height: 0 }, ctx);
}
...
function computeViewport() {
  ...
  if (cf) {
    let box = null;
    try { box = cf.par.__csimFrameContentBox(cf.fe); } catch (_) {}
    // A container that isn't rendered (display:none, detached, no box) gives its document an EMPTY
    // viewport — nothing inside a non-rendered frame is clickable — rather than silently falling
    // back to the top-level size, which would report its content as in-view.
...
// Replaced elements with no intrinsic size get the CSS "default object size" (300×150) — an
// `<iframe>` with no width/height is 300×150 in every browser. The width/height CONTENT attributes
// are presentational hints, so those arrive through `resolveLayoutProp`; this is only the
// no-declaration default. (`<img>` is excluded: its intrinsic size is the decoded image's.)
const REPLACED_TAGS = new Set(['iframe', 'frame', 'embed', 'object', 'video']);
const OBJECT_WIDTH  = 300;
const OBJECT_HEIGHT = 150;

// The used border-box size of `el`: its declared width/height, else the default object size for a
// replaced element, else the caller's auto fallback. `autoHeight` reports that the height really is
// auto, so the caller can back-fill it from content (an explicit `height: 0` stays 0).
function usedSize(el, autoW, autoH) {
  const w = resolveLayoutProp(el, 'width');
  const h = resolveLayoutProp(el, 'height');
  const replaced = REPLACED_TAGS.has(el._tag);
  return {
    width:      w != null ? w : (replaced ? OBJECT_WIDTH  : autoW),
    height:     h != null ? h : (replaced ? OBJECT_HEIGHT : autoH),
    autoHeight: h == null && !replaced
  };
}
...
function layoutElement(el, box, ctx) {
  ...
  for (const child of layoutChildren(el)) {
    ...
    layoutElement(child, { x: box.x, y: flowY, width: size.width, height: size.height }, ctx);
    if (size.autoHeight && child._lb.height === 0 && hasOwnText(child)) child._lb.height = LINE_HEIGHT;
...
function hasOwnText(el) {
  for (const c of layoutChildren(el)) if (c.nodeType === 3 /* text */ && /\S/.test(c._data || c.data || '')) return true;
  return false;
}
