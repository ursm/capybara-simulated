...
const NL_STRIDE = 68;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start, 56-61 = declared sizing, 62-67 = flex sizing)
...
function nlIntrinsicMeasurable(el) {
  ...
  if (intrinsicSize(el)) return false;                                   // replaced: sized from its intrinsic size
  const d = displayOf(el);
...
function nlFlexNativeSizable(el, plan, items, itemsAlign) {
  for (const c of items) {
    if (nlReplacedLeaf(c) || intrinsicSize(c)) return false;
...
function nlSubtreeHasBaselineHazard(el) {
  for (const c of layoutChildren(el)) {
    if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c) || displayOf(c) === 'none') continue;
    const d = displayOf(c);
    if ((INLINE_LEVEL.has(d) && d !== 'inline') || intrinsicSize(c) || laysOutAsTable(c)) return true;
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    // A REPLACED LEAF (svg / img / input / …): replay the oracle's border box and emit no subtree. It keeps
    // its own margins / relative offset / rtl (set above) for flow placement, but its box is the pushed
    // border box, not derived from children — and with a real box its margins never adjoin (no collapse-
    // through), so clear the padding/border (no children need the content edges) and the adjoin flags.
    if (nlReplacedLeaf(el)) {
      rec[2] = 1;                                          // DISPLAY_BLOCK — a childless box…
      rec[3] = 1; rec[4] = el._lb.width; rec[5] = el._lb.height; // …sized to the pushed border box
      rec[6] = rec[7] = rec[8] = rec[9] = NaN;             // already sized — no min/max re-clamp
      rec[14] = rec[15] = rec[16] = rec[17] = 0;           // padding (no children to inset)
      rec[18] = rec[19] = rec[20] = rec[21] = 0;           // border
    ...
    if (laysOutAsFlex(el)) {
      ...
      for (const c of items) {
        ...
        if (nativeSizing) {
          ...
          r[66] = plan.mainIsX
            ? (resolveLayoutProp(c, 'height', c._lbCbH != null ? c._lbCbH : null) == null && stretchesInCross(align, c, 'height', ce) ? 1 : 0)
    ...
    for (const c of layoutChildren(el)) {
      ...
      // A BLOCK-LEVEL replaced leaf (svg / img / input with display:block) is a block child native replays;
      // an INLINE replaced element is an atomic inline (handled by the text path, which still declines) so it
      // is NOT taken here — it falls through to the inline fold below.
