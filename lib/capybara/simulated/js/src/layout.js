...
// A flex item's `flex-basis` for the record: `{frac}` for a plain percentage (native resolves it against the main
// size), `{px}` for a length, `{mixed: true}` for a percentage inside a `calc()` (resolved against the oracle's box
// by the caller), null for `auto` / `content`. Resolved as the oracle resolves it — `inherit` of a percentage is a
// percentage — and told apart by asking at two bases: a plain percentage scales with the basis, a length does not.
...
function nlShadowRun(rootEl, trace) {
  ...
  const newRecord = () => {
    ...
    r[100] = r[101] = r[102] = r[103] = r[104] = r[105] = NaN; // no percentage sizes
  ...
  const pushBorderBox = (r, c, keepHeightClamps) => {
    ...
    if (keepHeightClamps || r[2] === 3) {
      if (!r[61]) {
        const vEdges = r[14] + r[16] + r[18] + r[20];
