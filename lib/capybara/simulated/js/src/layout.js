...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  // A table CELL carries `growFloor`: its declared height is a MINIMUM, so it lays out as a definite box at
  // that floor (its percentage-height children resolve against it) and STILL grows here when the content is
  // taller — unlike an ordinary declared height, which caps the box. An auto-height box back-fills as usual.
  const growsToFlow = box.growFloor != null || (box.height === 0 && box.autoHeight !== false);
  if (growsToFlow) {
    ...
    // A cell keeps its content's NATURAL height (`_lbCellContentH`) before the floor raises the box, so the
    // table pass can vertical-align the content within the row-tall cell: the box fills the row, the content
    // aligns inside it.
    if (box.growFloor != null) el._lbCellContentH = grown;
    box.height = box.growFloor != null ? Math.max(box.growFloor, grown) : grown;
  }
  stampExtent(el, box);
}
...
function layoutTable(el, box, content, ctx) {
  ...
  if (hasGrid) grid.rows.forEach((row, r) => {
    ...
    for (const cell of row.cells) {
      ...
      // than it grows the cell. But unlike a block's `min-height`, a declared cell height is also a DEFINITE
      // containing block for its percentage-height children (a `height: 50%` child of a `height: 100px` cell
      // is 50 in Chrome, not 0). So lay the cell out as a definite box at that floor — the children resolve
      // their percentages against it — and let the flow GROW it above the floor for taller content
      // (`growFloor`, honoured by the block-flow auto-height back-fill).
      const cellFloor = size.autoHeight ? 0 : size.height;
      const cbox = { x: cellX, y, width: cw, height: cellFloor, autoHeight: cellFloor === 0, growFloor: cellFloor };
      ...
      // from the content or from a taller declared floor.
      const contentH = cell.el._lbCellContentH != null ? cell.el._lbCellContentH : cbox.height;
      const span = { cell, box: cbox, startRow: r, contentH, runsFrom, runsTo: recordedRunCount() };
      placed.push(span);
