...
const NL_CB_RECT = -2;
const NL_RUN_STRIDE = 8; // inline run record [kind, font, size, ls, ws, line_height, metric, asc]; must match layout::Run
...
function nlIntrinsicMeasurableOf(el) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (cd === 'inline' && !isFloated(c) && !intrinsicSize(c)) {
      if (whiteSpaceOf(c) !== ws || !nlInlineMeasurable(c, ws, atomicsPushed())) return false;
      continue;
    }
...
function nlInlineMeasurable(el, wsMode, atomicsPushed) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (whiteSpaceOf(c) !== wsMode && cd === 'inline') return false;     // a per-run wrap difference
...
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode, atomic = null, rel = null, oof = null,
  ...
  const appendText = (fi, text) => {
    ...
    const mergeable = last && last.kind === NL_RUN_TEXT && last.shift === shift && last.wrapMode === wrapMode && nlSameFi(last.fi, fi);
    ...
    else runs.push({ kind: NL_RUN_TEXT, fi, text, shift, wrapMode });
  };
  for (const c of children) {
    ...
    if (isOutOfFlowChild(c)) {
      ...
      if (oi !== NL_OOF_NO_MARKER) runs.push({ kind: NL_RUN_OOF, rec: oi, rel, ownerOpen });
      continue;
    }
    if (c._tag === 'br') {
    ...
    if (c._tag === 'wbr') { runs.push({ kind: NL_RUN_WBR }); continue; }
    ...
    if (cdisp === 'inline-block' || cdisp === 'inline-flex' || cdisp === 'inline-table' || cdisp === 'inline-grid' ||
      ...
      if (atomic && (!va || va.mode === 'shift') && nlAtomicNative(c, cdisp)) {
        ...
        if (ci !== NL_ATOMIC_PUSHED) {
          runs.push({ kind: NL_RUN_ATOMIC, rec: ci, asc: va ? va.px : 0 });
          hasContent = true;
          hasReal = true;
          continue;
        }
      ...
      runs.push({ kind: NL_RUN_ATOMIC, rec: NL_ATOMIC_PUSHED, metric: c._lb.width + ce.ml + ce.mr, asc, lh: outer });
      hasContent = true;
      hasReal = true;
      continue;
    }
    if (cdisp !== 'inline') return { ok: false };               // a block-level box in the IFC → JS
    if (whiteSpaceOf(c) !== wsMode) return { ok: false };   // a child overriding the block's white-space (a per-run wrap difference) → JS
    ...
    if (openW !== 0 || closeW !== 0 || openPlain !== 0 || closePlain !== 0) {
      ...
      runs.push({ kind: NL_RUN_OPEN, metric: openW, plain: openPlain });
      const sub = nlGatherRuns(layoutChildren(c), cf, runs, wsMode, cShift, cWrap, atomic, cRel, oof, openW !== 0);
      ...
      runs.push({ kind: NL_RUN_CLOSE, metric: closeW, plain: closePlain });
      hasContent = true;
      hasReal = true;
    } else {
      const sub = nlGatherRuns(layoutChildren(c), cf, runs, wsMode, cShift, cWrap, atomic, cRel, oof, false);
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (child.nodeType === 3) {
      ...
      if (!NON_WS_RE.test(t)) {
        ...
        // one line).
        if (t) barrier = modeWraps(whiteSpaceOf(owner)) ? null : 'hard';
