...
function nlGatherRuns(el, owner, runs, wsMode) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (c._tag === 'br') { runs.push({ kind: NL_RUN_BR }); hasContent = true; continue; }
    ...
    if (openW !== 0 || closeW !== 0) {
      ...
      if (!sub.ok || !sub.hasContent) return { ok: false };     // empty edged inline is fiddly → JS
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    for (const c of layoutChildren(el)) {
      if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) {
        if ((c._data || '').trim() !== '') hasInline = true;
        continue;
      }
    ...
    if (hasInline) {
      ...
      // `white-space: normal` wraps; `nowrap` collapses whitespace the same but never soft-wraps (only <br>).
      // Both native models. `pre` / `pre-wrap` / `pre-line` PRESERVE whitespace + newlines — deferred.
      const wsMode = whiteSpaceOf(el);
      if (wsMode !== 'normal' && wsMode !== 'nowrap') return false;
      ...
      rec[53] = wsMode === 'nowrap' ? 1 : 0;                   // no soft-wrap (only <br> breaks the line)
      rec[22] = runStart;
      rec[23] = local.length;
