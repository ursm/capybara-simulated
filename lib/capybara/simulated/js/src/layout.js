...
const NL_STRIDE = 56;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start)
...
// (no atomic inline — its box is replayed, not measured), and its block children must pass in turn. A box whose
...
function nlIntrinsicMeasurable(el) {
  ...
  const d = displayOf(el);
  if (d === 'contents' || laysOutAsFlex(el) || laysOutAsTable(el) || laysOutAsGrid(el)) return false;
...
// Whether the box's inline-axis sizing reads the same with NO percentage basis as with one. An intrinsic measure
// resolves a percentage (or a calc() holding one) as `auto` — `edgeInsets(el, null)` / `resolveLayoutProp(el,
// 'width', null)` — while the walk's record carries the cbW-resolved value; and an intrinsic-size keyword width
// pins the box to a measure the record can't express. Either makes the record the wrong input.
const NL_INTRINSIC_PROPS = ['width', 'min-width', 'max-width', 'padding-left', 'padding-right', 'margin-left', 'margin-right'];
function nlIntrinsicSizingIsPlain(el) {
  const info = { percent: false, keyword: null };
  const dv = declaredValueEntry(el);
  for (const prop of NL_INTRINSIC_PROPS) {
    resolveLayoutProp(el, prop, null, info, dv);
    if (info.percent || info.keyword) return false;
  }
  return true;
}
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    const rec = new Array(NL_STRIDE).fill(0);
    rec[0] = el._nid;
    rec[1] = parentIdx;
    ...
    if (laysOutAsTable(el)) {
      ...
      const emitRow = (row, parentIdx) => {
        ...
        const rr = new Array(NL_STRIDE).fill(0);
      ...
      if (grid.groups.length) {
        ...
        for (const g of orderedGroups) {
          ...
          nodes.push(g.el);
          const gr = new Array(NL_STRIDE).fill(0);
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      const flushGroup = () => {
        ...
        const anon = new Array(NL_STRIDE).fill(0);
