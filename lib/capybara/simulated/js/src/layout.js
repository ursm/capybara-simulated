...
export function applyScrollTo(self, target, pos, x, y) {
  ...
  if (target) {
    ...
    const b = target._lb;
    if (b) {
      sy = b.y;                                                 // align:top — target top at the scroll box top
      if (pos === 'center')      sy = b.y - (VIEWPORT_H - b.height) / 2;
      else if (pos === 'bottom') sy = b.y - (VIEWPORT_H - b.height);
      sx = b.x;                                                 // bring a horizontally-offscreen box in too
    }
