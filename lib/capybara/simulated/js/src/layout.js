...
function nlTableSupported(el) {
  ...
  for (const row of grid.rows) {
    ...
    for (const cell of row.cells) {
      ...
      // makes it definite.)
      if (cellHasPctHeightChild(cell.el) && (nlLengthSpec(cell.el, 'height')?.px != null || imposedH())) return false;
    }
...
//   * a percentage inside a MATH FUNCTION (`calc()` / `min()` / `max()` / `clamp()`), which has no fraction to
//     send — `declaredPctFraction` answers NaN for one, and that is how it is told apart;
...
// 2,268-case math-function sweep and 28 into a 1,200-case route sweep, both 0 at the parent commit — a
// `height: calc(50% + 2px)` item in a wrapping flex row came out 55.5 where Chrome and the oracle say 58.
...
function nlSubtreeDeclaresWalkPct(el, props) {
  for (const c of layoutChildren(el)) {
    ...
    for (const prop of props) {
      ...
      if (Number.isNaN(declaredPctFraction(c, prop)) || nlWalkResolvesPct(c)) return true;
    }
...
function nlHeightIsAuto(el, cbH = () => el._lbCbH ?? null, spec = nlLengthSpec(el, 'height')) {
  return spec == null || (spec.px == null && cbH() == null);
}
// A flex item's `flex-basis` for the record (`nlLengthSpec`).
function nlFlexBasisSpec(c) {
  return nlLengthSpec(c, 'flex-basis');
}
// A length property as the record wants it: `{frac}` for a plain percentage (native resolves it against the basis
// it has), `{px}` for a length, `{mixed: true}` for a percentage inside a math function (the caller resolves it
// against the oracle's basis), null for `auto` / `none` / a keyword (`info.keyword` says which). Resolved as the
// oracle resolves it — `inherit` of a percentage is a percentage — and a PLAIN percentage is told by its text once
// a CSS-wide keyword has resolved: `calc()`, `min(50%, 300px)` and `clamp()` are not fractions of their basis, and
// probing one at two bases cannot tell (`min(50%, 300px)` scales at 1 and 2).
function nlLengthSpec(el, prop, info = { percent: false, keyword: null }) {
  ...
  return m ? { frac: parseFloat(m[1]) / 100 } : { mixed: true };
}
...
function nlShadowRun(rootEl, trace) {
  ...
  const pushBorderBox = (r, c, keepHeightClamps) => {
    ...
    for (const slot of [8, 9]) {
      if (!Number.isNaN(r[slot + 96])) r[slot] = c._lbCbH != null ? r[slot + 96] * c._lbCbH : NaN;
    }
  ...
  const emitOutOfFlow = (c, el, idx, crossAlignCode) => {
    ...
    if (native) {
      ...
      // when it places the box; a percentage inside a math function against the oracle's rectangle.
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    const size = (slot, prop, basis, info) => {
      ...
      if (spec.px != null) { rec[slot] = spec.px; return; }
      if (spec.frac != null && nativeBasis && !underImposedRoot(slot)) { rec[slot] = NaN; rec[slot + 96] = spec.frac; return; }
      ...
      if (spec.frac != null && slot === 4 && displayOf(el) === 'table-cell') { rec[slot] = NaN; return; }
