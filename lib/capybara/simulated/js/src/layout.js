...
function nlIntrinsicMeasurableOf(el) {
  ...
  // A GRID answers with the GRID algorithm in both engines now (`contentIntrinsicWidths`'s grid arm /
  ...
  // An ANONYMOUS one is where that stops: a contiguous run of TEXT is a grid item CSS Grid §4 sizes a column
  // from, and the walk emits neither a record nor a run stream for it, so neither engine's grid algorithm can
  // see it. `contentIntrinsicWidths` walks its pen for such a grid instead — which at least measures the text —
  // and native, having no run stream at all, refuses.
  //
  ...
  // the walk comes to emit it. That is a decline either
  // way — never a wrong answer — but it moves from the gate to mid-walk, where the caller's rollback pays for
  // it (`walkAttempt`).
  // The reverse costs something too, and it is what the anonymous-item refusal below buys: a floated grid
  // holding one is not a decline where `float` does not apply — as a FLEX ITEM it is a plain item, and the
  // container falls to the PUSHED path instead, trading a decline for a replayed oracle box. 32 of the
  // `pseudo` sweep's shapes do exactly that (against 352 the same change made oracle-free).
  if ((d === 'grid' || d === 'inline-grid') && gridHasAnonymousItem(el)) return false;
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsGrid(el)) {
      ...
      const emitTracksAndItems = (native) => {
        ...
        rec[52] = anonymousItemHeight(el);                        // bare-text floor on the auto height (0 when none)
        rec[55] = gridStart;
        for (const c of items) {
          const ci = nodes.length;
...
function flatTreeChildren(el) {
  ...
  if (!el._pseudo && !el._anonCell && documentHasGeneratedContent() &&
...
// Whether a grid holds an ANONYMOUS grid item: a contiguous run of text (CSS Grid §4), which becomes an item of
// its own but has no element to hang a box on. Neither engine models one, so a grid holding one is not measured
// by the grid algorithm at all — see `nlIntrinsicMeasurableOf`. A run of pure WHITESPACE is not one: §4 leaves
// it unrendered whatever the `white-space` mode says, so a `pre` grid of ten spaces is 0 wide, not 40.
//
// The LAYOUT is a separate matter and is NOT covered here: `layoutGrid` skips the text node outright, so the run
// occupies no column and the item after it takes the one it should have had (`<div style="display:grid;
// grid-template-columns:auto 20px">text<div>b</div></div>` puts `b` in column 0 where Chrome puts it in
// column 1). A divergence BOTH engines share — recorded, not mirrored — waiting on an anonymous-item box.
function gridHasAnonymousItem(el) {
  return layoutChildren(el).some((c) => (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA)
    && CSS_CONTENT_RE.test(c._data || ''));
}
...
function contentIntrinsicWidths(el) {
  ...
  if (laysOutAsGrid(el) && !gridHasAnonymousItem(el)) {
