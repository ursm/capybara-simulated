...
function nlFlexNativeSizable(el, plan, items, itemsAlign) {
  for (const c of items) {
    ...
    if (nlSubtreeDeclaresPct(c, plan.mainIsX ? NL_PCT_BLOCK_PROPS : NL_PCT_BOTH_PROPS)) return false;
    ...
    // basis, a gap, a track) resolves differently there than the records' final-width px say. Decline them all.
...
// Whether any element in a box's subtree declares a PERCENTAGE among `props`. Native MEASURES a flex item at a
// size that is not its final one — its content height with the item's height AUTO (the oracle's
// `measureItemHeight`), a column item at its shrink-to-fit width before it is re-stretched — where such a
// descendant's percentage resolves against that provisional size (or, for a height, to nothing at all). But the
// records carry it resolved against the size the oracle's FINAL layout gave the item, so the measure would read
// the wrong px there. Until native resolves percentages itself, such an item keeps the pushed path. A row's
// items are measured at their final width, so only the block axis is at stake there.
