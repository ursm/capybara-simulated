  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      for (const c of layoutChildren(el)) {
        ...
        if (laysOutAsFlex(c)) return false;                // nested flex → f2
        if (!nlBlockDisplay(c)) return false;              // inline / inline-block / table item → JS
        if (intrinsicSize(c)) return false;                // replaced / form control (ratio / stretch)
