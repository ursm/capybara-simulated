...
const NL_STRIDE = 96;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start / table columns, 56-61 = declared sizing, 62-67 = flex sizing, 68-70 = replaced intrinsic, 71-76 = out-of-flow, 77-79 = control baseline, 80-88 = table, 89-90 = basis-less edges, 91 = block axis is horizontal, 92-95 = out-of-flow containing-block rect)
...
function nlAtomicNative(c, cdisp) {
  // Two declarations the WALK refuses on ANY record it emits, this box's included: a horizontal AUTO margin
  // (native's block engine does not centre) and an intrinsic-size KEYWORD (no length for the record to carry).
  // Answered here so this predicate says what the walk will actually DO — where it does not, a caller that
  // reads "measurable" as "native lays it out" marks a subtree measured that then declines mid-walk, and a
  // `<td>` holding a `margin: 0 auto` inline-block took its whole table down that way. Saying no here pushes
  // the cell's own contribution instead (rec[84..85]) and the table is laid out. Asked before the `inline`
  // branch because the walk refuses an inline `<img>` on the same two grounds.
  if (edgeInsets(c, 0).autoMargins & AUTO_MARGIN_X) return false;
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  const emitOutOfFlow = (c, el, idx, crossAlignCode) => {
    ...
    if (native) {
      ...
      const am = edgeInsets(c, cb.width).autoMargins;
      r[76] = ((am & AUTO_MARGIN_BIT.left) ? 1 : 0) | ((am & AUTO_MARGIN_BIT.right) ? 2 : 0)
            | ((am & AUTO_MARGIN_BIT.top) ? 4 : 0) | ((am & AUTO_MARGIN_BIT.bottom) ? 8 : 0);
      r[33] = crossAlignCode;
  ...
  function walk(el, parentIdx) {
    ...
    // A FLEX ITEM's auto margins are handled by the flex branch (main-axis ones absorb free space, cross-axis
    // ones bail there); a GRID ITEM's are already resolved by the oracle and its box is replayed at `_lb`, so
    // its auto margin needs no native centring either. So the block-centring guard below must not pre-bail
    // either of them.
    ...
    const isGridItem = parIsElem && laysOutAsGrid(parLayout);
    // A horizontal `auto` margin centres a block (§10.3.3), which the native block engine doesn't do — decline.
    // (An OUT-OF-FLOW box's auto margins take the slack between its insets, which `place_out_of_flow` shares out.)
    const isOutOfFlow = positionOf(el) === 'absolute' || positionOf(el) === 'fixed';
    if (!isFlexItem && !isGridItem && !isOutOfFlow && (e.autoMargins & AUTO_MARGIN_X)) return false;
