...
function layoutTable(el, box, content, ctx) {
  ...
  if (hasGrid) grid.rows.forEach((row, r) => {
    ...
    for (const cell of row.cells) {
      ...
      // A DECLARED-height cell is a definite containing block for its percentage-height descendants, but they
      // resolve against its USED (row) height — known only once every row is placed. So it is laid out TWICE:
      // HERE, indefinitely, to SIZE it with those descendants treated as auto (they must not inflate it); and
      // again in the placed pass at the final height, to resolve them (`pass2`, below). A cell with no
      // percentage descendant just lays out definite at its floor.
      const pass2 = size.autoHeight === false && cellHasPctHeightChild(cell.el);
      const cbox = pass2
      ...
      const span = { cell, box: cbox, startRow: r, contentH, pass2, runsFrom, runsTo: recordedRunCount() };
      placed.push(span);
  ...
  for (const s of placed) {
    ...
    if (s.pass2 && h > 0) {
      // Second layout: the cell now has a definite height (its row's), so its percentage-height descendants
      // resolve against THAT — a `height: 50%` child of a 30px-used cell is 15, not the pass-1 auto 0. Lay it
      // at the final row position and height; `cellFillsRow` keeps the box at `h` (a % child that overflows it
      // does not grow it) while recording the now-resolved content flow for vertical-align. This records the
      // cell's text runs once, here, at their final position (pass 1 suppressed them).
      const runsFrom = recordedRunCount();
      const cbox2 = { x: s.box.x, y: rowTops[s.startRow], width: s.box.width, height: h, autoHeight: false, cellFillsRow: true };
