...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const takeBreak = () => {
    if (barrier && barrier.shy) {
      // …placed as the LAST thing on the line: the inline boxes opened since the piece take no
      // fragment for it (`aaaa&shy;<b>bbbb</b>`: the hyphen closes the line, the `<b>` starts on
      // the next one — Chrome), so they step aside while it goes on.
      const owner = barrier.shy, hy = measureRun('-', owner);
      const opened = openInlines.splice(barrier.depth);
    ...
    segments.forEach((segment, i) => {
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        for (let u = 0; u < units.length; u++) {
          ...
          let hyphen = false;
          if (shy && shy[u]) {
            const hy = measureRun('-', owner), nextW = u + 1 < units.length ? widths[u + 1] : 0;
  ...
  const placeInlineChild = (child, owner) => {
    ...
    const at   = placeOnLine(ce.ml + size.width + ce.mr, 0, barrier === 'hard', false, false, 0);
...
function contentIntrinsicWidths(el) {
  ...
  const addUnit = (text, owner, from, tab) => {
    ...
    if (HYPHEN_ANY_RE.test(text)) {
      ...
      if (pieces.length > 1 || (pieces.length === 1 && pieces[0].shy)) {
        for (let i = 0; i < pieces.length; i++) {
          const piece = pieces[i];
          const w = measureRun(piece.text, owner, line, tab);
          line += w; word += w;
          if (piece.shy) { const hy = word + measureRun('-', owner); if (hy > min) min = hy; }
...
function minBreaksAnywhere(el) {
  return wordBreakMode(el) === 'break-all' || overflowWrapOf(el) === 'anywhere';
}
...
function breakUnits(word, el, avail) {
  ...
  if (HYPHEN_ANY_RE.test(word)) {
    ...
    // A run that is nothing but a soft hyphen (`aaaa<b>&shy;</b>bbbb`) is a zero-width piece
    // that carries the opportunity to the text after it.
    if (!pieces.length) return { units: [''], widths: [0], freshLine: false, shy: [true], fresh: null };
...
// declarations that let a line break inside a word. Both INHERIT: Discourse's `.cooked` and
// Forem's article body declare `word-wrap: break-word` on the container, and reading the run's
// own element left every long URL in a post unbroken.
function breaksAnywhere(el) {
  if (el._lbAnyPass === layoutPass) return el._lbAny;
  el._lbAnyPass = layoutPass;
  const ow = overflowWrapOf(el);
  el._lbAny = wordBreakMode(el) === 'break-all' || ow === 'break-word' || ow === 'anywhere';
  return el._lbAny;
}
function wordBreakMode(el) { return inheritedKeyword(el, 'word-break', '_lbWb'); }
function overflowWrapOf(el) {
  const own = declaredValue(el, 'overflow-wrap') || declaredValue(el, 'word-wrap');
  if (own != null) { const v = String(own).trim().toLowerCase(); if (v !== 'inherit' && v !== 'unset') return v; }
  const parent = flatTreeParent(el);
  return parent && parent.nodeType === NODE_ELEMENT ? overflowWrapOf(parent) : '';
}
// `hyphens: none` takes the soft hyphens out of a word (Chrome: `aaaa\u00ADbbbb` stays whole).
function hyphensNone(el) { return inheritedKeyword(el, 'hyphens', '_lbHy') === 'none'; }
// An inherited keyword property, read up the flat tree and memoised per pass under `slot`.
function inheritedKeyword(el, prop, slot) {
  if (el[slot + 'Pass'] === layoutPass) return el[slot];
  el[slot + 'Pass'] = layoutPass;
  const own = declaredValue(el, prop);
  let v = own == null ? '' : String(own).trim().toLowerCase();
  if (v === '' || v === 'inherit' || v === 'unset') {
    const parent = flatTreeParent(el);
    v = parent && parent.nodeType === NODE_ELEMENT ? inheritedKeyword(parent, prop, slot) : '';
  }
  return (el[slot] = v);
}
