...
// `atomic` = the harness's hook for an atomic inline native lays out ITSELF: it walks the element's subtree as
// child records of the text block, offsets it by the `rel` it is handed (the accumulated `position: relative` of
// the inlines above it), and returns the record index — or NL_ATOMIC_PUSHED to fall back to the pushed box, or
// NL_ATOMIC_DECLINED where no pushed box may be (the text block is inside a subtree native measures) — see
// `nlAtomicNative`. Null in a context that has no record to hang the subtree on (an anonymous block's group), or
// whose lines `justify`.
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  // declines its text block there instead of rolling the atomic back to the pushed box. The flag is monotone —
  // a nested walk inside a measured subtree stays measured — and symmetric on failure.
  let measured = 0;
  ...
  const emitOutOfFlow = (c, el, idx, crossAlignCode) => {
    ...
    if (!walkMeasured(c, idx, native && !!shrinksToFit)) return false;
    const r = recs[ci];
    r[43] = 1;                                               // out of flow
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsTable(el)) {
      ...
      const emitRow = (row, parentIdx) => {
        ...
        for (const cell of row.cells) {
          const ci = nodes.length;
          if (!walkMeasured(cell.el, ri)) return false;     // the cell's own subtree (block / text), MEASURED:
                                                            // native sizes its column from the cell's intrinsics
          const cr = recs[ci];
      ...
      if (grid.captions.length) {
        ...
        if (!walk(cap, idx)) return false;                 // its own block/text subtree (display kept)
    ...
    if (hasInline) {
      ...
      const atomic = align === 'justify' ? null : (c, rel) => {
        const ci = nodes.length, runsAt = runsAll.length, gridsAt = gridsAll.length;
        const stats = [nativeIntrinsicGrids, nativeFlexRows, nativeOutOfFlow, nativeAtomics];
        ...
        [nativeIntrinsicGrids, nativeFlexRows, nativeOutOfFlow, nativeAtomics] = stats;
  ...
  return { ok: true, nodes: nodes.length, compared, mismatches, sample, nativeIntrinsicGrids, nativeFlexRows, nativeOutOfFlow, nativeAtomics };
};
