...
function layoutElement(el, box, ctx, cbW = box.width) {
  ...
  const placeInlineBox = (child, pos) => {
    ...
    // An inline box with nothing in it (and no edges to place) never reached a line: it
    // sits empty at the cursor, generating no line box at all — Chrome gives the block
    // around a lone empty `<span>` a height of 0.
    // A box whose whole content is one collapsible space is as wide as that space —
    // nothing came after it to carry the edge.
