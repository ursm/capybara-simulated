...
function nlGatherRuns(el, owner, runs) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (displayOf(c) !== 'inline') return { ok: false };        // inline-block / inline-flex / block atomic in the IFC → JS (deferred)
    if (intrinsicSize(c)) {
      // An INLINE REPLACED element (svg / img / an inline control) is an ATOMIC inline: a single box on the
      // line, contributing its margin-box WIDTH as an advance and its own baseline ASCENT (+ descent) to the
      // line box. The oracle resolved its box (`_lb`) and its baseline (`atomicInlineAscent`); native replays
      // those as a RUN_ATOMIC. Its OWN box is not emitted (like every inline fragment here) — only its effect
      // on the line. A non-baseline vertical-align (sub / sup / top …) shifts it off the baseline, which this
      // path doesn't model — decline.
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (displayOf(c) === 'inline') { hasInline = true; continue; }  // inline element — folded below
      if (nlSupported(c) && c._nid != null && c._nid >= 0) { orderedKids.push(c); hasNonFloatBlock = true; continue; }
      return false;                                            // inline-block / flex / abspos → JS
    }
