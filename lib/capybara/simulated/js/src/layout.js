...
function nlGatherRuns(children, owner, runs, wsMode) {
  ...
  const appendText = (fi, text) => {
    ...
    if (last && last.kind === NL_RUN_TEXT && nlSameFi(last.fi, fi)) last.text += text;
    else runs.push({ kind: NL_RUN_TEXT, fi, text });
  };
  for (const c of children) {
    ...
    if (verticalAlignFor(c) !== null) return { ok: false };     // sub/sup/shift grows the line box → JS
    ...
    if (openW !== 0 || closeW !== 0) {
      ...
      const sub = nlGatherRuns(layoutChildren(c), cf, runs, wsMode);
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  for (let i = 0; i < runsAll.length; i++) {
    ...
    if (r.kind === NL_RUN_TEXT) {
      runBuf[o + 1] = r.fi.nf; runBuf[o + 2] = r.fi.size; runBuf[o + 3] = r.fi.ls; runBuf[o + 4] = r.fi.ws; runBuf[o + 5] = r.fi.lh; runBuf[o + 7] = r.fi.asc;
