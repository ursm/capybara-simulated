...
function intrinsicWidths(el) {
  ...
  // A table cell's own min-width / max-width clamp the width it contributes to its column (CSS Tables 3
  // §4.1): min-width raises both figures, max-width caps them — even below the content's own min-content
  // (Chrome: a `max-width: 3px` cell holding an unbreakable word is 3 wide; `width: 50px; min-width: 80px`
  // is 80). box-sizing and the (absent) intrinsic percentage basis are handled by clampToMinMax. The column
  // may still grow PAST a max-width when a fixed table width shares out a surplus (distributeColumns) — the
  // cap is on the intrinsic contribution, not the used width. `_lbTable` is set on every grid cell only.
  if (el._lbTable) {
    el._lbIw = { min: clampToMinMax(el, el._lbIw.min, 'width', null, extra),
                 max: clampToMinMax(el, el._lbIw.max, 'width', null, extra) };
  }
  return el._lbIw;
}
