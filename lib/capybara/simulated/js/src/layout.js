...
function nlFlexSupported(el) {
  ...
  // like a block does, so a row may declare it. A COLUMN's MAIN clamp is min/max-HEIGHT too (a min-height
  // FLOOR that only divides once the content underflows it, a max-height CAPACITY), which native handles for
  // a column that lays out as ONE line (a single floor/capacity extent; see measure_flex) — but a WRAPPING
  // column breaks its lines against that capacity, which native doesn't model, so it still bails. A COLUMN's
  // cross axis is width, so min/max-WIDTH there is its cross clamp (still deferred). (A ROW's min/max-width
  // is its MAIN clamp, which native already applies in resolve_width.)
  if (!plan.mainIsX && plan.wrapMode !== 'nowrap' && (declaresLayoutProp(el, 'min-height') || declaresLayoutProp(el, 'max-height'))) return false;
  if (!plan.mainIsX && (declaresLayoutProp(el, 'min-width') || declaresLayoutProp(el, 'max-width'))) return false;
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      // A min/max-height flex container whose final height native can't reconstruct as the oracle used it
      // declines rather than mislay the main extent / gap basis. With an UNDECLARED (auto) height the oracle's
      // two-phase is: lay out against the pre-clamp extent (the stretch room, or the content floored by
      // min-height), THEN clamp the box — but native gets only the POST-clamp height:
      //   - definiteness from cross-STRETCH / abspos (autoHeight false): the oracle sizes the main axis
      //     against the pre-clamp stretch room (definiteMainHeight), native has the clamped el._lb.height;
      //   - this same container as a flex ITEM (autoHeight true): the parent's item-push overwrites its record
      //     with the post-clamp c._lb.height and wipes min/max (rec[6..9]=NaN), so measure_flex re-lays it out
      //     as a plain DECLARED-height box — losing the auto two-phase (a ROW would align items in the full
      //     clamped cross instead of the content cross).
      // Plus (2) PERCENTAGE padding: the min-height floor's edges resolve against a different width than the
      // oracle's definiteMainHeight (own box.width vs our containing-block width). el._lbEdgePct set above.
      // A DECLARED height is pre-clamped consistently (box.height === el._lb.height), so it stays native.
      ...
      for (const c of layoutChildren(el)) {
        ...
        // FIRST-baseline alignment is handled (native groups the items on a shared baseline from each item's
        // pushed ascent); `last baseline` (anchored at the cross-END) is deferred.
        if (crossAlignPhysical(itemsAlign, c, plan, plan.baselineMode) === 'last baseline') return false;
        items.push(c);
      }
      ...
      for (const c of items) {
        ...
        r[33] = align === 'center' ? 1 : align === 'flex-end' ? 2 : align === 'baseline' ? 3 : 0; // stretch / flex-start → 0
        const ce = edgeInsets(c, contentW);
        // A baseline item rides its FIRST-baseline ascent within its margin box (baselineParts.asc — the
        // oracle's own recursive baseline, incl. the synthesised bottom-edge baseline of a box with no line);
        // native hangs every baseline item in the line from the deepest such ascent.
        if (align === 'baseline') r[42] = baselineParts({ child: c, edges: ce }, false).asc;
