...
import { isLaidOutNode, selfNotRendered, resolveLayoutProp, resolveCascadeDisplay, FALLBACK_ONLY_TAGS,
...
function layoutChildren(el) {
  ...
  if (FALLBACK_ONLY_TAGS.has(el._tag)) return NO_CHILDREN;
...
function crossAlign(containerAlign, child, axes = null, baselineFollowsAxis = false) {
  ...
  // The two answer in DIFFERENT frames, measured in a `row wrap-reverse` container: an IN-FLOW item
  // aligns its first baseline to its LINE's, which is at the line's cross-start (an empty item sits
  // at the line's bottom, 70 of a 30px line), while `align-self: baseline` on an ABSPOS child of
  // that same container — which has no line to share a baseline with — is at the physical top,
  // where `start` is. So the baselines follow the flex axis for the one and the flow for the other.
  if (align === 'last baseline') return baselineFollowsAxis ? 'flex-end' : atEnd;
  if (align === 'baseline') return baselineFollowsAxis ? 'flex-start' : atStart;
  return align;
}
...
// (60/30/0).
function crossAlignPhysical(containerAlign, child, axes) {
  if (axes == null || !axes.crossFlip) return crossAlign(containerAlign, child);
  const align = crossAlign(containerAlign, child, axes, true);
...
// main size still fits, and starts a new line when the next would not. An item too wide for the
// line on its own still gets a line (and overflows it), which is what keeps the loop terminating.
//
// The hypothetical size is the flex base CLAMPED — the row's `metrics` answer both, and they are
// the same figures the distribution then flexes, so the breaker and the sizer cannot disagree about
// where an item ends. Each line remembers where in the row it STARTS, which is what lets it take a
// slice of those metrics rather than measuring its own.
function flexRowLines(items, content, gap, metrics) {
  const { bases, clampOf } = metrics;
  const lines = [];
  let members = [], from = 0, used = 0;
  for (let i = 0; i < items.length; i++) {
    const outer = clampOf(i, bases[i]) + items[i].edges.ml + items[i].edges.mr;
    if (members.length && used + gap + outer > content.width) {
      lines.push({ members, from, cross: 0, y: 0 });
...
// (`flexRowLines`), stacked across the container by `align-content`.
function layoutFlexRow(el, box, content, ctx, { equalShare = false } = {}) {
  ...
  const lines = (multiline && items.length > 1)
    ? flexRowLines(items, content, gap, metrics)
    : [{ members: items, from: 0, cross: 0, y: 0 }];
  ...
  if (!multiline) {
    ...
    // `wrap-reverse` stacks the lines from the far edge, so they are placed in reverse order and
    // each still runs top-to-bottom inside itself.
    const order = crossFlip ? [...lines].reverse() : lines;
    let y = content.y + spread.lead;
    for (const line of order) {
      line.cross += spread.grow;
      line.y = y;
      y += line.cross + crossGap + spread.between;
    }
    // Lines that STRETCH fill the cross size exactly, so the last one is closed against the
    // container's far edge rather than left wherever an equal share of the free space accumulated
    // to. Five lines sharing 40px otherwise end at 48.10000000000001 where the edge is 48.1, and a
    // test that checks the last item meets the end edge is right to call that a miss.
    if (spread.grow > 0) {
      const last = order[order.length - 1];
      last.cross = Math.max(0, content.y + containerCross - last.y);
    }
  }
  ...
  for (const line of lines) {
    for (const it of line.members) {
      ...
      if (it.stretch) {
        ...
        shiftSubtreeY(child, line.y + edges.mt + (it.rel ? it.rel.y : 0) - child._lb.y);
        continue;
      }
      ...
      const off = edges.autoMargins
        ? 0
        : crossOffset(crossPlacement(it.align, crossFlip), line.cross, b.height + edges.mt + edges.mb);
      shiftSubtreeY(child, line.y + off + m.lead + (it.rel ? it.rel.y : 0) - b.y);
...
//
// A column does NOT break into lines: `flex-wrap: wrap` on one stacks every item in a single column
// where a browser starts a second beside it (the row's §9.3 breaker is written to take either axis
// — it wants this container's `outerOf` and its own placement loop). Its reversed CROSS axis waits
// for that same increment, for the same reason: `wrap-reverse` puts the first LINE at the
// container's right (measured: a lone 30px item sits at x=70 of a 100px column, where
// `align-items: start` leaves it at 0), but where the second line goes is a question only the
// breaker can answer — and flipping the one column we DO lay out turns the overflowing case's
// accidentally-right answers into confidently wrong ones, which is how the abspos-axes increment
// learnt to leave a mode it cannot place alone.
...
// `placeTextRun`), never per run.
let RUN_SINK = null;
export function recordingRuns(fn) {
  const prevSink = RUN_SINK, runs = [];
  const doc = globalThis.document;
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeTextRun = (text, owner) => {
    ...
    segments.forEach((segment, i) => {
      ...
      if (!wraps) {
        ...
        const at0 = placeOnLine(runW, lh, !outerWraps);
        if (rec) noteRun(rec, run, at0, lh, owner, runW);
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        if (CSS_WS_RE.test(token)) {
          ...
          if (lineX > content.x || preserves) {
            ...
            const at = placeOnLine(tokenW, lh, true, !preserves);
            if (rec && preserves) noteRun(rec, token, at, lh, owner, tokenW);
          }
          continue;
        }
        ...
        for (const unit of split.units) {
          ...
          const at1 = placeOnLine(w, lh, true);
          if (rec) noteRun(rec, unit, at1, lh, owner, w);
        }
      }
    });
  };
  ...
  const placeInlineChild = (child, owner) => {
    ...
    const contentSized = laysOutAsTable(child) || laysOutAsFlex(child);
    const autoInlineW = contentSized
...
function flexIntrinsicWidths(el) {
  ...
  const wrap = !column && wrapsFlexLines(flexWrapMode(el));
  ...
  for (const child of layoutChildren(el)) {
    ...
    if (!column) {
      ...
      if (basis != null) itemMin = itemMax = basis + extra;
      if (mayConstrainSize(child)) {
...
function noteRun(sink, text, at, lh, owner, width) {
  if (!at) return;
  const f = fontOf(owner);
  const asc = f && f.table ? Math.round(f.table.asc * f.size) : Math.round((lh || 16) * 0.8);
  const half = Math.max(0, (lh - (f && f.table ? fontBoxHeight(f, false) : lh)) / 2);
  ...
  sink({ text, x: at.x, y: at.y, baseline: at.y + half + asc, owner, width });
}
...
function insetContainingBox(el, pos) {
  ...
  if (pos === 'absolute') return containingBlockFor(el);
  for (let p = flatTreeParent(el); p; p = flatTreeParent(p)) {
