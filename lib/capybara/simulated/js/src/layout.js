...
function contentIntrinsicWidths(el) {
  ...
  for (const child of layoutChildren(el)) {
    if (child.nodeType === 3 /* text */) {
      ...
      // …a COLLAPSIBLE one: a preserved trailing tab or space is content (a `<pre>` cell holding
      // "ab\t" is a stop wide, not two glyphs).
      const trails = CSS_WS_RE.test(run.slice(-1)) && !PRESERVING_WS.has(whiteSpaceOf(el));
      const body = trails ? run.slice(0, -1) : run;
      line += measureRun(body, el, line);
