...
function nlGatherRuns(el, owner, runs, wsMode) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (cdisp === 'inline-block' || cdisp === 'inline-flex' || cdisp === 'inline-table' || cdisp === 'inline-grid' ||
      ...
      // A non-baseline vertical-align (sub / sup / top / bottom) shifts it off the line baseline — deferred.
      if (verticalAlignFor(c) !== null) return { ok: false, reason: 'atomic-valign' };
      ...
      const asc = ce.mt + (inner == null ? c._lb.height + ce.mb : inner);
