...
function nlIntrinsicMeasurableOf(el) {
  ...
  // and native, having no run stream at all, refuses. So is a grid placing an item by a line counted from the
  // END under an `auto-fill` repeat, whose column depends on a track count the two engines do not share
  // (`gridEndLineUnderAutoRepeat`).
  //
  ...
  // it (`walkAttempt`).
  if ((d === 'grid' || d === 'inline-grid') && (gridHasAnonymousItem(el) || gridEndLineUnderAutoRepeat(el))) return false;
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsGrid(el)) {
      ...
      // The gaps and every percentage track go over unresolved (`gapSpec`, `nlTrackSideSpec`): native resolves
      // them against the content box it lays the grid out in — a row gap against its content height where that
      // is definite, as `layoutGrid` does. What still reads the ORACLE's content width is only what native does
      // not count yet: an `auto-fill` / `auto-fit` repeat, whose count is how many fit, and the fallback below
      // that resolves every side against the oracle's column contributions.
      let oracleW = null;
      ...
      const autoRepeat = /repeat\(\s*auto-(?:fill|fit)/i.test(String(declaredValue(el, 'grid-template-columns') ?? ''));
      const autoSpan = { start: -1, len: 0 };
      let tracks = autoRepeat
        ? gridTrackList(el, oracleContentW(), axisGap(el, 'column', oracleContentW()), autoSpan)
        : gridTrackList(el, NaN, 0);
      // An empty / invalid template (`none`, an unparseable token) is ONE implicit column the full width.
      if (!tracks.length) tracks = [{ px: null, pct: 100, fr: null, auto: false, floor: null }];
      ...
      const emitTracksAndItems = (native) => {
        const cols = needsContent && !native ? gridColumnContent(el, tracks.length) : null;
        const gridStart = gridsAll.length;
        gridsAll.push(
          tracks.length, colGap.px, colGap.frac, rowGap.px, rowGap.frac, declaredRowH != null ? declaredRowH : NaN,
          // The extra `auto-fill` / `auto-fit` copies: an intrinsic measure drops them (see `gridTrackList`).
          autoSpan.start, autoSpan.len
        );
        ...
        // Per in-flow item placement, in the SAME order native collects them (record order, doc order).
        for (const c of items) {
          const span = Math.max(1, Math.min(gridColumnSpan(c, tracks.length), tracks.length));
          const start = gridColumnStart(c, tracks.length);
          gridsAll.push(start != null ? start : -1, span);
        }
...
// Whether a grid places an item by a line counted FROM THE END (`grid-column-start: -2`) under an `auto-fill` /
// `auto-fit` repeat. Such a line names a different column at the laid-out track count than at the one copy an
// intrinsic measure sees, and the placement the walk marshals was resolved at the laid-out one — so native would
// place the item in a column the oracle, which re-resolves at the reduced count, does not. Refused rather than
// marshalled twice: both engines answer differently from Chrome here anyway (368.19 against Chrome's 336.77 on
// `repeat(auto-fill, 50px) auto` with a `grid-column-start: -2` item), so there is nothing to mirror.
const GRID_COLUMN_PROPS = ['grid-column', 'grid-column-start', 'grid-column-end'];
function gridEndLineUnderAutoRepeat(el) {
  if (!/repeat\(\s*auto-(?:fill|fit)/i.test(String(declaredValue(el, 'grid-template-columns') ?? ''))) return false;
  return layoutChildren(el).some((c) => c.nodeType === NODE_ELEMENT && !selfNotRendered(c)
    && GRID_COLUMN_PROPS.some((prop) => /-\s*\d/.test(String(declaredValue(c, prop) ?? ''))));
}
...
//
// An `auto-fill` / `auto-fit` repeat expands to as many copies as `contentW` fits, so the SAME template is a
// different list at a different width — and an INTRINSIC measure, which has no width, gets one copy (§7.2.3.2).
// `autoSpan`, when given, is filled with `{start, len}` — where in the returned list the extra copies begin and
// how many of them there are — so a caller holding the laid-out list can cut it back to the intrinsic one.
function gridTrackList(el, contentW, gap, autoSpan) {
  const raw = declaredValue(el, 'grid-template-columns');
  const s = raw == null ? '' : String(raw).trim();
  if (!s || s === 'none' || s === 'auto') return s === 'auto' ? [parseTrack(el, 'auto')] : [];
  const out = [];
  // …recorded locally and handed over only once the whole list has parsed: every `return []` below abandons the
  // list, and a span left on the caller's object would describe tracks it never got.
  let autoStart = -1, autoLen = 0;
  for (const tok of splitTopLevel(s)) {
    ...
    if (rep) {
      ...
      if (count > 1 && /^auto-(?:fill|fit)$/i.test(rep[1].trim())) {
        autoStart = out.length + body.length;
        autoLen = body.length * (count - 1);
      }
  ...
  if (autoSpan) {
    autoSpan.start = autoStart;
    autoSpan.len = autoLen;
  }
  return out;
}
