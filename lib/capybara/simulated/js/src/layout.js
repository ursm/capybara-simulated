...
function edgeInsets(el, cbW) {
  ...
  for (const side of SIDES) {
    ...
    if (tableOuter) {
      ...
      pad = resolveLayoutProp(el, 'padding-' + side, cbW, info, dv) || 0;
      if (cellHalves) {
...
function flexRowMetrics(items, content) {
  ...
  // BACKLOG (both engines, Chrome-measured): the minimum answers with BASIS-LESS edges (`minContentWidth` reads
  // `edgeInsets(el, null)`) where the BASE above now carries the item's resolved `%` padding, so a
  // `padding: 0 10%` item shrinks below its own content box (Chrome floors it at 52, both engines at 31.99) —
  // the same delta the base takes belongs here. The `scrollsInAxis` arm below already uses the real edges.
  const minima = [];
  const minOf = (i) => {
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeInlineChild = (child, owner) => {
    ...
    // Except a FLEX container, whose width can depend on its own layout — a column-wrap
    ...
    // the intrinsic figure knows only the widest item).
...
// BACKLOG (both engines, Chrome-measured): `intrinsicWidths` answers with BASIS-LESS edges — a percentage
// resolves against nothing in an intrinsic measure — while this is the box's OWN used width, whose padding is
// resolved against its containing block like any other box's. So every shrink-to-fit box carrying a `%`
// horizontal padding is short by exactly that padding (a `float: left; padding: 0 10%` box around "hello there"
// in a 400px block is 147.97 in Chrome and 67.97 here), and so is a flex COLUMN item — the mirror image of the
// ROW base's correction in `flexRowMetrics`. The fix is that same delta, at each call site's own basis.
