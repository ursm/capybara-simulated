...
function atomicInlineAscent(el) {
  const own = scrollsInAxis(el, 'y') ? null : boxBaselineOffset(el, true);
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const forceBreak = () => {
    ...
    if (linePlaced || sawBreak) {
      lastLineY = flowY;
      lastLineH = linePlaced ? lineH : lineHeightOf(el);
...
function noteRun(sink, text, at, lh, owner, width) {
  ...
  sink({ text, x: at.x, y: at.y, baseline: at.y + baselineWithin(owner), owner, width });
}
