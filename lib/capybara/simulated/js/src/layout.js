...
function placeAbsolute(child, pos, staticX, staticY, ctx, order = null, staticAlign = null) {
  ...
  if (cbEl && LAYING_OUT.has(cbEl) &&
    ...
    if (CELL_ROW_PENDING.has(cbEl)) cbEl._lbRowAnchored = true;
    return entry;
  }
...
function shiftSubtree(el, dx, dy, root = el) {
  if (!dx && !dy) return;
  if (el === root) shiftPendingStatics(root, dx, dy);
  ...
  for (const child of layoutChildren(el)) {
    ...
    if (b && (b.fixed || (b.outOfFlow && !isSelfOrAncestor(b.cbEl, root)))) {
      ...
      // This RE-ROOTS the walk on the child, so `shiftSubtree` sweeps `PENDING_BY_CB` a second time for a box
      // held inside it. Unreachable today — every pending containing block is flushed inside the layout of any
      // out-of-flow box that could be this root — but nothing enforces that, so a held entry under a re-rooted
      // out-of-flow box would take the shift twice.
      const sdx = b.staticInline ? dx : 0;
      const sdy = b.staticBlock ? dy : 0;
      if (sdx || sdy) { shiftSubtree(child, sdx, sdy, child); }
      continue;
    }
