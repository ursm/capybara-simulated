  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    const nativeBasis = parentIdx >= 0 && !TABLE_INTERNAL_DISPLAY.has(displayOf(el)) &&
                        (isOutOfFlow ? oofBasis()
                          : parIsElem && recParent != null && (recParent === parLayout || isFloated(el)) && !laysOutAsGrid(recParent) &&
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      // Scope: a normal-collapsing white-space, no floats or out-of-flow children in the mix, all-native runs —
      // otherwise decline. (Preserve white-space's non-collapsing groups, and float/abspos interaction with
      // anonymous blocks, are deferred.)
      if (oof.length) return false;                            // abspos in the mix — defer
      if (orderedKids.some((c) => isFloated(c))) return false; // a float in the mix — defer
      ...
      const blockSet = new globalThis.Set(orderedKids);
      ...
      const flushGroup = () => {
        ...
        emitAttempt(() => {
          ...
          const gr = nlGatherRuns(kids, bf, {runs: g, block: el, atomic, oof: null}, wsMode, 0, nlWrapMode(el));
    ...
    if (hasInline) {
      ...
      // …and the float hook: its record and subtree, walked MEASURED where its width is its content's (an auto or
      // keyword width shrinks to fit, §10.3.5), as the block path walks a float child. A positioned float still
      // defers, as it does there.
      // The inlines it is written in may be `position: relative`: their paint-time offset moves the float with the
      // rest of their content (§9.4.3) — not the rectangle the lines route around — and rec[39..40] is where
      // `place` applies one, as for an atomic.
      const floatHook = (c, rel) => {
        const cpos = positionOf(c);
        ...
        const ci = nodes.length;
        if (!walkMeasured(c, idx, sized)) return null;
        if (rel) { recs[ci][39] += rel.x; recs[ci][40] += rel.y; }
        return ci;
      };
      const g = nlGatherRuns(layoutChildren(el), bf, {runs: local, block: el, atomic, oof: oofHook, float: floatHook}, wsMode, 0, nlWrapMode(el));
