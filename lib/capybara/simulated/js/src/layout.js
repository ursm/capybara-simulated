...
function nlFlexNativeSizable(el, plan, items, itemsAlign) {
  for (const c of items) {
    ...
    // basis, a gap, a track) resolves against a width the re-stretch changes, whoever resolved it. Closing it
    // wants the re-stretch to re-measure.
    if (!plan.mainIsX && plan.wrapMode !== 'nowrap' && stretchesInCross(align, c, 'width', edgeInsets(c, 0)) &&
        nlSubtreeDeclaresPct(c, NL_PCT_ANY_PROPS)) return false;
  }
  return true;
}
