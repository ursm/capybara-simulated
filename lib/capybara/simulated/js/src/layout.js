...
function nlIntrinsicMeasurableOf(el) {
  ...
  // A GRID is measured as a BLOCK — which is what the ORACLE does with one: `contentIntrinsicWidths` has no
  // grid arm, and it blockifies only a FLEX container's children. So native's `content_intrinsic` answers for
  // it with the same walk over the same child records… but only where those records are the whole story.
  // INLINE-LEVEL content is where they are not: the oracle walks it with a PEN, putting two `<span>` items on
  // ONE line and summing them for max-content, while the walk hands native a blockified record per item and
  // it takes the widest. Measured on `<div style="width:max-content"><div style="display:grid"><span>aa
  // bb</span><span>cc</span></div></div>`: the oracle's 48.41 against the widest item's 34.2.
  //
  // TEXT is the same thing with no record at all — a contiguous run of it is an anonymous grid item (CSS Grid
  // §6) that the walk emits neither a record nor a run stream for — and under a PRESERVING `white-space` that
  // includes a run of pure SPACES, which the oracle measures character by character (a grid of ten spaces
  // under `pre` is 40 wide there and 0 to native). A collapsible one is content to neither: it sets no
  // `inlineOnLine`, so the oracle contributes nothing for it either.
  //
  // So: every child block-level, or refuse. (A grid ITEM is blockified for LAYOUT, which is why the flow path
  // takes these happily — it is only the oracle's intrinsic MEASURE that keeps the declared display.)
  //
  ...
  // it (`walkAttempt`).
  if ((d === 'grid' || d === 'inline-grid') && layoutChildren(el).some((c) => {
    if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) {
      return CSS_CONTENT_RE.test(c._data || '') || (!!c._data && PRESERVING_WS.has(whiteSpaceOf(el)));
    }
    return c.nodeType === NODE_ELEMENT && !selfNotRendered(c) && INLINE_LEVEL.has(displayOf(c));
  })) {
    return false;
  }
  ...
  // NATIVELY. It hands the hook to no atomic of a JUSTIFIED block (the oracle spreads a line's spaces, positions
  // native does not hold) — a MIXED block's anonymous groups take it like any text block — and it threads that
  // same absence down every nested inline, so ONE fact decides the whole inline formatting context. Asked once,
  // lazily, and only when an atomic turns up anywhere in it.
  let pushed = null;
  const atomicsPushed = () => {
    if (pushed === null) pushed = textAlignOf(flowContainer(el), startsInlineAtRight(el)) === 'justify';
    return pushed;
  };
  for (const c of layoutChildren(el)) {
  ...
  return !(hasBlock && (ws === 'nowrap' || ws === 'pre'));
}
...
function nlShadowRun(rootEl, trace) {
  ...
  const emitOutOfFlow = (c, el, idx, crossAlignCode, forceReplay = false) => {
    ...
    // function needs it.
    // …and `forceReplay` where the CALLER cannot hand native a static position to place against: a JUSTIFIED
    // line's offsets are the oracle's (it holds per-space positions native does not), so a box that would take
    // one off that line keeps the oracle's own box instead of declining the pass.
    const placeable = !forceReplay && !(cbIdx != null && cbEl._lbEdgePct);
    let cbMemo;
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsGrid(el)) {
      ...
      let tracks = autoRepeat ? gridTrackList(el, oracleContentW(), axisGap(el, 'column', oracleContentW())) : gridTrackList(el, NaN, 0);
      ...
      const emitTracksAndItems = (native) => {
        ...
        gridsAll.push(tracks.length, colGap.px, colGap.frac, rowGap.px, rowGap.frac, declaredRowH != null ? declaredRowH : NaN);
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      // (`justify`, which native cannot spread, maps to 0 here as it does in the text-block branch, and its atomics
      // keep the pushed box there too). Without this an rtl mixed block laid its lines out ltr.
      const groupAlign = textAlignOf(flowContainer(el), startsAtRight);
      const groupAlignBits = (groupAlign === 'right' ? 1 : groupAlign === 'center' ? 2 : 0) << 5;
      ...
      const flushGroup = () => {
        ...
        const kept = emitAttempt(() => {
          ...
          recs.push(anon);
          const g = [];
          const atomic = groupAlign === 'justify' ? null : atomicHook(anonIdx);
          floats = [];
    ...
    if (hasInline) {
      ...
      // aligns its lines the same way (rec[65] bits 5-6: 0 left, 1 right, 2 center). `justify` spreads a line's
      // spaces, for which native holds no per-space positions, so under it an atomic keeps the pushed box.
      const align = textAlignOf(flowContainer(el), startsAtRight);
      rec[65] |= (align === 'right' ? 1 : align === 'center' ? 2 : 0) << 5;
      ...
      if (align === 'justify' && local.some((r) => r.kind === NL_RUN_OOF)) return false;
...
function trackBaseSpec(t, contentW) {
  if (t.fr != null) return t.floor ? trackSideSpec(t.floor, contentW, 'min') : SIDE_ZERO;
  if (t.floor) return trackSideSpec(t.floor, contentW, 'min');
  return trackSideSpec(t, contentW, 'min');
}
...
const SIDE_ZERO = [0, 0], SIDE_MIN = [1, 0], SIDE_MAX = [2, 0];
...
function nlTrackBaseSpec(t) {
  if (t.fr != null) return t.floor ? nlTrackSideSpec(t.floor, 'min') : SIDE_ZERO;
