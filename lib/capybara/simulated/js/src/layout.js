...
// a horizontal-tb `row` OR `column`, `nowrap` OR `wrap`, whose MAIN axis may run either way (row-reverse /
// column-reverse / rtl-row). Its CROSS axis runs FORWARD (block-start=top for a row, inline-start=left for a
// column) OR, for an rtl COLUMN, right→left — whose items the align flip below places from the right, minus a
// cross auto margin / wrap it declines. In normal flow, with no cross-axis min/max clamp and no bare text.
// Everything else — inline-flex (except as a flex/grid item, which blockifies to flex — see below),
// wrap-reverse, rtl-column with a cross auto margin or wrap, vertical writing modes — declines to JS.
function nlFlexSupported(el) {
  ...
  // Horizontal-tb, not wrap-reverse, and the cross axis must run FORWARD (top for a row's block axis, left
  // for a column's inline axis) — which declines wrap-reverse and an rtl COLUMN (cross runs right→left);
  // those reverse the cross axis, deferred. The MAIN axis may be reversed (handled by flex_main_reverse).
  if (plan.mode !== 'horizontal-tb' || plan.wrapMode === 'wrap-reverse') return false;
  // The cross axis may run right→left — an rtl COLUMN, whose items pack from the right edge. `crossAlignPhysical`
  // already flips each item's align (flex-start ↔ flex-end) onto the physical cross, so native's forward-frame
  // code-2 (end) placement lands a flex-start item at the right and vice-versa; the r[33] fallback below carries a
  // non-stretching `stretch` item the same way. A row's cross is the block axis (top), only reversed by
  // wrap-reverse (declined above); a vertical writing mode is declined too — so `right` here is exactly the rtl
  // column, and `bottom` never reaches this point.
...
function nlFlexNativeSizable(el, plan, items, itemsAlign) {
  for (const c of items) {
    ...
    const align = crossAlignPhysical(itemsAlign, c, plan, plan.baselineMode);
    ...
    if ((align === 'baseline' || align === 'last baseline') && (!plan.mainIsX || nlSubtreeHasBaselineHazard(c))) return false;
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      // wins over align-self). The cross axis is FORWARD here (nlFlexSupported requires crossStart ∈
      // {top,left}), so its start side IS plan.crossStart.
      ...
      const pushedCrossRecoverable = () => {
        ...
        for (const c of items) {
          if (stretchesInCross(crossAlignPhysical(itemsAlign, c, plan, plan.baselineMode), c, crossProp, edgeInsets(c, contentW))) anyStretch = true;
          else anyPlain = true;
        }
      ...
      const emitItems = (native) => {
        ...
        for (const c of items) {
          ...
          let align = crossAlignPhysical(itemsAlign, c, plan, plan.baselineMode);
          if (native) {
