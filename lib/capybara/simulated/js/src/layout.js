...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  for (const c of children) {
    ...
    if (isOutOfFlowChild(c)) {
      ...
      const oi = oof(c);
      // …defensive only, unlike the float below: the sole caller that supplies an `oof` hook is the PLAIN
      // block path, which walks it directly, so the latch is already set and this read changes nothing. A
      // mixed block passes `oof: null` (its direct out-of-flow children refuse earlier, at
      // `abspos-in-mixed-block`) — the day one gets a hook, its group's `emitAttempt` is what this reads past.
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      if (oof.length) return nlNo('abspos-in-mixed-block');    // …defer
      ...
      const flushGroup = () => {
        ...
        const kept = emitAttempt(() => {
          ...
          floats = [];
          const gr = nlGatherRuns(kids, bf, {runs: g, block: el, atomic, oof: null, float: floatHook(anonIdx, floats)}, wsMode, 0, nlWrapMode(el));
    ...
    if (hasInline) {
      ...
      // among the runs and the marker carries the index. Under `justify` that offset is not the oracle's (it
      // holds per-space positions native does not), so a block with a marker anywhere in it declines — tested
      // on the RUN STREAM below rather than on `oof`, which holds only the block's DIRECT children.
      const local = [];
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (pos === 'absolute' || pos === 'fixed') {
      ...
      if (handle && !staticAlignFor()) {
        lineStatics.push({ x: lineX, handle });
      }
      return true;
    }
