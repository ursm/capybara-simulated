...
//   - an inline box split by a BLOCK child, which a browser breaks into anonymous blocks: it falls
//     back to holding its content in one box (see `isContinuedInline`);
...
// a line. `placeInlineChild` hands exactly these back, and `isContinuedInline` refuses
// to fragment a box that has one, so the two agree by construction.
function isBlockLevelChild(node) {
  if (node.nodeType !== NODE_ELEMENT || selfNotRendered(node)) return false;
  const pos = positionOf(node);
  return pos !== 'absolute' && pos !== 'fixed' && !isInlineLevel(node);
}
...
// Move an already-laid-out subtree. Only the bottom-anchored auto-height case above and a
// relatively-positioned inline box (whose content is placed on the line BEFORE the offset can
// be applied to it) need it, so the walk is not on any hot path.
function shiftSubtree(el, dx, dy) {
  ...
  for (const child of layoutChildren(el)) {
    if (child.nodeType === NODE_ELEMENT) shiftSubtree(child, dx, dy);
  }
}
...
function layoutElement(el, box, ctx, cbW = box.width) {
  ...
  const placeTextRun = (text, owner) => {
    ...
    const wraps = mode !== 'nowrap' && mode !== 'pre';
    ...
    segments.forEach((segment, i) => {
      if (i > 0) forceBreak();
      const run = collapseRun(segment, owner, lineX === content.x);
      if (!run) return;
      if (!wraps) { placeOnLine(measureRun(run, owner), lh, true); return; }
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        if (split.freshLine && lineX > content.x) forceBreak();
        for (const unit of split.units) {
          const w = measureRun(unit, owner);
          if (lineX > content.x && (lineX - content.x) + w > content.width) forceBreak();
  ...
  const placeOnLine = (w, h, decided, hangs) => {
    ...
    flushMargin();
    if (!decided && lineX > content.x && lineX + w > right) { flowY += lineH || lineHeightOf(el); lineX = content.x; lineH = 0; }
    const at = { x: lineX, y: flowY };
    lineX += w;
    if (h > lineH) lineH = h;
    linePlaced = true;
    hadInFlow = true;
    // Every inline box still open around this placement grows to hold it — that union
    // IS the box a browser reports for an inline whose content spans lines.
    for (const frag of openInlines) {
      if (at.x < frag.minX) frag.minX = at.x;
      if (at.y < frag.top) frag.top = at.y;
      if (at.y > frag.lastY) frag.lastY = at.y;
      // Content on the same line past a hanging space carries the edge with it, so
      // only white space that ends up LAST needs the fallback below.
      if (hangs) { if (lineX > frag.hangRight) frag.hangRight = lineX; continue; }
      if (lineX > frag.maxRight) frag.maxRight = lineX;
    }
    return at;
  };
  // The inline boxes whose content is being placed right now, outermost first.
  const openInlines = [];
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (child.nodeType === 3) {
      ...
      if (!NON_WS_RE.test(t)) {
        ...
        if (lineX > content.x && t) { flushMargin(); placeOnLine(measureRun(' ', owner), lineHeightOf(owner), true, true); }
        return true;
      }
    ...
    const pos = positionOf(child);
    if (pos === 'absolute' || pos === 'fixed') { placeAbsolute(child, pos, flowY, ctx); return true; }
    ...
    if (child._tag === 'br') {
      flushMargin();
      const bf = fontOf(child);
      child._lb = { x: lineX, y: flowY, width: 0,
                    height: bf && bf.table && bf.table.asc ? fontBoxHeight(bf, false) : lineHeightOf(child) };
  ...
  const placeInlineBox = (child, pos) => {
    ...
    // Each fragment is as tall as the font's CONTENT box (ascent + descent), which is
    // shorter than the line box the gap makes: Chrome makes a 16px inline span 17 tall
    // on an 18px line.
    const ownH = cf && cf.table && cf.table.asc ? fontBoxHeight(cf, false) : lineHeightOf(child);
    child._lbCbW = content.width;
    child._lbOrder = ctx.order++;
    // A horizontal margin advances the line OUTSIDE the box, so it is placed before the
    // fragment opens (and after it closes) — where it still grows any enclosing inline.
    if (ce.ml) placeOnLine(ce.ml, ownH, true);
    const frag = { minX: Infinity, maxRight: -Infinity, hangRight: -Infinity, top: flowY, lastY: flowY };
    openInlines.push(frag);
    // The left padding/border opens the box on the line the content starts on; the right
    // one closes it wherever that content ended.
    if (ce.left) placeOnLine(ce.left, ownH, true);
    ...
    if (ce.right) placeOnLine(ce.right, ownH, true);
    openInlines.pop();
    if (ce.mr) placeOnLine(ce.mr, ownH, true);
    // `position: relative` moves the box AND everything in it, without moving anything else
    // on the line — and this box's content went onto that line before the offset was known,
    // so it is carried over now. The fragment record keeps the UNSHIFTED positions: an
    // enclosing inline box, and the line itself, are laid out as if this one had not moved.
    const rel = pos === 'relative' ? relativeOffset(child, content.width, content.height || null) : null;
    if (rel) for (const grand of layoutChildren(child)) {
      if (grand.nodeType === NODE_ELEMENT) shiftSubtree(grand, rel.x, rel.y);
    }
    // A box whose whole content is one collapsible space is as wide as that space —
    // nothing came after it to carry the edge past it.
    if (frag.maxRight === -Infinity) frag.maxRight = frag.hangRight;
    // Nothing was ever placed — not even a padding edge — so there is no fragment and
    // no line box: Chrome reports an EMPTY rect for a lone `<span></span>`, and gives
    // the block around it a height of 0.
    const box = frag.minX === Infinity
      ? { x: lineX, y: flowY, width: 0, height: 0 }
      : {
        x: frag.minX + (rel ? rel.x : 0),
        y: frag.top - ce.top + (rel ? rel.y : 0),
        width: Math.max(0, frag.maxRight - frag.minX),
        // The union of the fragments — each as tall as the font, however tall the line
        // around it grew (Chrome makes the `<span>` around a 60px image 17 tall, and
        // only the LINE 60) — plus this box's own vertical padding and border, which
        // grow the border box without growing the line: a padded `<span>` measures 27
        // tall on an 18px line and overflows it.
        height: (frag.lastY - frag.top) + ownH + ce.top + ce.bottom
      };
    child._lb = box;
    stampExtent(child, box);
  };
