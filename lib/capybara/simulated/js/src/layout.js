...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeTextRun = (text, owner) => {
    ...
    segments.forEach((segment, i) => {
      ...
      if (!wraps) {
        ...
        // Placed whole, its spaces stay INSIDE it — and each one is still a justification
        // opportunity (Chrome widens a `white-space: pre` span's inner space like any other:
        // "aaaa bbbb" measures 90.72 on a justified line where its natural width is 86.4). Asked
        // only on a justified line, and only of a run that has a space to widen.
        if (lineup().align === 'justify' && WS_ANY_RE.test(run)) noteGapsInside(run, owner, at0.x);
  ...
  // The x of every white-space character inside a whole-placed run, from the face's own advances,
  // as this line's justification gaps.
  const noteGapsInside = (text, owner, x) => {
    const advances = charAdvances(text, owner);
    let pen = x, k = 0;
    for (const ch of text) {
      if (ch === ' ' || ch === '\t' || ch === '\n' || ch === '\r' || ch === '\f') lineGaps.push(pen);
      pen += advances[k++];
    }
  };
...
function noteRun(bucket, text, at, owner, width) {
  ...
  const run = { text, x: at.x, y: at.y, baseline: at.y, owner, width, justify: 0 };
  bucket.push(run);
  RUN_SINK(run);
}
