...
function fetchCssImage(url) {
  ...
  globalThis.__csimRecordResource({ name: abs, initiatorType: 'css', startTime: started,
...
function fetchCssImagesOf(el) {
  ...
  for (let i = 0; i < out.length; i++) fetchCssImage(out[i]);
}
// The rendering-update pass: called from the event loop after `flushAnimationFrame`.
export function flushCssImages() {
  if (typeof globalThis.__csimDocHasCssImage === 'function' && !globalThis.__csimDocHasCssImage()) return;
  const doc = globalThis.document;
