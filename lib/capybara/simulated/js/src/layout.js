...
function nlAtomicMeasurable(c, cd) {
  ...
  const va = verticalAlignFor(c);
  return (!va || va.mode === 'shift') && nlAtomicNative(c, cd);
}
...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  for (const c of children) {
    ...
    if (cdisp === 'inline-block' || cdisp === 'inline-flex' || cdisp === 'inline-table' || cdisp === 'inline-grid' ||
      ...
      // PARENT's font box. `growAtomic` applies exactly this via `alignedAscent(el, va, asc, outer)`, so native
      // reproduces it by pushing that same va-adjusted ascent — the Rust line layout then grows the line box
      // around it (asc above the baseline, outer − asc below) with no change of its own.
      const va = verticalAlignFor(c);
      ...
      // lays it out, hangs it from its own last baseline — raised by its baseline SHIFT, the one `alignedAscent`
      // adds — and places it on the line; the run carries the record index and that shift. Else (the hook
      // declined the subtree, or the atomic is aligned against the parent's font box) the oracle's box +
      // baseline are pushed on the run.
      if (atomic && (!va || va.mode === 'shift') && nlAtomicNative(c, cdisp)) {
        const ci = atomic.lay(c, rel);
        if (ci === NL_ATOMIC_DECLINED) return { ok: false };
        if (ci !== NL_ATOMIC_PUSHED) {
          runs.push({ kind: NL_RUN_ATOMIC, rec: ci, asc: va ? va.px : 0, wsMode: wsCode });
          hasContent = true;
          hasReal = true;
          continue;
        }
...
function nlShadowRun(rootEl, trace) {
  ...
  for (let i = 0; i < runsAll.length; i++) {
    ...
    if (r.kind === NL_RUN_TEXT) {
      ...
      runBuf[o + 5] = r.lh || 0;       // margin-box height (asc + desc), for the line box
      runBuf[o + 6] = r.metric || 0;   // advance = margin-box width
      runBuf[o + 7] = r.asc || 0;      // ascent above the line's baseline (a native atomic: its baseline SHIFT alone)
    } else {
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeInlineChild = (child, owner) => {
    ...
    const at   = placeOnLine(need, 0, barrier === 'hard', false, false, 0);
...
function alignedAscent(el, va, own, outer) {
  ...
  if (va.mode === 'middle') return outer / 2 + exHeightOf(parent) / 2 + shift;
  if (va.mode === 'text-top') return fontAscent(parent) + shift;
  if (va.mode === 'text-bottom') return outer - fontDescent(parent) + shift;
  return own;
}
