...
function nlAtomicNative(c, cdisp) {
  // The declaration the WALK refuses on ANY record it emits, this box's included: an intrinsic-size KEYWORD
  // (no length for the record to carry). Answered here so this predicate says what the walk will actually DO —
  // where it does not, a caller that reads "measurable" as "native lays it out" marks a subtree measured that
  // then declines mid-walk, and a `<td>` holding such an inline-block took its whole table down that way.
  // Saying no here pushes the cell's own contribution instead (rec[84..85]) and the table is laid out. Asked
  // before the `inline` branch because the walk refuses an inline `<img>` on the same ground.
...
// Whether any of the box's own sizes is an INTRINSIC-SIZE KEYWORD (`min-content` / `max-content` /
// `fit-content`). The record carries lengths and `auto` only, so the walk refuses such a box (rec[4..9], the
// same six properties) — asked here with the same resolver, whose `info.keyword` is how the walk finds out.
// NO basis: the keyword answer needs none, and a null one returns before a `calc()` is reduced for a value
// nobody reads. One declared-value entry serves all six reads, as the walk's own burst does.
const NL_SIZE_PROPS = ['width', 'height', 'min-width', 'max-width', 'min-height', 'max-height'];
...
function nlDeclaresSizeKeyword(el) {
  ...
  for (const prop of NL_SIZE_PROPS) {
    resolveLayoutProp(el, prop, null, info, dv);
    if (info.keyword) return true;
  }
  return false;
}
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    // An INTRINSIC-SIZE KEYWORD (`min-content` / `max-content` / `fit-content`) on a size or a min/max is no
    // length: the oracle sizes such a box from its content (`usedSize` / `intrinsicWidths`), where the record
    // can only say `auto` — which fills the containing block instead. Decline it (the read reports the keyword
    // through `info`), rather than lay the box out at a width it never has.
    const sizeInfo = { percent: false, keyword: null };
    rec[4] = num(resolveLayoutProp(el, 'width', cbW, sizeInfo));
    rec[5] = num(resolveLayoutProp(el, 'height', cbH, sizeInfo));
    rec[6] = num(resolveLayoutProp(el, 'min-width', cbW, sizeInfo));
    rec[7] = num(resolveLayoutProp(el, 'max-width', cbW, sizeInfo));
    rec[8] = num(resolveLayoutProp(el, 'min-height', cbH, sizeInfo));
    rec[9] = num(resolveLayoutProp(el, 'max-height', cbH, sizeInfo));
    if (sizeInfo.keyword) return false;
    ...
    for (const c of orderedKids) {
      if (blockAxisOf(c) !== 'width' || isFloated(c)) continue;
