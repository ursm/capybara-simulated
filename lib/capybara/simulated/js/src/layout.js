...
function nlAtomicNative(c, cdisp) {
  ...
  if (cdisp === 'inline-flex') {
    ...
    // against), so a percentage counts as declared here exactly as it does in `size.autoWidth` — and so does an
    // intrinsic-size keyword, which gives the box its width before any growing.
    const grown = wrapsFlexLines(flexAxisPlan(c).wrapMode) && !nlDeclaresSizeKeyword(c) &&
