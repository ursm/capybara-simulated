...
function ensureLayout() {
  ...
  if (root) {
    ...
    // …and the root wraps a body wider than the viewport, which is what makes the page scroll
    // sideways (`body { width: 3000px }`).
    const consumedW = bodyRendered ? bb.x + bb.width + marginOf(body, 'margin-right') : 0;
    root._lb = { x: 0, y: 0, width: Math.max(rootW, consumedW), height: rootDeclH != null ? rootDeclH : consumed };
    root._lbOrder = -1;
...
function stickyDelta(el) {
  ...
  for (let p = layoutParent(el); p; p = layoutParent(p)) {
    if (p === root || clipsContent(p)) {
      port = p === root ? { x: 0, y: 0, ...viewport() } : { x: p._lb.x, y: p._lb.y, width: p._lb.width, height: p._lb.height };
...
export function scrollTargetFor(self, target, pos, x, y) {
  ...
  if (target) {
    ...
    if (b) {
      ...
      const smTop    = resolveLayoutProp(target, 'scroll-margin-top',    vpH) || 0;
      const smBottom = resolveLayoutProp(target, 'scroll-margin-bottom', vpH) || 0;
      const smLeft   = resolveLayoutProp(target, 'scroll-margin-left',   vpH) || 0;
