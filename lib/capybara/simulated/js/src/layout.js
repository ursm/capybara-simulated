...
// Whether native can COMPUTE this grid's column tracks (Phase 1a), rather than replay the oracle's boxes.
// The bounded subset: an in-flow (static / relative), LTR grid with a column template of only fixed lengths,
// percentages, and PLAIN `fr` (no `auto` / `min|max-content` / `minmax` / `fit-content` — those are intrinsic
// tracks that need item content measurement, Phase 1b); content-height rows (no `grid-auto-rows` length yet);
// no anonymous item (bare text) floor; and no out-of-flow item (the oracle static-corners those). Everything
// else stays on the REPLAY path (nlGridSupported). measure_grid reproduces gridColumnWidths + layoutGrid.
...
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode) {
  ...
  const appendText = (fi, text) => {
    if (text === '') return;
    if (text.trim() !== '') { hasContent = true; hasReal = true; }
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    rec[4] = num(resolveLayoutProp(el, 'width', cbW));
    ...
    if (laysOutAsGrid(el)) {
      ...
      // COMPUTE path (Phase 1a): native sizes the columns (fixed / % / fr) and lays each item out at its track
      ...
      if (nlGridComputable(el)) {
        ...
        // Each column's BASE and LIMIT (§12.4), resolved here with the oracle's trackBase / trackLimit — which,
        // for an INTRINSIC track (auto / min|max-content / minmax / fit-content), read the per-column content
        // contributions `gridColumnContent` measures from the items' `intrinsicWidths`. Marshalling the resolved
        // base/limit (plus the fr weight and whether the track is `auto`, for the distribution) is the Phase-1b
        // shortcut: native runs the §12.6 maximize + §12.7 fr arithmetic on them; a Phase-2 native min/max-content
        // will replace `cols` so native computes the base/limit itself too.
        ...
        for (let i = 0; i < tracks.length; i++) {
          const t = tracks[i];
          const base = trackBase(t, cols, i, contentW);
          const limit = t.fr != null ? base : trackLimit(t, cols, i, contentW);
          const isAuto = t.auto || (t.floor && t.floor.auto);
          gridsAll.push(base, limit, t.fr != null ? 1 : 0, t.fr || 0, isAuto ? 1 : 0);
        }
  ...
  return { ok: true, nodes: nodes.length, compared, mismatches, sample };
};
