...
function nlShadowRun(rootEl, trace) {
  ...
  const emitOutOfFlow = (c, el, idx, crossAlignCode) => {
    ...
    // WHICH box the insets measure against was already resolved by the placement, which stamped the element it
    // resolved against (`_lb.cbEl`) — `insetContainingBox` reads that stamp for the same reason: re-deriving it
    // lets the two answers drift. They do drift: `containingBlockElementFor` skips an ancestor whose `_lb` did
    // not exist YET when the placement ran (the root's box is assigned at the end of the pass), so a
    // `position: relative` on `<html>` is the viewport to the oracle and the root's padding box to a later
    // walk. Read the stamp, and the walk cannot disagree with the geometry it is marshalling.
    const cbEl = c._lb && c._lb.outOfFlow ? c._lb.cbEl : containingBlockElementFor(c, pos === 'fixed');
    ...
    const cb = cbIdx != null && cbEl._lbEdgePct ? null : containingBlockBox(cbEl);
    const dv = declaredValueEntry(c);
    const num = (v) => (v == null ? NaN : v);
    const shrinksToFit = cb && resolveLayoutProp(c, 'width', cb.width, null, dv) == null &&
      (resolveLayoutProp(c, 'left', cb.width, null, dv) == null || resolveLayoutProp(c, 'right', cb.width, null, dv) == null);
    ...
    if (native) {
      ...
      if (cbIdx == null) { r[92] = cb.x; r[93] = cb.y; r[94] = cb.width; r[95] = cb.height; }
      r[72] = num(resolveLayoutProp(c, 'top', cb.height, null, dv));
      r[73] = num(resolveLayoutProp(c, 'right', cb.width, null, dv));
      r[74] = num(resolveLayoutProp(c, 'bottom', cb.height, null, dv));
      r[75] = num(resolveLayoutProp(c, 'left', cb.width, null, dv));
      r[76] = nlAutoMarginMask(edgeInsets(c, cb.width).autoMargins);
      r[33] = crossAlignCode;
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    const nativeBasis = parentIdx >= 0 && !isOutOfFlow && parIsElem && nodes[parentIdx] === parLayout &&
                        !laysOutAsGrid(parLayout) && !laysOutAsTable(parLayout) && !laysOutAsTable(el) &&
                        !TABLE_INTERNAL_DISPLAY.has(displayOf(el)) && displayOf(el) !== 'table-caption';
    ...
    const underImposedRoot = (slot) => (slot === 5 || slot === 8 || slot === 9) && parentIdx === 0 &&
    ...
    if (parentIdx >= 0 && positionOf(el) === 'relative') {
      const r = relativeOffset(el, cbW(), cbH());
      rec[39] = r.x;
      rec[40] = r.y;
    }
...
function declaresPercentHeight(el) {
  for (const prop of PERCENT_HEIGHT_PROPS) {
