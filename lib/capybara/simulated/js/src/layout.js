...
import { isLaidOutNode, selfNotRendered, resolveLayoutProp, resolveCascadeDisplay, hasFallbackOnlyContent,
         ...
         cascadeDeclaresProperty, inlineDecls, dynamicReadSeq, visibilityHidden, ownWhiteSpace } from './cascade.js';
...
import { usedDisplay, uaDisplay, blockify, WIDGET_TAGS, declaredValue, declaredValueEntry, declaredValueIn, usedOverflow, propagatedOverflow,
         ...
         isListBox, inputType, buttonInputLabel, usedTransformMatrix } from './style-proxy.js';
...
function transformChain(el) {
  ...
  for (let node = el; node && node.nodeType === NODE_ELEMENT; node = flatTreeParent(node)) {
    // …and an ancestor that has already answered carries the whole chain above it with it.
    if (node !== el && memoFresh(node, '_lbTfPass')) {
      m = m && node._lbTf ? multiplyAffine(node._lbTf, m) : (node._lbTf || m);
      break;
    }
    if (!declaresTransform(node)) continue;
    const t = transformStepOf(node);
    if (!t) continue;
    m = m ? multiplyAffine(t, m) : t;                       // an ancestor's map applies OVER ours
  }
  el._lbTf = m;
...
function transformStepOf(el) {
  ...
  return multiplyAffine([1, 0, 0, 1, ox, oy], multiplyAffine(t.m, [1, 0, 0, 1, -ox, -oy]));
}
...
const applyAffine = (m, x, y) => ({ x: m[0] * x + m[2] * y + m[4], y: m[1] * x + m[3] * y + m[5] });
// The axis-aligned box the transformed quad occupies — which is what both rect APIs report.
function transformedRect(m, r) {
  const p = [
    applyAffine(m, r.x, r.y), applyAffine(m, r.x + r.width, r.y),
    applyAffine(m, r.x, r.y + r.height), applyAffine(m, r.x + r.width, r.y + r.height)
  ];
  const xs = p.map((q) => q.x), ys = p.map((q) => q.y);
  const x = Math.min(...xs), y = Math.min(...ys);
  return { x, y, width: Math.max(...xs) - x, height: Math.max(...ys) - y };
}
// …and the inverse, for a hit test: a point is inside a transformed box when its preimage is
// inside the box the layout engine placed. Null when the matrix is singular — a `scale(0)` box
// occupies no area and can be hit nowhere.
...
// The box the PAINTER draws: the one layout placed, with no transform applied.
export function paintRectOf(el) {
  return renderedBoxUntransformed(el) || { x: 0, y: 0, width: 0, height: 0 };
}
// The painter is deliberately not a consumer of the matrix yet. Moving a box without moving the
// text, borders and replaced content inside it puts them in different places — measured: a
// `translate(300px)` box drew at x 320 with its own text still at x 22 — so painting a transform
// is its own piece of work (the painter has to carry the matrix down through the runs, and
// rasterize a quad for the rotations it cannot express as a rectangle). Until then it draws the
// untransformed box, exactly as it did before this change, and the reftests that measure a
// transform keep failing for a reason that names the painter.
