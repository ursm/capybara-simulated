...
function nlFlexSupported(el) {
  ...
  // `wrap-reverse` reverses the cross axis against the keywords, deferred. The MAIN axis may be reversed
  // (handled by flex_main_reverse), and the cross axis is asked about below. The writing MODE is not asked at
  // all: `flexAxisPlan` resolves the axes through `flowSides`, so a vertical mode's `row` already arrives as
  // main = Y and takes the column routine on both sides — the `horizontal-tb` test that used to stand here
  // outlived the physical flex axes it was written for, and cost the frozen corpus 60 shapes, a quarter of
  // its remaining declines.
  if (plan.wrapMode === 'wrap-reverse') return false;
  // The cross axis may run right→left — an rtl COLUMN, or a `*-rl` mode's ROW — whose items pack from the right
  // edge. `crossAlignPhysical` already flips each item's align (flex-start ↔ flex-end) onto the physical cross,
  // so native's forward-frame code-2 (end) placement lands a flex-start item at the right and vice-versa; the
  // r[33] fallback below carries a non-stretching `stretch` item the same way.
  //
  // `bottom` DOES reach this point, and is declined: nothing flips an offset measured from the near edge onto
  // an axis that starts at the far one. WHICH containers those are is a question about the mode AND the
  // direction together, never the mode alone — a COLUMN's cross is the inline axis, so it points up for a
  // `sideways-lr` in ltr and for every OTHER vertical mode in rtl, and `sideways-lr` + rtl is placed. (The
  // measured table in native_layout_flex_spec is the readable form of that.)
  if (plan.crossStart !== 'top' && plan.crossStart !== 'left' && plan.crossStart !== 'right') return false;
  // What that flip does NOT cover on a reversed cross: a cross (horizontal) auto MARGIN and WRAP line-stacking are
  // placed from the near edge in native's forward frame, which is the wrong end here — decline those for the rtl
  // column. (A MAIN-axis vertical auto margin and a nowrap single line are fine.)
  if (plan.crossStart === 'right') {
    if (plan.wrapMode !== 'nowrap') return false;
    for (const c of layoutChildren(el)) {
      if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c) || displayOf(c) === 'none') continue;
      if (edgeInsets(c, 0).autoMargins & AUTO_MARGIN_X) return false;   // cross (horizontal) auto margin
    }
  }
...
function flexAlignContentCode(el) {
  switch (alignKeyword(declaredValue(el, 'align-content'))) {
    case 'start': case 'flex-start': return 0;
    case 'center': return 1;
    case 'end': case 'flex-end': return 2;
    case 'space-between': return 3;
    case 'space-around': return 4;
    case 'space-evenly': return 5;
    default: return 6;                                     // normal / stretch / auto / left / right / null
  }
}
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      rec[35] = plan.wrapMode !== 'nowrap' ? 1 : 0;        // wrap (wrap-reverse is bailed)
      ...
      // The main/cross physical margin sides, for resolving an item's `auto` margins into the abstract
      // start/end-side bits native reads (main § absorbs free space before justify; cross § autoMarginSplit
      // wins over align-self). `plan.crossStart` IS the start side whichever way the axis points — the gate has
      // ruled out only `bottom`, so this is `top`, `left`, or the `right` of an rtl column / a `*-rl` row.
      ...
      const crossLeadSide = plan.crossStart;
      const crossTrailSide = OPPOSITE_SIDE[plan.crossStart];
      const items = [], oof = [];
      for (const c of layoutChildren(el)) {
      ...
      // …and which way the CROSS axis physically points, which `rtl` alone does not say: a `vertical-rl` row's
      // cross runs right-to-left in an ltr container. Only an out-of-flow child reads it (its static position is
      // measured along the axis); an in-flow item's align is already flipped by `crossAlignPhysical`. `crossFar`
      // is also true for a `bottom` cross, which the gate above declines — and native is not left relying on
      // that: it reads this only under `!flex_main_is_x`, and a `bottom` cross always has main = X.
