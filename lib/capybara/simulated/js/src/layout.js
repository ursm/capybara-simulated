...
// blockifies to flex — see below), wrap-reverse, a float, and a WRAPPING auto-height
// column with a max-height (its lines break against that capacity).
function nlFlexSupported(el) {
  // `inline-flex` is normally an atomic inline (shrink-to-fit) → JS. But a flex / grid ITEM is BLOCKIFIED
  // (§4: inline-flex → flex), so as an item it lays out exactly as a block-level flex container (its flex-
  // resolved box is pushed, its items flexed within it) — admit it there. Elsewhere (root / block child) an
  // inline-flex is still an atomic inline and declines.
  if (displayOf(el) !== 'flex' && !(displayOf(el) === 'inline-flex' && isFlexOrGridItem(el))) return false;
...
// The gate is minimal: a block-level `display:grid` whose items native can lay out. (`inline-grid` is an atomic
// inline EXCEPT as a flex / grid item, where it blockifies to `grid` and lays out as a block-level grid
// container; an abspos / floated grid is placed by its PARENT's out-of-flow replay, then computed within that
// box.)
...
function nlGridSupported(el) {
  // `inline-grid` is normally an atomic inline (shrink-to-fit) → JS. But a flex / grid ITEM is BLOCKIFIED
  // (§4: inline-grid → grid), so as an item it lays out exactly as a block-level grid container (its
  // oracle-resolved box is pushed, its tracks computed within it) — admit it there. Elsewhere (root / block
  // child) an inline-grid is still an atomic inline and declines. Mirrors nlFlexSupported's inline-flex arm.
  if (displayOf(el) !== 'grid' && !(displayOf(el) === 'inline-grid' && isFlexOrGridItem(el))) return false;
...
function nlIntrinsicMeasurableOf(el) {
  ...
  // the WIDTH question, and a grid `nlGridSupported` refuses (a floated one, an `inline-grid` that is nobody's
  // item, a `display: grid` <button>) still declines when the walk comes to emit it. That is a decline either
...
// An atomic inline native lays out itself: an `inline-block` (a block container or a TABLE inside — not a flex
// or grid container, whose width can depend on their own layout, the oracle's `_lbFlowRight` growth) or an
// inline IMAGE (its baseline is its bottom margin edge; a text-drawing control's is its font's, still the
// oracle's), at its baseline or a shift, whose content native can measure for the shrink-to-fit width.
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      for (const c of layoutChildren(el)) {
        ...
        if (selfNotRendered(c) || displayOf(c) === 'none') continue;
        if (c._nid == null || c._nid < 0) return false;    // generated-content child with no arena id
    ...
    if (laysOutAsGrid(el)) {
      ...
      for (const c of layoutChildren(el)) {
        if (c.nodeType !== NODE_ELEMENT || selfNotRendered(c) || displayOf(c) === 'none') continue;
        if (c._nid == null || c._nid < 0) return false;          // generated-content item with no arena id
