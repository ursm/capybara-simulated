...
export function contentExtent(el) {
  ...
  let right = b.x + b.width, bottom = b.y + b.height;
  walkInclShadow(el, (n) => {
    if (n.nodeType !== NODE_ELEMENT || !n._lb) return;
    right  = Math.max(right,  n._lb.x + n._lb.width);
    bottom = Math.max(bottom, n._lb.y + n._lb.height);
  });
  return { width: Math.round(right - b.x), height: Math.round(bottom - b.y) };
}
