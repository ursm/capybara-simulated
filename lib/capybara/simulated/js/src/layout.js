...
function placeAbsolute(child, pos, staticX, staticY, ctx, order = null, staticAlign = null) {
  if (OPEN_INLINE_BOXES.length) {
    ...
    const entry = {
      ...
      staticAlign: staticAlign && ((w, h, sx, sy, m) => { const p = staticAlign(w, h, sx, sy, m); return [p[0] + dx, p[1] + dy]; })
    };
  ...
  if (stat && size.autoHeight && box.height !== size.height) {
    ...
    const dx = left == null && right == null ? again[0] - stat[0] : 0;
    const dy = top == null && bottom == null ? again[1] - stat[1] : 0;
