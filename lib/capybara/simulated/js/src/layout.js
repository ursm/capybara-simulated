...
const NL_STRIDE = 89;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start / table columns, 56-61 = declared sizing, 62-67 = flex sizing, 68-70 = replaced intrinsic, 71-76 = out-of-flow, 77-79 = control baseline, 80-88 = table)
...
// Whether an element's DECLARED value for a length property is a percentage — used to detect the % vertical
// padding whose resolution basis diverges between the oracle's grid auto-height and native's pushed edges.
...
// way). So this mirrors that: the element's inline sizing must read the same with no percentage basis as the
// walk's cbW-resolved record (`nlIntrinsicSizingIsPlain`), its content must be what native's pen-walk measures
// (an atomic inline only when native lays it out itself — `nlAtomicMeasurable` — else its box is pushed, not
// measured), and its block children / flex items must pass in turn. A box whose
...
function nlIntrinsicMeasurable(el) {
  if (!nlIntrinsicSizingIsPlain(el)) return false;
...
function nlInlineMeasurable(el, wsMode, atomicsPushed) {
  ...
  // line-height and it carries real content — the two conditions the walk applies at its OPEN run.
  const e = edgeInsets(el, el._lbCbW != null ? el._lbCbW : 0);
  if (e.ml + e.left !== 0 || e.right + e.mr !== 0) {
...
// Whether the box's horizontal EDGES read the same with NO percentage basis as with one. An intrinsic measure
// resolves a percentage (or a calc() holding one) as `auto` — `edgeInsets(el, null)` — while the walk's record
// carries the cbW-resolved value, so a % padding / margin makes the record's edges the wrong input. (The
// width / min / max / flex-basis an intrinsic reads ride the record's own basis-less declared sizing, rec[56..],
// so a % there needs no gate.)
const NL_INTRINSIC_EDGE_PROPS = ['padding-left', 'padding-right', 'margin-left', 'margin-right'];
function nlIntrinsicSizingIsPlain(el) {
  const info = { percent: false, keyword: null };
  const dv = declaredValueEntry(el);
  for (const prop of NL_INTRINSIC_EDGE_PROPS) {
    resolveLayoutProp(el, prop, null, info, dv);
    if (info.percent) return false;
  }
  return true;
}
...
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode, atomic = null, rel = null) {
  ...
  for (const c of children) {
    ...
    if (openW !== 0 || closeW !== 0) {
      ...
      runs.push({ kind: NL_RUN_OPEN, metric: openW });
      ...
      runs.push({ kind: NL_RUN_CLOSE, metric: closeW });
      hasContent = true;
      hasReal = true;
    } else {
      const sub = nlGatherRuns(layoutChildren(c), cf, runs, wsMode, cShift, cWrap, atomic, cRel);
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsFlex(el)) {
      if (!nlFlexSupported(el)) return false;
      // (1) PERCENTAGE vertical padding: native's pushed edges resolve % padding against the CONTAINING-BLOCK
      // width, but the oracle resolves a flex container's % VERTICAL padding against the container's OWN
      // box.width (its auto-height and content-box use edgeInsets(el, box.width)). They diverge exactly when
      // those widths differ, so an explicitly-sized flex container carrying percentage padding would mislay
      // its height / cross alignment — decline it. (Over-bails %-horizontal-only padding, which does not
      // actually diverge; width == cb, the common case, is left native.)
      if (el._lbEdgePct && el._lb.width !== cbW) return false;
      // (2) A min/max-height flex container aligns its items against the height the FLOW gave it, which the
    ...
    if (laysOutAsGrid(el)) {
      ...
      // PERCENTAGE VERTICAL padding on a grid whose used width ≠ its containing-block width (an explicitly-sized
      // grid): `layoutGrid` resolves the container's OWN top/bottom % padding against its box.width for the
      // auto-height back-fill, but native's edges resolve % against cbW — they diverge on the block axis, so
      // decline the whole grid. (Horizontal % padding is resolved against cbW consistently by both, during width
      // determination, so it stays native.)
      if (el._lb.width !== cbW && (declaredPct(el, 'padding-top') || declaredPct(el, 'padding-bottom'))) return false;
...
function layoutTable(el, box, content, ctx) {
  ...
  const tEdge = edgeInsets(el, box.width);   // the table's own border+padding — the caption sits outside it
  let y = content.y;
