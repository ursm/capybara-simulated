...
function nlTableSupported(el) {
  ...
  // t4: a SINGLE caption (top or bottom) is placed natively — a block box spanning the table's content width,
...
function layoutTable(el, box, content, ctx) {
  ...
  const layCaption = (caption) => {
    ...
    // into the box before laying the subtree out, advance `y` by the unshifted height.
    const rel = flowShift(caption, positionOf(caption) === 'relative' ? relativeOffset(caption, content.width, content.height || null) : null);
    const capBox = { x: content.x + (rel ? rel.x : 0), y: y + (rel ? rel.y : 0), width: content.width, height: 0 };
