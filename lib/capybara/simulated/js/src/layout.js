...
function nlGatherRuns(el, owner, runs, wsMode) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (openW !== 0 || closeW !== 0) {
      ...
      const sub = nlGatherRuns(c, cf, runs, wsMode);
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (hasNonFloatBlock && hasInline) return false;           // mixed block + inline (anonymous blocks) → JS
    ...
    if (hasInline) {
      ...
      const local = [];
      const g = nlGatherRuns(el, bf, local, wsMode);
