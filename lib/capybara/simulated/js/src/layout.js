...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  // Under a PRESERVE mode (pre / pre-wrap) whitespace is content that produces line boxes — a whitespace-only
  // run (a blank/indent line, a lone newline) still makes the block NON-empty, so the empty-block short-circuit
  // must not zero its height. (normal / nowrap collapse it away, and pre-line collapses an all-whitespace block
  // too — matching the oracle — so only the preserve modes flip `hasContent` for whitespace.)
  ...
  const appendText = (fi, text) => {
    ...
    else if (preserveWs) hasContent = true;
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    let hasInline = false;
    const wsPreserve = (() => { const w = whiteSpaceOf(el); return w === 'pre' || w === 'pre-wrap'; })();
    let sawPreWs = false;
    for (const c of layoutChildren(el)) {
      if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) {
        const td = c._data || '';
        if (CSS_CONTENT_RE.test(td)) hasInline = true;
        else if (wsPreserve && td !== '') sawPreWs = true;   // direct whitespace-only text in a preserve block
        continue;
      }
    ...
    // Whitespace-only DIRECT text in a preserve block with no real inline content: it is line content the
    // oracle lays out (a line box per run of it) — an ENTIRELY-whitespace block gets a height of its own that
    // does NOT propagate to the parent's (measured: the block is 22 tall, its body 0), and beside block children
    // it makes anonymous lines between them, both of which a plain block-container record would drop. Decline.
    ...
    if (sawPreWs && !hasInline) return false;
...
function separatesMargins(child) {
  if (child.nodeType === NODE_TEXT) {
    // …white space included where its owner PRESERVES it: a `pre-wrap` blank line is a line (CodeMirror's shape).
    const t = child._data || child.data || '';
    if (NON_WS_RE.test(t)) return true;
    const owner = flatTreeParent(child);
    return !!t && owner != null && owner.nodeType === NODE_ELEMENT && PRESERVING_WS.has(whiteSpaceOf(owner));
  }
