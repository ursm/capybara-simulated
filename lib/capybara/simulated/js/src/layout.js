...
const NL_RUN_STRIDE = 5; // inline run record [font, size, ls, ws, line_height]; must match layout::Run
...
// Does an inline element carry any margin / border / padding (an "open edge" that adds to the line)?
// L2 only folds edge-less inline elements; a non-zero edge bails to JS.
function nlHasInlineEdges(c) {
  const e = edgeInsets(c, c._lbCbW != null ? c._lbCbW : 0);
  return !!(e.top || e.right || e.bottom || e.left || e.mt || e.mr || e.mb || e.ml);
}
...
// Build the RUN SEQUENCE of a text block, descending through inline elements: contiguous text in one
// font is one run; a different font (bold/italic/size) starts a new run. `owner` is the current font-
// info (el's, for direct text). Appends {fi,text} runs to `runs` (fi merged when adjacent runs share a
// font) and returns {ok, hasNonWs}. ok=false when an inline construct isn't modelled: an atomic /
// inline-block / block inside the IFC, <br>, non-normal white-space, inline edges, or a non-system font
// (no native handle) — the caller then falls back to JS for the whole subtree.
function nlGatherRuns(el, owner, runs) {
  let hasNonWs = false;
  const append = (fi, text) => {
    if (text === '') return;
    if (text.trim() !== '') hasNonWs = true;
    const last = runs[runs.length - 1];
    if (last && last.fi.nf === fi.nf && last.fi.size === fi.size && last.fi.ls === fi.ls &&
        last.fi.ws === fi.ws && last.fi.lh === fi.lh) {
      last.text += text;
    } else {
      runs.push({ fi, text });
    }
  ...
  for (const c of layoutChildren(el)) {
    if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) { append(owner, c._data || ''); continue; }
    ...
    if (c._tag === 'br') return { ok: false };                 // hard break → later stage
    if (whiteSpaceOf(c) !== 'normal') return { ok: false };
    if (nlHasInlineEdges(c)) return { ok: false };             // margin/border/padding → later stage
    ...
    if (sub.hasNonWs) hasNonWs = true;
  }
  return { ok: true, hasNonWs };
}
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (hasInline) {
      // TEXT BLOCK (inline formatting context): build the run sequence across the block's text and its
      // inline elements (each contributing runs in its own font). Bails to JS on an atomic/inline-block,
      // <br>, non-normal white-space, inline edges, or a non-system-font run (nlGatherRuns).
      ...
      if (!g.hasNonWs) { rec[2] = 1; return true; }            // only whitespace/empty inlines → empty block
      const runStart = runsAll.length;
      for (const r of local) { runsAll.push(r.fi); runTexts.push(r.text); }
  ...
  for (let i = 0; i < runsAll.length; i++) {
    const fi = runsAll[i], o = i * NL_RUN_STRIDE;
    runBuf[o] = fi.nf; runBuf[o + 1] = fi.size; runBuf[o + 2] = fi.ls; runBuf[o + 3] = fi.ws; runBuf[o + 4] = fi.lh;
  }
