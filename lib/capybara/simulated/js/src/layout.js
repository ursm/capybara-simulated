...
function nlFlexNativeSizable(el, plan, items, itemsAlign) {
  for (const c of items) {
    ...
    // A WRAP column's STRETCHING item is measured at its shrink-to-fit width and then RE-STRETCHED — the one
    // measure at a width that is not the final one — so ANY percentage in its subtree (an edge, an indent, a
    // basis, a gap, a track) resolves against a width the re-stretch changes.
    // …resolved by whoever, which is what this used to add, and why it refused every such item. That half is
    // a CONFORMANCE worry and not a parity one: both engines measure at the same provisional width, so both
    // are wrong together and the harness sees nothing. What the harness DOES see is the oracle's margin
    // basis being a PREDICTION — `nlSubtreeWidthUnpredicted` is that question, and it is the whole of the
    // refusal now. `nlSubtreeDeclaresPct` is asked FIRST because it returns on the first percentage it finds
    // where the other has to walk the subtree out to answer no, which is the common way round.
    if (!plan.mainIsX && plan.wrapMode !== 'nowrap' && stretchesInCross(align, c, 'width', edgeInsets(c, 0)) &&
        nlSubtreeDeclaresPct(c, NL_PCT_ANY_PROPS) && nlSubtreeWidthUnpredicted(c)) return false;
  }
  return true;
}
...
const NL_PCT_ANY_PROPS = NL_PCT_BOTH_PROPS.concat(['padding-top', 'padding-bottom', 'margin-top', 'margin-bottom', 'text-indent', 'flex-basis', 'gap', 'row-gap', 'column-gap', 'grid-gap', 'grid-template-columns', 'grid-template-rows', 'top', 'right', 'bottom', 'left']);
...
function ensureLayout() {
  ...
  const bodySize = usedSize(body, Math.max(0, rootW - be.ml - be.mr), 0, rootW, vp.height);
...
function layoutFlexRow(el, box, content, ctx, { equalShare = false, plan }) {
  ...
  for (const line of lines) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const size = usedSize(child, widths[i], 0, content.width, content.definiteHeight);
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeInlineChild = (child, boxOwner) => {
    ...
    const size = usedSize(child, autoInlineW, 0, content.width, content.definiteHeight);
  ...
  for (const child of layoutChildren(el)) {
    ...
    const size = usedSize(child, autoW, 0, content.width, content.definiteHeight);
...
function layoutGrid(el, box, content, ctx) {
  ...
  for (const child of layoutChildren(el)) {
    ...
    const size = usedSize(child, avail, declaredRowH ?? 0, trackW, rowBasis);
...
function placeFloat(child, pos, ctx, content, right, top0) {
  ...
  const size  = usedSize(child, shrinkToFitWidth(child, avail, content.width), 0, content.width, content.definiteHeight);
...
// Carried as an argument rather than read off `_lbCbH`, which `usedSize` stamps LATER than the margins are
...
// Only its NULL-NESS is read (`autoHeightAdjoinsBottom` asks `== null`), so the border-box subtraction below
// is documentation of what the value would be rather than a value anyone uses; give it a min/max clamp
// before reading the number itself.
...
function marginInfo(el, cbW, cbH) {
  ...
  const innerH = marginBasisH(el, cbW, cbH);
...
function layoutTable(el, box, content, ctx) {
  ...
  const layCaption = (caption, atBottom) => {
    ...
    const size = usedSize(caption, availW, 0, box.width, null);
