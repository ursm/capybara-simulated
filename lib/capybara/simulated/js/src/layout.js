...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (!nlSupported(c) || c._nid == null || c._nid < 0) return false;  // inline / float / abspos / unsupported
      blockKids.push(c);
    }
    if (blockKids.length > 0 && hasText) return false;         // mixed block + text (anonymous blocks) → JS

    if (blockKids.length === 0 && hasText) {
      // TEXT BLOCK (inline formatting context, pure text in this block's own font).
      if (whiteSpaceOf(el) !== 'normal') return false;         // pre/nowrap/pre-wrap → JS
      const f = fontOf(el);
