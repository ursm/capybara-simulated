...
function nlAtomicNative(c, cdisp) {
  ...
  if (intrinsicSize(c)) return false;                                    // an inline-block CONTROL keeps its chrome
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (nlListBoxWithRows(el)) {
      ...
      // A text-drawing control's BASELINE is its font's (`controlBaseline`): the font box centred in the
      // content box, or a list box's content bottom — the block / flex / inline-block baseline it gives its
      // container. rec[77] kind: 0 none (an image, a chromeless control), 1 text (rec[78] font box height,
      // rec[79] ascent), 2 list box.
