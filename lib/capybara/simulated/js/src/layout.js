...
function placeAbsolute(child, pos, staticX, staticY, ctx, order = null, staticAlign = null) {
  ...
  if (cbEl && LAYING_OUT.has(cbEl) &&
      (CELL_ROW_PENDING.has(cbEl) || (cbEl._lb.height === 0 && cbEl._lb.autoHeight !== false))) {
...
function layoutFlexRow(el, box, content, ctx, { equalShare = false, plan }) {
  ...
  for (const line of lines) {
    ...
    // An item can end up WIDER than the main size it was given: a table is never narrower than its caption's
    // margin box (§17.4), so the items after it start past what it actually took (Chrome: a 60px table under a
    // 279px unbreakable caption is 279 wide and its sibling sits at 279). The distribution is not re-run — only
    // the cursor moves — which is what Chrome does with the overflow.
    let grown = 0;
    for (let i = 0; i < n; i++) {
      ...
      const x = reverse ? content.x + content.width - offsets[i] - size.width - grown : content.x + offsets[i] + grown;
      layoutElement(child, {
      ...
      if (child._lb.width > size.width) grown += child._lb.width - size.width;
    }
...
function layoutFlexColumn(el, box, content, ctx, { plan }) {
  ...
  for (const line of lines) {
    ...
    // An item can end up TALLER than the main size it was given: a table's own height is a MINIMUM (§17.5.3), so
    // a `flex: 0 0 120px` table holding a caption is 138 tall (Chrome), and the items after it start below that.
    // The distribution above is not re-run — only the cursor moves — which is what Chrome does with the overflow.
    let grown = 0;
    for (let i = 0; i < k; i++) {
      const it = members[i], { child, edges } = it;
      const h = heights[i];
      const y = reverse ? content.y + extent - offsets[i] - h - grown : content.y + offsets[i] + grown;
      ...
      const used = child._lb ? child._lb.height : h;
      if (used > h) grown += used - h;
    }
    // …and what the line reaches grows with them (an auto-height column wraps the overflow).
    if (grown && extent + grown > contentExtent) contentExtent = extent + grown;
  }
...
function layoutTable(el, box, content, ctx) {
  ...
  const cappedH = tMaxContentH != null ? Math.min(content.height, tMaxContentH) : content.height;
  // …less what the CAPTIONS take, where the height was imposed from outside rather than declared here (see the
  // caption pass above): the rows share what is left inside the wrapper.
  const imposedOuter = box.outerImposed === true;
  const imposedContentH = Math.max(0, Math.max(cappedH, tMinContentH) - (imposedOuter ? captionsTop + captionsBottom : 0));
