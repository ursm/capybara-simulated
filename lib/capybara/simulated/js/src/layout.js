...
import { advanceTableFor, faceStackFor, rangesCover }    from './font-metrics.js';
...
// ── Native layout L1 shadow-parity harness (throwaway until native layout is authoritative) ──
// Builds the block subtree's used values with the SAME helpers the JS pass uses, runs the native pass
// (__dom.layoutPass), and compares each node's native border-box (__dom.boxOf) to the JS `_lb`. Returns
// {ok, nodes, compared, mismatches, sample}. Spec-driven (no hot-path hook). L1 handles pure block flow
// only: it BAILS (ok:false) on any inline/text content, non-block display, non-static/relative position,
// or float — so on a real page it declines, and on a synthetic block page it validates the algorithm.
const NL_STRIDE = 22;
function nlBlockCode(el) {
  const d = displayOf(el);
  return d === 'block' || d === 'list-item' || d === 'flow-root' ? 1 : 255;
}
function nlSupported(el) {
  if (nlBlockCode(el) !== 1) return false;
  const p = positionOf(el);
  if (p !== 'static' && p !== 'relative') return false;   // abspos / fixed / sticky → L2+/JS
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  const nodes = [];
  const recs = [];
  // DFS in document order; returns false the moment the subtree needs a feature L1 can't model.
  function walk(el, parentIdx) {
    ...
    const rec = new Array(NL_STRIDE);
    rec[0] = el._nid;
    rec[1] = parentIdx;
    rec[2] = 1;                                   // DISPLAY_BLOCK
    ...
    rec[14] = e.top - e.bt; rec[15] = e.right - e.br; rec[16] = e.bottom - e.bb; rec[17] = e.left - e.bl;  // padding = (pad+border) − border
    ...
    for (const c of layoutChildren(el)) {
      if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) {
        if (c._data && c._data.trim() !== '') return false;      // real inline text → needs L2
        continue;                                                // whitespace-only collapses to nothing
      }
      if (c.nodeType !== NODE_ELEMENT) continue;                 // comment / PI
      if (selfNotRendered(c) || displayOf(c) === 'none') continue;
      if (!nlSupported(c) || c._nid == null || c._nid < 0) return false;
  ...
  if (!d.layoutPass(buf, root._lb.x, root._lb.y, rootCbW)) return { ok: false, reason: 'native declined' };
