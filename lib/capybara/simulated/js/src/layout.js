...
// So `direction: rtl` flips the inline axis — block flow honours it, and a 300px child in a 100px
// box lands at -200..100 exactly as in Chrome — while a VERTICAL writing mode does not, because
// our block flow is still physical there (the same child lands at 0..300 where Chrome puts it at
// -200..100). When the flex and vertical axes stop being physical, this collapses back to
// `flowSides` alone.
function scrollOriginSides(el) {
  // Memoised on the layout stamp, like `displayOf`: this sits on the `scrollWidth` / `scrollHeight`
  // read path, and `flexPhysicalAxes` under it resolved `flex-direction` and `flex-wrap` uncached —
...
function computeScrollOriginSides(el) {
  // A flex container's axes are PHYSICAL here, so `flex-direction` alone decides them — and only on
  // a SCROLL CONTAINER: Chrome reports 100 for a `row-reverse` row overflowing 200px to the left
  // while it is `overflow: visible`, and 300 for the same row once it can scroll.
  if (laysOutAsFlex(el) && scrollsContent(el)) {
    const far = isReverseFlex(el), crossFar = flexWrapMode(el) === 'wrap-reverse';
    return isColumnFlex(el) ? { x: crossFar ? 'right' : 'left', y: far ? 'bottom' : 'top' }
                            : { x: far ? 'right' : 'left', y: crossFar ? 'bottom' : 'top' };
  }
  const sides = flowSides(el);
