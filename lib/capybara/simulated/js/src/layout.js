...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  if (!nlSupported(root) && !nlFlexSupported(root) && !nlTableSupported(root)) return { ok: false, reason: 'root unsupported' };
  const nodes = [];
  const recs = [];
  ...
  function walk(el, parentIdx) {
    ...
    // A FLEX ITEM's auto margins are handled by the flex branch (main-axis ones absorb free space,
    // cross-axis ones bail there), so the block-centring guard below must not pre-bail it.
    const parForFlex = flatTreeParent(el);
    const isFlexItem = parForFlex && parForFlex.nodeType === NODE_ELEMENT && laysOutAsFlex(parForFlex);
    // A horizontal `auto` margin centres a block (§10.3.3), which the native block engine doesn't do — decline.
    if (!isFlexItem && (e.autoMargins & AUTO_MARGIN_X)) return false;
