...
function layoutTable(el, box, content, ctx) {
  ...
  if (hasGrid) grid.rows.forEach((row, r) => {
    ...
    for (const cell of row.cells) {
      ...
      const cellBase = va === 'baseline' ? boxBaselineOffset(cell.el, false) : null;
  ...
  for (const s of placed) {
    ...
    if (vaShift > 0) {
      shiftSubtreeY(s.cell.el, vaShift); s.box.y -= vaShift;
      ...
      const unshift = (n) => {
        for (const c of layoutChildren(n)) {
          if (c.nodeType !== NODE_ELEMENT) continue;
          const b = c._lb;
          if (b && b.outOfFlow && b.cbEl === s.cell.el && !b.staticBlock) { shiftSubtreeY(c, -vaShift); continue; }
          unshift(c);
        }
      };
      unshift(s.cell.el);
    ...
    if (grew || vaShift > 0) stampExtent(s.cell.el, s.box);
  }
  grid.rows.forEach((row, r) => {
