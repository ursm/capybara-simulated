  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (isFloated(c)) {
        ...
        // Still deferred: a float out of normal flow (positioned), and an AUTO width, which is
        // shrink-to-fit and resolved JS-side.
        ...
        const fcbW = c._lbCbW != null ? c._lbCbW : cbW;
        if (resolveLayoutProp(c, 'width', fcbW) == null) return false;   // auto width = shrink-to-fit → JS
        if (c._nid == null || c._nid < 0) return false;
        orderedKids.push(c);
        continue;
      }
    ...
    // (Not a float: the float branch above declines an auto width outright.)
    let measuredKids = null;
    for (const c of orderedKids) {
      if (isFloated(c)) continue;
      // Two children native sizes from their OWN content, and so must be able to MEASURE: one whose block axis
      // is the horizontal one (a vertical `writing-mode`, no inline size to fill) and one whose width is an
      // intrinsic-size KEYWORD. Both are settled here rather than in Rust, where a subtree it cannot measure
      // fails the whole pass instead of this one box — and both are walked as MEASURED subtrees, which is what
      // keeps a measure-only gap (native's `text_intrinsic` has no `text-indent`) from reaching them.
      const keywordWidth = nlDeclaresSizeKeyword(c);
      if (!keywordWidth) {
        if (blockAxisOf(c) !== 'width') continue;
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const retakeBand = (need) => {
    ...
    if (need && !linePlaced && need > band.right - band.left - ind) {
    ...
    segments.forEach((segment, i) => {
      ...
      if (!wraps) {
        ...
        if (body) {
          ...
          if (outerWraps) {
            if (lineHasContent && textMayBreak() && lineX + openEdgeWidth() + runW > lineRight) takeBreak();
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        for (let u = 0; u < units.length; u++) {
          ...
          } else if (lineHasContent && mayBreak && lineX + openEdgeWidth() + w > lineRight) {
            takeBreak();
          }
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (barrier && barrier.shy && lineHasContent && lineX + openEdgeWidth() + need > lineRight) takeBreak();
...
function charUnits(word, el, avail) {
  ...
  if (!wide && (whole <= avail || !breaksAnywhere(el))) return { units: [word], widths: [whole], freshLine: false };
