...
import { isLaidOutNode, selfNotRendered, resolveLayoutProp, resolveCascadeDisplay, hasFallbackOnlyContent,
         ...
         ownWhiteSpace, documentHasGeneratedContent, pseudoDeclaresProperty } from './cascade.js';
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    // The §8.3.1 margin barrier native applies is keyed on starts_bfc (= startsFloatContext); the oracle
    // keys it on the broader establishesBFC (also `contain: layout|paint|content|strict` and multicol),
    // which also carries size containment native doesn't model. Decline a block that establishes a BFC
    // that way but isn't a float context, so native never lays out a case whose margins it would collapse.
    if (establishesBFC(el, displayOf(el)) && !startsFloatContext(el, displayOf(el))) return false;
    const num = (v) => (v == null ? NaN : v);
    const rec = newRecord();
    ...
    rec[30] = startsFloatContext(el, displayOf(el)) ? 1 : 0;
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (isFloated(c)) {
        ...
        if (!startsFloatContext(el, displayOf(el))) return false;
