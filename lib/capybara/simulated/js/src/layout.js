...
import { advanceTableFor, faceStackFor, rangesCover, fontIsSystem }    from './font-metrics.js';
...
// The native (fontations) handle for a fontconfig SYSTEM font, memoised per (family, weight/style); -1
// when there's no native DOM / no host path resolver, or the family resolved to an @font-face.
function nativeFontHandle(family, ws) {
  const d = globalThis.__dom;
  if (!d || typeof d.registerFontPath !== 'function' || typeof globalThis.__csim_fontFile !== 'function') return -1;
  if (!fontIsSystem(family, ws)) return -1;
  const key = family + '|' + ws;
