...
const NL_STRIDE = 86;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start / table columns, 56-61 = declared sizing, 62-67 = flex sizing, 68-70 = replaced intrinsic, 71-76 = out-of-flow, 77-79 = control baseline, 80-85 = table)
...
// The plain PERCENTAGE a property declares, as a fraction (0 for a length, a calc(), or nothing) — what a
// table column's `pct` is keyed on (`tableColumns`' own `constrain`, which reads the declaration rather than a
// resolved length: a percentage has no basis until the columns share out a width).
...
// table-cell*, plus at most one caption — whose COLUMNS native sizes itself (`table_columns` /
// `distribute_columns` / `fixed_column_widths`, from each cell's own min/max-content or, where native can't
// measure a cell, its resolved contribution) and whose ROW heights are still the oracle's, imposed on each
// cell. Spans, `<col>`, thead/tfoot reorder, fixed layout, rtl and border-collapse are all in scope. The grid
...
function nlTableSupported(el) {
  ...
  // a declared table WIDTH is what it distributes. An imposed table HEIGHT is checked below, once the grid is
  // known — the rows are still the oracle's.
  ...
  // An IMPOSED table height (declared / attribute / min / max) is reproducible only when the tracks FILL the box.
  // They now always do: a height/min-height TALLER than the grid shares its excess over the rows (Chrome), one
  // SMALLER is a floor the box grows past, and a `max-height` never clips below the content (§17.5.3 — a table
  // height is a minimum) — so the box always ends level with Σtracks and native (which self-sizes from Σtracks)
  // reproduces it. A caption or a collapsed border adds extents OUTSIDE the plain row tracks, so an imposed
  // height alongside either still declines. The `gridFillH` check below is the real gate (a safety net now that
  // the box always fills).
  if ((el._lb && el._lb.autoHeight === false) || declaresLayoutProp(el, 'min-height') || declaresLayoutProp(el, 'max-height')) {
    if (grid.captions.length || tableCollapses(el)) return false;
    const te = edgeInsets(el, el._lb ? el._lb.width : 0);
    let rowsH = 0;
    for (const row of grid.rows) rowsH += (row.el && row.el._lb) ? row.el._lb.height : 0;
    const gridFillH = rowsH + (grid.rows.length + 1) * borderSpacingOf(el).y;
    // The tracks fill the box to float noise (~1e-13) when the oracle distributed the height evenly; a box the
    // tracks DON'T fill is off by the whole clip / gap. Gate on the SAME epsilon the shadow parity compare uses
    // (0.01) — a looser slack would admit a box off by up to that slack, which the shadow then flags: a
    // silent-wrong.
    if (Math.abs(gridFillH - ((el._lb ? el._lb.height : 0) - te.top - te.bottom)) > 0.01) return false;
  }
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  const newRecord = () => {
    ...
    r[82] = r[84] = r[85] = NaN;                             // it is no table cell: no row height, no pushed contribution
  ...
  function walk(el, parentIdx) {
    ...
    // TABLE (§17, t1): a fourth container kind. Native COMPUTES the column tracks from the cells' own content
    // (`table_columns` / `distribute_columns` / `fixed_column_widths`) and the table's own width, and positions
    // every cell / row / row-group and the table box; each cell's ROW height is still the oracle's, imposed on
    // its layout (rec[82]). The structure is emitted from the oracle's `tableGrid` (render order, anonymous-box
    // handling), pushing a record per row-group (2), row (6) and cell (via walk) under the table (4), with the
    // per-column `<col>` declarations on the side-channel (rec[55]).
    if (laysOutAsTable(el)) {
      ...
      // per-column data: the px length it declares (NaN for none) and the fraction a `%` declares (0 for none)
      // — the two figures the column algorithm reads, resolved by native against the width the columns share
      // out (`table_columns` / `fixed_column_widths`). A `<col span>` covers that many columns.
      ...
      const emitRow = (row, parentIdx) => {
        ...
        for (const cell of row.cells) {
          ...
          const cr = recs[ci];
          // Native sizes the COLUMNS itself (`table_columns` / `distribute_columns`) from the cells' own
          // intrinsic widths, so the cell keeps its declared width (and the `%` fraction in rec[81], which
          // resolves against the width the columns share out). A cell native CANNOT measure rides the oracle's
          // resolved contribution instead (rec[84..85]), exactly as an un-measurable grid track does — never a
          // wrong measure, and never a declined table. Its HEIGHT is still the oracle's used ROW height, imposed
          // on its layout (rec[82]) — the native row algorithm is the next increment — so its own height
          // declaration and min/max-height must not clamp that box a second time.
          cr[5] = cr[8] = cr[9] = NaN;
          cr[81] = declaredPctFraction(cell.el, 'width');
          cr[82] = cell.el._lb.height;
          if (!nlIntrinsicMeasurable(cell.el)) {
            const iw = intrinsicWidths(cell.el);
            cr[84] = iw.min; cr[85] = iw.max;
          }
          cr[54] = 0;                                       // cell height imposed — not self-sized, no two-phase
          ...
          cr[48] = cell.rowSpan;
          cr[51] = cell.el._lbVaOffset || 0;                // vertical-align: px the oracle pushed the content down
        }
        return true;
      };
...
// gives Chrome the same 150/150 as no declaration at all).
function fixedDeclaredWidth(el, assignable) {
  const pct = declaredPctFraction(el, 'width');
  if (pct) return pct * assignable;
...
function fixedColumnWidths(table, grid, assignable) {
  ...
  for (const col of grid.columns) {
    const span = spanAttr(col, 'span');
    const w = fixedDeclaredWidth(col, assignable);
  ...
  for (const cell of (grid.rows[0] ? grid.rows[0].cells : [])) {
    ...
    const cw = fixedDeclaredWidth(cell.el, assignable);
    if (cw == null) continue;
...
// (the walk only runs for a declared-height cell, a small subset). `_pctAny` covers height / min / max.
...
function layoutTable(el, box, content, ctx) {
  ...
  if (hasGrid) grid.rows.forEach((row, r) => {
    ...
    const rowHInfo = { percent: false };
    let declaredRowH = row.el ? resolveLayoutProp(row.el, 'height', rowPctBasis, rowHInfo) : null;
    // Percentage rows share the basis in source order and cannot overflow it: two `height: 60%` rows in a
    // 100px table are 60 and 40, not 60 and 60 (Chrome squeezes the later one into the space left).
    if (declaredRowH != null && rowHInfo.percent) {
      declaredRowH = Math.max(0, Math.min(declaredRowH, rowPctBasis - rowPctUsed));
  ...
  for (const s of placed) {
    ...
    // than the content by. The box still FILLS the row either way — a click lands in the box, not the content —
    // so shift the whole cell then put the box back, moving the content while the box stays at the row top.
    let vaShift = 0;
    ...
    // The shift is pushed to the native engine (rec[51]), which lays cell content top-aligned and moves the
    // cell's own descendant boxes down to match — so the shadow parity holds without a bail.
    s.cell.el._lbVaOffset = vaShift;
    if (h > 0 && (h !== s.box.height || vaShift > 0)) { s.box.height = h; stampExtent(s.cell.el, s.box); }
  }
  grid.rows.forEach((row, r) => {
