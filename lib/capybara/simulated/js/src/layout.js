...
function isClipped(el) {
  ...
  for (let p = layoutParent(el); p; p = layoutParent(p)) {
    ...
    // already applied where it belongs, against `viewport()`.
    if (p === root) continue;
...
export function ensureInView(el) {
  ...
  const r = rectOf(el);
  const vp = viewport();
  // Already showing? Then leave the page alone. WebDriver clicks the IN-VIEW centre point, so any
  // box with a part inside the viewport already has a clickable one — scrolling anyway would move
  // the page out from under everything the test looks at next (a panel taller than the window,
  // with its top just below the fold's edge, is the case that bit us).
