...
// `pre` block keeps) — see `placeTextRun`.
//
// DELIBERATELY NOT, each documented at the box it affects:
//   - glyph SHAPING (kerning, ligatures, bidi), and per-run font FALLBACK — a CJK line breaks in
//     the right places but is as tall as the element's own font, not the fallback's (Chrome: 24
//     to our 18);
//   - the rest of UAX #14 beyond white space and wide characters — no break after a hyphen;
//   - text INSIDE an inline element: it is measured on one line, so a `<span>` whose own text
//     would wrap leaves its containing block half as tall as a browser's, and a `<br>` inside one
...
//     at all); parent/first-child margin collapsing; `vertical-align`; and a FLAT paint order (no
//     nested stacking-context tree).
//
...
function layoutElement(el, box, ctx, cbW = box.width) {
  ...
  const placeTextRun = (text) => {
    ...
    const spaceW = measureRun(' ', el);           // hoisted: every collapsed gap is one space
    const segments = keepsNewlines ? String(text).split('\n') : [String(text)];
    segments.forEach((segment, i) => {
      if (i > 0) forceBreak();
      const run = collapseRun(segment, el, lineX === content.x);
      if (!run) return;
      if (!wraps) { placeOnLine(measureRun(run, el), lh, true); return; }
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        if (CSS_WS_RE.test(token)) {
          ...
          if (lineX > content.x || preserves) {
            placeOnLine(preserves ? measureRun(token, el) : spaceW, lh, true);
          }
          continue;
        }
        const split = breakUnits(token, el, content.width);
  ...
  // Reserve `w` x `h` on the current line, wrapping first if it doesn't fit; returns the origin.  // Reserve `w` x `h` on the current line, wrapping first if it doesn't fit; returns the origin.
  ...
  for (const child of layoutChildren(el)) {
    ...
    if (child.nodeType === 3) {
      ...
      if (!NON_WS_RE.test(t)) {
        ...
        if (t && PRESERVING_WS.has(whiteSpaceOf(el))) { flushMargin(); placeTextRun(t); continue; }
        if (lineX > content.x && t) { flushMargin(); placeOnLine(measureRun(' ', el), lineHeightOf(el), true); }
        continue;
      }
