...
function nlSupported(el) {
  ...
  if (p !== 'static' && p !== 'relative' && p !== 'sticky') return false;   // abspos / fixed → later/JS
...
function nlAtomicMeasurable(c, cd) {
  ...
  if (cp !== 'static' && cp !== 'relative' && cp !== 'sticky') return false;
...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  for (const c of children) {
    ...
    if (cdisp === 'inline-block' || cdisp === 'inline-flex' || cdisp === 'inline-table' || cdisp === 'inline-grid' ||
      ...
      const cp = positionOf(c);
      if (cp !== 'static' && cp !== 'relative' && cp !== 'sticky') return { ok: false, reason: 'atomic-oof' };
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    if (laysOutAsFlex(el)) {
      ...
      for (const c of layoutChildren(el)) {
        ...
        if (cp !== 'static' && cp !== 'relative' && cp !== 'sticky') return false; // abspos / fixed → JS
    ...
    for (const c of layoutChildren(el)) {
      ...
      if (isFloated(c)) {
        ...
        // Still deferred: a float out of normal flow (positioned). An AUTO width is native's own now —
        // §10.3.5 shrink-to-fit, measured from the float's own content like every other content-sized box
        // (the `measuredKids` loop below marks it, and `nlIntrinsicMeasurable` is what native needs).
        // …a STICKY one included: its box is where a static float's would be (the shift is a read-time one,
        // see `stickyDelta`), so it needs nothing a static float does not. A RELATIVE float still defers —
        // that one carries an offset native would have to apply.
        // …an ancestor's RELATIVE shift no longer separates the two engines: the oracle lays a relative
        // block out where the flow put it and moves it afterwards, as native does with rec[39..40], so the
        // float rectangle both record is the unshifted one Chrome excludes at.
        if (cpos !== 'static' && cpos !== 'sticky') return false;
        if (c._nid == null || c._nid < 0) return false;
        orderedKids.push(c);
        continue;
      }
    ...
    // shrinks to fit, §10.3.5), as the block path walks a float child. A positioned float still defers, as it does
    // there. The inlines it is written in may be `position: relative`: their paint-time offset moves the float with
    // the rest of their content (§9.4.3) — not the rectangle the lines route around — and rec[39..40] is where
    // `place` applies one, as for an atomic. `seen`, where given, collects each float with that offset.
    const floatHook = (parentIdx, seen = null) => (c, rel) => {
      const cpos = positionOf(c);
      if ((cpos !== 'static' && cpos !== 'sticky') || c._nid == null || c._nid < 0) return null;
