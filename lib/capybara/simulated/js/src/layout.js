...
const NL_STRIDE = 46;   // node record; must match layout::Input / LAYOUT_STRIDE
...
function nlTableSupported(el) {
  ...
  for (const row of grid.rows) {
    ...
    if (row.cells.length !== grid.colCount) return false;  // ragged grid → JS
    for (let c = 0; c < row.cells.length; c++) {
      const cell = row.cells[c];
      if (cell.col !== c || cell.colSpan !== 1 || cell.rowSpan !== 1) return false; // spans / gaps → JS
      if (cell.el._nid == null || cell.el._nid < 0) return false;
      if (displayOf(cell.el) !== 'table-cell') return false; // anonymous / stray cell → JS
      if (laysOutAsTable(cell.el)) return false;             // nested table → JS (t1)
    }
  }
