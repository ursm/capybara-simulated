...
function nlGridComputable(el) {
  ...
  for (const t of tracks) {
    // A track native computes today: a fixed length, a percentage, or a plain `fr` (no floor). A `minmax()`
    // (`t.floor` set) or an intrinsic keyword needs content measurement — defer to replay.
    const plain = t.px != null || t.pct != null || (t.fr != null && !t.floor);
  ...
  function walk(el, parentIdx) {
    ...
    if (laysOutAsGrid(el)) {
      ...
      if (nlGridComputable(el)) {
        ...
        for (const t of tracks) {
          if (t.px != null) gridsAll.push(0, t.px);            // fixed length
          else if (t.pct != null) gridsAll.push(1, t.pct / 100); // percentage → fraction of the content width
          else gridsAll.push(2, t.fr);                         // plain fr (gate ensured no floor)
        }
