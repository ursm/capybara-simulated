...
import { usedDisplay, uaDisplay, blockify, WIDGET_TAGS, declaredValue, declaredValueEntry, declaredValueIn, usedOverflow, propagatedOverflow,
         ...
         computedLetterSpacingPx, computedWordSpacingPx, declaresSpacing, textAlignOf, textIndentOf,
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const lineup = () => {
    ...
    const raw = textAlignOf(flowContainer(el), rtl);
    // `-webkit-center` & co — HTML's `align` attribute, `<center>` — line the text up AND align the
    // block-level descendants (see the block-child placement below).
    const legacy = raw.startsWith('-webkit-') ? raw.slice(8) : null;
    return (lineupMemo = { rtl, align: legacy || raw, descendants: legacy });
  };
  ...
  const moveLine = (shiftFor) => {
    for (const run of lineRuns) run.x += shiftFor(run.x);
  ...
  const breakLine = () => {
    ...
    if (indentNext) { indentNext = false; openLine(); }
  };
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        if (CSS_WS_RE.test(token)) {
          ...
          if (lineX > lineLeft || preserves) {
            ...
            const at = preserves ? placePreservedSpace(token, tokenW, lh, ownAsc)
  ...
  // `trailingPreserved`).
  const placePreservedSpace = (token, w, h, asc) => {
    const before = trailingPreserved;
    const at = placeOnLine(w, h, true, false, false, asc);
    trailingPreserved = before + w;
    const each = w / token.length;
    for (let k = 0; k < token.length; k++) lineGaps.push(at.x + k * each);
    return at;
  };
  ...
  const placeOnLine = (w, h, decided, hangs, edge, asc = h) => {
    ...
    lineX += w;
    if (hangs) { trailingHang += w; lineGaps.push(at.x); } else if (!edge) { trailingHang = 0; trailingPreserved = 0; }
    if (hangs) {
  ...
  for (const child of layoutChildren(el)) {
    ...
    // HTML's `align` attribute and `<center>` (`text-align: -webkit-center` & co) align the
    // block-level descendants as well: a narrower block moves in its band the way `margin: auto`
    // would (Chrome: a 100px block in a 300px `<center>` sits at 100). Behind the same latch the
    // line gate uses — a page with no such markup asks nothing.
    let legacyShift = 0;
...
export function charAdvances(text, el) {
  const f = fontOf(el);
  const out = [];
  ...
  for (let i = 0; i < text.length; i++) {
    const cp = text.codePointAt(i);
    if (cp > 0xFFFF) i++;
    out.push(unitOf(text, i, cp, prev, f.table) * f.size);
    prev = cp;
  }
  return out;
}
