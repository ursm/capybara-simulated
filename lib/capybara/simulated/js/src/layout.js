...
const NL_STRIDE = 71;   // node record; must match layout::Input / LAYOUT_STRIDE (rec[55] = grid_start, 56-61 = declared sizing, 62-67 = flex sizing, 68-70 = replaced intrinsic)
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (!isFlexItem && !isGridItem && (e.autoMargins & AUTO_MARGIN_X)) return false;
    ...
    if (laysOutAsFlex(el)) {
      ...
      // OUT-OF-FLOW children (§4.1): lay out each subtree at its resolved border box, but place it at the
      // container's border-box origin + the displacement the oracle already resolved (insets, or the
      // justify/align static position). r[43] marks it out of flow (native drops it from sizing/flow);
      // rel_x/rel_y (r[39]/r[40], unused by an abspos box) carry that displacement for `place`.
      for (const c of oof) {
        const itemIdx = nodes.length;
        ...
        r[6] = r[7] = r[8] = r[9] = NaN;
        r[54] = 0;                                         // box replayed definite (out of flow) — not self-sized, no two-phase
        r[43] = 1;
        r[39] = c._lb.x - el._lb.x;
    ...
    if (laysOutAsGrid(el)) {
      ...
      for (const c of oof) {
        ...
        r[54] = 0;                                              // box replayed definite — not self-sized, no two-phase
        r[43] = 1;                                              // out of flow
        r[39] = c._lb.x - el._lb.x;                             // displacement from this grid's border-box origin
        r[40] = c._lb.y - el._lb.y;
      }
    ...
    // (rec[28] != 0) + subtree is emitted here in flow order; native places it in its band.
    rec[2] = 1;
    for (const c of orderedKids) {
      if (!walk(c, idx)) return false;
    }
    // Out-of-flow (absolute / fixed) children: emit each subtree, then override with the oracle's resolved box
    // and mark it out of flow with its displacement from THIS block's border box — native lays the subtree out
    // and `place` positions it by that displacement alone (the same replay the flex path uses, §4.1).
    for (const c of oof) {
      const ci = nodes.length;
      if (!walk(c, idx)) return false;
      const r = recs[ci];
      r[3] = 1; r[4] = c._lb.width; r[5] = c._lb.height;   // pushed border box (insets / shrink-to-fit resolved)
      r[6] = r[7] = r[8] = r[9] = NaN;                     // already sized — no min/max re-clamp
      r[54] = 0;                                           // box replayed definite (out of flow) — not self-sized, no two-phase
      r[43] = 1;                                           // out of flow
      r[39] = c._lb.x - el._lb.x;                          // displacement from this block's border-box origin
      r[40] = c._lb.y - el._lb.y;
    }
    return true;
  }
  ...
  return { ok: true, nodes: nodes.length, compared, mismatches, sample, nativeIntrinsicGrids, nativeFlexRows };
};
