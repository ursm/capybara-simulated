...
function nlFlexSupported(el) {
  ...
  // A `<button>` SHRINK-WRAPS to its content (`shrinkWrapsToFit`) where native sizes a block-level flex
  // container to its containing block like any block — it laid a `display:flex` button out 400px wide against
  // the oracle's 16, and nothing refused it. The block path already declines one for the same reason; the grid
  // path asks the same question of its own containers.
  if (nlShrinkWrapsInFlow(el)) return false;
...
function nlGridSupported(el) {
  ...
  if (nlShrinkWrapsInFlow(el)) return false;   // a `display: grid` <button>, the twin of the flex case
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    for (const c of layoutChildren(el)) {
      ...
      // A block-level `<button>` SHRINK-WRAPS to its content in the oracle's block flow (`shrinkWrapsToFit`),
      // where native's block flow would fill the containing block — decline until native shrink-wraps it.
      if (nlShrinkWrapsInFlow(c)) return false;
    ...
    for (const c of orderedKids) {
      // THREE children native sizes from their OWN content, and so must be able to MEASURE: one whose block
      // axis is the horizontal one (a vertical `writing-mode`, no inline size to fill), one whose width is an
      // intrinsic-size KEYWORD, and a FLOAT with an auto width (§10.3.5 — shrink-to-fit is what a float's
      // `auto` means, where a block's fills). All three are settled here rather than in Rust, where a subtree
      // it cannot measure fails the whole pass instead of this one box — and all three are walked as MEASURED
      // subtrees, which is what keeps a measure-only gap (native's `text_intrinsic` has no `text-indent`)
      // from reaching them.
      ...
      if (!keywordWidth) {
        ...
        if (!isFloated(c) && blockAxisOf(c) !== 'width') continue;
...
// …and whether that shrink-wrap actually DECIDES this box's width, which is the question native has to ask:
// only in the block-flow path (`layoutBlockChildren`'s `autoW`), so only for a normal-flow BLOCK-LEVEL box
// whose width is `auto` and not a keyword. A flex or grid ITEM is sized by its container's algorithm, an
// out-of-flow box by the inset rules, a float by its own shrink-to-fit (which native does), an inline box by
// its content, and a declared or keyword width by itself — native is right in all of those, and refusing them
// cost 48 of a 200-shape A/B nothing but coverage. What it still refuses too broadly is a box whose CONTAINING
// BLOCK shrink-to-fits (a `<td>`, an out-of-flow parent), where filling the block and shrink-wrapping coincide
// — conservative rather than wrong, and on the decline census.
function nlShrinkWrapsInFlow(el) {
