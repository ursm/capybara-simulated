...
function nlFlexBasisSpec(c) {
  ...
  const px = resolveLayoutProp(c, 'flex-basis', 0, info);
  if (px == null) return null;
  if (!info.percent) return { px };
  // …a PLAIN percentage only, by its text once a CSS-wide keyword has resolved: any math function holding one —
  // `calc()`, `min(50%, 300px)`, `clamp()` — is not a fraction of the main size, and probing it at two bases
  // cannot tell (`min(50%, 300px)` scales at 1 and 2).
  let raw = declaredValue(c, 'flex-basis');
  if (raw != null && maybeCssWide(raw)) raw = cssWideStandsFor(c, 'flex-basis', raw);
...
function nlShadowRun(rootEl, trace) {
  ...
  function walkRecord(el, parentIdx, floated, bfc) {
    ...
    rec[4] = num(resolveLayoutProp(el, 'width', cbW, widthInfo));
    rec[5] = num(resolveLayoutProp(el, 'height', cbH));
    rec[6] = num(resolveLayoutProp(el, 'min-width', cbW));
    rec[7] = num(resolveLayoutProp(el, 'max-width', cbW));
    rec[8] = num(resolveLayoutProp(el, 'min-height', cbH));
    rec[9] = num(resolveLayoutProp(el, 'max-height', cbH));
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeInlineBox = (child, pos) => {
    ...
    child._lbCbH = content.height || null;
    child._lbOrder = ctx.order++;
    const frag = {
  ...
  for (const child of layoutChildren(el)) {
    ...
    const childCbH = content.height || null;
...
function layoutGrid(el, box, content, ctx) {
  ...
  // …resolved against the content height where that is DEFINITE, and as the box will be CLAMPED: a height imposed
  // on the grid (a flex stretch, both insets) arrives here before its min/max apply, and Chrome resolves the gap
  // against the clamped box (`row-gap: 10%` under a 200px stretch capped by `max-height: 100px` is 10, not 20).
  const vEdges = box.height - content.height;
  const rowBasis = content.height
    ? Math.max(0, clampToMinMax(el, box.height, 'height', el._lbCbH != null ? el._lbCbH : null, vEdges) - vEdges)
    : null;
  const rowGap = axisGap(el, 'row', rowBasis);
  ...
  for (const child of layoutChildren(el)) {
    ...
    const rel = flowShift(child, pos === 'relative' ? relativeOffset(child, trackW, content.height || null) : null);
...
function placeFloat(child, pos, ctx, content, right, top0) {
  ...
  const size  = usedSize(child, shrinkToFitWidth(child, avail, content.width), 0, content.width, content.height || null);
