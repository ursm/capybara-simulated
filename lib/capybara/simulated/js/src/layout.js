...
const NL_RUN_TEXT = 0, NL_RUN_OPEN = 1, NL_RUN_CLOSE = 2, NL_RUN_BR = 3, NL_RUN_ATOMIC = 4;
...
function nlGatherRuns(children, owner, runs, wsMode, shift) {
  ...
  const appendText = (fi, text) => {
    ...
    if (last && last.kind === NL_RUN_TEXT && last.shift === shift && nlSameFi(last.fi, fi)) last.text += text;
    else runs.push({ kind: NL_RUN_TEXT, fi, text, shift });
  };
  for (const c of children) {
    ...
    if (breaksAnywhere(c)) return { ok: false };                // overflow-wrap / word-break in-word breaking → JS
    ...
    if (openW !== 0 || closeW !== 0) {
      ...
      const sub = nlGatherRuns(layoutChildren(c), cf, runs, wsMode, cShift);
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (hasNonFloatBlock && hasInline) {
      ...
      if (breaksAnywhere(el)) return false;
      ...
      const flushGroup = () => {
        ...
        const g = [];
        const gr = nlGatherRuns(group, bf, g, wsMode);
        group = [];
    ...
    if (hasInline) {
      ...
      if (breaksAnywhere(el)) return false;                    // overflow-wrap / word-break in-word breaking → JS
      if (textIndentOf(flowContainer(el), el._lb.width) !== null) return false; // text-indent shifts wrapping → JS
      const bf = nlFontInfo(el);
      if (bf.nf < 0) return false;                             // block's own font non-system → JS
      const local = [];
      const g = nlGatherRuns(layoutChildren(el), bf, local, wsMode);
