...
const NL_STRIDE = 53;   // node record; must match layout::Input / LAYOUT_STRIDE
...
function nlGatherRuns(el, owner, runs) {
  ...
  for (const c of layoutChildren(el)) {
    ...
    if (whiteSpaceOf(c) !== 'normal') return { ok: false };
    ...
    if (openW !== 0 || closeW !== 0) {
      ...
      const sub = nlGatherRuns(c, cf, runs);
...
globalThis.__csimLayoutShadowRun = function (rootEl) {
  ...
  function walk(el, parentIdx) {
    ...
    if (hasInline) {
      ...
      if (whiteSpaceOf(el) !== 'normal') return false;
      ...
      const local = [];
      const g = nlGatherRuns(el, bf, local);
