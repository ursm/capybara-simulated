...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const lineFrags = [];
  // No line break may follow a run placed WHOLE — a `nowrap` / `pre` run, its trailing space
  // included: the opportunity after a space belongs to the space's own `white-space`, so the
  // word after `<span style="white-space:nowrap">… ff </span>` stays on its line and overflows,
  // and the line breaks at the next collapsible space of a wrapping context (Chrome: "gggg" at
  // 268.81 past a 300px block). Cleared by the next content placed after the run, or a break.
  let noBreakAfter = false;
  let trailingHang = 0;
  const lineGaps = [];
  ...
  const placeTextRun = (text, owner) => {
    ...
    segments.forEach((segment, i) => {
      ...
      // `…ff </span>gggg`).
      if (noBreakAfter && wraps && !preserves && lineEndsWithSpace && CSS_WS_RE.test(segment.charAt(0))) noBreakAfter = false;
      ...
      for (const token of run.split(WS_SPLIT_RE)) {
        ...
        if (split.freshLine && lineHasContent && !noBreakAfter) forceBreak('wrap');
        const units = split.units, shy = split.shy;
        for (let u = 0; u < units.length; u++) {
          let unit = units[u];
          let w = measureRun(unit, owner);
          // A SOFT HYPHEN shows a hyphen only where the line breaks at it: this piece takes one
          // when the next piece would not fit beside it (Chrome: "aaaabbbb\u00ADccccdddd" wraps
          // as "aaaabbbb-" / "ccccdddd", and stays one unhyphenated word where it fits).
          if (shy && shy[u] && u + 1 < units.length &&
              lineX + openEdgeWidth() + w + measureRun(units[u + 1], owner) > lineRight) {
            unit += '-';
            w = measureRun(unit, owner);
          }
          if (lineHasContent && !noBreakAfter && lineX + openEdgeWidth() + w > lineRight) forceBreak('wrap');
  ...
  const placeOnLine = (w, h, decided, hangs, edge, asc = h) => {
    ...
    if (!decided && lineHasContent && !noBreakAfter && lineX + pending + w > lineRight) forceBreak('wrap');
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (child.nodeType === 3) {
      ...
      if (!NON_WS_RE.test(t)) {
        ...
        // …and even collapsed away against a whole-placed run's own trailing space, a wrapping
        // context's space brings the break opportunity that run's could not (see `noBreakAfter`).
        if (t && modeWraps(whiteSpaceOf(owner))) noBreakAfter = false;
...
function contentIntrinsicWidths(el) {
  ...
  const addUnit = (text, owner, from, tab) => {
    ...
    if (text.indexOf('-') !== -1 || text.indexOf('\u00AD') !== -1 || text.indexOf('\u2010') !== -1) {
      const pieces = hyphenPieces(text);
      if (pieces.length > 1) {
...
function minBreaksAnywhere(el) {
  if (wordBreakMode(el) === 'break-all') return true;
...
function breakUnits(word, el, avail) {
  // A HYPHEN between two letters or digits, and a soft hyphen anywhere, is a break opportunity
  // like a space (UAX #14 HY; Chrome breaks "12-34-" | "56" and "aaaa-" | "bb"): the pieces are
  // units of their own whatever the room, the soft-hyphen ones flagged so the flow can show the
  // hyphen where it breaks. Split here so the flow and min-content agree.
  if (word.indexOf('-') !== -1 || word.indexOf('\u00AD') !== -1 || word.indexOf('\u2010') !== -1) {
    const pieces = hyphenPieces(word);
    if (pieces.length > 1) {
      const units = [], shy = [];
      let anyShy = false;
      for (const piece of pieces) {
        const sub = breakUnits(piece.text, el, avail);
      ...
      return { units, freshLine: false, shy: anyShy ? shy : null };
    }
  }
  let wide = false;
...
// dropped from the text — zero width, shown only at a break — and flagged instead).
const HYPHEN_BREAK_RE = /([\p{L}\p{N}][-\u2010])(?=[\p{L}\p{N}])/gu;
function hyphenPieces(word) {
  const out = [];
  const marked = word.replace(HYPHEN_BREAK_RE, '$1\u0000');
