...
function ensureCollapseBorders(table) {
  ...
  // facing cell is its own SEGMENT, collapsed independently against this cell's own side (`own`) — so a
  // `hidden` on ONE facing cell suppresses only ITS segment (§17.6.2.1), not the cell's whole edge — and the
  // cell's box owns the widest surviving half. (A non-spanning cell has one segment, so this is just
  // halfOf(combineW(own, facing)).) A ragged gap faces a 0-width border.
  const faceRows = (own, r0, rs, col, prop) => {
    let h = 0;
    for (let dr = 0; dr < rs && r0 + dr < rowCount; dr++) {
      const nb = occ[(r0 + dr) * n + col];
      const seg = halfOf(combineW(own, nb ? nb[prop] : 0));
      if (seg > h) h = seg;
    }
    return h;
  };
  const faceCols = (own, c0, cs, row, prop) => {
    let h = 0;
    const base = row * n;
  ...
  for (const row of grid.rows) {
    for (const cell of row.cells) {
      ...
      // An OUTER edge is one segment against the table's own border; an INTERNAL one is resolved per facing cell.
      const bl = atLeft   ? halfOf(combineW(cell._rbL, tbL)) : faceRows(cell._rbL, cell.row, cell.rowSpan, cell.col - 1, '_rbR');
      const br = atRight  ? halfOf(combineW(cell._rbR, tbR)) : faceRows(cell._rbR, cell.row, cell.rowSpan, cell.col + cell.colSpan, '_rbL');
      const bt = atTop    ? halfOf(combineW(cell._rbT, tbT)) : faceCols(cell._rbT, cell.col, cell.colSpan, cell.row - 1, '_rbB');
      const bb = atBottom ? halfOf(combineW(cell._rbB, tbB)) : faceCols(cell._rbB, cell.col, cell.colSpan, cell.row + cell.rowSpan, '_rbT');
