...
// Text native's pen-walk (`text_intrinsic` / `line_layout`) measures: a tab / CR / FF only where the mode
// PRESERVES it (tab stops are the block's business, so `line_layout` declines one it has to advance to — a
// collapsing mode never measures the whitespace run at all), and, where the owner breaks in-word per character
// (`nlWrapMode` 1 / 3), no ZWJ (the oracle's per-character advance carries the previous one). A WIDE character
// does NOT decline: native cuts the same break units the oracle's `charUnits` does (`break_unit_len`).
...
function nlGatherRuns(children, owner, runs, wsMode, shift, wrapMode, atomic = null, rel = null) {
  ...
  const appendText = (fi, text) => {
    ...
    if (last && last.kind === NL_RUN_TEXT && last.shift === shift && last.wrapMode === wrapMode && nlSameFi(last.fi, fi)) last.text += text;
    else runs.push({ kind: NL_RUN_TEXT, fi, text, shift, wrapMode });
  };
  for (const c of children) {
    if (c.nodeType === NODE_TEXT || c.nodeType === NODE_CDATA) {
      const td = c._data || '';
      if (HYPHEN_ANY_RE.test(td)) return { ok: false };         // hyphen / dash / soft-hyphen break opportunity → JS
      // …and text native's pen-walk cannot measure. A ZWJ where the owner breaks per character (the oracle's
