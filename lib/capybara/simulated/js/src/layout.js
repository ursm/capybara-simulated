...
import { isLaidOutNode, selfNotRendered, resolveLayoutProp, resolveCascadeDisplay,
         cascadeLayoutEpoch, mayConstrainSize, inlineAxisIsHorizontal } from './cascade.js';
...
import { usedDisplay, DEFAULT_DISPLAY, declaredValue, computedFontSizePx, computedLineHeight,
         ...
         fontRelativeToPx, uaDefault, computedBorderCollapse, computedBorderSpacing } from './style-proxy.js';

// Set the first time this pass sees a `position: sticky` box (in `positionOf`). Until then every
// `scrollShift` — the hot path behind every rect read — skips the sticky walk on one read.
let sawSticky = false;
...
// The cascade side of every layout memo is the STYLE epoch, not the rule-set version: it moves for a
// CSSOM edit *and* for a change in the dynamic style state a selector can read
// (`:placeholder-shown`, `:checked`, `:popover-open`, hover / focus — everything `bumpStyleState`
// records). Keyed on the rule-set version alone, layout answered from a cache no state change could
// invalidate: `#t:placeholder-shown { width: 300px }` kept its 300px box — through
// `getBoundingClientRect`, not just the CSSOM — after the field was filled. The cascade's own memos
// have always keyed on this epoch; layout shares it, so the two invalidate together.
...
// own update instead.
export function layoutGeneration() { return `${settleGen()}:${cascadeLayoutEpoch()}:${scrollEpoch}`; }
...
function clampToMinMax(el, size, axis, basis, extra) {
  ...
  const max = resolveLayoutProp(el, 'max-' + axis, basis);
  const min = resolveLayoutProp(el, 'min-' + axis, basis);
...
function positionOf(el) {
  ...
  // Noted here because every laid-out box passes through: until a page HAS a sticky box,
  // `scrollShift` — behind every rect read — skips the sticky walk on one property read.
  if (v === 'sticky') sawSticky = true;
  return v;
}
...
function autoMarginLead(el, cbW, borderBoxW, edges = null) {
  const cm = edges || edgeInsets(el, cbW);
  const leftAuto  = hasAutoMargin(cm, 'left');
  const rightAuto = hasAutoMargin(cm, 'right');
  if (!leftAuto && !rightAuto) return cm.ml;
  // The leftover is what the box doesn't take. Nothing spare (or a negative
  // remainder, the over-constrained case) leaves an `auto` margin at zero.
  const spare = cbW - borderBoxW - (leftAuto ? 0 : cm.ml) - (rightAuto ? 0 : cm.mr);
  if (!(spare > 0)) return leftAuto ? 0 : cm.ml;
  if (leftAuto && rightAuto) return spare / 2;
  return leftAuto ? spare : cm.ml;
}
// The used RIGHT margin, by the same rule — the flow itself never needs it (the box's own width
// and lead place it), but it is half of what the box reports.
function autoMarginTrail(el, cbW, borderBoxW, edges = null) {
  const cm = edges || edgeInsets(el, cbW);
  const leftAuto  = hasAutoMargin(cm, 'left');
  const rightAuto = hasAutoMargin(cm, 'right');
  if (!rightAuto) return cm.mr;
  const spare = cbW - borderBoxW - (leftAuto ? 0 : cm.ml);
  // OVER-CONSTRAINED (§10.3.3): a box wider than its containing block has a NEGATIVE remainder,
  ...
  // gives. The leading margin still takes 0 (`autoMarginLead`), so the box stays flush left.
  if (!(spare > 0)) return spare;
  return leftAuto ? spare / 2 : spare;
}
// The vertical half of the same rule, for an absolutely positioned box stretched between `top` and
// `bottom` (§10.6.4). Block flow never needs it: a vertical `auto` margin there computes to zero.
function autoMarginBlockLead(el, cbH, borderBoxH) {
  const e = edgeInsets(el, el._lbCbW != null ? el._lbCbW : cbH);
  const topAuto = hasAutoMargin(e, 'top'), bottomAuto = hasAutoMargin(e, 'bottom');
  if (!topAuto && !bottomAuto) return 0;
  const spare = cbH - borderBoxH - (topAuto ? 0 : e.mt) - (bottomAuto ? 0 : e.mb);
  if (!(spare > 0)) return topAuto ? 0 : e.mt;
  if (topAuto && bottomAuto) return spare / 2;
  return topAuto ? spare : e.mt;
}
...
function marginIsAuto(el, side) {
  const v = declaredValue(el, 'margin-' + side);
...
function placeAbsolute(child, pos, staticX, staticY, ctx, order = null) {
  ...
  const availH  = Math.max(0, cb.height - (top || 0) - (bottom || 0));
  const marLead = stretched  ? autoMarginLead(child, availW, size.width) : 0;
  const marTop  = stretchedV ? autoMarginBlockLead(child, availH, size.height) : 0;
  const cx = left != null ? cb.x + left + marLead
           : right != null ? cb.x + cb.width - right - size.width
           : staticX;
  const cy = top != null ? cb.y + top + marTop
  ...
  const box = { x: cx, y: cy, width: size.width, height: size.height, autoHeight: size.autoHeight,
                fixed: pos === 'fixed', outOfFlow: true };
  const from = ctx.order;
  layoutElement(child, box, ctx, cb.width);
  if (stretched) {
    child._lbMarH = { left: marLead, right: autoMarginTrail(child, availW, size.width) };
  }
...
function layoutFlexRow(el, box, content, ctx, { equalShare = false } = {}) {
  ...
  for (const idx of order) {
    ...
    if (size.autoHeight && child._lb.height === 0) child._lb.height = lineHeightOf(child);
    x += child._lb.width;
    if (child._lb.height > rowH) rowH = child._lb.height;
    if (size.autoHeight) stretchable.push({ child, cbW: content.width });
  }
  ...
  // CROSS-AXIS STRETCH (the default `align-items`): an item with an AUTO cross size grows to the
  // line's cross size, so a row of short items all end up as tall as its tallest. Without it a
  // single-icon flex item was as tall as the icon — 16px where Chrome gives 40 — and a click aimed
  // at its centre landed on whatever sat above.
  //
  // A second pass, because the line's cross size isn't known until every item has been measured;
  // an item that actually grows is laid out again so its own contents see the taller box, which is
  // what a percentage-height descendant resolves against.
  const lineCross = Math.max(0, (box.height || 0) - contentEdges(el, box.width));
  const cross = Math.max(rowH, lineCross);
  for (const { child, cbW: icbW } of stretchable) {
    if (!stretchesInCross(el, child) || child._lb.height >= cross) continue;
    const b = child._lb;
    layoutElement(child, { x: b.x, y: b.y, width: b.width, height: cross, autoHeight: false },
                  ctx, icbW);
  }
  stampExtent(el, box);
}
...
// Does this item take the line's cross size? Only when its own alignment resolves to `stretch`
// (the initial value, and what `normal` computes to for a flex item) and neither cross margin is
// `auto` — an `auto` margin eats the free space instead of the box growing into it.
function stretchesInCross(container, child) {
  // The cross size has to be `auto` — SPECIFIED auto, not merely unresolved. `height: 100%` against
  // an auto-height container resolves to nothing and the box falls back to its content, but the
  // declaration is still there, so Chrome does not stretch it (measured: 18px in a 40px row).
  const h = declaredValue(child, 'height');
  if (h != null && String(h).trim().toLowerCase() !== 'auto') return false;
  const self = alignKeyword(declaredValue(child, 'align-self'));
  const align = (self && self !== 'auto') ? self : alignKeyword(declaredValue(container, 'align-items'));
  if (align && align !== 'stretch' && align !== 'normal') return false;
  const e = edgeInsets(child, child._lbCbW != null ? child._lbCbW : 0);
  return !hasAutoMargin(e, 'top') && !hasAutoMargin(e, 'bottom');
}
function alignKeyword(v) { return v == null ? null : String(v).trim().toLowerCase(); }
...
function resolveFlexRowWidths(items, box, share) {
  ...
  const fallback = (w, i) => (w > 0 || outOfFlowOnly(items[i].child) ? w : share);
  ...
  if (free > 0) {
    ...
    return bases.map((b, i) => fallback(Math.round(b + (free * parts[i].grow) / totalGrow), i));
  }
  if (free < 0) {
    ...
    return bases.map((b, i) => {
      ...
      return fallback(Math.round(Math.max(min, b + (free * weights[i]) / totalW)), i);
    });
  }
...
function shiftSubtree(el, dx, dy) {
  ...
  for (const child of layoutChildren(el)) {
    // A `position: fixed` box is laid out against the VIEWPORT, so no ANCESTOR's offset —
    // a relative inline's, a bottom-anchored box's own correction — may carry it along.
    // (The root of the walk is the box being moved on purpose, so it always moves.)
    if (child.nodeType === NODE_ELEMENT && !(child._lb && child._lb.fixed)) shiftSubtree(child, dx, dy);
  }
}
...
function reuseSubtree(el, box, cbW, ctx) {
  ...
  // Only when it is asked for the same SIZE. A caller that passes height 0 is asking it to fill
  // from its own content, which it already did; one that passes a real height is imposing it.
  if (prev.width !== box.width) return false;
  if (box.height !== 0 && box.height !== prev.height) return false;
...
function layoutElement(el, box, ctx, cbW = box.width) {
  ...
  try {
    ...
    if (waiting) {
      ...
      for (const w of waiting) placeAbsolute(w.child, w.pos, w.staticX, w.staticY, w.ctx, w.order);
      stampExtent(el, box);
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const settleInlineBoxes = () => {
    ...
    for (const d of deferredOutOfFlow) {
      ...
      placeAbsolute(d.child, d.pos, x, y, d.ctx, d.order);
    }
  ...
  for (const child of layoutChildren(el)) {
    ...
    const lead  = distributes ? autoMarginLead(child, content.width, size.width, cm)  : cm.ml;
    const trail = distributes ? autoMarginTrail(child, content.width, size.width, cm) : cm.mr;
    const rel  = pos === 'relative' ? relativeOffset(child, content.width, content.height || null) : null;
    ...
    child._lbMarH = { left: lead, right: trail };
  ...
  if (box.height === 0 && box.autoHeight !== false && hadInFlow) {
...
function layoutGrid(el, box, content, ctx) {
  const gap    = gridGap(el, content.width);
...
// The COLUMN gap. `gap: <row> <column>` puts the column figure SECOND (a single
// value covers both), and `column-gap` — the longhand — wins over the shorthand
// whichever order they cascade in, because `declaredValue` already resolved that.
// Reading the first token of `gap: 10px 40px` took the ROW gap, which then fed the
// track widths, the item offsets AND the auto-fill count.
function gridGap(el, contentW) {
  const own = declaredValue(el, 'column-gap');
  if (own != null && String(own).trim() !== 'normal') {
    const px = gapLength(el, String(own).trim(), contentW);
  ...
  return gapLength(el, parts.length > 1 ? parts[1] : parts[0], contentW) ?? 0;
}
// A gap is a length or a percentage of the container's own content box.
function gapLength(el, text, contentW) {
  const pct = /^(-?\d*\.?\d+)%$/.exec(text);
  if (pct) return contentW == null ? 0 : (parseFloat(pct[1]) / 100) * contentW;
...
function stickyDelta(el) {
  ...
  let parent = layoutParent(el);
  while (parent && parent._lb && TABLE_INTERNAL_DISPLAY.has(displayOf(parent))) parent = layoutParent(parent);
  while (parent && parent._lb && displayOf(parent) === 'inline') parent = layoutParent(parent);
...
export function scrollTargetFor(self, target, pos, x, y) {
  ...
  if (target) {
    ...
    if (b) {
      ...
      // `scroll-margin` is the gap a page asks to be left around a box when it is scrolled to —
      // how a site with a fixed header keeps an anchor target from landing UNDER it (Redmine's
      // `#update { scroll-margin-block-start: 50px }`). The scroll snaps to the box's margin box,
      // not its border box, so each alignment measures against that.
      // `<length>` only — a percentage is invalid here, so it resolves against nothing.
      const smTop    = resolveLayoutProp(target, 'scroll-margin-top')    || 0;
      const smBottom = resolveLayoutProp(target, 'scroll-margin-bottom') || 0;
      const smLeft   = resolveLayoutProp(target, 'scroll-margin-left')   || 0;
      const outerH = b.height + smTop + smBottom;
      sy = b.y - smTop - oy;                                    // align:top
      if (pos === 'center')      sy = b.y - smTop - oy - (vpH - outerH) / 2;
      else if (pos === 'bottom') sy = b.y - smTop - oy - (vpH - outerH);
      sx = b.x - smLeft - ox;                                   // bring a horizontally-offscreen box in too
    }
...
globalThis.__csimUsedStyle = function (el, prop) {
  ...
  switch (prop) {
    ...
    // A horizontal `auto` margin resolves to the slack it took (§10.3.3 / §10.3.7);
    // a vertical one is zero, which is what `edgeInsets` already answers.
    case 'margin-left':   return el._lbMarH ? el._lbMarH.left  : e.ml;
    case 'margin-right':  return el._lbMarH ? el._lbMarH.right : e.mr;
    case 'margin-top':    return e.mt;
    case 'margin-bottom': return e.mb;
