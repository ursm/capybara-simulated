...
function nlGatherRuns(children, owner, ctx, wsMode, shift, wrapMode, rel = null, ownerOpen = false) {
  ...
  for (const c of children) {
    ...
    if (openW !== 0 || closeW !== 0 || openPlain !== 0 || closePlain !== 0) {
      ...
      // An edged (padded / bordered) inline is admitted only when it carries REAL content — a whitespace-only
      // one (even under preserve) stays declined: its line box height is mis-sized by a separate pre-existing
      // bug (native ignores a padded inline's own font-metric box), which an empty-but-for-whitespace shape
      // would silently expose. (An empty edged inline was always fiddly → JS.)
      if (!sub.hasReal) return { ok: false, reason: 'edged-inline-without-content' };
...
function layoutElementInner(el, box, ctx, cbW = box.width) {
  ...
  const placeInlineChild = (child, owner) => {
    ...
    if (pos === 'absolute' || pos === 'fixed') {
      ...
      if (open && open.pendingOpen && openEdgeWidth()) pendingStatic.set(child, open);
  ...
  const settleInlineBoxes = () => {
    ...
    for (const d of deferredOutOfFlow) {
      ...
      const x = line ? line.startX + (line.afterOpenEdge ? from.ce.left : 0) : d.staticX;
