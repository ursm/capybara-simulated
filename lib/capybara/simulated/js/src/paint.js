...
import { recordingRuns, viewportSize, rectOf, clipBoxesFor } from './layout.js';
...
export function paintPage({ full = false } = {}) {
  ...
  return recordingRuns((runs) => {
    ...
    const height = Math.max(1, Math.ceil(full ? Math.max(vp.height, root._lbExt ? root._lbExt.bottom : 0) : vp.height));
    ...
    const dx = full ? (root._scrollLeft || 0) : 0;
    const dy = full ? (root._scrollTop  || 0) : 0;
    // A page with no background of its own is white, as a browser's canvas is.
    g.fillStyle = paintColor(root, 'background-color') || paintColor(doc.body, 'background-color') || '#ffffff';
    g.fillRect(0, 0, width, height);

    for (const el of boxesInPaintOrder(root)) {
      ...
      clipped(g, el, dx, dy, () => {
        ...
        // and padding, which is where layout sized it to sit.
        if (el._pixels && el._naturalWidth) {
