...
export function paintPage({ full = false } = {}) {
  ...
  return recordingRuns((runs) => {
    ...
    for (const run of runs) {
      ...
      const m = paintTransformOf(owner);
      if (m === false) continue;
      if (m && singular(m)) continue;
      ...
      clipped(g, owner, m, dx, dy, () => {
        ...
        const ls = computedLetterSpacingPx(owner), ws = computedWordSpacingPx(owner);
        if (ls || ws) {
          const advances = charAdvances(run.text, owner), slots = spacingSlots(run.text);
          let pen = x, k = 0;
          for (const ch of run.text) {
            const adv = advances[k], spaced = slots[k];
            k++;
            if (adv > 0 && ch !== ' ' && ch !== '\u00A0') g.fillText(ch, pen, y, adv);
