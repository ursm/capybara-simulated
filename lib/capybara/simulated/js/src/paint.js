...
import { recordingRuns, viewportSize, rectOf, paintRectOf, laidOutBox, clipBoxesFor } from './layout.js';
...
export function paintPage({ full = false } = {}) {
  ...
  return recordingRuns((runs) => {
    ...
    for (const el of painted) {
      ...
      // …the UNTRANSFORMED box: this painter moves neither the text runs inside a box nor its
      // borders, so drawing the box somewhere its content is not would put the two in different
      // places. Painting a transform is its own piece of work (carry the matrix down through the
      // runs, and rasterize a quad for what is not a rectangle); until then the rect APIs know
      // about transforms and the picture does not.
