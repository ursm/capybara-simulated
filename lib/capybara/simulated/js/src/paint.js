...
// what a click lands on. A box's text is a layer of its own there, painted in the inline phase
// rather than with the box's background. (Opacity groups are not modelled.)
import { NODE_ELEMENT } from './constants.js';
import { stashTransfer } from './bytes.js';
import { recordingRuns, viewportSize, paintRectOf, paintTransformOf, paintQuadOf, paintOrder,
         laidOutBox, clipBoxesFor, charAdvances, charFaceSizes, spacingSlots } from './layout.js';
...
function fontString(el, size) {
  if (size == null) size = computedFontSizePx(el) || 16;
...
function runBoxOwner(run) {
  ...
  if (!owner || owner.nodeType !== NODE_ELEMENT) return null;
  ...
  return boxOwner && boxOwner.nodeType === NODE_ELEMENT && boxOwner._lb ? boxOwner : null;
}

// One box: its background, its borders and its bitmap. `dx` / `dy` undo the root's scroll for a
// full-page paint; `width` / `height` are the frame, for culling.
function paintBox(g, el, dx, dy, width, height) {
  ...
  if (m === false) return;                        // a transform the painter cannot express at all
  ...
  if (offFrame(m ? transformedBounds(m, r.x, r.y, r.width, r.height, dx, dy)
                 : { x, y, width: r.width, height: r.height }, width, height)) return;
...
function paintRun(g, run, boxOwner, dx, dy, width, height) {
  ...
  clipped(g, boxOwner, m, dx, dy, () => {
    ...
    if (ls || ws || jw || run.tab) {
      ...
      // A unicode-range split draws each glyph at its own face's size (a `size-adjust` face is
      // bigger), so the font string is re-set per character when the size changes.
      const sizes = charFaceSizes(run.text, owner);
      let pen = x, k = 0, curFont = null;
      for (const ch of run.text) {
        const adv = advances[k], spaced = slots[k];
        if (sizes) { const fs = fontString(owner, sizes[k]); if (fs !== curFont) { g.font = fs; curFont = fs; } }
        k++;
...
export function paintPage({ full = false } = {}) {
  ...
  return recordingRuns((runs) => {
    ...
    // The layers, in the order they paint: every laid-out box, and the text each box owns.
    const layers = [], textOf = new Map();
    for (const run of runs) {
      if (!run.text || !/\S/.test(run.text)) continue;
      const boxOwner = runBoxOwner(run);
      if (!boxOwner) continue;
      let text = textOf.get(boxOwner);
      if (!text) { text = { textOf: boxOwner, runs: [] }; textOf.set(boxOwner, text); layers.push(text); }
      text.runs.push(run);
    }
    if (root) {
      const walk = (n) => {
        if (n.nodeType === NODE_ELEMENT && n._lb && n !== root) layers.push(n);   // the root's background filled the canvas
    ...
    for (const layer of paintOrder(layers)) {
      if (layer.textOf) for (const run of layer.runs) paintRun(g, run, layer.textOf, dx, dy, width, height);
      else paintBox(g, layer, dx, dy, width, height);
    }
    return canvas;
  });
}
