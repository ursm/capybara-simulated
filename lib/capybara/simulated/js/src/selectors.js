...
function namespaceFallback(key, context) {
  ...
  let compiled;
  try { compiled = cssSelect.compile(cssWhat.stringify(groups), {adapter, pseudos: userPseudos, cacheResults: false, lowerCaseAttributeNames: false}, context); }
  catch (_) { return null; }
...
function pseudoElementFallback(key, context) {
  ...
  for (const arm of arms) {
    ...
    try { fns.push(cssSelect.compile(arm, {adapter, pseudos: userPseudos, cacheResults: false, lowerCaseAttributeNames: false}, context)); }
