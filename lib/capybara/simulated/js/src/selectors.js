...
// CSS Selectors-4 user-action pseudos plus `:scope`. css-select v7
// has its own scope mechanism that requires passing a context option
// to `selectAll`, but since we pre-compile selectors we route `:scope`
// through a tracker module-local instead — `selectAll` sets the
// scope root before invoking the matcher and clears it afterwards.
// jQuery UI's `find('> *')` (normalised to `:scope > *`) exercises
// this: the menu widget's `refresh` finds direct children to add the
// `ui-menu-item` class to.
let __scopeRoot = null;
const userPseudos = {
  ...focusPseudoMatchers,
  scope: (el) => __scopeRoot != null && el === __scopeRoot
};
...
export function selectAll(roots, sel, scopeRoot) {
  ...
  if (scopeRoot === undefined) return adapter.findAll(fn, roots);
  const prev = __scopeRoot; __scopeRoot = scopeRoot;
  try { return adapter.findAll(fn, roots); }
  finally { __scopeRoot = prev; }
}
export function selectFirst(roots, sel, scopeRoot) {
  const fn = compile(sel);
  if (scopeRoot === undefined) return adapter.findOne(fn, roots);
  const prev = __scopeRoot; __scopeRoot = scopeRoot;
  try { return adapter.findOne(fn, roots); }
  finally { __scopeRoot = prev; }
}
