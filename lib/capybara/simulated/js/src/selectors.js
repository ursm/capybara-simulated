...
// declaration in the FLAT tree. An `xml:lang` attribute counts on any element; an
// unnamespaced `lang` attribute counts ONLY on an HTML-namespace element (so a
// non-HTML element's bare `lang` is ignored and it inherits — lang-attribute). At a
// shadow root the walk crosses to the host (lang-attribute-shadow); a slotted
// element reaches the host through its light-DOM parent. '' when undeclared.
...
// match lang `en-CA`, `en-NZ` does not), ASCII case-insensitively.
function matchesLang(el, arg) {
  const lang = langOfElement(el).toLowerCase();
  if (!lang) return false;
  for (let range of String(arg).split(',')) {
    range = range.trim().replace(/^["']|["']$/g, '').toLowerCase();
...
function rewriteLang(key) {
  if (key.indexOf('lang(') === -1) return null;
  let groups;
  ...
  const walk = (list) => {
    for (const tokens of list) for (const t of tokens) {
      ...
      if (t.name === 'lang') { t.name = '__csimlang'; found = true; }
