...
// into the tree, regardless of delegatesFocus). We don't model the
// keyboard-vs-pointer heuristic that sets :focus-visible apart, so it matches
// wherever :focus does (they were identical before shadow support too). The
// host-chain walk compares only HOSTS (DOM ancestors are :focus-within's job)
// and is gated on the page having a shadow host so the common shadow-free read
// stays O(1) (rule 3).
