...
import { getCheckedness, controlLiveValue, isNodeActuallyDisabled, isContenteditable, READONLY_INPUT_TYPES } from './form-helpers.js';
...
function isReadWriteForMatch(el) {
  const t = el._tag;
  if (t === 'input') return READONLY_INPUT_TYPES.has((el._attrs.type || 'text').toLowerCase()) && el._attrs.readonly == null && !isNodeActuallyDisabled(el);
...
function isEnabledForMatch(el) {
  return (ENABLEABLE_TAGS.has(el._tag) || isFormAssociatedCustomElement(el)) && !isNodeActuallyDisabled(el);
}
...
function langOfElement(el) {
  for (let n = el; n; n = n._parent || (n.host || null)) {
    if (n.nodeType === NODE_ELEMENT) {
      const a = n._attrs;
      if (a) {
        if (a['xml:lang'] != null) return String(a['xml:lang']);
        if (n._ns === HTML_NS && a.lang != null) return String(a.lang);
      }
    }
  }
  return '';
...
function langRanges(arg) {
  ...
  if (!r) {
    r = String(arg).split(',').map((s) => s.trim().replace(/^["']|["']$/g, '').toLowerCase()).filter(Boolean);
...
function matchesLang(el, arg) {
  const lang = langOfElement(el).toLowerCase();
...
const userPseudos = {
  ...
  state: (el, arg) => {
    ...
    return !!(internals && internals._states && internals._states.has(String(arg)));
  },
  ...
  'placeholder-shown': (el) => {
    ...
    const v = controlLiveValue(el);   // the live value, not the `value` attribute
    return v == null || v === '';
  },
  default: (el) => {
    ...
    if (el._tag === 'button') {
      if ((el._attrs.type || 'submit').toLowerCase() !== 'submit') return false;
      // A <button> inside a <select> (the customizable-select display button) is
      // not a form submit button, so never the form's :default button.
      for (let p = el._parent; p; p = p._parent) if (p._tag === 'select') return false;
      return true;
    }
    return false;
  },
  ...
  open: (el) => (el._tag === 'details' || el._tag === 'dialog') && el._attrs.open != null,
  ...
  'user-valid':   (el) => el._userInteracted === true && isValidationCandidate(el) && !controlIsInvalid(el),
  'user-invalid': (el) => el._userInteracted === true && controlIsInvalid(el),
...
function compile(sel, scopeRoot) {
  ...
  if (scopeRoot == null || !selectorNeedsScope(key)) {
    const fn = compiledCacheNoScope.get(key);
    ...
    try {
      const compiled = compileRaw(key);
      compiledCacheNoScope.set(key, compiled);
      return compiled;
    } catch (e) {
      if (e instanceof globalThis.DOMException && e.name === 'SyntaxError') {
        compiledCacheNoScope.set(key, { message: e.message });
      }
      throw e;
    }
  }
...
function buildNsMatcher(selectorText, ns) {
  let groups;
  ...
  if (bad) return () => false;
  let compiled;
  try { compiled = cssSelect.compile(cssWhat.stringify([group]), {adapter, pseudos: userPseudos, cacheResults: false, lowerCaseAttributeNames: false}); }
  catch (_) { return null; }
...
export function matchesSelectorNS(el, selectorText, ns) {
  ...
  const key = selectorText + '\x01' + nsSignature(ns);
  let fn = nsMatcherCache.get(key);
  if (fn === undefined) { fn = buildNsMatcher(selectorText, ns); nsMatcherCache.set(key, fn); }
