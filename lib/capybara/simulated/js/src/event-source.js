...
// socket access.
//
// Exported as `install()` because the class extends
// `globalThis.EventTarget` and uses `globalThis.MessageEvent` /
// `globalThis.Event`, none of which exist until bridge.entry.js's
// IIFE has run far enough to define them. The bridge calls
// `install()` after those classes are in place.
  ...
  globalThis.__csim_deliverEventSourceEvents = function (events) {
    ...
    for (const e of events) {
      ...
      if (e.type === '__open') {
        if (src.readyState === 0) {
          src.readyState = 1;
          const evt = new globalThis.Event('open');
      ...
      if (e.type === '__error') {
        src.readyState = 2;
        const evt = new globalThis.Event('error');
