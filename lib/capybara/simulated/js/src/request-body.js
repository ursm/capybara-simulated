...
import { NODE_ELEMENT, NODE_COMMENT, NODE_DOCTYPE, NODE_PI } from './constants.js';
...
// Serialize an HTML Document as HTML (the HTML fragment serialization of its
// child nodes), NOT XML — `<img>` not `<img />`, no xmlns, with the doctype. Each
// element delegates to `outerHTML`, which is already the HTML serializer for an
// HTML-document node. Works on a cross-realm document (childNodes / outerHTML are
// plain property reads over the remote-ref), which is why it can't reuse
// html-parser's `_children`-based serializeChildren. A Document's children are only
// the doctype, the root element, and comments / PIs (a Text node can't be a direct
// document child), so no text-escaping case is needed.
function serializeDocumentAsHtml(doc) {
  let out = '';
  const kids = doc.childNodes;
  for (let i = 0; i < kids.length; i++) {
    const c = kids[i];
    switch (c.nodeType) {
      case NODE_DOCTYPE: out += '<!DOCTYPE ' + (c.name || '') + '>'; break;
      case NODE_ELEMENT: out += c.outerHTML; break;
      case NODE_COMMENT: out += '<!--' + (c.data || '') + '-->'; break;
      case NODE_PI:      out += '<?' + (c.target || '') + ' ' + (c.data || '') + '>'; break;
    }
  }
  return out;
}
...
// Body-type brand checks that work CROSS-REALM: a fetch / XHR body created in ANOTHER realm
// (e.g. `frame.contentWindow.fetch(url, {body: blobFromThisWindow})`) fails
// `instanceof globalThis.X` because each realm has its own Blob / ReadableStream class.
// Symbol.toStringTag is realm-independent (the same well-known symbol), matching the spirit of
// the cross-realm ArrayBuffer brand-check in serializeRequestBody.
export const isReadableStreamBody = body => Object.prototype.toString.call(body) === '[object ReadableStream]';
...
export function serializeRequestBody(body, headers) {
  ...
  if (body && body.nodeType === 9) {
    // A Document body (XHR send(document)) is serialized + UTF-8 encoded. The choice
    // of serializer + default Content-Type follows the document's TYPE, not its
    // contentType MIME: an HTML document (incl. one a frame built from text/plain or
    // an image) serializes with the HTML serializer (`<img>`, no xmlns) as
    // text/html; an XML document uses XMLSerializer as application/xml.
    const html = isHtmlDocument(body);
    setContentTypeIfMissing(headers, html ? 'text/html;charset=UTF-8' : 'application/xml;charset=UTF-8');
    const out = html ? serializeDocumentAsHtml(body) : new globalThis.XMLSerializer().serializeToString(body);
    return { body: out, b64: false, charsetFixable: true };
  }
  ...
  if (isBlobBody(body))                            return serializeBlob(body, headers);
  ...
  // A ReadableStream is a valid BodyInit with NO default Content-Type (Fetch "extract a
  // body"). The async drain happens in the caller (collectBodyStream); this fallback only
  // guards a stream that reaches here from not gaining a bogus text/plain default.
  if (isReadableStreamBody(body)) {
    return { body: String(body), b64: false };
  }
  // Any other value is coerced to a USVString (ToString) and, like a string body,
  // defaults to text/plain;charset=UTF-8 (Fetch "extract a body" — the union's
  // fallback member is USVString), e.g. `{toString: () => 'hi'}` (request-init-002).
  setContentTypeIfMissing(headers, 'text/plain;charset=UTF-8');
  return { body: String(body), b64: false, charsetFixable: true };
}
