...
// Absolute <font-size> keywords → px, the CSS scaling table anchored at medium = 16px.
...
export class CanvasRenderingContext2D {
  ...
  createPattern(image, repetition) {
    ...
    // An <img> or <video> with no usable bitmap: per "check the usability of the image
    // argument", only a request that FAILED is "broken" (throw InvalidStateError). A
    // srcless / still-loading <img>, or a <video> with no decoded frame (readyState
    // HAVE_NOTHING / HAVE_METADATA), is merely "bad" usability → null.
    if (image._tag === 'img' || image._tag === 'video') {
      if (image._imgBroken) throw new globalThis.DOMException('the image is broken', 'InvalidStateError');
      return null;
    }
    // An SVG <image> with no bitmap we can produce (we don't yet model SVG-image
    // resource loading, so we can't tell a broken href from a zero-size / still-loading
    // one) stays an InvalidStateError. Distinguishing bad→null from broken→throw here
    // needs the SVG-image load-state backlog.
    if (image._tag === 'image') {
  ...
  _serializeFont(input) {
    ...
    else if (st.type === 'Identifier') sizePx = ABSOLUTE_SIZE_PX[st.name.toLowerCase()];
  ...
  measureText(text) {
    ...
    // Letter/word spacing widen the advance; they depend on the current spacing state
    // (not the font+text cache key), so add them to a copy rather than the cached base.
    const extra = this._spacingWidth(text);
    return extra ? {...m, width: m.width + extra} : m;
  }
