...
export function createImageBitmap(source, optionsOrSx, sy, sw, sh, opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // A source that can't be decoded (empty / corrupt / unsupported bytes) rejects
    // with an InvalidStateError, per "the source image could not be decoded".
    const undecodable = () => new globalThis.DOMException('the source image could not be decoded', 'InvalidStateError');
    if (!bytes) return reject(undecodable());
    const decoded = globalThis.__csim_decodeImage(globalThis.btoa(bytes), options.resizeWidth | 0, options.resizeHeight | 0);
    if (!decoded) return reject(undecodable());
    const pixelBytes = fetchTransfer(decoded.refId) || fetchedToBytes(decoded.pixels);
    if (!pixelBytes) return reject(undecodable());
    const bm = new ImageBitmap();
