...
export class CanvasRenderingContext2D {
  ...
  // py)) that passes the clip mask. No shadow — clearRect and the shadow pass
  // itself use this directly.
  ...
  // Paint an op, casting a shadow first when one is active.
  _paintWith(painter, enumerate) {
    if (!painter) return;
    if (this._shadowActive()) this._shadowPass(set => enumerate((px, py) => set(px, py, 1)));
  ...
  // current path under the winding rule? A Path2D first arg isn't supported (the
  // current path is tested). The point and path share user space (the CTM applies
  // to both), so no transform is needed — consistent with how fill() paints.
  isPointInPath(a, b, c, d) {
    let x, y, rule;
    if (typeof a === 'object' && a !== null) { x = +b; y = +c; rule = d; }
    else { x = +a; y = +b; rule = c; }
  ...
  isPointInStroke(a, b, c) {
    let x, y;
    if (typeof a === 'object' && a !== null) { x = +b; y = +c; } else { x = +a; y = +b; }
    if (!allFinite(x, y)) return false;
  ...
  _drawText(text, x, y, maxWidth, obj, solid) {
    ...
    const cw = this.canvas.width | 0, ch = this.canvas.height | 0, clip = this._clip, ga = this.globalAlpha;
    ...
    if (obj instanceof CanvasGradient) {
      ...
      paint = (i, px, py, cov) => {
        ...
        if (c) blendPixel(buf, i, c, clamp01(c.a * ga * cov));
      };
    } else {
      if (clamp01(solid.a * ga) <= 0) return;
      paint = (i, px, py, cov) => blendPixel(buf, i, solid, clamp01(solid.a * ga * cov));
    }
  ...
  clearRect(x, y, w, h) {
    if (!allFinite(x = +x, y = +y, w = +w, h = +h) || !w || !h) return;
    // clearRect ignores fillStyle / globalAlpha and casts NO shadow — it zeroes
    // covered pixels (still honoring the clip mask), so it composites directly.
    this._composite((buf, i) => { buf[i] = buf[i + 1] = buf[i + 2] = buf[i + 3] = 0; },
