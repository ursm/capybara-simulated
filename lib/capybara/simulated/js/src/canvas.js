...
export class ImageData {
  constructor(a, b, c) {
    if (a instanceof globalThis.Uint8ClampedArray) {
      ...
      this.width  = a | 0;
      this.height = b | 0;
      assertImageArea(this.width, this.height);
...
export class CanvasRenderingContext2D {
  ...
  // createPattern(image, repetition): a tiled-image fill/stroke style. A null
  // repetition (or '') defaults to 'repeat'; an unknown one is a SyntaxError. The
  // image's current pixels are snapshotted; a source with no usable bitmap (a
  // broken/undecoded image, a zero-area canvas) is an InvalidStateError.
  createPattern(image, repetition) {
    ...
    if (src) { px = src.pixels; w = src.width; h = src.height; }
    else if (isCanvasSource(image)) {          // a blank (undrawn) canvas → transparent tile
      w = image.width | 0; h = image.height | 0;
      px = image._pixels || new globalThis.Uint8ClampedArray(Math.max(0, w * h) * 4);
    }
    if (!px || !w || !h) throw new globalThis.DOMException('image has no usable bitmap', 'InvalidStateError');
    // Snapshot the source pixels — a later draw to the source (e.g. a live canvas)
    // must not change the pattern.
  ...
  getImageData(x, y, w, h) {
    x |= 0; y |= 0; w |= 0; h |= 0;
    if (w < 0) { x += w; w = -w; }   // negative width/height normalizes the rect (spec)
    if (h < 0) { y += h; h = -h; }
