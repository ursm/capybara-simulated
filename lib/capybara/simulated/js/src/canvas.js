...
// pixels): they need a whole-canvas pass, not the per-covered-pixel path.
...
export class CanvasPattern {
  constructor(pixels, w, h, repetition) {
...
export class CanvasPath {
  ...
  _arcImpl(cx, cy, rx, ry, rot, a0, a1, ccw) {
    ...
    for (let i = 0; i <= n; i++) { const p = pointAt(a0 + delta * (i / n)); sub.pts.push(this._store(p[0], p[1])); }
...
export class CanvasRenderingContext2D {
  ...
  set fillStyle(v) {
    if (v instanceof CanvasGradient || v instanceof CanvasPattern) { this._fillObj = v; return; }
    const c = parseColorRGBA(this._resolveCurrentColor(v)); if (c) { this._fill = c; this._fillObj = null; }
  }
  get strokeStyle() { return this._strokeObj || serializeCanvasColor(this._stroke); }
  set strokeStyle(v) {
    if (v instanceof CanvasGradient || v instanceof CanvasPattern) { this._strokeObj = v; return; }
    const c = parseColorRGBA(this._resolveCurrentColor(v)); if (c) { this._stroke = c; this._strokeObj = null; }
  }
  get shadowColor() { return serializeCanvasColor(this._shadow); }
  set shadowColor(v) { const c = parseColorRGBA(this._resolveCurrentColor(v)); if (c) this._shadow = c; }

  // Resolve the CSS `currentColor` keyword to the canvas element's computed `color`
  // at assignment time (a detached canvas / OffscreenCanvas has no element, so it
  // falls back to the initial black). Any other value passes through untouched.
  _resolveCurrentColor(v) {
    if (typeof v !== 'string' || v.trim().toLowerCase() !== 'currentcolor') return v;
    try {
  ...
  createPattern(image, repetition) {
    ...
    if (src) {
      ...
      // change the pattern.
      return new CanvasPattern(new globalThis.Uint8ClampedArray(src.pixels), src.width, src.height, repetition);
    }
    if (isCanvasSource(image)) {
      ...
      return new CanvasPattern(new globalThis.Uint8ClampedArray(px), w, h, repetition);
    }
    // An <img> with no usable bitmap: a failed request is "broken" (throw), while a
    // srcless / still-loading / zero-natural-size image is merely "bad" → null.
    if (image._tag === 'img') {
      if (image._imgBroken) throw new globalThis.DOMException('the image is broken', 'InvalidStateError');
      return null;
    }
    // A CanvasImageSource with no bitmap we can produce — an SVG <image> (we don't
    // decode SVG document images) or a <video> with no decoded frame — is an
    // InvalidStateError, per "check the usability" for a broken/undrawn source.
    if (image._tag === 'image' || image._tag === 'video') {
      throw new globalThis.DOMException('the image has no usable bitmap', 'InvalidStateError');
    }
  ...
  // No arguments resolves to the matrix overload with a default (identity) init, so
  // it resets the transform.
  setTransform(a, b, c, d, e, f) {
    if (a === undefined) { this._m = [1, 0, 0, 1, 0, 0]; return; }
    if (a != null && typeof a === 'object') {   // the single-matrix overload
      const m = matrix2DInit(a);
      if (m.every(isFinite)) this._m = m;
      return;
    }
  ...
  _painter(obj, solid) {
    ...
    if (obj instanceof CanvasGradient || obj instanceof CanvasPattern) {
      ...
      if (!inv) return null;
      const sample = obj._sampler();        // user space → colour (gradient or tiled pattern)
  ...
  isPointInPath(a, b, c, d) {
    ...
    return pointInRings(this._devicePath(path), x, y, rule === 'evenodd');
  }
  ...
  isPointInStroke(a, b, c) {
    ...
    if (pointInRings(rings, x, y, false)) return true;
    // A point lying exactly on a ring edge (e.g. the centre of a butt cap at a dash's
    // start) is "on" the stroke and counts as inside, but the winding parity drops it
    // because the crossing sits on the boundary — catch it with an on-edge test.
    for (const ring of rings) {
      const n = ring.length;
      for (let i = 0; i < n; i++) {
  ...
  _fontFamily() {
    ...
    const sm = /(-?\d*\.?\d+)(px|pt|em|rem|%)?/.exec(s);
    // Everything after the size token (minus any /line-height); first comma-item.
    const fam = sm ? (s.slice(sm.index + sm[0].length).replace(/^\s*\/\s*[\d.]+\w*/, '').trim().split(',')[0] || '').trim().replace(/['"]/g, '') : '';
  ...
  measureText(text) {
    ...
    let m = measureCache.get(font + '\0' + faceUrl + '\0' + text);
    if (!m) {
      const r = globalThis.__csim_renderText ? globalThis.__csim_renderText(text, font, true, faceUrl) : null;
      ...
      measureCache.set(font + '\0' + faceUrl + '\0' + text, m);
    }
    return m;
  }
  ...
  _drawText(text, x, y, maxWidth, obj, solid) {
    ...
    const r = globalThis.__csim_renderText(text, this._pangoFont(scale), false, this._fontFaceURL());
    if (!r || !r.refId) return;
  ...
  drawImage(source, ...args) {
    ...
    if (!src) {
      // No decoded pixels. A canvas source with a zero dimension is an
      // InvalidStateError; a blank (undrawn) canvas is fully transparent and draws
      // nothing. A broken / not-yet-loaded / srcless image also draws nothing —
      // drawImage returns silently when the image usability is merely "bad".
      if (isCanvasSource(source)) {
        if (!(source.width | 0) || !(source.height | 0)) {
          throw new globalThis.DOMException('the source canvas has zero size', 'InvalidStateError');
        }
      }
      return;
    }
  ...
  _imagePainter(src, sx, sy, sw, sh, dx, dy, dw, dh) {
    ...
    return (buf, i, px, py, cov) => {
      ...
      if (sa <= 0 && !WHOLE_CANVAS_GCO.has(op)) return;
  ...
  createImageData(arg1, arg2) {
    ...
    if (arg2 === undefined) {   // createImageData(imagedata) — copy its dimensions
