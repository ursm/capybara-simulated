...
export class CanvasPattern {
  ...
  // by this matrix (a–f or m11–m42 alias forms).
  setTransform(t) {
    if (!t) return;
    const v = (k, alias, dflt) => t[k] != null ? +t[k] : t[alias] != null ? +t[alias] : dflt;
    this._m = [v('a', 'm11', 1), v('b', 'm12', 0), v('c', 'm21', 0),
               v('d', 'm22', 1), v('e', 'm41', 0), v('f', 'm42', 0)];
  }
...
export class CanvasRenderingContext2D {
  ...
  _strokeRings(lw, pathObj = this._pathObj, cap = this._lineCap, join = this._lineJoin, miterLimit = this._miterLimit) {
    ...
    for (const sub of pathObj._path) {
      const pts = toU ? sub.pts.map(toU) : sub.pts, n = pts.length;
      if (n < 2) continue;
      const closed = sub.closed, segCount = closed ? n : n - 1;
      // Per-segment unit tangent + half-width left normal (user space); skip
      // zero-length segments so a repeated point doesn't spawn a NaN normal.
      const segs = [];
  ...
  // lineCap / lineJoin geometry) and paint the whole set in one nonzero pass — the
  // union covers overlaps exactly once, so a translucent strokeStyle doesn't darken
  // at corners. (Dashes are stored but not yet honored.)
  stroke(path) {
  ...
  isPointInPath(a, b, c, d) {
    ...
    for (const ring of this._devicePath(path)) {
      const n = ring.length;
  ...
  // isPointInStroke([path,] x, y) — is the (user-space) point within lineWidth/2 of
  // the path? Tested in device space against the device path, with the half-width
  // scaled by the CTM (butt caps / joins / a non-uniform pen are approximated).
  isPointInStroke(a, b, c) {
    let path = this._pathObj, x, y;
    if (a instanceof CanvasPath) { path = a; x = +b; y = +c; } else { x = +a; y = +b; }
    if (!allFinite(x, y)) return false;
    // (x, y) is in canvas coordinate space (transform-independent); test against the
    // DEVICE path with the half-width scaled by the CTM.
    const m = this._m, baked = path._baked;
    const half = ((this.lineWidth > 0 ? this.lineWidth : 1) / 2) * this._ctmScale();
    for (const sub of path._path) {
      const raw = sub.pts, n = raw.length;
      if (n < 2) continue;
      // Default-path points are already device-space; a Path2D's need the CTM.
      const pts = baked ? raw : raw.map(p => [m[0] * p[0] + m[2] * p[1] + m[4], m[1] * p[0] + m[3] * p[1] + m[5]]);
      const segs = sub.closed ? n : n - 1;   // an open subpath has no closing segment
  ...
  _paintImage(src, sx, sy, sw, sh, dx, dy, dw, dh) {
    ...
    if (this._shadowActive()) {
      ...
      this._shadowPass(set => rasterize((px, py, cov) => {
