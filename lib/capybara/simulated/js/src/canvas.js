...
export class ImageBitmap {
  ...
  close() { this._pixels = null; this.width = this.height = 0; this._closed = true; }
...
export function createImageBitmap(source, optionsOrSx, sy, sw, sh, opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const finish = (px, w, h, ownsBuffer, colorSpace) => {
      try {
    ...
    if (source instanceof globalThis.Blob) {
      ...
      return finish(px, decoded.width | 0, decoded.height | 0, true, blobCS);
    }
    ...
    if (source instanceof ImageBitmap) {
      ...
      return finish(source._pixels, source.width, source.height, false);
    }
    ...
    if (isImageSourceType(source)) {
      ...
      return finish(ip.pixels, ip.width, ip.height, false);
    }
    return typeErr();
  });
}
...
export class CanvasRenderingContext2D {
  ...
  createPattern(image, repetition) {
    ...
    if (src) {
      ...
      // change the pattern.
      return new CanvasPattern(new globalThis.Uint8ClampedArray(src.pixels), src.width, src.height, repetition, sourceColorSpace(image));
    }
    if (isCanvasSource(image)) {
      ...
      return new CanvasPattern(new globalThis.Uint8ClampedArray(px), w, h, repetition, sourceColorSpace(image));
    }
  ...
  drawImage(source, ...args) {
    ...
    const srcCS = sourceColorSpace(source), ctxCS = this._attrs.colorSpace;
    if (srcCS !== ctxCS) {
  ...
  getImageData(x, y, w, h, settings) {
    ...
    // requested one on readback (default sRGB). `out` is a fresh copy, so this
    // doesn't touch the backing store.
    const colorSpace = imageDataColorSpace(settings);
