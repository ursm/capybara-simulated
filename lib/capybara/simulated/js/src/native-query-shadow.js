...
// nid -> WeakRef of the JS node, for mapping native results back and testing document-linkage. WEAK so
// a node dropped from the document (its only strong refs were the tree) can be collected — that death
// is what drives slot reclamation (finRegistry below), so a strong Map here would pin every node
// forever and defeat it. Reads deref (a collected-but-not-yet-reclaimed entry reads undefined);
// finRegistry deletes the entry when it reclaims the slot. Wiped per page in invalidateArena.
...
// Reclaim a node's arena slot when its JS wrapper is garbage-collected. registerNativeElement
// registers EVERY element (held value = its `_nid`); when V8 collects the wrapper — which can only
// happen once the node has left the document tree and nothing else holds it — this frees the slot.
// `dropNode` bumps the slot's generation, so any surviving reference (a stale arena edge, a nid still
// held somewhere) reads absent rather than aliasing the reoccupant: the generational arena is what
// makes reclaiming + reusing a slot SAFE. Without it, arena nodes accumulate for a whole no-navigation
// session (bounded only by per-page resetArena). The callback runs only when the isolate's foreground
// message loop is pumped (the browser does so at settle). WeakRef/FinalizationRegistry are standard on
// V8; guarded so an engine lacking them (or QuickJS, where `__dom` is absent) simply never reclaims —
// still correct, just back to the per-page-reset growth bound. A stale nid (its slot already recycled
// by resetArena) is a safe no-op: dropNode's gen check declines it, and the Map delete misses.
const finRegistry = (typeof FinalizationRegistry !== 'undefined')
  ? new FinalizationRegistry((nid) => {
      const d = globalThis.__dom;
      if (d) d.dropNode(nid);
      nidToNode.delete(nid);
    })
  : null;
...
// …and the same for a node that is only ever a BOX: a generated-content pseudo, which is no part of the DOM.
...
export function registerNativeBoxNode(node, localName) {
  const d = globalThis.__dom;
  if (!d) return;
  node._nid = d.importNode(localName, localName, '', false, -1, []);
  if (finRegistry) {
    finRegistry.unregister(node);
    finRegistry.register(node, node._nid, node);
  }
}
...
export function registerNativeElement(el) {
  ...
  // Reclaim this node's slot when the wrapper is collected (see finRegistry). Held value = the nid, and
  // `el` is the unregister token: resetReusedElement re-registers the reused <html>/<head>/<body>
  // skeleton on every navigation, and those elements never die — so without unregistering first, each
  // reload would leave a dead entry in V8's FR table, growth proportional to navigation count (ironic in
  // a change meant to bound growth). unregister(el) drops any prior registration for this wrapper first;
  // it's a no-op on the common first registration.
  if (finRegistry) {
    finRegistry.unregister(el);
    finRegistry.register(el, nid, el);
  }
...
function mirrorCreate(el) {
  ...
  el._nidGen = pageGen;
  nidToNode.set(el._nid, new WeakRef(el));
  const kids = el._children;
...
export function ensureBuilt() {
  ...
  if (built) {
    if (docEl == null || (docEl._nid != null && nidToNode.get(docEl._nid)?.deref() === docEl)) return;
    built = false;
  }
  if (!docEl) return;
  const t0 = now();
  ...
  if (docRootNid < 0 || nidToNode.get(docRootNid)?.deref() !== doc) {
    docRootNid = globalThis.__dom.importNode('#document', '#document', '', false, -1, []);
    nidToNode.set(docRootNid, new WeakRef(doc));
  }
  mirrorSubtree(docEl);
...
export function syncArenaChildList(parent) {
  ...
  if (!built || !parent || parent.nodeType !== NODE_ELEMENT || nidToNode.get(parent._nid)?.deref() !== parent) return;
  const t = now();
  const kids = parent._children;
  if (kids) for (let i = 0; i < kids.length; i++) {
    const c = kids[i];
    if (c.nodeType === NODE_ELEMENT && !(c._nid != null && nidToNode.get(c._nid)?.deref() === c)) mirrorSubtree(c);
  }
...
export function syncArenaCharData(textNode) {
  ...
  if (!parent || parent.nodeType !== NODE_ELEMENT || nidToNode.get(parent._nid)?.deref() !== parent) return;   // document-linked only (see syncArenaChildList)
  const t = now();
  const { nids, hasText } = elementChildrenAndText(parent);
  globalThis.__dom.syncChildren(parent._nid, nids, hasText);
  stats.syncNs += now() - t;
...
export function shadowQuery(root, selector) {
  ...
  try {
    ...
    if (!root || root.nodeType === NODE_DOC) {
      ...
      rootNid = root._nid;
      if (rootNid == null || nidToNode.get(rootNid)?.deref() !== root) { stats.fallbacks++; return cssMatches; }
    }
...
function checkParity(ids, cssMatches, selector) {
  ...
  if (ok) {
    for (let i = 0; i < ids.length; i++) {
      if (nidToNode.get(ids[i])?.deref() !== cssMatches[i]) { ok = false; break; }
    }
  }
  if (!ok) {
    stats.mismatches++;
