...
class ServiceWorkerContainer extends EventTarget {
  ...
  _scheduleLifecycle(reg, worker) {
    ...
    const run = i => {
      ...
      if (worker._state === 'redundant') return;
      ...
      if (!globalThis.__CSIM_NO_PHASE && i === ACTIVATED_STEP && !swPhaseDone.has(worker._handle | 0)) {
...
globalThis.__csim_swPhaseDone = function (handle, _phase) {
  const h = handle | 0;
  swPhaseDone.add(h);
...
function mayActivate(reg, worker) {
  ...
  // The OUTGOING worker's pending work — an unsettled event.waitUntil, a message it
  // hasn't dispatched — holds activation even PAST skipWaiting (activation.https
  // "skipWaiting bypasses no controllee requirement": only the controllee half is
  // bypassed; the in-flight request still holds until it settles).
  if (!globalThis.__CSIM_NO_EXT) { try {
    if (globalThis.__csim_swWorkerExtended && globalThis.__csim_swWorkerExtended(outgoing._handle | 0)) return false;
  } catch (_) {} }
