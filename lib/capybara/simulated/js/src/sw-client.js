...
import { EventTarget, Event, dispatchWithOnHandler, defineEventHandler } from './events.js';
...
class ServiceWorker extends EventTarget {
  ...
  // Client → SW postMessage. Real client↔SW messaging is a later increment; for now
  // the message is dropped rather than throwing (a page that posts before messaging
  // is wired must not crash).
  postMessage() {}
}
