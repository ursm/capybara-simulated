...
class ServiceWorkerContainer extends EventTarget {
  ...
  _updateRegistration(reg, uvcOverride) {
    ...
    // The scope's JOB QUEUE serializes an Update behind a version still coming in:
    // an update() issued while a worker is installing (its install waitUntil
    // unsettled) — or sitting freshly installed on its way to active — waits for
    // that version to settle into active/redundant before the update's fetch runs
    ...
    // that kills the very worker the test is about to finish installing).
    // KNOWN BOUND: a worker parked long-term in `waiting` (outgoing version still
    // controlling clients) parks an update() with it — no vendored test updates
    // during a held waiting phase.
    const pending = reg._installing || reg._waiting;
    if (pending && (pending._state === 'installing' || pending._state === 'installed')) {
