...
class ServiceWorkerContainer extends EventTarget {
  ...
  register(scriptURL, options) {
    ...
    try {
      ...
      if (docOrigin && scriptU.origin !== docOrigin) {
      ...
      if (docOrigin && scopeU.origin !== docOrigin) {
...
globalThis.__csim_swClaimClient = function (handle, hasFetch, scriptURL, scope) {
  ...
  // LONGEST-matching registration is the claiming one. If this client already sees a registration
  // with a strictly longer scope (it's controlled by a more-specific SW), don't steal it — the
  // claiming scope is merely a prefix. A client's own directly-wired controller synthesized such a
  // registration in its realm-local map, so registrationForURL sees it.
  const existing = registrationForURL(href);
  if (existing && existing._active && existing.scope.length > scope.length) return;
