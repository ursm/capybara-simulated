...
// response shape (for `new Response(shape, url)`), or null to fall through to the network.
globalThis.__csimSWInterceptFetch = function (handle, method, url, headers, bodyStr, b64, cb) {
  const id = ++fetchSeq;
  ...
  try {
    ...
    ok = !!globalThis.__csim_serviceWorkerControllerFetch(handle, JSON.stringify({method, url, headers: headers || {}, body_b64}), id);
  } catch (_) {}
