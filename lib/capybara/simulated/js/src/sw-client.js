...
class ServiceWorkerContainer extends EventTarget {
  ...
  register(scriptURL, options) {
    ...
    if (reg) {
      ...
      if (newest && newest._scriptURL === abs && (reg._workerType || 'classic') === workerType) {
      ...
      // same rule _updateRegistration applies): a register() with a DIFFERENT script
      ...
      const pending = reg._installing;
      if (pending && pending._state === 'installing') {
        return new Promise(resolve => {
          const proceed = () => {
            pending.removeEventListener('statechange', proceed);
            resolve(this.register(scriptURL, options));
          };
    ...
    if (!reg) {
      ...
      if (activeHandle > 0) {
        ...
        // the 'classic' default.
        try { reg._workerType = String(globalThis.__csim_swScopeWorkerType(scope) || 'classic'); } catch (_) {}
  ...
  _updateRegistration(reg, uvcOverride) {
    ...
    if (pending && pending._state === 'installing') {
      return new Promise(resolve => {
        const proceed = () => {
          pending.removeEventListener('statechange', proceed);
          resolve(this._updateRegistration(reg, uvcOverride));
        };
        pending.addEventListener('statechange', proceed);
      });
    }
    ...
    reportAsClient();
    let handle = 0;
    try { handle = globalThis.__csim_serviceWorkerRegister(newest._scriptURL, globalThis.__csimBcOriginKey(), reg._scope, reg._workerType || 'classic') | 0; } catch (_) {}
    if (!(handle > 0)) {
  ...
  _scheduleLifecycle(reg, worker) {
    ...
    const steps = [
      ...
      () => {
        const prevWaiting = reg._waiting;
        if (prevWaiting && prevWaiting !== worker) {
          try { prevWaiting._setState('redundant'); } catch (_) {}
          if (!prevWaiting._hostWired) {
            workerByHandle.delete(prevWaiting._handle);
            try { globalThis.__csim_workerTerminate(prevWaiting._handle); } catch (_) {}
          }
        }
      ...
      if (i === INSTALLED_STEP) {
        ...
        if (!ok) {
          ...
          if (!reg._installing && !reg._waiting && !reg._active && registrations.get(reg._scope) === reg) {
            ...
            try { globalThis.__csim_swUnregisterScope(reg._scope); } catch (_) {}
          }
          return;
        }
      }
