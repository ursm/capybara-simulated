...
// the lifecycle surface the executor uses (skipWaiting / clients / self.registration)
// and, critically, does NOT expose blob-URL minting — `URL` in a SW has no
// create/revokeObjectURL (cross-partition.https asserts `'revokeObjectURL' in URL`
// is false in a SW). `addEventListener('install'|'fetch', …)` already works (the
// global is an EventTarget); those events simply never fire in this minimal model.
globalThis.__csim_installServiceWorkerScope = function () {
  try { delete globalThis.URL.createObjectURL; } catch (_) { try { globalThis.URL.createObjectURL = undefined; } catch (__) {} }
  try { delete globalThis.URL.revokeObjectURL; } catch (_) { try { globalThis.URL.revokeObjectURL = undefined; } catch (__) {} }
  ...
  globalThis.ServiceWorkerGlobalScope = globalThis.ServiceWorkerGlobalScope || function ServiceWorkerGlobalScope() {};
};
