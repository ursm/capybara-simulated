...
globalThis.__csim_installServiceWorkerScope = function () {
  ...
  class WindowClient extends Client {
    ...
    navigate(url) {
      ...
      let target;
      // "Parse url with this's associated service worker's script url as base"; a URL that
      // doesn't parse rejects with TypeError, as does about:blank ("if url is about:blank,
      // reject") — a client may not be navigated back to its initial document.
      try { target = new globalThis.URL(String(url), base).href; } catch (_) {
        return Promise.reject(new TypeError('Failed to parse URL: ' + url));
      }
      if (/^about:blank$/i.test(target)) {
        return Promise.reject(new TypeError('about:blank cannot be navigated to'));
      }
  ...
  globalThis.__csim_swClientNavigateResult = function (navId, url, error) {
    ...
    pending.client._url = String(url);
