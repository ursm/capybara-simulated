...
globalThis.__csim_installWorkerScope = function () {
  ...
  globalThis.importScripts = function (...urls) {
    ...
    for (const u of urls) {
      ...
      // A CONTROLLED worker's import routes through its SW's fetch event first —
      ...
      resp = resp || globalThis.__rackFetch('GET', url, '', null, 'follow');
  ...
  // lives in the main VM, unreachable from this thread).
  {
    ...
    class Worker extends EventTarget {
      constructor(url, _options) {
        ...
        this.url     = abs;
        this._handle = globalThis.__csim_workerSpawn(
          this.url, false, globalThis.__csimBcOriginKey(), -(globalThis.__csimWorkerHandle | 0), controllerHandle()) | 0;
