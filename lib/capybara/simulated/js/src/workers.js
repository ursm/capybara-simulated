...
export function decode(s) {
  const NS = globalThis.RustyRacer;
  return unwrapUndefined(JSON.parse(s, function (_key, value) {
    if (!value || typeof value !== 'object') return value;
    const tag = value.__csimType;
    if (tag === 'undefined') return UNDEF_SENTINEL;
...
globalThis.__csim_installServiceWorkerScope = function () {
  ...
  function clientFor(id, url) {
    ...
    // A client that reaches us by message or fetch is by definition one we control.
    if (!c) { c = makeClient(id, url, null, null, true); clientsById.set(String(id), c); }
