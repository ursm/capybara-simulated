...
globalThis.__csim_installServiceWorkerScope = function () {
  ...
  globalThis.__csim_swDispatchFetch = function (reqJson, fetchId, realmId) {
    ...
    // 'race-network-and-fetch-handler': tell the HOST to start the network leg on
    // its own thread, then dispatch the fetch event as usual — first usable
    // response wins (the host prefers the handler over a ≥400 network response).
    // Host-side because the handler may BLOCK this thread (the WPT worker
    // busy-waits 200 ms before respondWith): a network leg run in this isolate
