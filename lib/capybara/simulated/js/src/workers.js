...
export function encode(data, transferSet) {
  ...
  return JSON.stringify(data, function (_key, value) {
    ...
    if (DEC && value instanceof DEC) return { __csimType: 'DOMException', name: value.name, message: value.message };
    if (value instanceof globalThis.Error) return { __csimType: 'Error', name: value.name, message: value.message, kind: value.constructor && value.constructor.name };
...
export function decode(s) {
  ...
  const parsed = JSON.parse(s, function (_key, value) {
    ...
    if (tag === 'DOMException') return new globalThis.DOMException(value.message || '', value.name || 'Error');
    if (tag === 'Error') {
      const Ctor = (value.kind && typeof globalThis[value.kind] === 'function') ? globalThis[value.kind] : globalThis.Error;
      const e = new Ctor(value.message || '');
      if (value.name && value.name !== e.name) { try { e.name = value.name; } catch (_) {} }
      return e;
    }
