...
globalThis.__csim_installServiceWorkerScope = function () {
  ...
  class Client {
    constructor(id, url) { this.id = String(id); this._url = url || ''; }
    get url()       { return this._url; }
    get type()      { return 'window'; }
    get frameType() { return 'top-level'; }
    postMessage(data, transferList) {
      let payload;
      try { payload = encode(data, transferSetFrom(transferList)); } catch (_) { payload = 'null'; }
  ...
  // Serialize a Response the SW handed to respondWith into the raw shape the client's Response
  // ctor accepts (matches __rackFetch's return: status/statusText/headers/body_b64). `bytes` is
  // the already-collected body (via arrayBuffer()) so stream-constructed bodies serialize too.
  function serializeSwResponse(resp, bytes) {
    const headers = {};
  ...
  globalThis.__csim_swDispatchFetch = function (reqJson, fetchId) {
    let req;
    try {
    ...
    const ev = new globalThis.FetchEvent('fetch', {request: req, clientId: 'client-window'});
    dispatchWithOnHandler(globalThis, ev);
    const done = respJson => { try { globalThis.__csim_swFetchRespond(fetchId, respJson); } catch (_) {} };
    ...
    ev._responsePromise
      .then(resp => {
        ...
        return resp.arrayBuffer().then(buf => JSON.stringify(serializeSwResponse(resp, new globalThis.Uint8Array(buf))));
      })
