...
export function encode(data, transferSet) {
  ...
  return JSON.stringify(data, function (_key, value) {
    ...
    // DOMException / Error are structured-clone serializable: carry the name + message (and an
    // Error's kind) so the far isolate rebuilds a real one — a worker posting a rejection
    // (`self.fonts.load('inherit')` → SyntaxError) must arrive as `instanceof DOMException`.
    const DEC = globalThis.DOMException;
    if (DEC && value instanceof DEC) return { __csimType: 'DOMException', name: value.name, message: value.message, stack: value.stack };
    if (value instanceof globalThis.Error) {
      const t = { __csimType: 'Error', name: value.name, message: value.message, stack: value.stack };
      if ('cause' in value) t.cause = value.cause;   // JSON.stringify recurses into it
      return t;
    }
...
export function decode(s) {
  ...
  const parsed = JSON.parse(s, function (_key, value) {
    ...
    if (tag === 'DOMException') {
      const e = new globalThis.DOMException(value.message || '', value.name || 'Error');
      if (typeof value.stack === 'string') { try { e.stack = value.stack; } catch (_) {} }
      return e;
    }
    if (tag === 'Error') {
      // Deserialize by the standard error NAME (as structured clone / cloneInto do), not the
      // sender's constructor — a `.name` set to a standard name becomes that subtype, a
      // non-standard one a plain Error.
      const ERROR_NAMES = ['Error', 'EvalError', 'RangeError', 'ReferenceError', 'SyntaxError', 'TypeError', 'URIError'];
      const name = ERROR_NAMES.indexOf(value.name) !== -1 ? value.name : 'Error';
      const Ctor = globalThis[name] || globalThis.Error;
      const e = Object.prototype.hasOwnProperty.call(value, 'message') ? new Ctor(String(value.message)) : new Ctor();
      if (typeof value.stack === 'string') { try { e.stack = value.stack; } catch (_) {} }
...
if (!globalThis.__csim_isWorker) {
  globalThis.__csim_installWorker = function () {
    ...
    // Resource Timing for a DEDICATED worker's own main-script fetch: a classic worker files an
    // 'other' entry, a module worker a 'script' entry (resource-timing/initiator-type/workers).
    // `worker_spawn` stashed the fetch fact for the script it just fetched (only for a dedicated
    // worker — a shared worker's script is not a document subresource, so it files nothing); file it
    // here so the entry lands in the creating realm's timeline (this constructor's realm), with the
    // type the constructor knows.
    ...
    class Worker extends EventTarget {
      constructor(url, options) {
        ...
        const type          = workerScriptType(options);
        ...
        this._handle        = globalThis.__csim_workerSpawn(
          this.url, false, globalThis.__csimBcOriginKey(), globalThis.__csimRealmId(), controllerHandle(), type) | 0;
        if (this._handle > 0) byHandle.set(this._handle, this);
        recordWorkerRt(this.url, type);
      }
    ...
    class SharedWorker extends EventTarget {
      constructor(url, nameOrOptions) {
        super();
        this.url     = resolveWorkerUrl(url);
        const type   = workerScriptType(nameOrOptions);
        this._handle = globalThis.__csim_workerSpawn(this.url, true, globalThis.__csimBcOriginKey(), globalThis.__csimRealmId(), 0, type) | 0;
        const handle = this._handle;
        // A shared worker's script is not a document subresource — it files no Resource Timing entry
        // (resource-timing/shared-worker-rt-entry), so no recordWorkerRt here (worker_spawn also
        // stashes nothing for a shared worker).
...
globalThis.__csim_installWorkerScope = function () {
  ...
  // `self.fonts` — a WorkerGlobalScope FontFaceSet (CSS Font Loading in a worker). No document,
  // so no CSS-connected faces and nothing to lay out; the surface a worker uses is `add()` /
  // `check()` / `load()` (which parse the font shorthand and reject a css-wide keyword with a
  // SyntaxError) and a face's own `load()` fetch. Installed lazily so the class (platform-
  // globals.js, evaluated earlier in this same bundle) is ready.
  if (typeof globalThis.__csimNewFontFaceSet === 'function' && !globalThis.fonts) {
    try {
      const set = globalThis.__csimNewFontFaceSet(null);
      Object.defineProperty(globalThis, 'fonts', { configurable: true, enumerable: true, get() { return set; } });
    } catch (_) {}
  }
