...
globalThis.__csim_installServiceWorkerScope = function () {
  ...
  // The id of the client whose browsing context holds focus, mirrored from the host
  // (`note_focused_realm`) — a worker isolate can't ask the browser, and the answer is
  // cross-realm, so it's pushed on every change. null before any focus has been committed.
  ...
  class WindowClient extends Client {
    ...
    get focused()         { return focusedClientId === this.id; }
    focus() {
  ...
  function clientFor(id, url) {
    ...
    if (!c) { c = makeClient(id, url); clientsById.set(String(id), c); }
  ...
  globalThis.__csim_swRegisterClient = function (rec) {
    ...
    clientsById.set(String(rec.id), makeClient(rec.id, rec.url, rec.type, rec.frameType));
  };
  ...
  globalThis.__csim_swNoteFocusedClient = function (id) {
    focusedClientId = id == null ? null : String(id);
  };
  class Clients {
    ...
    // Only CONTROLLED clients are mirrored here, which is exactly the includeUncontrolled=false
    // default — `includeUncontrolled: true` would need the uncontrolled set the host doesn't push.
    matchAll(options) {
      const type    = (options && options.type) || 'window';
      const matched = Array.from(clientsById.values()).filter((c) => type === 'all' || c.type === type);
      const focused = matched.findIndex((c) => c.focused);
      if (focused > 0) matched.unshift(matched.splice(focused, 1)[0]);
