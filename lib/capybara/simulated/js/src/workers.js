...
globalThis.__csim_installServiceWorkerScope = function () {
  ...
  globalThis.__csim_swDispatchFetch = function (reqJson, fetchId, realmId) {
    ...
    // client (the document it will load, here the single window client); a subresource is the
    // reverse — its client made it, and it creates none (resultingClientId '').
    const isNav = req && req._mode === 'navigate';
    const ev = new globalThis.FetchEvent('fetch', {
      request:           req,
      clientId:          isNav ? '' : 'client-window',
      resultingClientId: isNav ? 'client-window' : ''
    });
