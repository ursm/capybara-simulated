...
import { updateNativeElementNs } from './native-query-shadow.js';   // fix the eager arena node's ns after XML sets a non-HTML namespace
...
export function installXmlParser({ Element, Text, Comment, ProcessingInstruction, CDATASection, DocumentType, DocumentFragment, finalizeElementProto, syncInlineEventHandler }) {
  ...
  function inScopeNamespaces(el) {
    ...
    // A context element created with createElementNS carries its namespace in
    // `_ns` with no literal xmlns attribute — that namespace is still the
    // in-scope default for fragment children (XML fragment parsing).
    if (scope[''] === undefined && start instanceof Element && start._ns) scope[''] = start._ns;
    return scope;
  }
  ...
  function parseXml(src, opts) {
    ...
    const append = (node) => {
      if (stack.length) {
        // An HTML-namespace `<template>`'s children live in its `content`
        // DocumentFragment, not as direct children (template-element); other
        // elements (and a non-HTML `<template>`) take the node directly.
        const open = stack[stack.length - 1];
        const parent = (open._ns === HTML_NS && open._localName === 'template')
          ? (open._templateContent || (open._templateContent = new DocumentFragment()))
          : open;
        node._parent = parent;
    ...
    while (i < n) {
      if (s[i] === '<') {
        if (s.startsWith('<!--', i)) {
          ...
          const { dt } = makeDoctype(decl);   // makeDoctype only ever errs together with dt=null, covered below
          ...
          // Keep the ctor's eager `_attrs` (attrsView over the arena node, or a plain `{}` on QuickJS) —
          // reassigning a fresh object here would strand that arena node empty. The loop below writes each
          // attribute straight into it.
          for (const a of attrs) {
            el._attrs[a.name] = a.value;
            syncInlineEventHandler(el, a.name, a.value);   // an on* attr (SVG-in-XML) activates its handler
            const ac = a.name.indexOf(':');
            if (a.name === 'xmlns') {
          ...
          // …while a DOCUMENT parse's scripts are parser-inserted, the ones the after-parse pass runs.
          else if (el._tag === 'script') el._csimParserInserted = true;
        ...
        // not part of the DOM; text inside elements is preserved verbatim. (Bare
        // top-level character data isn't flagged: entity-replacement-text and
        // fragment sub-parses legitimately have no enclosing element.)
