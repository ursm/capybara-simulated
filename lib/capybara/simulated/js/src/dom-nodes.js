...
import { liveHTMLCollection, liveNodeList, nodeList, newChildList, liveNamedNodeMap }   from './dom-collections.js';
...
// `this` is the select. `options` is a plain Array, so delegate to its values
// iterator — one subtree walk, snapshotted at iteration start (the only
// consumer, the `sequence<BlobPart>` conversion in `new Blob(select)`, copies
// eagerly, so per-step liveness is never observed and not worth N re-walks).
...
class Element extends Node {
  constructor(tagName) {
    ...
    // methods exist only on HTMLSelectElement). Same shared-prototype rationale
    // as FormNamedProto.
    else if (this._tag === 'select') Object.setPrototypeOf(this, SelectProto);
  }
  _cloneShell() {
  ...
  get options() {
    ...
    arr.namedItem = function (name) { return (name == null || name === '') ? null : (arr.find(o => o._attrs.id === name || o._attrs.name === name) || null); };
    // HTMLOptionsCollection.selectedIndex (select only) reflects the owning
    // <select>'s selectedIndex. Non-enumerable so it doesn't pollute Object.keys
    // / for-in / array copies of the collection.
    if (this._tag === 'select') {
      Object.defineProperty(arr, 'selectedIndex', {
        configurable: true,
        get() { return owner.selectedIndex; },
        set(v) { owner.selectedIndex = v; }
      });
    }
    return arr;
  }
...
// `item(index)` → the option at that index (null out of range); `namedItem(name)`
// → the FIRST option whose id OR name matches, in tree order.
const SelectProto = Object.create(Element.prototype);
Object.defineProperty(SelectProto, 'item', {
  configurable: true, writable: true,
  value(index) { const opts = this.options; return (opts && opts[index >>> 0]) || null; }
});
Object.defineProperty(SelectProto, 'namedItem', {
  configurable: true, writable: true,
  value(name) {
    // The empty string is never a supported property name, so namedItem("")
    // returns null even if an option carries id="" / name="" (HTML named getter
    // / WebIDL named-property semantics — same guard as HTMLCollection.namedItem).
    if (name == null || name === '') return null;
    for (const o of (this.options || [])) {
      if (o._attrs.id === name || o._attrs.name === name) return o;
    }
    return null;
  }
});
...
// name/id feeds its form's named access (FormNamedProto), the window
// named-properties object (WindowNamedProps), and the document
