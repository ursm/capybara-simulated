...
// filters the `<select id=ID>`'s options against the typed value. Setting input.value
// fires a cancelable `beforefilter` on the input; unless prevented, every option whose
// text doesn't start with the value (case-insensitive, trimmed) is marked `_filtered`
// — the `:filtered` pseudo + the UA `option:filtered { display:none }` hide it.
function runComboboxFilter(input) {
  const id = input._attrs.filter;
  if (id == null) return;
  const root = input.getRootNode();
  const select = root && root.getElementById ? root.getElementById(id) : null;
  ...
  for (const opt of listOfOptions(select)) {
...
class Element extends Node {
  ...
  set value(v)   {
    ...
    // A filter combobox re-filters its associated <select> on every value change
    // (programmatic or typed) — the `filter` attribute is the tentative opt-in.
    if (this._tag === 'input' && this._attrs.filter != null) runComboboxFilter(this);
  }
