...
export function constructEntryListAndRecord(form, submitter, fromSubmitMethod) {
  ...
  const entryList = new globalThis.FormData(form, submitter || undefined);
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      // per entry sent as a plain string; everything else is urlencoded.
        ...
        reqBody = entryList || new globalThis.FormData(form, submitter || undefined);
...
function legacyEncodedSubmissionBody(enctype, entryList, spec, form, submitter, encName) {
  ...
  if (enctype.indexOf('multipart/form-data') === 0) {
    const fd = entryList || new globalThis.FormData(form, submitter || undefined);
