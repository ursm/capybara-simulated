...
import { NODE_ELEMENT, NODE_ATTRIBUTE, NODE_TEXT, NODE_COMMENT, NODE_DOC, NODE_DOCTYPE, NODE_FRAGMENT, NODE_PI } from './constants.js';
...
import { walk, walkSubtree, isConnected, scriptText, stripOneLeadingNewline } from './walk.js';
...
class Node {
  ...
  get textContent() {
    ...
    // descendants in tree order. Comments and processing instructions
    // are not Text nodes and contribute nothing.
    let s = '';
    for (const c of this._children) {
...
class Element extends Node {
  ...
  get attributes() {
    ...
    const methods = {
      ...
      item: index => { const a = list[index >>> 0]; return a === undefined ? null : a; },
      getNamedItem: name => {
        const k = attrKey(el, name);
        return Object.prototype.hasOwnProperty.call(el._attrs, k) ? makeAttr(el, k) : null;
      },
      getNamedItemNS: (ns, ln) => el.getAttributeNodeNS(ns, ln),
...
class Document extends Node {
  ...
  createEvent(interfaceName) {
    ...
    // initEvent must throw InvalidStateError. initEvent / initCustomEvent set it.
    ev._initialized = false;
    ev.initEvent = function (type, bubbles, cancelable) {
      if (ev._dispatchFlag) return;   // spec: no-op while mid-dispatch
      ev._initialized = true;
      // initEvent unsets the stop-propagation / stop-immediate / canceled flags.
  ...
  get implementation() {
    return {
      ...
      createDocument: (...args) => {
        ...
        d._children = [];                     // drop the HTML skeleton the Document ctor built
