...
class Element extends Node {
  ...
  setHTMLUnsafe(markup) {
    ...
    // other element holds the parsed nodes as its own children.
...
// frame[set], iframe, img, object, applet). Same shared-prototype trick as
// FormNamedProto, but on the GLOBAL: a single `WindowNamedProps` object spliced
// into globalThis's prototype chain, carrying lazily-registered LIVE getters.
//
// It sits BELOW globalThis, so real globals (`document`, `location`, framework
// vars) — which are OWN properties of globalThis — always win over a same-named
// getter here (own properties beat the prototype), exactly as the spec's
// named-properties object requires, and unresolved bare identifiers are the
// only ones that reach it. The cascade/find hot path reads element fields, not
// globals, so it never touches this chain.
...
const WindowNamedProps    = Object.create(Object.getPrototypeOf(globalThis));
...
function registerWindowName(el, attrName, value) {
  ...
  // Named access must NOT shadow a real global. Own globals (document, …)
  // already win, but globals INHERITED from the original prototype
  // (hasOwnProperty / toString / … and Window members) sit BELOW
  // WindowNamedProps, so a getter here WOULD shadow them. `value in globalThis`
  // — evaluated before this getter is defined — is true for any such existing
  // global; skip it, mirroring registerFormName's `value in FormNamedProto`.
  if (value in globalThis) return;
  Object.defineProperty(globalThis.__csimWindowNamedProps, value, {
    configurable: true, enumerable: false,
    get() { return windowNamedLookup(value); },
...
export function processDeclarativeShadowRoots(root) {
...
class Document extends Node {
    ...
    const accept = (n) => {
      ...
      if (filter) {
        ...
        if (fn) {
          ...
          // (a filter that itself calls nextNode) is an InvalidStateError.
          if (state.active) {
          ...
          state.active = true;
          let r;
          try { r = fn.call(filter || null, n); } finally { state.active = false; }
  ...
  createTreeWalker(root, whatToShow, filter) {
    ...
    const accept = (n) => {
      ...
      // WebIDL callback-interface invocation: a filter object whose `acceptNode`
      // is missing or not callable throws a TypeError when the filter runs.
      ...
      active = true;
      let r;
      try { r = fn.call(filter, n); } finally { active = false; }
      return r & 0xFFFF;   // NodeFilter callback returns `unsigned short` (ToUint16)
    };
