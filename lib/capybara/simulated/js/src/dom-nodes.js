...
import { isContenteditable, toggleChecked, setRadio, checkedRadioInGroup, uncheckOtherRadios, forEachRadioInGroup, getCheckedness, setCheckedness, isSubmitButton, formForControl, closeDialog, labeledControlFor, isLabelableControl, isHtmlLabel, labelToActivateFor, LABELABLE } from './form-helpers.js';
...
class Element extends Node {
  ...
  _cloneShell() {
    ...
    // HTML input/textarea "cloning steps": carry the dirty value-flag snapshot,
    // dirty checkedness, and indeterminateness. NOT the selection — the cloning
    // steps don't copy it, and a fresh clone must start with the default
    // selection (textfieldselection/select-event relies on this).
    if (this._tag === 'input' || this._tag === 'textarea') {
      if (this._defaultValue !== undefined)     e._defaultValue = this._defaultValue;
  ...
  set type(v) {
    ...
    if (this._tag === 'input') {
      // HTML: changing the type re-runs the value-sanitization algorithm on the
      // current value (so e.g. text → number drops a non-numeric value to '').
      if (this._attrs.value != null) {
        this._attrs.value = sanitizeInputValue(this, this.type, this._attrs.value);
      }
  ...
  get value() {
    ...
    // `value` content attribute, or 'on' when it's absent (treated here as
    // empty, since the live value + content attribute share `_attrs.value`).
    if (this._tag === 'input') {
      const t = this.type;
      if (t === 'checkbox' || t === 'radio') {
        return this._attrs.value != null && this._attrs.value !== '' ? this._attrs.value : 'on';
      }
      // The IDL value is the SANITIZED internal value — the content `value`
      // attribute run through the per-type value-sanitization algorithm. (The
      // raw content attribute stays in `_attrs.value` / `getAttribute('value')`;
      // only the IDL `.value` is sanitized.) So a parse-time `value="2013-13"` on
      // a month input, or whitespace on a range, reads back '' / the default just
      // like an IDL set would — the parser writes `_attrs` directly, bypassing
      // the set-time sanitization, so this is where a parsed value gets cleaned.
      return sanitizeInputValue(this, t, this._attrs.value != null ? this._attrs.value : '');
    }
  ...
  set value(v)   {
    ...
    const isTextControl = this._tag === 'input' || this._tag === 'textarea';
    const oldSelVal = isTextControl ? this.value : null;
    ...
    // `input`/`textarea` expose `value` as [LegacyNullToEmptyString] (null → ""),
    // matching browsers; every other element's `value` is a plain DOMString
    // (null → "null", undefined → "undefined").
    this._attrs.value = isTextControl
      ? (v === null ? '' : String(v))
      : String(v);
    // Value sanitization / normalization on set: input runs the per-type
    // algorithm; textarea normalizes newlines (CRLF / CR → LF).
    if (this._tag === 'input') this._attrs.value = sanitizeInputValue(this, this.type, this._attrs.value);
    else if (this._tag === 'textarea') this._attrs.value = this._attrs.value.replace(/\r\n?/g, '\n');
    ...
    if (isTextControl) this._dirtyByUser = false;
    ...
    if (isTextControl && this._attrs.value !== oldSelVal) {
      const vlen = this._attrs.value.length;
      this._selectionStart     = vlen;
      this._selectionEnd       = vlen;
      this._selectionDirection = 'none';
    }
  }
  ...
  // (`checked` / `value`). The live checkedness lives in `_checkedness`
  // (the dirty-checkedness state, separate from this attribute view); the
  // dirty-value snapshot is still `_defaultValue` until the value half of
  // the live/attribute split lands.
  ...
  // `<textarea>.defaultValue` reflects the element's CHILD TEXT CONTENT (its
  // raw default), not a `value` content attribute; `<input>.defaultValue`
  // reflects the `value` content attribute. Once the live value is dirtied
  // (`set value`), `_defaultValue` holds the pre-dirty snapshot for both.
  get defaultValue()  {
    if (this._defaultValue !== undefined) return this._defaultValue;
    return this._tag === 'textarea' ? this.textContent : (this.getAttribute('value') || '');
  }
  set defaultValue(v) {
    ...
    if (this._defaultValue !== undefined) { this._defaultValue = s; return; }
    // When clean, write the backing default so the (clean) live value tracks
    // it — child text for textarea, the `value` attribute for input.
  ...
  setRangeText(replacement, start, end, selectMode) {
    ...
    // setRangeText is a value mutation: snapshot the default (dirty-value flag)
    // once before the first write so `defaultValue` keeps reporting the parsed
    // default and `<form>.reset()` restores it — mirrors `set value`. For a
    // textarea the pre-edit default is its current child text.
    if (this._defaultValue === undefined) {
      this._defaultValue = (this._tag === 'textarea') ? this.textContent : (this._attrs.value || '');
    }
