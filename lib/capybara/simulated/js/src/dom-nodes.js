...
function listOfOptions(select) {
  ...
  const walk = (node, inOptgroup) => {
    for (const c of node._children) {
      ...
      const t = c._tag;
      if (t === 'option') { out.push(c); continue; }        // never recurse into an option
      if (inOptgroup) continue;                              // inside an optgroup: direct options only
      if (t === 'optgroup') walk(c, true);                  // a top-level optgroup's direct options
      else if (t === 'hr' || t === 'datalist' || t === 'select') continue;   // barriers
      else walk(c, false);                                  // transparent wrapper (e.g. <div>)
    }
  };
  walk(select, false);
  return out;
...
class Element extends Node {
  ...
  add(element, before) {
    ...
    if (typeof before === 'number') {
      reference = this.querySelectorAll('option')[before] || null;
    } else if (before != null) {
  ...
  get selectedOptions() {
    ...
    if (!this._selectedOptionsColl) {
      ...
      this._selectedOptionsColl = liveHTMLCollection(() =>
        selectAll(self._children, 'option', self).filter(o => o._selectedness === true));
    }
