...
class Element extends Node {
  ...
  set value(v)   {
    ...
    if (this._tag === 'select') {
      ...
      for (const o of opts) {
        ...
        if (!matched && o.value === target) { matched = true; o._selectedness = true; o._dirtySel = true; }
  ...
  set selectedIndex(v) {
    ...
    for (let i = 0; i < opts.length; i++) {
      ...
      if (i === idx) { opts[i]._selectedness = true; opts[i]._dirtySel = true; }
  ...
  setCustomValidity(msg)  { this._validationMessage = String(msg || '').replace(/\r\n?/g, '\n'); }
