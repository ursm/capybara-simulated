...
import { ceState, getCustomElementCtor, ceUpgradeTree, fireCEDisconnect, fireCEMoveReactions, fireAttrChangedCallback, askForReset } from './custom-elements.js';
...
class Element extends Node {
  ...
  _setAttrNodeValue(key, value) {
    ...
    recordAttrMutation(this, key, old == null ? null : old);
    if (old !== value) fireAttrChangedCallback(this, key, old == null ? null : old, value, Element);
  }
...
export function rangeIntersectsNode(range, node) {
  ...
  // Spec: different root → no intersection.
...
// serializer passes all of them. We follow what Chrome / Firefox actually do
// (CLAUDE.md rule 1: "spec-correct means what real browsers do"), verified
// directly against Chrome. The residual allowlisted (wpt_expected_failures.yml)
// failures are subtests REAL BROWSERS ALSO FAIL, encoding abandoned revisions:
//   - "Drop inconsistent xmlns by matching on local name" — Chrome keeps the
//     literal xmlns attrs (byte-identical to our output).
//   - "...prefix of an attribute is NOT preserved..." (issue #29) — Chrome
//     preserves the author prefix; only the old revision generated `ns1`.
//   - "...prefix bound to an empty namespace URI..." — Chrome keeps `xmlns=""`,
//     but that directly conflicts with the passing "redundant xmlns is dropped"
//     subtest; no serializer satisfies both.
//
