...
class Element extends Node {
  ...
  set src(v) {
    ...
    // A `<video>` src assignment kicks its media load; setAttribute has no media hook.
  ...
  _imageResourceSrc() {
    if (this._tag === 'img') return this._attrs.src;
    if (this._tag === 'image') {
      const href = this._attrs.href;
  ...
  _isImageResourceAttr(name) {
    if (this._tag === 'img')   return name === 'src' || name === 'crossorigin';
  ...
  _loadImageResource() {
    ...
    const ctrl = /^https?:/i.test(src) &&
      globalThis.__csimSWControllerHandle && globalThis.__csimSWControllerHandle();
    if (ctrl) { this._loadImageViaServiceWorker(ctrl, src); return; }
    this._finishImageLoadFromNetwork(src);
  }
