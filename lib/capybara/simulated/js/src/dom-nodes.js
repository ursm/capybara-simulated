...
globalThis.__csimSubmitFormToNamedFrame = function (form, submitter, entryList, fromSubmitMethod) {
  ...
  // CHEAP target pre-check BEFORE the full form serialize (rule 3): only a named,
  // same-document frame target is handled here; ordinary top/self submits bail
  // immediately without the O(N) document walk __csimFormSerialize does.
  ...
  // handle is registered in the lookup table, so `__csimFormSerialize(handle)`
  ...
  try { spec = globalThis.__csimFormSerialize(form, submitter || 0); } catch (_) { return false; }
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      // empty (`action` → `action?`).
      const qs = (entryList ? entryListParams(entryList) : formEntryParams(spec)).toString();
      ...
      if (outputEnc !== 'UTF-8' && isSingleByteEncoding(outputEnc)) {
        const { bytes, contentType } = legacyEncodedSubmissionBody(enctype, entryList, spec, form, submitter, outputEnc);
        reqBody = new globalThis.Blob([Uint8Array.from(bytes, (c) => c.charCodeAt(0) & 0xFF)], { type: contentType });
      // Reuse the already-built entry list when the caller constructed one (so
      // `formdata` fired once and handler mutations apply); otherwise build it now.
      ...
      // per entry sent as a plain string; everything else is urlencoded.
        ...
        reqBody = entryList || new globalThis.FormData(form, submitter || undefined, true);
      } else if (enctype === 'text/plain') {
        reqBody = entryList ? entryListTextPlain(entryList) : formEntryTextPlain(spec);
      } else {
        reqBody = entryList ? entryListParams(entryList) : formEntryParams(spec);
      }
...
function formEntryParams(spec) {
  const params = new globalThis.URLSearchParams();
  for (const pair of (spec.fields || [])) params.append(normalizeNL(pair[0]), normalizeNL(pair[1]));
  for (const fi of (spec.fileInputs || [])) {
    const f = fi.files && fi.files[0];
    params.append(normalizeNL(fi.name), f ? entryFileName(f) : '');
  }
  return params;
}

// An already-constructed entry list (a FormData) as URLSearchParams: each File
// value contributes its filename only, matching urlencoded/GET submission. Used
// when `__runFormSubmit` threads its post-`formdata` entry list in.
...
function formEntryTextPlain(spec) {
  let out = '';
  for (const pair of (spec.fields || [])) out += plainEntry(pair[0], pair[1]);
  for (const fi of (spec.fileInputs || [])) {
    const f = fi.files && fi.files[0];
    out += plainEntry(fi.name, f ? entryFileName(f) : '');
  }
  return out;
}
...
// The submission entry list as `[name, stringOrFile]` pairs: the already-built
// `entryList` (a FormData, so `formdata`-event mutations are included) when the
// caller threaded one, else the freshly-serialized form spec.
function submissionEntries(entryList, spec) {
  const File = globalThis.File;
  const out = [];
  if (entryList) {
    for (const [name, value] of entryList) out.push([name, value]);
  } else {
    for (const pair of (spec.fields || [])) out.push([pair[0], pair[1]]);
    for (const fi of (spec.fileInputs || [])) out.push([fi.name, (fi.files && fi.files[0]) || '']);
  }
  return out;
}
...
function legacyEncodedSubmissionBody(enctype, entryList, spec, form, submitter, encName) {
  const File = globalThis.File;
  const enc = (s) => legacyFormEncode(s, encName);
  if (enctype.indexOf('multipart/form-data') === 0) {
    const fd = entryList || new globalThis.FormData(form, submitter || undefined, true);
    const { body, boundary } = serializeMultipart(fd, enc);
    return { bytes: body, contentType: 'multipart/form-data; boundary=' + boundary };
  }
  const entries = submissionEntries(entryList, spec);
...
class Element extends Node {
  ...
  setAttribute(name, value) {
    ...
    // Form-associated custom element "reset the form owner": the control's own
    // `form` attribute change re-points its owner; a <form>'s `id` change re-points
    // every element referencing it via `form="…"`. Both enqueue formAssociatedCallback
    // on an actual owner change (see resetFormAssociatedOwnersForAttr).
    if (old !== next && (n === 'form' || n === 'id')) resetFormAssociatedOwnersForAttr(this, n);
  ...
  _removeAttrKey(n) {
    ...
    // Form-associated custom element "reset the form owner" — symmetric with
    // setAttribute: removing a control's `form` attribute (falls back to the
    // ancestor form), or a <form>'s `id` (drops every `form="…"` reference).
    if (n === 'form' || n === 'id') resetFormAssociatedOwnersForAttr(this, n);
...
// HTML "reset the form owner" reaction for a form-associated custom element. Injected
// into custom-elements.js (via setFormOwnerReset) so its upgrade / connect / disconnect
// seams can drive it, and called directly from the attribute-change seams below. It (a)
...
setFormOwnerReset(resetCustomElementFormOwner);

// A form-associated custom element's owner can also change through an attribute
// mutation: its own `form` attribute re-points it directly, and a <form>'s `id`
// re-points every element that references (or referenced) that id via `form="…"`.
// The `id`-on-<form> case fans out across the document's form-associated elements —
// rare, and gated on any form-associated element existing at all (rule 3).
...
const WINDOW_REFLECTED_HANDLER_ATTRS = ['onstorage'];
...
class Document extends Node {
  ...
  _createElement(ns, prefix, localName) {
    ...
    const ctor = (ns === HTML_NS && this === globalThis.document) ? getCustomElementCtor(localName.toLowerCase()) : null;
    if (ctor) {
