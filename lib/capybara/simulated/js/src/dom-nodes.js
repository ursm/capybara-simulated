...
class Node {
  ...
  // the root element in standards mode, the BODY in quirks mode — whose `scrollTop` is the viewport's there too
  // (`scrollOffsetHolder`), so `scrollHeight - clientHeight - scrollTop` stays one question. The root of a quirks
  // document is an ordinary box.
  _isViewportElement() {
    const doc = this.ownerDocument;
    return !!doc && this === (doc._quirks ? doc.body : doc.documentElement);
  }
