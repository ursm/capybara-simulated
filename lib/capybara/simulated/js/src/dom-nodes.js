...
// HTML "rules for parsing (non-negative) integers": skip ASCII whitespace
// [ \t\n\f\r] (NOT other Unicode spaces / vertical tab), optional sign, require
// >=1 ASCII digit, then read the digit run. Returns a number, or null on failure.
const REFLECT_MAX_INT = 2147483647;    // 2^31 - 1
const REFLECT_MIN_INT = -2147483648;   // -2^31
function parseHtmlInteger(s) {
  const str = String(s);
  const n = str.length;
  let i = 0;
