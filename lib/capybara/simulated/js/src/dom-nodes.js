...
// on any node). Without a layout engine we can't model scroll
// position, but the SIGNAL is what DLoadMore-shaped sentinels (and a
// few scroll-driven UI swaps) gate on. Fire a `scroll` event on
// `document` and force-refire any observed IntersectionObserver
// targets so a paginated list advances past its first page.
function __notifyScroll() {
  try {
    const doc = globalThis.document;
    if (doc) dispatchEvent(doc, new Event('scroll', { bubbles: false }));
  } catch (_) {}
  try {
...
class Node {
  ...
  // DLoadMore-style pagination sentinels gate on. Fire a `scroll`
  // event on the document + force-refire any observed IO targets so
  // sentinel-based loadMore patterns advance past the first page.
  scrollIntoView(_opts) { __notifyScroll(); }
  scrollIntoViewIfNeeded(_opts) { __notifyScroll(); }
  scrollTo()  { __notifyScroll(); }
  scrollBy()  { __notifyScroll(); }
