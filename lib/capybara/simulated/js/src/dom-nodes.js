...
class Element extends Node {
  constructor(tagName) {
    ...
    // sets `text-align` (here), and the `align` content attribute (`recordAttrMutation`, and the
    // parser's attribute loop). A per-block ancestor walk would be the alternative (rule 3).
    ...
    if (/^https?:/i.test(src) && typeof globalThis.__csim_imageLoadStart === 'function') {
      ...
      if (id > 0) {
        ...
        (globalThis.__csimPendingImages || (globalThis.__csimPendingImages = new Map()))
          .set(id, { el: this, gen: this._imgLoadGen | 0 });
        return;
      }
    }
    let r = null;
