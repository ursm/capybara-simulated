...
class Element extends Node {
  ...
  get options() {
    if (this._tag === 'select') {
      return this._optionsColl ||
        (this._optionsColl = liveOptionsCollection(this, () => selectAll(this._children, 'option', this)));
    }
  ...
  get form() {
    ...
    // HTMLOptionElement.form: the form owner of the option's <select> (option is
    // NOT itself form-associated), else null. The select is the option's parent,
    // or the parent <optgroup>'s parent.
    if (this._tag === 'option') {
      const p = this._parent;
      let sel = null;
      if (p && p._tag === 'select') sel = p;
      else if (p && p._tag === 'optgroup' && p._parent && p._parent._tag === 'select') sel = p._parent;
