...
class Element extends Node {
  ...
  set value(v)   {
    ...
    if (this._tag === 'meter') { this.setAttribute('value', String(Number(v))); return; }
    ...
    if (this._tag === 'progress') { this.setAttribute('value', String(Number(v))); return; }
    if (this._tag === 'input' && (this._attrs.type || '').toLowerCase() === 'file') {
  ...
  set min(v)        { if (this._tag === 'meter') { this.setAttribute('min', String(Number(v))); return; } this.setAttribute('min', String(v)); }
  ...
  set max(v) {
    ...
    if (this._tag === 'meter') { this.setAttribute('max', String(Number(v))); return; }
      ...
      if (!empty && tag === 'input' && STEP_SCALE[type] != null) {
        ...
        if (!Number.isNaN(num)) {
          ...
          if (!(stepAttr != null && String(stepAttr).toLowerCase() === 'any')) {
            ...
            if (stepAttr != null && isValidFloatingPoint(String(stepAttr)) && Number(stepAttr) > 0) stepN = Number(stepAttr);
            const stepValue = stepN * STEP_SCALE[type];
            const base = !Number.isNaN(minNum) ? minNum : defaultStepBase(type);
            if (stepValue > 0) {
  ...
  set low(v) { if (this._tag === 'meter') this.setAttribute('low', String(Number(v))); }
  ...
  set high(v) { if (this._tag === 'meter') this.setAttribute('high', String(Number(v))); }
  ...
  set optimum(v) { if (this._tag === 'meter') this.setAttribute('optimum', String(Number(v))); }
