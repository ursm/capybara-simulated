...
import { ceState, getCustomElementCtor, ceCtorInRegistry, customElementLocalName, isValidCustomElementName, isFormAssociatedCustomElement, hasFormAssociatedCustomElements, setFormAssociatedReset, ceUpgradeTree, fireCEDisconnect, fireCEMoveReactions, fireCEAdopted, fireAttrChangedCallback, askForReset, askForResetBatch, askForResetAfterRemoval, runSelectednessAlgorithm, finalizeSelectOptions, ensureOptionSelInit, selectDisplaySize, updateSelectedContent, scheduleSelectedContentUpdate } from './custom-elements.js';
...
class Element extends Node {
  constructor(tagName) {
    ...
    if (!tag && new.target) tag = customElementLocalName(new.target);
    ...
    // parser builds elements this way, not via createElement). `new.target ===
    // Element` excludes custom-element subclasses (their own prototype is kept).
    // A non-HTML element (createElementNS / parser foreign content) is reverted to
    // Element.prototype where its namespace is stamped. The registry is empty until
    // installDomClassAliases runs (before any element), so this no-ops at bootstrap.
    else if (new.target === Element && this._tag !== 'form' && this._tag) {
  ...
  set value(v)   {
    ...
    if (usesLiveValue) {
      ...
      // / …) and other elements (<option>): `value` reflects the content
      // attribute. Plain DOMString (null → "null", undefined → "undefined").
  ...
  set src(v) {
    ...
    if (this._tag === 'img') this._loadImageResource();
  }
  ...
  get sizes() {
    ...
    if (this._localName === 'source') return this._attrs.sizes == null ? '' : String(this._attrs.sizes);
    return undefined;
  }
  set sizes(v) {
    if (this._ns !== HTML_NS) return;
    if (this._localName === 'link')   { tokenListFor(this, 'sizes').value = String(v); return; }
    if (this._localName === 'source') this.setAttribute('sizes', String(v));
  }
...
class Document extends Node {
  ...
  createElement(tag, options) {
    ...
    const el = this._createElement(ns, null, localName, ceRegistryOption(options));
  ...
  createElementNS(namespace, qualifiedName, options) {
    ...
    const el = this._createElement(rns, prefix, localName, ceRegistryOption(options));
    if (options && options.is != null) el._isValue = String(options.is);
    return el;
  }
  ...
  _createElement(ns, prefix, localName, registry) {
    let el;
    ...
    // Default (unset) → the global registry, via the unchanged getCustomElementCtor;
    // an explicit scoped registry → its own definitions; explicit null → no definition.
    const ctor = !inBrowsingContext ? null
               : !explicit          ? getCustomElementCtor(localName.toLowerCase())
               : registry           ? ceCtorInRegistry(registry, localName.toLowerCase())
               : null;
    if (ctor) {
