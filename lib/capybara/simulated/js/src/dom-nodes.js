...
import { rectOf, clientBoxOf, documentBoxOf, viewportSize, contentExtent, isPositionedElement, scrollTargetFor, clampScrollOffset, hitTest, bumpScrollEpoch } from './layout.js';
...
class Node {
  ...
  getClientRects() {
    const r = rectOf(this);
    // A non-rendered element has no boxes; a rendered zero-height one (an empty block) still has
    // one, which is what keeps `:visible`-style probes answering true for it.
    return (r.width === 0 && r.height === 0) ? [] : [new globalThis.DOMRect(r.x, r.y, r.width, r.height)];
  }
