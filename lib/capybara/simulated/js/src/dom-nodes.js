...
function findSlottables(slot) {
  ...
  if (!sr || sr.slotAssignment === 'manual') return [];
...
// lazy. Manual shadow roots auto-assign nothing (slot.assign(), inc2).
function assignSlottablesForShadowRoot(sr) {
  if (sr.slotAssignment === 'manual') return;
  const slots = [];
...
// serializer passes all of them (real browsers fail a different subset than this
// literal-spec algorithm). This implements the literal current algorithm, which
// passes the maximal self-consistent subset; the residual conflicts are
// allowlisted (see wpt_expected_failures.yml).
//
