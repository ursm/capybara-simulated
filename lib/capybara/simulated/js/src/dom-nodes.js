...
import { rectOf, clientRectsOf, clientBoxOf, documentBoxOf, viewportSize, contentExtent, isPositionedElement, scrollTargetFor, clampScrollOffset, hitTest, bumpScrollEpoch } from './layout.js';
...
// from timers.js runRenderPhase) dispatches it. Deferral is load-bearing: a
// scroller removed before the render step (e.g. by its own scroll handler) has
// its pending scrollend DROPPED (isConnected check), matching real browsers.
...
function __notifyScrollEnd(target) {
  pendingScrollEnd.add(target || globalThis.document);
}
globalThis.__csimFlushScrollEnd = function () {
  ...
  for (const target of targets) {
    try {
      ...
      const isRoot = !!doc && (target === doc || target === doc.documentElement ||
        (doc.scrollingElement && target === doc.scrollingElement));
      if (isRoot) {
      ...
      } else if (isConnected(target)) {
        dispatchEvent(target, new Event('scrollend', { bubbles: false, cancelable: false }));
      }
      // else: target disconnected before the render update → pending scrollend dropped.
    } catch (_) {}
  }
};
...
class Node {
  ...
  scrollIntoView(_opts) {
    ...
    if (root) {
      ...
      if (to) { to.el._scrollLeft = to.x; to.el._scrollTop = to.y; bumpScrollEpoch(); }
    }
    ...
    __notifyScroll(this);
    // It scrolls the ROOT scroller (root._scrollTop above), so scrollend — when
    // the position actually changed — fires on the viewport, not on `this`.
    if (moved) __notifyScrollEnd(root);
  }
  ...
  scrollTo() {
    ...
    if (typeof x === 'number') {
      ...
      if (nx !== (this._scrollLeft || 0)) { this._scrollLeft = nx; changed = true; bumpScrollEpoch(); }
    }
    if (typeof y === 'number') {
      const ny = clampDocScroll(this, 'y', y);
      if (ny !== (this._scrollTop || 0)) { this._scrollTop = ny; changed = true; bumpScrollEpoch(); }
    }
    if (changed) { __notifyScroll(this); __notifyScrollEnd(this); }
  }
  scrollBy() {
...
function applyInputTypeChange(el, prevMode, wasSelectable) {
  ...
  if (prevMode === 'value' && (newMode === 'default' || newMode === 'default/on')) {
    ...
    delete el._value;   // value tracks the content attribute; dirty flag cleared
  } else if (prevMode !== 'filename' && newMode === 'filename') {
    delete el._value;   // value → "" (file value derives from `_files`)
  }
  ...
  if (el._value !== undefined) el._value = sanitizeInputValue(el, el.type, el._value);
...
class Element extends Node {
  ...
  set value(v)   {
    ...
    if (usesLiveValue) {
      ...
      const valueChanged = this._value !== sv;
      this._value = sv;
      // The LIVE value is a cascade input for `:placeholder-shown`, `:valid` / `:invalid` and their
      // `:user-` variants, and it is stored here WITHOUT touching the `value` content attribute —
      // so no attribute mutation, and no other generation, moves for it.
      if (valueChanged) bumpStyleState();
...
class Document extends Node {
  ...
  execCommand(command, _showUI, value) {
    ...
    if (cmd === 'inserthtml' &&
      ...
      active._value = cur.slice(0, ss) + str + cur.slice(se);
...
function spliceTextControlValue(el, insert) {
  ...
  el._value = cur.slice(0, ss) + insert + cur.slice(se);
