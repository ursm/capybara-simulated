...
function isFormNamedCandidate(form, el) {
  ...
  if (!FORM_LISTED_TAGS.has(t)) return false;
...
function formControlElements(form) {
  ...
  walkSubtree(root, el => {
    ...
    const t = el._tag;
    if (!FORM_LISTED_TAGS.has(t)) return;
...
function fieldsetControlElements(fieldset) {
  ...
  walkSubtree(fieldset, el => {
    ...
    if (FORM_LISTED_TAGS.has(el._tag)) out.push(el);
  });
  return out;
}
...
function registerFormName(el, value) {
  if (value && FORM_GATE_TAGS.has(el._tag)) FORM_NAMED_VALUES.add(value);
}
...
setScopeFormOwnerResolver((node) => {
  let associated = SCOPE_FORM_ASSOCIATED.has(node._tag);
  if (!associated && node._ns === HTML_NS) {
    const ctor = getCustomElementCtor(node._localName);
    associated = !!(ctor && ctor.formAssociated === true);
  }
