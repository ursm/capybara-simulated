...
import { isVisibleNode, isLaidOutNode, visibilityHidden, INVISIBLE_TAGS, selfHidden, cascadedProperty, scheduleCascadeRefresh, bumpCascadeVersion, bumpCtxAll } from './cascade.js';
...
class Node {
  ...
  // `visibility: hidden` element. (What it does not model yet: `content-visibility: hidden` and a closed `<details>`
  // skip their contents, which Chrome reports false.)
  checkVisibility(opts) {
    if (!isLaidOutNode(this) || !globalThis.__csimGeneratesBox(this)) return false;
  ...
  get offsetWidth()  { return this._tag === 'br' ? 0 : Math.round(rectOf(this).width); }
  get offsetHeight() { return this._tag === 'br' ? 0 : Math.round(rectOf(this).height); }
  ...
  _offsetFrom(axis) {
    // A `<br>` has no CSS box — it is a line break whatever it declares (layout.js `displayOf`) — and CSSOM View
    // answers 0 for every offset of an element without one. Chrome: all four 0, `offsetParent` still the container.
    if (this._tag === 'br') return 0;
...
function reassignSlot(slot, assigned) {
  ...
  for (const n of was) if (!assigned.includes(n)) markMovedSubtree(n);
