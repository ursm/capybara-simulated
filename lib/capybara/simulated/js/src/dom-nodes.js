...
import { recordAttrMutation, recordChildList, recordCharacterData, bumpSettleGen, bumpStyleState, FOCUS_KINDS, HOVER_KINDS, currentSettleGen, signalSlotChange, setSlotChangeFirer, setSlotMutationHooks, markLayoutDirty } from './mutation-observer.js';
...
import { ensureInView, rectOf, clientRectsOf, clientDims, clientBorderTopLeft, documentBoxOf, paddingBoxOriginOf, viewportSize, contentExtent, isPositionedElement, isFixedElement, applyScrollIntoView, clampScrollOffset, hitTest, bumpScrollEpoch } from './layout.js';
...
class Node {
  ...
  get clientWidth()  { return this._isRootElement() ? viewportSize().width  : Math.round(clientDims(this).width); }
  get clientHeight() { return this._isRootElement() ? viewportSize().height : Math.round(clientDims(this).height); }
  ...
  get scrollWidth()  { return contentExtent(this).width; }
  get scrollHeight() { return contentExtent(this).height; }
  _isRootElement() {
    const doc = this.ownerDocument;
    return !!doc && doc.documentElement === this;
  }
  ...
  set scrollTop(v) {
    if (!holdsScrollOffset(this)) return;      // not a scroll container: the write is a no-op
    // Clamped for the DOCUMENT scroller, as a browser clamps everywhere: `scrollTo(0,
    // scrollHeight)` — the everyday "scroll to the bottom" — lands at `scrollHeight -
    // clientHeight` and not past it, which is what puts a list's sentinel INSIDE the viewport
    // where an IntersectionObserver can see it. An ELEMENT scroller is still unclamped: its
    // content extent is the coarse one, and clamping against an under-measured extent turns a
    // real scroll into one that silently goes nowhere (measured: a net LOSS across the scrolling
    // WPT cluster). The page's own height is exact, which is why the document can have it.
    const next = clampDocScroll(this, 'y', Number(v) || 0);
    if (next === (this._scrollTop || 0)) return;
    this._scrollTop = next;
    // Queued BEFORE the epoch bump: both are delivered by a microtask on a page with nothing else
    // pending, and the rendering update runs the scroll steps before the intersection observations
    // — so the scroll flush has to be the microtask that was queued first.
    __notifyScroll(this);
    __notifyScrollEnd(this);
    bumpScrollEpoch();
  }
  get scrollLeft()   { return this._scrollLeft || 0; }
  set scrollLeft(v) {
    if (!holdsScrollOffset(this)) return;                     // see scrollTop
    const next = clampDocScroll(this, 'x', Number(v) || 0);   // see scrollTop
  ...
  // …and the same clamp the setters apply (see `scrollTop`): `window.scrollTo(0,
  ...
  scrollTo() {
    ...
    let changed = false;
    if (typeof x === 'number') {
      const nx = clampDocScroll(this, 'x', x);
      if (nx !== (this._scrollLeft || 0)) { this._scrollLeft = nx; changed = true; }
    }
    if (typeof y === 'number') {
      const ny = clampDocScroll(this, 'y', y);
      if (ny !== (this._scrollTop || 0)) { this._scrollTop = ny; changed = true; }
    }
    // …then the events, and the epoch LAST — see the note in the `scrollTop` setter.
    if (changed) { __notifyScroll(this); __notifyScrollEnd(this); bumpScrollEpoch(); }
  }
  scrollBy() {
    if (!holdsScrollOffset(this)) return;      // see the scrollTop setter
    ...
    if (typeof dx === 'number' && dx !== 0) {
      const nx = clampDocScroll(this, 'x', (this._scrollLeft || 0) + dx);
      if (nx !== (this._scrollLeft || 0)) { this._scrollLeft = nx; changed = true; }
    }
    if (typeof dy === 'number' && dy !== 0) {
      const ny = clampDocScroll(this, 'y', (this._scrollTop || 0) + dy);
      if (ny !== (this._scrollTop || 0)) { this._scrollTop = ny; changed = true; }
    }
    if (changed) { __notifyScroll(this); __notifyScrollEnd(this); bumpScrollEpoch(); }
  }
...
class Element extends Node {
  constructor(tagName) {
    ...
    if (this._tag === 'bdi') globalThis.__csimFlowHint = true;
...
// which is what Chrome reports for a click landing inside a web component. A point outside the viewport has none, and
// neither has one where only the ROOT element is hit when a shadow root asks (Chrome: `null` below the content, where
// the document answers `<html>`).
function elementFromPointIn(scope, x, y) {
  const hit = scopedHit(scope, x, y);
  return hit ? retarget(hit, scope) : null;
}
function scopedHit(scope, x, y) {
  if (x < 0 || y < 0) return null;
  const hit = hitTest(x, y);
  return hit && scope._isShadowRoot && hit === hit.ownerDocument.documentElement ? null : hit;
}
// …and `elementsFromPoint`: the hit and every element it sits in, innermost first, up through the hosts of the shadow
// trees on the way — each retargeted, once.
function elementsFromPointIn(scope, x, y) {
  const out = [];
  for (let cur = scopedHit(scope, x, y); cur; cur = cur._isShadowRoot ? cur.host : cur._parent) {
    if (cur.nodeType !== NODE_ELEMENT) continue;
    const el = retarget(cur, scope);
...
class Document extends Node {
  ...
  get scrollingElement() {
    if (this._quirks) return this.body || null;
