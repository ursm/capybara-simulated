...
function reflectUnsignedLongSet(el, attr, v) { el.setAttribute(attr, String(v >>> 0)); }
...
// value throws IndexSizeError; otherwise store the ToUint32 value.
...
class Element extends Node {
  ...
  get formAction()  { return FORMACTION_REFLECTING_TAGS.has(this._tag) ? reflectURLAttr(this, 'formaction', FORMACTION_REFLECTING_TAGS) : ''; }
  ...
  set size(v) {
    if (this._tag === 'input')  { reflectLimitedUnsignedLongSet(this, 'size', v); return; }
    if (this._tag === 'select') { reflectUnsignedLongSet(this, 'size', v); return; }
  ...
  set colSpan(v) { if (this._tag === 'td' || this._tag === 'th') reflectUnsignedLongSet(this, 'colspan', v); }
  get rowSpan()  { return (this._tag === 'td' || this._tag === 'th') ? reflectClampedUnsignedLongGet(this._attrs.rowspan, 1, 0, 65534) : undefined; }
  set rowSpan(v) { if (this._tag === 'td' || this._tag === 'th') reflectUnsignedLongSet(this, 'rowspan', v); }
  get span()  { return (this._tag === 'col' || this._tag === 'colgroup') ? reflectClampedUnsignedLongGet(this._attrs.span, 1, 1, 1000) : undefined; }
  set span(v) { if (this._tag === 'col' || this._tag === 'colgroup') reflectUnsignedLongSet(this, 'span', v); }
  ...
  set hspace(v) { if (HVSPACE_TAGS.has(this._tag)) reflectUnsignedLongSet(this, 'hspace', v); }
  get vspace()  { return HVSPACE_TAGS.has(this._tag) ? reflectUnsignedLongGet(this._attrs.vspace, 0) : undefined; }
  set vspace(v) { if (HVSPACE_TAGS.has(this._tag)) reflectUnsignedLongSet(this, 'vspace', v); }
  ...
  set scrollAmount(v) { if (this._tag === 'marquee') reflectUnsignedLongSet(this, 'scrollamount', v); }
  get scrollDelay()  { return this._tag === 'marquee' ? reflectUnsignedLongGet(this._attrs.scrolldelay, 85) : undefined; }
  set scrollDelay(v) { if (this._tag === 'marquee') reflectUnsignedLongSet(this, 'scrolldelay', v); }
...
// all strictly rarer): (1) `Object.getOwnPropertyNames(document)` does NOT list
// the named properties — that needs an ownKeys trap on the live document object
// itself (making `document` a Proxy), whose get-trap would tax every internal
// `document.<member>` read on the cascade/find hot path (rule 3) for a single
// enumeration test no app/library depends on; (2) `'x' in <non-main-document>`
// resolves against `globalThis.document` (the has trap has no receiver), so for
// a secondary same-realm document (DOMParser / createHTMLDocument result) the
// `in` check is approximate — `document.<x>` get is exact (it uses the receiver).
// The exotic applies to every Document instance (incl. XML / createHTMLDocument),
// which matches the spec — named access is a Document, not HTMLDocument, surface.
