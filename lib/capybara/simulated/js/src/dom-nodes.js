...
class Element extends Node {
  ...
  // Load an <img>'s resource: fetch + decode the current src to an RGBA buffer
  // (host libvips, memoized per URL), populate the pixel bitmap + natural
  // dimensions + `complete`, and fire `load` (or `error` on a broken/undecodable
  // image). The decode is synchronous but the event is async — HTML fires it from
  // a queued task, approximated here by a microtask — so an `onload`/`onerror` set
  // after `src=` still receives it. `_pixels` is what drawImage / createPattern read.
  ...
  _loadImageResource() {
    ...
    // A scheme the host can't read yet (blob:, …): leave the element inert instead
    // of firing a spurious `error` — the resource may well be loadable.
    if (r && r.unsupported) return;
    if (r && r.zeroSize) {
      // A valid image with a zero intrinsic dimension (e.g. an SVG whose width|height is
      // 0): the request SUCCEEDED, so the element is complete and NOT broken — it just
      // has no pixels. Fire `load` and report a zero-area size; createPattern/drawImage
      // then treat it as an available-but-empty source (null / no-op), never broken.
      this._pixels        = null;
    ...
    if (bytes) {
      ...
      // A wide-gamut (Adobe/CMYK) source also carries a Display-P3 rendering, used
      // when drawn into a P3 canvas (the sRGB `_pixels` above is the clipped one).
