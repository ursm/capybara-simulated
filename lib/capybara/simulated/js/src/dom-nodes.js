...
function routeChildRealmLinkNav(anchor, target) {
  ...
  if (t && t !== '_self' && t !== '_top' && t !== '_parent') {
    if ((inNestedBrowsingContext() || globalThis.__csimIsWindowRealm) &&
      ...
      const relTokens = String(anchor._attrs.rel || '').toLowerCase().split(/\s+/);
      const opener = relTokens.indexOf('opener') !== -1;   // target=_blank is noopener unless rel=opener
...
export function anchorActivateNavigate(anchor) {
  ...
  const relTokens = String(anchor._attrs.rel || '').toLowerCase().split(/\s+/);
  const pn = { url: String(anchor._attrs.href), target: tgt, opener: relTokens.indexOf('opener') !== -1 };
