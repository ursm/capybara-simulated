...
class Element extends Node {
  ...
  get classList() {
    ...
    // Each writer goes through `commit(next)` so the same mutation
    // record + cascade-invalidation path setAttribute fires (notably:
    // Avo's tabs controller calls `classList.remove('hidden')`, and
    // without an `attributes` MutationRecord, IntersectionObserver's
    // visibility recheck never fires for the revealed lazy frame).
    const commit = (next) => {
