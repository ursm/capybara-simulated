...
class Element extends Node {
  ...
  _imageResourceSrc() {
    if (this._tag === 'img') {
      if (this._attrs.src != null && this._attrs.src !== '') return this._attrs.src;
      // No src: HTML "update the image data" selects from `srcset` (ours is the
      // MINIMAL selection — the first candidate's URL; real density/width choice
      // needs layout/DPR we don't model, and a single-candidate srcset — the
      // common authoring and every vendored test — is exact), then from a
      // `<picture>` parent's first `<source srcset>`.
      const firstCandidate = (set) => {
        const c = String(set || '').trim().split(',')[0];
        return c ? c.trim().split(/\s+/)[0] : '';
      };
      const own = firstCandidate(this._attrs.srcset);
      if (own) return own;
      // …then from the `<picture>` parent's selected `<source>` — the same selection the cascade
      // reads that source's dimension attributes through (walk.js `pictureSourceFor`).
      const picked = pictureSourceFor(this);
      if (picked) return firstCandidate(picked._attrs && picked._attrs.srcset);
  ...
  _loadImageResource() {
    if (this._tag !== 'img' && this._tag !== 'image') return;
  ...
  get currentSrc()    { return this._tag === 'img' ? ((this._imgComplete || this._imgRequestSrc) && this._attrs.src ? String(this._attrs.src) : '') : undefined; }
...
function recordImageTiming(el, src, timingStart, r) {
  ...
  globalThis.__csimRecordResource({ name: src, initiatorType: el._tag === 'input' ? 'input' : 'img', startTime: timingStart,
