...
import { walk, walkFind, walkSubtree, walkInclShadow, isConnected, scriptText, findById, setTemplateDocResolver, setSlotResolver, pictureSourceFor } from './walk.js';
...
import { isVisibleNode, isLaidOutNode, INVISIBLE_TAGS, selfHidden, cascadedProperty, scheduleCascadeRefresh, bumpCascadeVersion, bumpCtxAll } from './cascade.js';
...
// …remembered on the element (`_autoDir`), so a mutation under it can ask whether the answer everything under it
// inherited has FLIPPED (`autoDirectionFlipped`) — the one case that moves the rest of its subtree.
function autoDirectionality(root) {
  return (root._autoDir = firstStrongDirectionality(root));
}
// Did a `dir=auto` element's (or a bare `<bdi>`'s) resolved direction change since it was last resolved? False for one
// never resolved: nothing has read it, so nothing laid out from it. (Asked by mutation-observer.js's
// `markDirAutoScopes`.)
setAutoDirectionProbe((el) => {
  const was = el._autoDir;
  return was !== undefined && autoDirectionality(el) !== was;
});
function firstStrongDirectionality(root) {
...
class Element extends Node {
  ...
  _ownDirectionality() {
    ...
    if (raw != null) {
      ...
      return (this._tag === 'input' || this._tag === 'textarea') ? controlAutoDir(this) : autoDirectionality(this);
    }
    return this._tag === 'bdi' ? autoDirectionality(this) : null;
  }
  _cssDirectionality() {
    let cur = this;
