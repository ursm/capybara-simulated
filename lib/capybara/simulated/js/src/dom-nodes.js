...
// The "focus delegate" of a shadow host with delegatesFocus: the first focusable
// element in the SHADOW TREE in tree (pre)order. Slotted light-DOM content is NOT
// a shadow-tree node, so it's naturally excluded — a `<slot>`'s own subtree (its
...
// `isFocusable` in preorder (which counts tabindex=-1 + inherently-focusable and
// excludes a no-tabindex div). A nested shadow host that itself delegatesFocus is
// recursed into ITS OWN shadow tree (not its light children): the delegate is that
// inner tree's already-focused descendant or its own first focus delegate. A nested
// host that does NOT delegate is descended as an ordinary element — its node-tree
// children are walked, but its SHADOW content stays unreachable via delegation
// (only the host itself, if focusable, can be picked from the inner shadow).
