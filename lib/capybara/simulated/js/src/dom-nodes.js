...
function clampDocScroll(el, axis, value) {
  ...
  const isDoc = el === root || (doc && el === doc.body) || (doc && doc.scrollingElement === el);
  return isDoc ? clampScrollOffset(el, axis, value) : (Number(value) || 0);
}
...
class Node {
  ...
  // Per-element scroll state: whatever a caller stored, on any element. A browser keeps an offset
  // only on a SCROLL CONTAINER and reports 0 for everything else — layout knows which is which
  // (`scrollsContent`), so this is a deliberate remaining gap rather than the missing-engine one
  // the comment used to claim: the offset is stored and read back, but only a real scroll
  // container's is applied to geometry, so an `overflow: clip` box reports the value it was given
  // while its content correctly does not move. Discourse's RouteScrollManager saves
