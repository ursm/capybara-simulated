...
import { ceState, customElements as globalCERegistry, getCustomElementCtor, lookupCEDefinitionCtor, customElementLocalName, customElementIsValueForCtor, isValidCustomElementName, isFormAssociatedCustomElement, hasFormAssociatedCustomElements, setFormAssociatedReset, ceUpgradeTree, fireCEDisconnect, fireCEMoveReactions, fireCEAdopted, fireAttrChangedCallback, askForReset, askForResetBatch, askForResetAfterRemoval, runSelectednessAlgorithm, finalizeSelectOptions, ensureOptionSelInit, selectDisplaySize, updateSelectedContent, scheduleSelectedContentUpdate } from './custom-elements.js';
...
class Node {
  ...
  _insertFragmentChildren(frag, ref) {
    ...
    for (const c of moved) { c._parent = this; registerSubtree(c); runSourceInsertionStep(c, this); }
...
class Element extends Node {
  constructor(tagName) {
    ...
    if (!tag && new.target) {
      ...
      // clone re-resolve the definition by local name + is value.
...
// (`_ceRegistry === undefined`) means the global registry; an explicit scoped
// registry or the null-registry state (`_ceRegistry === null`) is reflected as-is.
Object.defineProperty(Element.prototype, 'customElementRegistry', {
  configurable: true, enumerable: false,
  get() { return this._ceRegistry === undefined ? (globalThis.customElements || null) : this._ceRegistry; }
});
...
class ShadowRoot extends DocumentFragment {
  ...
  // The shadow tree's custom element registry (set at attachShadow): unset → the
  // global registry, else the scoped / null registry the tree was created with.
  get customElementRegistry() {
    return this._ceRegistry === undefined ? (globalThis.customElements || null) : this._ceRegistry;
  }
...
function ceRegistryOption(options) {
  ...
  if (reg !== null && !(reg instanceof globalThis.CustomElementRegistry)) {
...
class Document extends Node {
  ...
  _createElement(ns, prefix, localName, registry, isValue) {
    ...
    const inBrowsingContext = ns === HTML_NS && this === globalThis.document;
    ...
    const lookupReg = !inBrowsingContext ? null : !explicit ? globalCERegistry : registry;
  ...
  importNode(node, deep) {
    ...
    // Skip cloneNode's own CE upgrade: the fresh clone still carries the SOURCE's node
    // document, so upgrading here would key the browsing-context gate off the wrong
    // document (a custom element cloned out of the live doc would construct even when
    // imported INTO a window-less document). Re-own to `this` first, then upgrade below.
    const out = node.cloneNode(!!deep, true);
    // Re-own the clone subtree to THIS document (importNode's "clone with document =
    // this"). The clone otherwise keeps the source's node document, which would key
    // the browsing-context upgrade gate off the wrong document — importing a
    // turbo-stream element parsed in an inert DOMParser doc into the live document
    // must upgrade (so `streamElement.templateElement.content` resolves on the
    // StreamElement prototype), while importing INTO a window-less document must not.
    walkSubtree(out, n => { n._ownerDoc = this; });
    // Elements inert in `<template>.content` / a DOMParser doc aren't upgraded at
    // creation; upgrade them now iff this document has a browsing context (ceUpgradeTree).
    ceUpgradeTree(out);
    return out;
  }
  adoptNode(node) {
    ...
    walkSubtree(node, n => {
      ...
      if (crossDoc && n._shadowRoot && n._shadowRoot._adoptedStyleSheets) {
        const arr  = n._shadowRoot._adoptedStyleSheets;
