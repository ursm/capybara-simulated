...
class Element extends Node {
  constructor(tagName) {
    ...
    // last statement — the returned object replaces `this`.
    else if (this._tag === 'form') return makeFormProxy(this);
  }
  _cloneShell() {
  ...
  get type()  {
    ...
    // `<fieldset>.type` is the constant 'fieldset' (HTMLFieldSetElement.type).
...
// .prototype (which IS Element.prototype here, the load-bearing shared-prototype
// alias). Re-flag ONLY that accessor: because the alias puts it on every element,
// flipping the whole sibling group would leak six names into `for…in` over any
// element — diverging from real browsers, where they live only on <template>. No
// test asserts the siblings are enumerable, so they stay at the uniform
// non-enumerable default the rest of Element.prototype uses.
{
