...
class Element extends Node {
  ...
  get willValidate() {
    ...
    // Barred when disabled, readonly, or inside a <datalist>.
    //
    // NOTE: per spec a readonly BUTTON is NOT barred (button-willvalidate-readonly),
    // because `readonly` doesn't apply to buttons. We can't honour that yet: the
    // constraint-validation harness only tests a readonly variant when
    // `'readOnly' in ctl`, and our `readOnly` IDL lives on the shared Element
    // prototype, so `'readOnly' in button` is true (a real browser: false). Making
    // it false needs per-tag IDL-property existence — a separate, larger change.
    // Until then we keep the blanket readonly bar (so customError/willValidate stay
    // correct for button/select), and button-willvalidate-readonly stays a tracked
    // in-scope gap.
    if (isActuallyDisabled(this)) return false;
    if (this._attrs.readonly != null) return false;
