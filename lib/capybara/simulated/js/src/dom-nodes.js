...
import { walk, walkFind, walkSubtree, isConnected, scriptText, stripOneLeadingNewline, findById } from './walk.js';
...
import { installHtmlParser, serializeChildren, serializeChildrenWithShadow, serializeElement, escapeText } from './html-parser.js';
...
class Node {
  ...
  focus() {
    ...
    if (prev) {
      ...
      try { dispatchEvent(prev, new FocusEvent('blur',     { bubbles: false, cancelable: false, relatedTarget: this })); } catch (_) {}
      try { dispatchEvent(prev, new FocusEvent('focusout', { bubbles: true,  cancelable: false, relatedTarget: this })); } catch (_) {}
    }
    ...
    try { dispatchEvent(this, new FocusEvent('focus',    { bubbles: false, cancelable: false, relatedTarget: prev })); } catch (_) {}
    try { dispatchEvent(this, new FocusEvent('focusin',  { bubbles: true,  cancelable: false, relatedTarget: prev })); } catch (_) {}
  }
  blur() {
    if (globalThis.document._activeElement !== this) return;
    globalThis.document._activeElement = null;
    try { dispatchEvent(this, new FocusEvent('blur',     { bubbles: false, cancelable: false })); } catch (_) {}
    try { dispatchEvent(this, new FocusEvent('focusout', { bubbles: true,  cancelable: false })); } catch (_) {}
  }
...
class Element extends Node {
  ...
  getElementsByTagName(tag) {
    ...
    // reflected through the held reference.
  ...
  set outerHTML(html) {
    ...
    const nodes = isHtmlDocument(this.ownerDocument)
      ? parseFragment(html === null ? '' : String(html))
  ...
  insertAdjacentHTML(position, html) {
    ...
    const frag = isHtmlDocument(this.ownerDocument)
      ? parseFragment(String(html === null ? '' : html))
      : parseXmlFragment(html, (pos === 'beforebegin' || pos === 'afterend') ? this._parent : this);
  ...
  attachShadow(init) {
    ...
    if (this._shadowRoot) {
      // HTML attachShadow: a DECLARATIVE shadow root (from `<template
      // shadowrootmode>`) of the same mode is reused — emptied, its declarative
      // flag cleared, and its flags updated from init — so frameworks can
      // "upgrade" it to an imperative root. Any other existing root throws.
      ...
      // slotAssignment is FIXED at declarative-creation time — re-attaching does
      // NOT change it (HTML: the reuse path keeps the existing root's mode/slot
      // assignment; a later attachShadow({slotAssignment:'named'}) is ignored).
...
const WindowNamedProps  = new Proxy(WindowNamedTarget, {
  has(target, prop) {
    ...
    if (typeof prop === 'string' && WINDOW_NAMED_PROPS.has(prop)) {
      return windowNamedLookup(prop) !== undefined;
    }
    return false;
  },
  get(target, prop, receiver) {
...
function canAttachShadow(el) {
  ...
  if (ln.indexOf('-') !== -1) {
    // Custom element: a definition whose `static disabledFeatures` includes
    // "shadow" cannot host a shadow root (HTML attachShadow step). Both an
    // imperative attachShadow and declarative `<template shadowrootmode>`
    // must respect it.
...
// Classify an element within its scope: `stop` = a sequential focus stop;
// `owner` = 'host' | 'slot' whose sub-scope is traversed; `group` = tabindex
// ordering bucket (0 = auto/zero). Returns null for a plain element we descend
// through but don't collect.
function __seqCandidate(el) {
  ...
  let n = null;
  if (tiAttr != null) { const p = parseInt(tiAttr, 10); if (!isNaN(p)) n = p; }
  // A slot is never itself a focus stop; its assigned content is traversed in
  // place. Position it by an explicit non-negative tabindex, else group 0.
  if (isSlot) return { el, stop: false, owner, group: (n != null && n >= 0) ? n : 0 };
  let stop, group;
  if (n != null && n < 0) {
    // Negative tabindex: not a sequential focus stop. A negative-tabindex shadow
    // HOST additionally excludes its whole shadow tree from sequential navigation
    // (verified against Chrome — Tab skips over a tabindex=-1 host's contents).
    // Return it as an owner with group -1 so __seqEmit drops it (neither positive
    // nor 0) and __seqGather won't descend its slotted light children; a plain
    // negative element returns null so its in-scope children are still descended.
    if (owner === 'host') return { el, stop: false, owner, group: -1 };
    return null;
  }
  if (n != null)               { stop = isFocusable(el);  group = n; }  // explicit >= 0
  else                         { stop = isFocusable(el);  group = 0; }  // auto: default-focusable -> group 0
  if (!stop && !owner) return null;
  return { el, stop, owner, group };
}
// Gather a scope's candidates from its top-level node list (a shadow root's
// children, or a slot's assigned nodes). Descends plain elements (same scope)
// but NOT shadow hosts (light children are slotted away), slots (assigned nodes
// are a sub-scope), or contenteditable hosts (one focus stop — inner content is
// not independently tabbable).
...
// Emit a scope's focus stops in tabindex order, recursing into each owner's
// sub-scope at its position. `Array.sort` is stable (ES2019), so equal-tabindex
// candidates keep flat-tree preorder.
...
// Move focus to the next (or previous, for Shift-Tab) sequential focus stop,
// relative to the currently focused element. Wraps at the ends.
...
function resetFocusIfUnfocusableAfterMove() {
  ...
  globalThis.setTimeout(() => {
    ...
    try { dispatchEvent(ae, new FocusEvent('blur',     { bubbles: false, cancelable: false })); } catch (_) {}
    try { dispatchEvent(ae, new FocusEvent('focusout', { bubbles: true,  cancelable: false })); } catch (_) {}
  }, 0);
}
...
const { parseDocument, parseFragment } = installHtmlParser({ Document, Element, Text, Comment, DocumentFragment, DocumentType, createHtmlPageDocument, registerNamedAccess });
