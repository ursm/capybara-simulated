...
class Node {
  ...
  // Geometry comes from the coarse box-layout engine (layout.js) — ONE geometry, the same boxes
  // that back `obscured?` / `Node#rect` / `drag_to`, so a synthetic pointer and the page's own math
  // agree the way they do in a real browser. It is coarse (no glyph metrics, no inline flow, no
  // flex/grid track sizing), but it is real: document order gives increasing tops, a block fills its
  // containing width, and a non-rendered element measures 0×0 at the origin like it should.
  //
  ...
  getClientRects() {
    ...
    // boxes; a rendered zero-height one (an empty block) still has one, which is what keeps
    // `:visible`-style probes answering true for it.
    return clientRectsOf(this)
      .filter((r) => r.width !== 0 || r.height !== 0)
      .map((r) => new globalThis.DOMRect(r.x, r.y, r.width, r.height));
  }
  ...
  set scrollTop(v) {
    // NOT clamped to the scrollable range yet, though a browser clamps: our content
    // extents are still coarse, so clamping turns every under-measured scroller into
    // a scroll that silently goes nowhere — measured a net LOSS across the scrolling
    // WPT cluster. Revisit once inline flow makes extents exact (the clamp itself is
    // ready: layout.js clampScrollOffset).
  ...
  set scrollLeft(v) {
    const next = Number(v) || 0;   // see scrollTop: clamping waits for exact extents
  ...
  scrollTo() {
    ...
    let changed = false;
    if (typeof x === 'number' && x !== (this._scrollLeft || 0)) { this._scrollLeft = x; changed = true; bumpScrollEpoch(); }
    if (typeof y === 'number' && y !== (this._scrollTop  || 0)) { this._scrollTop  = y; changed = true; bumpScrollEpoch(); }
    if (changed) { __notifyScroll(this); __notifyScrollEnd(this); }
  }
  scrollBy() {
    const [dx, dy] = __scrollArgsToXY(arguments);
    let changed = false;
    if (typeof dx === 'number' && dx !== 0) { this._scrollLeft = (this._scrollLeft || 0) + dx; changed = true; bumpScrollEpoch(); }
    if (typeof dy === 'number' && dy !== 0) { this._scrollTop  = (this._scrollTop  || 0) + dy; changed = true; bumpScrollEpoch(); }
    if (changed) { __notifyScroll(this); __notifyScrollEnd(this); }
  }
