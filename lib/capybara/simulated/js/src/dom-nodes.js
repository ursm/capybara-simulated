...
// parse5 streaming-parser spike (behind CSIM_STREAMING_PARSE). The adapter maps
// parse5's tree ops onto these same node ctors; exposed for now as a test hook
// so we can validate output + benchmark vs the hand-rolled parser before wiring
// it into the load path.
const { parse5ParseDocument: __p5ParseDocument } = installParse5Adapter({ Document, Element, Text, Comment, DocumentFragment, DocumentType, createHtmlPageDocument });
