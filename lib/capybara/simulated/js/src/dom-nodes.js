...
class Element extends Node {
  ...
  setAttribute(name, value) {
    ...
    if ((n === 'src' || n === 'srcdoc') && (this._tag === 'iframe' || this._tag === 'frame') && old !== next) {
      ...
      // A pending scripted-nav response (form-target content not yet consumed) is
      // superseded by this src change — drop it so the new src isn't ignored.
      this._frameNavContent = null;
      this._frameNavPending = false;
      // Navigating the frame away fires `beforeunload` on the old document, then
      // disposes the superseded realm (so it doesn't linger / get re-drained).
      disposeFrameRealmForNav(oldRealmId);
