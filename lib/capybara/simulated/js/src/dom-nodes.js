...
import { recordAttrMutation, recordChildList, recordCharacterData, bumpSettleGen, bumpStyleState, currentSettleGen, signalSlotChange, setSlotChangeFirer, setSlotMutationHooks, markLayoutDirty } from './mutation-observer.js';
...
import { ensureInView, rectOf, clientRectsOf, clientBoxOf, documentBoxOf, paddingBoxOriginOf, viewportSize, contentExtent, isPositionedElement, isFixedElement, scrollTargetFor, clampScrollOffset, hitTest, bumpScrollEpoch } from './layout.js';
...
class Node {
  ...
  scrollIntoView(_opts) {
    ...
    const before = root ? (root._scrollTop || 0) : 0;
    // A real scroll, to the same offset `rect` / `obscured?` read back. `block` picks the alignment
    // (default `start`; the legacy boolean argument means align-top, which is the default anyway).
    // Assigned WITHOUT the scrollTop setter: this method fires exactly one scroll notification of
    // its own below, and the setter's would make every scrollIntoView refire observers twice.
    if (root) {
      const block = (_opts && typeof _opts === 'object' && _opts.block) || 'start';
      const to = scrollTargetFor(root, this, block === 'center' ? 'center' : block === 'end' ? 'bottom' : 'top');
      if (to) { to.el._scrollLeft = to.x; to.el._scrollTop = to.y; }
    }
    const moved = root ? (root._scrollTop || 0) !== before : false;
    // It scrolls the ROOT scroller (root._scrollTop above), so both events fire on the VIEWPORT,
    // not on `this` — a `scroll` dispatched at the element scrolled INTO view reaches neither the
    // window listeners that care nor anything else. And only when it actually moved: Chrome fires
    // nothing for a `scrollIntoView` of a box already in place. (The IO recheck is separate — the
    // `bumpScrollEpoch` above asks for it either way, which is what a pagination sentinel needs.)
    if (moved) { __notifyScroll(root); __notifyScrollEnd(root); }
