...
class Element extends Node {
  ...
  select() {
    // HTML spec: select() focuses the input as a side effect, so
    // `document.execCommand('copy')` can read activeElement's range.
    this.focus();
...
class Document extends Node {
  ...
  execCommand(command, _showUI, value) {
    ...
    if (cmd === 'copy') {
      // Clipboard API: write the current input/textarea range or
      // Selection text to the clipboard buffer. Runs before the
      // `!active` gate below so a Selection on a non-focused subtree
      // still copies.
      let text = '';
