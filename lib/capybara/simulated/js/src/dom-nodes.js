...
import { isContenteditable, toggleChecked, setRadio, checkedRadioInGroup, uncheckOtherRadios, forEachRadioInGroup, getCheckedness, setCheckedness, controlLiveValue, textareaRawValue, isSubmitButton, formForControl, closeDialog, labeledControlFor, isLabelableControl, isHtmlLabel, labelToActivateFor, LABELABLE } from './form-helpers.js';
import { makeStyleProxy }                                             from './style-proxy.js';
import { liveHTMLCollection, liveOptionsCollection, liveNodeList, nodeList, newChildList, liveNamedNodeMap }   from './dom-collections.js';
    ...
    try {
      ...
      if (!ev.defaultPrevented && isSubmitButton(this) && isConnected(this) && !isActuallyDisabled(this)) {
        ...
        if (form && typeof form._submitForm === 'function') {
          ...
          didSubmit = form._submitForm(this, true);
...
class Element extends Node {
  ...
  get value() {
    ...
    // `<option>.value` (HTMLOptionElement): the `value` content attribute if
    // present, else the option's `text` IDL (its collapsed/trimmed descendant
    // text). namedItem / option-list reads depend on this fallback.
    if (this._tag === 'option') {
      return this._attrs.value != null ? this._attrs.value : this.text;
    }
  ...
  get willValidate() {
    ...
    if (tag === 'input') {
      ...
      if ((this._attrs.type || 'submit').toLowerCase() !== 'submit') return false;
    } else if (tag !== 'select' && tag !== 'textarea') {
      return false;
    }
    // Barred when disabled, readonly (the attribute alone bars it), or inside a
    // <datalist>.
...
// / delete). It is exactly `form.elements`, read via Reflect on the TARGET so
// (a) a control named "elements" can't shadow it through the proxy, (b) it is the
// SINGLE source of truth — form[i] === form.elements[i] and form.length ===
// form.elements.length always agree (form-indexed-element), and (c) it reuses
// the elements getter's cached subtree walk rather than a second, document-wide
// scan. `t` is the form target. (form= associated controls outside the subtree
// are reachable by NAME — formNamedCandidates — but, like `form.elements` itself,
// not by index; a pre-existing bound of the subtree-scoped elements getter.)
function formIndexedElements(t) {
  return Reflect.get(t, 'elements', t) || EMPTY_NODES;
}
...
function formNamedCandidates(form, name) {
  ...
  const out  = [];
  const root = isConnected(form) ? (form._ownerDoc || globalThis.document || form) : form;
  walkSubtree(root, el => {
...
// names map); several → a cached [SameObject] live NodeList (the spec's
// RadioNodeList — its radio-specific `.value` accessor is not modeled, as no WPT
// subtest / app depends on it; `instanceof NodeList`, item/index/length all hold);
// none → the past names map (an element keeps resolving under an old name while it
// stays an eligible control of the form), else undefined.
function formNamedItem(form, name) {
  ...
  if (cands.length > 1) {
    ...
    let nl = lm.get(name);
    if (!nl) { nl = liveNodeList(() => formNamedCandidates(form, name)); lm.set(name, nl); }
    return nl;
  }
