...
// Input types whose `value` is text the user reads, so it feeds `dir=auto`
// directionality (dir-auto-form-associated). For every OTHER type (checkbox / radio /
// image / date / number / range / color / …) the value is not directional text → ltr.
const DIR_AUTO_VALUE_INPUT_TYPES = new Set(
...
// an input only for the text-like types above, else ltr.
function controlAutoDir(el) {
  ...
  return DIR_AUTO_VALUE_INPUT_TYPES.has(type) ? (firstStrongDir(el.value || '') || 'ltr') : 'ltr';
}
