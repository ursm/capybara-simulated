...
// DOM "flatten options" for the add/removeEventListener `options` argument,
// whose WebIDL type is `(AddEventListenerOptions or boolean)`: an object is a
// dictionary (read `capture`), null/undefined are an empty dictionary
// (capture false), and ANY other value (boolean, number, string, …) is
// converted to a boolean — so `addEventListener(t, fn, 2.3)` captures and
// `addEventListener(t, fn, "")` does not.
