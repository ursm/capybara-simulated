...
class Node {
  ...
  focus() {
    ...
    if (isChangeOnBlurControl(this)) this._changeBaseline = controlLiveValue(this);
  ...
  submit() {
    ...
    recordFormSubmission(this, null);
  }
  ...
  // HTML "submit a form" — the parts we model. Shared by requestSubmit() and a
  // submit-button activation; `interactive` true means run constraint validation
  // and fire the submit event. (The programmatic submit() does NOT route here —
  // it neither validates nor fires submit; see submit() above.) A disconnected
  // form never submits; `_constructingEntryList` guards re-entrant submission
  // while the entry list is built. Validation is skipped when the form has
  // `novalidate` or the submitter has `formnovalidate`.
  //
  ...
  _submitForm(submitter, interactive) {
    ...
    // Reentrancy guard, set for the WHOLE submission (HTML's "firing submission
    // events" + "constructing entry list" flags): a nested submit triggered from
    // an `invalid` / `submit` handler — or a button click inside one — returns
    // early rather than re-running. (Set before validation + the submit event,
    // not just around the entry-list build.)
    if (this._constructingEntryList) return false;
    ...
    try {
      ...
      if (interactive && !noValidate && !this.checkValidity()) return false;   // fires `invalid`; aborts when invalid
      ...
      recordFormSubmission(this, submitter || null);
      return true;
    } finally { this._constructingEntryList = false; }
  }
  ...
  removeChild(child) {
    ...
    if (ae && (ae === child || nodeContains(child, ae))) {
      ...
      // change-on-blur baseline so it doesn't carry focus-cycle state while detached.
...
// the target iframe).
export function recordFormSubmission(form, submitter) {
  if (globalThis.__csimSubmitFormToNamedFrame && globalThis.__csimSubmitFormToNamedFrame(form, submitter || null)) return true;
  globalThis.__csimPendingFormSubmit = { form, submitter: submitter || null };
...
globalThis.__csimSubmitFormToNamedFrame = function (form, submitter) {
  ...
  try {
    let promise;
    if (method === 'GET') {
      const qs = formEntryParams(spec).toString();
      actionUrl = actionUrl.split('#')[0].split('?')[0] + (qs ? '?' + qs : '');
      ...
      const reqBody = enctype.indexOf('multipart/form-data') === 0
        ? new globalThis.FormData(form, submitter || undefined)   // escaping + File bytes
        : formEntryParams(spec);                                   // urlencoded
    ...
    promise
      .then((resp) => resp.text().then((text) => {
        ...
        navigateFrameElementToContent(frameEl, resp.url || actionUrl, text, ct);
      }))
      .catch(() => navigateFrameElementToContent(frameEl, actionUrl, '', 'text/html'));
  } catch (_) {
    navigateFrameElementToContent(frameEl, actionUrl, '', 'text/html');
  }
  return true;
};
...
// Controls that fire `change` only when they LOSE FOCUS, iff the value changed
// since they gained it (text-like inputs + <textarea>). checkbox/radio/select
// commit `change` on the interaction itself, not on blur. `focus()` records the
// baseline; `commitChangeOnBlur` compares + fires when the element is blurred.
...
function commitChangeOnBlur(el) {
  ...
  const changed = controlLiveValue(el) !== el._changeBaseline;
...
export function commitChangeKeepingFocus(el) {
  if (!el || el._changeBaseline === undefined || !isChangeOnBlurControl(el)) return;
  if (controlLiveValue(el) === el._changeBaseline) return;
  el._changeBaseline = controlLiveValue(el);   // re-baseline BEFORE dispatch (re-entrancy)
  try { dispatchEvent(el, new Event('change', { bubbles: true, cancelable: false })); } catch (_) {}
}
// Re-sync the change-on-blur baseline after a PROGRAMMATIC value mutation (the
// `.value` setter, setRangeText, a clean control's default/textContent rewrite) so
// scripted changes don't make the control fire a spurious `change` on the next blur
// — only user edits since focus do. No-op when the control isn't focused.
function rebaselineFocusedValue(el) {
  if (el._changeBaseline !== undefined) el._changeBaseline = controlLiveValue(el);
}
...
class Element extends Node {
  ...
  set value(v)   {
    ...
    if (usesLiveValue) {
      ...
      // ...and re-baseline the change-on-blur value: setting `.value` by script
      // must NOT make the control fire `change` when it later loses focus (only a
      // user edit since focus does).
      rebaselineFocusedValue(this);
  ...
  set defaultValue(v) {
    ...
    if (this._tag === 'textarea') { this.textContent = s; rebaselineFocusedValue(this); }
  ...
  __setSelectionRange(start, end, direction) {
    // Read the live value via the getter, not `_attrs.value`: a <textarea>'s
    // value is its child text (a clean textarea has an empty `_attrs.value`),
    // and clamping to a stale 0 length would collapse every offset to 0.
    const cv = this.value;
  ...
  setRangeText(replacement, start, end, selectMode) {
    ...
    this._value = next;
    rebaselineFocusedValue(this);   // programmatic edit → no spurious change on blur
  ...
  select() {
    ...
    // browsers + existing callers' "focus and select all" expectation.
    if (!this.__selectionApplies()) return;
    this.focus();
    // Length from the live value getter, not `_attrs.value` — a clean
    // <textarea>'s value is its child text (otherwise select() selects nothing).
    const len = (this.value != null ? String(this.value) : '').length;
