...
// Minimal DOMTokenList over a space-separated content attribute,
// backing `relList`. Reads are live; mutators round-trip through
// setAttribute so observers see changes.
function makeTokenList(el, attr) {
  const split = () => String(el._attrs[attr] || '').split(/\s+/).filter(Boolean);
  const commit = (toks) => el.setAttribute(attr, toks.join(' '));
  const list = {
    get length() { return split().length; },
    get value()  { return el._attrs[attr] || ''; },
    set value(v) { el.setAttribute(attr, String(v == null ? '' : v)); },
    item(i)      { const t = split(); return i >= 0 && i < t.length ? t[i] : null; },
    contains(t)  { return split().includes(String(t)); },
    add(...names) {
      const toks = split();
      let changed = false;
      for (const n of names) if (!toks.includes(String(n))) { toks.push(String(n)); changed = true; }
      if (changed) commit(toks);
    },
    remove(...names) {
      const drop = new Set(names.map(String));
      const toks = split();
      if (!toks.some(t => drop.has(t))) return;
      commit(toks.filter(t => !drop.has(t)));
    },
    toggle(t, force) {
      const toks = split();
      const i = toks.indexOf(String(t));
      const present = i >= 0;
      if (force === true || (force === undefined && !present)) {
        if (!present) toks.push(String(t));
      } else if (present) {
        toks.splice(i, 1);
      }
      commit(toks);
      return toks.includes(String(t));
    },
    toString() { return el._attrs[attr] || ''; },
    forEach(fn) { split().forEach((t, i) => fn(t, i, list)); }
  };
  return list;
}
