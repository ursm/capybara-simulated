  ...
  execCommand(command, _showUI, value) {
    ...
    if (cmd === 'inserttext') {
      ...
      if (active._tag === 'textarea' || (active._tag === 'input' && /^(text|search|email|url|tel|password)?$/i.test(active._attrs.type || ''))) {
        ...
        try { dispatchEvent(active, new globalThis.InputEvent('input', { bubbles: true, cancelable: false, composed: true, data: str, inputType: 'insertText' })); } catch (_) {}
        return true;
      }
    }
    ...
    if (active._attrs.contenteditable != null && (active._attrs.contenteditable || '').toLowerCase() !== 'false') {
      ...
      const applyList = (listTag, inputType) => {
        const sel = globalThis.getSelection && globalThis.getSelection();
        let anchor = active;
        // Only follow the selection when it actually sits inside the editing host;
        // otherwise the ancestor walks below could escape `active` and mutate a
        // list / block outside it. A host-external selection falls back to `active`.
        const sc = sel && sel._ranges.length && sel._ranges[0].startContainer;
        if (sc && (sc === active || nodeContains(active, sc))) anchor = sc;
        let li = null;
        ...
        if (li) {
          ...
          let block = active;
          for (let n = anchor; n && n !== active; n = n._parent) {
            if (n.nodeType === NODE_ELEMENT && BLOCK_CONTAINER_TAGS.has(n._tag)) { block = n; break; }
          }
      ...
      const applyInlineStyle = (tag, inputType, exclusiveWith) => {
        ...
        for (const t of targets) {
          // sup/sub exclusivity: strip a conflicting immediate wrapper first — but
          // only when it wraps EXACTLY the selected run. If it also covers unselected
          // text, stripping it wholesale would silently drop that text's formatting,
          // so leave it (the deeper slice-scoped unwrap is editing-algorithm backlog).
          if (exclusiveWith && t._parent && t._parent._tag === exclusiveWith && t._parent !== active) {
        ...
        // A format command fires `input` only when it actually mutates the DOM — a
        // no-op selection (nothing wrapped) dispatches nothing, matching Chrome.
        if (targets.length) {
          // Re-cover the wrapped run so a chained styling command nests one level deeper.
      ...
      return false;   // color / justify / indent / link: editing-engine backlog
    }
    if (cmd === 'inserthtml' &&
