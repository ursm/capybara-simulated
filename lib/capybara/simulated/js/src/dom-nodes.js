...
import { rectOf, documentBoxOf, viewportSize, contentExtent, isPositionedElement, scrollTargetFor, hitTest, bumpScrollEpoch } from './layout.js';
...
class Node {
  ...
  get clientWidth()  { return this._isRootElement() ? viewportSize().width  : Math.round(rectOf(this).width); }
  get clientHeight() { return this._isRootElement() ? viewportSize().height : Math.round(rectOf(this).height); }
...
class Document extends Node {
  ...
  _createElement(ns, prefix, localName, registry, isValue) {
    ...
    if (ctor) {
      ...
      // HTML "create an element" with the synchronous flag set: if the constructor
      // throws — OR returns a malformed element (not this empty, unparented,
      // same-document, matching element) — REPORT the exception (don't propagate to
      // the createElement caller) and fall back to a plain element with this local
      // name (so a customized built-in whose constructor throws still yields a real
      // `<div>`; an autonomous failure yields an HTMLUnknownElement-matching element).
      try {
        el = new ctor();
        const err = constructedElementError(el, this, localName);
        if (err) throw err;
        el._ceUpgraded = true;   // once custom, always custom (lifecycle survives registry re-pointing)
      } catch (e) {
...
const { parse5ParseIntoLive, parse5ParseFragment, parse5ParseDocument } = installParse5Adapter({
  ...
  syncInlineEventHandler, finalizeElementProto, inertTemplateDocFor
});
