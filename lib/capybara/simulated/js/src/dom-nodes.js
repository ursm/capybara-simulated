...
class ProcessingInstruction extends CharacterData {
  ...
  get sheet() {
    ...
    if (!href) return null;
    if (!this._sheet) {
      // A failed fetch (null) still yields an (empty) sheet for an owned `<?xml-stylesheet?>`.
      this._sheet = buildOwnedStyleSheet(fetchStyleSheetText(href) || '', { baseURL: href });
...
class Element extends Node {
  ...
  get sheet() {
    ...
    if (this._tag === 'link') {
      ...
      if (!this._sheet) {
        // A failed fetch (null) still yields an (empty) sheet for an owned `<link>`.
        this._sheet = buildOwnedStyleSheet(fetchStyleSheetText(href) || '', { baseURL: href, media: this._attrs.media });
        this._sheet._ownerNode = this;
      }
      return this._sheet;
    }
