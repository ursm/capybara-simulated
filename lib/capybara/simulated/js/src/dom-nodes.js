...
import { NODE_ELEMENT, NODE_TEXT, NODE_COMMENT, NODE_DOC, NODE_DOCTYPE, NODE_FRAGMENT } from './constants.js';
...
function toNode(v) {
  if (v && (v.nodeType === NODE_ELEMENT || v.nodeType === NODE_TEXT || v.nodeType === NODE_COMMENT || v.nodeType === NODE_FRAGMENT || v.nodeType === NODE_DOC)) return v;
...
// Throws the spec's exception when inserting `node` into `parent` before
// `child` would be invalid. `child` null means "append". Implements steps
// 1–5; the document-only constraints (step 6: a Document may hold at most one
// element / one doctype in a fixed order) are intentionally not enforced —
// nothing in scope inserts elements straight into `document` via the API, and
// enforcing it risks false throws during page setup.
function ensurePreInsertionValidity(node, parent, child) {
  // 1. parent must be a Document, DocumentFragment, or Element.
  if (parent.nodeType !== NODE_DOC && parent.nodeType !== NODE_FRAGMENT && parent.nodeType !== NODE_ELEMENT) {
    throw new globalThis.DOMException(
      `Cannot add a child to a ${nodeTypeName(parent)} node`, 'HierarchyRequestError');
  }
  // 2. node must not be an inclusive ancestor of parent.
  if (isInclusiveAncestor(node, parent)) {
    throw new globalThis.DOMException(
      'The new child is an ancestor of the parent', 'HierarchyRequestError');
  }
  // 3. a given reference child must actually be a child of parent.
  if (child != null && child._parent !== parent) {
    throw new globalThis.DOMException(
      'The reference child is not a child of this node', 'NotFoundError');
  }
  // 4. node must be a DocumentFragment, Element, or CharacterData (Text /
  //    Comment). (This driver has no DocumentType / CDATA / PI node objects.)
  const t = node.nodeType;
  if (t !== NODE_FRAGMENT && t !== NODE_ELEMENT && t !== NODE_TEXT && t !== NODE_COMMENT) {
    throw new globalThis.DOMException(
      `Cannot insert a ${nodeTypeName(node)} node`, 'HierarchyRequestError');
  }
  // 5. a Text node cannot be a child of a Document.
  if (t === NODE_TEXT && parent.nodeType === NODE_DOC) {
    throw new globalThis.DOMException(
      'A Text node cannot be inserted into a Document', 'HierarchyRequestError');
  }
}
...
// A document is an "HTML document" (vs XML/XHTML) when its content type is
// text/html. The main document and createHTMLDocument leave `_contentType`
// unset (→ HTML); nested iframe documents set it from the response.
function isHtmlDocument(doc) { return !doc || !doc._contentType || doc._contentType === 'text/html'; }
...
class Node {
    ...
    for (let i = 0; i < kids.length; i++) {
      ...
      if (node.nodeType === NODE_TEXT) {
        if ((node._data || '').length === 0) {
          ...
          // drop connectivity, and null the parent.
          kids.splice(i, 1);
          node._parent = null;
          unregisterSubtree(node);
          recordChildList(this, [], [node]);
          i--;
          continue;
        }
        let next = kids[i + 1];
  ...
  // Namespace lookups — this DOM is HTML-only, so every element is in
  // the XHTML namespace and there are no prefixed namespaces.
  ...
  lookupPrefix(_namespaceURI) { return null; }
  ...
  replaceChild(neu, old) {
    // https://dom.spec.whatwg.org/#concept-node-replace — the validity rules
    // are pre-insertion's (parent type, ancestor, node type, Text-in-Document),
    // except the reference `old` must itself be a child (NotFoundError) rather
    // than just a sibling position.
    assertNodeArg(neu);
    assertNodeArg(old);
    ensurePreInsertionValidity(neu, this, old);
...
class Element extends Node {
  ...
  get attributes() {
    ...
    Object.defineProperty(list, 'getNamedItem', {
      value: name => {
        ...
        return Object.prototype.hasOwnProperty.call(el._attrs, k) ? makeAttr(el, k) : null;
      },
  ...
  // case-sensitive. (Collects only matches during one ordered walk — the same
  // O(N) cost as the old CSS query, no full-descendant array allocation.)
  getElementsByTagName(tag) {
    const q = String(tag);
    // querySelectorAll uses the iterative CSS walk (no recursion → deep-DOM
    // safe); '*' gives all descendants in document order, which we then refine
    // by qualified name. Drop the self-hit (our walk starts at the receiver).
    const all = this.querySelectorAll('*').filter(n => n !== this);
    if (q === '*') return htmlCollection(all);
    const htmlDoc = isHtmlDocument(this.ownerDocument);
    const qLower = htmlDoc ? asciiLower(q) : q;
    return htmlCollection(all.filter(n => {
      const qn = n._prefix ? n._prefix + ':' + n._localName : n._localName;
      return htmlDoc
        ? (n._ns === HTML_NS ? asciiLower(qn) === qLower : qn === q)
        : qn === q;
    }));
  }
  ...
  // roots, not `this`.
  querySelector(sel)    { return selectFirst(this._children, sel, this); }
  querySelectorAll(sel) { return selectAll(this._children,   sel, this); }
  ...
  getElementsByTagNameNS(_ns, local) { return this.getElementsByTagName(local); }
  getAttributeNodeNS(_ns, name) { return this.getAttributeNode(name); }
  // `setAttributeNode(attr)` / `setAttributeNodeNS(attr)` — read the
  // Attr's name/value and route through `setAttribute`. Per spec the
  // return is the previously-set Attr for that name (or null).
  setAttributeNode(attr) {
    if (!attr) return null;
    const name = String(attr.name != null ? attr.name : '').toLowerCase();
    const prior = Object.prototype.hasOwnProperty.call(this._attrs, name) ? makeAttr(this, name) : null;
    this.setAttribute(name, attr.value != null ? attr.value : '');
    // Per spec the attached Attr's `ownerElement` points at this
    // element. createAttribute returns a detached Attr-shaped object;
    // wiring ownerElement here completes the create → set → attach
    // round trip (matches real browsers).
    attr.ownerElement = this;
    return prior;
  }
  setAttributeNodeNS(attr) { return this.setAttributeNode(attr); }
  // `removeAttributeNode(attr)` — remove the named attribute and
  // return the Attr that was removed (snapshot taken before removal).
  removeAttributeNode(attr) {
    if (!attr) return null;
    const name = String(attr.name != null ? attr.name : '').toLowerCase();
    const removed = Object.prototype.hasOwnProperty.call(this._attrs, name) ? makeAttr(this, name) : null;
  ...
  get protocol() { const u = anchorURL(this); return u ? u.protocol : ''; }
  ...
  get relList() { return makeTokenList(this, 'rel'); }
...
// Live DOMTokenList over a space-separated content attribute, backing
// `relList` (and any other reflected token list). Cached per (element,
// attribute) so the IDL `[SameObject]` identity holds.
function makeTokenList(el, attr) {
  return tokenListFor(el, attr);
}
...
class Document extends Node {
  ...
  createElement(tag) {
    ...
    let el;
    // Custom-element upgrade is keyed on the (lowercased) name and only for the
    // HTML namespace.
  ...
  // Preact's `z` takes this path for SVG (Forem's crayons_icon_tag icons);
  // the matcher / cascade / event paths still key off the lowercased `_tag`,
  // so SVG-via-createElementNS keeps matching — while namespaceURI / prefix /
  // localName / tagName now reflect the real namespace.
  createElementNS(namespace, qualifiedName) {
    const ns = namespace == null ? null : String(namespace);
    const qn = String(qualifiedName);
    const { namespace: rns, prefix, localName } = validateAndExtract(ns, qn);
    let el;
    // Custom-element upgrade applies only to the HTML namespace (DOM "create an
    // element"); a same-named localName in another namespace stays inert.
    const ctor = rns === HTML_NS ? getCustomElementCtor(localName.toLowerCase()) : null;
    if (ctor) {
      const prev = __currentTag;
      __currentTag = localName.toLowerCase();
      try { el = new ctor(); } finally { __currentTag = prev; }
  ...
  querySelector(sel)    { return this.documentElement ? selectFirst([this.documentElement], sel, this.documentElement) : null; }
  querySelectorAll(sel) { return this.documentElement ? selectAll([this.documentElement],   sel, this.documentElement) : []; }
  ...
  getElementsByClassName(cls) {
    ...
    const has = (el) => wanted.every(c => classes(el).includes(c));
  ...
  // Encoding accessors — this DOM is always UTF-8. `charset` and
  ...
  // `createAttribute(name)` returns a detached Attr (no owner element,
  // empty value). makeAttr reads an element's attr store, so build the
  // Attr-shaped object directly for the orphan case.
  createAttribute(name) {
    const n = String(name).toLowerCase();
    return {
      nodeName: n, name: n, namespaceURI: null, prefix: null,
      localName: n, value: '', nodeType: 2, specified: true,
      ownerElement: null, textContent: ''
    };
  }
  createAttributeNS(_ns, name) { return this.createAttribute(name); }
...
function makeAttr(el, key) {
  // `key` is the stored qualified-name key; namespace metadata (for attrs set
  // via setAttributeNS) lives in `_attrNS`, defaulting to the no-namespace case.
  const meta = el._attrNS && el._attrNS[key];
  const prefix    = meta ? meta.prefix : null;
  const localName = meta ? meta.localName : key;
  return {
    name:      key,                       // qualified name
    nodeName:  key,
    value:     el._attrs[key],
    nodeValue: el._attrs[key],
    specified: true,
    namespaceURI: meta ? meta.ns : null,
    prefix,
    localName,
    ownerElement: el,
    ownerDocument: el.ownerDocument || globalThis.document,
