...
// HTML "auto directionality": first strong char in tree order over `root`'s text
// (Text + CDATASection) descendants, descending only through descendants that
// don't establish their own direction. Defaults 'ltr'.
function autoDirectionality(root) {
  const stack = (root._children || []).slice().reverse();
  while (stack.length) {
    const n = stack.pop();
    const nt = n.nodeType;
    if (nt === NODE_ELEMENT) {
...
class Element extends Node {
  ...
  // Light-tree only: flat-tree / slot directionality is not yet modelled.
  _directionality() {
    let cur = this;
