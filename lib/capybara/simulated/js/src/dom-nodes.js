...
import { serializeChildren, serializeChildrenWithShadow, serializeElement, escapeText } from './html-parser.js';
...
// True iff the sequential-focus `order` is already in document (tree) order —
// i.e. no positive-`tabindex` reordering moved any stop out of tree position.
// Only then can __csimAdvanceFocus locate a non-stop element's continuation by
// document position (compareDocOrder).
...
globalThis.__csimAdvanceFocus = function (reverse) {
  ...
  // `current` is focused but is NOT a sequential stop — e.g. script moved focus
  // into a tabindex=-1 host's shadow tree (excluded from the order), or onto a
  // non-tabbable element. Continue from its DOCUMENT POSITION rather than
  // wrapping to the ends: the next stop after it (forward) / the last stop before
  // it (backward), by tree order. compareDocOrder follows the ShadowRoot→host
  // chain, so a focused shadow-internal node compares at its host's flat-tree
  // position. (focus-tabindex-order-shadow-varying-tabindex-2)
  //
  // Only valid when `order` itself is in document order: positive `tabindex`
  // makes __seqEmit reorder stops (positives ahead of 0), so document position
  // no longer matches sequential position and we can't locate the continuation
  // by tree order — fall back to the old wrap there (no worse than before).
  if (idx === -1 && current && isConnected(current) && orderIsInDocumentOrder(order)) {
    if (reverse) {
      ...
      idx = pick === -1 ? order.length - 1 : pick;   // none before → wrap to last
    } else {
      let pick = -1;
      for (let i = 0; i < order.length; i++) { if (compareDocOrder(current, order[i]) < 0) { pick = i; break; } }
      idx = pick === -1 ? 0 : pick;                  // none after → wrap to first
    }
  } else {
