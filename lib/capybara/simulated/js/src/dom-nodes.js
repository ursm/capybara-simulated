...
class Element extends Node {
  ...
  // Whether attribute `name` sources this image element's bitmap — so a set / remove of
  // it (setAttribute[NS] / removeAttribute[NS]) re-fetches. `name` is the qualified name
  // (`xlink:href`), which is how both `_attrs` and setAttribute reflect it.
  _isImageResourceAttr(name) {
    if (this._tag === 'img')   return name === 'src';
    if (this._tag === 'image') return name === 'href' || name === 'xlink:href';
    return false;
  }
  _loadImageResource() {
    ...
    if (src == null || src === '') {
      ...
      this._imgLoadedSrc  = undefined;
      this._imgComplete   = true;
      this._imgBroken     = false;   // no request → unavailable, not broken
      return;
    }
    if (this._imgLoadedSrc === src) return;   // already handled this src (idempotent)
    this._imgLoadedSrc = src;
  ...
  // hands its fetch handler for an <img>: destination 'image', and a mode/credentials pair set by
  // `crossorigin` — absent → no-cors + credentials 'include'; 'anonymous' → cors + 'same-origin';
  ...
  _loadImageViaServiceWorker(ctrl, src) {
    ...
    const swReq = {
      ...
      credentials:    !cors ? 'include' : (String(co).toLowerCase() === 'use-credentials' ? 'include' : 'same-origin'),
      destination:    'image',
      referrerPolicy: 'strict-origin-when-cross-origin',
      referrerSource: (globalThis.location && globalThis.location.href) || ''
    };
    globalThis.__csimSWInterceptFetch(ctrl, 'GET', swUrl, {Accept: 'image/*,*/*;q=0.8'}, null, false, swReq, swResp => {
      // A `src=` reassignment (or clear) while this fetch was in flight superseded it — drop the
      // stale response so it can't overwrite the newer request's bitmap or fire a spurious event.
      if (this._imgLoadedSrc !== src)  return;
      ...
      this._applyImageResponse(swResp, src);                                          // respondWith bytes
    });
  }
  ...
  // async drain a streamed body adds, so a late reassignment still wins.
  _applyImageResponse(swResp, src) {
    const apply  = r   => { if (this._imgLoadedSrc === src) this._applyImageResult(r); };
    const decode = b64 => {
      let r = null;
    ...
    decode(swResp.body_b64 || null);
  }

  // Fetch + decode `src` host-side (libvips, memoized per URL) and apply the result — the
  // synchronous fast path for an uncontrolled document.
  _finishImageLoadFromNetwork(src) {
    let r = null;
    try { r = globalThis.__csim_loadImage(src); } catch (_) { /* host fault → broken */ }
  ...
  get disabled() {
    ...
    // content attribute — a null sheet reads false. (HTMLLinkElement.disabled has extra parse-time
    // / explicitly-enabled semantics not modelled here, so `<link>` keeps content-attr reflection.)
