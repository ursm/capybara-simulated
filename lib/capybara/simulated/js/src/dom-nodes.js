...
class Element extends Node {
  ...
  setAttribute(name, value) {
    ...
    this._attrs[n] = next;
    // A control's name/id becomes a form named-property — register the
    // getter on the shared FormNamedProto (no-op for non-control tags or
    // already-seen names; see registerFormName).
  ...
  _setAttrNodeValue(key, value) {
    ...
    if ((key === 'id' || key === 'name') && value) registerFormName(this, value);   // Attr#value mutation feeds form named access
  ...
  _bindAttrNode(attr) {
    ...
    if (!hasNs && (key === 'id' || key === 'name') && attr._value) registerFormName(this, attr._value);   // setAttributeNode feeds form named access
...
const { parseDocument, parseFragment } = installHtmlParser({ Document, Element, Text, Comment, DocumentFragment, DocumentType, createHtmlPageDocument, registerFormName });
