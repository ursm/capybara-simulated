...
const { parseDocument } = installHtmlParser({ Document, Element, Text, Comment, DocumentFragment, DocumentType, createHtmlPageDocument, registerNamedAccess });
...
// into the live document, reusing its skeleton); `parse5ParseFragment` backs
// `parseFragment` below (innerHTML / outerHTML / insertAdjacentHTML /
// createContextualFragment), context-aware. The hand-rolled `parseDocument`
// stays for the `<html>`-innerHTML / DOMParser fallback (and as an A/B
// reference) until parse5 is fully validated, then html-parser.js is removed.
const { parse5ParseIntoLive, parse5ParseFragment } = installParse5Adapter({
...
// context element's insertion mode (table / select / raw-text, …). Replaces the
// hand-rolled regex parser; callers thread the spec fragment context.
