...
function normalizeAdoptedStyleSheets(v) {
  ...
  // CSSOM: only a *constructed* stylesheet may be adopted; an owned `<style>`/`<link>`
  // sheet (`_constructed === false`) is a NotAllowedError. Non-sheet-shaped members
  // are dropped leniently (nothing downstream depends on strict typing).
  for (const s of arr) {
    if (s && s._constructed === false) {
