...
function listOfOptions(select) {
  ...
  const walk = (node, optgroupDepth) => {
    for (const c of node._children) {
      ...
      if (t === 'optgroup') {
        if (optgroupDepth >= 1) continue;                       // nested optgroup → barrier
        walk(c, optgroupDepth + 1);
      } else {
        walk(c, optgroupDepth);                                 // transparent wrapper (e.g. <div>)
      }
    }
  };
  walk(select, 0);
...
function tableRows(table) {
  ...
  for (const c of table._children) {
    if (c.nodeType !== NODE_ELEMENT) continue;
    if (c._tag === 'thead')      for (const r of c._children) { if (r.nodeType === NODE_ELEMENT && r._tag === 'tr') head.push(r); }
    else if (c._tag === 'tfoot') for (const r of c._children) { if (r.nodeType === NODE_ELEMENT && r._tag === 'tr') foot.push(r); }
    else if (c._tag === 'tbody') for (const r of c._children) { if (r.nodeType === NODE_ELEMENT && r._tag === 'tr') body.push(r); }
...
class Element extends Node {
  ...
  deleteCaption() { const c = elementChildrenByTag(this, 'caption')[0]; if (c) c.remove(); }
  createTHead() {
    const existing = elementChildrenByTag(this, 'thead')[0];
    if (existing) return existing;
  ...
  deleteTHead() { const t = elementChildrenByTag(this, 'thead')[0]; if (t) t.remove(); }
  createTFoot() {
    const existing = elementChildrenByTag(this, 'tfoot')[0];
    if (existing) return existing;
  ...
  deleteTFoot() { const t = elementChildrenByTag(this, 'tfoot')[0]; if (t) t.remove(); }
  createTBody() {
  ...
  get rowIndex() {
    ...
    return table ? tableRows(table).indexOf(this) : -1;
  }
  get sectionRowIndex() {
    if (this._tag !== 'tr') return undefined;
    const p = this.parentNode;
    if (!(p instanceof Element)) return -1;
    if (TABLE_SECTION_TAGS.has(p._tag) || p._tag === 'table') return elementChildrenByTag(p, 'tr').indexOf(this);
    return -1;
  }
