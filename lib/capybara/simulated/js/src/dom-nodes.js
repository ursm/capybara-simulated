...
class Element extends Node {
  ...
  set outerHTML(html) {
    ...
    const nodes = parseFragment(html === null ? '' : String(html));
...
// reference w3c-xmlserializer port). `requireWellFormed` is always off here
// (serializeToString never throws on well-formedness), so the WF checks are
// omitted.
//
...
function xmlSerSerializeNode(node, namespace, prefixMap, refs) {
  switch (node.nodeType) {
    ...
    case NODE_TEXT:     return escapeText(node._data == null ? '' : node._data);
