...
import { ceState, getCustomElementCtor, ceUpgradeTree, fireCEDisconnect, fireCEMoveReactions, fireAttrChangedCallback, askForReset, askForResetBatch, askForResetAfterRemoval, runSelectednessAlgorithm, finalizeSelectOptions, ensureOptionSelInit } from './custom-elements.js';
...
globalThis.__csimSubmitFormToNamedFrame = function (form, submitter, entryList) {
  ...
  let spec;
  try { spec = globalThis.__csimFormSerialize(form._id, submitter ? submitter._id : 0); } catch (_) { return false; }
...
class Element extends Node {
  ...
  set type(v) {
    // Whether selection applied under the OLD type, before the change.
    const wasSelectable = this._tag === 'input' && this.__selectionApplies();
    const prevMode = this._tag === 'input' ? inputValueMode(this.type) : null;
    this.setAttribute('type', String(v));
    if (this._tag === 'input') {
      // HTML "input type change" value-mode migration: move the value between the
      // live value (`_value`) and the `value` content attribute as the mode
      // changes. Without this the live/attribute split would strand a dirty value
      // in `_value` when switching INTO a mode whose `value` reflects the
      // attribute (e.g. text "foo" → checkbox should expose "foo" via the value
      // attribute, not the stale 'on' default).
      const newMode = inputValueMode(this.type);
      if (prevMode === 'value' && (newMode === 'default' || newMode === 'default/on')) {
        const lv = controlLiveValue(this);
        if (lv !== '') this._attrs.value = lv;   // value → content attribute
        delete this._value;
      } else if (prevMode !== 'value' && newMode === 'value') {
        delete this._value;   // value tracks the content attribute; dirty flag cleared
      } else if (prevMode !== 'filename' && newMode === 'filename') {
        delete this._value;   // value → "" (file value derives from `_files`)
      }
      // Then re-run the value-sanitization algorithm on the (live) value for the
      // new type (so e.g. text → number drops a non-numeric value to ''). A clean
      // value tracks the attribute and is sanitized on read by `get value`, so
      // re-sanitize only when the dirty value flag is still set.
      if (this._value !== undefined) {
        this._value = sanitizeInputValue(this, this.type, this._value);
      }
      // Becoming selectable from a non-selectable type starts the selection at
      // [0, 0] / 'none' (any cursor offset left by a value write while
      // non-selectable is discarded).
      if (!wasSelectable && this.__selectionApplies()) {
        this._selectionStart     = 0;
        this._selectionEnd       = 0;
        this._selectionDirection = 'none';
      }
      // HTML "input type change": if the element is now a CHECKED radio, enforce
      // the group's single-checked invariant — morphing a checked control into a
      // radio unchecks the other members of its group.
      if (this.type === 'radio' && getCheckedness(this)) uncheckOtherRadios(this);
    }
  }
  ...
  showPicker() {
    ...
    if (isActuallyDisabled(this)) {
      throw new globalThis.DOMException("Failed to execute 'showPicker' on 'HTMLInputElement': The input element is disabled.", 'InvalidStateError');
    }
    if (READONLY_INPUT_TYPES.has(type) && this._attrs.readonly != null) {
    ...
    if (!globalThis.__csimTransientActivation) {
      throw new globalThis.DOMException("Failed to execute 'showPicker' on 'HTMLInputElement': HTMLInputElement::showPicker() requires a user gesture.", 'NotAllowedError');
    }
  ...
  get validity() {
    ...
    if (tag === 'select') {
      ...
      empty = checkable ? !getCheckedness(this) : val === '';
    }
    ...
    if (tag === 'input' || tag === 'textarea' || tag === 'select') {
      ...
      // programmatic `.value` set (which marks the dirty-value flag but is not
      // a user edit). The constraints tests set values via the IDL setter, so
      // these must stay false there.
      if (!empty && tag === 'input' && this._dirtyByUser) {
  ...
  get files() {
    ...
    // Wrap in a real FileList (item/length on the prototype). Cache it keyed on
    // the backing array reference so repeated reads return the same object until
    // the selection is replaced (set files / __csimSetFiles reassign _files).
    if (!this._fileList || this._fileListArr !== arr) {
      this._fileList    = new globalThis.FileList(arr);
      this._fileListArr = arr;
    }
