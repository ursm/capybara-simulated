...
function sanitizeInputValue(el, type, v) {
  ...
  switch (type) {
    ...
    case 'email':
      // Multiple: split on commas, strip ASCII whitespace around each token,
      // rejoin with a single ','. Single: strip newlines, then strip leading /
      // trailing ASCII whitespace (as for url). Re-evaluated on every read, so
      // toggling the `multiple` attribute re-sanitizes for free.
      return el._attrs.multiple != null
        ? v.split(',').map((t) => t.replace(/^[\t\n\f\r ]+|[\t\n\f\r ]+$/g, '')).join(',')
        : stripNewlines(v).replace(/^[\t\n\f\r ]+|[\t\n\f\r ]+$/g, '');
    case 'number':
