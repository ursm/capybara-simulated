...
import { isContenteditable, formNamedAccess, toggleChecked, setRadio, checkedRadioInGroup, isSubmitButton, formForControl, closeDialog, enclosingLabelFor, labeledControlFor, LABELABLE, isInteractiveForLabel } from './form-helpers.js';
...
class DOMImplementation {
  ...
  createHTMLDocument(...args) {
    ...
    // A createHTMLDocument document has no browsing context: its URL is
    // "about:blank" and `location` is null (DOM §4.5.1).
    d._url = 'about:blank';
...
class DocumentOrderRange {
  constructor() {
    ...
    // (current global object's associated document, 0) — NOT null.
    // `new Range()` / `document.createRange()` are therefore collapsed
    // at the document, and the attribute getters reflect that.
...
export function deleteRangeContents (range) {
  ...
  if (sc === ec && sc && sc.nodeType === NODE_TEXT) {
    ...
    // Remove the contained children in tree order (start → end). Snapshot the
    // references first so `removeChild`'s index shifts don't skip nodes. Tree
    // order is observable: each removal queues its own childList record, and
    // the spec ("remove" per node) — like browsers — fires them first-to-last.
...
const { parseDocument, parseFragment } = installHtmlParser({ Document, Element, Text, Comment, DocumentFragment, DocumentType });
