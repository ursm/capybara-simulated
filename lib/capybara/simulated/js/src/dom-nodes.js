    ...
    try {
      ...
      if (!ev.defaultPrevented && !didSubmit && !selfActivated &&
        ...
        if (!fragmentNavigate(this)) {
          const tgt = this._attrs.target || '';
          if (!routeChildRealmLinkNav(this, tgt)) {
            const pn = { url: String(this._attrs.href), target: tgt };
            // A blob: link opening a NEW window takes its blob reference NOW (the
            // navigation is deferred and the page may revoke the URL first) —
            // snapshot the bytes so the aux window still loads after revoke.
...
function routeChildRealmLinkNav(anchor, target) {
  ...
  if (!(t === '' || t === '_self')) return false;   // named/_blank/_top from a frame: not routed (no in-scope need)
...
globalThis.__csimSubmitFormToNamedFrame = function (form, submitter, entryList, fromSubmitMethod) {
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      // name/value params.
      const reqBody = enctype.indexOf('multipart/form-data') === 0
        ? (entryList || new globalThis.FormData(form, submitter || undefined))
        : (entryList ? entryListParams(entryList) : formEntryParams(spec));
...
function formEntryParams(spec) {
  ...
  for (const pair of (spec.fields || [])) params.append(String(pair[0]), String(pair[1]));
  for (const fi of (spec.fileInputs || [])) {
    const f = fi.files && fi.files[0];
    params.append(String(fi.name), f ? String(f.name || '') : '');
  }
  return params;
}
...
function entryListParams(entryList) {
  ...
  for (const [name, value] of entryList) {
    params.append(String(name), (File && value instanceof File) ? String(value.name || '') : String(value));
  }
  return params;
}
