  ...
  return new Proxy(doc, {
    ...
    getOwnPropertyDescriptor(t, p) {
      ...
      if (typeof p === 'string' && DOC_NAMED_PROPS.has(p)) {
        ...
        if (els.length) {
          // enumerable:FALSE is a deliberate bounded divergence from the spec /
          // Chrome (where Document named props ARE enumerable — Document, unlike
          // Window, has no [LegacyUnenumerableNamedProperties]). Marking them
          // enumerable makes Object.keys / for-in / spread / our structuredClone
          // walker read `document.<iframeName>`, whose value is the live
          // contentWindow; that walker then recurses through the per-realm window
          // graph (window → document → iframe → contentWindow → …) and OOMs. The
          // WPT-tested and app-relevant surface — getOwnPropertyNames, via ownKeys
          // — lists them regardless of this flag, so conformance is unaffected; no
          // app depends on named props showing in Object.keys/for-in.
