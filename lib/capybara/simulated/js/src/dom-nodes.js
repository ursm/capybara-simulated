  ...
  execCommand(command, _showUI, value) {
    ...
    if (cmd === 'copy') {
      ...
      if (active && (active._tag === 'input' || active._tag === 'textarea')) {
        const v = String(controlLiveValue(active) || '');
    ...
    const isTextControl = active._tag === 'textarea' ||
      (active._tag === 'input' && /^(text|search|email|url|tel|password)?$/i.test(active._attrs.type || ''));
    if (isTextControl && (cmd === 'inserttext' || cmd === 'cut' || cmd === 'paste')) {
      ...
      const splice = (insert, caret, inputType, data, dataTransfer) => {
        active._value = cur.slice(0, ss) + insert + cur.slice(se);
        active.selectionStart = active.selectionEnd = caret;
        try {
          dispatchEvent(active, new globalThis.InputEvent('input', {
            bubbles: true, cancelable: false, composed: true, data, inputType, dataTransfer: dataTransfer || null
          }));
        } catch (_) {}
      };
      if (cmd === 'inserttext') {
        const s = value == null ? '' : String(value);
        splice(s, ss + s.length, 'insertText', s);
      } else if (cmd === 'cut') {
        globalThis.__csimClipboardSet(cur.slice(ss, se));
        splice('', ss, 'deleteByCut', null);
      } else {   // paste
        ...
        splice(clip, ss + clip.length, 'insertFromPaste', clip, null);
      }
      return true;
    }
    ...
    if (active._attrs.contenteditable != null && (active._attrs.contenteditable || '').toLowerCase() !== 'false') {
      ...
      switch (cmd) {
        ...
        case 'cut': {
          // Copy the selection to the clipboard, delete it, and fire deleteByCut.
          const sel = globalThis.getSelection && globalThis.getSelection();
          const range = sel && sel._ranges.length ? sel._ranges[0] : null;
          globalThis.__csimClipboardSet(sel && typeof sel.toString === 'function' ? sel.toString() : '');
        ...
        case 'paste': {
          ...
          // dataTransfer is a READ-ONLY DataTransfer carrying the pasted text — getData
          // reads it back; setData / clearData are no-ops (an input event's dataTransfer
