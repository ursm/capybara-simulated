...
import { walk, walkFind, walkSubtree, walkInclShadow, isConnected, scriptText, findById, setTemplateDocResolver, setSlotResolver } from './walk.js';
...
class Element extends Node {
  ...
  setAttribute(name, value) {
    ...
    if (old !== next && this._tag === 'source' && (n === 'srcset' || n === 'src') &&
        this._parent && this._parent._tag === 'picture') {
      for (const sib of this._parent._children || []) {
        if (sib.nodeType === 1 && sib._tag === 'img') { try { sib._loadImageResource(); } catch (_) {} }
      }
    }
    ...
    if (this._tag === 'img') {
      ...
      if (p && p._tag === 'picture') {
        for (const c of p._children || []) {
          if (c.nodeType === 1 && c._tag === 'source') {
            const cand = firstCandidate(c._attrs && c._attrs.srcset);
            if (cand) return cand;
          }
        }
      }
