...
class Node {
  ...
  click() {
    try {
      ...
      // ancestor between `this` and the label. (We deliberately do NOT
      // gate on the `__csimPending*` navigation/submit slots: those are
      // drained by Ruby, not within a pure-JS run, so a stale anchor-click
      // intent from an earlier sequence would otherwise wedge the hop
      // shut.) NOTE: unlike the UA path we intentionally skip the case of
      // `this` itself being the `<label>` (a direct `labelEl.click()`):
      // the spec/WPT activation cases always click a descendant, and no
      // app drives a label element through the IDL click() path.
      if (!ev.defaultPrevented &&
          this._tag !== 'label' &&
