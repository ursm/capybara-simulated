...
// HTML "valid floating-point number" — a pure grammar check. (Don't reject on
// Number.isFinite: '1e999' is a valid floating-point number whose value is
// Infinity, and browsers keep it; the regex already excludes NaN/Infinity
// literals and other non-numeric text.)
function isValidFloatingPoint(s) {
  return /^-?(?:\d+(?:\.\d+)?|\.\d+)(?:[eE][-+]?\d+)?$/.test(s);
}
// Minimal per-type temporal validity (date/time/etc.) — enough to keep VALID
// values and drop invalid ones to '' (the precise calendar-range checks aren't
// needed for conformance here).
const TEMPORAL_RE = {
  date:               /^\d{4,}-\d{2}-\d{2}$/,
  time:               /^\d{2}:\d{2}(?::\d{2}(?:\.\d{1,3})?)?$/,
  // A valid local date and time string allows a 'T' OR a U+0020 SPACE separator.
...
function sanitizeInputValue(el, type, v) {
  ...
  switch (type) {
    ...
    case 'date': case 'time': case 'datetime-local': case 'month': case 'week':
      return TEMPORAL_RE[type].test(v) ? v : '';
    default:
