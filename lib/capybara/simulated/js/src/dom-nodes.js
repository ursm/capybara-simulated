...
class Node {
  ...
  __runFormSubmit(submitter, fromSubmitMethod) {
    ...
    if (!fromSubmitMethod) {
      ...
      if (!isConnected(this)) return false;   // step 5h: re-check after the submit event
    }
    // Steps 6-7: construct the entry list. `new FormData(form, submitter)` is HTML's
    // "construct the entry list": its ctor fires `formdata` and sets/clears the form's
    // constructing-entry-list flag around it, so a submit re-entered from a handler
    // bails at step 2 above. Keep the result so the submission uses THIS list (firing
    // `formdata` exactly once and honouring handler mutations) rather than re-walking.
    const entryList = new globalThis.FormData(this, submitter || undefined);
    if (!isConnected(this)) return false;   // step 8: a `formdata` handler that removed the form aborts the submit
    recordFormSubmission(this, submitter || null, entryList);
    return true;
  }
