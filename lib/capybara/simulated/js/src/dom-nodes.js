...
// Template content is (spec-wise) another DOCUMENT sharing our document object:
...
// actual template-content traffic. Runs on BOTH insert paths — adoptIntoParent
// (single-node pre-insert) and _insertFragmentChildren (the fragment splice,
// which skips adoptIntoParent). A cloned CONTENT FRAGMENT has no slot of its
...
class Node {
  ...
  _insertFragmentChildren(frag, ref) {
    ...
    for (const c of moved) { registryOnInsert(this, c); c._parent = this; registerSubtree(c); runSourceInsertionStep(c, this); }
...
class Element extends Node {
  ...
  get content() {
    if (this._tag === 'template') {
      if (!this._templateContent) { this._templateContent = new DocumentFragment(); this._templateContent._ceRegistry = TRACKING_NULL; }
  ...
  set innerHTML(html) {
    ...
    if (this._tag === 'template') {
      ...
      for (const c of parsed) {
        ...
        registerSubtree(c);
        // The parsed nodes live in the template-contents "document": stamp the
        // tracking sentinel (same state the parser's setTemplateContent path
        // gives parse-time content) unless a stickier association applies.
        walkSubtree(c, n => { if (n.nodeType === NODE_ELEMENT && n._ceRegistry === undefined) n._ceRegistry = TRACKING_NULL; });
      }
...
const { parse5ParseIntoLive, parse5ParseFragment, parse5ParseDocument } = installParse5Adapter({
  ...
  syncInlineEventHandler, finalizeElementProto
});
