...
class Element extends Node {
  ...
  setAttribute(name, value) {
    ...
    if (n === 'src' && this._tag === 'img' && old !== next) this._loadImageResource();
  ...
  removeAttribute(name) {
    ...
    if (Object.prototype.hasOwnProperty.call(this._attrs, key)) {
      ...
      // Removing an <img>'s src discards the current request (resets intrinsic size
      // + bitmap), mirroring the `src=''` path in the setter.
      if (this._tag === 'img' && key === 'src') this._loadImageResource();
      return;
    }
  ...
  _loadImageResource() {
    if (this._tag !== 'img') return;
    // A realm without the host decoder wired (an early-boot / stripped build) keeps
    // the pre-feature behavior: <img> stays inert (no load, no error), rather than
