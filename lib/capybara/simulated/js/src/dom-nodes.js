...
class Node {
  ...
  getBoundingClientRect() {
    ...
    if (!isVisibleNode(this)) return new globalThis.DOMRect(0, 0, 0, 0);
    if (this._layoutY == null) this._layoutY = nextLayoutY();
    return new globalThis.DOMRect(0, this._layoutY, 1, 1);
  }
  getClientRects() {
    if (!isVisibleNode(this)) return [];
    if (this._layoutY == null) this._layoutY = nextLayoutY();
    return [new globalThis.DOMRect(0, this._layoutY, 1, 1)];
  }
...
// measured. Reset on document reload (`resetLayoutY` is exposed and
// called from `__csimLoadDocument`). Element instances cache their
// own assignment on `_layoutY`.
let __layoutYSeq = 0;
export function nextLayoutY() { return __layoutYSeq++; }
export function resetLayoutY() { __layoutYSeq = 0; }
globalThis.__csimResetLayoutY = resetLayoutY;
