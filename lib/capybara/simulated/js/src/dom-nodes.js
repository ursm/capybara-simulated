...
class Node {
  ...
  // No real layout, but the visibility BINARY is observable: a laid-out element
  // has offsetTop > 0, a display:none / hidden one has 0 (mirrors offsetWidth and
  // the getBoundingClientRect Y model). Reuse getBoundingClientRect's monotonic
  // document-order layout-Y, +1 so a laid-out element is strictly > 0 — the
  // first-measured element's layout-Y is 0 (kept 0-based because the header
  // measured at boot must land at gBCR.top 0; see _ensureLayoutY). The +1 means
  // offsetTop is deliberately 1 greater than gBCR.top; both are coarse
  // approximations (no pixel layout), so this skew is immaterial — only the >0
  // binary matters. An unrendered slottable — a light child of a shadow host not
  // assigned to a slot, so not in the flat tree — has no layout box, so offsetTop
  // is 0 even though `display` isn't none; scoped to offsetTop (not the shared
  // visibility predicate, which a broader flat-tree check regressed 65 slot
  // subtests). The `_shadowRoot` gate skips non-hosts; only an actual shadow
  // host's light children pay the slot lookup.
  get offsetTop() {
    const p = this._parent;
    if (p && p._shadowRoot && isSlottableNode(this) && !flatTreeAssignedSlot(this)) return 0;
    const y = this._ensureLayoutY();
    return y == null ? 0 : y + 1;
  }
  ...
  scrollIntoView(_opts) {
    ...
    let moved = false;
    if (root && typeof y === 'number' && y !== (root._scrollTop || 0)) {
      root._scrollTop = y;
      moved = true;
    }
...
class Document extends Node {
  ...
  elementFromPoint(x, y) {
    // CSSOM-View: negative (or out-of-viewport) coordinates return null.
