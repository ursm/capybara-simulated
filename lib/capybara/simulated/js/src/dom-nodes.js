...
class Node {
  ...
  __runFormSubmit(submitter, fromSubmitMethod) {
    ...
    // `formdata`), re-check, plan the navigation with that list.
    return constructEntryListAndRecord(this, submitter);
  }
...
export function recordFormSubmission(form, submitter, entryList) {
  if (globalThis.__csimSubmitFormToNamedFrame && globalThis.__csimSubmitFormToNamedFrame(form, submitter || null, entryList)) return true;
...
export function constructEntryListAndRecord(form, submitter) {
  ...
  recordFormSubmission(form, submitter || null, entryList);
  return true;
}
...
globalThis.__csimSubmitFormToNamedFrame = function (form, submitter, entryList) {
  ...
  // HTML "plan to navigate": a form has at most ONE planned navigation, and a new
  // submission REPLACES (removes) the prior one — last submission wins. So a
  // `form.submit()` in a click handler is superseded by the button's default submit
  // in the same task (form-double-submit.html). We keep the fetch synchronous (so a
  // single submit is unchanged) and tag it with a per-form token; superseding marks
  // the prior token cancelled and clears its frame's pending flag, and the prior
  // fetch's continuation then skips navigating that frame.
  const prevPlan = form._plannedNav;
  if (prevPlan) {
    prevPlan.cancelled = true;
    if (prevPlan.frameEl) prevPlan.frameEl._frameNavPending = false;
  }
  const plan = { cancelled: false, frameEl };
  form._plannedNav = plan;
  const land = (url, body, contentType) => {
    if (plan.cancelled) return;            // superseded by a later submission of this form
    if (form._plannedNav === plan) form._plannedNav = null;
        ...
        if (!Number.isNaN(num)) {
          ...
          if (!(stepAttr != null && String(stepAttr).toLowerCase() === 'any')) {
            ...
            if (stepValue > 0) {
              ...
              // small tolerance for float types; exact for the integer-ms temporals
              const tol = (type === 'number' || type === 'range') ? 1e-9 * Math.max(1, Math.abs(stepValue)) : 0;
              if (rem > tol && Math.abs(rem - stepValue) > tol) v.stepMismatch = true;
            }
          }
        }
      }
