...
class Document extends Node {
  ...
  _createElement(ns, prefix, localName, registry, isValue) {
    ...
    if (ctor) {
      ...
      // throws, REPORT the exception (don't propagate to the createElement caller) and
      // fall back to a plain element with this local name (so a customized built-in
      // whose constructor throws still yields e.g. a real `<div>`).
      try { el = new ctor(); }
      catch (e) { try { globalThis.reportError(e); } catch (_) {} el = new Element(localName); }
