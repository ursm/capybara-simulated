...
// parse5-backed HTML parser. The adapter maps parse5's tree ops onto these same
// node ctors. `parse5ParseIntoLive` is the document load path (parse directly
// into the live document, reusing its skeleton). The hand-rolled `parseDocument`
// stays for the fragment / DOMParser paths (and as an A/B reference) until parse5
// is validated across the streaming driver, then it's removed.
const { parse5ParseIntoLive } = installParse5Adapter({
