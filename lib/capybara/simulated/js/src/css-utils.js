...
export function isSupportedCssPropertyName(name) {
  ...
  // A VENDOR-prefixed name mdn doesn't carry is still a property browsers support —
  // `-webkit-font-smoothing` and `-moz-osx-font-smoothing` are in real stylesheets and absent from
  // the data. We can't enumerate what an engine implements, so we don't judge: keeping a
  // declaration we can't confirm is the safer error, and the same rule the VALUE check follows.
...
const LEGACY_VALUES = new Set(['outline-color:invert']);
...
// url needs requoting; and an UPPERCASE letter is a unit or function name a browser folds
...
export function parseStyleDeclList(css) {
  ...
  while (i < n) {
    ...
    const v = value.trim();
    if (name && v[0] !== ':') out.push({ prop: name, value: v });
  }
  return out;
}
