...
// Hand-listed: mdn's grammar for these is `<number> | <length>`, which the generated tables read as
// "numeric", and a percentage is numeric-shaped but not in the grammar.
const PERCENT_INVALID_PROPERTIES = new Set(['tab-size']);
...
// …and a lone DELIMITER, which is no property's value at all, and the cheap test for a math operator or a bare block
// that may stand among the value's top-level components (see `isValidDeclarationValue`).
const LONE_DELIM_RE = /^[^\w\s\u0080-\uffff]$/;
const TOP_LEVEL_MATH_RE = /(?:^|\s)(?:[+\-*](?:\s|$)|\()/;
...
// Where an unquoted `url( … )` opening at `i` ends — past its `)` — or -1 where `i` opens none: the `(` of a `url`
// that `before` ends with, no quote after it. Its content is OPAQUE, so a `/*` in it is part of the URL.
function unquotedUrlEnd(text, i, before) {
  if (text[i] !== '(' || !/(?:^|[^\w-])url$/i.test(before) || /^\s*["']/.test(text.slice(i + 1, i + 3))) return -1;
...
export function stripCssComments(value) {
  ...
  while (i < value.length) {
    ...
    // (…and so is an unquoted `url( … )`, whose `/*` is part of the URL: `url(a/*b*/c.png)` fetched `a c.png`.)
    const url = unquotedUrlEnd(value, i, out);
    if (url !== -1) { out += value.slice(i, url); i = url; continue; }
    if (c === '/' && value[i + 1] === '*') {
...
export function isValidDeclarationValue(name, value) {
  ...
  // A lone DELIMITER — `+`, `-`, `*`, `/` — is a value of no property: one only ever separates or
  // joins other components (`font: 12px/1.5`, `grid-area: 1 / 2`, `calc(a + b)`). Every length
  // grammar let it through as "not a dimension, so perhaps a keyword mdn omits", and a box shorthand
  // spread one onto a SIDE: `margin: 0 min(10px, 26%) + max(5em, 20px)` — a `calc()` written without
  // its `calc()` — kept `margin-bottom: +` and `margin-left: max(5em, 20px)`, where Chrome drops the
  // declaration whole.
  if (LONE_DELIM_RE.test(s)) return false;
  ...
  if (!substituted) {
    ...
    // …and neither is a top-level `+`, `-` or `*` BESIDE other components, nor a bare `( … )` block: those join and
    // group the operands of a math function, and outside one they are a `calc()` written without its `calc()` —
    // `text-indent: 10px + 5px`, `font: 12px + serif`, `margin-left: (10px)`, all dropped by Chrome (measured over
    // twenty properties). A grammar with more than one component took each as a keyword it did not know. (`/` is a
    // separator in `font`, `grid-area`, `border-radius`, and a comma was judged above.)
    if (TOP_LEVEL_MATH_RE.test(s) && splitTopLevelWhitespace(s).some((part) => /^(?:[+\-*]$|\()/.test(part))) return false;
    ...
    // …a `<number> | <length>` property takes no percentage (`tab-size: 50%` computes to `8` in
    // Chrome — dropped, not clamped).
    if (PERCENT_INVALID_PROPERTIES.has(name) && entries.some((entry) => /^\s*[+-]?[\d.]+%\s*$/.test(entry))) return false;
...
const POSITION_AXIS = Object.assign(Object.create(null), {
  ...
  'object-position':       'xy',
  'mask-position':         'xy',
  // …and the two ORIGINS, whose grammar gives each axis exactly ONE component and adds an optional
  // z LENGTH. `transform-origin: right 10px bottom 20px` — the edge-plus-offset form
  // `background-position` takes — is a parse error here (Chrome drops it).
  'transform-origin':      'origin',
  'perspective-origin':    'origin-2d'
});
...
function validPositionValue(entries, axis) {
  if (axis === 'origin' || axis === 'origin-2d') {
    return entries.length === 1 && validOriginValue(entries[0], axis === 'origin');
  }
...
// An ORIGIN: one component per axis, plus — for `transform-origin` only — a third z LENGTH.
//
// The grammar is `[ left | center | right | <lp> ] [ top | center | bottom | <lp> ] <length>?`,
// with ONE reorderable alternative: a pair of KEYWORDS may be written either way round
// (`top left`), and nothing else may. So `top 10px` and `10px left` are parse errors even though
// `left 10px` and `10px top` are not — Chrome drops all three of the first kind. No edge keyword
// binds a following offset either (that is `<bg-position>`'s grammar, not this one).
const ORIGIN_X_KEYWORDS = new Set(['left', 'right']);
const ORIGIN_Y_KEYWORDS = new Set(['top', 'bottom']);
const isOriginKeyword = (tok) => POSITION_EDGE_RE.test(tok);
function validOriginValue(value, allowsZ) {
  const toks = splitTopLevelWhitespace(value.trim()).filter(Boolean).map((t) => t.toLowerCase());
  if (!toks.length || toks.length > (allowsZ ? 3 : 2)) return false;
  if (toks.length === 3 && !isOriginZLength(toks[2])) return false;
  if (toks.length >= 2) {
    const [a, b] = toks;
    if (isOriginKeyword(a) && isOriginKeyword(b)) {
      // Either order, but neither axis twice — `left right` and `top top` name one axis each way.
      if (ORIGIN_X_KEYWORDS.has(a) && ORIGIN_X_KEYWORDS.has(b)) return false;
      if (ORIGIN_Y_KEYWORDS.has(a) && ORIGIN_Y_KEYWORDS.has(b)) return false;
    } else {
      if (ORIGIN_Y_KEYWORDS.has(a)) return false;                    // `top 10px`
      if (ORIGIN_X_KEYWORDS.has(b)) return false;                    // `10px left`
    }
  }
  return true;
}
// The z axis is a LENGTH: no percentage, no keyword — not even inside a `calc()`.
function isOriginZLength(tok) {
  if (tok.indexOf('%') !== -1) return false;
  if (/^calc\(/i.test(tok)) return true;
  return /^[+-]?(?:\d+\.?\d*|\.\d+)(?:e[+-]?\d+)?[a-z]*$/i.test(tok) && !POSITION_EDGE_RE.test(tok);
}
...
// The two axes of an ORIGIN — `transform-origin`, `perspective-origin` — each as the offset its
// keyword resolves to, with the optional third (z) length passed through.
//
// This is NOT `<bg-position>`: an origin's grammar gives each axis ONE component, so an edge
// keyword never binds a following offset. `transform-origin: left 10px` is x=`left`, y=`10px`
// (Chrome: `0px 10px`), where the same text in `background-position` means 10px FROM the left.
export function canonicalizeOrigin(value) {
  if (typeof value !== 'string') return value;
  const toks = splitTopLevelWhitespace(value.trim()).filter(Boolean);
  if (!toks.length || toks.length > 3) return value;
  const z = toks.length === 3 ? toks[2] : null;
  let x = null, y = null;
  const loose = [];
  for (const tok of toks.slice(0, z == null ? toks.length : 2)) {
    const low = tok.toLowerCase();
    if ((low === 'left' || low === 'right') && x == null) x = low;
    else if ((low === 'top' || low === 'bottom') && y == null) y = low;
    else loose.push(tok);
  }
  for (const tok of loose) { if (x == null) x = tok; else if (y == null) y = tok; }
  const axis = (v) => (v == null ? '50%' : POSITION_EDGE_RE.test(String(v).toLowerCase())
                                           ? resolveBgAxis(String(v).toLowerCase(), null) : v);
  const out = [axis(x), axis(y)];
...
export function parseStyleDeclList(src) {
  // A COMMENT is no part of a regular declaration: read as value text it was — `calc(10px /* c */ + 20px)` was
  // mangled and dropped, `padding-left: 1px /* y */` invalid — where Chrome computes 30px and 1px. So a comment
  // between declarations is skipped and one in a value leaves one space behind (`stripCssComments`'s rule)…
  // …except in a CUSTOM property's value, which is the author's token text verbatim, comments and all (Chrome:
  // `--x: a /* c */ b` reads back `a /* c */ b`), and inside an unquoted `url( … )`, where `/*` is part of the URL.
  // Stripping the whole block up front took both.
  const css = String(src);
  const out = [];
  let i = 0;
  const n = css.length;
  const commentEnd = (at) => { const end = css.indexOf('*/', at + 2); return end === -1 ? n : end + 2; };
  const skipBlank = () => {
    for (;;) {
      while (i < n && /\s/.test(css[i])) i++;
      if (css[i] === '/' && css[i + 1] === '*') i = commentEnd(i); else return;
    }
  };
  while (i < n) {
    for (;;) {
      while (i < n && (css[i] === ';' || /\s/.test(css[i]))) i++;
      if (css[i] === '/' && css[i + 1] === '*') i = commentEnd(i); else break;
    }
    if (i >= n) break;
    const nameStart = i;
    ...
    if (css[i] !== ':') continue;
    i++;
    if (custom) while (i < n && /\s/.test(css[i])) i++; else skipBlank();
    let value = '';
    let parenDepth = 0;
    ...
    while (i < n) {
      ...
      if (c === '/' && css[i + 1] === '*' && !custom) {
        ...
        else while (i < n && /\s/.test(css[i])) i++;
        continue;
      }
      // (…an unquoted `url(` runs to its `)` whatever it holds.)
      const url = unquotedUrlEnd(css, i, value);
      if (url !== -1) { value += css.slice(i, url); i = url; continue; }
      if (c === '(') parenDepth++;
