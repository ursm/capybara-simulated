...
// Join an already-final declaration map into block text (`color: red;` — trailing `;`,
// single-space separated, '' when empty). Values are used verbatim; callers whose map
// already holds canonical values (e.g. the cached storeDecls map) use this to avoid
// re-canonicalizing on every read.
export function joinStyleDecls(decls) {
  return Object.entries(decls).map(([k, v]) => k + ': ' + v + ';').join(' ');
}

// CSSOM serialization of a declaration block from RAW values: like joinStyleDecls but
// canonicalizes each value (serializeCssValue). A custom property (`--*`) is a token
// stream preserved verbatim (browsers only trim it), so it is NEVER canonicalized.
// Shared by the `Element.style` / rule.style write paths and cssom.js so all agree.
export function serializeStyleDecls(decls) {
  return Object.entries(decls)
    .map(([k, v]) => k + ': ' + (k.startsWith('--') ? v : serializeCssValue(v)) + ';')
    .join(' ');
}
