...
// The same declarations as a property-keyed map, last value winning.
// PROTOTYPE-LESS: the keys are property names straight from page text / script, and a plain
// object would answer `decls['constructor']` with an Object.prototype member — read as a
// declaration that is present, then handled as if it were a CSS value string.
export function parseStyleDecls(css) {
  const out = Object.create(null);
  for (const { prop, value } of parseStyleDeclList(css)) {
    ...
    // untouched.
    if ((prop === 'all' || out.all !== undefined) && out[prop] !== undefined) delete out[prop];
    out[prop] = value;
  }
  return out;
}
