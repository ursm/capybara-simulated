...
function serializeCssValueUncached(v, raw) {
  ...
  CT.walk(ast, (node) => {
    ...
    // So is a FUNCTION name (`TRANSLATEX(…)` → `translatex(…)`); only the name is rewritten, so a
    // string or custom-property reference inside keeps its case.
    else if (node.type === 'Function' && /[A-Z]/.test(node.name)) {
      edits.push([loc.start.offset, loc.start.offset + node.name.length, node.name.toLowerCase()]);
    }
