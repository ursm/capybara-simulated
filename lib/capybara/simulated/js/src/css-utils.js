...
export function isValidDeclarationValue(name, value) {
  ...
  if (s.indexOf('(') !== -1) {
    // …and how many values it is cannot be decided either while a SUBSTITUTION is unresolved:
    // `padding-left: var(--a) var(--b)` is one `var()` short of an answer, and Chrome stores it
    // (invalid-at-computed-value-time is a different time). The shorthand path guards the same way.
...
function splitTopLevelWhitespace(s) {
  const parts = [];
  let depth = 0, start = 0;
