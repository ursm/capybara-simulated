...
export function htmlDimensionValue(text) {
  ...
  if (!m) return null;
  // Normalised by TRIMMING, not by `Number()`: its exponential form is a value no consumer
  // downstream parses, so `width="0.000000001"` (`1e-9px`) fell through every branch and reported
  // an EMPTY computed value — worse than the imprecise one. Leading zeros and a bare trailing dot
  // are the only things that need removing (`   00523   ` -> `523px`, `200.` -> `200px`).
  const num = m[1].replace(/^0+(?=\d)/, '').replace(/\.$/, '');
  return `${num || '0'}${m[2] || 'px'}`;
}
