...
export function isValidDeclarationValue(name, value) {
  ...
  // leaves `el.style.marginLeft` at '' and writes no attribute. The cascade already drops it; the
  // two surfaces have to agree, or a page that writes a computed `calc()` and reads it back to see
  // whether it applied gets the wrong answer.
  if (isStaticallyInvalidMath(value)) return false;
