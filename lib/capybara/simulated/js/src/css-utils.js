...
//
// It covers the two explicit lists only. `isSupportedCssPropertyName` ALSO accepts a `-webkit-`
// mirror of ANY supported property, and those spellings get no attribute here — the one place the
// two surfaces still disagree. Chrome exposes 151 webkit-cased attributes, and a mirror among them
// is another NAME for the unprefixed property rather than a property of its own
// (`el.style.webkitTransform = 'none'` writes `transform: none`, measured); we model each as
// distinct and accept far more of them. Narrowing the rule to what browsers implement, and making
// those names aliases, is its own correction.
export const CSS_PROPERTY_BY_IDL_ATTRIBUTE = (() => {
  const attributes = { __proto__: null };
  for (const property of [...SUPPORTED_PROPERTY_NAMES, ...VENDOR_EXTRAS]) {
    attributes[cssPropertyToIdlAttribute(property, false)] = property;
    if (property.includes('-'))          attributes[property] = property;
    if (property.startsWith('-webkit-')) attributes[cssPropertyToIdlAttribute(property, true)] = property;
  }
