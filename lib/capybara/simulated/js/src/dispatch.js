...
import { hasShadowRoots, flatTreeAssignedSlot, isSlottableNode } from './dom-nodes.js';
        ...
        if (event instanceof GlobalMouseEvent &&
          ...
          try {
            ...
            if (parsed && !parsed.error && parsed.protocol === 'javascript:') {
              ...
              // Percent-decode byte-wise (HTML "javascript: URL" runs the decoded
              // bytes as UTF-8): decode each maximal run of %XX escapes, leaving a
              // lone/invalid `%` literal — so `f('%41','%')` runs as `f('A','%')`,
              // not the all-or-nothing fallback decodeURIComponent would force.
