  ...
  try {
    ...
    if (!event._propagationStopped && event.bubbles) {
      ...
      for (let i = 1; i < path.length; i++) {
        // Real Ember/Glimmer apps tear down a child component's
        // ancestor modifier (e.g. `{{on "click" @onClick}}` on a
        // parent `<li>`) when the click on a deeper descendant
        // mutates a `@tracked` property the ancestor's `{{#if}}`
        // depends on. The teardown removes the ancestor's listener
        // before the bubble reaches it. Real browsers see this via
        // Backburner's auto-flush between listener invocations;
        // our sync dispatcher otherwise fires the stale listener
        // (Discourse admin-editing schema tree click → parent
        // updateIndex(parentIdx) clobbers the child's
        // activeIndex=0). Flush Backburner explicitly between
        // bubble steps when Ember is present.
        flushEmberRunloop();
        if (event._propagationStopped) break;
...
// Backburner runloop lookup is cached after the first hit; we
// re-probe when the loader is present but the runloop module
// isn't yet (chunk hasn't loaded). When neither is present, this
// is a one-shot no-op.
let __emberBackburner = null;
let __emberLookupTried = false;
function flushEmberRunloop() {
  if (__emberBackburner) {
    try { __emberBackburner.run(noopFlush); }
    catch (_) { /* mid-flush re-entry — skip */ }
    return;
  }
  if (__emberLookupTried) return;
  const loader = globalThis.requirejs || globalThis.require;
  if (!loader || !loader.entries) return;
  // Probe a single key — cheap. If Ember isn't loaded yet, leave
  // the lookup unmarked so a later dispatch can retry.
  if (!loader.entries['@ember/runloop']) return;
  __emberLookupTried = true;
  let mod;
  try { mod = loader('@ember/runloop'); } catch (_) { return; }
  if (mod && mod._backburner) __emberBackburner = mod._backburner;
}
function noopFlush() {}
...
function fireListeners(node, event, capture) {
  ...
  for (const entry of list.slice()) {
    ...
    try { entry.handler.call(node, event); }
