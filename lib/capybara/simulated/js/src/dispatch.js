...
import { toggleChecked, setRadio, checkedRadioInGroup, getCheckedness, setCheckedness, labelToActivateFor, labeledControlFor, isSubmitButton, formForControl } from './form-helpers.js';
...
function dispatchPath(target, event, drain) {
  ...
  try {
    ...
    if (event.type === 'click' && !event._csimActivationHandled &&
      ...
      if (itype === 'checkbox') {
        ckActivation = { kind: 'checkbox', node: actNode, wasChecked: getCheckedness(actNode) };
    ...
    // keeps it single). The activation target is the nearest submit button in the
    // event path — the click target itself, or, when the event BUBBLES, an
    // ancestor (so a click on a `<span>` inside a `<button>` submits). A
    // disconnected or (actually-)disabled button doesn't submit.
    if (!event.defaultPrevented && !event._csimActivationHandled &&
        event.type === 'click' && event instanceof GlobalMouseEvent) {
      let btn = null;
      for (let i = 0; i < path.length; i++) {
        const n = path[i];
        if (n.nodeType === NODE_ELEMENT && isSubmitButton(n)) { btn = n; break; }
        if (!event.bubbles) break;   // non-bubbling: only the target activates
      }
      if (btn && isConnected(btn) && !isActuallyDisabled(btn)) {
        const form = formForControl(btn);
        if (form && typeof form._submitForm === 'function') form._submitForm(btn, true);
      }
    }
