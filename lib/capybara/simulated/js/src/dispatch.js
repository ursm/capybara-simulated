...
function dispatchPath(target, event, drain) {
  ...
  try {
    ...
    if (event.type === 'click' && !event._csimActivationHandled &&
        ...
        target.nodeType === NODE_ELEMENT && target._tag === 'input') {
      ...
      if (itype === 'checkbox') {
        ...
        // restore it (the click may even morph `target`'s type mid-dispatch).
    ...
    // Checkbox/radio post-activation: roll back on cancel, otherwise fire
    // input then change — but only when the control is connected (per HTML,
    // a detached control's activation mutates state yet dispatches no
    // input/change). `target` is the real control even under shadow retarget.
    if (ckActivation) {
      if (event.defaultPrevented) {
        ...
        const wasChecked = ckActivation.kind === 'radio'
          ? ckActivation.prevChecked === target
          : ckActivation.wasChecked;
        if (isConnected(target) && (target._attrs.checked != null) !== wasChecked) {
          try { dispatchEvent(target, new InputEvent('input', { bubbles: true, cancelable: true })); } catch (_) {}
          try { dispatchEvent(target, new GlobalEvent('change', { bubbles: true, cancelable: false })); } catch (_) {}
        }
      }
    }
