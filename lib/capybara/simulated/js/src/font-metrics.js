...
export function advanceTableFor(family, weightStyle) {
  ...
  const ft = resolvedFaceTable(face, doc);
  ...
  // native measureRun can parse from the fontconfig path (same file, same advances).
...
export function faceStackFor(family, weightStyle) {
  ...
  for (const fam of splitFontStack(family)) {
    for (const face of pickFaces(idx, fam, weightStyle)) {
      ...
      cands.push({ ranges: face.ranges, table, sizeMul: face.metrics ? face.metrics.sizeAdjust : 1 });
    }
  }
  ...
  if (anyRestricted) {
    ...
    if (sys) cands.push({ ranges: null, table: sys, sizeMul: 1 });
  }
...
function fontFaceIndex(doc) {
  ...
  const add = (family, src, weight, style, rule, face, metrics, unicodeRange) => {
    ...
    families.get(fam).push({ url, locals, wlo: w[0], whi: w[1], style: String(style || 'normal').toLowerCase(), rule: rule ? rule.rule : null, face: face || null, metrics, ranges });
  };
  ...
  if (set && set._faces) set._faces.forEach((f) => {
    if (typeof f._source === 'string') add(f.family, f._source, f.weight, f.style, null, f, faceMetrics(f.sizeAdjust, f.ascentOverride, f.descentOverride, f.lineGapOverride), f.unicodeRange);
      ...
      if (ranges) restricted = true;
      families.get(fam).push({ url: '', table: f._table, wlo: w[0], whi: w[1], style: String(f.style || 'normal').toLowerCase(), rule: null, face: f, metrics: faceMetrics(f.sizeAdjust, f.ascentOverride, f.descentOverride, f.lineGapOverride), ranges });
    }
  });
  return (doc.__csimFontFaceIndex = { gen, families, stackMemo: new globalThis.Map(), rules, restricted });
}
...
// Does a parsed `unicode-range` (null = universal) cover a codepoint?
...
// The on-disk SFNT path for a family's @font-face — the SAME decoded file the oracle measures advances from
// (the host's `font_file_for`: fetched once, WOFF/WOFF2 decoded, cached), so native (skrifa) and the oracle
// (host) read identical hmtx advances. Returns null — native then declines the run — for a face native can't
// reproduce this way: a metric-descriptor face (`size-adjust` / `*-override` reshape the advance table), a
// `unicode-range`-restricted face (a run splits across faces), or a `local()` / buffer face (no fetched URL).
export function webFontSfntPath(family, weightStyle) {
  const doc = globalThis.document;
  ...
  // Decline what native can't reproduce by registering the url's SFNT: a metric-descriptor face (the table is
  // reshaped), a unicode-range face (a run splits across faces), and — critically — a face that also lists a
  // `local()` source: `resolvedFaceTable` prefers an INSTALLED local font over the download, so the oracle
  // would measure the local face while native measured the url one. No url at all is a local/buffer face too.
  if (!face || !face.url || face.metrics || face.ranges || (face.locals && face.locals.length)) return null;
  try { return globalThis.__csim_webFontFile(face.url) || null; } catch (_) { return null; }
}
