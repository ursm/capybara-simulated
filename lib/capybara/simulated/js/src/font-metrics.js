...
export function advanceTableFor(family, weightStyle) {
  ...
  if (ft) return ft;
  // No matching face, or its file was unreadable: the system face fontconfig substitutes.
...
function resolvedFaceTable(face, doc) {
  ...
  if (face.url) {
    ...
    let t;
    if (FONT_TABLES.has(key)) t = FONT_TABLES.get(key);
    else { t = applyFaceMetrics(webFontTable(face.url, doc, face.rule ? { rule: face.rule } : face.face), face.metrics); FONT_TABLES.set(key, t); }
    if (t) return t;
  }
