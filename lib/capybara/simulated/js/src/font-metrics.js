...
export function advanceTableFor(family, weightStyle) {
  ...
  const idx = doc ? fontFaceIndex(doc) : null;
  let face = null;
  ...
  if (face && face.table) return face.table;
  const key = family + ' ' + weightStyle + (face ? ' @' + face.url : '');
  if (FONT_TABLES.has(key)) return FONT_TABLES.get(key);
  let t = face ? webFontTable(face.url, doc, face) : null;
  if (!t) {
    try {
...
function fontFaceIndex(doc) {
  ...
  const add = (family, src, weight, style, rule, face) => {
    ...
    families.get(fam).push({ url, weight: faceWeight(weight), style: String(style || 'normal').toLowerCase(), rule: rule ? rule.rule : null, face: face || null });
  };
  ...
  if (set && set._faces) set._faces.forEach((f) => {
    if (typeof f._source === 'string') add(f.family, f._source, f.weight, f.style, null, f);
    else if (f._table) {                                          // a face built from a buffer: its bytes are its table
      const fam = String(f.family || '').trim().replace(/^["']|["']$/g, '').toLowerCase();
      if (!families.has(fam)) families.set(fam, []);
      families.get(fam).push({ url: '', table: f._table, weight: faceWeight(f.weight), style: String(f.style || 'normal').toLowerCase(), rule: null, face: f });
    }
  });
  return (doc.__csimFontFaceIndex = { gen, families, stackMemo: new globalThis.Map(), rules });
}
// `font-weight` descriptor → a number (a range keeps its first bound; `bold` is 700).
function faceWeight(v) {
  const s = String(v || 'normal').trim().toLowerCase();
  if (s === 'bold') return 700;
  const n = parseFloat(s);
  return isFinite(n) ? n : 400;
}
// The face for `family` at the run's weight / style: same style first, then the nearest
// weight (CSS Fonts 4 §5.2, at 400 / 700 granularity — layout only distinguishes bold).
function pickFace(idx, family, weightStyle) {
  const faces = idx.families.get(family.toLowerCase());
  if (!faces || !faces.length) return null;
  const wantBold = /bold/.test(weightStyle), wantItalic = /italic/.test(weightStyle);
  const want = wantBold ? 700 : 400;
  let best = null, bestScore = Infinity;
  for (const f of faces) {
    const italic = f.style === 'italic' || f.style === 'oblique';
    const score = (italic === wantItalic ? 0 : 10000) + Math.abs(f.weight - want) + (wantBold ? (f.weight < want ? 500 : 0) : (f.weight > want ? 500 : 0));
...
globalThis.__csimWebFontLoad = function (doc, family) {
  ...
  for (const f of faces) webFontTable(f.url, doc, f.rule ? { rule: f.rule } : f.face);
  return faces.length;
};
...
const CSS_WIDE = new globalThis.Set(['inherit', 'initial', 'unset', 'revert', 'revert-layer']);
