...
export function advanceTableFor(family, weightStyle) {
  ...
  let t = face ? webFontTable(face.url, doc, face && (face.rule ? { rule: face.rule } : face.face)) : null;
  if (!t) {
    try {
...
function fontFaceIndex(doc) {
  ...
  const add = (family, src, weight, style, rule, face) => {
    ...
    families.get(fam).push({ url, wlo: w[0], whi: w[1], style: String(style || 'normal').toLowerCase(), rule: rule ? rule.rule : null, face: face || null });
  };
  ...
  if (set && set._faces) set._faces.forEach((f) => {
    if (typeof f._source === 'string') add(f.family, f._source, f.weight, f.style, null, f);
      ...
      families.get(fam).push({ url: '', table: f._table, wlo: w[0], whi: w[1], style: String(f.style || 'normal').toLowerCase(), rule: null, face: f });
    }
  });
