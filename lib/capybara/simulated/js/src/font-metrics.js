...
// expects. ONE answer for the oracle's table and native's file (`webFontSfntPath`).
...
export function webFontSfntPath(family, weightStyle) {
  ...
  // Decline what native can't reproduce by registering one SFNT: a metric-descriptor face (the table is reshaped) and
  // a unicode-range face (a run splits across faces).
  if (!face || face.metrics || face.ranges) return null;
  // …and take the file the ORACLE measures: `resolvedFaceTable` prefers a font INSTALLED under a `local()` name to the
  // download, so those come first, in order — the very file `local_font_table` read. A face listing one was declined
  ...
  try {
    ...
    return face.url ? globalThis.__csim_webFontFile(face.url) || null : null;
  } catch (_) { return null; }
}
