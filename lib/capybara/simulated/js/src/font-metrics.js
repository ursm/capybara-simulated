...
function resolvedFaceTable(face, doc) {
  ...
  if (FONT_TABLES.has(key)) return FONT_TABLES.get(key);
  const t = applyFaceMetrics(webFontTable(face.url, doc, face.rule ? { rule: face.rule } : face.face), face.metrics);
  FONT_TABLES.set(key, t);
  return t;
}
...
function fontFaceIndex(doc) {
  ...
  const add = (family, src, weight, style, rule, face, metrics, unicodeRange) => {
    ...
    families.get(fam).push({ url, wlo: w[0], whi: w[1], style: String(style || 'normal').toLowerCase(), rule: rule ? rule.rule : null, face: face || null, metrics, ranges });
  };
