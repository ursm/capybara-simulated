...
// Real browsers fire a `scroll` event on the document for
// `window.scrollTo / scrollBy / scroll`, which is what
// `DLoadMore`-style IntersectionObserver-driven infinite loaders
// (Discourse admin site-texts: scroll to bottom triggers more rows)
// hook into. Without dispatching here our IO recheck never runs
// and the sentinel stays at its initial-render state forever.
...
globalThis.scrollTo    = function () { fireWindowScroll(); };
globalThis.scrollBy    = function () { fireWindowScroll(); };
globalThis.scroll      = function () { fireWindowScroll(); };
