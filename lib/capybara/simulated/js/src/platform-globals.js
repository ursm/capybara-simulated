...
function defineWindowAccessor(name, get) {
  Object.defineProperty(__windowProto, name, {configurable: true, get});
}
...
function cloneInto(v, seen) {
  ...
  if (ArrayBuffer.isView && ArrayBuffer.isView(v)) {
    ...
    // A typed array's 3rd constructor arg is an element COUNT (`.length`); a DataView has no
    // `.length` and takes a BYTE length — passing `undefined` would size it to the rest of
    // the buffer (buffer − byteOffset), losing an explicit view length.
    const out = tag === '[object DataView]'
      ? new v.constructor(buf, v.byteOffset, v.byteLength)
  ...
  if (tag === '[object URL]' || tag === '[object URLSearchParams]') {
    throw new globalThis.DOMException("Failed to execute 'structuredClone' on 'Window': An object could not be cloned.", 'DataCloneError');
  }
