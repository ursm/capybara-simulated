...
function csimWindowProxy(handle) {
  ...
  // Navigate the aux window, then fire its OWN `load` on the next task (deferred,
  // like window.open) so the newly-loaded child's `window.onload` runs AFTER the
  // opener's current task — e.g. the loadResolver-reports-back form-restore pattern.
  const navAux = (v) => {
