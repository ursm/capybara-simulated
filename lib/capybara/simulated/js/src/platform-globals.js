...
// `self.origin` / `window.origin` — the document's origin, the serialized
// origin of the current location for a normal page / same-origin frame. Read
// for CORS / postMessage-target checks. (self-origin.sub.) (An opaque-origin
// document — about:blank / srcdoc — inherits its parent's origin rather than
// the location origin; that finer model isn't here yet.)
Object.defineProperty(globalThis, 'origin', {
  configurable: true,
  get() { try { return (globalThis.location && globalThis.location.origin) || ''; } catch (_) { return ''; } }
});
...
function frameWindowProxyFor(targetRealmId) {
  ...
  const pmsg = function (message, targetOrigin, transfer) {
...
globalThis.__csimPostMessageRealm = function (senderId, targetId, message, _targetOrigin, transfer) {
  try { detachTransferables(transfer); } catch (_) {}
...
// Runs in the TARGET realm: clone the payload into this realm and queue the
// `message` event task, with source = this realm's proxy for the sender.
...
class CSSStyleSheet {
  constructor(opts) {
    ...
    this.title = '';
    // Raw CSS text, kept so the shadow-tree cascade can re-parse an adopted
    // sheet's rules (the per-rule cssText round-trip is lossy).
    this._cssText = '';
  }
...
class MessagePort extends EventTarget {
  ...
  postMessage(data, transfer) {
    // Honor the transfer list: a transferred ArrayBuffer is DETACHED
    // synchronously at postMessage time (spec), independent of whether a peer or
    // listener exists — so a subsequent encodeInto / TextDecoder on it (or its
    // views) sees an empty, detached buffer. Same detach helper the window and
    // worker postMessage paths use.
    detachTransferables(transfer);
    // Transferred MessagePorts are delivered to the peer in the message event's
    // `ports` (HTML "message port post message steps"). We're single-isolate, so
    // the port objects shuttle across as-is rather than via a structured clone.
    const ports = Array.isArray(transfer) ? transfer.filter((t) => t instanceof MessagePort) : [];
...
class BroadcastChannel extends EventTarget {
  ...
  postMessage(data) {
    ...
    // Fan out to OTHER windows (separate isolates) via the Driver — same-origin
    // BroadcastChannel spans every same-origin browsing context, including ones
    // opened with window.open / target=_blank.
    if (typeof globalThis.__csimBroadcast === 'function') {
      try { globalThis.__csimBroadcast(this.name, data); } catch (_) {}
    }
  }
  close() {
    const set = _bcChannels.get(this.name);
...
function csimWindowProxy(handle) {
  ...
  const location = {
    ...
    set href(v)  { globalThis.__csimWindowSetLocation(handle, usv(v)); },
    assign(v)    { globalThis.__csimWindowSetLocation(handle, usv(v)); },
    replace(v)   { globalThis.__csimWindowSetLocation(handle, usv(v)); },
    get hash()   { const h = serializedHref(); const i = h.indexOf('#'); return i >= 0 ? h.slice(i) : ''; },
    toString()   { return serializedHref(); }
  };
  // `win.document.<prop>` reads forward to the aux window's VM (separate isolate),
  // e.g. `win.document.charset` (url-charset). Read-only, primitives only.
  ...
  proxy = new Proxy(base, {
    ...
    set(t, prop, v) { try { t[prop] = v; } catch (_) {} return true; }
  });
...
globalThis.open = function (url, name, _features) {
  ...
  // proxy's `load` on the NEXT task so an `onload` assigned right after
  // window.open() is registered first (url-charset / url-in-tags-revoke).
  if (u !== '' && proxy && typeof proxy.__csimFireLoad === 'function') {
    setTimeout(() => { try { proxy.__csimFireLoad(); } catch (_) {} }, 0);
  }
  return proxy;
};
