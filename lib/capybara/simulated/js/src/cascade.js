...
export function classWriteLayoutEffect(oldRaw, newRaw) {
  ...
  // invisible to the map, so a page hosting any shadow root answers with today's subtree mark
  // unconditionally. One of three document-wide gates still keyed on the host COUNT rather than on
  // what those sheets declare — `referencedAnimationNames` and `resolveCascadeDisplay` are the others,
  // where `cascadeDeclaresProperty`, `mayConstrainSize`, `documentHasKeyframes`,
  // `documentMayTransition` and `cascadeLayoutEpoch` now go through `shadowSheetFacts`. Narrowing THIS
  // one wants a token gate built over the shadow sheets; `referencedAnimationNames` needs only the
  // names, which `shadowSheetFacts` is already the place for, and the comment above it measures its
  // un-narrowed path at +19.5% — so that is the one to take first.
...
let shadowProps = new Set(), shadowKeyframeProps = new Set(), shadowSheets = [], shadowSheetSeen = new Set();
let shadowHasKeyframes = false, shadowUnsafe = false, shadowDocPartVer = -1, shadowTreesSeen = 0;
let shadowDynLayout = false;
function shadowSheetFacts () {
  ...
  if (!globalThis.__csimShadowHostCount && !shadowTreesSeen) return shadowProps;
  ...
  while (shadowFactsPending.length) {
    ...
    shadowTreesSeen++;                                         //   already fresh here, so it does not
  ...
  if (shadowTreesSeen && shadowDocPartVer !== cascadeVersion) {
...
function foldShadowSheet (sheet) {
  if (sheet.shadowUnsafe) shadowUnsafe = true;
  ...
  for (const kf of (sheet.keyframes || [])) {
    ...
    for (const block of kf.blocks) for (const d of block.decls) shadowKeyframeProps.add(d.prop);
  }
}
...
export function resetShadowSheetFacts () {
  ...
  shadowProps = new Set(); shadowKeyframeProps = new Set(); shadowSheets = []; shadowSheetSeen = new Set();
  shadowHasKeyframes = false; shadowUnsafe = false; shadowDocPartVer = -1; shadowTreesSeen = 0;
  shadowDynLayout = false;
}
...
function scopedRulesFor(sr) {
  ...
  const addSheet = (cssText, base) => {
    ...
    if (parsed.shadowUnsafe === undefined) {
      parsed.shadowUnsafe = /:host-context\(/i.test(cssText) || PART_RE.test(cssText);
    }
    const place = (rule) => {
...
function keyframePropIndex () {
  ...
  // …and every property a SHADOW tree's own `@keyframes` animates, unfiltered by the referenced-name
  // set (which is the document's). This index used to lean on `cascadeDeclaresProperty` answering yes
  // for any page with a host — with that gone, a `transform` animated only inside a component reached
  // `getComputedStyle` and not the geometry, and the two halves of ONE geometry disagreed. The drain
  // goes FIRST: guarding it on `shadowKeyframeProps` being non-empty is guarding it on the thing it
  // fills, so the first call — the one that matters — would skip it.
  shadowSheetFacts();
  for (const prop of shadowKeyframeProps) idx.add(prop);
  return idx;
}

// Which `@keyframes` names anything actually references, or `null` when that cannot be decided from
// the sheets alone (an inline `animation` declaration, or a shadow tree whose rules are not here) —
// and then every name counts, as it did before this filter.
//
...
// widens the answer — this is a "may" gate, and over-approximating is the safe direction.
const ANIMATION_NAME_KEYS = ['animation-name', 'animation'];
function referencedAnimationNames () {
  if (globalThis.__csimInlineAnimation || globalThis.__csimShadowHostCount) return null;
  let names = state.animNameIdx;
  if (names === null) {
    names = state.animNameIdx = new Set();
    for (const rule of state.layoutRules) {
      ...
      for (const key of ANIMATION_NAME_KEYS) {
        ...
        for (const token of String(d.value).split(/[\s,]+/)) if (token) names.add(token);
      }
    }
  }
  return names;
