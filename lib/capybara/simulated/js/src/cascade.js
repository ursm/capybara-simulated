...
import { bumpStyleState, currentStyleStateGen, currentDirtySeq } from './mutation-observer.js';
...
// The subject compound's query key: how DESC entries find the affected elements. Class beats
// id beats tag (same preference as `terminalKey`); the QUERY it feeds is a plain selector.
...
export function cascadeLayoutEpoch () {
  ensureCascadeFresh();
  const cv = globalThis.__csimCascadeVersion ? globalThis.__csimCascadeVersion() : 0;
  const ss = ((state.hasDynamicLayoutRule && dynamicLayoutRulesArmed()) || globalThis.__csimShadowHostCount)
