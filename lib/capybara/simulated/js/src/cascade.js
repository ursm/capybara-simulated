...
import { splitTopLevel, decodeDataUrlCss } from './css-utils.js';
...
// instead. A change to the element's own text reparses the sheet from it (matching
// a real browser, where setting `<style>.textContent` rebuilds the sheet and
// discards prior insertRule edits); `_sheetText` is shared with the `.sheet` getter
// so the two stay in step.
function effectiveStyleCss(s) {
  const sheet = s._sheet;
  if (!sheet) return scriptText(s);
  const text = scriptText(s);
  if (s._sheetText !== text) { sheet._reparse(text, false); s._sheetText = text; }
...
function cascadeCacheKey(doc, vp) {
  ...
  // text fingerprint so reassigning / mutating them re-keys and rebuilds.
...
const LOWERCASE_VALUE_PROPS = new Set(['display', 'visibility', 'text-transform', 'white-space', 'direction', 'appearance']);
...
const CAPTURED_PROPS = new Set(['top', 'left', 'width', 'height', 'color', 'background-color', 'position', 'z-index', 'border-width', 'border-style', 'border-color', 'outline-color', 'caret-color', 'font-size', 'font-weight', 'font-style', 'line-height', 'font-family']);
...
function __sheetCacheKey(text, vp) {
  ...
  return 'v2:' + text.length + ':' + (h >>> 0).toString(16) + ':' + vp.width + 'x' + vp.height;
}
...
function collectCascadeRules(doc, selectedSet) {
  ...
  // `docBase` is the base URL for relative imports in inline <style> elements.
  const importSeen = new Set();
  const docBase = (globalThis.location && globalThis.location.href) || undefined;
  ...
  const ordered = [];
  const seen = new Set();
  for (const sh of sheets) for (const nm of (sh.layers || [])) if (!seen.has(nm)) { seen.add(nm); ordered.push(nm); }
...
// captured set tight.
const LAYOUT_PROPS = [...CAPTURED_PROPS, 'text-transform', 'white-space', 'direction', 'appearance'];
...
function ownVisibility(el) {
  ...
  // Shadow-tree visibility rules incl. :host / ::slotted (gated; see matchesAnyHideRule).
  const shadowHide = shadowRulesForEl(el, 'hide');
  if (shadowHide) {
    for (const r of shadowHide) {
...
export function cascadedProperty (el, prop) {
  ...
  // for elements INSIDE a shadow tree only. Not yet modelled: `:host` /
  // `::slotted` cross-scope rules; shadow `display`/`visibility` hide rules
  // (matchesAnyHideRule doesn't consult shadow sheets); proper encapsulation
  // (document author rules still match shadow elements); in-place edits to a
  // shadow `<style>`'s text (cache keys on the document cascadeVersion, so a
  // shadow restyle with no document change can read stale until the next
  // document-sheet change — reassigning adoptedStyleSheets DOES invalidate).
