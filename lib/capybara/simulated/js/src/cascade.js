...
// layer that omits the component). `background-image` is deliberately EXCLUDED: its computed value
// is the `url(...)` resolved to absolute against the ORIGINATING SHEET's base URL (a constructable
// sheet can set a custom `baseURL`), and the cascade doesn't track per-rule sheet origin — so we
// leave background-image unmodelled rather than resolve against the wrong (document) base. The
// image token is still recognised during parsing so it isn't misread as a <position>.
const BG_LONGHANDS = [
...
function ruleDecls(block) {
  ...
  block.children.forEach(node => {
    ...
    // The `background` shorthand expands to its captured longhands (color / position / size /
    // repeat / attachment / origin / clip — background-image excluded, see BG_LONGHANDS), each
    // in computed form, so getComputedStyle reports them. An omitted component resets to its
    // initial (a later `background:` overrides an earlier explicit longhand) — in particular a
    // colorless shorthand resets background-color to transparent.
    if (prop === 'background') {
