...
export function inlineDecls (el) {
  ...
  for (const { prop, value } of parseStyleDeclList(String(s))) {
    ...
    // A value carrying a SUBSTITUTION can only be expanded once it resolves, which is a different
    // time — `transition: all var(--d) ease` is a real declaration, and dropping it whole made the
    // element look like it had no transition rather than one we can't decompose. Keep the
    // shorthand; the resolved-value gate then reports its longhands as unknowable.
    const unexpandable = isRegularShorthand(prop) && !shorthandExpand(prop, d.value) &&
                         prop !== 'flex' && prop !== 'inset' && prop !== 'background' && prop !== 'border';
    if (unexpandable && !/\b(var|env)\(/i.test(d.value)) continue;
    if (unexpandable) { put(prop, d.value, d.important); continue; }
...
const DEFAULT_FLOW_SIDES = Object.freeze({
