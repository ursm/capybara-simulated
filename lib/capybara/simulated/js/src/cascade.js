...
export function inlineDecls (el) {
  ...
  const put = (prop, value, important) => {
    ...
    if (prop.charCodeAt(0) === 116 /* t */ && LATCHED_LINE_PROPS.has(prop)) sawInlineLineProp.add(prop);
    const folded = value;
...
// §15.3.9), as the `-webkit-` values Chrome computes for it: those line the text up AND align the
// block-level descendants (`<div align=center>` centres a narrower `<table>` inside it).
...
const TEXT_ALIGN_HINTS = Object.assign(Object.create(null), {
  left: '-webkit-left', right: '-webkit-right', center: '-webkit-center', middle: '-webkit-center',
  justify: 'justify'
});
...
const LATCHED_LINE_PROPS = new Set(['text-align', 'text-indent']);
