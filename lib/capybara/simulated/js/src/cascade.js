...
export function collectVisibleText(node, transform, preserveWs) {
  ...
  if (node.nodeType === NODE_TEXT) {
    ...
    const outText = applyTextTransform(raw, transform || 'none');
    ...
    lastCollapsibleOnly = !preserveWs && !/[^ \t\n\r\f]/.test(outText);
    return outText;
  }
  ...
  if (node.nodeType === NODE_ELEMENT) {
    ...
    // (`abc<div><br></div>def` is three breaks in Chrome), where a collapsed space would not.
    if (node._tag === 'br') { lastCollapsibleOnly = false; return '\n'; }
