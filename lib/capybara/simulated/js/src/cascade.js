...
//
// The allowlist below is the maintained half, and it is the safe half to get wrong: a pseudo-class
// missing from it merely loses caching for the properties its rule declares. Everything not listed
// — including anything unrecognised — taints. Structural and attribute-driven selectors are listed
// because the tree and attributes DO move `settleGen`.
const STATIC_PSEUDOS = new Set([
  ...
  // attribute-driven — attributes move settleGen
  'disabled', 'enabled', 'required', 'optional', 'read-only', 'read-write', 'link', 'any-link',
  'visited', 'lang', 'dir',
  // shadow structure
  'host', 'host-context', 'slotted', 'part',
]);
// Single-colon pseudo-CLASS names. `::before` and friends are skipped: a pseudo-element rule doesn't
// contribute to the element's own computed style.
const PSEUDO_NAME_RE = /(^|[^:]):([a-z][a-z-]*)/gi;
