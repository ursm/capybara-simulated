...
export function inlineDecls (el) {
  ...
  // Declaration by declaration, in SOURCE ORDER, so a later one wins exactly as the cascade says.
  ...
  for (const prop in raw) {
    const d = splitImportant(raw[prop]);
    if (prop === 'flex')       for (const e of expandFlexShorthand(d.value)) put(e.prop, e.value, d.important);
      ...
      if (pairs) for (const [lh, v] of pairs) { const sub = splitImportant(v); put(lh, sub.value, sub.important); }
    }
