...
// The absolute-length table the STATIC math check works with — no element, so no em / vh.
const ABSOLUTE_UNIT_PX = { px: 1, cm: 96 / 2.54, mm: 96 / 25.4, q: 96 / 101.6, in: 96, pc: 16, pt: 96 / 72 };
const absoluteToPx = (n, unit) => (ABSOLUTE_UNIT_PX[unit] === undefined ? null : n * ABSOLUTE_UNIT_PX[unit]);
...
function expandDeclaration (prop, value, important, emit) {
  ...
  // that loser entirely.
  if (!hasSubstitution(value) && isStaticallyInvalidMath(value, absoluteToPx)) return;
