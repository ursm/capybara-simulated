...
export function inlineDecls (el) {
  ...
  // Each shorthand expands through the same code the stylesheet side uses — `flex` and `inset` have
  // expanders here (the generic registry disagrees on `flex: initial`, and doesn't treat `inset` as
  // a shorthand at all), everything else goes through the CSSOM's registry. The shorthand itself is
