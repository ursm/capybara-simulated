...
export function rebuildCascade(doc) {
  ...
  if (!cascadeTiming) return rebuildCascadeImpl(doc);
  const t = globalThis.__dom.nowNanos();
  try { return rebuildCascadeImpl(doc); }
...
function rebuildCascadeImpl(doc) {
  ...
  const { hide, layout, attrs, sheets, keyframes } = collectCascadeRules(doc, selectedSet);
...
function ruleMovesBoxes(r, reachable, isHide) {
  ...
  // any other pseudo-element (scrollbar parts, `::selection`) styles nothing laid out here.
  if (subjectIsPseudoElement(r)) {
    if (!PSEUDO_SUBJECT_RE.test(r.selectorText || '')) return 0;
...
// sheet change answers "unknown" → subtree, and the rebuild lands on the next read). It does drain
// the shadow-root fold queue, which usually costs nothing — `ctxAttrEffect` runs on this same write
// a few lines later and reaches the same drain through `ctxGateReady`. Usually, not always:
// `ctxGateReady` returns on `cascadeStale` BEFORE it drains, and `cascadeStale` outlives
// `cascadeRefreshScheduled` (the microtask clears one, `ensureCascadeFresh` the other), so in that
// window this is the only drainer. Draining early is self-healing either way: both `shadowDocPartVer`
// and a root's `_scopedRulesVer` re-key on the next version bump.
export function classWriteLayoutEffect(oldRaw, newRaw, target) {
  if (oldRaw === newRaw) return null;
  // The two free answers first: a page boot inserts sheets while the framework writes thousands of
  // classes, and every one of those writes would otherwise pay the shadow question below to reach
  // the same `true`.
  const gate = cascadeRefreshScheduled ? null : state.layoutTokenGate;
  if (!gate) return true;
  // Shadow sheets are not in this rule index: a shadow tree's own `.open .panel { … }` would be
  // invisible to the map. Unlike the sibling gates this one is asked about ONE ELEMENT, so it does
  // not need to know what those sheets declare — only whether any of them can style this element at
  // all, which is `shadowRulesMayReach`. A light-DOM element no shadow rule can reach is described
  // completely by the document's own gate. (`resolveCascadeDisplay` is the last document-wide gate
  // still keyed on the host COUNT; `cascadeDeclaresProperty`, `mayConstrainSize`,
  // `documentHasKeyframes`, `documentMayTransition`, `referencedAnimationNames` and
  // `cascadeLayoutEpoch` all ask `shadowSheetFacts` what the sheets declare instead.)
  if (shadowRulesMayReach(target)) return true;
...
function parseSheet(cssText, vp) {
  ...
  try { flat = cssTreeFlatten(cssText, vp); } catch (_) { return { hide, layout, attrs: [], count: 0, layers: [], imports: [], fontSrcs: [], keyframes: [], cssImage: false, cxIndex: newCtxIndex(), varGraph: finishVarGraph(newVarGraph()) }; }
  ...
  return { hide, layout, attrs: [...attrs], count: serial, layers: flat.layers, imports: flat.imports || [], fontSrcs: flat.fontSrcs || [], keyframes: flat.keyframes || [], cssImage: !!flat.cssImage, cxIndex, varGraph: finishVarGraph(varGraph) };
}
...
function collectCascadeRules(doc, selectedSet) {
  const empty = { hide: [], layout: [], attrs: new Set(), sheets: [], keyframes: new Map() };
  ...
  return { hide, layout, attrs, sheets, keyframes };
}
...
function buildPseudoIndex(which, rules) {
  ...
  for (const r of rules) {
    ...
    const term = terminalKey(stripped, null);
...
// browser doesn't support…" is empty in a browser; and the two hiding predicates are the cascade's.
function generatesTextBox(el) {
  return !INVISIBLE_TAGS.has(el._tag) && !hasFallbackOnlyContent(el) &&
         !uaNotRendered(el) && !selfHidden(el);
}
...
// Whether a host's light child renders is its slot's question, asked once per slot per walk (`slotRendersText`).
let slotRenders = null;
function slotRendersText(slot, shadowRoot) {
  if (!slotRenders) slotRenders = new Map();
  let v = slotRenders.get(slot);
  if (v === undefined) slotRenders.set(slot, v = isVisibleNodeImpl(slot, true, false, shadowRoot));
  return v;
}
export function collectVisibleText(node, transform, preserveWs) {
  ...
  const out = collectText(node, transform, preserveWs);
...
function collectText(node, transform, preserveWs) {
  ...
  if (node.nodeType === NODE_ELEMENT) {
    ...
    if (node._tag === 'br') {
      ...
      if (node === walkRoot || !globalThis.__csimGeneratesBox(node)) return '';
    ...
    if (node._tag === 'details' && node._attrs.open == null) {
      ...
      for (const c of node._children) {
        if (c.nodeType === NODE_ELEMENT && c._tag === 'summary') s += collectText(c, effTransform, preserveWs);
      }
      return s;
    }
  ...
  for (const c of node._children) {
    if (host) {
      const slot = assignedSlotFor(c);
      if (!slot || !slotRendersText(slot, host)) continue;
    }
    ...
    const part = collectText(c, transform, preserveWs);
    ...
    // Firefox have it.)
    const isBlock = isElement && (BLOCK_TAGS.has(c._tag) || flexContext || isCellWithInnerBlock ||
    ...
    if (isCell && hasNextCellSibling(c)) {
      if (pending) {
        if (spaceBefore && outEndsSpace) { out = out.slice(0, -1); outEndsSpace = false; }
