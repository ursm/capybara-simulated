...
import { splitTopLevel, decodeDataUrlCss, resolveCssUrls, documentBaseUrl, parseStyleDecls } from './css-utils.js';
import { expandShorthandsInMap } from './shorthands.js';
...
const CAPTURED_PROPS = new Set(['top', 'left', 'right', 'bottom', 'width', 'height', 'min-width', 'min-height', 'max-width', 'max-height', 'margin-top', 'margin-right', 'margin-bottom', 'margin-left', 'padding-top', 'padding-right', 'padding-bottom', 'padding-left', 'color', 'background-color', 'background-image', 'background-position', 'background-size', 'background-repeat', 'background-attachment', 'background-origin', 'background-clip', 'position', 'z-index', 'border-width', 'border-style', 'border-color', 'outline-color', 'caret-color', 'font-size', 'font-weight', 'font-style', 'line-height', 'font-family', 'opacity', 'text-align', 'cursor', 'pointer-events', 'flex-direction', 'flex-wrap', 'justify-content', 'align-items', 'align-content', 'align-self', 'order', 'flex-grow', 'flex-shrink', 'flex-basis', 'grid-template-columns', 'grid-auto-rows', 'gap', 'grid-gap', 'row-gap', 'column-gap', 'grid-column', 'overflow', 'overflow-x', 'overflow-y',
  // The CONTAINING-BLOCK / clipping signals. These are layout-free computed values we'd otherwise
  // report as their initial for every element — and a confident `transform: none` on an element a
  // stylesheet actually transforms is worse than the '' it replaced, because page code branches on
  // exactly this: Floating UI positions a fixed popper against the nearest ancestor whose
  // `transform` / `filter` / `will-change` / `contain` says it establishes a containing block.
  'transform', 'translate', 'rotate', 'scale', 'perspective', 'filter', 'backdrop-filter',
  'will-change', 'contain', 'container-type', 'clip-path', 'isolation', 'mix-blend-mode']);
...
export function inlineDecls (el) {
  ...
  const raw = parseStyleDecls(String(s));
  const expanded = expandShorthandsInMap(raw);
  for (const prop in expanded) {
    const d = splitImportant(expanded[prop]);
    // `inset` / `gap` aren't "regular" shorthands (their longhands aren't `inset-*` / `gap-*`), so
    // the generic expansion leaves them whole; the physical insets are what layout reads. Written
    // BEFORE the shorthand's own entry and in source order, so a later `top:` still wins.
    if (prop === 'inset') for (const e of expandBoxShorthand('inset', d.value)) put(e.prop, e.value, d.important);
    put(prop, d.value, d.important);
  }
  // Re-add any shorthand the expansion consumed (`overflow`, `margin`, `flex`): a resolved-value
  // read asks for those names directly, and reporting the initial value for one the element
  // plainly sets would be worse than not knowing it. Shorthand names never collide with longhand
  // ones, so nothing already resolved is disturbed.
  for (const prop in raw) if (!(prop in out)) { const d = splitImportant(raw[prop]); put(prop, d.value, d.important); }
  el._ilSrc = s;
  el._ilMap = out;
  return out;
}
