...
export function collectVisibleText(node, transform, preserveWs) {
  ...
  if (node.nodeType === NODE_ELEMENT) {
    ...
    // Fallback content is not rendered, so it is not visible text either — `innerText` of a
    // `<canvas>` / `<video>` carrying "your browser doesn't support…" is empty in a browser.
    if (hasFallbackOnlyContent(node)) return '';
    if (uaNotRendered(node)) return '';
