...
// The CSS box shorthands (`margin` / `padding`): 1–4 values map to top / right / bottom / left by
// the usual mirroring rules. A value count outside 1–4 is invalid and contributes nothing.
function expandBoxShorthand(prop, value) {
  ...
  return [
    { prop: `${prop}-top`,    value: top },
    { prop: `${prop}-right`,  value: right },
    { prop: `${prop}-bottom`, value: bottom },
    { prop: `${prop}-left`,   value: left }
  ];
}
...
function ruleDecls(block) {
  ...
  block.children.forEach(node => {
    ...
    // `margin` / `padding` expand to their four captured longhands in CSS box order (1 value = all
    // sides, 2 = block/inline, 3 = top/inline/bottom, 4 = top/right/bottom/left). The layout engine
    // reads the longhands — `body { margin: 0 }` has to actually move the body's origin — and
