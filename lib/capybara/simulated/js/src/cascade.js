...
export function resetCascadeState() {
  ...
  state.ctxPositionalVarNames = null;
}
...
function dynamicStateSubjects(rules) {
  ...
  for (const r of rules) {
    ...
    for (const g of groups) {
      ...
      if (!subject || !queryable(subject)) return null;
      const dedupe = subject.key + '\u0000' + [...subject.kinds].map(([k, v]) => k + (v.unscoped ? '*' : v.prefix + (v.subject ? 1 : 0) + (v.sibling ? 1 : 0))).sort().join('\u0001');
      const prev = byKey.get(dedupe);
      if (prev) prev.structural = prev.structural || !!structural;
      else byKey.set(dedupe, { key: subject.key, kinds: subject.kinds, structural: !!structural });
    }
  }
...
function __sheetCacheKey(text, vp) {
  ...
  return 'v6:' + text.length + ':' + (h >>> 0).toString(16) + ':' + vp.width + 'x' + vp.height;
}
...
function parseSheet(cssText, vp) {
  ...
  try { flat = cssTreeFlatten(cssText, vp); } catch (_) { return { hide, layout, attrs: [], count: 0, layers: [], imports: [], fontSrcs: [], cxIndex: newCtxIndex() }; }
  ...
  return { hide, layout, attrs: [...attrs], count: serial, layers: flat.layers, imports: flat.imports || [], fontSrcs: flat.fontSrcs || [], cxIndex };
}
...
function ctxGateReady() {
  ...
  if (gate === undefined) {
    ...
    state.ctxPositionalVarNames = gate.names;
    gate = state.ctxGate;
  }
  return gate;
}
...
export function ctxChildListEffect(el) {
  ...
  if (gate === null || state.ctxChildListDesc || (el._tag === 'fieldset' && gate.get('[disabled') !== undefined)) {
    ...
    // Nothing under the children is re-keyed, so what a positional / sibling-keyed declaration
    // (`li:first-child { --x }`) can still reach — a descendant's substitution — goes by name.
    ...
    const names = state.ctxPositionalVarNames;
