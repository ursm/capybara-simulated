...
// @supports (evaluated; non-matching dropped), CSS nesting via `&` substitution.
    ...
    for (const node of nodes) {
      if (node.type === 'at-rule') {
        ...
        if (node.name === 'supports') {
          // Evaluate the @supports condition; only descend when it holds
          // (mirrors the @media / @container branches).
