...
function ruleMovesBoxes(r, reachable, isHide) {
  ...
  if (subjectIsPseudoElement(r)) return PSEUDO_SUBJECT_RE.test(r.selectorText || '') ? TOKEN_SUBTREE : 0;
...
function buildPseudoIndex(which) {
  const idx = new Map();
...
function pseudoIndex(which) {
  ...
  if (both === null) both = state.pseudoIdx = { before: buildPseudoIndex('before'), after: buildPseudoIndex('after') };
  return both[which];
}
...
export function documentHasGeneratedContent () {
  return pseudoIndex('before').has('content') || pseudoIndex('after').has('content');
}
...
function cascadedPseudoRecord(pseudo, prop) {
  const sub = pseudoIndex(pseudo._pseudo).get(prop);
