...
import { splitTopLevel, decodeDataUrlCss, resolveCssUrls, documentBaseUrl, parseStyleDeclList } from './css-utils.js';
import { isRegularShorthand, shorthandExpand } from './shorthands.js';
...
function ruleDecls(block) {
  ...
  block.children.forEach(node => {
    ...
    // `margin` / `padding` / `inset` expand to their four captured longhands in CSS box order
    // (1 value = all sides, 2 = block/inline, 3 = top/inline/bottom, 4 = top/right/bottom/left).
    // The layout engine reads the longhands — `body { margin: 0 }` has to actually move the body's
    // origin, `inset: 0` has to stretch an overlay across its containing block — and
    // getComputedStyle reports them for the same reason it reports the border ones.
    if (prop === 'flex') {
      for (const d of expandFlexShorthand(value)) decls.push({ prop: d.prop, value: d.value, important: !!node.important });
      return;
    }
    if (prop === 'overflow') {
      for (const d of expandOverflowShorthand(value)) decls.push({ prop: d.prop, value: lowercaseKeywordValue(d.prop, d.value), important: !!node.important });
      return;
    }
    if (prop === 'margin' || prop === 'padding' || prop === 'inset') {
      for (const d of expandBoxShorthand(prop, value)) decls.push({ prop: d.prop, value: d.value, important: !!node.important });
      return;
    }
    ...
    // Any other shorthand expands through the CSSOM's registry — the same code the inline path
    // runs, so `ul { list-style: none }` and `style="list-style: none"` reach the longhands
    // identically. Without it the longhand was invisible here, and a child inheriting it fell
    // through to the initial (`<li>` reporting `disc` under a `list-style: none` nav).
    if (!custom && isRegularShorthand(prop)) {
      const pairs = shorthandExpand(prop, value);
      if (pairs) {
        for (const [lh, v] of pairs) {
          const sub = splitImportant(v);
          decls.push({ prop: lh, value: lowercaseKeywordValue(lh, sub.value), important: !!node.important || sub.important });
        }
      }
    }
...
// are EXPANDED — through the same `expandShorthandsInMap` the CSSOM side uses, so `style="flex:
// 1"` and `style="outline: 1px solid red"` reach the longhands here exactly as they do through
// `el.style.flexGrow` — and the shorthands themselves are kept, since a resolved-value read asks
// for some of them by name (`overflow`).
//
...
export function inlineDecls (el) {
  ...
  const put = (prop, value, important) => {
    ...
    // property name is case-sensitive, so lowercasing `var(--Foo)` loses the reference.
    out[prop] = { value: lowercaseKeywordValue(prop, value), important };
  };
  ...
  // Each shorthand expands through the same code the stylesheet side uses — `flex`, `inset` and
  // `background` have expanders here (the generic registry disagrees on `flex: initial` and doesn't
  // know the other two), everything else goes through the CSSOM's registry. The shorthand itself is
  // kept alongside its longhands: a resolved-value read asks for some by name (`overflow`), and the
  // two name spaces never collide.
  for (const { prop, value } of parseStyleDeclList(String(s))) {
    const d = splitImportant(value);
    let parsed = true;
    if (prop === 'flex' || prop === 'inset' || prop === 'background') {
      const parts = prop === 'flex'  ? expandFlexShorthand(d.value)
                  : prop === 'inset' ? expandBoxShorthand('inset', d.value)
                                     : expandBackgroundShorthand(d.value);
      parsed = parts.length > 0;
      for (const e of parts) put(e.prop, e.value, d.important);
    } else if (isRegularShorthand(prop)) {
      const pairs = shorthandExpand(prop, d.value);
      parsed = !!pairs;
      // The expander re-attaches `!important` per longhand only when its INPUT carried one, and
      // this input had it stripped two lines up — so the declaration's own importance is what
      // counts. Dropping it let an author `!important` rule beat an inline `!important` shorthand.
      if (pairs) for (const [lh, v] of pairs) { const sub = splitImportant(v); put(lh, sub.value, d.important || sub.important); }
    }
    // A shorthand that DOESN'T parse is a declaration a browser drops entirely; recording its name
    // would make the resolved-value gate treat every longhand it could set as unknowable, so
    // `style="transition: !!!"` would blank out `transition-duration` instead of leaving `0s`.
    if (parsed) put(prop, d.value, d.important);
  }
  el._ilSrc = s;
  el._ilMap = out;
  return out;
...
export function resolveLayoutProp (el, prop, basis = null) {
  ...
  const raw = best ? best.value : presentationalHint(el, prop);
  const px = parsePx(raw);
  if (px != null) return px;
  const pct = parsePercent(raw);
  return pct != null && basis != null ? pct * basis : null;
}
...
function rulesIndexHas (prop) {
  ...
  for (const r of state.layoutRules) {
    if (own(r.captured, prop)) { found = true; break; }
  }
