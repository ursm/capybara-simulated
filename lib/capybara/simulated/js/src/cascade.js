...
import { splitTopLevel, decodeDataUrlCss, resolveCssUrls, documentBaseUrl, parseStyleDeclList, cssPropertyName, isSupportedCssPropertyName, serializeCssValue, htmlDimensionValue } from './css-utils.js';
...
import { normalizeColor, declaredValue, declaredValueIn, fontRelativeToPx, uaDefault, inputType } from './style-proxy.js';
...
function isVisibleNodeImpl(el, ignoreVisibility) {
  ...
  while (cur) {
    ...
    if (cur.nodeType === NODE_ELEMENT) {
      ...
      if (cur !== el && FALLBACK_ONLY_TAGS.has(cur._tag)) return false;
...
const HINTED_PROPS = new Set(['width', 'height', 'margin-left', 'margin-right', 'margin-top', 'margin-bottom',
                              'list-style-type', 'vertical-align', 'float']);
...
export function collectVisibleText(node, transform, preserveWs) {
  ...
  if (node.nodeType === NODE_ELEMENT) {
    ...
    if (FALLBACK_ONLY_TAGS.has(node._tag)) return '';
