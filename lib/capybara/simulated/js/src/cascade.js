...
function nthLimit(data) {
  const text = String(data == null ? '' : data).toLowerCase().split(/\s+of\s+/)[0].replace(/\s+/g, '');
...
// One `:has()`, hung on the TOP-LEVEL compound carrying it: `sel` that compound whole (what the element's match is
// re-asked with, `hasCheck`), `key` a cheaper query naming its candidates (the compound without its `:has()`; null =
// any element), `follow` how far past its element the match reaches (the POS_ bits of a sibling combinator after it).
...
function layoutHasEntry(top, role, groups) {
  const h = { key: null, sel: null, follow: role > 0 ? role : 0, tokens: [], argKeys: [], empty: false, positions: 0, sideways: 0,
              opaque: !Array.isArray(groups) };
  try {
    h.sel = CW.stringify([top]);
...
function layoutIndexFor(el) {
  ...
  if (el._isShadowRoot || shadowRulesMayReach(el)) {
    ...
    if (shadowUnsafe || (shadowHostFn && el._shadowRoot) || (shadowSlotted && el._parent && el._parent._shadowRoot)) layoutConservative = true;
...
// per-compound entry is the index's own object, so a rebuilt rule set starts every element over.
function hasCheck(ix, n, mark) {
  for (const h of ix.has) {
    ...
    const now = h.sel === null ? !(memo && memo.get(h)) : safeElementMatches(n, h.sel);
...
let shadowHasKeyframes = false, shadowUnsafe = false, shadowDocPartVer = -1, shadowTreeFolds = 0;
...
function foldShadowSheet (sheet) {
  ...
  if (scope.unsafe)  shadowUnsafe  = true;
...
export function resetShadowSheetFacts () {
  ...
  shadowHasKeyframes = false; shadowUnsafe = false; shadowDocPartVer = -1; shadowTreeFolds = 0;
