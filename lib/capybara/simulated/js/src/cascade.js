...
import { ensureBuilt } from './native-query-shadow.js';   // native-authoritative cascade matching over the synced arena
import { normalizeColor, declaredValue, declaredValueIn, fontRelativeToPx, uaDefault, uaHidden, inputType } from './style-proxy.js';
...
// Native-AUTHORITATIVE cascade matching (store-flip reader-first foundation): when on, a rule that
// `matchesId` answers with a boolean over the synced arena is trusted — css-select is no longer run
// for it. undefined (a live-state / unsupported selector) and null (invalid) still fall through to
// css-select, which stays the authority for everything native defers or can't compile. Main-realm
// only (the arena is mirrored there alone — see native-query-shadow.js's realm invariant); a frame
// realm never sees the flag, so `el._nid` is unset and every match takes the css path. Re-resolved
// per rebuildCascade entry (a boot-time rebuild precedes the flag eval — caching-once latches off).
...
// ALONGSIDE css-select — css authoritative — to measure the speedup, tracking `cascNatNs` (native
// time on rules it answered), `cascNatCalls` (those), `cascNatFallback` (rules it deferred: state
// pseudo / pseudo-element / namespaced), `cascNatMismatch` (disagreements with css — must be 0). In
// AUTHORITATIVE mode (`__csimNativeCascadeAuthoritative`) native IS the answer, so `cascNatCalls`
// counts rules it decided and `cascNatFallback` those punted to css (mismatch/time unused there).
let cascNatNs = 0, cascNatCalls = 0, cascNatFallback = 0, cascNatMismatch = 0;
...
function safeMatches(el, r) {
  ...
  // Native-authoritative fast path: trust a boolean from the arena matcher; only a non-namespaced
  // rule on an element that HAS been mirrored (`_nid` set) qualifies, so a frame-realm element (never
  ...
  if (cascadeAuthoritative && !r.ns) {
    ...
    if (nid != null) {
      ...
      cascNatFallback++;   // undefined (live-state / unsupported) or null (invalid) → defer to css-select.
    } else {
      cascNatFallback++;   // element not mirrored yet (a frame-realm element never is)
    }
  }
  try {
    if (cascadeTiming) {
...
globalThis.__csimCascadeTimingStats = function (reset) {
  const snap = { cascMatchNs, cascMatchCalls, cascTotalNs, cascRuns, cascNatNs, cascNatCalls, cascNatFallback, cascNatMismatch };
  if (reset) {
    cascMatchNs = 0; cascMatchCalls = 0; cascTotalNs = 0; cascRuns = 0;
    cascNatNs = 0; cascNatCalls = 0; cascNatFallback = 0; cascNatMismatch = 0;
  }
  return snap;
};
...
export function rebuildCascade(doc) {
  ...
  //
  // Gated on the document being DONE parsing: parser tree-building bypasses the mutation seams the
  // arena syncs through (parse5-adapter appends don't call recordChildList), so an arena mirrored
  // mid-parse would go stale as later siblings are appended — a `<script>` that reads style mid-parse
  // must fall back to css. Once readyState leaves 'loading' the tree is final and every further change
  // is script-driven (and synced), so the mirror is safe to build and trust.
  const cascadeDoc = doc || globalThis.document;
  cascadeAuthoritative = globalThis.__csimNativeCascadeAuthoritative === true && globalThis.__dom != null &&
                         cascadeDoc != null && cascadeDoc.readyState !== 'loading';
  if (cascadeAuthoritative) ensureBuilt();
...
function cssWideStandsFor(el, prop, keyword, depth) {
...
// `inherit` / `revert` and returns null for `initial` (and for `unset` on these two, neither of
// which inherits) — where null means the property's INITIAL value, which for exactly these two is
// a keyword worth naming rather than looking up.
...
export function documentMayTransition () {
  ...
  ensureCascadeFresh();
  // A page with a SHADOW HOST answers yes without looking: a shadow tree's own sheets have no
  // document-level key set to ask, so the gate has to fail open there — the same thing
  // `documentHasKeyframes` and `cascadeDeclaresProperty` do two lines below. Without it a
  // transition declared only inside a component never ran at all.
  if (globalThis.__csimShadowHostCount) return true;
  return rulesIndexHas('transition-property') || rulesIndexHas('transition-duration') ||
         rulesIndexHas('transition-delay') || rulesIndexHas('transition-timing-function') ||
         rulesIndexHas('transition-behavior');
}
...
// animation-free page out of the animation model entirely (rule 3). A page with a shadow host
// answers yes without looking — the per-tree sheets have no document-level key set to ask, the same
// fail-open `cascadeDeclaresProperty` takes.
export function documentHasKeyframes () {
  ensureCascadeFresh();
  return !!(state.keyframes && state.keyframes.size) || !!globalThis.__csimShadowHostCount;
}
...
//
// No shadow fail-open of its own: a shadow tree's keyframes are in `scopedRulesFor`, not here, and
// every caller ORs this with `cascadeDeclaresProperty`, which already answers yes for any page with
// a shadow host.
