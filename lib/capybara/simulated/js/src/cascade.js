...
// F2 sizing: native single-element match run ALONGSIDE css-select in the cascade (css authoritative),
// timed + parity-checked, to measure the real cascade-match speedup a native matcher over the synced
// arena would give. nat = native time on rules it answered; natCalls = those; natFallback = rules it
// deferred (state pseudo / pseudo-element / namespaced); natMismatch = disagreements with css (must be 0).
