...
function ruleDecls(block) {
  ...
  block.children.forEach(node => {
    ...
    if (prop === 'overflow') {
      for (const d of expandOverflowShorthand(value)) decls.push({ prop: d.prop, value: d.value.toLowerCase(), important: !!node.important });
      return;
    }
...
// The layout properties an inline `style=` can carry — the same set the stylesheet side captures,
// because the inline layer sits at the TOP of the cascade `resolveLayoutProp` walks: a property
// missing here doesn't just get ignored, it lets a stylesheet rule win where the inline value
// should have. `right` / `bottom` are how JS-positioned UI (poppers, toasts, dropdowns) places
// itself at runtime (`el.style.bottom = …`), and `<body style="margin:0">` has to move the body's
// origin exactly as the equivalent rule does.
const INLINE_LAYOUT_PROPS = new Set([
  'top', 'left', 'right', 'bottom', 'width', 'height', 'min-width',
  'flex-grow', 'flex-shrink', 'flex-basis',
  'margin-top', 'margin-right', 'margin-bottom', 'margin-left',
  'padding-top', 'padding-right', 'padding-bottom', 'padding-left'
]);
// Box shorthands, expanded to the longhands above exactly as `ruleDecls` expands them.
const INLINE_BOX_SHORTHANDS = new Set(['inset', 'margin', 'padding']);
const INLINE_DECL_RE = /^\s*([a-zA-Z-]+)\s*:\s*([^;]+?)\s*$/;
// Cached on the element and keyed by the raw `style` string itself, so it survives across layout
// passes and invalidates the moment the attribute changes. `resolveLayoutProp` asks twice per
// element per pass (width and height, more for insets), and re-splitting + re-regexing the
// attribute each time was the single biggest cost in a layout pass on an editor-shaped page.
const EMPTY_INLINE_LAYOUT = Object.freeze({});
export function parseInlineLayout (el) {
  const s = el._attrs && el._attrs.style;
  if (!s) return EMPTY_INLINE_LAYOUT;
  if (el._ilSrc === s) return el._ilMap;
  const out = {};
  for (const part of String(s).split(';')) {
    const m = INLINE_DECL_RE.exec(part);
    if (!m) continue;
    const prop = m[1].toLowerCase();
    const shorthand = INLINE_BOX_SHORTHANDS.has(prop);
    if (!shorthand && prop !== 'flex' && !INLINE_LAYOUT_PROPS.has(prop)) continue;
    ...
    // Later declarations win — which is what this last-wins loop already gives us.
    if (prop === 'flex') {
      for (const e of expandFlexShorthand(d.value)) out[e.prop] = { value: e.value, important: d.important };
    } else if (shorthand) {
      for (const e of expandBoxShorthand(prop, d.value)) out[e.prop] = { value: e.value, important: d.important };
    } else {
      out[prop] = { value: d.value, important: d.important };
    }
  }
  el._ilSrc = s;
  el._ilMap = out;
