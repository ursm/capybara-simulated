...
import { bumpStyleState, currentStyleStateGen, currentDirtySeq, markLayoutDirty } from './mutation-observer.js';
...
import { normalizeColor, declaredValue, declaredValueIn, fontRelativeToPx, uaDefault } from './style-proxy.js';
...
export function rebuildCascade(doc) {
  ...
  const dynLayout = collectDynamicLayoutRules(hide, layout);
  state.dynLayoutVarOnly = dynLayout.varOnly;
  state.hasDynamicLayoutRule = dynLayout.rules.length > 0 || dynLayout.varOnly;
  state.dynLayoutReqs = dynLayout.rules.length ? dynamicLayoutRuleReqs(dynLayout.rules) : [];
  const scoped = dynamicStateSubjects(dynLayout.rules);
  ...
  state.layoutTokenGate = buildLayoutTokenGate(hide, layout);
}
...
// enumerate; `inlineVarLayoutSeen` covers them conservatively.
const VAR_REF_RE = /var\(\s*(--[^\s,)]+)/g;
function layoutReachableCustomProps(layout) {
  const seeds = new Set();
  const edges = new Map();                       // declared custom prop -> custom props its value reads
  for (const r of layout) {
    for (const prop in r.captured) {
      const v = r.captured[prop].value;
      if (typeof v !== 'string' || v.indexOf('var(') === -1) continue;
      VAR_REF_RE.lastIndex = 0;
      let m;
      while ((m = VAR_REF_RE.exec(v))) {
        if (prop.startsWith('--')) {
          let list = edges.get(prop);
          if (!list) edges.set(prop, list = []);
          list.push(m[1]);
        } else if (!PAINT_ONLY_PROPS.has(prop)) {
          seeds.add(m[1]);
        }
      }
    }
...
function buildLayoutTokenGate(hide, layout) {
  const reachable = layoutReachableCustomProps(layout);
...
function collectDynamicLayoutRules(hide, layout) {
  const rules = [];
  let varOnly = false;
  const reachable = layoutReachableCustomProps(layout);
  ...
  for (const r of layout) {
    ...
    if (!core) { if (hasVar) varOnly = true; continue; }
    if (subjectIsPseudoElement(r)) continue;
    rules.push(r);
  }
  return { rules, varOnly };
}
...
function dynamicLayoutRuleReqs(rules) {
  ...
  for (const r of rules) {
    let groups;
    try { groups = CW.parse(r.selectorText || ''); }
    catch (_) { return null; }                        // unparseable selector: cannot gate
...
// The per-GROUP subject keys of every dynamic layout rule, for the scoped state dirtying below
// (terminalKey reads only the first group — `.a:hover, .b:focus` would drop `.b`). Each entry
// carries whether the rule can flip rendering itself (`display` / `visibility`): those marks
// must also be STRUCTURAL, or a table's grid memo (structFresh, keyed on the epoch this hook
// keeps still) survives a row disappearing. `null` = some group's subject has no identifier —
// that rule's effect cannot be scoped, so dynamic state must stay in the layout epoch.
function dynamicStateSubjects(rules) {
  ...
  for (const r of rules) {
    ...
    for (const g of groups) {
      const compound = [];
      for (const t of g) {
        if (CW_COMBINATORS.has(t.type)) { compound.length = 0; continue; }
        compound.push(t);
      }
      const key = compoundKey(compound);
      if (!key) return null;
      byKey.set(key, (byKey.get(key) || false) || !!structural);
    }
  }
  return { subjects: [...byKey].map(([key, structural]) => ({ key, structural })) };
}
...
// parser gen) — one sweep per actual flip.
const SCOPED_STATE = { gen: -1, cv: -1, pt: -1 };
globalThis.__csimApplyScopedStateDirty = function () {
  if (!state.hasDynamicLayoutRule) return;
  // These arms keep dynamic state in the layout epoch itself (see cascadeLayoutEpoch): shadow
  // sheets aren't in the rule index, an inline var() consumer has no subjects to find, and a
  // keyless subject cannot be queried. The hook only acts where the epoch no longer does.
  if (globalThis.__csimShadowHostCount) return;
  if (state.dynLayoutVarOnly && inlineVarLayoutSeen) return;
  if (state.dynStateEpochFallback || !state.dynStateSubjects) return;
  const gen = styleStateGeneration();
  const cv = cascadeVersion;
  const pt = globalThis.__csimParserTreeGen ? globalThis.__csimParserTreeGen() : 0;
  if (SCOPED_STATE.gen === gen && SCOPED_STATE.cv === cv && SCOPED_STATE.pt === pt) return;
...
function dynamicLayoutRulesArmed() {
  // A rule dropped as "custom-properties-only" comes back the moment an inline style is seen
  // consuming ANY custom property in a box-moving position — maximally conservative, no
  // presence check, and immediate (this runs before the memo).
  if (state.dynLayoutVarOnly && inlineVarLayoutSeen) return true;
  const reqs = state.dynLayoutReqs;
  if (reqs === null) return true;
  if (reqs.length === 0) return false;           // no core rules at all (var-only or none)
  const cv = cascadeVersion;
...
export function cascadeLayoutEpoch () {
  ...
  // shadow sheets (not indexed), inline var() consumers (no subjects), a keyless subject.
  // Everything else is handled per-element by __csimApplyScopedStateDirty at ensureLayout entry.
  const ss = (globalThis.__csimShadowHostCount ||
              (state.dynLayoutVarOnly && inlineVarLayoutSeen) ||
