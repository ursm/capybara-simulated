...
//                                   specificity + selector parse via
//                                   selector-parser.js.
...
// css-tree (vendored) backs stylesheet parsing — replacing the hand-rolled
// parseCssTree/flattenCssTree. Selector matching + specificity still run
// through selector-parser.js downstream (parseSheet). `__csimVendor` is loaded
// (vendor bundle) before this module evaluates, same as selectors.js's cssSelect.
...
function cssTreeFlatten(cssText, vp) {
  const out = [];
  let ast;
  try { ast = CT.parse(cssText, { parseValue: false }); }
  catch (_) { return out; }
  const emitRule = (ruleNode, parentSel) => {
  ...
  const visit = (children, parentSel) => {
    if (!children) return;
    children.forEach(node => {
      if (node.type === 'Rule') {
        ...
        // Per CSS Nesting these are `&`-relative, so reparse with `& ` prepended
        // and emit. (Non-rule Raw — no `{`, or at top level — is genuine
        // garbage / stray text the old parser also dropped.)
        if (parentSel && node.value.indexOf('{') !== -1) {
          ...
          } catch (_) { /* unparseable — drop, as the old parser would */ }
        }
