...
function pushSheetWithImports(sheets, parsed, baseHref, vp, seen) {
  ...
  if (imps && imps.length) {
    for (const imp of imps) {
      ...
      // Resource Timing: the imported sheet is a `css` resource (its body sizes the entry; a
      // sheet that failed to load is a network error).
      if (typeof globalThis.__csimRecordResource === 'function') {
        const fields = typeof globalThis.__csimAssetTimingFields === 'function' ? globalThis.__csimAssetTimingFields(abs, parsed ? parsed.cssText : null)
                                                                                : { body: parsed ? parsed.cssText : undefined, status: parsed ? 200 : undefined };
        globalThis.__csimRecordResource({ name: abs, initiatorType: 'css', startTime: timingStart, ...fields });
      }
