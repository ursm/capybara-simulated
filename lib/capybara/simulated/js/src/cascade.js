...
import { normalizeColor, declaredValue, fontRelativeToPx } from './style-proxy.js';
...
export function rebuildCascade(doc) {
  ...
  // the new one. Indexes (`hideIdx`/`layoutIdx`) are rebuilt lazily from each
  // rule's precomputed `term`, so they aren't cached.
  ...
  const { hide, layout } = collectCascadeRules(doc, selectedSet);
...
function parseSheet(cssText, vp) {
  ...
  try { flat = cssTreeFlatten(cssText, vp); } catch (_) { return { hide, layout, count: 0, layers: [], imports: [], fontSrcs: [] }; }
  ...
  return { hide, layout, count: serial, layers: flat.layers, imports: flat.imports || [], fontSrcs: flat.fontSrcs || [] };
}
...
// The property-filtered variant of the candidate walk, for `cascadedRecord`: it wants only
// the candidate rules that CAPTURE `prop`, but a bucket holds every rule whose terminal
// matches the element — on a Discourse-scale sheet that's hundreds of own-property probes
// per (element, property) read, and `hasOwnProperty` on the captured maps was the #1 JS
// builtin in the tick profile. Filter each bucket once per (index build, property) instead:
// the sub-lists are memoised on the bucket array itself — rebuilt with the index at each
// cascade version, so no staleness — and carry the captured entry alongside the rule so the
// hot loop re-reads nothing. Considered-rule order and count are unchanged, so the
// dynamic-selector taint (`noteDynamic` on every rule with a capture) is exactly preserved.
function bucketPropList(bucket, prop) {
  let m = bucket.__byProp;
  if (!m) m = bucket.__byProp = new Map();
  let list = m.get(prop);
  if (!list) {
    list = [];
    for (const r of bucket) { const cap = own(r.captured, prop); if (cap) list.push([r, cap]); }
    m.set(prop, list);
  }
  return list;
}
function forEachCandidatePropRule(idx, el, prop, cb) {
  const tagBucket = idx.byTag.get(el._tag);
  if (tagBucket) for (const p of bucketPropList(tagBucket, prop)) cb(p[0], p[1]);
  const idAttr = el._attrs.id;
  if (idAttr) {
    const idBucket = idx.byId.get(idAttr);
    if (idBucket) for (const p of bucketPropList(idBucket, prop)) cb(p[0], p[1]);
  }
  for (const c of classes(el)) {
    const cb2 = idx.byClass.get(c);
    if (cb2) for (const p of bucketPropList(cb2, prop)) cb(p[0], p[1]);
  }
  if (idx.universal.length) for (const p of bucketPropList(idx.universal, prop)) cb(p[0], p[1]);
}
...
function cascadedRecord (el, prop) {
  ...
  if (!encapsulated && rules.length && rulesIndexHas(prop)) {
    ...
    forEachCandidatePropRule(state.layoutIdx, el, prop, (r, cap) => {
