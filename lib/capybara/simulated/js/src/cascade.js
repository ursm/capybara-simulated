...
import { splitTopLevel, decodeDataUrlCss, resolveCssUrls, documentBaseUrl, parseStyleDeclList, cssPropertyName, isSupportedCssPropertyName, serializeCssValue, htmlDimensionValue, htmlPixelLength, htmlInteger } from './css-utils.js';
...
// …and WHICH of them, because they do not process white space the same way: `pre-line` collapses
// spaces and tabs and preserves only newlines, where the other three preserve everything. Treating
// them alike collapsed nothing under `pre-line` and kept runs a browser folds.
export function elementWhiteSpace(node) {
  if (node._tag === 'pre') return 'pre';
  const v = cascadedProperty(node, 'white-space');
  return (v != null && WS_PRESERVING_VALUES.has(v)) ? v : null;
}
...
function collectText(node, transform, preserveWs) {
  ...
  if (node.nodeType === NODE_TEXT) {
    ...
    if (preserveWs === undefined) {
      for (let cur = node._parent; cur; cur = cur._parent) {
        ...
        const ws = elementWhiteSpace(cur);
        if (ws) { preserveWs = ws; break; }
      }
    }
    ...
    const collapsesSpaces = !preserveWs || preserveWs === 'pre-line';
  ...
  if (node.nodeType === NODE_ELEMENT) {
    ...
    if (!preserveWs) preserveWs = elementWhiteSpace(node) || preserveWs;
