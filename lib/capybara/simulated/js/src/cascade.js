...
export function fontFaceRulesOf(doc) {
    ...
    for (let i = 0; i < rules.length; i++) {
      ...
      switch (r.type) {
        ...
        case 3: {                                                     // @import
          ...
          if (!sheet || seen.has(sheet)) break;
          seen.add(sheet);
          walk(sheet.cssRules, sheet.href || base, depth + 1);
          break;
        }
        case 4:  if (mediaMatches(r.media ? r.media.mediaText : r.conditionText, vp)) walk(r.cssRules, base, depth + 1); break;
        case 12: if (supportsMatches(r.conditionText)) walk(r.cssRules, base, depth + 1); break;
        default: if (r.cssRules && r.type !== 7) walk(r.cssRules, base, depth + 1);   // @layer blocks and the like; not keyframes
      }
    }
  };
  ...
  for (let i = 0; sheets && i < sheets.length; i++) {
    ...
    walk(rules, sheet.href || docBase, 0);
  }
  const adopted = doc.adoptedStyleSheets;
