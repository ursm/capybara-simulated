...
import { walk, walkFind, classes, scriptText, pictureSourceFor, flatTreeParent, assignedSlotFor } from './walk.js';
...
import { splitTopLevel, stripCssComments, decodeDataUrlCss, resolveCssUrls, documentBaseUrl, parseStyleDeclList, cssPropertyName, isSupportedCssPropertyName, serializeCssValue, htmlDimensionValue, htmlPixelLength, htmlInteger } from './css-utils.js';
...
import { ensureBuilt, arenaPageGen } from './native-query-shadow.js';   // native-authoritative cascade matching over the synced arena
import { normalizeColor, declaredValue, declaredValueIn, fontRelativeToPx, uaDefault, uaHidden, uaDisplay, inputType } from './style-proxy.js';
import { hasMathFunction, reduceMathFunctions, absoluteToPx, unprefixMath } from './calc.js';
...
// realm never gets it, so `el._nid` is unset there and every match takes the css path, until the arena
// is partitioned per realm. Re-resolved per rebuildCascade entry (a boot-time rebuild precedes the seed
// — caching-once would latch it off).
let cascadeAuthoritative = false;
// Snapshotted once per rebuildCascade entry (right after ensureBuilt). Eager construction sets `_nid` on
// every element, so safeMatches can no longer read `_nid != null` as "mirrored + linked" — a mid-parse
// element, a shadow-tree element (kept on css-select for scoping), or a detached one all carry a `_nid`
// with no wired parent chain. mirrorCreate stamps `el._nidGen` with the page generation as it LINKS an
// element into the document arena, so `el._nidGen === cascadePageGen` is the precise per-element gate.
let cascadePageGen = -1;
...
function safeMatches(el, r) {
  ...
  if (cascadeAuthoritative && !r.ns) {
    ...
    if (h >= 0) {
      // Trust the arena only for an element mirrored + tree-linked THIS page (its `_nidGen` matches). A
      // shadow-tree / detached / still-parsing element carries an eager `_nid` but no wired parent chain.
      if (el._nidGen === cascadePageGen) {
        const nm = globalThis.__dom.matchesCompiled(el._nid, h);
        if (nm === true || nm === false) { cascNatCalls++; return nm; }
        cascNatUnmirrored++;   // node stale / not in the arena (coverage gap; ~0 once built) → css-select.
      } else {
        cascNatUnmirrored++;   // element not linked in the document arena → css-select.
      }
    } else {
  ...
  try {
    if (cascadeTiming) {
      ...
      const res = r.ns ? matchesSelectorNS(el, r.selectorText, r.ns) : matchesSelector(el, r.selectorText);
      ...
      if (nid != null && !r.ns) {
        ...
        const nm = globalThis.__dom.matchesId(nid, r.selectorText);
...
globalThis.__csimCascadeTimingStats = function (reset) {
  const snap = { cascMatchNs, cascMatchCalls, cascTotalNs, cascRuns, cascNatNs, cascNatCalls, cascNatFallback, cascNatMismatch, cascNatUnmirrored };
  if (reset) {
    cascMatchNs = 0; cascMatchCalls = 0; cascTotalNs = 0; cascRuns = 0;
    cascNatNs = 0; cascNatCalls = 0; cascNatFallback = 0; cascNatMismatch = 0; cascNatUnmirrored = 0;
  }
  return snap;
};
...
function isVisibleNodeImpl(el, ignoreVisibility, honourSkips, renderedAt = null) {
  ...
  let cur = el, prev = null, connected = false, summarySeen = false;
  while (cur) {
    ...
    if (cur.nodeType === NODE_ELEMENT) {
      ...
      // A CLOSED `<details>` SKIPS all but its `<summary>` — laid out, but not shown: Chrome gives the content a rect
      // and says `checkVisibility()` false, and a user sees none of it. The child on the path is asked whether an AUTHOR
      // rule gave it a display of its own (Discourse select-kit's `.is-expanded .select-kit-body { display: flex }`
      // does), which shows it.
      if (honourSkips && cur !== el && cur._tag === 'details' && cur._attrs.open == null && !summarySeen) {
...
const state = {
  ...
  // `@keyframes` by animation name (last declaration of a name wins — see `collectCascadeRules`).
  keyframes: new Map(),
  ...
  ctxChildListPositions: null,   // the POS_ bits those child-list reads move by (`forPositionedChildren`); null: all
  ctxEmptySibling: 2,
...
// Whether the document declares an `@font-face` with a `url()` src (font-metrics.js gates the
// per-run face lookup on this — a page without a web font never walks a stylesheet's rules).
globalThis.__csimDocHasFontFace = () => { ensureCascadeFresh(); return !!state.hasFontFace; };
...
globalThis.__csimDocHasCssImage = () => { ensureCascadeFresh(); return !!state.hasCssImage; };
...
export function rebuildCascade(doc) {
  ...
  // Native-authoritative matching needs the arena mirrored before the first match, since the initial
  // cascade can precede any querySelector (the other place the arena is lazily built). ensureBuilt is
  // idempotent (and self-heals across a navigation) — a no-op once built, then the mutation seams keep
  // it current — so this pays the one-time mirror at the first post-parse cascade and nothing after.
  // ensureBuilt itself declines to build WHILE the parser is appending (parse5-adapter's in-place
  // tokenize bypasses the mutation sync seams, so an arena mirrored then would go stale as later
  // siblings arrive); until it has built, `el._nid` is unset and safeMatches takes the css path. So a
  // mid-parse `<script>` reading style falls back to css, and the arena builds at the first cascade
  // once the parse has returned (the final cascade the loader runs on the complete tree).
  cascadeAuthoritative = globalThis.__csimNativeCascadeAuthoritative === true && globalThis.__dom != null;
  if (cascadeAuthoritative) { ensureBuilt(); cascadePageGen = arenaPageGen(); }
  ...
  // …and the writes that arrived while the rules were changing, answered now that they are known.
  if (!cascadeStale) flushLayoutWrites();
}
...
function rebuildCascadeImpl(doc) {
  ...
  const { hide, layout, sheets, keyframes } = collectCascadeRules(doc, selectedSet);
  ...
  state.layoutGate = buildLayoutGate(hide, layout, reachable);
  state.ctxGate = undefined;   // the next write that asks builds it from `state.sheets` (CtxGate)
  state.hasFontFace = sheets.some((e) => e.sheet && e.sheet.fontSrcs && e.sheet.fontSrcs.length);
  state.hasCssImage = sheets.some((e) => e.sheet && e.sheet.cssImage);
}
...
export function resetCascadeState() {
  ...
  state.layoutGate = null;   // no rule index = no answer: every write marks the writer's subtree
  state.dynStateSubjects = null;
  state.dynStateEpochFallback = false;
  state.sheets = [];
  CTX_SWEEPS.length = 0;
  // `null` here means "no cascade yet, stay conservative", and `ctxGateReady` keeps it until
  // `rebuildCascade` sets `undefined` — the staleness check there skips `null` for exactly that
  // reason, so there is no second version to keep in step.
  state.ctxGate = null;
  state.ctxGateSheets = -1;
  state.ctxChildListDesc = true;
  state.ctxChildListPositions = null;
...
const PAINT_ONLY_PROPS = new Set([
  ...
  'transition-behavior',
  // …and animations: an `animation` declaration moves no box itself — what an animation moves it
  // moves through the values it animates, read per property — and the stacking context a current
  // one makes is a PAINT question, asked live at hit-test time (layout.js `currentlyAnimatesAnyOf`
  // under `stackChain`'s taint bracket). Discourse's `[contenteditable=true]:focus-within
  // { animation }` otherwise counted as a dynamic LAYOUT rule — and a keyless one, which pushed
  // every focus flip into the epoch.
  'animation', 'animation-name', 'animation-duration', 'animation-timing-function', 'animation-delay',
  'animation-iteration-count', 'animation-direction', 'animation-fill-mode', 'animation-play-state',
  'animation-composition', 'animation-timeline',
  // `opacity` paints too: below 1 it makes a stacking context (layout.js `compositesAsGroup`),
  // which orders hit-testing and painting but moves nothing — asked live, under the same
  // `stackChain` taint bracket as `z-index` below.
  'opacity',
...
const PSEUDO_SUBJECT_RE = /:{1,2}(before|after)\s*$/i;
// A `::before` / `::after` rule: the one kind of pseudo-element rule that styles a box this engine
// lays out (through its originating element), so the dynamic-state and token gates keep it.
function isGeneratedContentRule(rule) { return PSEUDO_SUBJECT_RE.test(rule.selectorText || ''); }
...
// ── the layout gate ─────────────────────────────────────────────────────────────────────────
// Which BOXES a mutation can move through the selectors, read off the rules that move boxes (`ruleMovesBoxes`) and
// answered as layout-dirty marks. A flipped match on an element moves its whole SUBTREE (what it declares may
// inherit), so every answer is "these elements, with their subtrees" — and the art is in not naming too many.
//
// An identifier token a write flips (a class, an id, an attribute, with the value conditions an attribute selector
// carries) reaches, by the position of its compound:
//   the subject          — the writer itself;
//   an ancestor compound — the subjects matching the group's key UNDER the writer (`body.os-pc .os-host { … }` does
//                          not cost the whole page its memos when a scrollbar library stamps a class on `<body>`);
//   left of `+` / `~`    — the writer's NEXT sibling / its FOLLOWING siblings (and all its siblings for a token of
//                          `:nth-child(… of S)`), never its parent's subtree.
// A child-list change moves positions, and only from the CHANGE POINT, as Blink's sibling invalidation reads them:
// `:first-child` and `+` flip the element after it, `:last-child` the one before, `:nth-child()` and `~` everything
// after, `:nth-last-child()` everything before, the `-of-type` forms the nearest element of the changed tag. Appending
// a row to a table under `tr:nth-child(odd) td` flips nothing that exists — it once relaid the whole table out per row,
// 6.6x on 600 rows. `:empty` flips on the element whose child list or text changed, and through a sibling combinator on
// its siblings. And a `:has()` flips UP the chain: the elements matching the compound that carries it (at any depth
// of `:not()` / `:is()`), filtered by what its argument can be flipped by (its tokens, its subjects inserted or
// removed, `:empty`).
// Each of those used to be missed, and each left a subtree's layout stale while `getComputedStyle` was right:
// `.c:has(.flag) p` with the class added under `.c`, `#q:nth-child(2) p` after a sibling was prepended, the old first
// child keeping a `:first-child { white-space: pre }`, `.e:empty + div p` after `.e` was emptied — in BOTH engines,
// and the native walk's subtree reuse kept them too.
//
// The direction of every fallback is the expensive one: an unparseable selector reaches its tokens' writer and every
// sibling and moves every position; a sheet no index holds that can reach the writer adds the writer's subtree.
const TOKEN_SUBTREE = 2, TOKEN_VAR = 4;

function ruleMovesBoxes(r, reachable, isHide) {
  // A `::before` / `::after` rule moves boxes THROUGH its originating element — the generated
  // box is that element's first or last child — so a token flip it mentions relays the subtree;
  // so does a `::part()` rule, whose subject is a real element one tree in, under the host the
  // flip restyles (`#w:has(.f) x-a::part(p)` never relaid out). Any other pseudo-element
  // (scrollbar parts, `::selection`) styles nothing laid out here.
  if (subjectIsPseudoElement(r)) {
    if (!PSEUDO_SUBJECT_RE.test(r.selectorText || '') && !hasPartSelector(r)) return 0;
    if (isHide) return TOKEN_SUBTREE;
    for (const prop in r.captured) {
      if (prop === 'content' || (!prop.startsWith('--') && !PAINT_ONLY_PROPS.has(prop))) return TOKEN_SUBTREE;
    }
    return 0;
  }
...
// Positions a child-list change moves, as seen from the change point.
const POS_NEXT = 1, POS_FOLLOWING = 2, POS_PREV = 4, POS_PRECEDING = 8, POS_NEXT_OF_TYPE = 16, POS_PREV_OF_TYPE = 32;
const POSITION_KINDS = {
  'first-child': POS_NEXT, 'last-child': POS_PREV, 'only-child': POS_NEXT | POS_PREV,
  'nth-child': POS_FOLLOWING, 'nth-last-child': POS_PRECEDING,
  'first-of-type': POS_NEXT_OF_TYPE, 'last-of-type': POS_PREV_OF_TYPE, 'only-of-type': POS_NEXT_OF_TYPE | POS_PREV_OF_TYPE,
  'nth-of-type': POS_FOLLOWING, 'nth-last-of-type': POS_PRECEDING
};
// A compound's reach from its element to the subject: the element itself (the subject), its descendants, or its next /
// following siblings (with theirs) — decided by the RUN of combinators after it: a lone `+` reaches the next sibling;
// a `~`, or a second step (`.a + li + li`: the subject is two along), any following one.
const ROLE_SELF = 0, ROLE_DESC = -1;
function roleAt(combs, ci) {
  const first = combs[ci];
  if (first === undefined) return ROLE_SELF;
  if (first !== 'adjacent' && first !== 'sibling') return ROLE_DESC;
  const second = combs[ci + 1];
  return first === 'adjacent' && second !== 'adjacent' && second !== 'sibling' ? POS_NEXT : POS_FOLLOWING;
}
// The largest index `An+B` can match when A <= 0 (`:nth-child(-n+3)`: 3) — a finite set, so a change moves only the
// elements near that end — or Infinity.
function nthLimit(data) {
  // …counted among ALL the siblings, which `of S` does not: its index counts S's matches only, so no bound holds.
  if (/\sof\s/i.test(String(data))) return Infinity;
  const text = String(data == null ? '' : data).toLowerCase().replace(/\s+/g, '');
  const m = /^([+-]?\d*)n([+-]\d+)?$/.exec(text);
  if (m) {
    const a = m[1] === '' || m[1] === '+' ? 1 : m[1] === '-' ? -1 : parseInt(m[1], 10);
    return a > 0 ? Infinity : Math.max(m[2] ? parseInt(m[2], 10) : 0, 0);
  }
  return /^[+-]?\d+$/.test(text) ? Math.max(parseInt(text, 10), 0) : Infinity;
}
const IDENT_RE = /\.([\w -￿-]+)|#([\w -￿-]+)|\[\s*([\w -￿:|-]+)/g;
function identTokens(text) {
  const out = [];
  IDENT_RE.lastIndex = 0;
  let m;
  while ((m = IDENT_RE.exec(text))) out.push(m[1] ? '.' + m[1].toLowerCase() : m[2] ? '#' + m[2].toLowerCase() : '[' + m[3].toLowerCase().replace(/^.*[|:]/, ''));
  return out;
}
// What a box-moving selector can be flipped by, for the layout gate: `tokens` [token, role, subjectKey], `attrTests`,
// `positions` (the POS_ bits a child-list change moves), `empty` / `emptyFollow` (an `:empty`, and the siblings it
// reaches), `has` (one entry per compound carrying a `:has()`).
function layoutFeatures(selText) {
  const f = { tokens: [], attrTests: [], positions: 0, empty: false, emptyFollow: 0, has: [] };
  let groups;
  try { groups = CW.parse(selText); }
  catch (_) {
    for (const tok of identTokens(selText)) { f.tokens.push([tok, ROLE_SELF, null]); f.tokens.push([tok, POS_FOLLOWING | POS_PRECEDING, null]); }
    f.positions = POS_FOLLOWING | POS_PRECEDING; f.empty = true; f.emptyFollow = POS_FOLLOWING;
    return f;
  }
  for (const g of groups) {
    const { compounds, combs } = splitCompounds(g);
    const last = compounds.length - 1;
    const subjectKey = compoundKey(compounds[last]);
    for (let ci = 0; ci <= last; ci++) walkLayoutCompound(f, compounds[ci], roleAt(combs, ci), subjectKey, compounds[ci]);
  }
  const lim = { fwd: -1, bwd: -1 };
  f.positions = positionsOfGroups(groups, lim);
  f.fwdLimit = lim.fwd; f.bwdLimit = lim.bwd;
  return f;
}
// The POS_ bits a child-list change can move a match of these selector groups by: each positional pseudo-class, and
// each sibling combinator (an insertion right before an element changes what precedes it) — a run of them, or one
// after a positional compound, reaching any following element — nested `:not()` / `:is()` arguments included, a
// `:has()` argument not (that one flips upward, `layoutHasEffect`). `lim` collects how far past the change point the
// FOLLOWING / PRECEDING reach has to go: the largest finite `An+B` bound, or Infinity for an unbounded source.
function positionsOfGroups(groups, lim) {
  let bits = 0;
  for (const g of groups) {
    const { compounds, combs } = splitCompounds(g);
    for (let ci = 0; ci < compounds.length; ci++) {
      ...
      for (const t of compounds[ci]) {
        ...
        const name = String(t.name).toLowerCase();
        const kind = POSITION_KINDS[name];
        if (kind) {
          own |= kind;
          if (lim) {
            // …only the child-counting forms have a bound here: an `-of-type` index counts fewer elements than the walk.
            const bound = name === 'nth-child' || name === 'nth-last-child' ? nthLimit(t.data) : Infinity;
            if (kind & POS_FOLLOWING) lim.fwd = Math.max(lim.fwd, bound);
            if (kind & POS_PRECEDING) lim.bwd = Math.max(lim.bwd, bound);
          }
        }
        if (name !== 'has' && Array.isArray(t.data)) own |= positionsOfGroups(t.data, lim);
      }
      const role = roleAt(combs, ci);
      if (role > 0) {
        bits |= role;
        if (own) own |= POS_FOLLOWING;
        if (lim && (role | own) & POS_FOLLOWING) lim.fwd = Infinity;
      }
      bits |= own;
    }
  }
...
// The children of `el` a child-list change reaches through `positions`, from the change point — `before` / `after` the
// nodes either side of it (undefined where no one can say: then all of them). Shared by the layout gate and the
// cascade's context epochs (`ctxChildListEffect`), which ask it of different rules.
// With a finite `fwdLimit` (`:nth-child(-n+3)`: only the first 3 can match) the FOLLOWING reach stops at the elements
// whose new index can have crossed it — at most as many past it as elements arrived — and likewise `bwdLimit` from the
// other end: appending a row under `tr:nth-last-child(-n+2)` reaches the last three rows, not all of them.
function forPositionedChildren(el, positions, added, removed, before, after, fn, fwdLimit = Infinity, bwdLimit = Infinity) {
  if (!positions) return;
  if (before === undefined || after === undefined) {
    for (const c of el._children) if (c.nodeType === NODE_ELEMENT) fn(c);
    return;
  }
  const prev = elementAtOrBefore(before), next = elementAtOrAfter(after);
  let arrived = 0;
  for (const n of added) if (n.nodeType === NODE_ELEMENT) arrived++;
  if (positions & (POS_NEXT | POS_FOLLOWING) && next) {
    fn(next);
    if (positions & POS_FOLLOWING) {
      // …`index`: `next`'s new 1-based position, counted back only as far as the bound needs.
      const stop = fwdLimit + Math.max(arrived, 1);
      let index = 1 + arrived;
      if (stop !== Infinity) for (let x = prev; x && index <= stop; x = x.previousElementSibling) index++;
      for (let x = next.nextElementSibling; x && ++index <= stop; x = x.nextElementSibling) fn(x);
    }
  }
  if (positions & (POS_PREV | POS_PRECEDING) && prev) {
    fn(prev);
    if (positions & POS_PRECEDING) {
      const stop = bwdLimit + Math.max(arrived, 1);
      let index = 1 + arrived;
      if (stop !== Infinity) for (let x = next; x && index <= stop; x = x.nextElementSibling) index++;
      for (let x = prev.previousElementSibling; x && ++index <= stop; x = x.previousElementSibling) fn(x);
    }
  }
  if (positions & (POS_NEXT_OF_TYPE | POS_PREV_OF_TYPE)) {
    for (const n of added.concat(removed)) {
      if (n.nodeType !== NODE_ELEMENT) continue;
      if (positions & POS_NEXT_OF_TYPE) for (let x = next; x; x = x.nextElementSibling) if (x.localName === n.localName) { fn(x); break; }
      if (positions & POS_PREV_OF_TYPE) for (let x = prev; x; x = x.previousElementSibling) if (x.localName === n.localName) { fn(x); break; }
    }
  }
}
function splitCompounds(g) {
  const compounds = [[]], combs = [];
  for (const t of g) {
    if (CW_COMBINATORS.has(t.type)) { combs.push(t.type); compounds.push([]); continue; }
    compounds[compounds.length - 1].push(t);
  }
  return { compounds, combs };
}
// One compound whose element reaches the subject by `role`; `top` is the TOP-LEVEL compound it belongs to, which a
// `:has()` nested in its `:not()` / `:is()` still hangs on.
function walkLayoutCompound(f, tokens, role, subjectKey, top) {
  for (const t of tokens) {
    if (t.type === 'attribute') {
      const name = String(t.name).toLowerCase();
      let tok;
      if (name === 'class' && t.action === 'element') tok = '.' + String(t.value).toLowerCase();
      else if (name === 'id' && t.action === 'equals') tok = '#' + String(t.value).toLowerCase();
      else {
        tok = '[' + name;
        f.attrTests.push([tok, t.action, t.value == null ? '' : String(t.value), t.ignoreCase !== false]);
      }
      f.tokens.push([tok, role, subjectKey]);
      continue;
    }
    if (t.type !== 'pseudo') continue;
    const name = String(t.name).toLowerCase();
    // `:nth-child(… of S)`: which siblings count is S's, so a token of it reaches every sibling.
    if (POSITION_KINDS[name] && typeof t.data === 'string') for (const tok of identTokens(t.data)) f.tokens.push([tok, POS_FOLLOWING | POS_PRECEDING, null]);
    if (name === 'empty') { f.empty = true; if (role > 0) f.emptyFollow |= role; }
    const inh = INHERITED_ATTR_PSEUDOS[name];
    if (inh) for (const a of inh) f.tokens.push(['[' + a, ROLE_SELF, null]);
    const own = ELEMENT_ATTR_PSEUDOS[name];
    if (own) for (const a of own) f.tokens.push(['[' + a, role, subjectKey]);
    if (name === 'has') { f.has.push(layoutHasEntry(top, role, t.data)); continue; }
    if (Array.isArray(t.data)) {
      // `:not()` / `:is()` / `:where()`: a nested compound speaks for this element; one of a nested COMPLEX selector is
      // an ancestor of it (whatever this element reaches is under the writer) or a sibling before it (whatever it reaches
      // is among the writer's following siblings or under them).
      for (const ng of t.data) {
        const { compounds, combs } = splitCompounds(ng);
        const nlast = compounds.length - 1;
        for (let ci = 0; ci <= nlast; ci++) {
          walkLayoutCompound(f, compounds[ci], ci === nlast ? role : (roleAt(combs, ci) === ROLE_DESC ? ROLE_DESC : POS_FOLLOWING), subjectKey, top);
        }
      }
    }
  }
}
// One `:has()`, hung on the TOP-LEVEL compound carrying it: `sel` the `:has()` alone (what a candidate is re-asked,
// `hasCheck`), `key` the query naming its candidates (the compound without its `:has()`; null = any element), `follow` how far past its element the match reaches (the POS_ bits of a sibling combinator after it).
// And what its ARGUMENT can be flipped by: the tokens it names (the attributes behind `:disabled` / `:lang()` too), the
// subject keys of its groups (an element inserted or removed that matches or holds one; null: any), whether it reads
// `:empty` or a POSITION (`:has(.y:first-child)`: a child-list change under the element can flip it), `sideways` — an
// argument relative to the element's SIBLINGS (`:has(+ .x)`: POS_NEXT, `:has(~ .x)`: POS_FOLLOWING) — and `opaque`
// where its tokens cannot be told.
function layoutHasEntry(top, role, groups) {
  const h = { key: null, sel: null, follow: role > 0 ? role : 0, tokens: [], argKeys: [], empty: false, positions: 0, sideways: 0,
              opaque: !Array.isArray(groups), dynamic: false };
  try {
    const rest = top.filter((t) => !(t.type === 'pseudo' && String(t.name).toLowerCase() === 'has'));
    h.key = compoundKey(rest) || (rest.length ? CW.stringify([rest]) : null);
    // …and only the ARGUMENT is re-asked: the rest of the compound flips with writes to the element itself (its class, a
    // hover, its position), which mark it on their own and never pass through here — so a kept answer that included
    // them lagged behind the layout, and a later flip back read as no change.
    if (Array.isArray(groups)) h.sel = CW.stringify([[{ type: 'pseudo', name: 'has', data: groups }]]);
  } catch (_) { h.sel = null; }
  if (h.opaque) return h;
  // …a DYNAMIC state inside it (`:has(:checked)`): the scoped state sweep marks what that flips, and knows nothing of the
  // answer kept here — so the kept answer can lag behind what the layout last saw, and is never trusted.
  h.dynamic = hasDynamicPseudo(groups);
  const relative = [];
  for (const hg of groups) {
    const lead = hg.length ? hg[0].type : null;
    if (lead === 'adjacent') h.sideways |= POS_NEXT;
    else if (lead === 'sibling') h.sideways |= POS_FOLLOWING;
    relative.push(CW_COMBINATORS.has(lead) ? hg.slice(1) : hg);
  }
  // …the positions INSIDE the argument, not the combinator it starts with: that one is `sideways`.
  h.positions = positionsOfGroups(relative);
  const sub = { tokens: [], attrTests: [], positions: 0, empty: false, emptyFollow: 0, has: [] };
  for (const hg of relative) {
    const { compounds } = splitCompounds(hg);
    for (const c of compounds) walkLayoutCompound(sub, c, ROLE_SELF, null, c);
    h.argKeys.push(compoundKey(compounds[compounds.length - 1]));
  }
  for (const [tok] of sub.tokens) if (h.tokens.indexOf(tok) === -1) h.tokens.push(tok);
  h.empty = sub.empty;
  // …a `:has()` inside it: whatever flips that one can flip this one.
  for (const inner of sub.has) {
    for (const tok of inner.tokens) if (h.tokens.indexOf(tok) === -1) h.tokens.push(tok);
    for (const k of inner.argKeys) h.argKeys.push(k);
    if (inner.empty) h.empty = true;
    if (inner.opaque) h.opaque = true;
    h.positions |= inner.positions;
    h.sideways |= inner.sideways;
  }
  return h;
}
function newLayoutIndex() {
  return { map: Object.create(null), positions: 0, fwdLimit: -1, bwdLimit: -1, empty: false, emptyFollow: 0, has: [] };
}
function noteLayoutFeatures(ix, f) {
  const entry = (tok) => ix.map[tok] || (ix.map[tok] = { self: false, sib: 0, desc: null, t: null, a: false });
  for (const [tok, action, value, ic] of f.attrTests) {
    const e = entry(tok);
    if (action === 'exists' || action === 'not' || value === '') { e.a = true; continue; }
    (e.t || (e.t = [])).push([action, ic ? value.toLowerCase() : value, ic]);
  }
  for (const [tok, role, subjectKey] of f.tokens) {
    const e = entry(tok);
    if (role === ROLE_SELF) e.self = true;
    else if (role > 0) e.sib |= role;
    else if (subjectKey === null) e.self = true;       // keyless subject under the writer: its subtree
    else (e.desc || (e.desc = new Set())).add(subjectKey);
  }
  ix.positions |= f.positions;
  if (f.positions & POS_FOLLOWING) ix.fwdLimit = Math.max(ix.fwdLimit, f.fwdLimit);
  if (f.positions & POS_PRECEDING) ix.bwdLimit = Math.max(ix.bwdLimit, f.bwdLimit);
  if (f.empty) ix.empty = true;
  ix.emptyFollow |= f.emptyFollow;
  for (const h of f.has) ix.has.push(h);
}
// …built at rebuild, from the box-moving rules; the custom-property-only ones (`TOKEN_VAR`) join once an inline style
// has consumed a custom property in a box-moving position (`inlineVarLayoutSeen`), in an index of their own.
function buildLayoutGate(hide, layout, reachable) {
  const core = [], varOnly = [];
  for (const list of [hide, layout]) {
    const isHide = list === hide;
    for (const r of list) {
      const mode = ruleMovesBoxes(r, reachable, isHide);
      if (mode & TOKEN_SUBTREE) core.push(r);
      else if (mode & TOKEN_VAR) varOnly.push(r);
    }
  }
  return { core: layoutIndexOf(core), coreRules: core, varOnly, withVar: null };
}
function layoutIndexOf(rules) {
  const ix = newLayoutIndex();
  for (const r of rules) {
    let f = r.__layoutF;
    if (f === undefined) f = r.__layoutF = layoutFeatures(r.selectorText || '');
    noteLayoutFeatures(ix, f);
  }
  // …what can flip a `:has()` argument, gathered once.
  ix.hasSideways = 0; ix.hasOpaque = false; ix.hasEmpty = false; ix.hasArgAny = false; ix.hasPositions = false;
  ix.hasTokens = new Set();
  const argKeys = new Set();
  for (const h of ix.has) {
    ix.hasSideways |= h.sideways;
    if (h.opaque) ix.hasOpaque = true;
    if (h.empty) ix.hasEmpty = true;
    if (h.positions) ix.hasPositions = true;
    for (const t of h.tokens) ix.hasTokens.add(t);
    for (const k of h.argKeys) { if (k === null) ix.hasArgAny = true; else argKeys.add(k); }
  }
  ix.hasArgQuery = argKeys.size ? [...argKeys].join(',') : null;
  return ix;
}
// The index a mutation at `el` is answered from, or null where none can answer. Only asked once the rule set is known
// (`layoutGateKnown`).
function layoutIndexFor(el) {
  const gate = state.layoutGate;
  // Shadow sheets are not in the document's index: a shadow tree's own `.open .panel { … }` would be invisible to it. An
  // element inside a tree is answered by that tree's own rules (`shadowLayoutIndex`), one outside by the document's. And
  // where a sheet neither index holds can still reach the element (`shadowRulesMayReach`: a `:host(.x)` restyling a
  // host, a `::slotted()` a light child, a `:host-context()` / `::part()` anywhere), its subtree is marked BESIDE that
  // answer (`layoutConservative`) rather than instead of it — instead of it, one `::part()` rule on a page turned off
  // every `:has()` and sibling answer the document's rules gave.
  layoutConservative = false;
  if (el._isShadowRoot || shadowRulesMayReach(el)) {
    const sr = el._isShadowRoot ? el : enclosingShadowRootOf(el);
    // …where `shadowUnsafe` is the document's own `::part()` rule, a light element is answered in full by the document's
    // index (which holds that rule, its subject a box under the host it names) and needs no mark of its own: every
    // child-list change on a page with one `::part()` rule and one shadow tree marked the parent's whole subtree (3x).
    if ((sr !== null ? shadowUnsafe : shadowSheetUnsafe) || (shadowHostFn && el._shadowRoot) ||
        (shadowSlotted && el._parent && el._parent._shadowRoot)) layoutConservative = true;
    if (sr !== null) return shadowLayoutIndex(sr);
  }
  if (!inlineVarLayoutSeen || !gate.varOnly.length) return gate.core;
  if (!gate.withVar) gate.withVar = layoutIndexOf(gate.coreRules.concat(gate.varOnly));
  return gate.withVar;
}
let layoutConservative = false;
// A shadow tree's own box-moving rules as a layout index, kept with its scoped rules (which a cascade version rebuilds).
// A custom property counts as moving a box here: the tree's var graph is in no reachability set.
const NO_REACHABLE = new Set();
function shadowLayoutIndex(sr) {
  const scoped = scopedRulesFor(sr);
  if (scoped.layoutIx === undefined) {
    const rules = scoped.hide.filter((r) => ruleMovesBoxes(r, NO_REACHABLE, true))
      .concat(scoped.layout.filter((r) => ruleMovesBoxes(r, NO_REACHABLE, false)));
    scoped.layoutIx = layoutIndexOf(rules);
  }
  return scoped.layoutIx;
}
// A write that arrives while the rule set is CHANGING — a sheet inserted, removed or edited, and the cascade not yet
// rebuilt — cannot be answered: the rules that decide it are the ones about to arrive. It used to answer "the
// writer's subtree", which is no answer for a `:has()` above the writer or a sibling after it: a frame written with
// `document.write` and then mutated in the same task kept `.c:has(.flag) p` unapplied to its layout. So such a write
// is KEPT and answered once the rules are known — at the rebuild, or at a layout's entry (`settleLayoutInvalidation`),
// whichever comes first — and deliberately NOT by rebuilding at the write: a page boot inserts sheets while its
// framework writes thousands of attributes, and rebuilding per write multiplied the rebuilds. Past a bound, what is
// kept collapses into the one answer that needs no rules: the whole document.
const LAYOUT_WRITES = [];
const LAYOUT_WRITES_MAX = 4096;
let layoutWritesOverflow = null;              // the marker to mark the whole document with, once past the bound
function layoutGateKnown() { return !cascadeStale && state.layoutGate != null; }
function keepLayoutWrite(w, mark) {
  if (layoutWritesOverflow !== null) return;
  if (LAYOUT_WRITES.length >= LAYOUT_WRITES_MAX) { LAYOUT_WRITES.length = 0; layoutWritesOverflow = mark; return; }
  LAYOUT_WRITES.push(w);
}
function flushLayoutWrites() {
  if (layoutWritesOverflow !== null) {
    const mark = layoutWritesOverflow;
    layoutWritesOverflow = null;
    const root = globalThis.document && globalThis.document.documentElement;
    if (root) mark(root, true);
  }
  if (!LAYOUT_WRITES.length) return;
  for (const w of LAYOUT_WRITES.splice(0)) {
    if (w.kind === 0) layoutAttrEffect(w.el, w.lkey, w.oldRaw, w.newRaw, w.mark);
    else if (w.kind === 1) layoutChildListEffect(w.el, w.added, w.removed, w.before, w.after, w.mark);
    else layoutCharDataEffect(w.el, w.text, true, w.mark);
  }
}
// Called at a layout's ENTRY, before any memo is read: the rules made known and every kept write answered.
export function settleLayoutInvalidation() {
  ensureCascadeFresh();
  flushLayoutWrites();
}
// An attribute write on `el` (its own plain mark is the caller's), called mid-write from `recordAttrMutation`.
export function layoutAttrEffect(el, lkey, oldRaw, newRaw, mark) {
  if (!layoutGateKnown()) { keepLayoutWrite({ kind: 0, el, lkey, oldRaw, newRaw, mark }, mark); return; }
  const ix = layoutIndexFor(el);
  if (layoutConservative) mark(el, true);
  let self = false, sib = 0, sweep = null, has = ix.hasOpaque;
  const consider = (tok, oldV, newV) => {
    if (!has && ix.hasTokens.has(tok)) has = true;
    const e = ix.map[tok];
    if (e === undefined) return;
    if (tok[0] === '[' && !attrWriteMatters(e, oldV, newV)) return;
    if (e.self) self = true;
    sib |= e.sib;
    if (e.desc) { if (!sweep) sweep = new Set(); for (const key of e.desc) sweep.add(key); }
  };
  if (lkey === 'class') {
    const a = new Set(String(oldRaw || '').split(/\s+/)), b = new Set(String(newRaw || '').split(/\s+/));
    for (const t of a) if (t && !b.has(t)) consider('.' + t.toLowerCase());
    for (const t of b) if (t && !a.has(t)) consider('.' + t.toLowerCase());
    ...
    consider('[' + (lkey.indexOf(':') === -1 ? lkey : lkey.slice(lkey.lastIndexOf(':') + 1)), oldRaw, newRaw);
  }
  if (self) mark(el, true);
  else if (sweep) markMatchesUnder(el, [...sweep].join(','), mark);
  if (sib) markSiblings(el, sib, mark);
  if (has && ix.has.length) layoutHasEffect(ix, el, mark);
}
// A child-list change on `el` (its structural mark and the arriving nodes' are the caller's): `before` / `after` are the
// nodes on either side of the change (undefined where the caller could not say — then every position is reached).
export function layoutChildListEffect(el, added, removed, before, after, mark) {
  if (!layoutGateKnown()) { keepLayoutWrite({ kind: 1, el, added: added.slice(), removed: removed.slice(), before, after, mark }, mark); return; }
  const ix = layoutIndexFor(el);
  if (layoutConservative) mark(el, true);
  forPositionedChildren(el, ix.positions, added, removed, before, after, (c) => mark(c, true), ix.fwdLimit, ix.bwdLimit);
  // (…a shadow root's own child list is answered by its tree's rules too, and has no `:empty` of its own.)
  const emptied = el.nodeType === NODE_ELEMENT && (ix.empty || ix.hasEmpty) && emptinessMayHaveFlipped(el, added);
  if (emptied && ix.empty) { mark(el, true); if (ix.emptyFollow) markSiblings(el, ix.emptyFollow, mark); }
  if (ix.has.length && ((emptied && ix.hasEmpty) || ix.hasPositions || ix.hasArgAny || nodesHold(added, ix.hasArgQuery) || nodesHold(removed, ix.hasArgQuery))) {
    layoutHasEffect(ix, el, mark);
    // …and a sideways argument's subject is a sibling BEFORE the change point, not on the chain above it: the one right
    // before it for `:has(+ …)`, every one for `:has(~ …)`.
    if (ix.hasSideways) {
      if (before === undefined) { for (const c of el._children) if (c.nodeType === NODE_ELEMENT) hasCheck(ix, c, mark); }
      else {
        const prev = elementAtOrBefore(before);
        if (ix.hasSideways & POS_FOLLOWING) for (let x = prev; x; x = x.previousElementSibling) hasCheck(ix, x, mark);
        else if (prev) hasCheck(ix, prev, mark);
      }
    }
  }
}
// A text edit under `el`: only `:empty` reads text data, and only a text node going empty or leaving it can flip it.
export function layoutCharDataEffect(el, text, emptied, mark) {
  if (!emptied) return;
  if (!layoutGateKnown()) { keepLayoutWrite({ kind: 2, el, text, mark }, mark); return; }
  const ix = layoutIndexFor(el);
  if (layoutConservative) mark(el, true);
  if (!(ix.empty || ix.hasEmpty) || !emptinessMayHaveFlipped(el, [text])) return;
  if (ix.empty) { mark(el, true); if (ix.emptyFollow) markSiblings(el, ix.emptyFollow, mark); }
  if (ix.hasEmpty) layoutHasEffect(ix, el, mark);
}
// Up the chain from `el` (itself included — `:has(> .x)` is its parent's, and `el` may be the one gaining a child), and
// for a sideways argument the siblings before each (the one right before for `:has(+ …)`): each element a `:has()`
// compound can match is asked again (`hasCheck`).
function layoutHasEffect(ix, el, mark) {
  for (let n = el; n && n.nodeType === NODE_ELEMENT; n = n._parent) {
    hasCheck(ix, n, mark);
    if (ix.hasSideways & POS_FOLLOWING) for (let x = n.previousElementSibling; x; x = x.previousElementSibling) hasCheck(ix, x, mark);
    else if (ix.hasSideways) { const x = n.previousElementSibling; if (x) hasCheck(ix, x, mark); }
  }
}
// Whether `n`'s match of each `:has()` compound CHANGED, against what it was the last time it was asked (kept on the
// element, per compound): only a change moves anything — its subtree, or the siblings past it the compound reaches.
// Asked the other way — "could it have changed?" — every row appended under `div:has(> b)` relaid the page's wrapper
// out, and `:has(b)` the whole document, though neither's match ever moved. An element never asked has no answer to
// compare, and one that stops being a candidate forgets it, so a first answer marks: the conservative direction. The
// per-compound entry is the index's own object.
function hasCheck(ix, n, mark) {
  // …kept for the index that asked: a rebuilt rule set is new entries, and an answer kept under the old ones was never
  // tracked for the rules that arrived (nor would the old entries be asked again — a CSS-in-JS page inserting a rule per
  // render grew every chain element's map by one per render).
  if (n._lbHasIx !== ix) { n._lbHasIx = ix; n._lbHas = undefined; }
  for (const h of ix.has) {
    const candidate = h.key === null || safeElementMatches(n, h.key);
    let memo = n._lbHas;
    if (!candidate) { if (memo !== undefined) memo.delete(h); continue; }
    const now = h.sel === null || h.dynamic ? !(memo && memo.get(h)) : safeElementMatches(n, h.sel);
    if (memo === undefined) memo = n._lbHas = new Map();
    if (memo.get(h) === now) continue;
    memo.set(h, now);
    if (h.follow === 0) mark(n, true);
    else markSiblings(n, h.follow, mark);
  }
}
// `el`'s next element sibling (POS_NEXT), every following one (POS_FOLLOWING), every preceding one (POS_PRECEDING).
function markSiblings(el, which, mark) {
  if (which & POS_FOLLOWING) { for (let x = el.nextElementSibling; x; x = x.nextElementSibling) mark(x, true); }
  else if (which & POS_NEXT) { const x = el.nextElementSibling; if (x) mark(x, true); }
  if (which & POS_PRECEDING) for (let x = el.previousElementSibling; x; x = x.previousElementSibling) mark(x, true);
}
function elementAtOrBefore(n) {
  for (let x = n; x; x = x.previousSibling) if (x.nodeType === NODE_ELEMENT) return x;
  return null;
}
function elementAtOrAfter(n) {
  for (let x = n; x; x = x.nextSibling) if (x.nodeType === NODE_ELEMENT) return x;
  return null;
}
function safeElementMatches(el, query) {
  try { return el.matches(query); } catch (_) { return true; }
}
function markMatchesUnder(scope, query, mark) {
  if (typeof scope.querySelectorAll !== 'function') { mark(scope, true); return; }
  let found;
  try { found = scope.querySelectorAll(query); } catch (_) { mark(scope, true); return; }
  for (const x of found) mark(x, true);
}
...
function emptinessMayHaveFlipped(el, changed) {
  for (const c of el._children) {
    if ((c.nodeType === NODE_ELEMENT || (c.nodeType === NODE_TEXT && c._data)) && changed.indexOf(c) === -1) return false;
  }
  return true;
}
// Does any of `nodes` (added or removed) match `query`, or hold an element that does? No query: any element.
function nodesHold(nodes, query) {
  for (const n of nodes) {
    if (n.nodeType !== NODE_ELEMENT) continue;
    if (query === null) return true;
    if (safeElementMatches(n, query)) return true;
    try { if (n.querySelector(query)) return true; } catch (_) { return true; }
  }
  return false;
}
...
function collectDynamicLayoutRules(hide, layout, reachable) {
  ...
  for (const r of hide) {
    ...
    if (ruleIsDynamic(r) && (r.display != null || r.visibility != null) &&
        !(subjectIsPseudoElement(r) && !isGeneratedContentRule(r))) rules.push(r);
  }
  for (const r of layout) {
    ...
    if (subjectIsPseudoElement(r) && !isGeneratedContentRule(r)) continue;
...
globalThis.__csimApplyScopedStateDirty = function () {
  ...
  // These arms keep dynamic state in the layout epoch itself (see cascadeLayoutEpoch): a shadow
  // sheet's box-moving dynamic rules have no subjects this can query (`state.dynStateSubjects` is the
  // document's), and an unparseable selector cannot be queried at all. The hook only acts where the
  // epoch no longer does — the two are a PAIR, and narrowing one without the other is how a page
  // ends up sweeping nothing AND keying on nothing.
  if (shadowSheetFacts() && shadowDynLayout) { takeStyleStateHints(); return; }
...
// those three are the whole answer — plus a shadow tree's own sheets, which are in no rule index and
...
export function mayConstrainSize (el) {
  ensureCascadeFresh();
  if (state.hasMinMaxRule) return true;
...
export function fontFaceRulesOf(doc) {
  ...
  const walk = (rules, base, depth, chain) => {
    ...
    for (let i = 0; i < rules.length; i++) {
      ...
      if (!r) continue;
      switch (r.type) {
        case 5: out.push({ rule: r, base }); break;
        case 3: {                                                     // @import
          ...
          walk(sheet.cssRules, href || base, depth + 1, href ? chain : chain);
          if (href) chain.delete(href);
          break;
        }
        case 4:  if (mediaMatches(r.media ? r.media.mediaText : r.conditionText, vp)) walk(r.cssRules, base, depth + 1, chain); break;
        case 12: if (supportsMatches(r.conditionText)) walk(r.cssRules, base, depth + 1, chain); break;
        default: if (r.cssRules && r.type !== 7) walk(r.cssRules, base, depth + 1, chain);   // @layer blocks and the like; not keyframes
      }
    }
  };
  ...
  for (let i = 0; sheets && i < sheets.length; i++) {
    ...
    if (owner && owner._attrs) {
      ...
      if (owner._tag === 'link') {
        const tokens = (owner._attrs.rel || '').toLowerCase().split(/\s+/);
...
function expandDeclaration (prop, raw, important, emit) {
  const value = unprefixMath(raw);                     // (`-webkit-calc()` is `calc()`, for both origins)
...
function cssTreeFlatten(cssText, vp) {
  ...
  // which the document-level merge in `collectCascadeRules` decides.
  const keyframes = [];
  ...
  catch (_) { return { rules: out, layers, imports, fontSrcs, cssImage: false }; }
  visit(ast.children, cssText, null, null);
  // A coarse gate for the rendering-update fetch of CSS-embedded images: does any rule set
  // `background(-image)` / `cursor` / `list-style(-image)` to a `url()`? (`initiator-type/style`.)
  const cssImage = /(?:background|cursor|list-style)[^:;{}]*:[^;{}]*url\(/i.test(cssText);
  return { rules: out, layers, imports, fontSrcs, keyframes, cssImage };
}
...
function __sheetCacheKey(text, vp) {
  ...
  // their `__subjPE` / `dynReqs` / `dynSubs` (noteDynamicRuleData). v8: `cxIndex.positions`.
  return 'v8:' + text.length + ':' + (h >>> 0).toString(16) + ':' + vp.width + 'x' + vp.height;
}
...
function parseSheet(cssText, vp) {
  ...
  try { flat = cssTreeFlatten(cssText, vp); } catch (_) { return { hide, layout, count: 0, layers: [], imports: [], fontSrcs: [], keyframes: [], cssImage: false, cxIndex: newCtxIndex(), varGraph: finishVarGraph(newVarGraph()) }; }
  ...
  return { hide, layout, count: serial, layers: flat.layers, imports: flat.imports || [], fontSrcs: flat.fontSrcs || [], keyframes: flat.keyframes || [], cssImage: !!flat.cssImage, cxIndex, varGraph: finishVarGraph(varGraph) };
}
...
function collectCascadeRules(doc, selectedSet) {
  const empty = { hide: [], layout: [], sheets: [], keyframes: new Map() };
  ...
  return { hide, layout, sheets, keyframes };
}
...
export function noteUncacheableRead() { dynamicSeq++; }
...
function buildPseudoIndex(which, rules) {
  ...
  for (const r of rules) {
    ...
    const m = PSEUDO_SUBJECT_RE.exec(r.selectorText);
    if (!m || m[1].toLowerCase() !== which) continue;
    ...
    const derived = { ...r, selectorText: stripped, term, anc: ancestorHashes(stripped), pseudo: which };
    for (const prop in r.captured) {
...
function pseudoIndex(which) {
  ensureCascadeFresh();
  let both = state.pseudoIdx;
  if (both === null) both = state.pseudoIdx = { before: buildPseudoIndex('before', state.layoutRules), after: buildPseudoIndex('after', state.layoutRules), any: undefined };
  return both[which];
}
...
function shadowPseudoIndex(sr, which) {
  ...
  if (!cache || cache.rules !== rules) cache = sr._pseudoIdx = { rules, before: null, after: null };
...
export function pseudoContentCandidate (el, which) {
  ...
  const id = el._attrs.id;
  if (id && sub.byId.has(id)) return true;
  if (sub.byClass.size) for (const c of classes(el)) if (sub.byClass.has(c)) return true;
  if (sub.byAttr.size) for (const name in el._attrs) if (sub.byAttr.has(name) || sub.byAttr.has(name.toLowerCase())) return true;
...
export function documentHasGeneratedContent () {
  ...
  let both = state.pseudoIdx;
  if (both === null) both = state.pseudoIdx = { before: buildPseudoIndex('before', state.layoutRules), after: buildPseudoIndex('after', state.layoutRules), any: undefined };
  if (both.any === undefined) both.any = both.before.has('content') || both.after.has('content');
  return both.any;
}
...
export function inlineDecls (el) {
  ...
  const put = (prop, value, important) => {
    ...
    out[prop] = { value: prop.startsWith('--') ? folded : serializeCssValue(folded, prop), important, order };
  };
...
// every side. The reftests spell the three cases out: `<hr size=50>` is a 50px BORDER box,
// `<hr size=1>` drops its bottom border so only the 1px top one is left, and `<hr size=50 noshade>`
// is a 25px border all round.
function hrHint(el, prop) {
  ...
  return prop === 'height' ? `${size}px` : null;
}
...
function presentationalHint (el, prop) {
  ...
  if (prop === 'vertical-align' || prop === 'float') return alignHint(el, prop);
...
export function cascadeLayoutEpoch () {
  ensureCascadeFresh();
  const cv = globalThis.__csimCascadeVersion ? globalThis.__csimCascadeVersion() : 0;
  // Dynamic state enters the epoch only where the scoped hook above cannot stand in for it: a shadow
  // sheet with a box-moving dynamic rule, and a selector we could not parse. Everything else — keyless
  ...
  //
  // The shadow half used to be "is there a host at all", and that single `||` was the ENTIRE residual
  // cost of a shadow host: with it, every layout memo on the page keys on the style-state generation,
  // which moves on any attribute write — so a 400-row table beside a widget with NO stylesheet at all
  // relaid out 1.36x slower, and an empty shadow tree cost exactly as much as a styled one. The trees'
  // own sheets are asked now (`sheetHasDynamicLayoutRule`).
  const ss = (shadowSheetFacts() && shadowDynLayout ||
...
// walks each chain once. The chain crosses shadow boundaries (ShadowRoot._parent is the host), and a
// tree whose sheet carries `:host-context` or `::part()` takes the ungated path — those two read
// across the boundary in a direction this cannot index, and `ctxGateReady` refuses the whole page for
// them; everything else a shadow sheet declares is folded in by `shadowSheetFacts`. Shadow-SIDE slot
...
// …and those that read attributes of the element ITSELF, which name it like an attribute selector does: a write of one
// flips the compound (`input:required + p`, `.c:has(:open)` stayed as they were in both the cascade and layout).
const ELEMENT_ATTR_PSEUDOS = { 'required': ['required'], 'optional': ['required'], 'any-link': ['href'], 'link': ['href'],
  'open': ['open'], 'popover-open': ['popover'], 'default': ['checked', 'selected'], 'dir': ['dir'],
  'placeholder-shown': ['placeholder'], 'in-range': ['min', 'max', 'type'], 'out-of-range': ['min', 'max', 'type'],
  'valid': ['required', 'pattern', 'min', 'max', 'minlength', 'maxlength', 'step', 'type'],
  'invalid': ['required', 'pattern', 'min', 'max', 'minlength', 'maxlength', 'step', 'type'] };
function ctxFeatures(selText, captured) {
  const out = { sub: [], anc: [], sibling: false, kids: [], kidsDeep: false, positional: null, positionalSibling: 0, positions: POS_FOLLOWING | POS_PRECEDING, inherited: [], unsafe: false, names: null, attrTests: null };
  ...
  out.positions = positionsOfGroups(groups);
  for (const g of groups) {
    const compounds = [[]];
    ...
    for (let ci = 0; ci <= last; ci++) {
      ...
      for (const tok of tokens) {
        ...
        if (leftOfSibling) out.kids.push(tok);
      }
      // …whether or not the compound names a token: `li + li p` has none left of its `+`, and a child-list change still
      // moves which `li` is preceded by one — under it, the `p`s.
      if (leftOfSibling && deep) out.kidsDeep = true;
      for (const t of compounds[ci]) {
        if (t.type !== 'pseudo') continue;
        const name = String(t.name).toLowerCase();
        if (POSITIONAL_PSEUDOS.has(name)) {
          if (typeof t.data === 'string' && /[.#\[]/.test(t.data)) {
            // `:nth-child(… of S)`: which siblings count is S's, so a write of one of its tokens re-keys the siblings.
            out.unsafe = true;
            for (const tok of identTokens(t.data)) out.kids.push(tok);
            if (!isSubject) out.kidsDeep = true;
          }
  ...
  for (const t of tokens) {
    if (t.type === 'attribute') {
      ...
      if (name === 'class' && t.action === 'element') out.push('.' + String(t.value).toLowerCase());
        ...
        // every inline style write on the page). A bare `[name]` matches any value. Compared
        // case-INSENSITIVELY unless the selector says `s`: css-what leaves the case unset for the
        // attributes HTML matches case-insensitively (`[method=post]` matches `POST`), and a
        // too-wide test only ever widens what a write reaches.
        (feat.attrTests || (feat.attrTests = [])).push(['[' + name, t.action, t.value == null ? '' : String(t.value), t.ignoreCase !== false]);
      }
    } else if (t.type === 'pseudo' && ELEMENT_ATTR_PSEUDOS[String(t.name).toLowerCase()]) {
      for (const a of ELEMENT_ATTR_PSEUDOS[String(t.name).toLowerCase()]) out.push('[' + a);
...
function newCtxIndex() {
  return { map: Object.create(null), childListDesc: false, positions: 0, emptySibling: 0, names: [] };
}
...
function noteCtxFeatures(index, cx) {
  ...
  if (cx.unsafe) {
    ...
    index.childListDesc = true; index.positions |= POS_FOLLOWING | POS_PRECEDING; index.emptySibling = 2;
  ...
  if (cx.positional === 'anc' || cx.kidsDeep) { index.childListDesc = true; index.positions |= cx.positions; }
...
class CtxGate {
  constructor(sheets) {
    this.indexes = [];
    this.childListDesc = false; this.positions = 0; this.emptySibling = 0;
    const names = new Set();
    for (const { sheet: sh } of sheets) {
      const ix = sh.cxIndex;
      ...
      if (ix.childListDesc) { this.childListDesc = true; this.positions |= ix.positions === undefined ? POS_FOLLOWING | POS_PRECEDING : ix.positions; }
...
// What the SHADOW sheets on this page declare — the union of their property names, of the properties
// their `@keyframes` blocks animate, their parsed sheets for the structural-context gate, and whether
// any of them uses a form that gate cannot model. Shadow sheets are in no document index, so without
...
let shadowHasKeyframes = false, shadowUnsafe = false, shadowSheetUnsafe = false, shadowDocPartVer = -1, shadowTreeFolds = 0;
let shadowDynLayout = false, shadowAnimNames = new Set(), shadowAnimVar = false;
// …whether ANY folded sheet reaches OUT of its tree with a class-sensitive selector. Page-wide is
// not a simplification: `shadowSheetSeen` dedupes folds on the sheet's TEXT, so a thousand copies of
// one widget fold ONE sheet and the later trees are never visited — a per-tree flag would be unset
// on all but the first, and making it per-tree means giving up the fold-once-per-text design the
// note above `shadowFactsPending` measures. So the blast radius of one `:host(.x)` anywhere is every
// HOST on the page (its own component subtree each, not the document), and since `:host(.x)` is
// common in component CSS, expect that branch to narrow nothing on a real component page. The
// light-DOM answer below it is the one that carries the win.
let shadowHostFn = false, shadowSlotted = false;
function shadowSheetFacts () {
  // A page with no shadow tree, now or ever, pays one truthy check — this runs on every write, through
  // `ctxGateReady`, and on `grid_table` that is 1.37M calls.
  ...
  while (shadowFactsPending.length) {
    ...
    shadowDocPartVer = -1;   // a tree arrived: the document's `::part()` scan has to run again
  ...
  // …`::part()` in a DOCUMENT rule is the mirror image of one in a shadow sheet: indexed against the
  // outer element, subject one tree in. AFTER the drain, so the tree that arms it is already counted —
  // before it, the call that folds the very first tree answered with the scan still skipped.
  if (shadowTreeFolds && shadowDocPartVer !== cascadeVersion) {
    shadowDocPartVer = cascadeVersion;
    if (state.layoutRules.some(hasPartSelector) || state.hideRules.some(hasPartSelector)) shadowUnsafe = true;
    // …and a DYNAMIC one that moves a box is the other half of the same mirror: `collectDynamicLayoutRules`
    // drops every rule whose subject is a pseudo-element (`subjectIsPseudoElement`), so a
    // `#host::part(p):hover { width: 300px }` is in neither the document's dynamic-subject list nor any
    // shadow sheet. Nothing would carry it — the scoped hook cannot sweep a subject one tree in, and the
    // epoch would no longer key on the state — so it is what keeps `shadowDynLayout` armed.
    if (!shadowDynLayout && (state.layoutRules.some(dynamicPartRule) || state.hideRules.some(dynamicPartRule))) {
      shadowDynLayout = true;
    }
  }
  return shadowProps;
}
function foldShadowSheet (sheet) {
  // …set by `addSheet`, the only producer of a `scoped.sheets` entry, in the same synchronous call
  // that pushes it — so an entry cannot reach here without one.
  const scope = sheet.shadowScope;
  if (scope.unsafe)  shadowUnsafe  = shadowSheetUnsafe = true;
  if (scope.hostFn)  shadowHostFn  = true;
  if (scope.slotted) shadowSlotted = true;
  if (!shadowDynLayout && sheetHasDynamicLayoutRule(sheet)) shadowDynLayout = true;
...
// Whether this sheet has a rule that can MOVE A BOX when dynamic style state changes — the question
// `collectDynamicLayoutRules` asks of the document's sheets, asked of one shadow sheet and memoised
// on it. A shadow sheet's own var graph is not in `layoutReachableCustomProps`, so a custom property
// counts here where the document side would ask whether it reaches layout: the conservative answer is
// the only sound one, and it costs a component sheet that declares `--x` on `:hover` nothing it was
// not already paying.
function sheetHasDynamicLayoutRule (sheet) {
  if (sheet.shadowDynLayout !== undefined) return sheet.shadowDynLayout;
  let dyn = false;
  for (const r of sheet.hide) {
    if (ruleIsDynamic(r) && (r.display != null || r.visibility != null)) { dyn = true; break; }
  }
  if (!dyn) {
    for (const r of sheet.layout) {
      if (!ruleIsDynamic(r)) continue;
      for (const prop in r.captured) {
        if (prop.startsWith('--') || !PAINT_ONLY_PROPS.has(prop)) { dyn = true; break; }
      }
      if (dyn) break;
    }
  }
  return (sheet.shadowDynLayout = dyn);
}
...
// …memoised on the rule, the way `ruleLooksDown` is: without it this regex ran over EVERY document
// rule on every drain, and a shadow `<style>` insertion bumps the cascade version, which re-queues
// every existing tree — 300 components came to 45,750 full scans / 9.15M tests, triangular in the
// component count and linear in the sheet's size on top.
function hasPartSelector (r) {
  if (r.__hasPart === undefined) r.__hasPart = PART_RE.test(r.selectorText);
  return r.__hasPart;
}
function dynamicPartRule (r) {
  return hasPartSelector(r) && ruleIsDynamic(r) &&
         (r.display != null || r.visibility != null || (r.captured != null && declaresBoxMovingProp(r.captured)));
}
export function resetShadowSheetFacts () {
  ...
  shadowHasKeyframes = false; shadowUnsafe = false; shadowSheetUnsafe = false; shadowDocPartVer = -1; shadowTreeFolds = 0;
  shadowDynLayout = false; shadowAnimNames = new Set(); shadowAnimVar = false;
  shadowHostFn = false; shadowSlotted = false;
  kfIndexInline = false;
}

// The gate, or null when it cannot answer: no cascade yet / a pending rebuild (the next read
// builds it; a write in between must stay conservative), or a shadow tree carrying a selector it
// cannot index (`:host-context`, `::part()`).
function ctxGateReady() {
  ...
  shadowSheetFacts();
  if (shadowUnsafe) return null;
  let gate = state.ctxGate;
  // …and the shadow sheets move on their own schedule, so the gate is rebuilt when either side did.
  // `null` is not a stale build — it is "cannot answer", set at reset and kept until a real rebuild.
  if (gate !== undefined && gate !== null &&
      (state.ctxGateVer !== cascadeVersion || state.ctxGateSheets !== shadowSheets.length)) gate = undefined;
  if (gate === undefined) {
    state.ctxGateVer = cascadeVersion;
    state.ctxGateSheets = shadowSheets.length;
    gate = new CtxGate(shadowSheets.length ? state.sheets.concat(shadowSheets) : state.sheets);
    state.ctxGate = gate.unsafe ? null : gate;
    state.ctxChildListDesc = gate.unsafe || gate.childListDesc;
    state.ctxChildListPositions = gate.unsafe ? POS_FOLLOWING | POS_PRECEDING : gate.positions;
...
// A child-list change on `el`: its own `:empty`, its children's positions, and — only when some rule reads a position
// in a non-subject compound or through a deep sibling combinator — the subtrees of the children whose position (or
// predecessor) moved: the ones the change point reaches (`forPositionedChildren`), not all of `el`'s descendants.
// Appending a row under `tr + tr td` re-keys nothing that exists; it once re-keyed the whole table per row. `before` /
// `after` are the nodes either side of the change (undefined where the caller cannot say: every child). Its siblings
// when `:empty` sits left of a sibling combinator somewhere.
export function ctxChildListEffect(el, added, removed, before, after) {
  bumpTreeGen();   // …an insertion or removal: every enclosing-root memo is stale (see above)
  el._selEpoch  = (el._selEpoch  || 0) + 1;
  el._kidsEpoch = (el._kidsEpoch || 0) + 1;
  ...
  if (gate === null || (el._tag === 'fieldset' && gate.get('[disabled') !== undefined)) {
    el._descEpoch = (el._descEpoch || 0) + 1;
  } else if (state.ctxChildListDesc) {
    const positions = state.ctxChildListPositions === null ? POS_FOLLOWING | POS_PRECEDING : state.ctxChildListPositions;
    forPositionedChildren(el, positions, added || NO_NODES, removed || NO_NODES, before, after, (c) => {
      c._selEpoch = (c._selEpoch || 0) + 1;
      c._descEpoch = (c._descEpoch || 0) + 1;
    });
  } else {
...
// …and how many distinct shadow sheets have been folded in, which is the bound the dedupe has to hold:
// `parseSheetCached`'s LRU evicts past 256 texts, so keying this on the parsed OBJECT re-listed a sheet
// on every re-parse and rebuilt `CtxGate` over the pile (see `shadowSheetFacts`).
globalThis.__csimShadowSheetCount = () => shadowSheets.length;
...
function allRecord (el, prop) {
  ...
  const key = `${cascadeStyleEpoch()}:${currentDirtySeq()}`;
  if (el._allRecKey !== key) {
...
function preferRecord (own_, alt) {
  ...
  return winsProp(own_, alt.spec, alt.source, alt.important, alt.layerRank) ? alt : own_;
}
...
//
// Keyed on `treeGen`, which moves on every structural mutation (`bumpTreeGen`) — the only way the
...
// `ctxEpochOf` carries: two realms count their own `treeGen` from 1, so a parent-realm stamp must not
...
function enclosingShadowRootOf(el) {
  if (!el) return null;
  if (el._encTok === TREE_REALM_TOKEN && el._encGen === treeGen) return null;
  for (let n = el._parent; n; n = n._parent) {
...
function scopedRulesFor(sr) {
  ...
  const out = { hide: [], layout: [], hostHide: [], hostLayout: [], slottedHide: [], slottedLayout: [],
                // …the PARSED sheets, which is where `shadowSheetFacts` reads what this tree declares.
                sheets: [],
                // …and this tree's own `@keyframes`, which are looked up in the TREE the animated
                // element lives in (css-scoping §3.3), not in the document.
                keyframes: new Map() };
  ...
  const addSheet = (cssText, base) => {
    ...
    // with 500 distinct sheets grew `shadowSheets` to 92,610 entries and rebuilt `CtxGate` over all of
    // them per write, which was slower than not narrowing the gates at all. The key does not evict.
    out.sheets.push({ sheet: parsed, key: __sheetCacheKey(cssText, vp) });
    // `:host-context(<sel>)` matches on an ANCESTOR OF THE HOST, and `::part()` written HERE styles an
    // element one tree further in — neither is the element `ctxFeatures` indexed the rule against, so
    // either keeps the structural-context gate conservative. (`::slotted(.x)` is the light child's own
    // `.x` and `:host(.x)` the host's own, so those two ARE indexed on the element they style.)
    // Recorded on the SHEET, which is shared across every tree using it.
    // Every way this sheet reaches OUT of the tree it is scoped to, read off the TEXT and memoised
    // on the parsed object `parseSheetCached` shares across every component using it.
    //   `unsafe` — indexed against one element, styling another: no per-element question is sound;
    //   `hostFn` / `slotted` — indexed against the outer-scope element they style, so a per-element
    //     question IS sound, and `shadowRulesMayReach` asks it mid-write.
    // Off the text rather than the routed buckets below for two reasons: reading a bucket can mean
    // rebuilding a whole tree's scoped rules, which is not a thing to do on a write; and `:host(`
    // has to cover `:host(.x) .y`, which `place` leaves in the IN-TREE bucket — a class on the host
    // decides both, whatever the routing does with the rule.
    // ONE object behind ONE sentinel, deliberately: two of these three are soundness-critical, and
    // hanging them off `shadowUnsafe`'s own `undefined` would leave them quietly unset the day
    // anything learns to set that field first — every host on the page taking the narrow answer,
    // with nothing to fail.
    if (parsed.shadowScope === undefined) {
      parsed.shadowScope = {
        unsafe:  HOST_CONTEXT_RE.test(cssText) || PART_RE.test(cssText),
        hostFn:  HOST_FN_RE.test(cssText),
        slotted: SLOTTED_RE.test(cssText)
      };
    }
    const place = (rule) => {
    ...
    for (const kf of (parsed.keyframes || [])) out.keyframes.set(kf.name, kf.blocks);
    serial += parsed.count;
  };
...
// Whether a class written on `el` can change what any shadow-author rule matches — so that a gate
// keyed on the host COUNT can be asked per element instead. An element this answers NO for is one
// the DOCUMENT's own rule indexes describe completely.
//
// Deliberately NOT "which rules style `el`" (`shadowRulesForEl`, just above, answers that): a rule
// can be indexed against one element and decided by ANOTHER element's class. There are four ways a
// shadow sheet crosses its boundary and the split is between those two questions, not between the
// routed buckets:
//   - in-tree rules — `el` lives in the tree, and the tree's sheets are in no document index;
//   - `:host(.x)`   — decided by the HOST's class, in every form, including the `:host(.x) .y` that
//                     `scopedRulesFor` leaves in the in-tree bucket (which is why `shadowHostFn`
//                     comes off the sheet text and not off `hostLayout`; the combinator form does
//                     not match at all today, and this must not depend on that staying true);
//   - `::slotted(.x)` — decided by a LIGHT child's class;
//   - `:host-context(.x)` / `::part()` — decided by an element the rule is not indexed against at
//                     all, which no per-element question can answer: `shadowUnsafe` latches those
//                     for the page, the same latch `ctxGateReady` reads.
function shadowRulesMayReach(el) {
  // A page with no shadow tree, now or ever, pays one truthy check; this runs on every class write.
  if (!globalThis.__csimShadowHostCount && !shadowTreeFolds) return false;
  shadowSheetFacts();                                   // …drain first: folding a tree is what arms the flags
  if (shadowUnsafe || !el) return true;
  if (enclosingShadowRootOf(el)) return true;           // in-tree: its own tree's sheets style it, and are in no index
  if (shadowHostFn && el._shadowRoot) return true;      // `:host(.x)` — its own tree decides on its class
  return !!(shadowSlotted && el._parent && el._parent._shadowRoot);   // `::slotted(.x)` — its host's tree does
}
...
// …and the other three cross-scope forms, read off a shadow sheet's TEXT (`scopedRulesFor`).
const HOST_CONTEXT_RE = /:host-context\(/i, HOST_FN_RE = /:host\(/i, SLOTTED_RE = /::slotted\(/i;
...
// Whether anything in this document has ever declared a `transition-*` property — a rule, or an
// inline style / CSSOM write, which mutation-observer.js `noteDocumentWideAttribute` latches at the
// WRITE: a latch set when the declaration map is parsed stays unset for an element nobody has
// read, and the model is asked before that read.
export function documentMayTransition () {
  if (globalThis.__csimInlineTransition) return true;
  ensureCascadeFresh();
  // …and a shadow tree's own sheets are asked the same question (`shadowSheetFacts`) rather than
  // answering it for the whole document, which is what a host used to do. Without either, a
  // transition declared only inside a component never ran at all.
  const shadow = shadowSheetFacts();
  for (const prop of TRANSITION_PROPS) if (rulesIndexHas(prop) || shadow.has(prop)) return true;
  return false;
}

// The blocks of the `@keyframes` rule with this name for THIS element, or `null` when nothing
// declares one — the one door onto the document's keyframes, so the animation model never walks
// sheets itself. An element inside a shadow tree looks in its OWN tree first (css-scoping §3.3: an
// animation name resolves in the tree scope of the element it applies to), which is what lets a
// component ship its keyframes alongside its rules; the document's are the outer scope it falls
// back to, exactly as a `:host`-styled element still sees document rules.
export function keyframesNamed (name, el) {
  ensureCascadeFresh();
  if (el && globalThis.__csimShadowHostCount) {
    const sr = enclosingShadowRootOf(el);
    if (sr) {
      const own = scopedRulesFor(sr).keyframes.get(name);
      if (own) return own;
    }
  }
  const kf = state.keyframes;
  return (kf && kf.get(name)) || null;
}
// Whether ANY `@keyframes` is declared at all: the O(1) gate that keeps every element on an
// animation-free page out of the animation model entirely (rule 3). A shadow tree's own keyframes
// count — they are folded in by `shadowSheetFacts`, where a page with a host used to answer yes for
// the whole document without looking.
export function documentHasKeyframes () {
  ensureCascadeFresh();
  shadowSheetFacts();
  return !!(state.keyframes && state.keyframes.size) || shadowHasKeyframes;
}
...
// disagreeing again.
// The script half of the same question, registered rather than imported: web-animations.js imports
// this module, so an import the other way would be a cycle. Same shape as `onGetAnimationsSync`.
// Two shapes so neither caller has to allocate: one property, or a whole family.
let SCRIPT_ANIM_DECLARES = null;
let SCRIPT_ANIM_DECLARES_ANY = null;
export function onScriptAnimationProperties (one, any) {
  SCRIPT_ANIM_DECLARES = one;
  SCRIPT_ANIM_DECLARES_ANY = any;
}
...
// The CSS half is answered for the DOCUMENT and the script half for the ELEMENT, and the difference
// is measured: a `@keyframes` block is named by a rule that some set of elements matches, and
// deciding which without reading their style is the work the gate exists to avoid — but an
// `element.animate()` effect names its target outright, so asking per element there is free and
// keeps one animated element from opening the gate for the whole page (measured at +35% wall on a
// 1600-element page with 400 live animations, growing superlinearly).
export function animationsDeclareProperty (el, prop) {
  return keyframesDeclareProperty(prop) ||
         (SCRIPT_ANIM_DECLARES !== null && SCRIPT_ANIM_DECLARES(el, prop));
}
...
// eight index fetches and eight script calls where one of each does.
export function animationsDeclareAnyOf (el, props) {
  ...
  return SCRIPT_ANIM_DECLARES_ANY !== null && SCRIPT_ANIM_DECLARES_ANY(el, props);
}
...
const TRANSITION_PROPS = ['transition-property', 'transition-duration', 'transition-delay',
...
// Every property name the cascade could give THIS element — the union of what its matching rules
// capture and what its own style attribute declares. What it answers is "which properties could
// have changed here", which is what a transition has to be looked for in: reading them all is a
// style recalc of one element, and reading none is how a transition goes unnoticed.
export function declaredPropertyNamesFor (el) {
  ensureCascadeFresh();
  // The DOCUMENT's set, not this element's matching rules'. Asking which rules match costs a walk
  // over every rule per element per frame, and this is asked for every mutated element of a
  // `transition: all` page — the shape Tailwind and Bootstrap ship (measured: +39% of a rendering
  // update). The document's declared properties are bounded by the sheet's vocabulary (tens),
  // each read behind the declared-value memo, and the answer is the same set of CHANGES: a
  // property no rule gives this element reads the same before and after, so it transitions
  // nothing.
  const out = new globalThis.Set(layoutPropIndex().keys());
  for (const prop of Object.keys(inlineDecls(el))) out.add(prop);
  return out;
}

// The SELECTORS of every rule that declares `prop`, plus the elements whose own style attribute
// does. What this answers is "which elements could the cascade give this property to" — asked by
// the animation model once per cascade version, so an `@keyframes` animation on an element nothing
// has read still starts, ends and fires its events (a browser's style recalc finds it; this is the
// same question asked of the rule index instead of of every element).
export function elementsDeclaring (prop, doc) {
  ensureCascadeFresh();
  doc = doc || globalThis.document;
  if (!doc || !doc.documentElement) return [];
  const out = new globalThis.Set();
  for (const rule of state.layoutRules) {
    if (!rule.captured || rule.captured[prop] === undefined) continue;
    let found;
    try { found = doc.querySelectorAll(rule.selectorText); } catch (_) { continue; }
    for (const el of found) out.add(el);
  }
  // …and the inline half, which no rule index can see — where the property may be spelled as a
  // SHORTHAND (`style="animation: spin 1s"` names `animation-name` without containing it).
  let inline;
  try { inline = doc.querySelectorAll('[style]'); } catch (_) { inline = []; }
  for (const el of inline) {
    const decls = inlineDecls(el);
    if (prop in decls) { out.add(el); continue; }
    for (const name of Object.keys(decls)) {
      const subs = shorthandLonghands(name);
      if (subs && subs.indexOf(prop) !== -1) { out.add(el); break; }
    }
  }
  return [...out];
}
...
const UA_WS_TAGS = new Set(['pre', 'textarea', 'listing', 'xmp', 'plaintext']);
...
export function elementPreservesWhitespace(node) {
  return WS_PRESERVING_VALUES.has(inheritedWhiteSpace(node));
}
...
function generatesTextBox(el) {
  return !INVISIBLE_TAGS.has(el._tag) && !hasFallbackOnlyContent(el) &&
         !uaNotRendered(el) && !selfHidden(el, true);
}
