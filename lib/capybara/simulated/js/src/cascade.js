...
function slotTextState(slot, shadowRoot) {
  if (!slotRenders) slotRenders = new Map();
  let v = slotRenders.get(slot);
  if (v === undefined) {
    v = !isVisibleNodeImpl(slot, true, false, shadowRoot) ? 0 : visibilityHidden(slot) ? 2 : 1;
...
function collectText(node, transform, preserveWs, hidden = false) {
  ...
  for (const c of node._children) {
    ...
    if (host) {
      ...
      const state = slot ? slotTextState(slot, host) : 0;
      if (!state) continue;
      inherited = state === 2;
    }
