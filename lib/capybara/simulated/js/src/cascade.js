...
export function inlineDecls (el) {
  ...
  for (const { prop, value } of parseStyleDeclList(String(s))) {
    ...
    if (prop === 'flex')            for (const e of expandFlexShorthand(d.value)) put(e.prop, e.value, d.important);
    else if (prop === 'inset')      for (const e of expandBoxShorthand('inset', d.value)) put(e.prop, e.value, d.important);
    else if (prop === 'background') for (const e of expandBackgroundShorthand(d.value)) put(e.prop, e.value, d.important);
