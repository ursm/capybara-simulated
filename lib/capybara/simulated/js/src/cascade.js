...
import { splitTopLevel, stripCssComments, isValidDeclarationValue, decodeDataUrlCss, resolveCssUrls, documentBaseUrl, parseStyleDeclList, cssPropertyName, isSupportedCssPropertyName, serializeCssValue, htmlDimensionValue, htmlPixelLength, htmlInteger } from './css-utils.js';
...
function expandDeclaration (prop, value, important, emit) {
  ...
  // …and so is a value the property's grammar rejects. The INLINE surfaces have always run this
  // check; the cascade did not, so a stylesheet's `align-items: -1px` not only survived, it WON
  // over the valid declaration beneath it — the same failure the math guard above exists to
  // prevent. All three surfaces have to agree, or a page gets a different answer depending on which
  // way it wrote the declaration.
  if (!isValidDeclarationValue(prop, value)) return;
...
// `lengthPx` in layout.js is a THIRD and is still one: it knows neither a percentage nor a math function, and
// `parseTrack` reads a grid template through it, so a `calc()` track comes back null and invalidates the
// whole template — `grid-template-columns: calc(25% + 10px) 1fr` is ONE column in both engines where Chrome
// makes two. Its own increment.
