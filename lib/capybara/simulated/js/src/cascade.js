...
function cascadeCacheKey(doc, vp) {
  ...
  for (const s of root.querySelectorAll('style')) {
    ...
    const t = scriptText(s); if (t) acc += '\nS:' + fnv1a(t) + ':' + t.length;
  }
  for (const l of root.querySelectorAll('link')) {
    const rel = (l._attrs.rel || '').toLowerCase(); if (!rel.split(/\s+/).includes('stylesheet')) continue;
...
function cssTreeFlatten(cssText, vp) {
  ...
  const visit = (children, src, parentSel, layer) => {
    ...
    children.forEach(node => {
      if (node.type === 'Rule') {
        ...
        else                           { return; }  // keyframes/font-face/import/… skipped
  ...
  catch (_) { return { rules: out, layers }; }
...
function parseSheet(cssText, vp) {
  ...
  try { flat = cssTreeFlatten(cssText, vp); } catch (_) { return { hide, layout, count: 0, layers: [] }; }
...
function collectCascadeRules(doc) {
  ...
  for (const l of doc.documentElement.querySelectorAll('link')) {
    ...
    if (media && !mediaMatches(media, vp)) continue;
    try {
      // Cross-visit cached (same as classic <script src>): fingerprinted CSS is
      // content-stable at content-hashed URLs, so a fresh VM per visit shouldn't
      // re-fetch it. `__csimExternalAsset` serves from the survives-visit cache,
      // returning null on 4xx / failure.
...
// Captured layout rules (color / geometry / custom props) from a shadow root's
// own stylesheets — its `<style>` descendants plus `adoptedStyleSheets`. Cached
// on the root keyed on the global cascade version (rebuilt on page load / a
// document sheet mutation); a shadow tree restyled with no document change is a
