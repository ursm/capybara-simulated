...
export function resetCascadeState() {
  ...
  sawTransitionDecl = false;   // …and the latch it set, or one page arms every later one
  sawInlineAnimationName = false;
  sawInlineLineProp.clear();
...
export function inlineDecls (el) {
  ...
  const put = (prop, value, important) => {
    ...
    // One sighting of a `transition-*` declaration anywhere — a rule OR an inline block like this
    // one — arms the transition model for the whole document (`documentMayTransition`). It has to
    // be a document-wide latch rather than a per-element question: a transition is started by a
    // CHANGE, so the value a property had BEFORE the page declared any transition is the one the
    // first transition runs from, and by then it is too late to start remembering.
    if (prop.charCodeAt(0) === 116 /* t */ && prop.startsWith('transition')) sawTransitionDecl = true;
    // …and the same latch for an inline `animation` / `animation-name`, for a different reason:
    // `referencedAnimationNames` reads the SHEETS to decide which `@keyframes` blocks are live, and
    // a name an element brings with it in its style attribute is invisible there. One sighting and
    // every block counts again, as it did before the filter.
    if (prop.charCodeAt(0) === 97 /* a */ && prop.startsWith('animation')) sawInlineAnimationName = true;
    // …and the INHERITED line properties layout asks the document about before reading any element:
    // `text-align` and `text-indent` cascade down from wherever they are written, so an inline
    // declaration on a `<body>` is invisible to every block's own inline map. One sighting anywhere
    // and every block reads its computed value.
    if (LATCHED_LINE_PROPS.has(prop)) sawInlineLineProp.add(prop);
    const folded = value;
...
// Whether anything in this document has ever declared a `transition-*` property — a rule, an
// inline style, a CSSOM write. Latched rather than recomputed: see `inlineDecls`.
let sawTransitionDecl = false;
export function documentMayTransition () {
  if (sawTransitionDecl) return true;
  ensureCascadeFresh();
...
let sawInlineAnimationName = false;
const LATCHED_LINE_PROPS = new Set(['text-align', 'text-indent', 'direction', 'writing-mode']);
const sawInlineLineProp = new Set();
// Does ANYTHING in the document declare this inherited line property — a rule, an inline block
// anywhere (latched above), an animation? The O(1) question a layout pass asks per BLOCK before it
// pays a computed-value read for a property that inherits: on a page that never mentions
// `text-align`, every line starts at the content edge and nothing is read.
// Does ANYTHING turn the flow — a `direction` / `writing-mode` rule or inline declaration, a `dir`
// attribute or a `<bdi>` anywhere (the dom-nodes.js latch)? `flowSides` starts here: on the
...
export function documentTurnsFlow () {
  return !!globalThis.__csimFlowHint || sawInlineLineProp.has('direction') || sawInlineLineProp.has('writing-mode') ||
         cascadeDeclaresProperty('writing-mode') || cascadeDeclaresProperty('direction');
}
export function documentDeclaresLineProp (prop) {
  return sawInlineLineProp.has(prop) || cascadeDeclaresProperty(prop) || keyframesDeclareProperty(prop) ||
         (prop === 'text-align' && !!globalThis.__csimLineAlignHint);   // UA tags / `align` (dom-nodes.js)
}
...
function referencedAnimationNames () {
  if (sawInlineAnimationName || globalThis.__csimShadowHostCount) return null;
...
export function ownWhiteSpace(el) {
  ...
  let raw = (cascadeDeclaresProperty('white-space') || 'white-space' in inlineDecls(el) ||
             animationsDeclareProperty(el, 'white-space'))
    ? declaredValue(el, 'white-space') : null;
