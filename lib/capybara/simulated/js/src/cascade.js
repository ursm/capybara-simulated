...
import { isRegularShorthand, shorthandExpand, isCssWideKeyword, hasSubstitution, FONT_SHORTHAND } from './shorthands.js';
...
function expandDeclaration (prop, value, important, emit) {
  ...
  const hand = prop === 'border'     ? expandBorderShorthand(value)
             ...
             : null;
  if (hand) { expanded = true; for (const d of hand) emit(d.prop, d.value, important); }
  ...
  // `transition: opacity 1s,` where a browser leaves `0s`. A value carrying a SUBSTITUTION is the
  // one exception: it is a real declaration that simply can't be decomposed until it resolves, so
  // it is kept whole and re-expanded per element at read time.
  if (!expanded && isRegularShorthand(prop) && !hasSubstitution(value)) return;
