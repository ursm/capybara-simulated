...
// (`__csimTakePendingShadowRoots`) and again whenever `scopedRulesFor` rebuilds its buckets, which is
...
function shadowSheetFacts () {
  ...
  const queued = globalThis.__csimTakePendingShadowRoots ? globalThis.__csimTakePendingShadowRoots() : null;
  if (queued && queued.length) for (const sr of queued) shadowFactsPending.push(sr);
...
export function resetShadowSheetFacts () {
  ...
  if (globalThis.__csimTakePendingShadowRoots) globalThis.__csimTakePendingShadowRoots();
