...
const CAPTURED_PROPS = new Set(['top', 'left', 'width', 'height', 'min-width', 'min-height', 'max-width', 'max-height', 'margin-top', 'margin-right', 'margin-bottom', 'margin-left', 'padding-top', 'padding-right', 'padding-bottom', 'padding-left', 'color', 'background-color', 'position', 'z-index', 'border-width', 'border-style', 'border-color', 'outline-color', 'caret-color', 'font-size', 'font-weight', 'font-style', 'line-height', 'font-family', 'opacity', 'text-align', 'cursor', 'pointer-events', 'flex-direction', 'flex-wrap', 'justify-content', 'align-items', 'align-content', 'align-self', 'order']);
...
// The `background-color` component of a `background` shorthand. We model only the color for
// getComputedStyle (not image/position/repeat/etc.), and per spec only the FINAL layer may
// carry one — so take the last color token of the last comma-separated layer. A CSS-wide
// keyword (`inherit` / `initial` / …) applies to every longhand, color included. null → the
// shorthand sets no color (its initial `transparent`, left to the browser default).
function backgroundColorFromShorthand(value) {
  const v = value.trim();
  if (/^(inherit|initial|unset|revert|revert-layer)$/i.test(v)) return v;
  const layers = splitTopLevel(v, ',');
  let color = null;
  for (const tok of splitTopLevel(layers[layers.length - 1] || '', ' ')) {
    const t = tok.trim();
    if (t && isCssColorToken(t)) color = t;
  }
  return color;
}
...
function ruleDecls(block) {
  ...
  block.children.forEach(node => {
    ...
    // The `background` shorthand contributes its color to the captured `background-color`
    // longhand (the only background component getComputedStyle reports). Other components
    // aren't modelled. A colorless shorthand adds nothing (keeps any prior background-color).
    if (prop === 'background') {
      const bg = backgroundColorFromShorthand(value);
      if (bg != null) decls.push({ prop: 'background-color', value: bg, important: !!node.important });
      return;
    }
...
function scopedRulesFor(sr) {
  ...
  if (adopted) for (const sheet of adopted) if (adoptedSheetActive(sheet, vp)) addSheet(sheetCssText(sheet));
  sr._scopedRules = out;
