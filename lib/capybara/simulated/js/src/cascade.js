...
// FOCUS is derived, not signalled. `document._activeElement` is assigned from a dozen places
...
// counts as a change.
let lastActive = undefined, lastFocusVisible = undefined;
function styleStateGeneration () {
  const doc = globalThis.document;
  const active = doc ? doc._activeElement : null;
  const fv = globalThis.__csimFocusVisible;
  if (active !== lastActive || fv !== lastFocusVisible) {
    lastActive = active; lastFocusVisible = fv;
    bumpStyleState();
  }
