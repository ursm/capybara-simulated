...
import { splitTopLevel, decodeDataUrlCss, resolveCssUrls, documentBaseUrl, parseStyleDeclList, isSupportedCssPropertyName } from './css-utils.js';
import { isRegularShorthand, shorthandExpand, isCssWideKeyword, FONT_SHORTHAND } from './shorthands.js';
...
function expandTextDecorationShorthand(value) {
  ...
  const out = [{ prop: 'text-decoration-line', value: lines.length ? lines.join(' ') : 'none' }];
  if (style)     out.push({ prop: 'text-decoration-style', value: style });
  if (color)     out.push({ prop: 'text-decoration-color', value: color });
  if (thickness) out.push({ prop: 'text-decoration-thickness', value: thickness });
  return out;
}
...
function expandDeclaration (prop, value, important, emit) {
  if (isRegularShorthand(prop)) {
    ...
    if (pairs) for (const [lh, v] of pairs) { const sub = splitImportant(v); emit(lh, sub.value, important || sub.important); }
  }
  const hand = prop === 'border'     ? expandBorderShorthand(value)
             ...
             : null;
  if (hand) for (const d of hand) emit(d.prop, d.value, important);
...
function ruleDecls(block) {
  ...
  block.children.forEach(node => {
    ...
    expandDeclaration(prop, value, !!node.important, (p2, v2, imp) => {
      decls.push({ prop: p2, value: lowercaseKeywordValue(p2, v2), important: imp });
    });
  });
  return decls;
}
...
export function inlineDecls (el) {
  ...
  for (const { prop, value } of parseStyleDeclList(String(s))) {
    ...
    if (isRegularShorthand(prop) && !shorthandExpand(prop, d.value) &&
        prop !== 'flex' && prop !== 'inset' && prop !== 'background' && prop !== 'border') continue;
...
export function resolveLayoutProp (el, prop, basis = null) {
  ...
  const raw = cascadedProperty(el, prop);
...
function cascadedRecord (el, prop) {
  ...
  return hint == null ? null : { value: hint, important: false, spec: [0, 0, 0], source: -1, hint: true };
}
...
const DEFAULT_FLOW_SIDES = Object.freeze({ 'block-start': 'top', 'block-end': 'bottom', 'inline-start': 'left', 'inline-end': 'right' });
...
function flowSides (el) {
  ...
  if (!declared) return (el._fsVal = base);
  const wmRaw  = ownCascaded(el, 'writing-mode');
  const dirRaw = ownCascaded(el, 'direction');
  if (wmRaw == null && dirRaw == null) return (el._fsVal = base);
  const wm  = wmRaw != null ? String(wmRaw).trim().toLowerCase() : reverseFlowMode(base);
  const rtl = dirRaw != null ? String(dirRaw).trim().toLowerCase() === 'rtl' : isRtlSides(base);
  return (el._fsVal = sidesFor(wm, rtl));
}
// The writing mode a set of sides came from, so an element that redeclares only `direction` keeps
// the inherited mode (and vice versa).
function reverseFlowMode (sides) {
  for (const wm of Object.keys(FLOW_SIDES)) if (FLOW_SIDES[wm][0] === sides['block-start']) return wm;
  return 'horizontal-tb';
}
function isRtlSides (sides) {
  const wm = reverseFlowMode(sides);
  return FLOW_SIDES[wm][2] !== sides['inline-start'];
}
function ownCascaded (el, prop) {
  const rec = cascadedRecord(el, prop);
  return rec && !isCssWideKeyword(rec.value) ? rec.value : null;
}
function cascadeGeneration () {
  ensureCascadeFresh();
  return `${state.ruleSerial}:${globalThis.__settleGenGet ? globalThis.__settleGenGet() : 0}`;
}
...
export function logicalCounterpart (el, physicalProp) {
  ...
  if (!m) return LOGICAL_SIZE_FOR[physicalProp] || null;
...
// The two non-side logical properties: a size along an axis.
const LOGICAL_SIZE_FOR = {
...
const HAS_FLOW_TWIN = new Set([
  'top', 'right', 'bottom', 'left',
  ...['width', 'height'].flatMap(d => [d, `min-${d}`, `max-${d}`]),
  ...['inline-size', 'block-size'].flatMap(d => [d, `min-${d}`, `max-${d}`]),
  ...['margin', 'padding'].flatMap(g => ['top', 'right', 'bottom', 'left'].map(sd => `${g}-${sd}`)),
  ...['margin', 'padding'].flatMap(g => ['block-start', 'block-end', 'inline-start', 'inline-end'].map(sd => `${g}-${sd}`)),
  ...['inset-block-start', 'inset-block-end', 'inset-inline-start', 'inset-inline-end'],
  ...['top', 'right', 'bottom', 'left', 'block-start', 'block-end', 'inline-start', 'inline-end']
    .flatMap(sd => ['width', 'style', 'color'].map(part => `border-${sd}-${part}`)),
]);
...
export function cascadedProperty (el, prop) {
  ...
  if (!HAS_FLOW_TWIN.has(prop)) return own_ ? own_.value : null;
  const other = LOGICAL_SIDE_RE.test(prop) ? physicalCounterpart(el, prop) : logicalCounterpart(el, prop);
