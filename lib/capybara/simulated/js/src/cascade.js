...
export function isVisibleNode(el) {
    ...
    for (const node of nodes) {
      if (node.type === 'at-rule') {
        ...
        if (node.name === 'container') {
          ...
          // specific container). This is how Discourse's directory-
          // table mobile-table fallback (`@container (max-width: 47em)`
          // → `display: none` on `&__header`) was incorrectly applying
          // unconditionally and hiding `.btn.bulk-select` even on the
          // 1024 px default viewport. Drop the rule if the prelude
          // names a feature we can evaluate; descend if not.
          const prelude = (node.prelude || '').replace(/^[^\s(]*\s*/, ''); // strip optional container name
...
export function matchesAnyHideRule(el) {
