...
// `<pre>` (UA stylesheet: white-space:pre) and any element with
// `white-space: pre*` preserves newlines in visible_text. Without
// this, the text collapse turns `<pre><code>` blocks into
// space-joined runs and Capybara's `have_css('pre code', text:
// "a\\nb\\nc")` never matches. TODO: only inline `style=` is read;
// a stylesheet `pre { white-space: pre }` won't be honoured until
// the cascade resolver can answer for `white-space` like it does
// for `text-transform`.
