...
const __CE_BLOCK_TAGS = new Set([
  'p', 'div', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
  'blockquote', 'pre', 'li', 'td', 'th', 'figcaption', 'aside',
...
globalThis.__csimSendKeys = function (h, atoms) {
  ...
  const pressKey = (info, modifiers) => {
    ...
    if (!blocked && (info.key === 'Enter' || info.key === ' ') &&
        (n._tag === 'button' || (n._tag === 'a' && n._attrs.href != null)) &&
...
globalThis.__csimSelectWordAt = function (h) {
  ...
  let textNode = null;
  let off = 0;
  if (r && r.startContainer && n.contains && n.contains(r.startContainer) &&
    ...
    const visit = (x) => {
      if (textNode) return;
      if (x.nodeType === NODE_TEXT && (x.data || '').length > 0) { textNode = x; return; }
      if (x._children) for (const c of x._children) { visit(c); if (textNode) return; }
    };
    visit(n);
    if (!textNode) return;
    off = 0;
