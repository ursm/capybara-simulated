...
globalThis.__csimToggleOptionByClick = function (option) {
  ...
  if (!sel) return false;
  // A disabled option (own `disabled`, a disabled `<optgroup>` ancestor) or a disabled
  // select can't be selected by a click — real browsers ignore it.
  if (option._attrs.disabled != null || isActuallyDisabled(sel)) return false;
  for (let p = option._parent; p && p !== sel; p = p._parent) {
    if (p._tag === 'optgroup' && p._attrs.disabled != null) return false;
  }
  const multi = sel._attrs.multiple != null;
  const was = option._selectedness === true;
  if (multi) {
    option._selectedness = !was;
  ...
  for (const f of inputs) {
    ...
    if (tag === 'input') {
      ...
      for (const o of f._listOfOptions()) {
        ...
        fields.push([name, o._attrs.value != null ? o._attrs.value : o.textContent]);
        if (!multi) break;
      }
    } else if (tag === 'button') {
