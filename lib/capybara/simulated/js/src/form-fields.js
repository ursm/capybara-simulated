...
import { Text, deleteRangeContents, recordFormSubmission } from './dom-nodes.js';
      ...
      if (isModCombo && lowerKey === 'v') {
        ...
        if (types.length > 0 || plain || html) {
          ...
          if (!ev.defaultPrevented && typeable) {
            ...
            // splice into `_attrs.value`; for contenteditable, route
            // through the same path as typing (PM's own paste handler
...
globalThis.__csimSetValue = function (h, value) {
  ...
  let kind = 'value';
  if (tag === 'textarea') {
    // HTML dirty-value flag: snapshot the parsed default once, before
    // the first mutation clobbers `_children` / `_attrs.value`, so
    // `<form>.reset()` can restore it (see Element#reset).
    if (n._defaultValue === undefined) n._defaultValue = n.textContent;
    n._children = newChildList(); n._children.push(Object.assign(new Text(v), { _parent: n }));
    n._attrs.value = v;
    ...
    if (type === 'checkbox' || type === 'radio') {
      // HTML dirty flags: snapshot defaults once before mutating so
      // `<form>.reset()` can restore them (see Element#reset). The
      // `else` branch below writes `_attrs.value` for non-boolean
      // values, so snapshot both checkedness and value here.
      if (n._defaultChecked === undefined) n._defaultChecked = (n._attrs.checked != null);
      if (n._defaultValue === undefined) n._defaultValue = (n._attrs.value || '');
      ...
      // HTML dirty-value flag: snapshot the parsed default once.
      if (n._defaultValue === undefined) n._defaultValue = (n._attrs.value || '');
  ...
  dispatchEvent(n, new InputEvent('input', { bubbles: true, cancelable: false }));
  dispatchEvent(n, new Event('change', { bubbles: true, cancelable: false }));
  if (tag === 'input' || tag === 'textarea' || isContenteditable(n)) {
...
function __fireSelectChange (sel) {
  try { dispatchEvent(sel, new InputEvent('input', { bubbles: true, cancelable: false })); } catch (_) {}
...
globalThis.__csimSelectOption = function (h) {
  ...
  if (!sel) { n._selectedness = true; n._dirtySel = true; return true; }
...
globalThis.__csimFormSerialize = function (formHandle, submitterHandle) {
  ...
  for (const f of inputs) {
    ...
    if (tag === 'input') {
      ...
      // HTML form-submission normalizes textarea LF to CRLF. The "first newline
      // removal" already happened at parse time (the DOM text node lacks the
      // leading line terminator), so read the child text content as-is — matching
      // `globalThis.__csimValue` — then normalize line endings.
