...
globalThis.__csimSetValue = function (h, value) {
  ...
  if (tag === 'textarea') {
    ...
    if (type === 'checkbox' || type === 'radio') {
      // HTML dirty value flag: snapshot the default value once before mutating
      // so `<form>.reset()` can restore it (see Element#reset). Checkedness goes
      // through setCheckedness, which sets the dirty checkedness flag itself.
