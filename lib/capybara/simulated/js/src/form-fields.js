...
import { Text, deleteRangeContents, recordFormSubmission, commitChangeKeepingFocus } from './dom-nodes.js';
...
globalThis.__csimSetValue = function (h, value) {
  ...
  // re-baseline so a subsequent blur's change-on-blur doesn't fire it a second time.
  if (n._changeBaseline !== undefined) n._changeBaseline = controlLiveValue(n);
  if (tag === 'input' || tag === 'textarea' || isContenteditable(n)) {
