...
function constructEntryList(form, submitter) {
  ...
  const selector = ceTags ? 'input,textarea,select,button,' + ceTags : 'input,textarea,select,button';
  ...
  for (const f of inputs) {
    ...
    // A form-associated custom element submits whatever `internals.setFormValue()`
    // last handed it, NOT a `value` content attribute — including the FormData case,
    // which supplies its own names and so has to run before the nameless-skip below.
    // The selector above only matches these once one is defined, so the ordinary
    // page never reaches this check.
    if (isFormAssociatedCustomElement(f)) {
      if (f._attrs.disabled == null) appendCustomElementEntries(fields, f, name);
      continue;
    }
