...
import { dispatchEvent, fireCheckableActivation } from './dispatch.js';
...
import { updateSelectedContent, formAssociatedTagSelector, isFormAssociatedCustomElement, optionSelectednessDisabled } from './custom-elements.js';
import {
  ancestorForm,
  contenteditableHost,
  formForControl,
  isContenteditable,
  isNodeActuallyDisabled,
  setRadio,
  radioGroupMembers,
  getCheckedness,
  setCheckedness,
...
import { Text, deleteRangeContents, recordFormSubmission, commitChangeKeepingFocus, isActuallyDisabled, asciiLower, showComboboxDatalist, inputSupportsList } from './dom-nodes.js';
...
// Both walkers are bounded at the editing host: the caret never leaves the editable root
// (ArrowLeft at the very start of an editor stays put in a browser). Unbounded, the parent
// climb escaped the host and collapsed the Selection onto whatever text precedes the editor
// in the page — a heading above the composer.
function editingHostOf(n) {
  return contenteditableHost(n && n.nodeType === NODE_ELEMENT ? n : n && n._parent);
}

function previousTextLeaf(start) {
  const host = editingHostOf(start);
  let n = start;
  while (n && n !== host) {
    const prev = n.previousSibling;
    if (prev) {
      const leaf = lastCaretLeaf(prev);
...
function nextTextLeaf(start) {
  ...
  while (n && n !== host) {
    ...
    if (next) {
      const leaf = firstCaretLeaf(next);
      if (leaf && leaf.nodeType === NODE_TEXT) return leaf;
      n = next;
    } else {
      n = n._parent;
...
function moveContenteditableCaretToBlockEdge(edge) {
  ...
  if (!block) return;
  // A block with no text leaf (an empty paragraph, a lone <br>) still takes the
  // caret: collapse into the block itself, as before the edge-searching rewrite.
  const leaf = (edge === 'start' ? firstCaretLeaf(block) : lastCaretLeaf(block)) || block;
...
// The last/first CARET STOP under `root`, searching siblings inward from the edge. A caret
// stops at a TEXT leaf or at a VOID element (an image, a `<br>`) — the atoms a browser places
// the caret beside; a decorated hashtag's leading emoji `<img>` is where Home lands, and
// hunting past it into the anchor's text put the caret INSIDE the atom (ProseMirror's
// backspace-joins-paragraphs then deleted into the decoration instead of joining). What the
// search skips is an EMPTY non-void element: ProseMirror parks empty cursor/widget `<span>`s
// at mark boundaries (`<code>code<span></span></code>`), and a plain deepest-chain walk landed
// on the empty span, read "no text here" and made the caller skip the whole element (a
// dblclick word-selection then stopped at the `<code>` mark instead of crossing it, as a
// browser's does).
const CARET_VOID_TAGS = new Set([
  'audio',
  'br',
  'canvas',
  'embed',
...
function caretLeafFrom(root, fromEnd) {
  if (!root) return null;
  if (root.nodeType === NODE_TEXT) return root;
  if (root.nodeType !== NODE_ELEMENT) return null;
  const kids = root._children || [];
  if (kids.length === 0) return CARET_VOID_TAGS.has(root._tag) ? root : null;
  for (let i = fromEnd ? kids.length - 1 : 0; fromEnd ? i >= 0 : i < kids.length; fromEnd ? i-- : i++) {
    const t = caretLeafFrom(kids[i], fromEnd);
    if (t) return t;
  }
  return null;
}

function lastCaretLeaf(root)  { return caretLeafFrom(root, true); }
function firstCaretLeaf(root) { return caretLeafFrom(root, false); }
...
globalThis.__csimSendKeys = function (h, atoms) {
  ...
  const pressKey = (info, modifiers) => {
    ...
    if (!blocked && (info.key === 'ArrowDown' || info.key === 'ArrowUp' || info.key === 'ArrowLeft' || info.key === 'ArrowRight') &&
      ...
      dispatchEvent(globalThis.document.activeElement || n, new KeyboardEvent('keyup', init));
      return;
    }
    ...
    if (!blocked && (info.key === 'ArrowUp' || info.key === 'ArrowDown') &&
      ...
      if (!bi.defaultPrevented) {
        ...
        try { n._stepBy(info.key === 'ArrowUp' ? 1 : -1); } catch (_) {}
        if (n.value !== before) {
          dispatchEvent(n, new InputEvent('input', { bubbles: true, cancelable: false, composed: false, data: null, inputType: '' }));
          dispatchEvent(n, new Event('change', { bubbles: true }));
        }
      }
      dispatchEvent(n, new KeyboardEvent('keyup', init));
      return;
      ...
      if (form) {
        ...
        if (!(defaultBtn && isActuallyDisabled(defaultBtn))) {
          ...
          if (!submit.defaultPrevented) {
            recordFormSubmission(form, submitter, undefined, false);   // implicit (Enter) submit = interactive
          }
        }
      }
    }
        ...
        if (types.length > 0 || plain || html) {
          ...
          if (!ev.defaultPrevented && typeable) {
            ...
            if (ceTypeable) {
              ...
              const s = n._selectionStart != null ? n._selectionStart : cur.length;
              const e = n._selectionEnd   != null ? n._selectionEnd   : s;
              setControlLiveValue(n, cur.slice(0, s) + text + cur.slice(e));
...
function extendWordAcrossLeaves(startLeaf, dir, wordChar, anchorBlock) {
  ...
  while (cur && cur.nodeType === NODE_TEXT && findBlockAncestor(cur) === anchorBlock) {
    const d = cur.data || '';
    if (!d.length) {
      // An empty text leaf is invisible, not a word boundary — ProseMirror's
      // cursor bookkeeping leaves them between inline marks (they don't show in
      // innerHTML), and breaking here clipped a dblclick word-selection at the
      // `<code>` boundary the moment PM had one parked there.
...
globalThis.__csimSetValue = function (h, value) {
  ...
  if (tag === 'input' && typeof value === 'string' && value.endsWith('\n')) {
    ...
    if (form) {
      ...
      recordFormSubmission(form, null, undefined, false);   // implicit submit = interactive
    }
  }
  return true;
};
...
globalThis.__csimSelectOption = function (h) {
  ...
  if (!sel) { setSelectedness(n, true); n._dirtySel = true; bumpSettleGen(); return true; }
  if (!__optionPickable(n, sel)) return false;
  const wasSelected = n._selectedness === true;
  selectOptionExclusive(sel, n);
  sel._userInteracted = true;            // a user pick → :user-valid / :user-invalid
...
globalThis.__csimToggleOptionByClick = function (option) {
  ...
  if (multi) {
    ...
    if (was) { sel._userInteracted = true; return false; }   // re-picking the current option: no change
...
globalThis.__csimArrowKeyDefault = function (node, key) {
  if (!node || node.nodeType !== NODE_ELEMENT) return;
...
// HTML "constructing the form data set" — submitting element directionality.
// A control with a non-empty `dirname` attribute appends an extra entry right
// after its own: `<dirname value> = <the control's directionality>` (ltr/rtl).
// Applies to textarea and to the input types that support dirname (text /
// search / tel / url / email / password / hidden / submit) — NOT number / date /
// range / color / checkbox / radio / file / image / reset / button.
const DIRNAME_INPUT_TYPES = new Set(['hidden', 'text', 'search', 'tel', 'url', 'email', 'password', 'submit']);
function appendDirnameEntry(fields, f, tag, type) {
  const dirname = f._attrs.dirname;
  if (!dirname) return;   // absent or empty string → no entry
  if (tag === 'textarea' || (tag === 'input' && DIRNAME_INPUT_TYPES.has(type))) {
    fields.push([dirname, typeof f._directionality === 'function' ? f._directionality() : 'ltr']);
  }
}

// HTML "textarea wrapping transformation" for `wrap=hard`: greedily break each
// line at the `cols` width (prefer the last space within the width; hard-break a
// too-long word). The spec leaves the exact algorithm UA-defined — any wrap that
// inserts breaks is conformant.
function hardWrapTextarea(text, cols) {
  const out = [];
  for (const line of String(text).split('\n')) {
    let s = line;
    while (s.length > cols) {
      let brk = s.lastIndexOf(' ', cols);
      const cut = brk > 0 ? brk : cols;            // no space in range → hard break at cols
      out.push(s.slice(0, cut));
      s = s.slice(brk > 0 ? cut + 1 : cut);        // drop the break space when wrapping at one
    }
    out.push(s);
  }
  return out.join('\n');
...
// Resolve a form's `action` / submitter `formaction` for SUBMISSION — HTML
// "encoding-parse a URL", where the document's character encoding percent-encodes
// the URL's query. So a `<form action="?x=ß">` in a windows-1252 document submits to
// `?x=%DF`, not the UTF-8 `?x=%C3%9F` the `action` IDL getter (always UTF-8) produces
// (action-encoding). Resolved against the form's base URL (honouring <base>); the
// encoding defaults to UTF-8, so a UTF-8 document is unchanged.
function resolveActionURL(form, rawAttr) {
  const doc  = form.ownerDocument || globalThis.document;
  const base = form.baseURI || (doc && typeof doc.URL === 'string' && doc.URL) ||
               (globalThis.location && globalThis.location.href) || undefined;
  // `document.characterSet` is the canonical document encoding (the same value the
  // anchor-href resolver passes); falsy → UTF-8 inside __csim_parseUrl.
  const u = globalThis.__csim_parseUrl(String(rawAttr), base, doc && doc.characterSet);
  // On a parse failure fall back to the RAW attribute (what the action / formAction
  // IDL getters did), so a malformed action still reaches the Ruby resolver verbatim
  // rather than collapsing to the current URL.
  return (u && !u.error && u.href) ? u.href : String(rawAttr);
}
...
function __csimControlState(c) {
  ...
  if (tag === 'select') {
    ...
    if (c.multiple) return { k: 'selm', vals: Array.from(c.selectedOptions).map((o) => o.value) };
...
globalThis.__csimCaptureFormState = function () {
  ...
  for (const c of doc.querySelectorAll('input, textarea, select')) out.push(__csimControlState(c));
  return out;
};
...
globalThis.__csimRestoreFormState = function (state) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (s.k === 'val') c.value = s.v;
      ...
      if (Array.from(c.options).some((o) => o.value === s.v)) c.value = s.v;
    }
    else if (s.k === 'selm') {
      const want = new Set(s.vals);
      for (const o of c.options) o.selected = want.has(o.value);
    }
  }
};
...
// so we carry them base64-encoded (via the in-VM btoa, which is byte-safe) for the
// classic submit to encode.
export function serializeEntryList(entryList) {
  ...
  for (const [name, value] of entryList) {
    if (File && value instanceof File) {
      const inMemory = !value._csimHost;
      out.push({
        name:     String(name),
        file:     true,
        ...
        b64:      inMemory ? globalThis.btoa(blobBytes(value) || '') : null
      });
    } else {
...
// HTML "construct the entry list", the ordered walk every submission path shares.
// Returns `[name, value]` pairs in TREE ORDER; a value is a string OR a live `File`
// (a file control's selection, or a form-associated custom element's submission
// value). `new FormData(form)` IS this list, so it consumes the pairs as-is;
// `__csimFormSerialize` flattens them for the Ruby boundary, which can't hold a File.
function constructEntryList(form, submitter) {
  ...
  // Form-associated custom elements are submittable too, so their tag names join the
  // built-in selector — one ordered pass keeps every control in tree order (the
  // fragment is empty until a form-associated element is defined, rule 3).
  //
  // `datalist` / `fieldset` ride along in the SAME pass, not to be collected but purely
  // to detect whether the document holds any: HTML "construct the entry list" step 5.1
  // skips a field with a `<datalist>` ancestor or a disabled one, and both are
  // per-control ancestor walks that these flags let the ordinary page (which has
  // neither) skip entirely.
  //
  // MEASURED, 2000x `new FormData` on a 300-control / 8-deep form with neither element
  // — i.e. the page that must pay nothing (rule 3: profile, don't assume; every
  // alternative below was an intuition that lost):
  //   0.126 ms/call  before this skip step existed
  //   0.151 ms/call  THIS (two extra alternatives in the one querySelectorAll)
  //   0.200 ms/call  one ancestor-walk-to-root per collected control, no flags
  //   0.215 ms/call  a separate querySelector per detector (two whole-document scans)
  // The +20% buys two real spec fixes and is ~25µs per submission on a worst-case form;
  // entry-list construction runs once per submit, not per find.
  const ceTags   = formAssociatedTagSelector();
  const selector = 'input,textarea,select,button,datalist,fieldset' + (ceTags ? ',' + ceTags : '');
  let anyDatalist = false, anyDisabledFieldset = false;
  for (const f of docRoot.querySelectorAll(selector)) {
    // Neither tag is submittable — they are here only as detectors.
    if (f._tag === 'datalist') { anyDatalist = true; continue; }
    if (f._tag === 'fieldset') { if (f._attrs.disabled != null) anyDisabledFieldset = true; continue; }
  ...
  const inDatalist = (el) => {
    if (!anyDatalist) return false;
    for (let p = el._parent; p; p = p._parent) if (p._tag === 'datalist') return true;
    return false;
  };
  // `isNodeActuallyDisabled` owns the whole rule — own `[disabled]`, a disabled
  // `<fieldset>` ancestor (except inside that fieldset's FIRST `<legend>`), and the
  // option/optgroup shapes — so the fieldset case can't drift from `:disabled` matching.
  const isDisabledField = (el) => el._attrs.disabled != null ||
                                  (anyDisabledFieldset && isNodeActuallyDisabled(el));
  for (const f of inputs) {
    const tag  = f._tag;
    const name = f._attrs.name;
    // Step 5.1 — a `<datalist>`'s controls are suggestions, never submitted values.
    if (inDatalist(f)) continue;
    // Step 5.1 — skip a disabled field, EXCEPT the submitter: its name/value gets
    // appended by a later step that does NOT re-check disabled, and Rails-UJS'
    // `data-disable-with` mutates disabled during the submit event (before the form
    // data is read), so checking here would drop the very button the user clicked.
    // The exemption applies only to `f === submitter`; other disabled controls
    // (including unselected submit buttons) still skip.
    if (isDisabledField(f) && f !== submitter) continue;
    // Step 5.3 — a form-associated custom element submits whatever
    // `internals.setFormValue()` last handed it, NOT a `value` content attribute —
    // including the FormData case, which supplies its own names and so has to run
    // before the nameless-skip below. The selector above only matches these once one
    // is defined, so the ordinary page never reaches this check.
    if (isFormAssociatedCustomElement(f)) {
      appendCustomElementEntries(fields, f, name);
      continue;
    }
    // Step 5.4 — a nameless control contributes nothing, EXCEPT an image-button
    // submitter, which always contributes bare `x` / `y` coordinate entries (HTML).
    // Only the nameless case pays the extra type check, keeping the common path cheap.
    ...
    if (tag === 'input') {
      ...
      // submit as UTF-8, so emit that). The name is matched ASCII case-insensitively
      // (`_CHARSET_` / `_ChArSeT_` count; the non-ASCII `_charſet_` does NOT).
      if (name != null && asciiLower(name) === '_charset_' && type === 'hidden' && controlLiveValue(f) === '') {
        fields.push([name, 'UTF-8']);
        appendDirnameEntry(fields, f, tag, type);   // dirname runs after the value branch, _charset_ included
        continue;
      }
      if (type === 'image') {
      ...
      if (type === 'submit' || type === 'reset' || type === 'button') {
        ...
        appendDirnameEntry(fields, f, tag, type);
        continue;
      }
      if (type === 'checkbox' || type === 'radio') {
      ...
      if (type === 'file') {
        // Each selected File goes in at the control's OWN tree position, as the live
        // object: its host-backed source (`_handle`/`_index` → the Ruby `@file_picks`
        // slot) rides along on it, so the multipart serialiser resolves bytes even for
        // a file moved onto a DIFFERENT input via JS (`input.files =
        // dataTransfer.files`), where the input's own handle was never the target of
        // an `attach_file`. An input with no selection still contributes one empty
      ...
      // The entry-list value is the textarea's API value (LF-normalized) — the
      // CR/LF→CRLF wire normalization belongs to the urlencoded / text-plain /
      // multipart encoders, NOT the entry list (so `new FormData(form).get()`
      // returns LF, matching browsers). `controlLiveValue` is `_value` after a
      // set/typing edit, else the child text content (the parse-time "first
      // newline removal" already ran) — matching `globalThis.__csimValue`.
      let raw = String(controlLiveValue(f));
      // `wrap=hard` (an ASCII case-insensitive enumerated attribute) applies the
      // textarea wrapping transformation FIRST: break each line at the `cols` width
      // (default 20) so the submitted value carries the UA-inserted line breaks (any
      // valid wrap is acceptable — wrap-enumerated). The breaks are LF, like the value.
      if (asciiLower(f._attrs.wrap || '') === 'hard') {
        const cols = parseInt(f._attrs.cols, 10);
        raw = hardWrapTextarea(raw, cols > 0 ? cols : 20);
      }
      fields.push([name, raw]);
      appendDirnameEntry(fields, f, tag, 'textarea');
    } else if (tag === 'select') {
      // HTML: append an entry for each option whose selectedness is true AND that is
      // NOT disabled — a selected-but-disabled option (its own `[disabled]`, or a
      // disabled `<optgroup>`) submits nothing. Disabled does not prevent SELECTEDNESS,
      // only submission (`select.value` / `selectedIndex` / `<selectedcontent>` all
      // still report it), so this can't be folded into the selectedness bookkeeping.
      // `optionSelectednessDisabled` is the same predicate the selectedness default-pick
      // uses — one picking an option the other drops would be incoherent. It is also
      // the exact spec notion here: a disabled `<select>` / `<fieldset>` does NOT make
      // its options disabled, and that case is already handled at the control level.
      //
      // There is deliberately NO submit-time fallback for "nothing selected". Which
      // option is selected is settled by HTML's "ask for a reset" when the select is
      // parsed and on every subsequent mutation, and we already implement it exactly —
      // verified against headless Chrome, `selectedIndex` agrees on all 13 cases:
      // auto-select the first NON-DISABLED option, but only when `size` is 1 and
      // `multiple` is absent, and never when every option is disabled; and it re-runs
      // for a select built by appendChild / innerHTML, or after the selected option is
      // removed. So a select reporting `selectedIndex === -1` genuinely contributes
      // nothing, and the old fallback here (first non-disabled option whenever nothing
      // was chosen) was a submit-time re-implementation that ignored `size` — it
      // submitted `<select size=4>`'s first option, which no browser does.
      // `o.value` is the spec's "option's value" (the `value` attribute, else the
      // collapsed `text` IDL) — reading textContent submitted the raw source text.
      const multi = f._attrs.multiple != null;
      for (const o of f._listOfOptions()) {
        if (o._selectedness !== true || optionSelectednessDisabled(o)) continue;
        fields.push([name, o.value]);
        if (!multi) break;
      }
    } else if (tag === 'button') {
...
globalThis.__csimConstructEntryList = function (formHandle, submitterHandle) {
  const form = resolveFormArg(formHandle);
  if (!form || form._tag !== 'form') return null;
  return constructEntryList(form, resolveFormArg(submitterHandle));
};
...
function formSubmissionSpec(form, submitter) {
  ...
  // document (not the top page) and honour its <base>.
  let resolvedAction;
  const doc = form.ownerDocument;
  const docAddr = () => (doc && typeof doc.URL === 'string' && doc.URL) ||
                        (globalThis.location && globalThis.location.href) || '';
  if (subAction != null && subAction !== '') {
    resolvedAction = resolveActionURL(form, subAction);
  } else if (subAction === '') {
    resolvedAction = docAddr();                       // empty formaction → form doc address
  } else {
    const a = form._attrs.action;
    resolvedAction = (a != null && a !== '') ? resolveActionURL(form, a) : docAddr();
  }
  return {
...
globalThis.__csimFormSerialize = function (formHandle, submitterHandle) {
  ...
  spec.entries = serializeEntryList(constructEntryList(form, submitter));
  return spec;
};
