...
globalThis.__csimSendKeys = function (h, atoms) {
  ...
  // Radio / checkbox inputs aren't typeable — space-key on them
  // triggers the activation behavior (synthetic click) instead, per
  // HTML §4.10.5.2.16. Treating them as typeable would insert a
  // literal ' ' into the radio's `value` attribute (which holds the
  // choice id) and never fire the change event Discourse-style
  // wizards listen for.
