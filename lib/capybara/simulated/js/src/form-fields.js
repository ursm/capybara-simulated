...
function __csimControlState(c) {
  ...
  if (tag === 'select') {
    return { k: 'sel', i: c.selectedIndex, m: c.multiple ? Array.from(c.options).map((o) => o.selected) : null };
  }
...
globalThis.__csimRestoreFormState = function (state) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (s.k === 'val') c.value = s.v;
      ...
      if (s.m) Array.from(c.options).forEach((o, k) => { o.selected = !!s.m[k]; });
      else c.selectedIndex = s.i;
    }
  }
};
