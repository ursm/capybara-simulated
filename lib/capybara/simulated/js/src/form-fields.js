...
globalThis.__csimSendKeys = function (h, atoms) {
  ...
  const startValue = typeable ? controlLiveValue(n) : null;
    ...
    if (!blocked && wouldType) {
      const doDefault = () => {
        if (ceTypeable) {
          ...
          if (pos > 0) {
            // Live value edit (dirty value flag); a textarea's child-text
            // default is not rewritten.
            n._value = cur.slice(0, pos - 1) + cur.slice(pos);
  ...
  if (typeable && controlLiveValue(n) !== startValue) {
...
globalThis.__csimSetValue = function (h, value) {
  ...
  let kind = 'value';
  if (tag === 'textarea') {
    // Set the dirty value flag → live value in `_value`. The child text (the
    // default that `<form>.reset()` restores) is left untouched.
    n._value = v;
    ...
    if (type === 'checkbox' || type === 'radio') {
      ...
      // a text-like control. Sets the dirty value flag (`_value`).
      const maxlen = parseInt(n._attrs.maxlength || '', 10);
