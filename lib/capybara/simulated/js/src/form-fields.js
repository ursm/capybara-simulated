...
globalThis.__csimSetValue = function (h, value) {
  ...
  if (tag === 'textarea') {
    ...
    if (type === 'checkbox' || type === 'radio') {
      ...
      // same event chain a real user click does — click + input +
      // change (HTML activation behavior, mirroring `__csimClickResolve`).
      // Avo's `data-action='input->item-select-all#toggle'` listener
      // never reacts without the input event.
