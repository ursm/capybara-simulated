...
globalThis.__csimSendKeys = function (h, atoms) {
  ...
  const pressKey = (info, modifiers) => {
    ...
    // Tab's UI Events default action is to move focus through the
    // document's tabbable elements (reverse with shift). Real
    // browsers fire focusout/focusin during this move; menus that
    // close on `focusout` (Discourse's user-menu shift+tab → close)
    // rely on it. Skip if a handler preventDefault'd the keydown.
