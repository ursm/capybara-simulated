...
globalThis.__csimSendKeys = function (h, atoms) {
  ...
  const pressKey = (info, modifiers) => {
    ...
    // Enter / Space activation. `<a href>` with cmd/ctrl held still
    // fires the synthetic click even when the keydown was
    // preventDefault'd — real Chrome's UA-level "open in new window"
    // behaviour runs alongside the listener chain rather than as a
    // gated default action, and Discourse's global keymap (ItsATrap)
    // preventDefaults indiscriminately on Enter.
