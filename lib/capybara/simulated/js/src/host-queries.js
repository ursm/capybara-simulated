...
// Implicit-default option for a single-select <select>: the first
// non-disabled option when no option carries an explicit `selected`
// attribute (HTML's "ask for a reset" default, matching real
// browsers). Capybara's `have_select` / per-option `selected?` filter
// reads every option of a select in one poll iteration; computing the
// default per option was O(N) each ⇒ O(N²) per select. Memoise the
// result on the <select> node keyed on `settleGen` so the whole batch
// is O(N) amortised. The cache holds either the default option node or
// the `false` sentinel ("an explicit selection exists, so there is no
// implicit default"); `null` means "no enabled options".
function implicitDefaultOption(sel) {
  ...
  let result = null;
  for (const o of opts) {
    if (o._attrs.selected != null) { result = false; break; }
...
globalThis.__csimOptionSelected = function (h) {
  ...
  // `n` is the implicit selection iff it's the select's implicit
  // default (first non-disabled option, with no explicitly-selected
  // sibling). Computed once per select per settleGen — see
  // `implicitDefaultOption`.
