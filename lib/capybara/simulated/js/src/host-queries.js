...
globalThis.__csimDisabled = function (h) {
  ...
  if (n._tag === 'option') {
    // An option inherits disabledness from an ancestor optgroup / its owning
    // select. With the customizable-select content model an option can sit
    // inside wrapper elements (`<select><div><option>`), so walk PAST non-form
    // wrappers up to the owning select rather than stopping at the first one.
...
globalThis.__csimValue = function (h) {
  ...
  const tag = n._tag;
  if (tag === 'textarea') {
    // After a `set`, `_attrs.value` carries the user's intent verbatim.
    // Otherwise return the child text content as-is: HTML "first newline
    // removal" is a parse-time operation (the parser already dropped one
    // leading line terminator), so re-stripping here would double-strip.
