...
import { shadowEnabled, shadowQuery }   from './native-query-shadow.js';
...
globalThis.__csimQuery = function (rootHandle, selector) {
  ...
  if (!root) return [];
  // Shadow measurement: css-select stays authoritative; native runs alongside it,
  // timed + parity-checked (see native-query-shadow.js). Off → the direct path.
  if (shadowEnabled()) return shadowQuery(root, selector).map(el => el._id);
...
globalThis.__csimAttr = function (h, name) {
  ...
  switch (s) {
    ...
    case 'innerText': {
      ...
      // …through the SAME collapse the IDL getter applies. Keeping a second copy here let the two
      // drift: this one trimmed every kind of whitespace off a line, so a table-cell `\t` that
      // `el.innerText` reported was missing from `page.text` for the same element.
      return typeof globalThis.__csimCollapseInnerText === 'function'
        ? globalThis.__csimCollapseInnerText(raw)
...
// This realm's document-level Referrer-Policy — the content of the LAST valid
// `<meta name="referrer">` (HTML "meta referrer": later metas override earlier).
// The fetch/XHR path carries a per-request policy; a NAVIGATION has only the
// initiating document's default, which the Browser reads here to compute the
// request's Referer (compute_referrer). '' = none set → the Browser falls back to
// the platform default (strict-origin-when-cross-origin). Legacy tokens
// (never/default/always/origin-when-crossorigin) are mapped as real browsers do.
globalThis.__csimDocumentReferrerPolicy = function () {
  const VALID  = new Set(['no-referrer', 'no-referrer-when-downgrade', 'same-origin', 'origin',
                          'strict-origin', 'origin-when-cross-origin',
                          'strict-origin-when-cross-origin', 'unsafe-url']);
  const LEGACY = {never: 'no-referrer', default: 'no-referrer-when-downgrade',
  ...
  for (let i = 0; i < metas.length; i++) {
    const m = metas[i];
    if (!m || typeof m.getAttribute !== 'function') continue;
    if ((m.getAttribute('name') || '').toLowerCase() !== 'referrer') continue;
    let content = (m.getAttribute('content') || '').toLowerCase();
    content = LEGACY[content] || content;
    if (VALID.has(content)) policy = content;   // last valid wins
  }
  return policy;
};
...
// Form-field value reader — what Capybara reads via Node#value. Mirrors the
// SELENIUM driver (single select / input / textarea read the `value` IDL; a
// `multiple` select maps its selected options), not the rack-test driver: this
// is a browser-shaped driver, so real-browser semantics win where the two
// differ (rack-test falls back to the first option and reads raw option text).
// checkbox / radio surface their `.value` (defaulting to 'on') whether or not
// they are checked, as in a real browser; `checked?` is a separate query.
globalThis.__csimValue = function (h) {
  ...
  if (tag === 'select') {
    ...
    // collapsed/trimmed `text` IDL). Selection is read from selectedness only:
    // a selected option counts even when DISABLED (Chrome: `<option disabled
    // selected>X` ⇒ value "X"), and nothing selected reads back as '' — the
    // HTML "first option selected by default" rule is already materialised into
    // selectedness by runSelectednessAlgorithm, so a `size > 1` select or one
    // reset via `selectedIndex = -1` genuinely has no value. Both were verified
    // against headless Chrome.
    const opts = n._listOfOptions();
    if (n._attrs.multiple != null) {
      const out = [];
      for (const o of opts) {
        if (o._selectedness === true) out.push(o.value);
      }
      return out;
    }
    for (const o of opts) {
