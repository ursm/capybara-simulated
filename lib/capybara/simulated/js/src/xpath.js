...
function isHtmlDoc(doc) {
  // Mirror dom-nodes.js `isHtmlDocument`: a doc is XML-semantics (case-sensitive)
  // only if it was XML-PARSED (XML-MIME content type). A non-HTML, non-XML response
  // (text/plain, application/json, text/css) is HTML-parsed and keeps HTML XPath
  // semantics despite its `contentType` reflection — else a lowercase `/html`
  // query misses the uppercase nodeName (Discourse `become.json` sign-in).
...
const adapter = {
  ...
  localName:     n => n.localName,
  namespaceURI:  n => n.namespaceURI,
