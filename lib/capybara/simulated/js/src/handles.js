...
export function unregisterSubtree(node) {
  ...
  if (node._frameRealmId != null) {
    ...
    // captured before removal) must stay safe: its detached timers no-op rather than
    // throw once the realm is gone.
    if (globalThis.__csimNeuterDetachedWindow) globalThis.__csimNeuterDetachedWindow(rid);
