...
import { addValues } from './interpolate.js';
...
export function scriptAnimatedValue(el, prop, twin, underlying, resolve) {
  ...
  for (const anim of anims) {
    ...
    const got = valueAt(el, prop, stops, timing.progress, under, resolve);
    // `composite: 'add'` ADDS what the effect produces to the value underneath rather than
    // replacing it (web-animations §Compositing) — `flex-grow: 5` under keyframes 1 → 3 is 7 half
    // way, not 2.
    const composite = effect._frames[0].composite || effect._composite;
    value = composite === 'add' || composite === 'accumulate' ? addValues(prop, under, got) : got;
  }
  return value;
}
...
export function animationsForElement(el) { return animationsOn(el) || []; }
...
export function animationsForDocument(doc) {
  ...
  return out.sort((a, b) => a._sequence - b._sequence);
}
