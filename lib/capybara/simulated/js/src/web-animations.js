...
function putLonghands(into, prop, value, depth = 0) {
  ...
  // The cascade's expander answers that with a pending-substitution marker for each longhand — a
  // cascade-time device that means nothing in a keyframe, where the endpoint resolver expands the
  // `var()` itself. Each longhand takes the raw text instead.
  const pairs = hasSubstitution(String(value)) ? null : expandShorthandValue(prop, String(value));
  let any = false;
  if (pairs) {
    for (const [lh, v] of pairs) any = putLonghands(into, lh, v, depth + 1) || any;
    return any;
  }
  // An expander this shorthand has no entry for — or a value it cannot decompose — still fills
  // every slot: the value is all any of them has to go on.
