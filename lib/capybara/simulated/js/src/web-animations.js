...
// Whether an animation on `el` that is CURRENT or IN EFFECT animates any of `props` (web-animations, "Side effects
// of animations"): such an animation makes the element a stacking context, as `will-change` would — while it waits
// out its delay too — and one that has run out, or waits to run backwards, makes nothing.
export function currentAnimationAnimates(el, props) {
  for (const anim of animationsForElement(el)) {
    const effect = anim._effect;
    if (!effect || !effect._frames) continue;
