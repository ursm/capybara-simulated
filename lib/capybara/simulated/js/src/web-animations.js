  ...
  if (iterable) {
    for (const raw of input) {
      ...
      for (const key of Object.keys(raw)) {
        if (RESERVED.has(key) || !isKeyframeProperty(cssNameOf(key))) continue;
    ...
    const frameAt = (offset, index) => {
      ...
      if (!frame) {
        frame = newFrame(offsets[index] === undefined ? null : offsets[index],
                         easings.length ? easings[index % easings.length] : null,
                         composites.length ? composites[index % composites.length] : null);
...
export class KeyframeEffect extends AnimationEffect {
  ...
  set composite(v) { this._composite = effectComposite(v); }
  getKeyframes() {
