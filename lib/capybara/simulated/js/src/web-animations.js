...
import { animationNow, computedTimingAt, valueAt, canonicalEasing, onRenderingUpdate } from './animation.js';
import { markLayoutDirty } from './mutation-observer.js';
import { reduceMathFunctions } from './calc.js';
import { bumpCascadeVersion, expandShorthandValue, onScriptAnimationProperties } from './cascade.js';
...
const LIVE = new Set();
// Bumped whenever the live set changes, or an animation's keyframes or effect are replaced — the
// generation `scriptAnimationsDeclareProperty` memoises the document's animated-property set on.
// Through a funnel rather than at each `LIVE.add` / `LIVE.delete`, so a later membership change
// cannot forget to move it.
let liveGen = 0;
// …exported for a memo that must not outlive the start of an animation (layout.js `stackChain`: a script animation of
// `opacity` makes a stacking context the moment it starts, and nothing else moves then).
export function scriptAnimationGeneration() { return liveGen; }
function liveAdd(anim) {
  if (LIVE.has(anim)) return;              // `play()` on an animation already live changes nothing
  LIVE.add(anim);
  liveGen++;
}
function liveDelete(anim) {
  if (!LIVE.delete(anim)) return;
  liveGen++;
  // …and the index goes with the last animation. Every reader short-circuits on an empty `LIVE`, so
  // it would otherwise never be rebuilt — and it holds a strong reference to every animation that
  // was ever live AND to every element they targeted, for the life of the realm.
  if (!LIVE.size) { liveIndex = null; liveIndexGen = -1; }
}
let nextSequence = 1;
...
function effectComposite(value) {
...
export class AnimationEffect {
  ...
  // Changing an effect is a STYLE change for the element it targets, exactly as seeking the
  // animation is — so it goes through the animation's own invalidation rather than only writing the
  // field. Without it `getComputedStyle` reported the new value (it reads the effect) while the box
  // kept the old one (its layout memo never moved).
  //
  // THIS effect's target is marked here rather than left to `_invalidate`, which reads the
  // animation's CURRENT effect: one effect can be handed to a second animation (`a2.effect = e`),
  // and then `e._animation` is `a2` while `a1._effect` is still `e` — asking the animation would
  // dirty whichever element `a2` happens to point at.
  _changed() {
    if (this._target) markLayoutDirty(this._target, true);
    if (this._animation) this._animation._invalidate();
  }
}
...
export class KeyframeEffect extends AnimationEffect {
  ...
  // …and moving an effect to another element likewise leaves the old one to relax back.
  set target(v) {
    const outgoing = this._target;
    this._target = v || null;
    liveGen++;
    if (outgoing) markLayoutDirty(outgoing, true);
    this._changed();
  }
  get pseudoElement() { return this._pseudoElement; }
  // …and the latch with it: this decides whether the TARGET animates at all, so an effect that comes
  // back from `::before` to the element has the element's own memo to throw away again.
  set pseudoElement(v) {
    this._pseudoElement = v == null ? null : String(v);
    if (this._animation) this._animation._keyed = false;
    this._changed();
  }
  get composite() { return this._composite; }
  // An IDL ENUMERATION attribute IGNORES a value outside the enum — it is not a TypeError the way
  // a dictionary member is (Chrome-measured: `effect.composite = 'bogus'` leaves `replace`).
  set composite(v) {
    const name = String(v);
  ...
  getKeyframes() {
    return this._frames.map((f) => {
      // A keyframe with no easing of its own reports `linear` — the effect's easing is the
      // effect's, not the keyframe's (Chrome-measured).
      ...
      // …and the computed offset LAST, which is the order Chrome reports (measured).
      out.computedOffset = f.computedOffset;
      return out;
    });
  }
  setKeyframes(keyframes) { this._frames = normalizeKeyframes(keyframes); liveGen++; this._changed(); }
  getComputedTiming() {
...
export class Animation extends EventTarget {
  ...
  // Replacing the effect is a style change for BOTH elements: the one it was animating, which stops,
  // and the one it now animates. `_invalidate` only knows the current one, so the outgoing target is
  // marked here (measured: `anim.effect = null` left the box at x=100 where Chrome puts it back to 0).
  set effect(v) {
    const outgoing = this._effect && this._effect._target;
    this._effect = v || null;
    liveGen++;
    if (v) v._animation = this;
    if (outgoing) markLayoutDirty(outgoing, true);
    this._invalidate();
  }
  ...
  _invalidate() {
    ...
    //
    // …and not at all for an element the memo has never held a value for: there is no stale read to
    // throw away, and the re-key cold-starts the memo for EVERY element on the page. Once per
    // animation is still O(animations x elements), which is the staggered-entrance shape — a list
    // that animates each row in as it is added, where every target is brand new. Measured on a
    // 400-item list: 81,000 `margin-left` computes against 801 for the same list without the
    // animations, and the whole quadratic is this line.
    //
    // The latch is per (animation, TARGET), not per animation: an effect moved to another element
    // has a SECOND element's memo to throw away, and the one-shot latch never asked about it —
    // measured against Chrome, `a = elA.animate(…); a.effect.target = elB` left elB at `none` / x 0
    // where Chrome has `matrix(…200…)` / x 200. Note that case gets `getComputedStyle` and
    // `getBoundingClientRect` wrong TOGETHER, so the "computed value right, box stale" tell that
    // finds most of these does not fire for it.
    //
    // What the gate rests on: the animated element's own memo entry stands in for every
    // cascade-version-keyed memo that could hold a stale value of it. True today because a value
    // derived from this element is either resolved through the animation layer on every read or
    // warms this element's own entry on the way (measured over eight ancestor / descendant /
    // inherited / percentage shapes). Two things would break it when they land: an animatable
    // CUSTOM PROPERTY (reading a child's `var()` does not warm the parent's entry) and an animated
    // `visibility` / `display` (`__csimInnerText` is keyed on the cascade version and is not
    // covered by `markLayoutDirty`).
    if (!this._keyed || this._keyedFor !== target) {
      this._keyed = true;
      this._keyedFor = target;
      // FAIL OPEN: without the reader, or without a target, the re-key is paid as it always was.
      // A missing hook then costs time, never correctness.
      const cached = globalThis.__csimHasCachedDeclaredValues;
      if (!target || !cached || cached(target)) bumpCascadeVersion();
    }
  }
  ...
  play() {
    ...
    liveAdd(this);
    // …and it asks for the re-key again: while it was idle the element's static value was cacheable
    // once more, and that is exactly the value the re-key exists to discard (Chrome-measured:
    // `cancel(); read; play()` left the box at x 0 where Chrome puts it back at 200).
    if (this._idle) this._keyed = false;
  ...
  commitStyles() {
    ...
    // What Chrome commits is the COMPUTED value. For most properties this driver's resolved read IS
    // that; for the box properties and the transform family it is a used value or a composed matrix,
    // one stage too late — `width: 25%` and `translateX(25%)`, not `50px` and a matrix. Style-proxy
    // owns which is which (`__csimCommittedValue`), and answers null for the ones this read already
    // gets right.
    const committed = globalThis.__csimCommittedValue;
    ...
    for (const prop of props) {
      const animated = committed ? committed(target, prop) : null;
      const value = animated == null || animated === '' ? computed.getPropertyValue(prop) : String(animated);
...
// Every live animation, INDEXED BY THE ELEMENT IT TARGETS, in composite order — rebuilt whenever
// `liveGen` moves, which is the same generation the gate's property answer rides on.
//
// This used to be a scan of the whole `LIVE` set per element. Every style read of every element
// paid for every animation on the page, so a list that animates each row in as it is added — the
// staggered-entrance shape Turbo Streams and every "fade in the new item" UI uses — cost K reads x
// K elements x K animations. Measured on a 400-item list: 1,460,000 calls here and **388,729,400**
// iterations over `LIVE`, for 400 animations. The index makes it one Map lookup and a walk of the
// handful of animations that actually target this element.
//
// The MEMBERSHIP is indexed; the filters below stay per call, because `_idle` and `isConnected`
// move without `LIVE` moving.
let liveIndexGen = -1;
let liveIndex = null;
function animationsByTarget() {
  if (liveIndexGen !== liveGen) {
    liveIndexGen = liveGen;
    liveIndex = new globalThis.Map();
    for (const anim of LIVE) {
      const effect = anim._effect;
      if (!effect || !effect._target) continue;
      let list = liveIndex.get(effect._target);
      if (list === undefined) liveIndex.set(effect._target, list = []);
      list.push(anim);
    }
    // Composite order, per target rather than over the whole set: `play()` after a `cancel()` puts
    // an animation back at the END of the insertion-ordered `LIVE`, so iteration order is not
    // sequence order — and sorting each element's handful costs nothing next to sorting all of them.
    for (const list of liveIndex.values()) if (list.length > 1) list.sort((a, b) => a._sequence - b._sequence);
  }
  return liveIndex;
}
...
function animationsOn(el) {
  ...
  const list = animationsByTarget().get(el);
  ...
  for (const anim of list) {
    if (anim._idle || anim._effect._pseudoElement) continue;
    (out || (out = [])).push(anim);
  }
  return out;
}
...
// Does ANY live script-driven animation declare this property, anywhere in the document? The O(1)
// counterpart to `cascadeDeclaresProperty` for the animation origin — what the layout gates ask
// before deciding a property is absent from the page. Per DOCUMENT rather than per element, because
// the gates run per element per pass and `animationsOn` is a scan of every live animation (rule 3);
// the set is rebuilt only when the live set or an effect's keyframes move.
// Per TARGET, not per document: an effect names the element it animates, so the gate can be exact
// there for the price of the same walk. An IDLE animation counts (`animationsOn` filters those out
// for the VALUE, which is a different question — a gate may over-approximate and this one does), and
// so does one targeting a pseudo-element, whose box hangs off this element either way.
// Memoised per element on the same generation as the index it reads, so the repeated gate calls a
// single layout pass makes for one element cost one walk, not one per property.
//
// In a module-local WeakMap rather than on the element: `liveGen` is per REALM and every realm's
// starts at 0, so an expando would let a parent-realm read of a child-realm element write an answer
// the child realm reads back as current — the trap `DV_MEMO`'s own comment documents.
const LIVE_PROPS = new globalThis.WeakMap();
function livePropsFor (el) {
  const list = animationsByTarget().get(el);
  if (list === undefined) return undefined;
  const memo = LIVE_PROPS.get(el);
  if (memo !== undefined && memo.gen === liveGen) return memo.props;
  const props = new globalThis.Set();
  for (const anim of list) {
    const frames = anim._effect._frames;
    if (frames) for (const frame of frames) for (const p of Object.keys(frame.props)) props.add(p);
  }
  LIVE_PROPS.set(el, { gen: liveGen, props });
  return props;
}
export function scriptAnimationsDeclareProperty (el, prop) {
  if (!LIVE.size) return false;
  const props = livePropsFor(el);
  return props !== undefined && props.has(prop);
}
export function scriptAnimationsDeclareAnyOf (el, names) {
  if (!LIVE.size) return false;
  const props = livePropsFor(el);
  if (props === undefined) return false;
  for (const name of names) if (props.has(name)) return true;
  return false;
}
...
// Whether a script animation on `el` that is CURRENT or IN EFFECT animates any of `props` (web-animations, "Side
// effects of animations"): such an animation makes the element a stacking context, as `will-change` would — while it
// waits out its delay too — and one that has run out, or waits to run backwards, makes nothing. The CSS half is
// `cssAnimationCurrentlyDeclares`, asked of the cascade's own model: going through the objects here would sync every
// CSS animation in the document per question.
export function scriptAnimationCurrentlyAnimates(el, props) {
  const anims = animationsOn(el);
  if (!anims) return false;
  for (const anim of anims) {
    const effect = anim._effect;
    if (!effect._frames) continue;
    const t = computedTimingAt(timingRecord(effect._timing), anim._currentTime());
    const rate = anim._playbackRate;
    const current = t.phase === 'active' || (t.phase === 'before' && rate > 0) || (t.phase === 'after' && rate < 0);
    if (!current && t.progress == null) continue;
    for (const frame of effect._frames) for (const prop of props) if (frame.props[prop] !== undefined) return true;
  }
  return false;
}
...
// Each interface carries its own class string, which is how a page (and the IDL tests) tell an
// `Animation` from a plain object: `Object.prototype.toString.call(el.animate(…))` must read
// `[object Animation]`, not `[object Object]`.
for (const ctor of [AnimationEffect, KeyframeEffect, AnimationTimeline, DocumentTimeline, Animation]) {
