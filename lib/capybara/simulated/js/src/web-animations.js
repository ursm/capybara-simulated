...
let liveGen = 0;
function liveAdd(anim)    { LIVE.add(anim);    liveGen++; }
function liveDelete(anim) { LIVE.delete(anim); liveGen++; }
let nextSequence = 1;
...
export class KeyframeEffect extends AnimationEffect {
  ...
  set pseudoElement(v) { this._pseudoElement = v == null ? null : String(v); this._changed(); }
...
function livePropsFor (el) {
  if (livePropsGen !== liveGen) {
    livePropsGen = liveGen;
    livePropsByTarget = new globalThis.Map();
    for (const anim of LIVE) {
      const effect = anim._effect;
      if (!effect || !effect._frames || !effect._target) continue;
      let set = livePropsByTarget.get(effect._target);
      if (set === undefined) livePropsByTarget.set(effect._target, set = new globalThis.Set());
      for (const frame of effect._frames) for (const p of Object.keys(frame.props)) set.add(p);
    }
  }
  return livePropsByTarget.get(el);
}
