...
export const navigator = {
  ...
  serviceWorker: {
    // Service Workers are modeled only in a UNIVERSAL-SERVER context (the WPT
    // runner, where every host is served in-process — see allHostsLocal). A real
    // app keeps the "not supported" rejection so its registration-failed branch
    // runs, never a half-modeled success path that could alter app behaviour. In
    // the runner, `register` spawns a worker running the SW script (an executor
    // context) and resolves with a minimal ServiceWorkerRegistration.
    register(scriptURL, options) {
      ...
      try {
        ...
        const abs  = new globalThis.URL(String(scriptURL), base).href;
        const scope = (options && options.scope != null)
          ? new globalThis.URL(String(options.scope), base).href
          : new globalThis.URL('./', abs).href;
        const handle = globalThis.__csim_serviceWorkerRegister(abs, globalThis.__csimBcOriginKey());   // spawn the SW (executor) worker, inheriting this context's origin
        const sw = { scriptURL: abs, state: 'activated', addEventListener() {}, removeEventListener() {}, postMessage() {} };
        const reg = {
          scope, active: sw, installing: null, waiting: null,
          update()     { return Promise.resolve(reg); },
          // Actually stop the spawned SW worker (don't just resolve a lie).
          unregister() { try { globalThis.__csim_workerTerminate(handle); } catch (_) {} return Promise.resolve(true); },
