...
//
// The TYPE is known where the VALUE is not: `10%` and `1em` are lengths this stage cannot place, so
// they are carried as a type with a null value, and the check goes on through them. Stopping at the
// first unplaceable term kept every type error that had one beside it — `calc(min(10%, 30px) *
// max(1%, 2px))`, `calc(100px / min(10%, 3px))`, `calc(1em + 1)` — which Chrome drops at parse time.
// A PERCENTAGE is a type of its own until a basis places it: it joins a dimension (`calc(100% -
// 1em)` is a length) and never a number (`calc(50% + 0.1)` is dropped even for `opacity`).
...
const NUMBER = 'number', LENGTH = 'length', ANGLE = 'angle', TIME = 'time', PERCENT = 'percent', RESOLUTION = 'resolution';
...
// …and the RELATIVE ones — font-, viewport- and container-relative — which are lengths whatever they come to, so a
// term in one still has a type when `toPx` cannot place it.
const RELATIVE_LENGTH_RE = /^(?:r?(?:em|ex|cap|ch|ic|lh)|[sld]?v(?:w|h|i|b|min|max)|cq(?:w|h|i|b|min|max))$/;
// …and a RESOLUTION (`image-set()`'s `2x`), a type of its own — never a length (`calc(1x + 1px)` is dropped). A `<flex>`
// is no type a math function takes at all: `calc(1fr)` is dropped by Chrome, in a track list or anywhere.
const RESOLUTION_UNIT_RE = /^(?:x|dppx|dpi|dpcm)$/;
...
function makeParser(toks, toPx) {
  ...
  function dimension(tok) {
    ...
    if (tok.unit === '%') {
      ...
      return pct == null ? val(PERCENT, null) : val(LENGTH, pct);
    }
    ...
    if (RESOLUTION_UNIT_RE.test(tok.unit)) return val(RESOLUTION, null);   // (typed, not placed: kept as written)
    if (tok.unit === 'fr') return INVALID;
    const px = toPx(tok.n, tok.unit);
    if (px != null) return val(LENGTH, px);
    return RELATIVE_LENGTH_RE.test(tok.unit) ? val(LENGTH, null) : UNRESOLVABLE;
  }
  // The type two operands of a SUM (or of a comparison) come to, or null where they cannot meet.
  const join = (a, b) => (a === b ? a : a === PERCENT && b !== NUMBER ? b : b === PERCENT && a !== NUMBER ? a : null);
  // …and a value from operands' values, null (unplaced) where any of them is.
  const known = (ns, f) => (ns.some((n) => n == null) ? null : f(...ns));
  ...
  function product() {
    ...
    for (;;) {
      ...
      if (t.t === '*') {
        ...
        left = val(left.type === NUMBER ? right.type : left.type, known([left.n, right.n], (a, b) => a * b));
      } else {
        ...
        left = val(left.type, known([left.n, right.n], (a, b) => a / b));
      }
    }
  }
  ...
  function sum() {
    ...
    for (;;) {
      ...
      const right = product();
      if (!right || right.invalid) return right;
      const type = join(left.type, right.type);
      if (type == null) return INVALID;                      // `calc(1px + 1)`, `calc(10deg + 5px)`
      left = val(type, known([left.n, right.n], (a, b) => (t.t === '+' ? a + b : a - b)));
    }
  }

  function applyFn(name, list) {
    if (!list.length) return UNRESOLVABLE;
    const type = list.reduce((t, v) => (t == null ? null : join(t, v.type)), list[0].type);
    if (type == null) return INVALID;
    const ns = list.map(v => v.n);
    let f;
    if (name === 'calc')       { if (list.length !== 1) return INVALID; f = (a) => a; }
    else if (name === 'min')   f = Math.min;
    else if (name === 'max')   f = Math.max;
    ...
    else if (name === 'clamp') { if (list.length !== 3) return INVALID; f = (lo, v, hi) => Math.max(lo, Math.min(v, hi)); }
    else return UNRESOLVABLE;
    return val(type, known(ns, f));
  }
...
// `wrap`: each result goes back inside a `calc()` — the SPECIFIED surface's form (`simplifySpecifiedMath`).
export function reduceMathFunctions(value, toPx, wrap = false) {
  ...
  for (;;) {
    ...
    const res = v && v.n != null ? round(v.n) + SUFFIX[v.type] : null;
    out += rest.slice(0, start) + (res == null ? expr : wrap ? 'calc(' + res + ')' : res);
...
// `-webkit-calc()` is `calc()` under its legacy name, which Chrome still parses — and serializes as `calc()`
// (`width: -webkit-calc(100% - 10px)` reads back `calc(100% - 10px)` and lays out 998 of 1008, where keeping the
// prefix left an unknown function the layout ignored). So the alias is rewritten on the way IN, once per
// declaration on every surface, and nothing downstream — the static check, the reducer, the walk — ever meets it.
// Not inside a string, which is text.
const WEBKIT_CALC_RE = /-webkit-calc\(/gi;
export function unprefixMath(value) {
  const s = String(value);
  if (!/-webkit-calc\(/i.test(s)) return s;
  const scan = s.indexOf('"') < 0 && s.indexOf("'") < 0 ? s : blankStrings(s);
  let out = '', at = 0;
  for (const m of scan.matchAll(WEBKIT_CALC_RE)) { out += s.slice(at, m.index) + 'calc('; at = m.index + m[0].length; }
...
export function simplifySpecifiedMath(value) {
  ...
  // …each result back inside its own `calc()`, and nothing else: re-wrapping every bare number the reduced text held
  // turned the value's OTHER components into math too — `transition: opacity 1s calc(1s + 100ms)` read back
  // `opacity calc(1s) calc(1.1s)`, and `margin: 10px calc(5px + 5px)` `calc(10px) calc(10px)` (Chrome: `10px calc(10px)`).
  const reduced = reduceMathFunctions(src, absoluteToPx, true);
  return reduced === 'invalid' ? src : reduced;
}
