...
(function () {
  ...
  // Visibility predicate behind Capybara's own `visible?`, and the ancestor walk `isVisibleNode`
  // mirrors: "visible unless something says hidden" — INVISIBLE_TAGS (head/script/style/template/…),
  // the UA hides no page can override (`uaNotRendered`: a hidden input, an `<audio>` with no
  // controls, an `<embed>` with no resource), the `hidden` attribute, `display: none` /
  ...
  globalThis.__csimVisible = function (h) {
    ...
    if (uaNotRendered(n)) return false;
