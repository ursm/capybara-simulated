...
(function () {
  ...
  globalThis.__csimLoadDocument = function (html, contentType) {
    ...
    // convert `<template shadowrootmode>` into shadow roots before the parsed
    // head/body are grafted onto the live skeleton. The conversion registers
    // the shadow content itself; slot assignment is computed lazily on read
    // (assignedNodes / findSlotForSlottable), so no connect-time wiring is needed.
    // Gate on a cheap scan so the O(N) tree walk is skipped on the
    // (overwhelming) majority of pages that author no declarative shadow root.
    // Ingest the import map from the parsed tree first so a declarative
    // `shadowrootadoptedstylesheets` specifier resolves (the main ingest at
    // load runs only after the graft, too late for this parse-time step).
    if (/shadowrootmode/i.test(srcHtml)) { ingestImportmaps(freshDoc); processDeclarativeShadowRoots(freshDoc); }
    ...
    // Graft the parsed document prolog — the leading comments and the
    // `<!DOCTYPE …>` that precede the document element — onto the live
    // document (the skeleton built at boot has none). Drop the live doc's
    // prior comment / doctype children first, then splice the fresh prolog in
    // before the live <html> so `document.doctype` and `document.childNodes`
    ...
    for (const c of d._children.slice()) {
      if (c.nodeType === NODE_DOCTYPE || c.nodeType === NODE_COMMENT) {
        unregisterSubtree(c);   // drop its handle so a stale `_id` can't resolve to the detached node
        c._parent = null;
    ...
    const prolog = freshHtmlIdx > 0
      ? freshKids.slice(0, freshHtmlIdx).filter(c => c.nodeType === NODE_DOCTYPE || c.nodeType === NODE_COMMENT)
      : [];
    const liveHtmlIdx = d._children.indexOf(d.documentElement);
    if (liveHtmlIdx !== -1 && prolog.length) {
      for (const c of prolog) { c._parent = d; c._ownerDoc = d; registerSubtree(c); }
      d._children.splice(liveHtmlIdx, 0, ...prolog);
    }
    // Carry the parsed document's quirks flag onto the reused live document so
    // `document.compatMode` agrees with the grafted doctype (a no-doctype page
    // reports "BackCompat"); without this the live skeleton's undefined flag
    // pins the main page to "CSS1Compat" regardless of the served markup.
  ...
  function __isTabbable (el) {
    ...
    // `[hidden]` is resolved through `selfHidden` (the UA `[hidden]{display:none}`
    // rule), so an author `display` override correctly keeps the element tabbable.
