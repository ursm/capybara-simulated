...
import { isObscured, rectOf, pointerRectOf, applyScrollTo, applyScrollBy, frameContentBox, frameObscuredAt, invalidateLayout, ensureInView } from './layout.js';
  ...
  globalThis.__csimClickResolve = function (h, modifiers, opts) {
    ...
    if (label) {
      ...
      if (labeled && labeled !== n && labeled !== label) {
        ...
        // <label> for a submit button still leaves the submit intent for the drain.
        return __csimClickResolve(labeled._id, mods, opts);
      }
    }
    if (isSubmitButton(n)) {
