...
(function () {
    ...
    subresourceLoadTask(el, () => {
      ...
      if (/^data:/i.test(href)) {
        ...
        if (sw) {
          ...
          try {
            const resp = __rackFetch('GET', abs, '', null, 'follow');
  ...
  function __collapseInnerText(raw) {
    if (raw == null) return '';
    // Trim SPACES, not every whitespace: a `\t` on this line is a table-cell separator this walk
    // put there, and Chrome keeps the trailing ones (`<td>abc<td><td>` is "abc\t\t").
    return raw.split('\n').map(line => line.replace(/ {2,}/g, ' ').replace(/^ +| +$/g, ''))
      .join('\n').replace(/^\n+|\n+$/g, '');
  }
  // Whether the LAST rendered text under `root` sits in preserve-whitespace
  // context and ends with a newline. The innerText spec's start/end trimming
  // removes only the GENERATED (block-boundary) line breaks — a newline that is
  // literally part of preserved text is rendered text and survives: a text/plain
  // document's <pre> body "Network\n" reads back WITH its trailing newline
  // (static-router-main-resource pins Chrome). __collapseInnerText can't tell the
  // two apart from the collapsed string, so this reverse walk (visibility rules
  // mirroring collectVisibleText's skips; memoized with the same gen/cv key via
  // the caller) re-attaches exactly that one trailing break.
  function __trailingPreservedNewline(root) {
    let rootPre = false;
    for (let cur = root; cur; cur = cur._parent) {
      if (cur.nodeType === NODE_ELEMENT && elementPreservesWhitespace(cur)) { rootPre = true; break; }
    }
    // Returns undefined (keep looking backwards) or the count of trailing
    // preserved newlines on the LAST rendered text node (0 when it has none /
    // isn't preserved). The skip rules mirror collectVisibleText's rendering
    // model: closed <details> renders only its <summary> children, and a
    // whitespace-only text child of a flex-like container generates no box.
    const walk = (n, pre) => {
      const flexContext = n.nodeType === NODE_ELEMENT && isFlexLikeContainer(n);
      const kids = n._children || [];
      for (let i = kids.length - 1; i >= 0; i--) {
        const k = kids[i];
        if (k.nodeType === NODE_TEXT) {
          const t = String(k.data == null ? '' : k.data);
          if (!t) continue;
          if (!/\S/.test(t) && (flexContext || !pre)) continue;   // renders nothing — keep looking
          if (!pre) return 0;
          const run = /\n+$/.exec(t);
          return run ? run[0].length : 0;
        }
        if (k.nodeType !== NODE_ELEMENT || INVISIBLE_TAGS.has(k._tag) || selfHidden(k)) continue;
        let r;
        if (k._tag === 'details' && (!k._attrs || k._attrs.open == null)) {
          const dk = k._children || [];
          for (let j = dk.length - 1; j >= 0; j--) {
            const c = dk[j];
            if (c.nodeType !== NODE_ELEMENT || c._tag !== 'summary' || selfHidden(c)) continue;
            r = walk(c, pre || elementPreservesWhitespace(c));
            if (r !== undefined) break;
          }
        } else {
          r = walk(k, pre || elementPreservesWhitespace(k));
        }
        if (r !== undefined) return r;
      }
      return undefined;
    };
    return walk(root, rootPre) || 0;
  }
  ...
  globalThis.__csimInnerText = function (node) {
    ...
    if (__innerTextNotRendered(node)) {
      ...
      // A trailing newline run that is PRESERVED TEXT (inside a <pre>-like context)
      // is rendered content, not a trimmable generated break — re-attach it.
      const nlRun = out ? __trailingPreservedNewline(node) : 0;
      if (nlRun > 0) out += '\n'.repeat(nlRun);
    }
