...
(function () {
  ...
  globalThis.__csimClickResolve = function (h, modifiers) {
    ...
    // Tests that click the `<details>` element by class (Discourse's
    // select-kit `find('.translation-selector').click` pattern) hit
    // the details directly, not the summary; without layout we can't
    // know whether the click point would have landed in the visible
    // summary header — so accept either target when a summary child
    // exists. Select-kit's click handler preventDefaults, but its
    // open/close state listens on the `toggle` event, so we run this
    // before the defaultPrevented early-return to match the user-
    // perceived behavior in real browsers.
    {
      let details = null;
      if (n._tag === 'summary') {
        let p = n._parent;
      ...
      } else if (n._tag === 'details') {
        const hasSummary = (n._children || []).some(c => c && c._tag === 'summary');
        details = hasSummary ? n : null;
      }
      if (details) {
