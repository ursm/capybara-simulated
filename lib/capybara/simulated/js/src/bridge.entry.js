...
(function () {
  ...
  globalThis.__csimFrameWindow = function (frameEl) {
    ...
    if (typeof globalThis.__csim_createFrameRealm === 'function' && globalThis.RustyRacer && typeof globalThis.RustyRacer.contextGlobal === 'function') {
      ...
      if (id != null) {
        ...
        // attribute — `frames[i].name` / named `window.frames['name']` access
        // (shadow-dom/leaktests/window-frames.html). Only set it when the
        // attribute is present, so a frame whose load-time script set its own
        // window.name (and has no name attr) isn't clobbered back to ''.
        const frameName = frameEl.getAttribute('name');
        if (frameName != null) { try { win.name = frameName; } catch (e) {} }
        return win;
      }
    }
  ...
  globalThis.__csimInnerText = function (node) {
    ...
    // Memoize the subtree walk per (settleGen, cascadeVersion) — the getter is
    // read broadly and the old textContent path also walked, so this is a net win
    // and keeps a hot `.innerText` reader O(1) between mutations (rule 3). Same
    // key as __csimVisibleText.
