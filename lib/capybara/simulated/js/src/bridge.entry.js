...
(function () {
  ...
  // Minimal XML / XHTML document parse: unlike HTML there's no implicit
  // html/head/body skeleton — the documentElement is the source's first
  // element. It reuses the HTML fragment tokenizer (which honours self-closing
  // `<body/>`) after stripping any XML declaration / leading DOCTYPE; it is NOT
  // a full XML parser (no CDATA / custom entities / processing instructions),
  // which is sufficient for the well-formed XHTML/simple-XML frame docs in
  // scope. `documentElement` is null when the source has no element node.
  function parseXMLDocument(xml) {
    ...
    // XML namespace declarations: the shared HTML tokenizer stores `xmlns` /
    // `xmlns:foo` as plain attributes, but in XML they live in the XMLNS
    // namespace. Tag each element's declarations into `_attrNS` so
    // lookupNamespaceURI / lookupPrefix / isDefaultNamespace resolve them.
    ...
    walkSubtree(doc, (n) => {
      ...
      for (const key in n._attrs) {
        ...
        const prefix    = colon === -1 ? null : key.slice(0, colon);   // 'xmlns' for xmlns:foo
        const localName = colon === -1 ? key  : key.slice(colon + 1);  // 'xmlns' for the default decl
        (n._attrNS || (n._attrNS = {}))[key] = { ns: XMLNS_NS, prefix, localName };
      }
    });
    return doc;
  }
  ...
  // mini_racer's Ruby↔V8 round-trip floor is ~4 ms per call. Running
