...
(function () {
  ...
  function __csimHitTestRetarget(n) {
    ...
    const hit = dfs(n);
    if (hit) return hit;
    // Single-anchor wrapper (Discourse nav-pill `<li><a href>…</a></li>`):
    // when the wrapper has exactly one element descendant and it is an
    // anchor with href, the anchor fills the wrapper visually and a
    // geometric click would land on it. The Redmine multi-child row is
    // disambiguated by element count (≥2 → stay on wrapper).
    let single = n;
    while (single) {
