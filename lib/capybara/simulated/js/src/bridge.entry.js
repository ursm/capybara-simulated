...
(function () {
  ...
  // `PromiseRejectionEvent` is intentionally NOT exposed yet. Once
  // `isCallable(globalThis.PromiseRejectionEvent)` returns true,
  // core-js's promise-constructor-detection switches to the native
  // V8 Promise. Native is more spec-compliant in isolation, but our
  // settle protocol (`__settleGen` yields on first observable
  // change in `Browser#settle`) assumes the macrotask-paced
  // scheduling of core-js's polyfill — when chains collapse into a
  // single boundary, settle bails before Capybara samples the
  // intermediate state that Turbo's render path / IntersectionObserver
  // recheck depends on. Closing this gap needs settle to drain
  // chain-deep before yielding (or a microtask checkpoint model
  // that matches a real browser's task boundaries).
