...
import { Event, DOMException, CustomEvent, MouseEvent, KeyboardEvent, InputEvent, SubmitEvent, MessageEvent, ClipboardEvent, EventTarget } from './events.js';
import { AbortSignal, AbortController } from './abort.js';
import { EventSource } from './event-source.js';
import { FileReader } from './file-reader.js';
import { ImageData, ImageBitmap, CanvasRenderingContext2D, OffscreenCanvas, createImageBitmap } from './canvas.js';
import { btoa, atob, TextEncoder, TextDecoder } from './encoding.js';
import { URL, URLSearchParams, Headers } from './url.js';
...
import { indexedDB, IDBRequest, IDBDatabase, IDBObjectStore, IDBTransaction, IDBIndex, IDBCursor } from './idb.js';
import './workers.js';
...
(function () {
  ...
  // round-trips work via `.text()` / `.arrayBuffer()` / `.slice()`.
  // (FileReader lives in `src/file-reader.js`.)
  ...
  // Exposed on globalThis for canvas.js's `createImageBitmap` (and
  // any future module that needs raw Blob bytes) until the Blob /
  // File classes themselves move out of this IIFE.
  ...
  // Expose the Blob/FormData byte helpers so the xhr.js module can
  // reach them. Both go away when the Blob class itself moves out
  // of this IIFE.
  ...
  // Pixel buffers / canvas chain — OCR / Tesseract path. See canvas.js.
  ...
  // Wire `URL.createObjectURL` / `revokeObjectURL` to the shared
