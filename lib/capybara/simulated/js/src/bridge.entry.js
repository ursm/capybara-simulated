...
(function () {
  ...
  globalThis.__csimClickResolve = function (h, modifiers) {
    ...
    if (isSubmitButton(n)) {
      ...
      // A submit whose `target` (or the submitter's `formtarget`) names a
      // same-document frame is contained to that child context — handle it
      // JS-side (fetch + load into the frame) and return no top-page nav, the
      // same routing the programmatic submit() / requestSubmit() / click-
      // activation paths use via recordFormSubmission. Cheap target pre-check
      // inside, so an ordinary top/self submit falls straight through.
      if (globalThis.__csimSubmitFormToNamedFrame && globalThis.__csimSubmitFormToNamedFrame(form, n)) return null;
