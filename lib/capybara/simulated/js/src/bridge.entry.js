...
(function () {
  ...
  globalThis.__csimFrameWindow = function (frameEl) {
    ...
    if (typeof globalThis.__csim_createFrameRealm === 'function' && globalThis.RustyRacer && typeof globalThis.RustyRacer.contextGlobal === 'function') {
      ...
      const id = globalThis.__csim_createFrameRealm(frameLocationHref, body, contentType, parentId, frameEl.getAttribute('name'), frameDocOrigin, frameLocationOrigin, jsUrlSource, frameAboutBase);
      if (id != null) {
        ...
        // PUSH the frame's viewport (its container's content box) into the child realm. The child's
        // layout must never pull this back across the realm boundary: layout runs on the page's own
        ...
        try { win.__csimFrameViewport = frameContentBox(frameEl); } catch (e) {}
  ...
  // A frame's container just changed size. Called by the PARENT realm (never by page script), which
  // has already measured the new content box.
