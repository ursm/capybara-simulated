...
(function () {
  ...
  globalThis.__csimLoadDocument = function (html, contentType) {
    ...
    // A nested browsing context (iframe) can load a non-HTML, non-XML resource
    // (text/css, text/plain, …); its document reports the response MIME as
    // `contentType`. The main document is always text/html, so leave the default
    // (`_contentType` undefined → 'text/html', isHtmlDocument true) untouched.
    if (__ct && __ct !== 'text/html') d._contentType = __ct;
