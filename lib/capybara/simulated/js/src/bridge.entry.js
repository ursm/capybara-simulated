...
import { Animation, AnimationEffect, KeyframeEffect, AnimationTimeline, DocumentTimeline } from './web-animations.js';
import './css-animation-objects.js';   // registers the CSS animation/transition object mirror
import { NODE_ELEMENT, NODE_TEXT, NODE_DOC, NODE_DOCTYPE, NODE_COMMENT, NODE_FRAGMENT, installNodeConstants } from './constants.js';
import { walk, walkSubtree, walkInclShadow, assignOwnerDoc, isConnected, classes, scriptText } from './walk.js';
import { handles as __handles, lookup, registerNode, registerSubtree, unregisterSubtree } from './handles.js';
import { isObscured, rectOf, pointerRectOf, applyScrollTo, applyScrollBy, frameContentBox, frameObscuredAt, invalidateLayout, ensureInView, hitTest, viewportSize, isFlatAncestor } from './layout.js';
import { dragBegin, dragMove, dragFinish } from './drag.js';
...
import { ceTryConnect, runSelectednessAlgorithm, ensureOptionSelInit, connectSelectedContent, ownerSelectOf, updateSelectedContent, finalizeSelectOptions } from './custom-elements.js';
...
import { deliverMutations, flushMutationDelivery, hasQueuedRecords, hasObservers, recordAttrMutation, recordChildList, signalSlotChildList, noteParsedChange } from './mutation-observer.js';
...
// Side-effect import: installs `__csimScreenshot`, the paint entry `save_screenshot` calls.
import './paint.js';
...
import './websocket.js';
import './fetch.js';
import './cache-storage.js';   // CacheStorage/Cache (globalThis.caches) — depends on fetch.js's Request/Response
import './file-reader.js';
import './form-data.js';
import { serializeEntryList } from './form-fields.js';
import './history.js';
import './host-queries.js';
import './idb.js';
import { timeParse } from './native-query-shadow.js';   // DOM-in-Rust store-migration measurement (inert unless __csimNativeShadow)
import './navigator.js';
import './webauthn.js';
import './observers.js';
...
(function () {
  ...
  // The Web Animations constructors. A page feature-probes these before using `animate()`, and a
  // library that constructs a `KeyframeEffect` itself needs the real class.
  ...
  // Media / plugin resources fetched once for their Resource Timing entry (per URL + initiator).
  const MEDIA_RT_SEEN = new globalThis.Set();
  function mediaFetchAndRecord(el, rawUrl, type) {
    if (!rawUrl) return;
    let abs = rawUrl;
    try { abs = new globalThis.URL(rawUrl, el.baseURI || (globalThis.document && globalThis.document.baseURI) || undefined).href; } catch (_) {}
    const dedup = type + '|' + abs;
    if (MEDIA_RT_SEEN.has(dedup)) return;
    MEDIA_RT_SEEN.add(dedup);
    const cors = !!(el._attrs && el._attrs.crossorigin != null);
    const creds = cors && el._attrs.crossorigin === 'use-credentials' ? 'include' : 'same-origin';
    const started = globalThis.performance ? globalThis.performance.now() : 0;
    let meta = null;
    try { meta = globalThis.__csim_resourceTimingFetch ? globalThis.__csim_resourceTimingFetch(abs, cors, creds) : null; } catch (_) { meta = null; }
    if (typeof globalThis.__csimRecordResource === 'function') {
      globalThis.__csimRecordResource({ name: abs, initiatorType: type, startTime: started, resp: meta, noCors: !cors });
    }
  }
  function firstMediaSourceSrc(el) {
    const kids = el._children;
    if (kids) for (const c of kids) if (c._tag === 'source' && c._attrs && c._attrs.src) return c._attrs.src;
    return null;
  }
  // The `css`-style timing of a media / plugin element's resources: `<video>` poster ('video'), its
  // `src` or first `<source>` ('video'/'audio'), and each `default` `<track>` ('track'); an
  // `<embed src>` ('embed') / `<object data>` ('object'). Chrome picks the first PLAYABLE source and
  // loads a track only when its mode shows — approximated here as the first source and `default`
  // tracks, which the single-source WPT fixtures need.
  function recordMediaResourceTiming(el) {
    const tag = el._tag;
    if (tag === 'video' || tag === 'audio') {
      if (tag === 'video' && el._attrs.poster) mediaFetchAndRecord(el, el._attrs.poster, 'video');
      // `preload="none"` opts the media data out until play — a browser fetches neither `src` nor a
      // `<source>` then (the poster and a showing track still load).
      const src = el._attrs.preload === 'none' ? null : (el._attrs.src || firstMediaSourceSrc(el));
      if (src) mediaFetchAndRecord(el, src, tag);
      const kids = el._children;
      if (kids) for (const c of kids) {
        if (c._tag === 'track' && c._attrs && c._attrs.src && c._attrs.default != null) mediaFetchAndRecord(c, c._attrs.src, 'track');
      }
    } else if (tag === 'embed') {
      if (el._attrs.src) mediaFetchAndRecord(el, el._attrs.src, 'embed');
    } else if (tag === 'object') {
      if (el._attrs.data) mediaFetchAndRecord(el, el._attrs.data, 'object');
    }
  }
  // An SVG `<use href>` that points at an EXTERNAL document (not a same-document `#id` reference)
  // fetches it — a browser files that fetch as 'other' (resource-timing/initiator-type/svg). The
  // driver doesn't render the referenced tree; it fetches for the timing entry, like the media path.
  // The Fetch that mints the entry strips the fragment, so `dummy.svg#id` is timed as `dummy.svg`.
  function recordUseResourceTiming(el) {
    const attrs = el._attrs || {};
    // `href` wins over `xlink:href`, but an empty `href` defers to `xlink:href` — the same
    // precedence the SVG `<image>` path uses (dom-nodes.js `_imageResourceSrc`).
    const raw = attrs.href != null && attrs.href !== '' ? attrs.href : attrs['xlink:href'];
    if (raw == null) return;
    const s = String(raw).trim();
    if (s === '' || s[0] === '#') return;   // a same-document reference makes no request
    let abs;
    try { abs = new globalThis.URL(s, el.baseURI || (globalThis.document && globalThis.document.baseURI) || undefined).href; } catch (_) { return; }
    const hash = abs.indexOf('#');
    if (hash >= 0) abs = abs.slice(0, hash);
    mediaFetchAndRecord(el, abs, 'other');
  }
  function connectOneElement(el, selects, perOptionSelects) {
    ...
    } else if (el._tag === 'video' || el._tag === 'audio' || el._tag === 'embed' || el._tag === 'object') {
      // A media / plugin element's resource is fetched when it is connected — the driver does not
      // decode or play it, but a browser files its Resource Timing entry, which perf-monitoring
      // reads. `<video>`/`<audio>`: the poster (video), the `src` or first `<source>`, and each
      // showing `<track>`; `<embed src>` / `<object data>`: the one resource.
      recordMediaResourceTiming(el);
    } else if (el._tag === 'use') {
      // An SVG `<use>` referencing an external document fetches it (initiatorType 'other').
      recordUseResourceTiming(el);
    }
    ...
    // A connected CSS `<style>` fires `load` once its sheet is ready (an @import block is
    // trickle-delayed until the import is fetched + applied; a plain block fires async).
    else if (el._tag === 'style') maybeFireStyleLoad(el);
    // A CONTROLLED document's inline-style background image FETCHES (destination
    // 'image', response discarded — we don't paint, but the fetch is the
    // observable: fetch-destination-no-load-event). Gated hardest-first: the rare
    // `style` attribute with a url() before any host/controller call; uncontrolled
    // documents never fetch (rule 3 — no new cost for the app suites).
    maybeFetchInlineBackgroundImage(el);
    // A CONTROLLED document's SVG <use> external reference fetches (destination
    // 'image', discarded — whatwg/fetch#1012; same shape as the background-image
    // observable above).
    if (el._tag === 'use') maybeFetchSvgUseImage(el);
  ...
  function fireCEConnect(subtree) {
    ...
    for (const sel of connectedSelects) {
      if (!perOptionSelects.has(sel)) runSelectednessAlgorithm(sel, null);
      // A RE-connected customizable select (moved / re-inserted) re-clones its
      // selectedcontent per spec — a tree mutation replaces the children with fresh
      // clones even when the serialized content is unchanged (selectedcontent-
      // mutations test8). First connection (parse build) reconciles idempotently.
      if (sel._hasSelectedContent && sel._everConnected) updateSelectedContent(sel, true);
      sel._everConnected = true;
    }
  }
  ...
  function drainStreamConnect() {
    ...
    try {
      for (let i = 0; i < pending.length; i++) {
        const el = pending[i];
        connectOneElement(el, connectedSelects, perOptionSelects);
        // …and its SHADOW tree. The parser queues each LIGHT node it inserts, so the light side
        // needs no descent here — but it never queues a shadow node, and a custom element's
        // constructor runs at its START TAG, which is where `attachShadow` usually happens. So a
        // shadow tree built during parsing was never connected at all: a nested custom element
        // inside it stayed un-upgraded for the life of the page (`::part` forwarding tests, where
        // an outer element's template holds an inner one), and a shadow `<script>` never ran.
        // `fireCEConnect` has always used the shadow-including walk for exactly this; this is the
        // parser's incremental cursor agreeing with it.
        if (el._shadowRoot) {
          walkInclShadow(el._shadowRoot, m => connectOneElement(m, connectedSelects, perOptionSelects));
        }
      }
    } finally {
      globalThis.__csimConnectWalkDepth--;
  ...
  const __streamHooks = {
    onInsert(parent, node) {
      ...
      // `recordChildList` also drives the SLOT hooks — but only while an
      // observer exists, and slotchange is independent of MutationObserver, so
      // the no-observer case runs the same hook directly. A parsed child can
      // land under a host that already has a shadow root now that a custom
      // element's constructor runs at its start tag (attachShadow's usual
      // home); the hook self-gates on shadowHostCount, so a shadow-free page
      // pays one call (rule 3).
      if (hasObservers()) recordChildList(parent, [node], []);
      else { signalSlotChildList(parent, [node], []); noteParsedChange(parent, node); }
      ...
      if (__sawSelectedContent && (node._tag === 'option' || node._tag === 'selectedcontent')) {
        ...
        // A <select multiple> has no implicit selection and no incremental
        // selectedness to track, so its selectedcontent reconcile is DEFERRED to
        // the select's end tag (onPop below) — the authored content is replaced
        // once, after all options, not per-option (selectedcontent-mutations test6).
        if (sel && sel._hasSelectedContent && (node._tag === 'option' || sel.querySelector('option')) &&
            sel._attrs.multiple == null) {
          runSelectednessAlgorithm(sel, null);
        }
      }
    },
    // Is this `<script>` a parser-blocking one with a SOURCE — the kind the parser waits on as the pending
    // parsing-blocking script, rather than running it where it stands (parse5-adapter.js, `document.write`)?
    blocksParser(scriptEl) {
      return scriptEl._attrs.src != null && isParserBlockingScript(scriptEl);
    },
    onRemove(parent, node) {
      if (hasObservers()) recordChildList(parent, [], [node]);
      else noteParsedChange(parent, null, node);
    },
    // Text the parser COALESCED into an existing text node — a characterData change no observer is sent (the
    // streaming cluster watches childList only), but layout and a `dir=auto` scope around it still see it. Not for
    // inert template content, as for an insertion.
    onTextAppend(parent) {
      if (parent.__csimTemplateContent === true || parent._inStreamTC === true) return;
      noteParsedChange(parent);
    },
    ...
    onPop(node) {
      ...
      if (__sawSelectedContent && node._tag === 'option') {
        const sel = ownerSelectOf(node);
        // A <select multiple> defers its reconcile to its own end tag (below).
        if (sel && sel._hasSelectedContent && sel._attrs.multiple == null) updateSelectedContent(sel);
      }
      // A customizable <select multiple> closed its end tag: its selection is now
      // final (no implicit default), so reconcile the selectedcontent ONCE here —
      // after the select's content — replacing the authored content (empty when
      // nothing is selected). selectedcontent-mutations test6.
      if (__sawSelectedContent && node._tag === 'select' && node._attrs.multiple != null && node._hasSelectedContent) {
        finalizeSelectOptions(node);
      }
    ...
    onScript(scriptEl) {
      ...
      // Connect + upgrade everything appended up to this script (grafting still on) — with the markup-insertion counter
      // held, as for a constructor the parser runs at a start tag: an upgrade's `document.write` throws (Chrome), where it
      // wrote at the insertion point of the script about to run.
      const doc = globalThis.document;
      doc._markupInsertionCounter = (doc._markupInsertionCounter || 0) + 1;
      try { drainStreamConnect(); } finally { doc._markupInsertionCounter--; }
  ...
  // Resource Timing for a body the asset cache served: the host keeps the response's facts
  // beside the cached body (`__csimExternalAssetMeta`); a body with no facts (a service
  // worker's, a cache from before this ran) sizes the entry by its text.
  function assetTimingFields(url, body) {
    const meta = /^https?:/i.test(String(url)) && typeof globalThis.__csimExternalAssetMeta === 'function' ? globalThis.__csimExternalAssetMeta(url) : null;
    if (meta && typeof meta === 'object') return { resp: meta };
    return { body: body != null ? body : undefined, status: body != null ? 200 : undefined };
  }
  globalThis.__csimAssetTimingFields = assetTimingFields;
  globalThis.__csimReloadFrameByRealm = function (realmId) {
    const el = findFrameByRealmId(realmId);
    if (!el) return false;
    // A reload re-navigates to the document's CURRENT URL — a fragment set by script, a
    // `location.href` navigation — not the `src` attribute (a page that sets `location.hash`
    // and reloads in its load handler would otherwise reload its src forever).
    try {
      const href = el.contentWindow && el.contentWindow.location && String(el.contentWindow.location.href);
      el._frameReloadUrl = href && /^https?:/i.test(href) ? href : null;
    } catch (_) { el._frameReloadUrl = null; }
  ...
  // The base URL an iframe's `src` resolves against (HTML "parse a URL" uses the
  // node document's BASE URL, not location.href): for an about:blank / srcdoc frame
  // the two differ — its base is INHERITED from the parent (documentBaseURL), so a
  // child of an about:blank frame still resolves relative URLs against the real
  // origin. Shared by frameEagerEligible's same-origin check and the frame build.
  function frameSrcResolveBase() {
    return (globalThis.document && globalThis.document.baseURI) ||
  ...
  function frameEagerEligible(el) {
    ...
    try {
      const base = frameSrcResolveBase();
  ...
  // Fetch-and-discard a connected element's INLINE-style background image through
  // the document's controller (destination 'image'), once per URL per realm. Only
  // the inline `style` attribute is scanned — a SHEET's background images ride the
  // sheet's own (SW-routed) fetch, and painting isn't modeled either way.
  // Fetch-and-discard a CONTROLLED document's render-only subresource (inline-style
  // background image, SVG <use> reference, <link rel=prefetch>): nothing is painted,
  // the FETCH is the observable (fetch-destination-no-load-event). Once per URL per
  // realm; an uncontrolled document bails on the controller check.
  const __swDiscardFetched = new Set();
  function swDiscardFetch(el, rawUrl, destination, stripFragment) {
    const swCtrl = (typeof globalThis.__csimSWControllerHandle === 'function') ? (globalThis.__csimSWControllerHandle() | 0) : 0;
    if (!(swCtrl > 0)) return;
    let abs = rawUrl;
    try { abs = new globalThis.URL(rawUrl, el.baseURI || (globalThis.location && globalThis.location.href) || undefined).href; } catch (_) {}
    if (stripFragment) abs = abs.split('#')[0];   // the fragment names the piece, not the resource
    if (!/^https?:/i.test(abs) || __swDiscardFetched.has(abs)) return;
    __swDiscardFetched.add(abs);
    try {
      const swReq = buildSwRequest({ mode: 'no-cors', credentials: 'include', destination });
      globalThis.__csimSWInterceptFetch(swCtrl, 'GET', abs, {}, null, false, swReq, function () {});
    } catch (_) {}
  }
  function maybeFetchInlineBackgroundImage(el) {
    const st = el._attrs && el._attrs.style;
    if (!st || st.indexOf('url(') === -1 || !/background/i.test(st)) return;
    const m = /url\(\s*(['"]?)([^'")]+)\1\s*\)/i.exec(st);
    if (m) swDiscardFetch(el, m[2], 'image', false);
  }
  function maybeFetchSvgUseImage(el) {
    const href = (el._attrs && (el._attrs.href || el._attrs['xlink:href'])) || '';
    if (!href || href.charAt(0) === '#') return;   // local reference — no fetch
    swDiscardFetch(el, href, 'image', true);
  }
  function maybeFireFrameLoad(el) {
    ...
    const fireElementLoad = () => {
      ...
      const ev = new Event('load');
      // A `src` reassigned by this very handler must not rebuild the frame on THIS stack
      // (dom-nodes.js `_renavigateFrameDocument` defers it to a task while the flag is up):
      // a navigation is a task in HTML, and a handler that navigates its own frame on every
      // load — `iframe.src = 'about:blank'` in `onload`, 50 loads in 1.5 s in Chrome — recursed
      // here through a realm build per level until the process spent 3 minutes on one page.
      el._frameLoadDispatching = true;
      try { el.dispatchEvent(ev); } catch (_) {}
      el._frameLoadDispatching = false;
    ...
    if (canBuild) {
      ...
      // already touched contentWindow), loads + runs the nested document, and fires
      // its window `load` (in create_frame_realm). Discard the returned cross-realm
      // global so no handle to it lingers in this task's scope.
      const buildFrame = () => {
        if (!isConnected(el)) return;
        // Already materialised (the page touched `contentWindow` / `contentDocument`
        // during parsing): there is nothing for the parent's `load` to wait for, and
        // reaching into a frame that already has a document in flight is how a
        // planned navigation got lost (reparent-form-during-planned-navigation-task).
        if (el._frameRealmId != null || el._frameWindow) return;
        try { if (globalThis.__csimFrameWindow) globalThis.__csimFrameWindow(el); } catch (_) {}
      };
      // The parent document's `load` waits for its FRAMES' DOCUMENTS: a load handler
      // reads `iframe.contentWindow.document` (form-data-set-usv, and the
      // beforeunload-coercion test navigates one). Only the build is gated — the
      // ELEMENT's own load event stays on this task, where the page's other
      // navigation machinery expects it.
      const gate = subresourceGate(el, buildFrame);
      setTimeout(() => {
        gate();
        // The build parked on a still-activating service worker (Handle Fetch waits
        // for 'activated'): no load yet — __csim_swRetryDeferredNavs re-runs the
        // whole build+load when the activation settles.
        if (el._frameNavDeferredSw) return;
  ...
  // A CONTROLLED document's `<link rel=prefetch>` fetches through its service
  // worker — destination '' (a prefetch is a plain resource hint, the Request
  // default), mode no-cors — and fires `load`/`error` from the SW's answer (a
  // `respondWith(Response.error())` errors the hint, like a network failure).
  // Returns false when uncontrolled so the caller's network branch runs instead.
  function prefetchViaSw(el, abs) {
    const swCtrl = (typeof globalThis.__csimSWControllerHandle === 'function') ? (globalThis.__csimSWControllerHandle() | 0) : 0;
    if (!(swCtrl > 0)) return false;
    try {
      const swReq = buildSwRequest({ mode: 'no-cors', credentials: 'include', destination: '' });
      globalThis.__csimSWInterceptFetch(swCtrl, 'GET', abs, {}, null, false, swReq, function (resp) {
        let ok;
        if (resp == null) {
          // fallthrough — the network answers, like the uncontrolled branch
          try { const r = __rackFetch('GET', abs, '', null, 'follow'); ok = !!(r && r.status < 400); } catch (_) { ok = false; }
        } else if (resp.__networkError) {
          ok = false;
        } else {
          const status = (resp.status || 200) | 0;
          ok = resp.type === 'opaque' || (status >= 200 && status < 400);
        }
  ...
  // The subresource work a document's `load` event WAITS FOR: its style sheets and
  // its frames. Each is queued as a task here, so without this the window's `load`
  // could beat a `<style>`'s (css/cssom/HTMLStyleElement-load-event) or land before
  // a frame's document exists at all (form-data-set-usv reads
  // `iframe.contentWindow.document` from its load handler).
  //
  // Flushed by `__csimFireWindowLoad` rather than by draining the timer queue,
  // which would advance the driver's lazy clock past a DELAYED `setTimeout` the
  // page queued while parsing — one a test may still want to observe the state
  // before. (The zero-delay ones do run with the load, as they do in a browser;
  // the Ruby side drains them right after this fires — `flush_page_init`.)
  const __pendingSubresources = new globalThis.Set();
  // Register work the document's `load` must not precede, and return the thunk that
  // performs it. Runs at most once, whoever gets there first: the caller's own task,
  // or the flush below.
  function subresourceGate(el, run, delayed) {
    const gate = () => { if (__pendingSubresources.delete(gate)) run(); };
    gate.el = el;
    // A resource wptserve was asked to TRICKLE is late on purpose — the point of
    // `?pipe=trickle(dN)` is to be observed not-yet-loaded — so the flush leaves it
    // to its own timer rather than collapsing the delay it exists to demonstrate.
    gate.delayed = !!delayed;
    __pendingSubresources.add(gate);
    return gate;
  }
  // …and the common case, where the work IS a task of its own.
  function subresourceLoadTask(el, run, delay) {
    globalThis.setTimeout(subresourceGate(el, run, delay > 0), delay);
  }
  // Bounded: flushing one can queue another (a frame's document brings its own
  // sheets), and a document that keeps queuing them would otherwise never load.
  //
  // Deliberately NO microtask checkpoint here. A subresource's own `load` event is
  // dispatched from one, so draining would bring those forward too — and running a
  // frame's element `load` inside the window's load dispatch loses a navigation the
  // page had already planned (WPT reparent-form-during-planned-navigation-task).
  // The residual: an `iframe.onload` installed BY a window-load handler can still
  // catch that frame's initial element load, which a browser fired earlier.
  function flushPendingSubresources() {
    // THIS document's subresources only. An element a script built inside another
    // document (a `newRenderedHTMLDocument` test context, a DOMParser result) has
    // its own load to wait for, not ours — forcing its task here reached a frame a
    // browser hasn't built yet (shadow-dom inert-html-elements).
    const mine = (task) => !task.delayed && (!task.el || task.el.ownerDocument === globalThis.document);
    for (let round = 0; round < 8; round++) {
      const due = Array.from(__pendingSubresources).filter(mine);
      if (!due.length) break;
      for (const task of due) { try { task(); } catch (_) {} }
    }
  }

  function maybeFireLinkLoad(el) {
    // A `<link disabled>` (a disabled stylesheet link) obtains no resource — no fetch, no
    // `load` — until the `disabled` attribute is removed (which re-enters here). See the
    // HTMLLinkElement.disabled model in dom-nodes.js.
    if (el._attrs.disabled != null) return;
    // Register the link's crossorigin params for the style-fetch memo BEFORE any
    // consumer (cascade / .sheet / the load task below) dispatches the single
    // SW fetch for this URL.
    if ((el._attrs.rel || '').toLowerCase().split(/\s+/).includes('stylesheet') && el._attrs.href) {
      try {
        const a = new globalThis.URL(String(el._attrs.href), el.baseURI || (globalThis.location && globalThis.location.href) || undefined).href;
        __swStyleCors.set(a, swCorsOf(el));
        if (el._attrs.integrity) __swStyleIntegrity.set(a, el._attrs.integrity);
      } catch (_) {}
    }
    const rel = (el._attrs.rel || '').toLowerCase().split(/\s+/);
    const isStylesheet = rel.includes('stylesheet');
    const isPrefetch   = !isStylesheet && rel.includes('prefetch');
    if (!isStylesheet && !isPrefetch && !rel.includes('modulepreload') && !rel.includes('preload') && !rel.includes('manifest')) return;
    ...
    subresourceLoadTask(el, () => {
      if (!isConnected(el)) return;
      let ok = true;
      // A `data:` URL carries its own body — the Rack fetcher only knows http(s), so route it
      // like the cascade does (decodeDataUrlCss): a well-formed `data:` stylesheet always loads.
      // Only a real network resource goes through __rackFetch (a 4xx/throw → `error`).
      if (/^data:/i.test(href)) {
        ok = isStylesheet ? decodeDataUrlCss(href) != null : true;
      } else {
        // Resolve against THIS document's base (the raw attribute resolves against
        // the MAIN document Ruby-side — wrong directory for a subframe's relative
        // href; same class as the script srcKey fix), then let a controlling
        // service worker answer first (destination 'style' — one dispatch per URL,
        // shared with the cascade/.sheet consumers via the memo).
        let abs = String(href);
        try { abs = new globalThis.URL(abs, el.baseURI || (globalThis.location && globalThis.location.href) || undefined).href; } catch (_) {}
        let swMp = null, linkResp = null;
        const timingStart = globalThis.performance.now();
        const sw = isStylesheet ? swFetchStyle(abs) : null;
        if (sw) {
          ok = !sw.blocked && sw.body != null;
        } else if (rel.includes('modulepreload') &&
                   (swMp = swFetchSubresource(abs, 'script', { mode: 'cors', credentials: moduleCredentialsOf(el) }, false,
                                              el._attrs.integrity || importmapIntegrityFor(abs))) != null) {
          // A CONTROLLED document's modulepreload dispatches its fetch event on
          // the sync channel (destination 'script', mode 'cors', integrity from
          // the attribute else the import map); uncontrolled (null) falls through
          // to the network branch below.
          ok = !swMp.blocked && swMp.body != null;
        } else if (isPrefetch && prefetchViaSw(el, abs)) {
          // A controlling SW owns the load/error dispatch. UNCONTROLLED prefetch (prefetchViaSw
          // returned false) falls through to the network branch — a real low-priority fetch that
          // files a 'link' entry, as a browser prefetches a route hint.
          return;
        } else {
          try {
            linkResp = __rackFetch('GET', abs, '', null, 'follow');
            ok = !!(linkResp && linkResp.status < 400);
          } catch (_) { ok = false; }
        }
        // Resource Timing: one `link` entry per load (the load task can run twice for one
        // element: on connection and on the attribute change that completed it), sized by the
        // response (or the body a service worker answered with); a blocked / failed load is a
        // network error.
        const swServed = sw || swMp;
        // `modulepreload` files its entry as 'other' (a script fetched but not yet run — Resource
        // Timing L2); every other `<link>` load is 'link'.
        const linkInitiator = rel.includes('modulepreload') ? 'other' : 'link';
        if (el._csimTimingRecorded !== abs) el._csimTimingRecorded = abs, globalThis.__csimRecordResource({ name: abs, initiatorType: linkInitiator, startTime: timingStart, resp: linkResp,
                                          noCors: el._attrs.crossorigin == null,
                                          body: swServed && !swServed.blocked && swServed.body != null ? swServed.body : undefined,
                                          status: swServed && !swServed.blocked && swServed.body != null ? 200 : undefined });
      }
  ...
  // A connected CSS `<style>` fires `load` once its sheet is ready (HTML "update a
  // style block"). Both flavours fire on a TASK, never a microtask, so the load never
  // lands mid-parse: an `@import` block is deferred by the import URL's `trickle` delay
  // (so render-blocking "is cancellable" sees it not-yet-loaded within a frame, then
  // eventually loaded) and fetches + applies the import first; a plain block has no
  // critical subresource and fires on the next task. (We don't model the `@import`-
  // failure `error`.)
  function maybeFireStyleLoad(el) {
    if (el._styleLoadFired) return;
    // A `<style>` fires `load` once its sheet is ready (HTML "update a style block" → the
    // subresource-obtaining steps queue a load task) — for EVERY CSS `<style>`, not only
    // `@import` ones. A non-CSS type has no sheet, so no load event.
    if (!styleElementIsCss(el)) return;
    const txt       = el.textContent || '';
    const hasImport = /@import/i.test(txt);
    el._styleLoadFired = true;
    subresourceLoadTask(el, () => {
      if (!isConnected(el)) return;
      // Fetch + apply an @import before the load fires; a plain block has no critical
      // subresource, so it just fires the (async) load task.
      if (hasImport) { try { rebuildCascade(globalThis.document); } catch (_) {} }
      try { el.dispatchEvent(new Event('load')); } catch (_) {}
    }, hasImport ? trickleDelayMs(txt) : 0);
  }
  let __initialScriptsDone = false;
  // ONE window `load` per document load. Several places can be the one to fire it
  // — the driver for the page it just booted, the realm builder for a frame or an
  // auxiliary window, the WPT harness for the file it is running — and which one
  // gets there first depends on how the document was created. Whoever arrives
  // first fires; the rest are no-ops, so a page never sees the event twice.
  let __windowLoadFired = false;
  ...
  function dispatchScriptLoad(el, ok) {
    if (el._attrs.src == null) return;   // (an EMPTY src is a script that fires its `error` — `failsOnEmptySrc`)
  ...
  function runDeferredExternalScript(el, body) {
    ...
    try { __csim_runScript(el._attrs.src, body); }
      ...
      try { globalThis.reportError(scriptErrorOf(e)); } catch (_) { logThrew('dynamic script', e); }
    } finally {
  ...
  // A controlled document's external classic script routed through its service
  // worker (destination 'script'), synchronously — shared by the parse-time
  // (runOneScript) and dynamic-insertion (maybeRunScript) paths. Returns
  // {body} for a served script, {blocked: true} for a networkError respondWith,
  // null for no interception / fallthrough (→ the plain network fetch).
  // The element's `crossorigin` attribute → the request's mode/credentials (HTML
  // "fetch a classic script" / "obtain a stylesheet"; same mapping as the <img>
  // path's _imageCorsRequest).
  function swCorsOf(el) {
    const co = el && el._attrs ? el._attrs.crossorigin : undefined;
    if (co === undefined || co === null) return { mode: 'no-cors', credentials: 'include' };
    return String(co).toLowerCase() === 'use-credentials'
      ? { mode: 'cors', credentials: 'include' }
      : { mode: 'cors', credentials: 'same-origin' };
  }
  // This realm's SW-client identity: the adopted reserved id when the document
  // has one, else the realm-derived fallback (the same identity every controlled
  // fetch in this realm reports as event.clientId).
  function swClientIdOf() {
    let cid = globalThis.__csimClientId;
    if (!cid) {
      try {
        const NS  = globalThis.RustyRacer;
        const rid = (NS && typeof NS.contextOf === 'function') ? (NS.contextOf(globalThis) | 0) : 0;
        cid = rid > 0 ? ('client-' + rid) : 'client-window';
      } catch (_) { cid = 'client-window'; }
    }
    return cid;
  }
  function swFetchSubresource(absUrl, destination, cors, binary, integrity) {
    const swCtrl = (typeof globalThis.__csimSWControllerHandle === 'function') ? (globalThis.__csimSWControllerHandle() | 0) : 0;
    if (!(swCtrl > 0) || !/^https?:/i.test(absUrl)) return null;
    const c = cors || { mode: 'no-cors', credentials: 'include' };
    try { return globalThis.__csim_swScriptFetch(swCtrl, absUrl, swClientIdOf(), (globalThis.location && globalThis.location.href) || '', destination, c.mode, c.credentials, binary === true, integrity || '') || null; } catch (_) { return null; }
  }
  // Import-map "integrity" metadata for a RESOLVED module URL ('' when unmapped).
  function importmapIntegrityFor(absUrl) {
    const im = globalThis.__csim_importmap;
    return (im && im.integrity && im.integrity[absUrl]) || '';
  }
  // A module script's credentials mode: crossorigin absent/anonymous -> same-origin,
  // use-credentials -> include (mode is always 'cors').
  function moduleCredentialsOf(el) {
    const co = el && el._attrs && el._attrs.crossorigin;
    return String(co).toLowerCase() === 'use-credentials' ? 'include' : 'same-origin';
  }
  // `integrity` rides to `event.request.integrity` VERBATIM, valid or not (the
  // spec parses it only at verification time; the request carries the raw attribute).
  function swFetchScript(absUrl, el) { return swFetchSubresource(absUrl, 'script', swCorsOf(el), false, (el._attrs && el._attrs.integrity) || ''); }
  // A STYLESHEET fetch dispatches its fetch event exactly ONCE per URL per realm:
  // three consumers fetch a link's CSS independently (the load/error decision here,
  // the cascade's parseUrlSheetCached, the CSSOM `.sheet` getter) — each hitting
  // the SW would triple-count `event.request` where a browser fetches once
  // (static-router-request-destination's requests.length). The memo also hands all
  // three the SAME served body. The `<link>`'s crossorigin params are REGISTERED
  // at connect time (maybeFireLinkLoad, synchronously — before any consumer
  // fetches) so the single dispatch carries the element's mode/credentials even
  // when a URL-only consumer (the cascade) gets there first. Per-VM lifetime.
  const __swStyleMemo = new Map();
  const __swStyleCors = new Map();
  const __swStyleIntegrity = new Map();
  function swFetchStyle(absUrl) {
    // The controller gate runs BEFORE the memo: an UNCONTROLLED epoch must not
    // pin a null verdict a later clients.claim() can never overturn (a sheet
    // fetched pre-claim would otherwise stay invisible to the SW forever).
    const ctrl = (typeof globalThis.__csimSWControllerHandle === 'function') ? (globalThis.__csimSWControllerHandle() | 0) : 0;
    if (!(ctrl > 0)) return null;
    if (__swStyleMemo.has(absUrl)) return __swStyleMemo.get(absUrl);
    const r = swFetchSubresource(absUrl, 'style', __swStyleCors.get(absUrl), false, __swStyleIntegrity.get(absUrl) || '');
    __swStyleMemo.set(absUrl, r);
    return r;
  }
  globalThis.__csimSwFetchStyle = swFetchStyle;
  // Generic per-destination synchronous SW subresource fetch for modules outside
  // this one (video bytes, fonts): {body}|{body_b64 (binary)}|{blocked}|null.
  globalThis.__csimSwFetchDest = function (absUrl, destination, mode, credentials, binary) {
    return swFetchSubresource(absUrl, destination, { mode: mode, credentials: credentials }, binary);
  };
  // Element-aware cors registration for the style memo — called by whichever
  // consumer sees the `<link>` element (the cascade collector, the connect
  // path, the href-assign hook). LAST registration wins — see the body comment.
  globalThis.__csimSwRegisterStyleCors = function (absUrl, el) {
    // LAST-wins: the href-assignment hook re-enters maybeFireLinkLoad on a still-
    // DETACHED link whose crossorigin is not yet assigned (load_css sets it after
    // href) — the connect-time re-registration with the complete attributes must
    // replace that early snapshot. All registrations precede the first consumer
    // fetch, so the memoized dispatch reads the final params.
    __swStyleCors.set(absUrl, swCorsOf(el));
  };
  // A `src` that is present but EMPTY is a failed load, not an inline script (HTML "prepare the script element":
  // queue a task to fire `error` and stop) — it ran the element's text, and fired nothing. True when it handled `el`.
  function failsOnEmptySrc(el) {
    if (el._attrs.src !== '') return false;
    el._csimRan = true;
    el._csimExecuted = true;
    globalThis.setTimeout(() => dispatchScriptLoad(el, false), 0);
    return true;
  }
  function maybeRunScript(el) {
    const type = (el._attrs.type || '').toLowerCase();
    // A dynamically-appended MODULE script loads + evaluates its graph on a later
    // task (modules are always async) and fires load/error on the element —
    // previously nothing ran it at all, so `script.onload` on a dynamic
    // `<script type=module src>` hung forever (fetch-destination's import cases).
    if (type === 'module') {
      if (el._csimRan) return;
      if (failsOnEmptySrc(el)) return;
      // An EMPTY inline module is not yet prepared (same as the classic path):
      // text appended later re-enters here via the children-changed hook.
      if (!el._attrs.src && !scriptText(el)) return;
      el._csimRan = true;
      el._csimExecuted = true;
      globalThis.setTimeout(() => {
        // No isConnected gate: a subsequent removeChild can't cancel an
        // already-started script (same rule the classic path documents below).
        try { runModuleScript(el); } catch (_) { dispatchScriptLoad(el, false); }
      }, 0);
      return;
    }
    // Same gate as the initial parse-time scripts: classic only. Inline scripts in
    ...
    if (el._attrs.nomodule != null) return;   // `nomodule` classic: not run where modules are (as in `runOneScript`)
    if (failsOnEmptySrc(el)) return;
    if (el._attrs.src) {
      ...
      // "Already started" once preparation begins — and not the after-parse pass's either (`_csimExecuted`): only the
      // PARSER's scripts belong to "the end". A loader snippet's injected script ran on insertion and AGAIN there.
      el._csimRan = true;
      el._csimExecuted = true;
      let ok, body, recordTiming = null;
      if (/^data:/i.test(String(el._attrs.src))) {
        ...
        // Resolve the src against THIS document's base — the raw attribute would
        // resolve against the MAIN document Ruby-side (wrong directory for a
        // subframe's relative src; the same class as runOneScript's srcKey fix).
        let abs = String(el._attrs.src);
        try { abs = new globalThis.URL(abs, el.baseURI || (globalThis.location && globalThis.location.href) || undefined).href; } catch (_) {}
        // A controlled document's dynamic script is SW-intercepted like the
        // parse-time one (destination 'script').
        const timingStart = globalThis.performance.now();
        const sw = swFetchScript(abs, el);
        let resp = null;
        if (sw && sw.blocked) {
          ok = false; body = '';
        } else if (sw && sw.body != null) {
          ok = true; body = sw.body;
        } else {
          try { resp = __rackFetch('GET', abs, '', null, 'follow'); }
          catch (_) { resp = null; }
          ok   = !!(resp && resp.status < 400);
          body = ok ? (resp.body || '') : '';
        }
        // (Once per element: a script appended while the parser is still running is fetched
        // here AND by the parser's connect pass.) The entry is filed when the load COMPLETES — on the task below, as the
        // fetch finishing is what a browser reports — so a PerformanceObserver set up right after the `src` assignment
        // sees it (resource-timing/cross-origin-status-codes timed out waiting).
        if (!el._csimTimingRecorded) {
          el._csimTimingRecorded = true;
          recordTiming = () => globalThis.__csimRecordResource({
            name: abs, initiatorType: 'script', startTime: timingStart, resp,
            noCors: el._attrs.crossorigin == null,
            body: sw && !sw.blocked && sw.body != null ? sw.body : undefined,
            status: sw && !sw.blocked && sw.body != null ? 200 : undefined
          });
        }
      }
    ...
    if (!body) return;
    el._csimRan = true;
    el._csimExecuted = true;   // (see the external path above)
  ...
  // The events that PREPARE a connected, not-already-started, non-parser-inserted <script> (HTML "prepare the
  // script element" triggers): its children changed — `s.appendChild(textWithCode)` or setting its `text` /
  // `textContent` runs `s` synchronously (Node-appendChild-text-in-script / three-scripts / script-in-script) — and a
  // `src` attribute set where it had none. The DOM primitives call this after either. Gated like maybeRunScript: not
  // during the initial parse graft (runInlineScripts owns that ordering).
  globalThis.__csimPrepareScript = function (el) {
  ...
  installWindowForwardedHandlers();   // window.on{blur,error,…} ↔ winStore (element side: installEventHandlerAttrs)
  ...
  // `document.implementation.createHTMLDocument('').open()` (which works —
  // Document#open is implemented — but is the slow path). Providing native
  // DOMParser keeps Turndown on its fast path.
  ...
  // Every `sandbox` directive a CSP header value declares, as a list of token lists — one
  // entry per policy that declares one (`''` for a bare `sandbox`, the FULL set of
  // restrictions, granting nothing). A header carries comma-separated POLICIES, each a
  // `;`-separated directive list, and ALL of them apply, so the caller must satisfy every
  // entry. Within one policy only the FIRST occurrence of a directive counts.
  function cspSandboxDirectives(header) {
    const found = [];
    for (const policy of String(header).split(',')) {
      for (const directive of policy.split(';')) {
        const parts = directive.trim().split(/\s+/);
        if (parts[0] && parts[0].toLowerCase() === 'sandbox') { found.push(parts.slice(1).join(' ')); break; }
      }
    }
    return found;
  }
  // Whether a sandbox token list grants `allow-same-origin`.
  function allowsSameOrigin(tokens) {
    return /(^|\s)allow-same-origin(\s|$)/i.test(tokens);
  }
  ...
  globalThis.__csimFrameWindow = function (frameEl) {
    ...
    // Any build that gets past the SW-activation defer below clears the shelved
    // marker — a deferred frame whose src is REASSIGNED (re-navigation to an
    // out-of-scope URL, or to srcdoc) must not keep suppressing its element
    // `load` in the connect path. The defer branch re-sets it.
    frameEl._frameNavDeferredSw = false;
    // Whether THIS build is a reload navigation (set by reloadFrame() for location.reload() /
    // history.go(0)). Captured + cleared unconditionally so it can't leak to a later fresh load,
    // and read by the SW navigation-interception branch below.
    const isReloadNav = !!frameEl._frameReloadNav;
    frameEl._frameReloadNav = false;
    // …and the document URL a reload keeps (`__csimReloadFrameByRealm`), likewise consumed
    // here whatever branch this build takes — left on the element it would hijack the frame's
    // NEXT `src` navigation (a form-target frame reloaded, then re-pointed).
    const reloadUrl = isReloadNav && frameEl._frameReloadUrl ? frameEl._frameReloadUrl : null;
    frameEl._frameReloadUrl = null;
    try {

    let body = '', contentType = 'text/html', frameUrl = '', frameCharset = null, frameHttpCharset = null, jsUrlSource = null;
    // The navigation's reserved client id (`event.resultingClientId`), set when the
    // src-fetch chain below served the document — the new realm adopts it.
    let frameClientId = null;
    // The `sandbox` directives a `Content-Security-Policy` RESPONSE header applied to the
    // frame's navigation (empty when it carried none). They sandbox the frame's document
    // exactly as the container's `sandbox` attribute does — the flags simply arrive with the
    // response rather than the markup, which is why the navigation request itself still
    // reached the network (or a service worker) first.
    let frameCspSandbox = [];
    ...
    if (navContent) {
      ...
      const src = reloadUrl || frameEl.getAttribute('src');   // a reload keeps the document's current URL
      if (src && src !== 'about:blank') {
        try {
          frameUrl = new URL(src, frameSrcResolveBase()).href;   // the frame document's own URL (incl. #fragment, for :target)
          if (/^data:/i.test(frameUrl)) {
            ...
            // ONE host call owns the whole navigation chain: per-hop SW interception
            // across every window's registrations (mode 'navigate'; a redirect out of
            // one SW's scope re-enters interception at the new URL) + network fallback.
            // Returns __rackFetch's response wire shape, or null for a failed
            // navigation (respondWith(Response.error()) / redirect loop) → empty
            // about:blank-ish document. The navigation's referrer is the INITIATING
            // document — this realm builds the frame, so its own location is the
            // parent/initiator URL (the host resolves it per policy).
            const navReferrer = (globalThis.location && globalThis.location.href) || '';
            // Trailing `true` = this caller may be DEFERRED: a navigation into a scope
            // whose service worker is still 'activating' parks (Handle Fetch waits for
            // 'activated') instead of dispatching — the host answers {deferred} and the
            // activated marker replays the build via __csim_swRetryDeferredNavs.
            // A frameset-era <frame> navigates with destination 'frame' (its own
            // Request.destination / Sec-Fetch-Dest value); <iframe> with 'iframe'.
            const frameTimingStart = globalThis.performance.now();
            const resp = globalThis.__csim_frameNavigationFetch(frameUrl, isReloadNav, navReferrer, __csimSecureAncestorChain(), 'GET', '', null, true, frameEl._tag === 'frame' ? 'frame' : 'iframe');
            if (resp && resp.deferred) {
              // Shelve the element with NO realm built and no load event — the frame
              // simply hasn't finished navigating. `finally` resets _frameBuilding, so
              // the retry (or any contentWindow touch meanwhile, which re-defers) can
              // re-enter the build cleanly. `_frameNavDeferredSw` suppresses the connect
              // path's element `load` (and dedupes re-entrant defers); `_frameLoadFired`
              // is re-armed so the retry's maybeFireFrameLoad runs the full build+load.
              if (!frameEl._frameNavDeferredSw) {
                frameEl._frameNavDeferredSw = true;
                (globalThis.__csimSwDeferredNavEls || (globalThis.__csimSwDeferredNavEls = [])).push(frameEl);
              }
              frameEl._frameLoadFired = false;
              return null;
            }
            // Resource Timing: a frame whose navigation failed (an unreachable host, a
            // refused redirect) is a network-error entry all the same.
            if (!resp) globalThis.__csimRecordResource({ name: frameUrl, initiatorType: frameEl._tag === 'frame' ? 'frame' : 'iframe', startTime: frameTimingStart, resp: null });
            if (resp) {
              // The navigation's reserved client id — handed to create_frame_realm so
              // the committed document's realm adopts it (event.resultingClientId →
              // clients.get(id) resolves to THIS client).
              frameClientId = resp.resulting_client_id || null;
              body = (resp.body || '');
              // Resource Timing: the frame's document is a resource of THIS document (`iframe` /
              // `frame` initiator, named by its final URL).
              globalThis.__csimRecordResource({ name: frameUrl, initiatorType: frameEl._tag === 'frame' ? 'frame' : 'iframe', startTime: frameTimingStart, resp });
              ...
              const csp = resp.headers && (resp.headers['content-security-policy'] || resp.headers['Content-Security-Policy']);
              if (csp) frameCspSandbox = cspSandboxDirectives(csp);
    ...
    // origin (left unset). NOTE: `frameLocationHref` below reports the opaque
    // location (about:blank / about:srcdoc) as `location.href`, but the frame's
    // BASE URL for relative resolution is seeded separately (`frameAboutBase`) so
    // named-frame / base-target navigation still resolves against the inherited
    // parent base — an earlier version that set the location WITHOUT that base
    // seed regressed a shadow-tree inert test.
    let frameDocOrigin;
    // Sandbox flags come from the container's `sandbox` attribute AND the response's CSP
    // `sandbox` directives; they RESTRICT, so the document keeps its own origin only when
    // every source that sandboxes it also grants `allow-same-origin`.
    const sandboxAttr    = frameEl.getAttribute('sandbox');
    const sandboxSources = sandboxAttr == null ? frameCspSandbox : frameCspSandbox.concat(sandboxAttr);
    if (sandboxSources.length && !sandboxSources.every(allowsSameOrigin)) {
      frameDocOrigin = 'null';
    ...
    // A frame with no navigable URL is at "about:blank" (empty `<iframe>` / an
    // unreachable src), and a `srcdoc` frame is at "about:srcdoc" — those are the
    // documents' URLs (`location.href` / `document.URL`), distinct from the base
    // URL used for relative resolution, which an about: document INHERITS from its
    // creator. We seed that inherited base (`frameAboutBase`, the parent
    // document's base URL) so `documentBaseURL` resolves the frame's relative URLs
    // against the parent while `location.href` still reports about:blank/srcdoc.
    // The javascript:-URL case keeps its own frameUrl (its initial document's URL
    // is handled by the js-url source path), so it's excluded here.
    let frameLocationHref = frameUrl, frameAboutBase = '';
    if (!navContent && srcdoc != null)      frameLocationHref = 'about:srcdoc';
    else if (!navContent && !frameUrl)      frameLocationHref = 'about:blank';
    if (/^about:/i.test(frameLocationHref)) {
      try { frameAboutBase = (globalThis.document && globalThis.document.baseURI) || ''; } catch (_) {}
    }
    ...
    if (typeof globalThis.__csim_createFrameRealm === 'function' && globalThis.RustyRacer && typeof globalThis.RustyRacer.contextGlobal === 'function') {
      ...
      // Measure the container HERE, in the parent realm, and pass it in: the frame's viewport has
      // to be seeded before its document loads (same reason as `name` above — a load-time
      // `innerWidth` / `@media` read is too early for anything set afterwards).
      let frameViewport = null;
      try { frameViewport = frameContentBox(frameEl); } catch (e) {}
      const id = globalThis.__csim_createFrameRealm(frameLocationHref, body, contentType, parentId, frameEl.getAttribute('name'), frameDocOrigin, frameLocationOrigin, jsUrlSource, frameAboutBase, frameViewport, frameClientId);
      if (id != null) {
        ...
        // The viewport was seeded pre-load (above). Re-push here only if that didn't take — the
        // child's layout must never PULL it across the realm boundary: layout runs on the page's own
        // getBoundingClientRect path, and re-entering a realm from inside that callback trips a V8
        // stack assertion (IsOnCentralStack) — a hard crash. We're in the parent realm here, so
        // measuring the container is free and safe.
        try { if (!win.__csimFrameViewport) win.__csimFrameViewportChanged(frameViewport || null); } catch (e) {}
  ...
  // Replay the frame builds this realm parked on a still-activating service worker
  // (the {deferred} branch above). Broadcast by the host when the activation state
  // changed — the worker activated, died, or its scope was unregistered — and run in
  // a task of its own: the broadcast arrives mid-drain, where a synchronous realm
  // build re-enters the engine (the same reason WindowClient.navigate is queued).
  // A frame that navigated/built by other means meanwhile is skipped; one whose
  // scope is STILL activating (a different worker) simply re-defers.
  globalThis.__csim_swRetryDeferredNavs = function () {
    const els = globalThis.__csimSwDeferredNavEls;
    if (!els || !els.length) return;
    globalThis.__csimSwDeferredNavEls = [];
    globalThis.setTimeout(() => {
      for (const el of els) {
        el._frameNavDeferredSw = false;
        if (!el.isConnected) continue;
        // The full connect choreography — build the realm, then the element `load`
        // (its _frameLoadFired guard was re-armed when the build deferred). A scope
        // still activating under a DIFFERENT worker simply re-defers inside. An
        // element already BUILT meanwhile (a contentWindow touch after activation
        // beat this task) still comes through here: the build is idempotent and
        // the re-armed _frameLoadFired means ITS pending element `load` fires now;
        // a frame that navigated by other means set the guard and no-ops.
        try { maybeFireFrameLoad(el); } catch (_) {}
      }
    }, 0);
  };
  ...
  function __takePendingFormSubmit () {
    ...
    return {
      ...
      entryList:       serializeEntryList(p.entryList)
    };
  }
  ...
  globalThis.__csimLoadDocument = function (html, contentType, charset) {
    ...
    __windowLoadFired = false;   // this Document object is REUSED by an in-place reload
    // The previous page's outstanding subresource gates are moot — their elements
    // belong to a document being replaced — and left in they would accumulate one
    // dead entry per navigation.
    __pendingSubresources.clear();
    resetCascadeState();
    ...
    globalThis.__csimPendingFormSubmit = null;
    // A markup-insertion guard stranded >0 (a write() aborted by V8 termination
    // skips its finally) would make every later write() on this reused document
    // throw — reset it with the other per-visit transients.
    globalThis.document._markupInsertionCounter = 0;
    // Drop the previous page's inert template document (an in-place reload —
    // frame navigations — reuses this Document object): it pins that page's
    // template-content trees alive, and the XML branch below can flip the
    // document's flavor while a cached text/html inert doc lingered.
    globalThis.document._inertTemplateDoc = null;
    ...
    // A text/plain resource is rendered as a document holding the response bytes
    // VERBATIM — not HTML-parsed — so character references (`&#601;`) and tags
    // stay literal in `body.textContent` (the form-submission charset tests echo
    // the request body and read it back). Model it by HTML-escaping the text into
    // a `<pre>` (the spec's "convert to a preformatted document" — a real browser's
    // text/plain document is a <pre>, whose content keeps EVERY newline: a routed
    // direct.txt body "Network\n" must read back with its trailing newline,
    // static-router-main-resource) and reusing the HTML graft path below. The
    // extra leading newline feeds the parser's drop-first-newline-after-<pre>
    // rule, so a body that itself starts with one keeps it.
    if (__ct === 'text/plain') {
      html = '<pre>\n' + String(html == null ? '' : html).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</pre>';
    }
    ...
    timeParse(() => parse5ParseIntoLive(d, srcHtml, __streamHooks));   // shadow: time JS DOM construction (inert when off)
  ...
  globalThis.__csimBootContext = function (opts) {
    if (typeof opts.viewportW === 'number' && typeof opts.viewportH === 'number') {
      globalThis.__csimSetViewport(opts.viewportW, opts.viewportH);
    }
    if (opts.userAgent) {
  ...
  // match.
  // ABSOLUTE url (resolved against the script's node-document base at run
  // time) → body, so two documents sharing a relative name for DIFFERENT
  // files never collide, and re-visits of the same absolute bundle URL
  // dedup as before. Map (not Set) so the bodies stay retrievable.
  ...
  function runInlineScripts(doc) {
    ...
    // A parse that handed NO scripts to a handler (the XML parser) leaves its parser-blocking ones to this pass: they
    // run first, while the document is still 'loading' — as a streaming parse runs them at their end tags.
    for (const s of scripts) if (s._csimParserInserted && !s._csimExecuted && isParserBlockingScript(s)) runOneScript(s);
    // …and then "the end": 'interactive' FIRST (firing readystatechange), then the deferred scripts, then
    // DOMContentLoaded — so a `defer` or module script sees 'interactive' (Chrome and Firefox), where this ran them
    // while it still said 'loading'. The 'complete' transition happens later, at each call site.
    ...
    for (const s of scripts) {
      ...
      // …and only the PARSER's scripts: one a script inserted ran (or never runs — an innerHTML fragment's, a clone of
      // a started script) on its own terms. Swept up here, those ran again, or ran when they never should.
      if (s._csimExecuted || !s._csimParserInserted) continue;
      runOneScript(s);
    }
  ...
  // A COMPILE error comes back from the engine as a plain Error whose message is "SyntaxError: … at <eval>:N".
  // Reported as it stands, `error instanceof SyntaxError` was false and the message carried the engine's location —
  // what error-tracking code reads. Chrome and Firefox report the SyntaxError itself.
  function scriptErrorOf(e) {
    const m = e instanceof Error && e.constructor === Error && /^SyntaxError: ([\s\S]*?) at <[^>]*>:\d+$/.exec(e.message);
    return m ? new SyntaxError(m[1]) : e;
  }
  function runOneScript(s) {
    const type = (s._attrs.type || '').toLowerCase();
    if (type === 'importmap') return;  // already consumed
    if (type === 'module') {
      if (failsOnEmptySrc(s)) return;
    ...
    // A classic script marked `nomodule` does not run where modules are supported (HTML "prepare the script element") —
    // the other half of differential serving, which ran BOTH bundles here.
    if (s._attrs.nomodule != null) { s._csimRan = true; s._csimExecuted = true; return; }
    if (failsOnEmptySrc(s)) return;
    let body;
    if (s._attrs.src) {
      if (/^data:/i.test(s._attrs.src)) {
        ...
        // Resolve a RELATIVE src against THIS document's base — the host fn resolves
        // a relative URL against the MAIN document (resolve_against_current), which
        // is another directory for a subframe (same unify as __csimFetch's
        // relative-URL fix, 1b9de029). Resolving here also keys the ran-registry on
        // the ABSOLUTE URL, so two documents sharing a relative name for DIFFERENT
        // files don't collide. An unresolvable base keeps the raw attribute.
        let srcKey = s._attrs.src;
        try { srcKey = new globalThis.URL(s._attrs.src, s.baseURI || (globalThis.location && globalThis.location.href) || undefined).href; } catch (_) {}
        // De-dupe across page loads: each app-wide bundle runs once
        // per Context. See `__externalScriptsRun` comment above.
        if (__externalScriptsRun.has(srcKey)) return;
        // A CONTROLLED document's external script is a SUBRESOURCE its service
        // worker intercepts (destination 'script') — the fetch blocks here like the
        // network path below (a classic script blocks the parser per HTML), awaited
        // on the direct-reply queue while the worker thread answers. null = no
        // interception / fallthrough → the plain network fetch; {blocked} = a
        // networkError respondWith → the script's error event.
        const swServed = swFetchScript(srcKey, s);
        if (swServed && swServed.blocked) { s._csimRan = true; s._csimExecuted = true; globalThis.setTimeout(() => dispatchScriptLoad(s, false), 0); return; }
        ...
        const timingStart = globalThis.performance.now();
        body = (swServed && swServed.body != null) ? swServed.body : globalThis.__csimExternalAsset(srcKey);
        // Resource Timing: the parse-time script's entry, sized by its source text (the asset
        // cache hands back the body alone); a body-less load is a network error.
        if (!s._csimTimingRecorded) {
          s._csimTimingRecorded = true;
          globalThis.__csimRecordResource({ name: srcKey, initiatorType: 'script', startTime: timingStart, noCors: s._attrs.crossorigin == null,
                                            ...assetTimingFields(srcKey, swServed && swServed.body != null ? swServed.body : body) });
        }
        ...
        // doesn't re-process this element and fire a second error. An SW-SERVED
        // EMPTY body is a SUCCESSFUL load of an empty script (`respondWith(new
        // Response(''))` — nothing runs, `load` fires), not a failure: the
        // dynamic path already treats it so.
        if (swServed && swServed.body === '') {
          s._csimRan = true; s._csimExecuted = true;
          __externalScriptsRun.set(srcKey, '');
          globalThis.setTimeout(() => dispatchScriptLoad(s, true), 0);
          return;
        }
        if (!body) { s._csimRan = true; s._csimExecuted = true; globalThis.setTimeout(() => dispatchScriptLoad(s, false), 0); return; }
        __externalScriptsRun.set(srcKey, body);
      }
    } else {
      body = scriptText(s);
    }
    ...
    if (globalThis.document) globalThis.document._currentScript = currentScriptFor(s);   // null in a shadow tree
    s._csimRan = true;
    s._csimExecuted = true;
    let _ok = true;
    try { __csim_runScript(label, body); } catch (thrown) {
      _ok = false;
      const e = scriptErrorOf(thrown);
      // HTML "report the exception": an `error` event on the window (`window.onerror` included) — Chrome fires it for a
      // parser-run script's throw as for any other — and the log, where no listener cancelled it.
      globalThis.__csimReportException(e, () => {
  ...
  // The SW fetch context a module graph's source fetches carry (a CONTROLLED
  // document's module loads dispatch fetch events, destination 'script', mode
  // 'cors'): [controllerHandle, clientId, referrer, credentials, rootIntegrity].
  // Null when uncontrolled -- the Ruby side then fetches from the network as before.
  function moduleSwCtx(el) {
    const swCtrl = (typeof globalThis.__csimSWControllerHandle === 'function') ? (globalThis.__csimSWControllerHandle() | 0) : 0;
    if (!(swCtrl > 0)) return null;
    const href = (globalThis.location && globalThis.location.href) || '';
    return [swCtrl, swClientIdOf(), href, moduleCredentialsOf(el), (el && el._attrs && el._attrs.integrity) || ''];
  }
  function runModuleScript(s) {
    const baseUrl = (globalThis.location && globalThis.location.href) || null;
    const sw = moduleSwCtx(s);
    let ok = true;
    if (s._attrs.src) {
      const url = resolveAgainst(s._attrs.src, baseUrl);
      try { sw ? __csim_evalEsmEntry(url, null, sw[0], sw[1], sw[2], sw[3], sw[4]) : __csim_evalEsmEntry(url, null); }
      catch (e) {
        ok = false;
      ...
      try { sw ? __csim_evalEsmEntry(url, body, sw[0], sw[1], sw[2], sw[3], sw[4]) : __csim_evalEsmEntry(url, body); }
      catch (e) {
        ok = false;
        try { console.error('[csim] inline module failed:', e && e.message); } catch (_) {}
      }
    }
    // Resource Timing: file the entries for everything this graph fetched before the `load` event,
    // so a module's own load handler sees them (a browser adds the timing entry when the fetch
    // completes, ahead of the load). A dynamic `import()` has no runModuleScript after it — the host
    // `flush_module_rt` files those at the next driver read of `performance`.
    flushModuleRt();
  ...
  // Drain the module fetches the graph loaders collected host-side and file a 'script' Resource
  // Timing entry for each — the entry module and every module it imports, statically or via a
  // dynamic `import()` (each once per URL; `note_module_fetch` dedupes). Called after a
  // `<script type=module>` evaluates (so its graph is timed before the element's `load` fires) and
  // — via the host `flush_module_rt` before a script reads `performance` — for a dynamic import,
  // which has no runModuleScript after it.
  function flushModuleRt() {
    if (typeof globalThis.__csim_takeModuleRt !== 'function' || typeof globalThis.__csimRecordResource !== 'function') return;
    const mods = globalThis.__csim_takeModuleRt();
    if (!mods || !mods.length) return;
    const now = globalThis.performance ? globalThis.performance.now() : 0;
    for (let i = 0; i < mods.length; i++) {
      globalThis.__csimRecordResource({ name: mods[i].url, initiatorType: 'script', startTime: now, resp: mods[i].meta, noCors: false });
    }
  }
  globalThis.__csimFlushModuleRt = flushModuleRt;
  function hashStr(s) {
    let h = 5381;
  ...
  // Visibility predicate behind Capybara's own `visible?`: "visible unless something says hidden" — INVISIBLE_TAGS
  // (head/script/style/template/…), the UA hides no page can override (a hidden input, an `<audio>` with no controls,
  // an `<embed>` with no resource), the `hidden` attribute, `display: none` / `visibility: hidden`, or a cascade rule
  // the resolver agrees with. One walk with `isVisibleNode`, which it used to keep a copy of.

  globalThis.__csimVisible = function (h) {
    // The same question `checkVisibility({visibilityProperty: true})` asks, up the flat tree: a light child no slot
    // renders, a slot's fallback while something is assigned, a closed `<details>`'s content.
    return isVisibleNode(lookup(h));
  };

  // `obscured?` — a coarse occlusion / hit-test (layout.js): an element is obscured when it's not
  // visible, out of the viewport, or a click at its centre lands on a non-descendant element on
  // top. Backed by the pay-per-use coarse box-layout model; the public getBoundingClientRect /
  // elementFromPoint stay on their existing coarse model (a later increment).
  globalThis.__csimObscured = function (h) {
    const n = lookup(h);
    if (!n || n.nodeType !== NODE_ELEMENT) return true;
    return isObscured(n);
  };

  // Cross-realm geometry seam: a frame document lays out in its OWN realm, so it asks its parent
  // realm for the geometry of its container `<iframe>` — its viewport (content box), and whether a
  // click point that landed inside it also lands on the frame itself out through every ancestor
  // document. Called only from a child realm's layout.js (never from Ruby).
  globalThis.__csimFrameContentBox = function (frameEl) { return frameContentBox(frameEl); };
  globalThis.__csimFrameObscuredAt = function (frameEl, x, y) { return frameObscuredAt(frameEl, x, y); };

  // Move the window viewport (Capybara's `resize_to`). One slot, so `innerWidth`, the `@media`
  // cascade and the layout engine can't disagree. Called from Ruby only — never from inside a page
  // callback, which is what makes the frame walk below safe (re-entering a realm from a running JS
  // frame trips V8's IsOnCentralStack assertion).
  globalThis.__csimSetViewport = function (w, h) {
    globalThis.__csimViewport = {width: w, height: h};
    invalidateLayout();
    refreshFrameViewports();
  };

  // This realm's container `<iframe>` has a (new) content box. Called by the PARENT realm — which
  // has already measured it — both when the realm is built (BEFORE the frame's document loads, so
  // a load-time `innerWidth` read is right) and whenever the window is resized.
  globalThis.__csimFrameViewportChanged = function (vp) {
    globalThis.__csimFrameViewport = vp;
    // A frame's WINDOW viewport IS its container's content box: `innerWidth` / `innerHeight`,
    // `@media` and `matchMedia` inside the frame report the FRAME's size, not the top window's —
    // that is what makes a responsive component in a narrow frame take its narrow branch.
    // An unrendered container (`display: none`, detached) has no box, so the parent pushes nothing:
    // that frame's window is 0x0 — what Chrome reports for a `display: none` iframe's
    // `contentWindow` — and nothing inside it is rendered or clickable.
    globalThis.__csimViewport = vp ? {width: vp.width, height: vp.height} : {width: 0, height: 0};
    invalidateLayout();
    // The same cascade + listener refresh Ruby's `set_viewport` does for the top window. Skipped
    // before the frame's document exists (realm-build seeding), where there is nothing to restyle.
    if (!globalThis.document || !globalThis.document.body) return;
    try { globalThis.__csimRebuildCascade(); } catch (e) {}
    try { globalThis.__csimViewportChanged(); } catch (e) {}
  };

  // A frame lays out against its container's content box, PUSHED in by this realm when the frame
  // realm was built. Resizing the window resizes the container too, so the push has to be redone —
  // otherwise everything inside the frame keeps measuring against the pre-resize box. Recurses
  // through nested frames: each realm re-pushes to its own children.
  function refreshFrameViewports() {
    const ids = globalThis.__csimChildRealmIds;
    if (!ids || !globalThis.RustyRacer) return;
    for (const id of ids) {
      try {
        const win = globalThis.RustyRacer.contextGlobal(id);
        if (!win || !win.frameElement) continue;
        win.__csimFrameViewportChanged(frameContentBox(win.frameElement));
        if (typeof win.__csimRefreshFrameViewports === 'function') win.__csimRefreshFrameViewports();
      } catch (e) {}
    }
  }
  globalThis.__csimRefreshFrameViewports = refreshFrameViewports;

  // `Node#rect` — the element's coarse border-box (viewport-relative), backing the spatial selectors
  // and coordinate drag. Same additive layout seam as __csimObscured; the app-facing
  // getBoundingClientRect is unchanged.
  globalThis.__csimRect = function (h) {
    const n = lookup(h);
    if (!n || n.nodeType !== NODE_ELEMENT) return { x: 0, y: 0, width: 0, height: 0 };
    return rectOf(n);
  };

  // …and the box a synthetic POINTER is aimed at, which is not the same one: WebDriver measures
  // its in-view centre point on the element's FIRST client rect (they differ only for an inline
  // that wrapped — whose bounding box's centre is the paragraph between its two lines), and it
  // measures it AFTER scrolling the element into view. Reading it before, as the driver's Ruby
  // side does, meant clipping a rect that was still off-screen down to nothing and aiming the
  // pointer at the corner of the viewport; `__csimClickResolve` scrolls again and finds the
  // element where this left it, so the two agree.
  globalThis.__csimPointerRect = function (h) {
    const n = lookup(h);
    if (!n || n.nodeType !== NODE_ELEMENT) return { x: 0, y: 0, width: 0, height: 0 };
    ensureInView(n);
    return pointerRectOf(n);
  };

  // Capybara `scroll_to` (real): `selfH` is the element scroll_to was called on (the document root
  // for a session-level scroll, else an element); `targetH` a target element to bring into view, or
  // null with `pos`/`x`/`y` for a keyword / coordinate scroll. Drives the scroll offset so a
  // following obscured?/rect reflects it (layout.applyScrollTo).
  // Capybara `scroll_to(:current, offset: [dx, dy])` — a relative scroll from the current offset.
  globalThis.__csimScrollBy = function (selfH, dx, dy) {
    const self = lookup(selfH);
    if (!self || self.nodeType !== NODE_ELEMENT) return;
    applyScrollBy(self, dx, dy);
  };

  globalThis.__csimScrollTo = function (selfH, targetH, pos, x, y) {
    const self = lookup(selfH);
    if (!self || self.nodeType !== NODE_ELEMENT) return;
    const target = (targetH != null) ? lookup(targetH) : null;
    applyScrollTo(self, (target && target.nodeType === NODE_ELEMENT) ? target : null, pos || null, x, y);
  };
  ...
  function __computeVisibleText(n) {
    // A node that is not RENDERED, or is skipped, has no visible text — Capybara's `text` on a node found with
    // `visible: false` is '' (WebDriver's getText). Asked up the flat tree (`isUnskippedNode`): a walk up `_parent`
    // counted a light child no rendered slot takes, and a closed `<details>`'s skipped content, as shown. `visibility`
    // is the walk's own business, per text node: a `visibility: visible` child of a hidden element still reads.
    const el = n.nodeType === NODE_ELEMENT ? n : n._parent;
    if (el && el.nodeType === NODE_ELEMENT && !isUnskippedNode(el)) return '';
    ...
    // The mode in force where this walk STARTS — resolved once, from the ancestors, and threaded
    // down from there. A boolean "an ancestor preserves" walk used to stand here, which could not
    // tell `pre-line` from `pre` and is what the walk now replaces its answer with anyway.
    return collectVisibleText(n, startTransform, n._parent ? inheritedWhiteSpace(n._parent) : 'normal');
  }
  ...
  globalThis.__csimCollapseInnerText = __collapseInnerText;
  function __collapseInnerText(raw) {
    if (raw == null) return '';
    // Nothing is left to do here: white-space processing happens IN the walk now, per text node and
    // streaming across them (cascade.js). It collapses the runs, drops the spaces at a line's
    // edges, and never writes a required break at the start or end of what it returns — so the
    // start/end trim this used to apply would only ever eat a `<pre>`'s own leading newline, which
    // is rendered text (`<pre>\nabc` is "\nabc" in Chrome).
    return raw;
  }
  ...
  function __innerTextNotRendered(n) {
    // …"being rendered" is having a layout box: `isLaidOutNode`, up the FLAT tree — a light child no rendered slot
    // takes is not (Chrome and Firefox: its textContent, "ab", where a node-tree walk gave the rendered "a"), and a
    // detached subtree is not either. A skipped closed-`<details>` content IS rendered, and its rendered text is "".
    return !(n.nodeType === NODE_ELEMENT && isLaidOutNode(n));
  }
  globalThis.__csimInnerText = function (node) {
    ...
    // …and the STREAMING PARSER's generation, which neither moves: a parse-time script read `"one"` again after the
    // parse had added `two` and `three` (Chrome: all three).
    const gen = globalThis.__settleGenGet();
    const cv  = globalThis.__csimCascadeVersion ? globalThis.__csimCascadeVersion() : 0;
    const pt  = globalThis.__csimParserTreeGen();
    if (node._itGen === gen && node._itCV === cv && node._itPT === pt) return node._it;
    let out;
    if (__innerTextNotRendered(node)) {
      out = node.textContent == null ? '' : String(node.textContent);
    } else {
      // No re-attaching of a trailing preserved newline any more: it existed because the collapse
      // trimmed every newline off the end and could not tell a `<pre>`'s own from a generated
      // break. The walk keeps the distinction now, so a compensation here only doubles it.
      out = __collapseInnerText(__computeVisibleText(node));
    }
    node._itGen = gen; node._itCV = cv; node._itPT = pt; node._it = out;
    return out;
  };
  globalThis.__csimVisibleText = function (h) {
    ...
    const gen = globalThis.__settleGenGet();
    const cv  = globalThis.__csimCascadeVersion ? globalThis.__csimCascadeVersion() : 0;
    const pt  = globalThis.__csimParserTreeGen();   // (…and the parser's, as for `__csimInnerText`)
    if (n._vtGen === gen && n._vtCV === cv && n._vtPT === pt) return n._vt;
    const result = __computeVisibleText(n);
    n._vtGen = gen; n._vtCV = cv; n._vtPT = pt; n._vt = result;
    return result;
  };
  ...
  // Build the drag data store for a driver-initiated drop (`Element#drop`). `types` derives from
  // the items, so pushing an item is all it takes for the page to see the format.
  ...
  globalThis.__csimClickFinish = function (h, base) {
    ...
    dispatchEventForUserAction(n, new MouseEvent('mouseup', base));
  ...
  // Capybara `Element#drag_to(other)` — the driver-side drag emulation (drag.js): a mousedown on
  // the source (which also primes libraries that gate `draggable="true"` on a recent mousedown,
  // e.g. Discourse's sidebar-section-form-link, and decides pointer-driven vs HTML5), then either
  // mousemove/mouseup or the full drag-event chain over a shared DataTransfer. `opts` carries
  // Capybara's `drop_modifiers:` (as a `{altKey: true, …}` map) and an explicit `html5:` override.
  // Drag events leave `offsetX/Y` at 0, so listeners that compute `offsetY < height/2` to decide
  // "above vs below" land on "above" — matching drag_to's semantic of moving the source to *before*
  // the target.
  // Staged so the driver can settle between them — see drag.js: a real drag runs the page's timers
  // between the press, the move and the drop, and libraries schedule work in that gap.
  globalThis.__csimDragBegin = function (sourceHandle, targetHandle, opts) {
    return dragBegin(__handles.get(sourceHandle), __handles.get(targetHandle), opts);
  };
  globalThis.__csimDragMove   = function () { return dragMove(); };
  globalThis.__csimDragFinish = function () { return dragFinish(); };
  ...
  function __csimHitTestRetarget(n) {
    ...
    // A shadow host's light-DOM children are only rendered when slotted into its
    // shadow tree — the host's interactive content is its flat tree, not its raw
    // `_children`. Don't DFS into them: return the host so focus delegation (a
    // delegatesFocus host's focus() picks the flat-tree delegate) or the host's
    // own (non-)focusability decides. Descending here wrongly focused an unslotted
    // light <input> on a host click.
    // (shadow-dom/focus/click-focus-delegatesFocus-click.html)
    if (n._shadowRoot) return n;
  ...
  // WebDriver "element click intercepted": is `other` (the topmost element at the
  // click point) an acceptable recipient for a click AIMED at `target`? Itself,
  // its descendants (a button's icon receives the button's click) and its
  // ancestors are. Beyond that this is deliberately MUCH narrower than Chrome:
  // only an unrelated obstructor that covers the target's whole rect AND most of
  // the viewport counts — the shape of a modal backdrop mid-exit-animation or a
  // full-page overlay, where Chrome refuses the click and Capybara retries until
  // it is gone. Anything smaller is ignored because our coarse layout produces
  // spurious overlaps Chrome's geometry doesn't have — a neighbouring button's
  // icon straying over the target's centre, a toggle switch's slider fully
  // covering its intentionally tiny hidden checkbox — and the strict WebDriver
  // check turned each into a failure of a click a real browser performs fine
  // (measured on Discourse: 131 false interceptions, 105 of them one cloak).
  function clickObstructedBy(target, targetRect, other) {
    if (!other || other === target) return null;
    // FLAT-tree relatedness, matching the tree the hit-test resolved in: a
    // slotted light node's raw `_parent` chain skips the shadow wrapper it is
    // painted inside, and the node-tree walk called it an unrelated obstructor.
    if (isFlatAncestor(target, other)) return null;  // descendant (icon inside the button)
    if (isFlatAncestor(other, target)) return null;  // ancestor
    const or = rectOf(other);
    const coversTarget = or.x <= targetRect.x && or.y <= targetRect.y &&
                         or.x + or.width  >= targetRect.x + targetRect.width &&
                         or.y + or.height >= targetRect.y + targetRect.height;
    if (!coversTarget) return null;
    const vp = viewportSize();
    const ix = Math.max(0, Math.min(or.x + or.width,  vp.width)  - Math.max(or.x, 0));
    const iy = Math.max(0, Math.min(or.y + or.height, vp.height) - Math.max(or.y, 0));
    return (ix * iy >= 0.8 * vp.width * vp.height) ? other : null;
  }

  // The WebDriver obscured-click probe for a REAL top-level pointer click aimed
  // at `n`: hit-test the click point (the caller's client coordinates when it
  // supplied them, else the in-view centre) and answer the retryable
  // `{kind:'intercepted'}` action when an unrelated page-covering overlay would
  // receive it. Only genuine pointer clicks opt in — keyboard activation, label
  // forwarding and other synthetic re-entries through `__csimClickResolve` are
  // default actions a browser performs regardless of overlays. KNOWN LIMIT: the
  // probe is realm-local, so a PARENT document's overlay over an iframe is not
  // seen here (obscuredAtPoint's cross-frame recursion is the model to adopt if
  // an app depends on it).
  function clickInterceptionAt(n, mods) {
    const rect = pointerRectOf(n);
    if (!(rect.width > 0 && rect.height > 0)) return null;
    const px = typeof mods.clientX === 'number' ? mods.clientX : rect.x + rect.width / 2;
    const py = typeof mods.clientY === 'number' ? mods.clientY : rect.y + rect.height / 2;
    const other = clickObstructedBy(n, rect, hitTest(px, py));
    if (!other) return null;
    const desc = (el) => '<' + el._tag +
      (el._attrs && el._attrs.id ? ' id="' + el._attrs.id + '"' : '') +
      (el._attrs && el._attrs.class ? ' class="' + String(el._attrs.class).slice(0, 80) + '"' : '') + '>';
    return { kind: 'intercepted', target: desc(n), other: desc(other), x: Math.round(px), y: Math.round(py) };
  }

  // The same probe for the click chains that don't route through
  // `__csimClickResolve` (right click, double click) — Ruby asks first and
  // raises the retryable error before dispatching anything.
  globalThis.__csimClickInterceptedAt = function (h, mods) {
    const n = lookup(h);
    if (!n || n.nodeType !== NODE_ELEMENT) return null;
    ensureInView(n);
    return clickInterceptionAt(n, mods || {});
  };

  globalThis.__csimClickResolve = function (h, modifiers, opts) {
    const n = __csimHitTestRetarget(lookup(h));
    if (!n || n.nodeType !== NODE_ELEMENT) return null;
    // WebDriver's element-click algorithm scrolls the target into view first, so a real browser
    // never clicks something that is off-screen. Page code reads that: floating-ui's `hide`
    // middleware — which positions every select-kit dropdown in Discourse — marks its reference
    // "hidden" and the menu never appears if the trigger is below the fold when it is clicked.
    ensureInView(n);
    // …and, for a REAL pointer click (Ruby's Browser#click opts in; synthetic
    // and forwarded activations don't), it hit-tests the click point before
    // dispatching: an unrelated element painted on top (a modal backdrop still
    // finishing its exit animation, a full-page overlay) RECEIVES the click in a
    // browser, so the driver refuses with the retryable "click intercepted"
    // error instead of tunnelling the events to the covered target. Capybara's
    // retry then re-finds and re-clicks once the obstruction is gone — which is
    // also what lets an in-flight async close (parked on its exit-animation
    // timer) complete before the next modal opens, instead of its stale
    // continuation closing the new one.
    if (opts && opts.interceptCheck) {
      const intercepted = clickInterceptionAt(n, modifiers || {});
      if (intercepted) return intercepted;
    }
    const mods = modifiers || {};
    // A Ruby-driven Capybara click wants the page's resulting submit/navigate
    // intent RETURNED so the Ruby side actions it. A testdriver click
    // (`test_driver.click`) is JS-side and discards the return, so it asks us to
    // LEAVE the intent in its pending slot — the driver's normal drain then
    // actions it, exactly as for a page-initiated `form.submit()` / nav.
    const leavePending = !!(opts && opts.leavePending);
    ...
    // FloatKit popovers depend on that path firing. Dispatched as a real
    // PointerEvent (which extends MouseEvent, so every mouse-shaped
    // listener still matches) — a library that branches on `instanceof
    // PointerEvent` / `pointerType` to prefer the pointer path, as
    // SortableJS does, has to see one. Same shape drag.js uses.
    // The whole pointer/mouse sequence of a UA click (testdriver / Ruby action)
    // is TRUSTED, like the click itself — real browsers fire trusted mousedown/
    // mouseup/pointer events for an input-injected click, and a listener that
    // re-dispatches one must see the original as isTrusted=true
    // (dom/events/Event-dispatch-redispatch.html).
    try { dispatchEventForUserAction(n, new PointerEvent('pointerdown', pointerEventInit(base))); } catch (_) {}
    const mousedownEv = new MouseEvent('mousedown', base);
    dispatchEventForUserAction(n, mousedownEv);
    ...
    try { dispatchEventForUserAction(n, new PointerEvent('pointerup', pointerEventInit(base, 0))); } catch (_) {}
    dispatchEventForUserAction(n, new MouseEvent('mouseup',   base));
    const click = new MouseEvent('click', base);
    ...
    // A testdriver click leaves the submit/nav intent pending (above); the drain
    // picks it up. A Capybara click consumes + returns it for the Ruby side.
    if (!leavePending) {
      const pendingSubmit = __takePendingFormSubmit();
      if (pendingSubmit) return { kind: 'submit', formHandle: pendingSubmit.formHandle, submitter: pendingSubmit.submitterHandle || 0 };
      ...
      if (pendingNav && pendingNav.url) {
        globalThis.__csimPendingNavigation = null;
        const act = { kind: 'navigate', url: String(pendingNav.url), target: String(pendingNav.target || ''), opener: !!pendingNav.opener };
    ...
    // `<option>` activation: a UA / Capybara click on an option toggles (multiple) or
    // picks (single) it, firing input/change on the select. This path sets
    // `_csimActivationHandled`, so the dispatch.js twin is skipped — run it here.
    if (n._tag === 'option' && !click.defaultPrevented && typeof globalThis.__csimToggleOptionByClick === 'function') {
      globalThis.__csimToggleOptionByClick(n);
    }
    ...
    if (__anchor && __anchor.nodeType === NODE_ELEMENT &&
      ...
      // rel=opener keeps the opener on a target=_blank/named link (default noopener);
      // shared with the Element.click() / dispatchEvent activation paths so a UA click
      // agrees with a scripted one.
      return { kind: 'navigate', url: href, target, opener: anchorHasRelOpener(__anchor) };
    }
    ...
    if (label) {
      ...
      if (labeled && labeled !== n && labeled !== label) {
        // Forward modifiers + opts (leavePending) so a testdriver click on a
        // <label> for a submit button still leaves the submit intent for the
        // drain — but NOT interceptCheck: the pointer click landed on the LABEL
        // and passed its own hit-test; this re-entry is the label's default
        // action, which a browser performs however covered the control is.
        const fwd = opts && opts.interceptCheck ? { ...opts, interceptCheck: false } : opts;
        return __csimClickResolve(labeled._id, mods, fwd);
      }
    }
    if (isSubmitButton(n)) {
      ...
      // even though the original submit was preventDefault'd). Honour
      // the queued intent first; otherwise the queued submit sits
      // forever and the page never navigates. (leavePending: a testdriver
      // click leaves it for the drain instead of handing it back.)
      if (globalThis.__csimPendingFormSubmit) {
        if (leavePending) return null;
        const p = __takePendingFormSubmit();
        return { kind: 'submit', formHandle: p.formHandle, submitter: p.submitterHandle || 0, entryList: p.entryList };
      }
      ...
      constructEntryListAndRecord(form, n, false);   // a button click is an interactive (non-method) submit
      if (leavePending) return null;   // leave __csimPendingFormSubmit for the driver's drain
  ...
  //
  // Returns whether this call is the one that fired (see `__windowLoadFired`).
  globalThis.__csimFireWindowLoad = function () {
    if (__windowLoadFired) return false;
    __windowLoadFired = true;
    flushPendingSubresources();   // a document isn't loaded until its sheets and frames are
    const ev = new Event('load');
    // HTML fires the document's load event AT THE WINDOW with the "legacy target
    // override flag" set, which leaves `currentTarget` the window but makes
    // `target` the DOCUMENT (Chrome: `event.target.constructor.name` is
    // `HTMLDocument`). Page code that routes on `event.target` — jQuery's
    // delegation, any `if (e.target === document)` guard — reads it.
    try { ev.target = globalThis.document; } catch (_) {}
    ev._uaFired = true;    // the UA-dispatch marker `dispatchEvent` turns into isTrusted
