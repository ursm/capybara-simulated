...
import { deliverMutations, flushMutationDelivery, hasQueuedRecords, hasObservers, recordAttrMutation } from './mutation-observer.js';
...
(function () {
  ...
  globalThis.__csimLoadDocument = function (html, contentType) {
    ...
    // Parse the response directly into the live document via parse5. The adapter
    // REUSES the live document / <html> / <head> / <body> identities: library
    // IIFEs (jQuery 3.x in particular) capture `document.documentElement` at
    // evaluation time and reuse it for `createElement` / `appendChild` probes, so
    // replacing the skeleton would strand those references on a detached node.
    // parse5 drops the prior tree, rebuilds the prolog (doctype / comments before
    // <html>), the quirks flag (`compatMode`), the head/body/html opening-tag
    // attributes (Redmine's `body.controller-X.action-Y` cascade scoping rides on
    // this), and the full element tree. Scripts are NOT run during the build;
    // `runInlineScripts` executes them in tree order below — the incremental
    // script-blocking streaming driver layers onto this next.
    parse5ParseIntoLive(d, srcHtml);
    // Declarative Shadow DOM: main-document parsing is a trusted context, so
    // convert `<template shadowrootmode>` into shadow roots before scripts run.
    // The conversion registers the shadow content itself; slot assignment is
    // computed lazily on read (assignedNodes / findSlotForSlottable), so no
    // connect-time wiring is needed. Ingest the import map from the live tree
    // first so a declarative `shadowrootadoptedstylesheets` specifier resolves.
    // Gate on a cheap scan so the O(N) tree walk is skipped on the (overwhelming)
    // majority of pages that author no declarative shadow root.
    if (/shadowrootmode/i.test(srcHtml)) { ingestImportmaps(d); processDeclarativeShadowRoots(d); }
    // Stay on 'loading' through custom-element upgrade + inline-script
    // evaluation to match real browsers, where parser-blocking scripts see
    // readyState='loading' and register `DOMContentLoaded` listeners instead of
    // running their ready callbacks inline. Forem's `base.js` bundle is the
    // canonical case: it pulls in ahoy.js via `//= require`, then later in the
    // same bundle calls `ahoy.configure({cookies: false, trackVisits: false})`.
    // ahoy.js's IIFE schedules `ahoy.start()` via `documentReady` — if readyState
    // is already 'complete', that callback fires synchronously and the visit POST
    // goes out before `ahoy.configure` runs, undoing the test-environment opt-out.
    d.readyState = 'loading';
    // Connect the freshly-built tree: custom-element upgrades + deferred iframe /
    // link `load`. parse5 builds via raw tree-adapter pushes (no per-insertion
    // connect), so fire it once here — before scripts, mirroring a real browser
    // where elements are connected and upgraded as the parser builds them, all
  ...
  function runInlineScripts(doc) {
    ...
    // more scripts, or — pathologically — redefines `NodeList.prototype.length`).
    ...
    for (const s of scripts) {
      ...
      if (!body) { s._csimRan = false; continue; }
