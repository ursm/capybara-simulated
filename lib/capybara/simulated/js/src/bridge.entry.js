...
(function () {
  ...
  // PromiseRejectionEvent is intentionally NOT exposed: core-js's
  // promise-constructor-detection probes `globalThis.PromiseRejectionEvent`
  // and switches its rejection-tracking path when it's callable. Our
  // Promise implementation isn't a full spec match, so the native
  // branch silently breaks downstream consumers (Avo's lazy turbo
  // frames stop loading). Keep the class for internal use only.
