...
import { ceTryConnect, runSelectednessAlgorithm, ensureOptionSelInit, connectSelectedContent, ownerSelectOf } from './custom-elements.js';
...
(function () {
  ...
  // Sticky page-level flag: set the first time any <selectedcontent> connects. A
  // page that never uses a customizable <select> leaves it false, so the per-option
  // reconcile's ownerSelectOf walk is skipped for every ordinary <option> (rule 3).
