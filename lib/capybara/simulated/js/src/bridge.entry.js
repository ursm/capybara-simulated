...
(function () {
  ...
  function __computeVisibleText(n) {
    // A node that is not VISIBLE has no visible text — Capybara's `text` on a node found with `visible: false` is ''
    // (WebDriver's getText). Asked up the flat tree (`isVisibleNode`): a walk up `_parent` counted a light child no
    // rendered slot takes, and a closed `<details>`'s skipped content, as shown.
    const el = n.nodeType === NODE_ELEMENT ? n : n._parent;
    if (el && el.nodeType === NODE_ELEMENT && !isVisibleNode(el)) return '';
