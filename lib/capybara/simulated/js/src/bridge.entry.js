...
import { Blob, File, installBlobURL, resolveBlobBytes, blobBytes }  from './blob.js';
...
(function () {
  ...
  // has no slot — its bytes live in the VM, so we carry them base64-encoded (via
  // the in-VM btoa, which is byte-safe) for the classic submit to encode.
