      ...
      if (src && src !== 'about:blank') {
        try {
          ...
          if (/^data:/i.test(frameUrl)) {
            ...
            // (Document.currentScript "iframe-src".)
            // NOTE: strict decode (vs the spec's lenient percent-decode). A source
            // mixing a lone `%` with a valid `%XX` throws → raw fallback leaves the
            // `%XX` undecoded too; a fully-lenient decoder is non-trivial (multi-byte
            // UTF-8) and no app/WPT case hits this, so the fallback stands.
            try { jsUrlSource = decodeURIComponent(frameUrl.slice('javascript:'.length)); }
            catch (_) { jsUrlSource = frameUrl.slice('javascript:'.length); }
