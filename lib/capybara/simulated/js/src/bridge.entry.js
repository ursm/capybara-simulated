...
import { installHtmlParser, serializeElement, serializeChildren, escapeText } from './html-parser.js';
import {
  parseSelector,
  matchOne,
  matchComplex,
...
import { HTMLCollection, NamedNodeMap }           from './dom-collections.js';
...
(function () {
  ...
  // HTML parser lives in `./html-parser.js`. Install once with the
  // DOM ctors so parseDocument / parseFragment close over them. The
  // serializer half (serializeElement / serializeChildren) is
  // ctor-free and imported directly at module top.
  const { parseDocument, parseFragment } =
