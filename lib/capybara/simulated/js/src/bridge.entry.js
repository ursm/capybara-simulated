...
(function () {
  ...
  globalThis.__csimClickResolve = function (h, modifiers) {
    ...
    if (isSubmitButton(n)) {
      ...
      if (!isConnected(form)) return null;   // cannot navigate after the submit event
      // Construct the entry list (HTML steps 6-7) — `new FormData(form, n)` fires
      // `formdata`, so a handler's append/delete is honoured by the submission, and
      // it's threaded to Ruby rather than the form being re-serialised. (We skip our
      // partial constraint-validation model here, as on the JS click-activation path:
      // a false positive must not block a real app's submit-button.)
      const entryList = new FormData(form, n);
      if (!isConnected(form)) return null;   // a `formdata` handler removed the form → abort
      // Route through the shared submission entry point: a submit whose `target`
      // (or the submitter's `formtarget`) names a same-document frame is contained
      // to that child context and handled JS-side (returns nothing pending); an
      // ordinary top/self submit stashes the pending intent we hand back to Ruby.
      recordFormSubmission(form, n, entryList);
