...
(function () {
  ...
  function __csimHitTestRetarget(n) {
    ...
    const hit = dfs(n);
    if (hit) return hit;
    // Anchor-fronted wrapper. Two real-browser shapes:
    //
    //  (a) Single-href subtree (Discourse nav-pill `<li><a href>…</a></li>`,
    //      subcategory category-box with just `.parent-box-link` inside):
    //      the lone anchor visually fills the wrapper, so a geometric
    //      click anywhere lands on it.
    //
    //  (b) "Stretched link" wrapper: the wrapper has `data-url` pointing
    //      at a primary anchor (often styled with `position:absolute;
    //      inset:0` on a `::before`), with smaller secondary anchors
    //      overlaid. Discourse's parent category-box on `/categories`
    //      lists subcategory chips alongside `.parent-box-link`, but
    //      the stretched ::before makes the whole box navigate to the
    //      parent. Match the wrapper's data-url against an inner
    //      anchor's href to recover the primary activation surface.
    //
    // Multi-anchor wrappers without a data-url pointer stay on the
    // wrapper because the click coordinate is genuinely ambiguous.
    // The Redmine gantt row carries a hidden `<input>` checkbox inside
    // the subject span, so DFS short-circuits on the input before this
    // fallback runs.
    let onlyHref = null;
    let multiple = false;
    let dataUrlMatch = null;
    const dataUrl = n._attrs && n._attrs['data-url'];
    // Short-circuit returns `true` once the answer is fixed: a second
    // anchor with `data-url` already matched (or no data-url at all)
    // means more anchors can't change the outcome — important on
    // Discourse topic-list wrappers with thousands of descendants.
    function findHref(node) {
      ...
      for (let i = 0; i < kids.length; i++) {
        const k = kids[i];
        if (k.nodeType !== NODE_ELEMENT) continue;
        if (k._tag === 'a' && k._attrs.href != null) {
          if (dataUrl && !dataUrlMatch && k._attrs.href === dataUrl) dataUrlMatch = k;
          if (onlyHref) multiple = true;
          else onlyHref = k;
          if (multiple && (!dataUrl || dataUrlMatch)) return true;
        }
        if (findHref(k)) return true;
      }
      return false;
    }
    findHref(n);
    if (dataUrlMatch) return dataUrlMatch;
    if (onlyHref && !multiple) return onlyHref;
    return n;
  }
