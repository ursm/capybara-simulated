      ...
      if (src && src !== 'about:blank') {
        try {
          ...
          if (/^data:/i.test(frameUrl)) {
            ...
            // A controlled navigation goes through the SW's `fetch` event first (mode
            // 'navigate'): the host call blocks for respondWith and returns the response wire
            // shape, or null to load from the network. A network-error respondWith fails the
            // navigation (empty document). Adapt the wire shape to __rackFetch's `resp` form.
            // The navigation's referrer is the INITIATING document — this realm builds the frame,
            // so its own location is the parent/initiator URL (the host resolves it per policy).
            const navReferrer = (globalThis.location && globalThis.location.href) || '';
            const swNav = (globalThis.__csimSWMayInterceptNavigation && globalThis.__csimSWMayInterceptNavigation(frameUrl))
              ? globalThis.__csim_swNavigationFetch(frameUrl, isReloadNav, false, navReferrer)
              : null;
            let resp;
            if (swNav && swNav.networkError) {
              resp = null;   // respondWith(Response.error()) → failed navigation (empty document)
            } else if (swNav) {
              // SW served the navigation. Adapt the response wire shape to __rackFetch's `resp`
              // form: decode the base64 body to text through the content-type charset (like the
              // network path's `resp.body`, so a non-ASCII document isn't mojibaked).
              const swct = (swNav.headers && (swNav.headers['content-type'] || swNav.headers['Content-Type'])) || 'text/html';
              resp = {
                body:       swNav.body_b64 ? decodeBlobBody({bytes: globalThis.atob(swNav.body_b64), type: swct}) : '',
                headers:    swNav.headers || {},
                url:        swNav.url || frameUrl,
                redirected: false
              };
            } else {
              resp = globalThis.__rackFetch('GET', frameUrl, null, {}, 'follow');
            }
            if (resp) {
              body = (resp.body || '');
