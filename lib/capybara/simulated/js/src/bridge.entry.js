...
(function () {
  ...
  function maybeFireLinkLoad(el) {
    ...
    if (!isStylesheet && !isPrefetch && !rel.includes('modulepreload') && !rel.includes('preload')) return;
    ...
    subresourceLoadTask(el, () => {
      ...
      if (/^data:/i.test(href)) {
        ...
        if (sw) {
          ...
          // Controlled → the SW round-trip owns the load/error dispatch.
          // UNCONTROLLED prefetch stays inert (its pre-feature behavior): actually
          // fetching every route hint is real per-connect Rack traffic the app
          // suites never paid — an owned, measured change if ever wanted (rule 3).
          prefetchViaSw(el, abs);
          return;
        } else {
          try {
        ...
        if (el._csimTimingRecorded !== abs) el._csimTimingRecorded = abs, globalThis.__csimRecordResource({ name: abs, initiatorType: 'link', startTime: timingStart, resp: linkResp,
