...
(function () {
  ...
  // NOTE: a `<style>` `load` event (for @import) is deliberately NOT fired. It
  // would let render-blocking "is cancellable" tests observe the import as loaded
  // within one rAF (they rely on the `?pipe=trickle(dN)` server delay, which we
  // don't honor) → a regression. script-inserted-style-element (which observes the
  // style element's load) therefore stays out-of-scope pending trickle-delay support.
