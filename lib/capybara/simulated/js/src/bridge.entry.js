...
(function () {
  ...
  function runInlineScripts(doc) {
    ...
    // HTML "the end": the document becomes 'interactive' (firing readystatechange) FIRST, then the deferred scripts
    // run, then DOMContentLoaded — so a `defer` or module script sees readyState 'interactive' (Chrome and Firefox),
    // where this ran them while it still said 'loading'. The 'complete' transition happens later, at each call site.
