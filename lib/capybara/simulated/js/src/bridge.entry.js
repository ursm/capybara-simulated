...
(function () {
  ...
  globalThis.__csimLoadDocument = function (html, contentType, charset) {
    ...
    // a single text node, then reusing the HTML graft path below; the parser's
    // CR/LF→LF normalization is the spec's "convert to a preformatted document".
    if (__ct === 'text/plain') {
      html = String(html == null ? '' : html).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    }
