...
(function () {
  ...
  // `PromiseRejectionEvent` is NOT exposed yet. core-js's
  // `promise-constructor-detection` reads `isCallable(globalThis
  // .PromiseRejectionEvent)`; when true it stops polyfilling Promise
  // and the app uses the V8 native one. Native Promise here is fine
  // in isolation, but core-js's polyfilled Promise has subtly
  // different microtask scheduling that some apps (Avo's Turbo-frame
  // pagination flow) depend on. Until our microtask drain matches
  // the spec well enough to substitute for core-js's, leave the
  // class internal so the polyfill stays in place.
