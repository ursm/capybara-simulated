  ...
  globalThis.__csimVisible = function (h) {
    ...
    while (cur) {
      ...
      if (cur.nodeType === NODE_ELEMENT) {
        ...
        // rendered. Only flag the target as hidden when a *closed
        // ancestor* `<details>` sits above us and we never passed
        // through a `<summary>`. Discourse's select-kit wraps its
        // dropdown surface in `<details class="…tag-chooser">`, and
        // `find('…tag-chooser:not(.is-expanded)')` would otherwise
        // miss the visible trigger.
