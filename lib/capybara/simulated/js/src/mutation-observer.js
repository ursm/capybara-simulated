...
// `media` / `title` only re-select the sheet (which set it is in, whether its query holds), and the sheet's media list
...
const STYLE_SHEET_ATTRS = new Set(['media', 'title']);
...
const parsedParents = new Set();
export function noteParsedChange(parent) {
  if (layoutHasRun) parsedParents.add(parent);
  markDirAutoScopes(parent);
}
globalThis.__csimFlushPendingMarks = () => {
  layoutHasRun = true;
  if (parsedParents.size) {
    for (const p of parsedParents) dirtyLayoutSpine(p, false, true);
    parsedParents.clear();
...
export function recordAttrMutation(target, key, oldValue, meta) {
  ...
  if (lkey === 'class') {
    const effect = classWriteLayoutEffect(oldValue, target._attrs['class'], target);
    if (effect === true) {
