...
import { markArenaDirty } from './native-query-shadow.js';
...
export function recordAttrMutation(target, key, oldValue, meta) {
  markArenaDirty();   // shadow measurement: a mutation stales the native arena (inert when off)
...
export function recordChildList(target, added, removed, prevSibling, nextSibling) {
  markArenaDirty();   // shadow measurement: a structural mutation stales the native arena (inert when off)
...
export function recordCharacterData(target, oldValue) {
  markArenaDirty();   // shadow measurement: a text edit can flip the parent's :empty (inert when off)
