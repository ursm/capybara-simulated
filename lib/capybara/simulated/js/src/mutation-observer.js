...
import { syncArenaChildList, syncArenaCharData } from './native-query-shadow.js';
import { flatTreeParent } from './walk.js';
import { scheduleCascadeRefresh, bumpCascadeVersion, layoutAttrEffect, layoutChildListEffect, layoutCharDataEffect, ctxAttrEffect, ctxChildListEffect, ctxCharDataEffect, bumpCtxInserted } from './cascade.js';
...
// cascade (`scopedRulesFor` caches per version, so the empty answer sticks).
...
function stylesheetChanged(node) {
  for (let n = node; n; n = n._parent) {
    if (n._isShadowRoot) {
      // …and re-QUEUE the tree. `shadowSheetFacts` folds a root once and re-folds only what the
      // queue hands it, so a sheet edited or inserted HERE reached none of the document-wide gates
      // its rules arm — `shadowUnsafe` above all, which latches `:host-context()` / `::part()` and
      // is read as "no per-element question is sound on this page". A tree folded before the edit
      // kept answering for the sheet it used to carry. The fold dedupes on the sheet's cache KEY
      // (its text), so re-queueing an unchanged tree costs one cached `scopedRulesFor`.
      const pending = globalThis.__csimPendingShadowRoots;
      if (pending) pending.push(n);
      bumpCascadeVersion();
      return;
    }
  }
...
function sheetAttrChanged(el, name, value) {
  if (el._tag === 'link') {
    ...
    if (name === 'disabled') {
      if (value == null) el._explicitlyEnabled = true;
      else if (el._sheet) { el._sheet._ownerNode = null; el._sheet = null; }
    }
...
// (el) => did a `dir=auto` element's resolved direction flip — dom-nodes.js owns the first-strong rule, and hands it
// over the way it hands the slot hooks: importing it here instead reorders the bundle's module evaluation, and this
// module's `let`s then reset the hooks dom-nodes had already installed (every slot reassignment went unsignalled).
let autoDirectionFlipped = null;
export function setAutoDirectionProbe(fn) { autoDirectionFlipped = fn; }
...
export function signalSlotChange(slot) {
  ...
  // …and the FLAT tree changed shape under the slot: what it lays out is its new assigned set, inheriting from it. The
  // slot's ancestors there are shadow-side boxes no DOM mutation stamped (the light node that moved is under the HOST,
  // not under them), so a layout memo — and a native pass's kept subtree — above the slot still held.
  markLayoutDirty(slot, true, true);
  markDirAutoScopes(slot);   // …and a `dir=auto` slot, or one around it, resolves from what is assigned now
...
// …and WHICH nodes changed since the last rendering update. A CSS transition is started by a style
// CHANGE, and this driver computes values on demand — so without a list of what changed, a
// transition on an element nothing happens to read is never noticed, and the `transitionend` a page
// is waiting for never comes. Bounded: past the cap the flag says "everything", and the animation
// model falls back to its whole-document sweep.
const MUTATED = new globalThis.Set();
// Past this many, the flag says "everything" and the animation model falls back to a whole-document
// sweep — which the STYLE FLUSH runs at the start of every forced layout, not once a frame, so the
// cap is what decides whether a page that touches many nodes pays that sweep per layout (measured:
// 3x on 300 class toggles with `transition: all`).
const MUTATED_CAP = 4096;
let mutatedOverflow = false;
export function takeMutatedNodes() {
  const out = { nodes: [...MUTATED], overflow: mutatedOverflow };
  MUTATED.clear();
  mutatedOverflow = false;
  return out;
}
function stampMutation(node) {
  mutatedAt = globalThis.__virtualNow ? globalThis.__virtualNow() : 0;
  if (!node) return;
  if (MUTATED.size >= MUTATED_CAP) { mutatedOverflow = true; return; }
  MUTATED.add(node);
}
...
// …with what the selectors need of each change, as a childList record would carry it (`layoutChildListEffect`): the
// node before the first insertion (the parser appends, so everything after it is new; `undefined` once a removal or an
// insertion elsewhere makes the change point unknowable), and whether text was appended — `li:last-of-type` stayed on
// the old last `<li>`, and a parsed `.flag` never reached `.c:has(.flag)`.
const parsedParents = new Map();
export function noteParsedChange(parent, inserted, removed) {
  if (layoutHasRun) {
    let rec = parsedParents.get(parent);
    if (rec === undefined) parsedParents.set(parent, rec = { before: null, inserted: false, text: false });
    if (removed) { rec.before = undefined; rec.inserted = true; }
    else if (inserted) {
      if (!rec.inserted) rec.before = inserted.previousSibling;
      if (inserted.nextSibling !== null) rec.before = undefined;
      rec.inserted = true;
    } else rec.text = true;
  }
  markDirAutoScopes(parent);
}
globalThis.__csimFlushPendingMarks = () => {
  layoutHasRun = true;
  if (parsedParents.size) {
    for (const [p, rec] of parsedParents) {
      dirtyLayoutSpine(p, false, true);
      if (p.nodeType !== 1) continue;
      if (rec.inserted) {
        const kids = p._children;
        const from = rec.before === undefined ? 0 : rec.before === null ? 0 : kids.indexOf(rec.before) + 1;
        layoutChildListEffect(p, kids.slice(from), [], rec.before, rec.before === undefined ? undefined : null, markLayoutDirty);
      } else if (rec.text) {
        const last = p._children[p._children.length - 1];
        if (last && last.nodeType === 3) layoutCharDataEffect(p, last, true, markLayoutDirty);
      }
    }
    parsedParents.clear();
...
export function markLayoutDirty(node, alsoSubtree, structural) {
  if (!node) return;
  stampMutation(node);
  dirtyLayoutSpine(node, alsoSubtree, structural);
}
// …the layout half alone, for a mark made LATER than the change it stands for (`__csimFlushPendingMarks`, a `dir=auto`
// flip found at read time). Stamping there moved `lastMutationAt` — the time a transition takes its style change to
// have happened — to the moment of the READ: a transition started by a class write before a parse-time layout read
// then ran from the wrong time, and `transitionend` found the old 50px still in place (Chrome: 0px).
function dirtyLayoutSpine(node, alsoSubtree, structural) {
...
// A `dir="auto"` element (and a bare `<bdi>`) takes its direction from the first strong character of the TEXT under
// it, and its whole subtree inherits that — so a text edit, a child added or removed, or a `dir` written on a
// descendant (which takes that descendant's text out of the scan, or puts it back) can turn every box in it around.
// Such an ancestor whose resolved direction FLIPPED gets its subtree marked, not only the spine the edit stamps: a
// sibling paragraph kept its left-to-right box (and a native pass replayed it) after `hello` became Hebrew — 0 where
// Chrome says 200. One that did not flip is left alone: marking it anyway cost a `<body dir=auto>` page every memo in
// the document on every text edit (6x on a 3,000-element page).
// Up the FLAT tree: a `<slot dir=auto>` resolves from its ASSIGNED nodes, which a `_parent` walk from slotted text
// never passes through (falling back to `_parent` for a light child no slot takes, which its host still scans).
// Only a document that has had a `dir=auto` or a `<bdi>` pays the walk (`dirAutoInDocument`): `<html dir="ltr">`
// alone made every mutation walk its ancestors, 2.7x on a mutation loop.
// The scopes are COLLECTED here and tested where the answer is next read — a layout pass, or any cascade read
// (`cascadeGeneration`) — once per scope however many mutations touched it. Tested per mutation, the first-strong scan
// ran over the whole scope each time: 3,000 rows of digits appended under a `<body dir=auto>` scanned the body 3,000
// times (3.6x).
let dirAutoInDocument = false;
export function noteDirAutoElement() { dirAutoInDocument = true; }
const pendingDirAuto = new Set();
globalThis.__csimDirAutoPending = false;
function markDirAutoScopes(node) {
  if (!dirAutoInDocument) return;
  for (let n = node; n && n.nodeType === 1; n = flatTreeParent(n) || n._parent) {
    const dir = n._attrs.dir;
    if (dir == null ? n._tag === 'bdi' : /^auto$/i.test(dir)) {
      pendingDirAuto.add(n);
      globalThis.__csimDirAutoPending = true;
    }
  }
}
globalThis.__csimFlushDirAutoScopes = () => {
  globalThis.__csimDirAutoPending = false;
  let flipped = false;
  for (const n of pendingDirAuto) {
    if (autoDirectionFlipped(n)) { dirtyLayoutSpine(n, true); flipped = true; }
  }
  pendingDirAuto.clear();
  if (flipped) bumpSettleGen();   // …and the cascade's memos: the flipped scope's `direction` and every edge it maps
};
...
export function markMovedSubtree(node) {
  ...
  // …and what a sideways `:has()` last answered for it (cascade.js `hasCheck`): its siblings are new, and nothing that
  // tracks them was asked about it.
  node._lbHas = undefined;
}

// Attributes a box is built from directly, whatever the page's stylesheets say. What the stylesheets make of any
// attribute is the layout gate's answer (`layoutAttrEffect`).
...
// What an attribute can turn on DOCUMENT-WIDE, seen at write time — here for a script's write, in
// parse5-adapter.js for the parser's. The flow (cascade.js `documentTurnsFlow`): a `dir`, or an
// inline `direction` / `writing-mode`. The line properties (`documentDeclaresLineProp`): an
// `align` (with the UA-aligned tags, dom-nodes.js `LINE_ALIGN_TAGS`), or an inline `text-align` /
// `text-indent`. The inline ones are read off the style TEXT because the declaration map is
// parsed lazily: a `<div style="direction: rtl">` nobody had asked about yet had no map, and a
// gate asked before that read stayed shut.
// The transition model (`documentMayTransition`) and the live-keyframes filter
// (`referencedAnimationNames`) are armed the same way: an inline `transition` on an element
// nobody had read yet jumped straight to its new value (measured: 200px where the rule-declared
// control read 100px right after the write).
const FLOW_STYLE_RE       = /direction|writing-mode/i;
const LINE_STYLE_RE       = /text-align|text-indent/i;
const TRANSITION_STYLE_RE = /transition/i;
const ANIMATION_STYLE_RE  = /animation/i;
export function noteDocumentWideAttribute(lkey, value) {
  if (lkey === 'style') {
    if (value == null) return;
    const text = String(value);
    if (!globalThis.__csimFlowHint && FLOW_STYLE_RE.test(text)) globalThis.__csimFlowHint = true;
    if (!globalThis.__csimInlineLineStyle && LINE_STYLE_RE.test(text)) globalThis.__csimInlineLineStyle = true;
    if (!globalThis.__csimInlineTransition && TRANSITION_STYLE_RE.test(text)) globalThis.__csimInlineTransition = true;
    if (!globalThis.__csimInlineAnimation && ANIMATION_STYLE_RE.test(text)) globalThis.__csimInlineAnimation = true;
  } else if (lkey === 'dir') {
    globalThis.__csimFlowHint = true;
    if (value != null && /^auto$/i.test(value)) dirAutoInDocument = true;
  } else if (lkey === 'align') {
    globalThis.__csimLineAlignHint = true;
  }
}
...
export function recordAttrMutation(target, key, oldValue, meta) {
  ...
  noteDocumentWideAttribute(lkey, target._attrs ? target._attrs[key] : null);
  // A `dir` on a descendant takes its text out of a `dir=auto` ancestor's first-strong scan (or puts it back).
  if (lkey === 'dir') markDirAutoScopes(flatTreeParent(target) || target._parent);
  ...
  // The element's own plain mark always — its own box may read the attribute however the selectors fall, and the
  // ancestor walk it performs is what re-derives the flow around it — its subtree too for an attribute a box is built
  // from directly; and whatever else the SELECTORS say the write can flip, from the layout gate (cascade.js
  // `layoutAttrEffect`): `body.os-pc .os-host { … }` restyles the `.os-host`s under `<body>`, not all of it, and
  // `.c:has(.flag) p` the `.c` above the writer.
  markLayoutDirty(target, lkey !== 'class' && STYLE_ATTRS.has(lkey));
  layoutAttrEffect(target, lkey, oldValue, target._attrs[key], markLayoutDirty);
  bumpSettleGen();
...
export function recordChildList(target, added, removed, prevSibling, nextSibling) {
  syncArenaChildList(target);   // shadow measurement: relink target's arena children (inert when off)
  markLayoutDirty(target, false, true);
  markDirAutoScopes(target);
  ...
  // …and what the selectors say moves with the child list: the children matching by position, `:empty`, a `:has()`
  // above (`layoutChildListEffect`).
  // …from the CHANGE POINT: the nodes either side of it, as the record reports them (`undefined` where no one can say).
  let before = prevSibling, after = nextSibling;
  if (before === undefined && after === undefined && added.length && added[0]._parent === target) {
    before = added[0].previousSibling;
    after = added[added.length - 1].nextSibling;
  }
  if (target && (target.nodeType === 1 || target._isShadowRoot)) layoutChildListEffect(target, added, removed, before, after, markLayoutDirty);
  bumpSettleGen();
  ...
  ctxChildListEffect(target, added, removed, before, after);
  ...
  } else if ((added.length || removed.length) && stylesheetTouch(target, added, removed)) {
    stylesheetChanged(target);
  }
  if (slotChildListHook) slotChildListHook(target, added, removed);
  if (observers.size === 0) return;
...
export function recordCharacterData(target, oldValue) {
  syncArenaCharData(target);   // shadow measurement: a text edit can flip the parent's :empty (inert when off)
  markLayoutDirty(target);
  markDirAutoScopes(flatTreeParent(target) || target._parent);
  bumpSettleGen();
  ...
  if (parent && parent.nodeType === 1) layoutCharDataEffect(parent, target, !oldValue !== !target._data, markLayoutDirty);
