...
// `alsoSubtree` is for a change that can alter INHERITED style — a `class` or `style` write, or an
// attribute some selector reads: what is measured inside those elements (their text, in their
// font) is no longer what was measured. Everything else — a child added, text edited, a `data-*`
// written — only invalidates the boxes AROUND it.
export function markLayoutDirty(node, alsoSubtree, structural) {
  ...
  for (let n = node; n; n = n._parent) { n._lbDirty = seq; if (structural) n._lbStruct = seq; }
  if (!alsoSubtree) return;
  const down = (n) => {
    n._lbDirty = seq;
    const kids = n._children;
    if (kids) for (const k of kids) if (k && k.nodeType === 1) down(k);
    if (n._shadowRoot) down(n._shadowRoot);
  };
  down(node);
}
...
export function recordAttrMutation(target, key, oldValue, meta) {
  markLayoutDirty(target, attrChangesStyle(String(key).toLowerCase()));
  bumpSettleGen();
