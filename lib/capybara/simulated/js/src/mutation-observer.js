...
// it, and its whole subtree inherits that — so a text edit, or a child added or removed, anywhere under one can turn
// every box in it around. Such an ancestor gets its SUBTREE marked, not only the spine the edit stamps: a sibling
// paragraph kept its left-to-right box (and a native pass replayed it) after `hello` became Hebrew — 0 where Chrome
// says 200. Only a document with a `dir` attribute or a `<bdi>` pays the walk (`__csimFlowHint`, the flow gate's latch).
function markDirAutoScopes(node) {
  if (!globalThis.__csimFlowHint) return;
  for (let n = node; n && n.nodeType === 1; n = n._parent) {
    const dir = n._attrs.dir;
    if (dir == null ? n._tag === 'bdi' : /^auto$/i.test(dir)) markLayoutDirty(n, true);
  }
}
...
export function recordCharacterData(target, oldValue) {
  ...
  markDirAutoScopes(target._parent);
  bumpSettleGen();
