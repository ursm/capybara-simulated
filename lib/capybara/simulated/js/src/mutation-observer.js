...
// node) and marked where the next pass starts (`ensureLayout` → `__csimFlushParsedParents`), as a childList record
...
globalThis.__csimFlushParsedParents = () => {
  layoutHasRun = true;
  if (!parsedParents.size) return;
  for (const p of parsedParents) {
    markLayoutDirty(p, false, true);
    markDirAutoScopes(p);
  }
  parsedParents.clear();
  bumpSettleGen();   // …and the cascade's memos: a `dir=auto` parent the parse flipped computed `ltr` still
};
...
function markDirAutoScopes(node) {
  ...
  for (let n = node; n && n.nodeType === 1; n = flatTreeParent(n) || n._parent) {
    ...
    if ((dir == null ? n._tag === 'bdi' : /^auto$/i.test(dir)) && autoDirectionFlipped(n)) markLayoutDirty(n, true);
  }
}
