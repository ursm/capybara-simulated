...
(function () {
  ...
  //
  // Wrap each import in a Proxy with a function target so the same
  // binding works for callable imports (`a(...)` → apply trap →
  // re-read `module[key]`), constructor imports (`new X(...)`), and
  // property reads (`a.foo`). Limitations: `typeof` always returns
  // 'function' because the Proxy target is a function; `instanceof`
  // operator can't be intercepted on the right side. For ~99 % of
  // bundled code that imports functions/classes/objects, this is
