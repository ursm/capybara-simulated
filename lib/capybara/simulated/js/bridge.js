...
(function () {
  ...
  function __csimSerializeMultipart(formData) {
    ...
    formData.forEach((value, key) => {
      ...
      if (value instanceof globalThis.Blob) {
        ...
        if (value._csimHost && typeof __csimReadFilePick === 'function') {
          body += __csimReadHostFile(value);
        } else if (value._parts) {
          body += value._parts.join('');
        }
        body += '\r\n';
      } else {
  ...
  globalThis.createImageBitmap = function (source, optionsOrSx, sy, sw, sh, opts) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (source instanceof globalThis.Blob) {
        if (source._csimHost && typeof __csimReadFilePick === 'function') {
          const b64 = __csimReadFilePick(source._handle, source._index, source._start, source._end);
          bytes = b64 ? globalThis.atob(String(b64)) : '';
        } else if (source._parts) {
          bytes = source._parts.join('');
        }
  ...
  class CanvasRenderingContext2D {
    ...
    drawImage(source, ...args) {
      ...
      // Nearest-neighbour blit. OCR doesn't care about resample
      // quality; if a future test does we can hook libvips for the
      // resize step.
      for (let row = 0; row < dh; row++) {
        const srcRow = sy + ((row * sh / dh) | 0);
        const dstRow = dy + row;
        if (dstRow < 0 || dstRow >= ch || srcRow < 0 || srcRow >= src.height) continue;
        for (let col = 0; col < dw; col++) {
          const srcCol = sx + ((col * sw / dw) | 0);
          const dstCol = dx + col;
          if (dstCol < 0 || dstCol >= cw || srcCol < 0 || srcCol >= src.width) continue;
          const sIdx = (srcRow * src.width + srcCol) * 4;
          const dIdx = (dstRow * cw + dstCol) * 4;
          dst[dIdx]     = src.pixels[sIdx];
          dst[dIdx + 1] = src.pixels[sIdx + 1];
          dst[dIdx + 2] = src.pixels[sIdx + 2];
          dst[dIdx + 3] = src.pixels[sIdx + 3];
        }
      }
    }
    ...
    putImageData(imageData, dx, dy, dirtyX, dirtyY, dirtyW, dirtyH) {
      ...
      const dx0 = dx | 0;
      const dy0 = dy | 0;
      // Default dirty rect = full ImageData.
      ...
      for (let row = 0; row < drH; row++) {
        const sy = drY + row;
        const dyR = dy0 + sy;
        if (dyR < 0 || dyR >= ch || sy < 0 || sy >= ih) continue;
        for (let col = 0; col < drW; col++) {
          const sx = drX + col;
          const dxR = dx0 + sx;
          if (dxR < 0 || dxR >= cw || sx < 0 || sx >= iw) continue;
          const sIdx = (sy * iw + sx) * 4;
          const dIdx = (dyR * cw + dxR) * 4;
          dst[dIdx]     = src[sIdx];
  ...
  // JSON-cloned (full structured clone — ArrayBuffer / TypedArray /
  // Map / Set — comes when a real test demands it).
  //
  ...
  if (typeof Worker === 'undefined' && !globalThis.__csim_isWorker) {
    ...
    class Worker extends globalThis.EventTarget {
      ...
      postMessage(data, _transferList) {
        ...
        let payload;
        try { payload = JSON.stringify(data); } catch (_) { payload = 'null'; }
  ...
  // Worker-scope `postMessage` (worker → main). Active only when
  // `globalThis.__csim_isWorker` was set by the Ruby host before
  // user script eval. Main-scope calls fall through to no-op since
  // we don't support Window.postMessage cross-iframe.
  if (typeof globalThis.postMessage !== 'function') {
    globalThis.postMessage = function (data, _transferList) {
      if (!globalThis.__csim_isWorker) return;
      let payload;
      try { payload = JSON.stringify(data); } catch (_) { payload = 'null'; }
  ...
  if (typeof indexedDB === 'undefined') {
    ...
    globalThis.indexedDB = {
      open() {
        ...
        req.error = new Error('indexedDB unavailable in csim');
        Promise.resolve().then(() => req.dispatchEvent({ type: 'error', target: req }));
        return req;
      },
      deleteDatabase() {
        const req = new IDBRequest();
