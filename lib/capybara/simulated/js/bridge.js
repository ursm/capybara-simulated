...
(function () {
  ...
  class Element extends Node {
    ...
    // as roots, not `this`.
    querySelector(sel)    { return __csimSelectFirst(this._children || [], __normaliseScopedSelector(sel)); }
    querySelectorAll(sel) { return __csimSelectAll(this._children || [], __normaliseScopedSelector(sel)); }
    matches(sel)          { return __csimMatchesSelector(this, sel); }
    closest(sel)          { return __csimClosestSelector(this, sel); }
  ...
  class Document extends Node {
    ...
    // document tree — documentElement itself IS a valid match.
    // Use `documentElement` as a root (not its children) so e.g.
    // `document.querySelector('html')` returns the html element.
    querySelector(sel)    { return this.documentElement ? __csimSelectFirst([this.documentElement], __normaliseScopedSelector(sel)) : null; }
    querySelectorAll(sel) { return this.documentElement ? __csimSelectAll([this.documentElement],   __normaliseScopedSelector(sel)) : []; }
  ...
  // ── css-select integration ────────────────────────────────────
  //
  // css-select v7 + css-what v8 (vendored in `vendor/js/vendor.bundle.js`)
  // back the user-facing query API. The hand-rolled engine below
  // (`parseSelector` / `findAll` / `matchOne`) still drives the
  // cascade rule index — that path will migrate alongside the
  // upcoming bridge.js modular split.
  //
  // The adapter speaks css-select's tree API in terms of our bridge's
  // Element model. `__csimSelectAdapter` is shared across every
  // call; compiled selectors are memoised since Capybara emits a
  // small recurring set per suite.
  const __cssSelect      = globalThis.__csimVendor && globalThis.__csimVendor.cssSelect;
  const __cssSelectOpts  = null;  // populated lazily once the adapter exists
  const __compiledCache  = new Map();

  const __csimSelectAdapter = {
    isTag: (n) => n && n.nodeType === NODE_ELEMENT,
    existsOne(test, elems) {
      for (const e of elems) {
        if (this.isTag(e) && (test(e) || this.existsOne(test, e._children))) return true;
      }
      return false;
    },
    ...
    getName: (el) => el._tag,
    getParent: (n) => n._parent || null,
    getSiblings(n) {
      const p = n._parent;
      return p ? p._children : [n];
    },
    prevElementSibling: (n) => n.previousElementSibling,
    getText(n) {
      if (n.nodeType === NODE_TEXT) return n._data || '';
      let s = '';
      const kids = n._children || [];
      for (const c of kids) s += this.getText(c);
      return s;
    },
    hasAttrib: (el, name) => Object.prototype.hasOwnProperty.call(el._attrs, name),
    removeSubsets(nodes) {
      // Drop nodes whose ancestor is also in the list (css-select
      // calls this to dedup before iterating).
    ...
    findAll(test, nodes) {
      ...
      const walk = (n) => {
        if (!n) return;
        if (this.isTag(n) && test(n)) out.push(n);
        // Descend into children whether the current node is an
        // Element (NODE_ELEMENT), a Document (NODE_DOC), or a
        // DocumentFragment / ShadowRoot — the spec's "tree" for
        // qsa includes all of these.
        const kids = n._children || [];
    ...
    findOne(test, nodes) {
      const walk = (n) => {
        if (!n) return null;
        if (this.isTag(n) && test(n)) return n;
        const kids = n._children || [];
        for (const k of kids) {
          const r = walk(k);
          if (r) return r;
    ...
    equals: (a, b) => a === b,
    // We don't track hover/visited/active at the matcher level; the
    // cascade does its own `:hover` propagation. Selectors that lean
    // on these (`button:hover .icon`) miss here — match real-browser
    // behavior of "false unless layout says yes".
    isHovered: () => false,
    isVisited: () => false,
    isActive:  () => false
  };
  ...
  function __csimCompile(sel) {
    if (!__cssSelect) throw new Error('csim: vendor bundle missing __csimVendor.cssSelect');
    let fn = __compiledCache.get(sel);
    if (!fn) {
      try {
        fn = __cssSelect.compile(sel, {adapter: __csimSelectAdapter});
      } catch (e) {
  ...
  function __csimSelectAll(roots, sel) {
    ...
    return __csimSelectAdapter.findAll(fn, Array.isArray(roots) ? roots : [roots]);
  }

  function __csimSelectFirst(roots, sel) {
    const fn = __csimCompile(sel);
    return __csimSelectAdapter.findOne(fn, Array.isArray(roots) ? roots : [roots]);
  }

  function __csimMatchesSelector(el, sel) {
    if (!el || el.nodeType !== NODE_ELEMENT) return false;
    return __csimCompile(sel)(el);
  }
  ...
  // this. css-select expects `:scope <combinator> …`; normalise so
  ...
  // `self.constructor === Window` is the identity check
  // framework `hasDOM` precondition chains rely on; falling back to
  // "non-DOM mode" means they hand raw selector strings to renderers
  // instead of `document.querySelector(selector)`. Pin the property
  // slot directly — `Object.setPrototypeOf(globalThis, Window.prototype)`
  // would risk swapping out the engine-provided global prototype chain.
  try {
