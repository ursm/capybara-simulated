...
(function () {
  ...
  // binding to whatever the source variable points at. Vite/Rolldown's
  // bundles rely on this for CJS-interop lazy modules (the export
  // is registered before the variable is assigned; the assignment
  // happens inside a thunk the consumer runs later). Eager snapshot
  // imports break those modules.
  //
  // Fast path: if the export is *already bound* and not callable,
  // return the value directly. The function-target Proxy hides the
  // underlying value's prototype (`Object.getPrototypeOf(proxy) ===
  // Function.prototype`), which makes Immutable.js's `fromJS` —
  // along with anything else that plain-object-detects via prototype
  // — pass the proxy through unchanged. Plain-object exports rarely
  // participate in load-time cycles, so we lose nothing in practice
  // by snapshotting the binding at first access.
  //
  // Slow path (callable / not-yet-bound): wrap in a function-target
  // Proxy so `a(...)` / `new X(...)` / `a.foo` all re-read
  // `module[key]`. `typeof` always reports 'function' here;
  // `instanceof` on the right side can't be intercepted. For ~99 %
  // of bundled code that imports functions/classes/objects, this is
  // close enough to real-browser semantics.
