...
(function () {
  ...
  function __csimInstallBlobURL () {
    ...
    globalThis.URL.createObjectURL = function (blob) {
      ...
      // Mirror into the Ruby-side registry so Worker isolates (which
      // hold a separate `__csimBlobs` Map) can resolve the same URL.
  ...
  globalThis.navigator = {
    ...
    userAgent: 'Mozilla/5.0 (X11; Linux x86_64) capybara-simulated (V8-resident DOM)',
    appName:   'Netscape',
    appVersion:'5.0',
    platform:  'Linux',
    language:  'en-US',
  ...
  globalThis.fetch = function fetch (input, init) {
    ...
    // blob: URLs are page-local — Rack can't parse the scheme.
    // Look up in the in-process blob registry and synthesize a
    // 200 Response from the bytes.
    if (typeof url === 'string' && url.startsWith('blob:')) {
      return new Promise(function (resolve, reject) {
        const blob = (globalThis.__csimBlobs || new Map()).get(url);
        let bodyStr, contentType;
        if (blob) {
          bodyStr     = __csimBlobBytes(blob);
          contentType = blob.type || 'application/octet-stream';
        } else {
          // Worker isolate: the blob was registered in the main scope's
          // Map but not here. Fall back to the cross-isolate Ruby registry.
          let b64;
          try { b64 = __csim_blobResolve(url); } catch (_) { b64 = null; }
          if (b64 == null) return reject(new TypeError('blob URL not found: ' + url));
          try { bodyStr = globalThis.atob(String(b64)); } catch (_) { bodyStr = ''; }
  ...
  globalThis.__csim_liveImport = function (module, key) {
    ...
    return new Proxy(target, {
      apply(_, thisArg, args) {
        return Reflect.apply(module[key], thisArg, args);
      },
      construct(_, args, newTarget) {
        return Reflect.construct(module[key], args, newTarget);
      },
      get(_, prop) {
        const v = module[key];
  ...
  globalThis.__csimVisibleText = function (h) {
    ...
    // WebDriver §13.5: `getText` for `<textarea>` returns its `value`
    // (React-controlled textareas have an empty subtree — the typed /
    // OCR'd text sits on the IDL `value` property). Mirror that here so
    // `have_css('#desc', text: /Hello/)` matches the same content
    // Selenium would see.
  ...
  CsimXMLHttpRequest.prototype.send = function (body) {
    ...
    const doFetch = () => {
      ...
      // blob: URLs — page-local, same registry the fetch() wrapper uses.
      if (typeof self._url === 'string' && self._url.startsWith('blob:')) {
        const blob = (globalThis.__csimBlobs || new Map()).get(self._url);
        let bytes, contentType, ok;
        if (blob) {
          bytes       = __csimBlobBytes(blob);
          contentType = blob.type || 'application/octet-stream';
          ok          = true;
        } else {
          // Worker isolate fallback: pull bytes from Ruby-side registry.
          let b64;
          try { b64 = __csim_blobResolve(self._url); } catch (_) { b64 = null; }
          if (b64 == null) {
            bytes = ''; contentType = ''; ok = false;
          } else {
            try { bytes = globalThis.atob(String(b64)); } catch (_) { bytes = ''; }
            contentType = 'application/octet-stream';
            ok          = true;
          }
        }
        const status  = ok ? 200 : 404;
        const headers = ok ? { 'content-type': contentType } : {};
        self.status       = status;
        self.statusText   = ok ? 'OK' : 'Not Found';
        self.responseURL  = self._url;
        self.responseText = bytes;
        self.response     = self.responseType === 'arraybuffer'
          ? (() => { const ab = new ArrayBuffer(bytes.length); const v = new Uint8Array(ab); for (let i = 0; i < bytes.length; i++) v[i] = bytes.charCodeAt(i) & 0xff; return ab; })()
          : self.responseType === 'blob'
            ? new globalThis.Blob([bytes], { type: contentType })
            : bytes;
        self._respHeaders = headers;
        self.readyState   = 4;
        self._fireReady();
        self._fireEvent(ok ? 'load' : 'error');
  ...
  // JSON-cloned with extra encoding for binary types — Tesseract
  // posts image bytes as `Uint8Array`, which raw `JSON.stringify`
  // would flatten to a numeric-keyed object on the receiver.
  function __csimWorkerEncode(data) {
    return JSON.stringify(data, function (_key, value) {
      if (value instanceof Uint8Array) {
        let s = '';
        for (let i = 0; i < value.length; i++) s += String.fromCharCode(value[i]);
        return { __csimType: 'Uint8Array', b64: globalThis.btoa(s) };
      }
      if (value instanceof ArrayBuffer) {
        const u = new Uint8Array(value);
        let s = '';
        for (let i = 0; i < u.length; i++) s += String.fromCharCode(u[i]);
        return { __csimType: 'ArrayBuffer', b64: globalThis.btoa(s) };
      }
      return value;
    });
  }
  function __csimWorkerDecode(s) {
    return JSON.parse(s, function (_key, value) {
      if (value && typeof value === 'object' && value.__csimType === 'Uint8Array') {
        const bin = globalThis.atob(value.b64);
        const u = new Uint8Array(bin.length);
        for (let i = 0; i < bin.length; i++) u[i] = bin.charCodeAt(i);
        return u;
      }
      if (value && typeof value === 'object' && value.__csimType === 'ArrayBuffer') {
        const bin = globalThis.atob(value.b64);
        const u = new Uint8Array(bin.length);
        for (let i = 0; i < bin.length; i++) u[i] = bin.charCodeAt(i);
        return u.buffer;
