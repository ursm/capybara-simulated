...
(function () {
  ...
  class Element extends Node {
    ...
    set rel(v) { this._attrs.rel = v == null ? '' : String(v); }
  ...
  class Document extends Node {
    ...
    constructor() {
      ...
      // and crashes calling `element.attachEvent` (IE-only). Listed
      // explicitly rather than via defineProperty so `'on… in
      // document'` walks the own-property table.
      this.onabort = this.onblur = this.oncancel = this.oncanplay =
        this.oncanplaythrough = this.onchange = this.onclick =
        this.onclose = this.oncontextmenu = this.oncopy = this.oncuechange =
        this.oncut = this.ondblclick = this.ondrag = this.ondragend =
        this.ondragenter = this.ondragleave = this.ondragover =
        this.ondragstart = this.ondrop = this.ondurationchange =
        this.onemptied = this.onended = this.onerror = this.onfocus =
        this.oninput = this.oninvalid = this.onkeydown = this.onkeypress =
        this.onkeyup = this.onload = this.onloadeddata =
        this.onloadedmetadata = this.onloadstart = this.onmousedown =
        this.onmouseenter = this.onmouseleave = this.onmousemove =
        this.onmouseout = this.onmouseover = this.onmouseup = this.onpaste =
        this.onpause = this.onplay = this.onplaying = this.onprogress =
        this.onratechange = this.onreset = this.onresize = this.onscroll =
        this.onseeked = this.onseeking = this.onselect = this.onstalled =
        this.onsubmit = this.onsuspend = this.ontimeupdate = this.ontoggle =
        this.onvolumechange = this.onwaiting = this.onwheel = null;
    }
  ...
  if (typeof EventSource === 'undefined') {
    ...
    class EventSource extends globalThis.EventTarget {
      constructor(url, options) {
        ...
        this.onmessage       = null;
        this.onerror         = null;
        this._id             = __csim_eventSourceOpen(this.url, !!this.withCredentials) | 0;
  ...
  if (typeof indexedDB === 'undefined') {
    ...
    class IDBEventTarget {
      ...
      dispatchEvent(evt) {
        ...
        if (l) for (const cb of l.slice()) { try { cb.call(this, evt); } catch (_) {} }
