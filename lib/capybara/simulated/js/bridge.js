...
(function () {
  ...
  // creates one against `${streamingAPIBaseURL}/api/v1/streaming/...`
  // and gates the "connected to stream" state machine on `onopen`.
  // Without the class, the constructor throws and the SPA halts at
  // module-init time (silent under QuickJS's no-await module path).
  //
  // No-op-but-pretending stub: instance exists, `onopen` fires once
  // on the next microtask, no messages ever arrive. Enough to let
  // logged-in dashboard render past the "waiting for stream" gate;
  // tests that assert on real-time updates need a richer
  // implementation that pipes through to the live streaming server.
  if (typeof EventSource === 'undefined') {
    globalThis.EventSource = class EventSource {
      constructor(url) {
        ...
        Promise.resolve().then(() => {
          if (this.readyState === 1 && typeof this.onopen === 'function') {
  ...
  if (typeof Audio === 'undefined') {
    globalThis.Audio = class Audio {
      ...
      addEventListener(t, cb) {
        const l = this._listeners.get(t) || [];
        l.push(cb);
        this._listeners.set(t, l);
      }
  ...
  if (typeof indexedDB === 'undefined') {
    // Stub `IDBRequest` and friends — Mastodon's `idb` wrapper does
    // `e instanceof IDBRequest` to discriminate request shapes; without
    // the class, the wrap rejects and the dynamic-import chain that
    // mounts MediaGallery rejects with it.
    class IDBRequest {
      constructor() {
        this.result      = null;
        this.error       = null;
    ...
    globalThis.indexedDB = {
      open() {
        ...
        Promise.resolve().then(() => { try { req.onerror && req.onerror({ target: req }); } catch (_) {} });
        return req;
      },
      deleteDatabase() {
        const req = new IDBRequest();
        Promise.resolve().then(() => { try { req.onsuccess && req.onsuccess({ target: req }); } catch (_) {} });
        return req;
      }
    };
  }
