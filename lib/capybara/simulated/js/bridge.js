  ...
  globalThis.fetch = function fetch (input, init) {
    ...
    if (typeof url === 'string' && url.startsWith('blob:')) {
      return new Promise(function (resolve, reject) {
        ...
        if (!blob) return reject(new TypeError('blob URL not found: ' + url));
        resolve(__makeFetchResponse({
          status:  200,
          body:    __csimBlobBytes(blob),
  ...
  CsimXMLHttpRequest.prototype.send = function (body) {
    ...
    const doFetch = () => {
      ...
      if (typeof self._url === 'string' && self._url.startsWith('blob:')) {
        ...
        const bytes   = blob ? __csimBlobBytes(blob) : '';
        const status  = blob ? 200 : 404;
        const headers = blob ? { 'content-type': blob.type || 'application/octet-stream' } : {};
        self.status       = status;
        self.statusText   = blob ? 'OK' : 'Not Found';
        ...
        self.response     = self.responseType === 'arraybuffer'
          ...
          : self.responseType === 'blob'
            ? new globalThis.Blob([bytes], { type: blob ? blob.type : '' })
            : bytes;
        self._respHeaders = headers;
        self.readyState   = 4;
        self._fireReady();
        self._fireEvent(blob ? 'load' : 'error');
