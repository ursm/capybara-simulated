...
(function () {
  ...
  class Element extends Node {
    ...
    // as roots, not `this`.
    querySelector(sel)    { return __csimSelectFirst(this._children, sel); }
    querySelectorAll(sel) { return __csimSelectAll(this._children,   sel); }
  ...
  class Document extends Node {
    ...
    querySelector(sel)    { return this.documentElement ? __csimSelectFirst([this.documentElement], sel) : null; }
    querySelectorAll(sel) { return this.documentElement ? __csimSelectAll([this.documentElement],   sel) : []; }
  ...
  // concatenated before bridge.js in the snapshot) back the user-facing
  // query API. The hand-rolled engine above (`parseSelector` / `findAll`
  // / `matchOne`) still drives the cascade rule index — that path will
  // migrate alongside the upcoming bridge.js modular split.
  //
  // The adapter speaks css-select's tree API in terms of our bridge's
  // Node / Element model. Compiled selectors are memoised since
  // Capybara emits a small recurring set per suite.
