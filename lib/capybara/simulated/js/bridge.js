...
(function () {
  ...
  // Tags whose `src` IDL attribute returns the URL resolved against
  // the document base (per HTML spec). The bare attribute value alone
  // would leave webpack/embroider's `document.currentScript.src` as
  // an unresolved `/assets/…`, breaking their public-path detection.
  ...
  class Element extends Node {
    ...
    // `<script>` / `<img>` / `<iframe>` / `<source>`.src: IDL attribute
    // returns the resolved URL against the document base, same shape as
    // `href`. Webpack/embroider bundles read `document.currentScript.src`
    // at top level to derive the public path; an unresolved `/assets/…`
    // crashes their auto-detection ("Automatic publicPath is not
    // supported in this browser").
    get src() {
      if (!SRC_REFLECTING_TAGS.has(this._tag)) return this._attrs.src;
      const v = this._attrs.src;
      if (v == null) return '';
      try {
        const base = (globalThis.location && globalThis.location.href) || null;
        const u = __csim_parseUrl(v, base);
  ...
  class Document extends Node {
    ...
    // The currently-executing `<script>` element (set by
    // `runInlineScripts` around `__csim_runScript`). Webpack /
    // embroider chunks read `currentScript.src` at top level to
    // derive the bundle's public-path origin; without this they throw
    // "Automatic publicPath is not supported in this browser" and the
    // whole bundle aborts.
    get currentScript() { return this._currentScript || null; }
    // Standards-mode viewport scroll root. Ember's router-scroll
    // addon reads `scrollingElement.scrollLeft` during
    // `routeWillChange` to remember the position; undefined here
    // throws and aborts the transition (Discourse's about-route then
    // never renders its template).
  ...
  // `<script>` tags that fireCEConnect surfaces during that phase —
  // those are the parse-time scripts that `runInlineScripts` will run
  // in DOM order afterward. Once `runInlineScripts` starts, dynamic
  // appends (e.g. webpack's chunk loader doing `head.appendChild(script)`
  // mid-evaluation) need to evaluate eagerly so their `onload` can
  // resolve the chunk-import Promise the caller is awaiting.
  ...
  function maybeRunScript(el) {
    ...
    if (el._attrs.src) {
      try { el.dispatchEvent(new Event(_ok ? 'load' : 'error')); } catch (_) {}
    }
  }
  ...
  // Real browsers expose `self.constructor === Window`; Ember's
  // `hasDOM` precondition checks this exact identity and falls back
  // to "non-DOM mode" if it fails (target = literal selector string
  // instead of `document.querySelector(selector)`). Pin the global's
  // `constructor` slot directly — `Object.setPrototypeOf(globalThis,
  // Window.prototype)` would risk swapping out the engine-provided
  ...
  globalThis.CSS = {
    ...
    // CSS.supports has no real layout / cascade behind it, but
    // returning blanket-false makes Discourse and similar apps flag us
    // as an "unsupported browser" and refuse to bootstrap. Default to
    // true for any input — apps that gate progressive enhancement on
    // a specific feature still take the modern-browser path; tests
    // that rely on the legacy fallback are out of scope (they'd need
    // a real cascade to verify anyway).
  ...
  function runInlineScripts(doc) {
    ...
    try { __csim_logConsole('warn', '[csim:scripts-found] count=' + scripts.length + ' srcs=' + scripts.map(s => s._attrs.src || '[in]').join(',')); } catch (_) {}
    for (const s of scripts) {
      ...
      // External classic scripts fire `load` (or `error`) on the
      // element when they finish — webpack's chunk loader awaits this
      // to resolve dynamic-import Promises that triggered the script
      // injection.
      if (s._attrs.src) {
        try { s.dispatchEvent(new Event(_ok ? 'load' : 'error')); } catch (_) {}
      }
    }
    if (__observers.size && __hasQueuedRecords()) deliverMutations();
    if (typeof __recheckIntersectionObservers === 'function') __recheckIntersectionObservers();
