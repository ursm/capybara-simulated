  ...
  globalThis.__csim_liveImport = function (module, key) {
    ...
    return new Proxy(target, {
      apply(_, thisArg, args) {
        const fn = module[key];
        // When the export is null/undefined at call time (a config-flag
        // import that's permanently absent in the current test setup),
        // `Reflect.apply(undefined, …)` throws "apply on null". React-DOM's
        // `useState`'s `typeof a === 'function' && (a = a())` lazy-init
        // hits this whenever an `initial_state` meta key is missing, then
        // its error-boundary chain crashes Mastodon's SPA mount entirely.
        // Match the shape of accessing a property on the underlying value
        // (the `get` trap below returns `undefined` for null v) and let
        // the consumer continue — the proxy stays a placeholder for an
        // undefined value, not a callable.
