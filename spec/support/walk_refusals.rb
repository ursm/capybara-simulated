...
module WalkRefusals
  ...
  # …an orphan `display: table-row` holding an INLINE element (`flex-container-unsupported`: the oracle MEASURES one by
  # putting it on a line, where the block walk native measures a flex container's children with stacks it). Its
  # predecessors: a `pre` inline-block of nothing but spaces (`white-space-only-block`) until preserved white space
  # went down the text path, a CENTRED mixed block's out-of-flow box in a group that opens no line
  # (`oof-in-collapsed-group`) until an empty line stopped moving what waits on it — both 2026-09-24 — and orphan rows
  # of bare TEXT and of BLOCK children, both native since 2026-09-26.
  ORPHAN_ROW       = '<span style="display:inline-block"><div style="display:table-row"><span>aa bb</span></div></span>'
